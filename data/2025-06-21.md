...
Motivation: We present a manually annotated corpus for Adverse Event (AE) extraction from discharge summaries of elderly patients, a population often underrepresented in clinical NLP resources.

Method: We evaluate multiple models using FlairNLP across three annotation granularities: fine-grained, coarse-grained, and coarse-grained with negation.

Result: Transformer-based models (e.g., BERT-cased) achieve strong performance on document-level coarse-grained extraction (F1 = 0.943), performance drops notably for fine-grained entity-level tasks (e.g., F1 = 0.675), particularly for rare events and complex attributes.
...
Method: a novel weakly supervised framework for assessing the credibility of context documents during LLM inference--without requiring manual annotations. automated credibility estimation through inter-document agreement. two integration strategies: a black-box approach and a white-box method that directly modifies attention mechanisms.

Result: the integration of contextual information has significantly enhanced the performance of large language models (LLMs) on knowledge-intensive tasks.
...
Result: introduce MDBench, a new dataset for evaluating LLMs on the task of multi-document reasoning.

Conclusion: MDBENCH poses significant challenges for all methods, even with relatively short document sets. the knowledge-guided generation technique (1) allows us to readily perform targeted analysis of MD-specific reasoning capabilities and (2) can be adapted quickly to account for new challenges and future modeling improvements.
...
<details>
  <summary>Details</summary>
Motivation: While Machine Learning (ML) and Deep Learning (DL) models have been widely used for diabetes prediction, the use of Large Language Models (LLMs) for structured numerical data is still not well explored.

Method: We conduct an empirical analysis using the Pima Indian Diabetes Database (PIDD). We evaluate six LLMs, including four open-source models: Gemma-2-27B, Mistral-7B, Llama-3.1-8B, and Llama-3.2-2B. We also test two proprietary models: GPT-4o and Gemini Flash 2.0. In addition, we compare their performance with three traditional machine learning models: Random Forest, Logistic Regression, and Support Vector Machine (SVM). We use accuracy, precision, recall, and F1-score as evaluation metrics.

Result: proprietary LLMs perform better than open-source ones, with GPT-4o and Gemma-2-27B achieving the highest accuracy in few-shot settings. Notably, Gemma-2-27B also outperforms the traditional ML models in terms of F1-score. However, there are still issues such as performance variation across prompting strategies and the need for domain-specific fine-tuning.

Conclusion: LLMs can be useful for medical prediction tasks and encourages future work on prompt engineering and hybrid approaches to improve healthcare predictions.
...
<details>
  <summary>Details</summary>
Motivation: 观察到一个有趣的现象：可以生成可逆的句子嵌入，允许LLM精确地重建原始文本。

Method: 引入一个特殊的记忆token，并通过在固定序列上训练来优化其嵌入。

Result: Llama 3.1 8B 成功地重建了所有测试序列。在英语和西班牙数据集、高达约240个token的序列以及从100M到8B参数的模型尺度上评估了这一现象。

Conclusion: LLMs展现出一种有趣的能力，能够通过可逆的句子嵌入精确地重建原始文本，而无需修改模型权重。这为基于记忆的检索、压缩和受控文本生成提供了潜在的应用。
...
<details>
  <summary>Details</summary>
Motivation: 近年来，社会隔离和孤独感日益加剧，对自杀率有很大影响。美国国家暴力死亡报告系统（NVDRS）的结构化变量中目前没有记录社会隔离和孤独感。

Method: 使用主题建模生成词汇表开发和监督学习分类器

Result: 开发了高质量的分类器（平均 F1：0.86，准确率：0.82）。在 2002 年至 2020 年的 300,000 多起自杀事件中，我们发现 1,198 起提到了长期社会隔离。如果死者是男性（OR = 1.44；CI：1.24，1.69，p<.0001）、同性恋（OR = 3.68；1.97，6.33，p<.0001）或已离婚（OR = 3.34；2.68，4.19，p<.0001），则他们被归类为长期社会隔离的几率更高。我们发现了近期或即将离婚、失去子女监护权、驱逐或近期搬家以及分手等其他社会隔离话题的重要预测因素。

Conclusion: 该研究表明，通过自然语言处理技术，可以在执法部门和验尸官的医疗检查报告中识别出社会隔离和孤独感，从而改进对美国社会隔离和孤独感的监测和预防。
...
<details>
  <summary>Details</summary>
Motivation: 开放式长文本生成的评估具有挑战性，因为很难明确区分好坏输出。现有方法经常忽略连贯性、风格或相关性等关键方面，或者受到预训练数据的偏差，使得开放式长文本评估成为一个尚未充分探索的问题。

Method: 提出 PrefBERT，一个用于评估 GRPO 中开放式长文本生成并指导其训练的评分模型，对好坏输出提供不同的奖励。

Result: PrefBERT 在多句和段落长度的响应上训练，在不同的长段落中保持可靠，并且与 GRPO 需要的可验证奖励非常吻合。使用 PrefBERT 作为奖励信号训练策略模型，产生的回复与人类偏好更一致。

Conclusion: PrefBERT作为奖励信号训练策略模型，产生的回复与人类偏好更一致。
...
<details>
  <summary>Details</summary>
Motivation: 对话语篇分析旨在识别和分析对话中话语之间的语篇关系。然而，对话中的语言特征，如省略和习语，经常引入歧义，使预期的语篇关系模糊不清，给解析器带来了重大挑战。

Method: 提出了一种Discourse-aware Clarification Module (DCM)来提高对话语篇分析器的性能。DCM采用两种不同的推理过程:澄清类型推理和语篇目标推理。此外，我们还引入了Contribution-aware Preference Optimization (CPO)来降低错误澄清的风险，从而减少级联错误。CPO使解析器能够评估来自DCM的澄清的贡献，并提供反馈以优化DCM，从而增强其适应性和与解析器需求的一致性。

Result: 该方法有效地解决了歧义，并显著优于当前最优(SOTA)基线。

Conclusion: 该方法在STAC和Molweni数据集上进行了大量实验，结果表明，该方法有效地解决了歧义，并显著优于当前最优(SOTA)基线。
...
<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型在医学知识表示不足和临床部署效率低。

Method: 提出CKD-EHR框架，通过医学知识增强数据微调大型语言模型Qwen2.5-7B作为教师模型，并通过多粒度注意力蒸馏机制生成可解释的软标签，最终将知识转移到轻量级BERT学生模型。

Result: 在MIMIC-III数据集上，CKD-EHR的诊断准确率提高了9%，F1-score提高了27%，推理速度提高了22.2倍。

Conclusion: CKD-EHR通过知识蒸馏技术，显著提高了资源利用效率，增强了诊断的准确性和及时性，为临床环境中的资源优化提供了实用的技术方法。
...
<details>
  <summary>Details</summary>
Motivation: most modern LLM-based dialogue agents do not explicitly model the one-to-many (o2m) property, whereby multiple appropriate responses exist for a single dialogue context. In this work, we model the o2m property of OD in LLMs
...
Method: a rule-based pre-tokenization method that aligns with the linguistic structure of the Korean language, a seed vocabulary containing tokens that resemble linguistic units and employ a branching entropy-based selection algorithm
...
<details>
  <summary>Details</summary>
Motivation: Study memory in state-space language models using primacy and recency effects

Method: Applying structured recall tasks to the Mamba architecture

Result: Observe a consistent U-shaped accuracy profile, indicating strong performance at the beginning and end of input sequences. Identify three mechanisms that give rise to this pattern. First, long-term memory is supported by a sparse subset of channels within the model's selective state space block, which persistently encode early input tokens and are causally linked to primacy effects. Second, short-term memory is governed by delta-modulated recurrence: recent inputs receive more weight due to exponential decay, but this recency advantage collapses when distractor items are introduced, revealing a clear limit to memory depth.

Conclusion: Memory allocation is dynamically modulated by semantic regularity: repeated relations in the input sequence shift the delta gating behavior, increasing the tendency to forget intermediate items. 
...
Result: The results reveal that smaller models, when optimized through prompt engineering, can perform on par with larger models like OpenAI's GPT (Generative Pre-trained Transformer).
...
<details>
  <summary>Details</summary>
Motivation: The underlying mechanisms supporting such transfer remain poorly understood. We hypothesize that cross-domain generalization arises from shared abstract reasoning prototypes -- fundamental reasoning patterns that capture the essence of problems across domains. These prototypes minimize the nuances of the representation, revealing that seemingly diverse tasks are grounded in shared reasoning structures.
...
Result: ProtoReasoning achieves 4.7% improvement over baseline models on logical reasoning (Enigmata-Eval), 6.3% improvement on planning tasks, 4.0% improvement on general reasoning (MMLU) and 1.0% on mathematics (AIME24).

Conclusion: Learning in prototype space demonstrates enhanced generalization to structurally similar problems compared to training solely on natural language representations, validating our hypothesis that reasoning prototypes serve as the foundation for generalizable reasoning in large language models.
...
<details>
  <summary>Details</summary>
Motivation: 传统指标（如ROUGE和BERTScore）难以捕捉语义相似性，而当前的基于LLM的评估方法缺乏直观的可解释性，并且忽略了事实性问题和非事实性问题之间的区别。

Method: MinosEval，一种新颖的评估方法，它首先区分开放式问题，然后使用不同的评估策略对候选答案进行排序。对于事实性问题，它采用自适应关键点评分策略，而对于非事实性问题，它使用实例感知列表式排序策略。

Result: 在多个开放式QA数据集（包括自建的具有更多候选响应的数据集）上的实验表明

Conclusion: MinosEval更好地与人工标注对齐，并提供更可解释的结果。
...
Method: Using Natural Language Inference (NLI) as a pivot task with a novel, manually-curated parallel dataset in Basque and Spanish variants and crosslingual and in-context learning experiments using encoder-only and decoder-based Large Language Models (LLMs).

Result: Performance drop when handling linguistic variation, especially in Basque. Encoder-only models particularly struggle with Western Basque.

Conclusion: Current language technologies struggle with linguistic variation in Basque and Spanish, especially in Basque.
...
<details>
  <summary>Details</summary>
Motivation: This article addresses domain knowledge gaps in general large language models for historical text analysis in the context of computational humanities and AIGC technology.

Method: We propose the Graph RAG framework, combining chain-of-thought prompting, self-instruction generation, and process supervision to create a The First Four Histories character relationship dataset with minimal manual annotation. In the graph-augmented generation phase, we introduce a collaborative mechanism between knowledge graphs and retrieval-augmented generation

Result: achieves optimal performance in relation extraction (F1 = 0.68). The DeepSeek model integrated with GraphRAG improves F1 by 11% (0.08-0.19) on the open-domain C-CLUE relation extraction dataset, surpassing the F1 value of Xunzi-Qwen1.5-14B (0.12), effectively alleviating hallucinations phenomenon, and improving interpretability.
...
<details>
  <summary>Details</summary>
Motivation: 为了解决大规模网络语料库上的端到端问答问题，开发了一个检索增强生成 (RAG) 系统。

Method: 采用混合检索策略，结合稀疏和密集索引，然后进行K-Means聚类以对语义相似的段落进行分组。从每个集群中选择代表性段落来构建特定于集群的提示，用于大型语言模型 (LLM)，生成中间答案，这些答案经过过滤、重新排序和最终合成为单个、全面的响应。

Result: TopClustRAG在FineWeb Sample-10BT数据集上进行了评估，在官方排行榜上，其忠实度排名第二，正确性排名第七。

Conclusion: TopClustRAG在FineWeb Sample-10BT数据集上，在忠实度方面排名第二，在正确性方面排名第七，证明了基于聚类的上下文过滤和提示聚合在大型RAG系统中的有效性。
...
Method: The authors develop an end-to-end deep neural network (DNN)-based de-identification pipeline.

Result: The authors construct and release the first Korean legal dataset containing annotated judgments along with corresponding lists of entity mentions, and introduce a systematic categorization of Personally Identifiable Information (PII).

Conclusion: This paper's model achieves state-of-the-art performance in the de-identification of court judgments.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to address the poor performance of LID models on low-resource languages due to class imbalance and bias issues, often limited to single-domain data.

Method: The study uses a supervised contrastive learning (SCL) approach to learn domain-invariant representations.

Result: The approach improves LID performance on out-of-domain data for low-resource languages by 3.2%.

Conclusion: This paper introduces a supervised contrastive learning (SCL) approach to enhance LID models, particularly for low-resource languages.
...
<details>
  <summary>Details</summary>
Motivation: Current evaluation methods often fail to distinguish genuine medical reasoning from superficial patterns in large language models (LLMs) used in clinical decision support.

Method: A behavioral testing framework called DeVisE (Demographics and Vital signs Evaluation) was introduced. A dataset of ICU discharge notes from MIMIC-IV was constructed, generating both raw (real-world) and template-based (synthetic) versions with controlled single-variable counterfactuals targeting demographic (age, gender, ethnicity) and vital sign attributes.  The behavior of five LLMs was assessed via (1) input-level sensitivity and (2) downstream reasoning.

Result: Zero-shot models exhibit more coherent counterfactual reasoning patterns, while fine-tuned models tend to be more stable yet less responsive to clinically meaningful changes.

Conclusion: Behavioral testing is useful for understanding clinical LLM reasoning and designing safer, more transparent medical AI systems. Demographic factors subtly but consistently influence outputs, emphasizing the importance of fairness-aware evaluation.
...
<details>
  <summary>Details</summary>
Motivation: their global effectiveness depends on understanding local socio-cultural contexts
...
<details>
  <summary>Details</summary>
Motivation: research in Indian languages remains limited in comment-aware multimodal and multilingual summarization

Method: integrating reader insights and feedback to enhance summaries; employing state-of-the-art language models such as LLama3 and GPT-4; identifying supportive comments, filtering out noise using a dedicated comment classifier using IndicBERT, and extracting valuable insights from images with a multilingual CLIP-based classifier

Result: introducing COSMMIC, a pioneering comment-sensitive multimodal, multilingual dataset featuring nine major Indian languages. COSMMIC comprises 4,959 article-image pairs and 24,484 reader comments, with ground-truth summaries available in all included languages. We explore summarization and headline generation across four configurations

Conclusion: COSMMIC uniquely integrates text, images, and user feedback, bridging gaps in Indian language resources, advancing NLP research and fostering inclusivity.
...
<details>
  <summary>Details</summary>
Motivation: MLLMs for GUI agents exhibit hallucinations (systematic localization errors) that compromise reliability.

Method: The paper proposes a fine-grained evaluation framework and the Peak Sharpness Score (PSS) to quantify model uncertainty. Additionally, it introduces Context-Aware Cropping, a training-free technique to improve model performance.
...
Conclusion: The paper introduces a framework and a training-free technique (Context-Aware Cropping) to enhance the interpretability and robustness of GUI agent behavior, demonstrating their effectiveness through experiments.
...
<details>
  <summary>Details</summary>
Motivation: 基于大型语言模型的多智能体系统在社会模拟和复杂任务解决领域已显示出巨大的潜力。然而，当前的框架在系统架构设计、跨领域通用性和性能保证方面面临着严峻的挑战，尤其是在任务复杂性和智能体数量增加的情况下。
...
Result: AgentGroupChat-V2 在不同领域表现出卓越的性能，在 GSM8K 上的准确率达到 91.50%（超过最佳基线 5.6 个百分点），在竞赛级别的 AIME 上达到 30.4% 的准确率（几乎是其他方法的两倍），在 HumanEval 上达到 79.20% 的 pass@1。随着任务难度的增加，性能优势变得越来越明显，尤其是在 5 级 MATH 问题上，与最先进的基线相比，改进超过 11 个百分点。

Conclusion: AgentGroupChat-V2 为构建高效、通用的 LLM 多智能体系统提供了一个全面的解决方案，在复杂的推理场景中具有显著优势。
...
<details>
  <summary>Details</summary>
Motivation: 目前尚不清楚观察到的结果是来自真正的推理还是来自训练集的统计回忆。

Method: 通过改变中间符号表示中的问题，RE-IMAGINE 生成任意数量的仅使用记忆无法解决的问题。

Result: 通过在四个广泛使用的基准测试中展示我们的框架来评估几个大语言模型系列，并且观察到当使用问题变体查询模型时性能下降。

Conclusion: 评估表明，大语言模型在一定程度上依赖于统计回忆来获得过去的性能，并为进一步研究针对推理层次结构中的技能打开了大门。
...
<details>
  <summary>Details</summary>
Motivation: Simply appending context at inference time does not ensure grounded generation in large language models (LLMs).
...
Result: CINGS outperforms other training methods across 11 information-seeking datasets in the text domain and reduces hallucinations across four benchmarks in the vision-language domain. CINGS is complementary to inference-time grounding techniques.

Conclusion: Models trained with CINGS exhibit stronger grounding in both textual and visual domains compared to standard instruction-tuned models, without degradation in general downstream performance. CINGS induces a shift in the model's prior knowledge and behavior, implicitly encouraging greater reliance on the external context.
...
<details>
  <summary>Details</summary>
Motivation: 高效、高质量的自动过程注释仍然是一个巨大的挑战。

Method: Single-Pass Annotation with Reference-Guided Evaluation (SPARE)

Result: 参考引导的步骤级别评估有效地促进了跨越三个领域的四个数据集的过程监督：数学推理、多跳组合问题回答和空间推理。

Conclusion: SPARE在用于离线RL设置中微调模型以进行推理时贪婪解码，以及训练奖励模型以对多个LLM生成的输出进行排序/聚合时，提高了推理性能。此外，SPARE在具有挑战性的数学数据集上实现了具有竞争力的性能，同时提供了2.6倍的效率，与基于树搜索的自动注释相比，只需要38%的运行时间。
...
<details>
  <summary>Details</summary>
Motivation: 基于文本的夸张和隐喻检测对于自然语言处理(NLP)任务具有重要意义。然而，由于它们的语义模糊性和表达多样性，识别它们相当具有挑战性。现有方法大多侧重于表面文本特征，忽略了夸张和隐喻的关联以及隐含情感对感知这些修辞手段的影响。

Method: 提出了一种基于双向动态交互的情感引导双曲线和隐喻检测框架(EmoBi)。

Result: EmoBi在四个数据集上优于所有基线方法。

Conclusion: EmoBi在四个数据集上优于所有基线方法。与当前SoTA相比，在TroFi数据集上，双曲线检测的F1评分提高了28.1%，在HYPO-L数据集上，隐喻检测的F1评分提高了23.1%。
...
Method: Reinforcement learning (RL) with the GRPO method to train models using verifiable outcome-based rewards targeting answer correctness, citation sufficiency, and refusal quality.
...
<details>
  <summary>Details</summary>
Motivation: The crucial choice of window size presents a Pareto tradeoff: larger windows maintain performance akin to full attention but offer minimal efficiency gains in short-context scenarios, while smaller windows can lead to performance degradation. Our core motivation is to address the intrinsic limitation of local attention -- its complete disregard for tokens outside the defined window.

Method: RATTENTION, a variant of local attention integrated with a specialized linear attention mechanism designed to capture information from these out-of-window tokens.

Result: Pretraining experiments at the 3B and 12B scales demonstrate that RATTENTION achieves a superior Pareto tradeoff between performance and efficiency.

Conclusion: RATTENTION with a window size of just 512 consistently matches the performance of full-attention models across diverse settings, and the recurrent nature inherent in the linear attention component of RATTENTION contributes to enhanced long-context performance. Crucially, these improvements do not compromise training efficiency.
...
<details>
  <summary>Details</summary>
Motivation: 现代语言模型通常具有开放权重但封闭的训练数据。我们将从模型权重进行数据近似的问题形式化，并提出了几个基线和指标。

Method: 开发了一种基于梯度的方法，该方法从大型公共文本语料库中选择最匹配的数据，并展示了其在仅给定原始模型和微调模型的权重的情况下恢复有用数据的有效性。

Result: 在 AG News 分类任务中，该方法将性能从 65%（使用随机选择的数据）提高到 80%，接近 88% 的专家基准。当应用于使用 MSMARCO Web 文档上的 SFT 训练的模型时，与专家 LLAMA 模型的 2.0 困惑度相比，该方法将困惑度从 3.3 降低到 2.3。

Conclusion: 即使在不知道任何真实训练数据的情况下，该方法也能够定位一小部分公共 Web 文档，这些文档可用于训练模型，使其性能接近原始模型，适用于为分类和监督微调训练的模型。在 AG News 分类任务中，该方法将性能从 65%（使用随机选择的数据）提高到 80%，接近 88% 的专家基准。当应用于使用 MSMARCO Web 文档上的 SFT 训练的模型时，与专家 LLAMA 模型的 2.0 困惑度相比，该方法将困惑度从 3.3 降低到 2.3。
...
<details>
  <summary>Details</summary>
Motivation: Large Language Models (LLMs) in voice chat applications suffer from noticeable latency between user input and audio output, especially on consumer-grade hardware. This latency is primarily due to the time it takes for LLMs to generate the first sentence for TTS systems.

Method: Predictive Generation (PredGen), a novel framework that mitigates delay through speculative decoding at input time.

Result: Simulated experiments on the Lmsys and MT-Bench datasets show that the proposed method can effectively reduce the latency by around 2x across a wide range of use cases.

Conclusion: The proposed Predictive Generation (PredGen) framework effectively reduces latency in LLM-based voice chat applications by around 2x with minimal additional computation cost.
...
<details>
  <summary>Details</summary>
Motivation: Evaluate gender fairness in large language models (LLMs), focusing on their ability to handle both binary and non-binary genders. Previous studies primarily focus on binary gender distinctions.
...
<details>
  <summary>Details</summary>
Motivation: 为了能够进行细粒度的评估，每个示例都包含专家注释的支持证据。

Method: 我们引入了 SciVer，这是第一个专门用于评估基础模型在多模态科学环境中验证声明的能力的基准。

Result: 我们评估了 21 个最先进的多模态基础模型的性能，包括 o4-mini、Gemini-2.5-Flash、Llama-3.2-Vision 和 Qwen2.5-VL。

Conclusion: 实验表明，当前开源模型与 SciVer 上的专家相比存在显著的性能差距，通过对 RAG 和人工进行的错误评估的深入分析，我们发现了当前开源模型的关键局限性，为推进模型在多模态科学文献任务中的理解和推理提供了关键见解。
...
Conclusion: DiscoSG-Refiner improves SPICE by approximately 30% over the best baseline while achieving 86 times faster inference than GPT-4. It also consistently improves downstream VLM tasks.
...
<details>
  <summary>Details</summary>
Motivation: Documents are fundamental to preserving and disseminating information, often incorporating complex layouts, tables, and charts that pose significant challenges for automatic document understanding (DU). While vision-language large models (VLLMs) have demonstrated improvements across various tasks, their effectiveness in processing long-context vision inputs remains unclear.

Method: introducing WikiMixQA, a benchmark comprising 1,000 multiple-choice questions (MCQs) designed to evaluate cross-modal reasoning over tables and charts extracted from 4,000 Wikipedia pages spanning seven distinct topics

Result: We evaluate 12 state-of-the-art vision-language models, revealing that while proprietary models achieve ~70% accuracy when provided with direct context, their performance deteriorates significantly when retrieval from long documents is required. Among these, GPT-4-o is the only model exceeding 50% accuracy in this setting, whereas open-source models perform considerably worse, with a maximum accuracy of 27%.

Conclusion: This paper underscores the challenges of long-context, multi-modal reasoning and establishes WikiMixQA as a crucial benchmark for advancing document understanding research.
...
Conclusion: current models can generate MCQs of comparable quality to human-authored ones, but issues related to semantic clarity and answerability, and challenges remain in generating distractors that engage students and meet established criteria for high-quality MCQ option design.
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型中的后悔是指当呈现与其先前生成的错误信息相矛盾的证据时，它们会明确表达后悔。研究后悔机制对于提高模型的可靠性至关重要，并有助于揭示认知如何在神经网络中编码。

Method: 提出：（1）通过战略性设计的提示场景构建综合后悔数据集的工作流程，（2）有监督的压缩-解耦指数（S-CDI）指标来识别最佳后悔表示层，以及（3）后悔支配分数（RDS）指标来识别后悔神经元和组影响系数（GIC）来分析激活模式。

Result: 我们的实验结果使用S-CDI指标成功识别出最佳的后悔表示层，这显着提高了探针分类实验的性能。此外，我们还在模型层中发现了一个M形解耦模式，揭示了信息处理如何在耦合和解耦阶段之间交替。

Conclusion: 通过RDS指标，将神经元分为三个不同的功能组：后悔神经元、非后悔神经元和双重神经元。
...
<details>
  <summary>Details</summary>
Motivation: it is unclear whether differences in cross-cultural communication arise from the literal meanings assigned to words or from more general pragmatic factors such as norms around politeness and brevity

Method: report three experiments examining how speakers of British and American English interpret intensifiers like "quite" and "very," and develop a computational cognitive model where listeners recursively reason about speakers who balance informativity, politeness, and utterance cost
...
<details>
  <summary>Details</summary>
Motivation: 解决罗马尼亚语文本的作者身份归属问题。

Method: 使用字符n-gram特征，系统地评估六种机器学习技术：支持向量机（SVM）、Logistic回归（LR）、k-最近邻（k-NN）、决策树（DT）、随机森林（RF）和人工神经网络（ANN）。

Result: ANN模型取得了最高的性能，在使用5-gram特征时，在15次运行中的4次实现了完美的分类。

Conclusion: 轻量级的、可解释的字符n-gram方法可以为罗马尼亚语作者身份归属提供最先进的准确性，与更复杂的方法相媲美。简单的文体特征在资源受限或研究不足的语言环境中具有潜力。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型擅长于许多任务，但仍然难以进行一致、稳健的推理。

Method: 引入了基于队列一致性学习 (CC-Learn)，这是一种强化学习框架，通过训练来自共享编程抽象的类似问题的队列，提高 LLM 推理的可靠性。

Result: 为了加强队列级别的一致性，我们定义了一个复合目标，将队列准确性、有效问题分解的检索奖励以及对强化学习可以直接优化的琐碎或无效查找的拒绝惩罚结合起来，这与监督微调不同。优化这种奖励可以引导模型在所有队列成员中采用统一的推理模式。

Conclusion: CC-Learn通过在具有挑战性的推理基准测试（包括 ARC-Challenge 和 StrategyQA）上的实验表明，与预训练和 SFT 基线相比，它提高了准确性和推理稳定性。这些结果表明，cohort-level RL 有效地增强了 LLM 中的推理一致性。
...
<details>
  <summary>Details</summary>
Motivation: Reasoning traces frequently contain sensitive user data, which can be extracted via prompt injections or accidentally leak into outputs. Reasoning improves utility but enlarges the privacy attack surface.

Method: Probing and agentic evaluations

Result: Test-time compute approaches, particularly increased reasoning steps, amplify such leakage. Increasing the budget of those test-time compute approaches makes models more cautious in their final answers, but also leads them to reason more verbosely and leak more in their own thinking.
...
<details>
  <summary>Details</summary>
Motivation: Gender-inclusive machine translation (MT) should preserve gender ambiguity in the source to avoid misgendering and representational harms. While gender ambiguity often occurs naturally in notional gender languages such as English, maintaining that gender neutrality in grammatical gender languages is a challenge.

Method: Assess the sensitivity of 21 MT systems to the need for gender neutrality in response to gender ambiguity in three translation directions of varying difficulty. The specific gender-neutral strategies that are observed in practice are categorized and discussed. Additionally, we examine the effect of binary gender stereotypes on the use of gender-neutral translation.

Result: Observed a small handful of MT systems that switch to gender neutral translation using specific strategies, depending on the target language.

Conclusion: We report a disappointing absence of gender-neutral translations in response to gender ambiguity. However, we observe a small handful of MT systems that switch to gender neutral translation using specific strategies, depending on the target language.
...
Method: 我们提出了Generation after Recalibration (GenRecal)，这是一种用于VLM的新型通用蒸馏框架。GenRecal包含一个Recalibrator，它可以对齐和调整异构VLM之间的特征表示，从而实现跨不同类型VLM的有效知识转移。

Result: 通过在多个具有挑战性的基准上进行的大量实验，我们证明了GenRecal显著提高了基线性能，最终优于大规模开源和闭源VLM。

Conclusion: GenRecal显著提高了基线性能，最终优于大规模开源和闭源VLM。
...
TL;DR: PhantomHunter是一种新的LLM生成文本检测器，专门用于检测来自私下调整的LLM的文本。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）的普及使得诸如错误信息生成和学术不端行为等不良社会问题更加严重，使得LLM生成的文本检测现在具有前所未有的重要性。来自私下调整的LLM的文本构成了一个新的挑战，但尚未被充分探索。用户可以通过使用私有语料库对开源LLM进行微调来轻松拥有私有LLM，导致现有检测器在实践中性能显著下降。

Method: 提出了PhantomHunter，一个专门用于检测来自未见过的、私下调整的LLM的文本的LLM生成文本检测器。其family-aware学习框架捕获了基础模型及其衍生物之间共享的家族级特征，而不是记忆个体特征。

Result: 在来自LLaMA、Gemma和Mistral家族的数据上进行的实验表明，PhantomHunter优于7个基线和3个工业服务。
...
TL;DR: This paper introduces SemIRNet, a new network for multimodal irony detection that uses ConceptNet, cross-modal semantic similarity detection, and contrastive learning to improve accuracy and F1 score.
...
Result: the accuracy and F1 value of this model are improved by 1.64% and 2.88% to 88.87% and 86.33%, respectively, compared with the existing optimal methods.

Conclusion: The accuracy and F1 value of this model are improved by 1.64% and 2.88% to 88.87% and 86.33%, respectively, compared with the existing optimal methods. Further ablation experiments verify the important role of knowledge fusion and semantic similarity detection in improving the model performance.
...
TL;DR: This paper introduces Argus Inspection, a multimodal benchmark, and the Eye of Panoptes framework for evaluating MLLMs' visual fine-grained perception and commonsense causal inference abilities. Experiments show considerable potential for enhancement in MLLMs.
...
TL;DR: A new AI model accurately detects diseases in falcons, improving upon existing methods.


<details>
  <summary>Details</summary>
Motivation: Falconry requires meticulous health surveillance to ensure the health and safety of falcons, especially in hunting.

Method: A hybrid of ConvNeXt and EfficientNet AI models is used for classification of falcon diseases.

Result: The concatenated AI model outperforms traditional diagnostic methods and individual model architectures in identifying Normal, Liver Disease, and Aspergillosis.

Conclusion: This hybrid AI model improves falcon disease detection, outperforming traditional methods and individual models, paving the way for AI in avian healthcare.
...
TL;DR: ViLLa (Vision-Language-Logic Approach), a neuro-symbolic framework designed for interpretable animal monitoring.
...
Result: Given an image and a question such as "How many dogs are in the scene?" or "Where is the buffalo?", the system grounds visual detections into symbolic facts and uses predefined rules to compute accurate answers related to count, presence, and location.
...
TL;DR: This paper introduces GraphGSOcc, a novel framework that combines semantic and geometric graph Transformer for 3D Gaussian Splating-based Occupancy Prediction. It addresses the issues of unified feature aggregation neglecting semantic correlations and boundary ambiguities. The experiments on the SurroundOcc dataset show a 1.97% mIoU improvement and 13.7% memory reduction compared to GaussianWorld.
...
Result: We propose the GraphGSOcc model, a novel framework that combines semantic and geometric graph Transformer for 3D Gaussian Splating-based Occupancy Prediction
...
TL;DR: The paper presents a new dataset and model for explainable AI-generated video detection, offering transparent and verifiable diagnostics.


<details>
  <summary>Details</summary>
Motivation: The ability to distinguish AI-generated video from authentic footage is crucial, but current methods lack interpretability and persuasive evidence.

Method: The paper introduces DAVID-X, a dataset of AI-generated videos with defect-level annotations and rationales, and uses it to fine-tune DAVID-XR1, a video-language model with chain-of-thought distillation.

Result: DAVID-XR1 achieves strong generalization in detecting AI-generated videos and provides interpretable reasoning through defect categorization, localization, and natural language explanations.

Conclusion: This paper introduces an interpretable video-language model (DAVID-XR1) for AI-generated video detection, demonstrating strong generalization across generators and modes.
...
TL;DR: This survey reviews deep learning methods for multi-agent trajectory prediction (2020-2024), categorizes them, and highlights challenges and future directions.
...
Result: reviews some of the most recent advancements in deep learning-based multi-agent trajectory prediction, focusing on studies published between 2020 and 2024.
...
TL;DR: Compared human-designed and AI-generated architectural forms using a new dataset and neural network, finding AI can distinguish origins better than humans and highlighting areas for generative tool improvement.
...
Method: Built ArchForms-4000 dataset; Proposed ArchShapeNet, a 3D convolutional neural network with a saliency module; Conducted comparative experiments.
...
TL;DR: This paper presents a high-performance, low-latency video surveillance system for resource-constrained environments using an entropy-based adaptive frame buffering algorithm and MobileNetV2.
...
TL;DR: MonoVQD是一种用于单目3D检测的新框架，它通过引入Mask Separated Self-Attention、Variational Query Denoising和自蒸馏策略来改进DETR架构。


<details>
  <summary>Details</summary>
Motivation: 单目3D检测中的一个核心挑战是从单个图像精确定位3D对象。虽然DETR类的架构提供了一个强大的范例，但它们在该领域的直接应用遇到了固有的局限性，从而阻碍了最佳性能。

Method: 提出Mask Separated Self-Attention机制，将去噪过程集成到DETR架构中；提出Variational Query Denoising技术，解决传统去噪方法的梯度消失问题；引入自蒸馏策略，利用来自后面解码器层的信息来协同提高前面层中的查询质量。

Result: MonoVQD在具有挑战性的KITTI单目基准测试中实现了卓越的性能。MonoVQD的核心组件无缝集成到其他架构中，即使在nuScenes数据集的多视图3D检测场景中也能提供显著的性能提升。

Conclusion: MonoVQD在KITTI单目基准测试中表现出色，并且核心组件可以无缝集成到其他架构中，即使在nuScenes数据集的多视图3D检测场景中也能提供显著的性能提升，从而突显了其强大的泛化能力。
...
TL;DR: ChartIR, an iterative refinement method, improves chart-to-code generation in MLLMs by using structured instructions for visual understanding and a two-stage generation pipeline.


<details>
  <summary>Details</summary>
Motivation: Multimodal large language models (MLLMs) perform suboptimally on chart-to-code generation, a task requiring precise visual understanding and accurate translation of visual elements into structured code. Directly prompting MLLMs yields unsatisfactory results.

Method: The paper proposes ChartIR, an iterative refinement method based on structured instruction. It distinguishes between visual understanding and code translation, using description and difference instructions to transform visual features into language representations. The chart generation pipeline is divided into initial code generation and iterative refinement stages.

Result: The ChartIR method achieves superior performance on both the open-source model Qwen2-VL and the closed-source model GPT-4o, compared to other methods.

Conclusion: The proposed ChartIR method achieves superior performance on chart-to-code generation compared to other methods, as demonstrated on both open-source (Qwen2-VL) and closed-source (GPT-4o) models.
...
TL;DR: PictSure 是一种 ICL 框架，它将嵌入模型置于分析中心，并通过改变视觉编码器类型、预训练目标和微调策略，在少样本图像分类任务上取得了优异的性能。


<details>
  <summary>Details</summary>
Motivation: 在数据稀缺的领域中，构建图像分类模型仍然很麻烦，在这些领域中，收集大型标记数据集是不切实际的。以往的工作在很大程度上忽略了基于 ICL 的 FSIC 管道的一个关键组成部分：图像嵌入的作用。

Method: 提出了一种 ICL 框架 PictSure，该框架将嵌入模型置于分析中心，并系统地检查了不同视觉编码器类型、预训练目标和微调策略对下游 FSIC 性能的影响。

Result: 训练的成功和域外性能高度依赖于嵌入模型是如何进行预训练的。

Conclusion: PictSure在与训练分布差异显著的域外基准测试中，优于现有的基于 ICL 的 FSIC 模型，同时在域内任务中保持了相当的结果。
...
TL;DR: This paper proposes BKSEF, a framework for optimal kernel size determination in CNNs, achieving accuracy improvements and FLOPs reduction. It is validated on multiple datasets and real-world case studies, showing its potential for efficient and application-aware CNN designs.


<details>
  <summary>Details</summary>
Motivation: Kernel size selection in Convolutional Neural Networks (CNNs) is a critical but often overlooked design decision that affects receptive field, feature extraction, computational cost, and model accuracy.

Method: Best Kernel Size Estimation Function (BKSEF), a mathematically grounded and empirically validated framework for optimal, layer-wise kernel size determination. BKSEF balances information gain, computational efficiency, and accuracy improvements by integrating principles from information theory, signal processing, and learning theory.

Result: BKSEF-guided architectures achieve up to 3.1 percent accuracy improvement and 42.8 percent reduction in FLOPs compared to traditional models using uniform 3x3 kernels. Two real-world case studies further validate the approach: one for medical image classification in a cloud-based setup, and another for traffic sign recognition on edge devices.

Conclusion: kernel size can be an active, optimizable parameter rather than a fixed heuristic. BKSEF provides practical heuristics and theoretical support for researchers and developers seeking efficient and application-aware CNN designs. It is suitable for integration into neural architecture search pipelines and real-time systems, offering a new perspective on CNN optimization.
...
TL;DR: This paper proposes a deep learning method to automate key-frame identification and annotation in retail videos, achieving comparable accuracy to human annotators with significant cost and time savings.
...
TL;DR: A new active view selection (AVS) method using neural uncertainty maps achieves comparable 3D reconstruction accuracy with less computation and generalizes well.


<details>
  <summary>Details</summary>
Motivation: Identifying the minimal set of views that yields the most accurate 3D reconstruction is a fundamental challenge in computer vision.

Method: A novel AVS approach guided by neural uncertainty maps predicted by a lightweight feedforward deep neural network (UPNet).

Result: The method achieves up to a 400 times speedup along with over 50% reductions in CPU, RAM, and GPU usage compared to baseline methods, while using half of the viewpoints.

Conclusion: The proposed AVS method achieves comparable reconstruction accuracy with significantly reduced computational overhead and generalizes effectively to novel object categories.
...
TL;DR: 本文提出了一种新的DPO-Kernels方法，用于提高文本到图像(T2I)模型的一致性，并引入了DETONATE基准来评估社会偏见和歧视。


<details>
  <summary>Details</summary>
Motivation: 对齐对于文本到图像(T2I)模型至关重要，以确保生成的图像忠实地捕捉用户意图，同时保持安全性和公平性。直接偏好优化(DPO)在大语言模型(llm)中非常突出，正将其影响力扩展到T2I系统。

Method: DPO-Kernels for T2I models，一种新颖的扩展，增强了三个维度上的一致性：(i) 混合损失，将基于嵌入的目标与传统的基于概率的损失相结合，以改进优化；(ii) 核化表示，采用径向基函数(RBF)、多项式和Wavelet核进行更丰富的特征转换，并在安全和不安全输入之间实现更好的分离；(iii) 发散选择，通过结合Wasserstein和R'enyi散度来扩展DPO的默认Kullback-Leibler (KL)正则化器，以增强稳定性和鲁棒性。

Result: 我们引入了DETONATE，这是第一个同类大规模基准，包含大约10万个精选的图像对，分为已选择和已拒绝。DETONATE封装了社会偏见和歧视的三个轴：种族、性别和残疾。提示来自仇恨言论数据集，图像由领先的T2I模型生成，包括Stable Diffusion 3.5 Large、Stable Diffusion XL和Midjourney。此外，我们提出了对齐质量指数(AQI)，这是一种新颖的几何测量方法，用于量化安全/不安全图像激活的潜在空间可分离性，揭示隐藏的漏洞。

Conclusion: DPO-Kernels保持了通过重尾自正则化(HT-SR)实现的强大泛化边界。DETONATE和完整的代码已公开发布。
...
TL;DR: 提出了一种新的强化学习方法PeRL，用于增强视觉语言模型在多图像推理任务中的能力，并在多个基准测试中取得了显著成果。
...
Result: PeRL训练的模型始终大幅超过R1相关和交错VLM基线，在多图像基准测试上实现了最先进的性能，同时在单图像任务上保持了可比的性能。

Conclusion: PeRL在多图像基准测试上实现了最先进的性能，同时在单图像任务上保持了可比的性能。
...
TL;DR: The paper proposes a Frequency-Calibrated Reconstruction Error (FCRE) method to improve Membership Inference Attacks (MIAs) on medical image diffusion models by focusing on mid-frequency reconstruction errors, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: The increasing use of diffusion models for image generation in sensitive areas like medical imaging has raised significant privacy concerns. Existing MIA methods often rely on diffusion reconstruction errors, but applying these methods directly to medical images faces challenges because reconstruction error is influenced by inherent image difficulty, and diffusion models struggle with high-frequency detail reconstruction.

Method: A Frequency-Calibrated Reconstruction Error (FCRE) method for MIAs on medical image diffusion models. It analyzes the reverse diffusion process, obtains the mid-frequency reconstruction error, and computes the structural similarity index score between the reconstructed and original images. Membership is determined by comparing this score to a threshold.
...
TL;DR: ViT模型在夸克-胶子喷注分类中优于CNN，为进一步研究奠定了基础。


<details>
  <summary>Details</summary>
Motivation: 区分夸克和胶子引发的喷注是高能物理中一项关键且具有挑战性的任务，对于改进大型强子对撞机上的新物理搜索和精确测量至关重要。虽然深度学习，特别是卷积神经网络（CNN），已经推进了使用基于图像表示的喷注标记，但视觉转换器（ViT）架构的潜力，以建模全局上下文信息而闻名，在直接量热仪图像分析方面仍 largely 未被探索，尤其是在真实的探测器和堆积条件下。

Method: 使用模拟的2012 CMS开放数据，构建来自探测器级能量沉积（ECAL、HCAL）和重建轨迹的多通道喷注视图图像，从而实现端到端学习。

Result: ViT+MaxViT和ViT+ConvNeXt混合模型在F1分数、ROC-AUC和准确率方面始终优于已建立的CNN基线。

Conclusion: ViT及其混合模型在夸克-胶子喷注分类中优于CNN基线，突出了捕获喷注子结构中远程空间相关性的优势。这项工作建立了首个系统框架和稳健的性能基线，用于将ViT架构应用于基于量热仪图像的喷注分类，并提供了一个结构化数据集。
...
TL;DR: 提出使用长期循环卷积网络（LRCN）和Transformer架构的模型，利用RGB触觉图像和视觉传感器GelSight捕获的其他信息来准确预测顺应性指标


<details>
  <summary>Details</summary>
Motivation: 传统的顺应性检测方法受到其缺乏便携性和可扩展性的限制，依赖于专业的、通常昂贵的设备，并且不适合机器人应用。此外，现有的基于视觉触觉传感器的神经网络方法仍然存在预测精度不足的问题。

Method: 我们提出了两种基于长期循环卷积网络（LRCN）和Transformer架构的模型，这些模型利用RGB触觉图像和视觉传感器GelSight捕获的其他信息来准确预测顺应性指标。

Result: 所提出的模型比基线表现出显著的性能改进

Conclusion: 我们验证了这些模型的性能，并证明了它们在准确估计顺应性方面的有效性。所提出的模型比基线表现出显著的性能改进。此外，我们研究了传感器顺应性和物体顺应性估计之间的相关性，结果表明，比传感器更硬的物体更难估计。
...
TL;DR: This paper proposes PALETTE, an end-to-end text spotter for scanned historical maps, and SynthMap+, a novel approach to automatically generate synthetic map images, for training text spotters for historical maps. The experiment shows that PALETTE with SynthMap+ outperforms SOTA text spotters on two new benchmark datasets of historical maps, particularly for long and angled text. We have deployed PALETTE with SynthMap+ to process over 60,000 maps and generated over 100 million text labels to support map searching.
...
Method: a semantic-based style injection method that uses BLIP to generate text descriptions strictly aligned with the semantics of the style image in CLIP space; a data augmentation strategy based on human feedback; a training-free triple diffusion process using the fine-tuned model, which manipulates the features of self-attention layers in a manner similar to the cross-attention mechanism

Result: enable efficient and drift-free injection of style knowledge; reduce its overfitting; inject style while maintaining text control over the model; mitigate disruptions to the original content
...
TL;DR: This paper addresses training instability and codebook collapse in vector quantization by using the Wasserstein distance to align feature and codebook distributions, achieving near 100% codebook utilization and reducing quantization error.


<details>
  <summary>Details</summary>
Motivation: Training instability and codebook collapse in existing vector quantization methods are primarily driven by a mismatch between the distributions of the features and code vectors, leading to unrepresentative code vectors and significant data information loss during compression.

Method: The Wasserstein distance is employed to align the distributions of the features and code vectors.

Result: achieving near 100% codebook utilization and significantly reducing the quantization error.

Conclusion: The effectiveness of the proposed approach is validated through empirical and theoretical analyses.
...
TL;DR: SynPo is a training-free few-shot method that improves negative prompts for better medical image segmentation using LVMs.
...
Method: SynPo, a training-free few-shot method based on LVMs, with a Confidence Map Synergy Module combining DINOv2 and SAM to select high-quality prompts for SAM using Gaussian distribution and K-means clustering.
...
TL;DR: propose a Point Distribution Set Abstraction module (PDSA) that utilizes the correlation in the high-dimensional space to correct the feature distribution during aggregation, which improves the computational efficiency and robustness


<details>
  <summary>Details</summary>
Motivation: there are irrelevant point interference and feature hierarchy gap problems due to the limitation of local coordinates. Although some works address this limitation by refining spatial description though explicit modeling of cross-stage structure, these enhancement methods based on direct geometric structure encoding have problems of high computational overhead and noise sensitivity

Method: propose the Point Distribution Set Abstraction module (PDSA) that utilizes the correlation in the high-dimensional space to correct the feature distribution during aggregation
...
TL;DR: This paper introduces Echo-DND, a novel dual-noise diffusion model for segmenting the left ventricle in echocardiograms. It outperforms existing models on two datasets.
...
TL;DR: ReSeDis unifies corpus-level retrieval with pixel-level grounding and introduces a new benchmark and metric for multimodal search systems.
...
TL;DR: This paper explores how to train generalist models for retinal optical coherence tomography using visual in-context learning (VICL).


<details>
  <summary>Details</summary>
Motivation: Recent advancements in medical image analysis have led to the development of highly specialized models tailored to specific clinical tasks. Yet, their applicability is limited to predefined tasks, requiring expertise and extensive resources for development and adaptation. In contrast, generalist models offer a different form of utility: allowing medical practitioners to define tasks on the fly without the need for task-specific model development.

Method: This work explores how to train generalist models for the domain of retinal optical coherence tomography using visual in-context learning (VICL).

Result: The paper proposes a broad evaluation protocol tailored to VICL in OCT.

Conclusion: This paper establishes a first baseline to highlight the potential and current limitations of in-context learning for OCT by  extensively evaluating a state-of-the-art medical VICL approach on multiple retinal OCT datasets.
...
TL;DR: Proposes Privacy-Shielded Image Compression (PSIC) to protect user privacy by preventing VLP models from exploiting publicly posted images. It uses a flexible coding method with multiple decoding options, balancing perceptual quality and semantic information preservation.
...
Method: a flexible coding method, termed Privacy-Shielded Image Compression (PSIC), that can produce bitstreams with multiple decoding options. A Conditional Latent Trigger Generation (CLTG) module is proposed to produce bias information based on customizable conditions to guide the decoding process into different reconstructed versions, and an Uncertainty-Aware Encryption-Oriented (UAEO) optimization function is designed to leverage the soft labels inferred from the target VLP model's uncertainty on the training data. This paper further incorporates an adaptive multi-objective optimization strategy to obtain improved encrypting performance and perceptual quality simultaneously within a unified training process.

Result: By default, the bitstream is decoded to preserve satisfactory perceptual quality while preventing interpretation by VLP models. Our method also retains the original image compression functionality. With a customizable input condition, the proposed scheme can reconstruct the image that preserves its full semantic information.
...
TL;DR: This paper proposes DM-FNet, a two-stage diffusion model-based fusion network, to address the limitations of existing MMIF methods in capturing detailed features and cross-modal feature interaction. The experimental results demonstrate that the proposed method performs exceptionally well.
...
TL;DR: 提出了 video-SALMONN 2，一个用于增强视频字幕的音频-视觉大语言模型，通过定向偏好优化 (DPO) 和多轮 DPO (MrDPO) 方法进行优化，并在视频字幕任务上超越了 GPT-4o 和 Gemini-1.5-Pro。


<details>
  <summary>Details</summary>
Motivation: 生成自然语言的详细和准确的描述是视频理解的关键方面。

Method: 提出了一种新颖的多轮 DPO (MrDPO) 方法，该方法涉及定期更新 DPO 参考模型，在每个训练轮次（1,000 步）后合并并重新初始化 LoRA 模块作为参数更新的代理，并结合来自 ground-truth 视频字幕的指导以稳定该过程。

Result: MrDPO 显着提高了 video-SALMONN 2 的字幕准确性，将字幕错误率降低了 28%。

Conclusion: video-SALMONN 2 模型仅用 70 亿参数，在视频字幕任务上超越了 GPT-4o 和 Gemini-1.5-Pro 等领先模型，并在常用的视频问答基准测试中保持了与同等规模模型相比极具竞争力的性能。
...
TL;DR: This paper proposes C-AFBiFPN, a novel feature enhancement and fusion framework for SAR ship detection, which enhances detection accuracy, robustness, and adaptability to multi-scale features.
...
Conclusion: The proposed C-AFBiFPN framework substantially enhances detection accuracy for small targets, robustness against occlusions, and adaptability to multi-scale features on the SAR Ship Detection Dataset (SSDD).
...
TL;DR: RA-NeRF predicts highly accurate camera poses even with complex camera trajectories by using NeRF with photometric consistency, flow-driven pose regulation, and an implicit pose filter.
...
TL;DR: RetroMem通过整合相关的历史知识来动态地调节伪装模式的感知和推理，从而改进了伪装目标检测。


<details>
  <summary>Details</summary>
Motivation: 现有方法缺乏获取历史背景的明确机制，限制了它们在处理具有挑战性的伪装场景中的适应性和有效性。

Method: RetroMem采用两阶段训练模式，包括学习阶段和召回阶段，以有效地构建、更新和利用记忆表征。学习阶段设计了密集多尺度适配器（DMA），以提高预训练编码器捕获丰富多尺度视觉信息的能力。在召回阶段，提出了动态记忆机制（DMM）和推理模式重建（IPR）。

Result: 在几个广泛使用的数据集上的大量实验表明，RetroMem显著优于现有的最先进方法。

Conclusion: RetroMem显著优于现有技术。
...
TL;DR: This paper proposes and validates the DBACS approach, a CycleGAN-inspired model, for Domain Adaptation in the semiconductor field using real-world Electron Microscope images.


<details>
  <summary>Details</summary>
Motivation: Time to market and quality are key factors in securing significant market share in the semiconductor sector. Domain Adaptation (DA) has proven highly effective in improving robustness and scalability, minimizing the need for extensive manual re-labeling or re-training of models.

Method: The DBACS approach, a CycleGAN-inspired model enhanced with additional loss terms.

Result: DA techniques' efficacy in semi-supervised and unsupervised settings within the context of the semiconductor field was tested.

Conclusion: The usefulness of the DBACS approach in advancing DA techniques for the semiconductor field is validated on real-world Electron Microscope images considering the unsupervised and semi-supervised settings.
...
TL;DR: MSNeRV是一种用于神经视频表示的多尺度特征融合框架，它在动态场景中的压缩效率超过了VTM-23.7 (Random Access)。


<details>
  <summary>Details</summary>
Motivation: 现有的基于INR的方法难以有效地表示细节密集和快速变化的视频内容。这种限制主要源于内部网络特征的利用不足以及网络设计中缺乏对视频的特定考虑。

Method: 提出了一种用于神经视频表示的多尺度特征融合框架MSNeRV

Result: MSNeRV在INR方法中表现出卓越的表示能力

Conclusion: MSNeRV在动态场景中的压缩效率超过了VTM-23.7 (Random Access)
...
TL;DR: 本文提出了一种新的地标检测框架BCRNet，用于增强腹腔镜肝脏手术中的手术导航，并在L3D和P2ILF数据集上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 腹腔镜肝脏手术虽然是微创手术，但在准确识别关键解剖结构方面提出了重大挑战。增强现实(AR)系统，将MRI/CT与基于2D-3D配准的腹腔镜图像相结合，为增强手术导航提供了一个有希望的解决方案。

Method: 提出了一种新的框架，即Bezier曲线细化网络(BCRNet)，该网络主要通过Bezier曲线细化策略来显著增强腹腔镜肝脏手术中的地标检测。

Result: 在L3D和P2ILF数据集上进行了广泛的评估，证明BCRNet优于现有方法，实现了显著的性能改进。
...
<details>
  <summary>Details</summary>
Motivation: Visual monitoring of industrial assembly tasks is critical for preventing equipment damage and ensuring worker safety, but current solutions require rigid setups or visual markers.

Method: ViMAT combines a perception module extracting visual observations with a reasoning module inferring actions based on assembly state and prior knowledge.
...
TL;DR: MEGC 2025 挑战赛引入了 ME 发现-识别和 ME 视觉问答两个任务，旨在利用大型语言模型提升面部微表情分析技术。


<details>
  <summary>Details</summary>
Motivation: 传统的将发现和识别作为单独任务的方法是次优的，特别是对于分析真实场景中的长时程视频。多模态大型语言模型 (MLLM) 和大型视觉-语言模型 (LVLM) 的出现为通过其强大的多模态推理能力增强 ME 分析提供了有希望的新途径。

Method: 提出了 ME 发现-识别 (ME-STR) 任务，集成了 ME 发现和识别；提出了 ME 视觉问答 (ME-VQA) 任务，通过视觉问答探索 ME 理解。

Result: 所有参与的算法都需要在这个测试集上运行，并在排行榜上提交他们的结果。

Conclusion: MEGC 2025 提出了两个任务，分别是 ME 发现-识别 (ME-STR) 和 ME 视觉问答 (ME-VQA)，利用 MLLM 或 LVLM 来理解 ME。
...
TL;DR: MapFM, an enhanced end-to-end model, generates vectorized HD maps online by using a foundation model for camera image encoding and auxiliary prediction heads for semantic segmentation, boosting feature representation and improving accuracy.


<details>
  <summary>Details</summary>
Motivation: Accurate localization, planning, and decision-making in autonomous driving require high-definition (HD) maps and semantic maps in bird's-eye view (BEV).
...
Result: Significantly boosted feature representation quality by incorporating a powerful foundation model for encoding camera images; richer contextual supervision and a more comprehensive scene representation.

Conclusion: The paper's multi-task learning approach, incorporating semantic segmentation, enhances scene representation, leading to higher accuracy and improved quality in predicted vectorized HD maps.
...
TL;DR: OpenPath is a new active learning method for pathology images that uses a Vision-Language Model to handle OOD data, improving performance and reducing labeling costs.


<details>
  <summary>Details</summary>
Motivation: Traditional Active Learning (AL) methods for pathology image classification require large datasets and struggle with Out-Of-Distribution (OOD) data in real-world clinical environments, leading to inefficient annotation and wasted labeling costs.

Method: A novel open-set active learning approach for pathological image classification leveraging a pre-trained Vision-Language Model (VLM). Includes task-specific prompts and Diverse Informative ID Sampling (DIS) with Prototype-based ID candidate Selection (PIS) and Entropy-Guided Stochastic Sampling (EGSS).

Result: Experiments on two public pathology image datasets demonstrate that OpenPath significantly enhances model performance and outperforms several state-of-the-art open-set AL methods.

Conclusion: OpenPath significantly enhances model performance due to high purity of selected samples and outperforms state-of-the-art open-set AL methods.
...
TL;DR: Introduce a new task of open-world object counting in videos and a model, CountVid, for this task. A new dataset, VideoCount, is also introduced to evaluate its performance.


<details>
  <summary>Details</summary>
Motivation: The task is especially challenging in crowded scenes with occlusions and similar objects, where avoiding double counting and identifying reappearances is crucial.

Method: It leverages an image-based counting model, and a promptable video segmentation and tracking model to enable automated, open-world object counting across video frames.
...
TL;DR: DDIS, a Diffusion-assisted Data-free Image Synthesis method, leverages a text-to-image diffusion model and Domain Alignment Guidance (DAG) to generate high-quality synthetic images that better reflect the training data distribution.


<details>
  <summary>Details</summary>
Motivation: existing DFIS meth ods produce samples that deviate from the training data distribution due to the lack of prior knowl edge about natural images. To overcome this limitation

Method: Diffusion-assisted Data-free Image Synthesis method that leverages a text-to-image diffusion model as a powerful image prior, improving synthetic image quality. Domain Alignment Guidance (DAG) that aligns the synthetic data domain with the training data domain during the diffusion sampling process. optimize a single Class Alignment Token (CAT) embedding to effectively capture class-specific attributes in the training dataset.

Result: Experiments on PACS and Ima geNet demonstrate that DDIS outperforms prior DFIS methods by generating samples that better reflect the training data distribution, achieving SOTA performance in data-free applications.

Conclusion: DDIS outperforms prior DFIS methods by generating samples that better reflect the training data distribution, achieving SOTA performance in data-free applications.
...
TL;DR: A new neuron-level relevance based method (NERO) for out-of-distribution detection in medical imaging, improving performance and explainability.


<details>
  <summary>Details</summary>
Motivation: Existing OOD detection methods may not fully capture OOD diversity, which is critical in medical imaging to flag potential anomalies.

Method: A novel OOD scoring mechanism, called NERO, that leverages neuron-level relevance at the feature layer. It clusters neuron-level relevance for each in-distribution (ID) class to form representative centroids and introduces a relevance distance metric.

Result: Improvements over state-of-the-art OOD detection methods on the gastrointestinal imaging benchmarks Kvasir and GastroVision.

Conclusion: The proposed NERO framework improves OOD detection performance and explainability on gastrointestinal imaging benchmarks.
...
TL;DR: 本教程提供了一个关于使用 Hunyuan3D 2.1 训练和评估 3D 生成模型的逐步指南，旨在使该领域对更广泛的受众开放。


<details>
  <summary>Details</summary>
Motivation: 3D AI 生成内容 (AIGC) 是一个充满激情的领域，它极大地加速了游戏、电影和设计领域中 3D 模型的创建。尽管开发了几种革命性的 3D 生成模型，但由于收集、处理和训练 3D 模型所涉及的复杂性，该领域在很大程度上仍然只对研究人员、开发人员和设计人员开放。

Method: 本教程提供了一个全面的、逐步的指南，介绍了如何使用 Hunyuan3D 2.1（一种用于生成高分辨率、纹理 3D 资产的先进系统）处理 3D 数据、训练 3D 生成模型并评估其性能。

Result: 我们介绍了 Hunyuan3D 2.1 作为本教程中的一个案例研究。该系统包含两个核心组件：用于形状生成的 Hunyuan3D-DiT 和用于纹理合成的 Hunyuan3D-Paint。

Conclusion: 读者将能够掌握微调或开发适用于游戏、虚拟现实和工业设计的强大 3D 生成模型所需的知识。
...
TL;DR: MRG-LLM是一种新型多模态大型语言模型（MLLM），它结合了冻结的LLM与可学习的视觉编码器，并引入了一种动态提示定制机制，在医学报告生成方面取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 医学影像数据的医学报告生成仍然是临床实践中一项具有挑战性的任务。大型语言模型（LLM）在应对这一挑战方面显示出巨大的希望，但它们与医学影像数据的有效整合仍然值得深入探索。

Method: 结合了冻结的LLM与可学习的视觉编码器，并引入了动态提示定制机制。

Result: MRG-LLM在IU X-ray和MIMIC-CXR数据集上取得了最先进的性能。

Conclusion: MRG-LLM在医学报告生成方面取得了最先进的性能。
...
TL;DR: introduce GenHOI, a novel two-stage framework for 4D human-object interaction (HOI) sequence generation
...
TL;DR: NTIRE 2025 Shadow Removal Challenge results: 17 teams competed using WSRD+ dataset, evaluated on reconstruction fidelity and visual perception.
...
Result: Evaluated with images from the WSRD+ dataset, simulating interactions between self- and cast-shadows with a large number of diverse objects, textures, and materials.
...
<details>
  <summary>Details</summary>
Motivation: the limited availability and variability of LGE images with high-quality scar labels restrict the development of robust segmentation models
...
TL;DR: RaCalNet is a novel framework that eliminates the need for dense supervision by using sparse LiDAR to supervise the learning of refined radar measurements, enabling metric-scale estimation without resorting to dense supervision. It surpasses state-of-the-art dense-supervised methods.
...
Method: RaCalNet: recalibrates and refines sparse radar points to construct accurate depth priors. These priors then serve as reliable anchors to guide monocular depth prediction, enabling metric-scale estimation without resorting to dense supervision.

Result: RaCalNet surpasses state-of-the-art dense-supervised methods, producing depth maps with clear object contours and fine-grained textures. Extensive experiments on the ZJU-4DRadarCam dataset and real-world deployment scenarios demonstrate its effectiveness, reducing RMSE by 35.30% and 34.89%, respectively.
...
TL;DR: WinWinLay 是一种新颖的无训练方法，通过非局部注意力能量函数和自适应更新来增强控制精度和真实感，从而改进 Layout-to-Image 生成。


<details>
  <summary>Details</summary>
Motivation: 现有研究表明，预训练的文本到图像扩散模型可以在没有任何特定数据训练的情况下实现这一目标；但是，它们经常面临不精确定位和不真实的伪像的挑战。

Method: WinWinLay 提出两种关键策略：非局部注意力能量函数和自适应更新，以协同增强控制精度和真实感。非局部注意力先验被探索用于重新分配注意力分数，促进对象更好地符合指定的空间条件。引入了基于 Langevin 动力学的自适应更新方案，作为一种补救措施，可以在尊重布局约束的同时促进域内更新。

Result: WinWinLay 在控制元素布局和实现照片般逼真的视觉保真度方面表现出色，优于当前最先进的方法。

Conclusion: WinWinLay在控制元素布局和实现照片般逼真的视觉保真度方面表现出色，优于当前最先进的方法。
...
TL;DR: Show-o2 是一种改进的统一多模态模型，它利用自回归建模和流匹配来处理各种模态的理解和生成任务。


<details>
  <summary>Details</summary>
Motivation: 本文提出了改进的本地统一多模态模型，即 Show-o2，它利用自回归建模和流匹配。

Method: 利用自回归建模和流匹配。基于3D因果变分自动编码器空间，通过空间（-时间）融合的双路径构建统一的视觉表示。

Result: Show-o2 模型在处理包括文本、图像和视频在内的各种模态的广泛多模态理解和生成任务中表现出多功能性。

Conclusion: Show-o2模型在处理包括文本、图像和视频在内的各种模态的广泛多模态理解和生成任务中表现出多功能性。
...
TL;DR: A parameter-efficient semi-supervised segmentation framework is proposed for 0.3 m spatial resolution imagery, which leverages the knowledge of SAM2 and introduces a remote sensing-specific FreqWeaver Adapter to enhance fine-grained detail modeling. The proposed method delivers robust segmentation results with superior structural consistency.
...
TL;DR: This paper introduces a new end-to-end framework for estimating forest AGB using point clouds and graph operations, overcoming limitations of existing QSM methods and enabling large-scale applications with ULS data.


<details>
  <summary>Details</summary>
Motivation: Current QSM methods face significant limitations, as they are primarily designed for individual trees,depend on high-quality point cloud data from terrestrial laser scanning (TLS), and also require multiple pre-processing steps that hinder scalability and practical deployment.
...
Conclusion: The proposed framework provides a robust and scalable solution for large-scale, non-destructive AGB estimation. It significantly reduces dependency on specialized pre-processing tools and establishes ULS as a viable alternative to TLS. This advancement substantially improves the feasibility of QSM-based AGB estimation, paving the way for broader applications in forest inventory and climate change research.
...
Method: a Dual LoRA Learning (DLoRAL) paradigm to train an effective SD-based one-step diffusion model
...
TL;DR: M2M-Reg是一种用于多模态图像配准的新框架，它使用单模态相似性进行训练，并具有正则化器以促进微分同胚。在ADNI数据集上的实验表明，M2M-Reg在PET-MRI和FA-MRI配准方面优于现有方法。
...
Method: M2M-Reg（多对单配准），一种新颖的框架，该框架仅使用单模态相似性来训练多模态DIR模型，同时保留已建立的架构范例，以实现无缝集成到现有模型中。此外，我们还引入了GradCyCon，这是一种正则化器，它利用M2M-Reg的循环训练方案来促进微分同胚。

Result: M2M-Reg实现了高达2倍的更高的DSC
...
TL;DR: 提出了一种新颖的无重建在线框架，用于内存高效和实时的 3D 检测，并在 ScanNetV2 和 CA-1M 数据集上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 由于开放词汇 3D 对象检测在自动驾驶和具身人工智能中的关键应用而引起了极大的兴趣。现有的检测方法，无论是离线还是在线，通常都依赖于密集的点云重建，这会带来大量的计算开销和内存限制，从而阻碍了下游任务中的实时部署。为了解决这个问题

Method: 我们提出了一个新颖的无重建在线框架，该框架专为内存高效的实时 3D 检测而定制。具体来说，给定流式姿势 RGB-D 视频输入，我们利用 Cubify Anything 作为预训练的视觉基础模型 (VFM)，通过边界框进行单视图 3D 对象检测，并结合 CLIP 来捕获检测对象的开放词汇语义。为了将跨不同视图检测到的所有边界框融合为一个统一的边界框，我们采用关联模块来实现多视图的对应关系，并采用优化模块来融合在多视图中预测的同一实例的 3D 边界框。

Result: 我们的方法在在线方法中实现了最先进的性能。受益于这种用于 3D 对象检测的新型无重建范例，我们的方法在各种场景中表现出出色的泛化能力，即使在超过 1000 平方米的环境中也能实现实时感知。

Conclusion: 该方法在 ScanNetV2 和 CA-1M 数据集上进行了大量实验，结果表明该方法在在线方法中实现了最先进的性能。受益于这种用于 3D 对象检测的新型无重建范例，该方法在各种场景中表现出出色的泛化能力，即使在超过 1000 平方米的环境中也能实现实时感知。
...
TL;DR: HOIDiNi 是一种文本驱动的扩散框架，用于合成逼真且合理的人与物体交互 (HOI)。


<details>
  <summary>Details</summary>
Motivation: HOI 生成极具挑战性，因为它会产生严格的接触精度以及多样化的运动流形。当前的文献在真实感和物理正确性之间进行权衡。

Method: 在预训练扩散模型的噪声空间中使用扩散噪声优化 (DNO) 进行优化。

Result: HOIDiNi 在接触精度、物理有效性和整体质量方面优于先前的工作和基线。
...
TL;DR: introduce a new benchmark for long-range embodied tasks in the Habitat simulator to evaluate VLMs on memory-intensive tasks


<details>
  <summary>Details</summary>
Motivation: deploying these models for reasoning in embodied contexts is limited by their ability to incorporate long-term experience collected across multiple days and represented by vast collections of images. Current VLMs typically struggle to process more than a few hundred images concurrently, highlighting the need for more efficient mechanisms to handle long-term memory in embodied settings

Method: introducing a new benchmark for long-range embodied tasks in the Habitat simulator

Result: assessing their performance on these memory-intensive tasks and highlight areas for improvement

Conclusion: This work introduces a new benchmark for long-range embodied tasks in the Habitat simulator. This benchmark evaluates memory-based capabilities across 60 tasks requiring sustained engagement and contextual awareness in an environment.
...
TL;DR: find that model performance can improve when images deviate from human-perceived fidelity, and introduce VQ-TTT to align each input image with task-specific model preferences, which lifts significant average accuracy


<details>
  <summary>Details</summary>
Motivation: little is known about how input visual quality shapes MLLMs' responses. Does higher perceptual quality of images already translate to better MLLM understanding?

Method: introduce Visual-Quality Test-Time Tuning (VQ-TTT)-a lightweight adaptation module that: (1) inserts a learnable, low-rank kernel before the frozen vision encoder to modulate frequency content; and (2) fine-tunes only shallow vision-encoder layers via LoRA

Result: model, task, and even individual-instance performance can improve when images deviate from human-perceived fidelity. Off-the-shelf restoration pipelines fail to reconcile these idiosyncratic preferences. VQ-TTT lifts significant average accuracy, with no external models, cached features, or extra training data

Conclusion: introduce Visual-Quality Test-Time Tuning (VQ-TTT) to adjusts each input image in a single forward pass, aligning it with task-specific model preferences. VQ-TTT lifts significant average accuracy
...
TL;DR: ViMaR is a fast and accurate two-stage inference framework for VLMs that improves caption quality and efficiency by using a value model and margin-based reward adjustment. It also generalizes well and can be used for self-training.


<details>
  <summary>Details</summary>
Motivation: Existing inference-time search methods for VLMs are computationally expensive and prone to unpenalized, low-confidence generations, leading to hallucinations.

Method: Value-guided Inference with Margin-based Reward (ViMaR): a two-stage inference framework that improves both efficiency and output fidelity by combining a temporal-difference value model with a margin-aware reward adjustment. Includes a first stage to identify the highest-value caption and a second stage to selectively refine segments with weak visual grounding. A calibrated margin-based penalty discourages low-confidence continuations.

Result: ViMaR achieves over 4x speedup compared to existing value-guided methods and generates more reliable, factually accurate, detailed, and explanatory captions. It also generalizes effectively to guide decoding in stronger unseen models and improves performance on visual comprehension benchmarks when used for self-training.

Conclusion: ViMaR generates captions that are significantly more reliable, factually accurate, detailed, and explanatory, while achieving over 4x speedup compared to existing value-guided methods. ViMaR trained solely on LLaVA Mistral-7B, generalizes effectively to guide decoding in a stronger unseen model. ViMaR-generated captions are used for self-training, the underlying models achieve substantial gains across a broad suite of visual comprehension benchmarks, underscoring the potential of fast, accurate, and self-improving VLM pipelines.
...
TL;DR: 我们提出了一个单通视频扩散模型，用于从单一图像或视频中进行重新照明，该模型在各种领域中表现出强大的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有的端到端重新照明模型通常受到配对的多照明数据稀缺的限制，从而限制了它们在各种场景中进行泛化的能力。

Method: 我们引入了一种通用方法，该方法在单次传递中联合估计反照率并合成重新照明的输出，利用视频扩散模型的生成能力。

Result: 我们的模型展示了强大的泛化能力，并在视觉保真度和时间一致性方面超越了以前的方法。

Conclusion: 该模型在各种领域表现出强大的泛化能力，并在视觉保真度和时间一致性方面超越了以前的方法。
...
TL;DR: introduce Sekai, a high-quality first-person view worldwide video dataset with rich annotations for world exploration
...
Method: an efficient and effective toolbox to collect, pre-process and annotate videos with location, scene, weather, crowd density, captions, and camera trajectories.

Result: Sekai consists of over 5,000 hours of walking or drone view (FPV and UVA) videos from over 100 countries and regions across 750 cities. Experiments demonstrate the quality of the dataset. And, we use a subset to train an interactive video world exploration model, named YUME
...
TL;DR: ECAD是一个遗传算法，可以学习有效的缓存策略，加速扩散模型的推理，并在质量和延迟之间实现细粒度的控制。


<details>
  <summary>Details</summary>
Motivation: 基于扩散的图像生成模型在生成高质量合成内容方面表现出色，但存在推理速度慢和计算成本高的问题。先前的工作试图通过在推理步骤中缓存和重用扩散转换器中的特征来缓解这个问题，但是，这些方法通常依赖于刚性的启发式方法，导致加速效果有限或跨架构的泛化能力差。

Method: 使用遗传算法学习有效的、每个模型的缓存策略，形成帕累托前沿。

Result: ECAD在PixArt-alpha上，找到了一个超越先前最先进方法的策略，COCO FID提高了4.47，同时推理加速从2.35倍提高到2.58倍。在PixArt-Sigma和FLUX-1.dev上使用多个指标（FID、CLIP、图像奖励）在不同的基准（COCO、MJHQ-30k、PartiPrompts）上进行了评估，证明了相对于先前方法的持续改进。
...
TL;DR: CALM 结合了符号推理和神经生成，使系统能够在基于真实世界多模态数据的上下文中做出决策。


<details>
  <summary>Details</summary>
Motivation: 经典的二价逻辑系统无法捕捉人类决策的细微差别。它们还需要在多模态环境中进行人工 grounding，这可能是临时的、僵化的和脆弱的。神经网络擅长从多模态数据中提取丰富的上下文信息，但缺乏用于推理的可解释结构。

Method: CALM使用域树表示每个谓词，当确定其实体的上下文基础时，域树会迭代地细化其模拟真值。迭代细化由能够捕获多模态信息的神经网络预测，并通过符号推理模块进行过滤，以确保约束满足。

Result: 在填空对象放置任务中，CALM实现了 92.2% 的准确率，优于经典逻辑 (86.3%) 和 LLM (59.4%) 基线。它还展示了与逻辑约束和微妙的人类偏好对齐的空间热图生成，如一项人类研究所示。

Conclusion: CALM展示了在多模态环境中以逻辑结构进行推理并与偏好对齐的潜力。它为需要逻辑的精确性和解释以及神经网络的多模态信息处理的下一代人工智能系统奠定了基础。
...
TL;DR: Introduces MEAL, a benchmark for continual multi-agent reinforcement learning (CMARL) that uses GPU acceleration to enable learning across long task sequences.


<details>
  <summary>Details</summary>
Motivation: Underexplored intersection is continual learning (CL) in cooperative multi-agent settings. To remedy this, we introduce MEAL (Multi-agent Environments for Adaptive Learning), the first benchmark tailored for continual multi-agent reinforcement learning (CMARL). Existing CL benchmarks run environments on the CPU, leading to computational bottlenecks and limiting the length of task sequences.

Method: Leverages JAX for GPU acceleration

Result: Enabling continual learning across sequences of 100 tasks on a standard desktop PC in a few hours.

Conclusion: Naively combining popular CL and MARL methods yields strong performance on simple environments, but fails to scale to more complex settings requiring sustained coordination and adaptation. Architectural and algorithmic features critical for CMARL on MEAL are identified.
...
TL;DR: T-PPO, a new PPO extension, improves training efficiency of reasoning LLMs by up to 2.5x by streamlining policy update and using length-restricted response generation.


<details>
  <summary>Details</summary>
Motivation: PPO can be time-consuming due to its inherent on-policy nature, which is further exacerbated by increasing response lengths.  Low hardware utilization in fully synchronized long-generation procedures.

Method: Truncated Proximal Policy Optimization (T-PPO), a novel extension to PPO that improves training efficiency by streamlining policy update and length-restricted response generation. It includes Extended Generalized Advantage Estimation (EGAE) and a computationally optimized mechanism for independent optimization of policy and value models.

Result: T-PPO improves training efficiency by up to 2.5x and outperforms existing competitors on AIME 2024 with a 32B base model.
...
TL;DR: HeurAgenix是一个由LLM驱动的超启发式框架，通过进化和选择启发式算法来解决组合优化问题，并在基准测试中表现出色。


<details>
  <summary>Details</summary>
Motivation: 传统的启发式算法设计严重依赖于手动专业知识，并且难以在不同的实例中推广。

Method: HeurAgenix是一个由大型语言模型（LLM）驱动的两阶段超启发式框架，该框架首先进化启发式算法，然后在它们之间自动选择。

Result: HeurAgenix利用LLM比较种子启发式解决方案与更高质量的解决方案，并提取可重用的进化策略。在问题解决过程中，它在LLM的感知能力的指导下，动态地为每个问题状态选择最有希望的启发式算法。为了缓解CO复杂性导致可靠监督的稀缺性，我们使用双重奖励机制对轻量级启发式选择器进行微调，该机制共同利用来自选择偏好和状态感知的信号，从而能够在嘈杂的注释下进行鲁棒的选择。

Conclusion: HeurAgenix在典型基准测试中不仅优于现有的基于LLM的超启发式算法，而且与专用求解器相匹配或超过了专用求解器。
...
TL;DR: 本文研究了基于MARL的自主地球观测任务规划，解决了能源和数据存储限制、卫星观测的不确定性以及分散协调的复杂性等关键挑战。


<details>
  <summary>Details</summary>
Motivation: 低地球轨道（LEO）卫星的指数增长彻底改变了地球观测（EO）任务，解决了气候监测、灾害管理等方面的挑战。然而，多卫星系统中的自主协调仍然是一个根本性的挑战。传统的优化方法难以处理动态EO任务的实时决策需求，因此需要使用强化学习（RL）和多智能体强化学习（MARL）。

Method: 通过建模单卫星操作并使用MARL框架扩展到多卫星星座，研究基于RL的自主EO任务规划。利用近乎真实的卫星仿真环境，评估了最先进的MARL算法（包括PPO、IPPO、MAPPO和HAPPO）的训练稳定性和性能。

Result: 结果表明，MARL可以有效地平衡成像和资源管理，同时解决多卫星协调中的非平稳性和奖励相互依赖性。

Conclusion: MARL可以有效地平衡成像和资源管理，同时解决多卫星协调中的非平稳性和奖励相互依赖性。这项研究获得的见解为自主卫星操作奠定了基础，为改进分散式EO任务中的策略学习提供了实用的指导。
...
TL;DR: Proposes a cooperative MEC framework for computation offloading and resource allocation in maritime environments using UAVs and vessels, addressing uncertain tasks with Lyapunov optimization and a heterogeneous-agent soft actor-critic.


<details>
  <summary>Details</summary>
Motivation: The computation demands from the maritime Internet of Things (MIoT) increase rapidly, and the uncertain maritime tasks present significant challenges of inefficient computation offloading and resource allocation.

Method: A heterogeneous-agent soft actor-critic is proposed to sequentially update various neural networks and effectively solve the MG problem.

Result: The optimization problem is formulated to minimize the total execution time. Lyapunov optimization is leveraged to tackle the unpredictable task arrivals and varying computational resource availability. The small-scale optimization problem is reformulated into a Markov game (MG).

Conclusion: Simulations verify the effectiveness in addressing computational offloading and resource allocation.
...
TL;DR: This paper introduces Causality-Aware Navigation (CAN) to improve visual navigation by incorporating a Causal Understanding Module, which enhances the agent's environmental understanding capability and outperforms baselines without computational overhead.
...
Method: introducing a causal framework and proposing Causality-Aware Navigation (CAN), which incorporates a Causal Understanding Module

Result: CAN consistently outperforms baselines across various tasks and simulation environments, and the gains are attributed to the Causal Understanding Module

Conclusion: The proposed Causality-Aware Navigation (CAN) method consistently outperforms baselines across various tasks and simulation environments. The Causal Understanding Module generalizes effectively in both Reinforcement and Supervised Learning settings without computational overhead.
...
TL;DR: This paper introduces an LLM-based Planning Agent (LPA) to help FA engineers solve analysis cases by integrating LLMs with advanced planning capabilities and external tool utilization.
...
TL;DR: This paper presents a framework for constructing natural language state representations for LLM agents in repeated multi-agent games and finds that the representation significantly impacts agent behavior, with summarized history and regret information leading to more stable and equilibrium-aligned play.


<details>
  <summary>Details</summary>
Motivation: LLMs have shown promise as decision-makers in dynamic settings, but their stateless nature necessitates creating a natural language representation of history. Previous work has taken an ad hoc approach to encoding game history, which obscures the impact of state representation on agents' behavior and limits comparability between studies.

Method: a unifying framework for systematically constructing natural language state representations for prompting LLM agents in repeated multi-agent games

Result: Representations which provide agents with summarized history, regret information, and limited information about others' actions lead to behavior that more closely matches game theoretic equilibrium predictions, and with more stable game play. Other representations can exhibit deviations from equilibrium or higher variation in dynamic game play over time.

Conclusion: LLM agent behavior is dependent on the natural language state representation. Representations that provide agents with summarized past history, information about regrets, and limited information about others' actions lead to behavior that more closely matches game theoretic equilibrium predictions, and with more stable game play by the agents.
...
TL;DR: 开发了一个AI政策模块，以将AI政策的讨论引入CS课程，发现学生对AI伦理的担忧有所增加，并且对参与AI监管讨论的能力更有信心。


<details>
  <summary>Details</summary>
Motivation: 目前的高等教育计算机课程不足以让未来的AI从业者能够将抽象的伦理原则和规范的政策偏好融入到AI系统的设计和开发中。熟悉“AI政策环境”以及将伦理原则转化为实践的能力，将构成即使是最注重技术的AI工程师的重要责任。

Method: 开发了一个人工智能政策模块，以将人工智能政策的讨论引入计算机科学课程。在2024年秋季成功试点的基础上，提出了模块的更新和扩展版本，包括一项关于“人工智能监管”的技术任务。

Result: 学生对人工智能技术的伦理影响的担忧有所增加，同时也表达了对参与人工智能监管讨论的能力更有信心。人工智能监管任务是探索人工智能对齐限制和强调“政策”在解决伦理挑战中的作用的有效工具。

Conclusion: 学生在学习该模块后，对人工智能技术的伦理影响的担忧有所增加，同时也表达了对参与人工智能监管讨论的能力更有信心。人工智能监管任务是探索人工智能对齐限制和强调“政策”在解决伦理挑战中的作用的有效工具。
...
Method: Efficiency Steering, a training-free activation steering technique that modulates reasoning behavior via a single direction in the model's representation space; Self-Rewarded Efficiency RL, a reinforcement learning framework that dynamically balances task accuracy and brevity by rewarding concise correct solutions.

Result: The methods significantly reduce reasoning length while preserving or improving task performance on seven LRM backbones across multiple mathematical reasoning benchmarks.
...
TL;DR: SwarmAgentic is a framework for fully automated agentic system generation that constructs agentic systems from scratch and jointly optimizes agent functionality and collaboration. It outperforms all baselines, achieving a +261.8% relative improvement over ADAS on the TravelPlanner benchmark.
...
Result: SwarmAgentic outperforms all baselines, achieving a +261.8% relative improvement over ADAS on the TravelPlanner benchmark, highlighting the effectiveness of full automation in structurally unconstrained tasks.
...
TL;DR: This paper introduces Embodied Web Agents, a new paradigm for AI agents that combines embodiment and web-scale reasoning. They also developed a simulation platform and benchmark for evaluating these agents on tasks requiring both physical and digital intelligence, and found that current AI systems still lag behind human capabilities.
...
TL;DR: introduce SimBank: a simulator designed for accurate benchmarking of PresPM methods, which address the limitations of current literature.
...
Method: introduce SimBank: a simulator designed for accurate benchmarking of PresPM methods

Result: SimBank's benchmarking capabilities through experiments with various PresPM methods across different interventions
...
TL;DR: 本文深入分析了 Softmax Loss (SL) 和 Cosine Contrastive Loss (CCL)， 发现它们分别存在对假阴性敏感和数据利用率低的问题， 因此提出 DrRL 损失函数来解决这些问题。


<details>
  <summary>Details</summary>
Motivation: Softmax Loss (SL) 和 Cosine Contrastive Loss (CCL) 在优化推荐模型中非常有效，但它们的理论联系和差异值得深入探索。

Method: 对Softmax Loss (SL) 和 Cosine Contrastive Loss (CCL) 进行了全面的分析，并提出了一种新的损失函数 DrRL。

Result: SL和CCL都可以看作是传统损失的增强，具有分布鲁棒优化(DRO)，增强了对分布偏移的鲁棒性。SL对假阴性实例表现出高度敏感性，而CCL则存在数据利用率低的问题。

Conclusion: 提出了新的损失函数DrRL，通过在DRO优化中利用R'enyi散度，推广了SL和CCL，并有效缓解了它们的局限性。大量的实验验证了DrRL在推荐准确性和鲁棒性方面的优越性。
...
TL;DR: This paper introduces Next-User Retrieval, a transformer-based framework for enhancing cold-start recommendations by generating potential users who are likely to interact with the item. It outperforms existing methods and demonstrates practical applicability with significant improvements in Douyin.


<details>
  <summary>Details</summary>
Motivation: Modern recommendation systems still struggle to address item cold-start challenges due to the heavy reliance on item and historical interactions, which are non-trivial for cold-start items lacking sufficient exposure and feedback.Traditional lookalike algorithms face such limitations: (1) failing to effectively model the lookalike users and further improve recommendations with the existing rule- or model-based methods; and (2) struggling to utilize the interaction signals and incorporate diverse features in modern recommendation systems.

Method: We employ a transformer-based model to capture the unidirectional relationships among recently interacted users and utilize these sequences to generate the next potential user who is most likely to interact with the item. The additional item features are also integrated as prefix prompt embeddings to assist the next-user generation.

Result: Our method achieves significant improvements with increases of 0.0142% in daily active users and +0.1144% in publications in Douyin.

Conclusion: The Next-User Retrieval method achieves significant improvements in daily active users and publications in Douyin, showcasing its practical applicability and scalability.
...
TL;DR: This survey reviews multi-interest recommendation, addressing its importance, focused aspects, applications, and technical details. It aims to provide a framework for researchers in this field.
...
Method: DiscRec, a novel framework with item-level position embeddings and a dual-branch module to disentangle semantic and collaborative signals, using a gating mechanism for fusion.
...
TL;DR: MedSyn is a human-AI framework using interactive dialogues between doctors and LLMs to improve clinical decisions. Results from simulations are promising, but real-world validation is required.


<details>
  <summary>Details</summary>
Motivation: Clinical decision-making is complex, influenced by biases, incomplete information, and ambiguity. Typical LLM usage in medicine may overlook real-world complexities.

Method: A hybrid human-AI framework, MedSyn, involving multi-step, interactive dialogues between physicians and LLMs to refine diagnoses and treatment decisions.

Result: Simulated physician-LLM interactions show the potential of open-source LLMs as physician assistants.

Conclusion: Open-source LLMs hold promise as physician assistants, but further validation with real physician interactions is needed to confirm MedSyn's usefulness in improving diagnostic accuracy and patient outcomes.
...
Result: In a representative example of graph sparsification with copy constraints, 2D-PT achieves near-ideal mixing, with Kullback-Leibler divergence decaying as O(1/t). When applied to sparsified Wishart instances, 2D-PT yields orders of magnitude speedup over conventional PT with the same number of replicas.
...
TL;DR: NetraAI, a dynamical system-based AI, uses contraction mappings, information geometry, and evolutionary algorithms with an LLM Strategist to identify predictive patient subgroups from small clinical trial datasets, achieving near-perfect classification with a few features.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the need for stable and interpretable AI in clinical discovery, contrasting scale-driven generality with specialized AI systems.

Method: NetraAI, a dynamical system-based framework engineered for stability and interpretability on small clinical trial datasets, combines contraction mappings, information geometry, and evolutionary algorithms to identify predictive patient cohorts. An LLM Strategist acts as a meta-evolutionary layer, observing Persona outputs, prioritizing promising variables, injecting domain knowledge, and assessing robustness.

Result: NetraAI uncovered small, high-effect-size subpopulations that transformed weak baseline models (AUC ~0.50-0.68) into near-perfect classifiers using only a few features in case studies (schizophrenia, depression, pancreatic cancer).

Conclusion: NetraAI, positioned at the intersection of dynamical systems, information geometry, and evolutionary learning, offers a new generation of adaptive, self-reflective AI to accelerate clinical discovery by prioritizing reliable, explainable knowledge.
...
TL;DR: This paper introduces ETS, a framework using EEG and eye-tracking data for open-vocabulary text generation and sentiment classification, achieving better results than existing methods.


<details>
  <summary>Details</summary>
Motivation: Decoding natural language from brain activity using non-invasive EEG is challenging, especially for open-vocabulary scenarios where traditional methods struggle with noise and variability. Previous studies have achieved high accuracy on small-closed vocabularies, but it still struggles on open vocabularies.
...
TL;DR: 提出了一种迁移学习方法来预测上游参数，并在适应变化的数据分布、域扩展和任务更新方面取得了成功。


<details>
  <summary>Details</summary>
Motivation: 从风洞测试、常规飞行和风力涡轮机运行的角度来看，确定上游参数至关重要。传统上，这些参数是通过直接测量来预测的，但在传感器出现故障时可能会带来挑战。或者，可以使用基于表面压力数据的data-driven预测模型来确定这些参数。这种预测器必须实现接近实时的学习，这由实际应用（例如监测风洞运行或了解航空航天和风能系统的空气动力性能变化）决定。

Method: 提出了一种迁移学习方法来预测上游参数，该方法首先离线训练一个卷积神经网络 (ConvNet) 模型用于核心预测任务，然后冻结该模型的权重（除了输出节点之前的选定层），最后通过重新训练这些层来执行迁移学习。

Result: 结果成功地证明了该方法在适应变化的数据分布、域扩展和任务更新方面的潜力，但对于噪声数据的应用效果不佳。

Conclusion: 该方法在适应变化的数据分布、域扩展和任务更新方面具有潜力，但对于噪声数据的应用效果不佳。
...
TL;DR: This paper proposes PIPE, a novel physics-informed positional encoding method for multimodal time series forecasting using visual data, achieving state-of-the-art performance in typhoon intensity forecasting.
...
TL;DR: This paper proposes a deep reinforcement learning approach using a GNN-Transformer model to optimize retrieval operations in multi-deep storage systems, achieving better performance than heuristic methods.


<details>
  <summary>Details</summary>
Motivation: The increasing demand for high-efficiency, high-density storage systems in modern industrial and logistics environments and the challenges of lane blockages in multi-deep autonomous vehicle storage and retrieval systems (AVS/RS) during retrieval operations are the motivation.

Method: A deep reinforcement learning-based framework with a graph-based state representation and a novel neural network architecture combining a Graph Neural Network (GNN) with a Transformer model is proposed.

Result: Extensive numerical experiments demonstrate the superiority of the proposed neural network architecture and the effectiveness of the trained agent in optimizing retrieval tardiness.

Conclusion: The proposed deep reinforcement learning framework with a novel GNN-Transformer architecture outperforms heuristic methods in minimizing retrieval tardiness in multi-deep storage systems, demonstrating strong generalization across diverse layouts.
...
TL;DR: AZT1D是一个包含25名T1D患者的详细数据的公开数据集，可用于推进个性化治疗和改善临床决策。


<details>
  <summary>Details</summary>
Motivation: 高质量的真实世界数据集对于推进1型糖尿病（T1D）管理中的数据驱动方法至关重要，包括个性化治疗设计、数字孪生系统和葡萄糖预测模型。然而，由于缺乏提供详细和全面的患者数据的公开可用数据集，这一领域的进展受到了限制。

Method: 收集了25名使用自动胰岛素输送（AID）系统的T1D患者的数据，包括CGM数据、胰岛素泵和胰岛素给药数据、碳水化合物摄入量以及设备模式（常规、睡眠和运动），每个患者的数据收集时间为6到8周。

Result: AZT1D包含从25名使用自动胰岛素输送（AID）系统的T1D患者收集的数据。数据包括连续血糖监测（CGM）数据、胰岛素泵和胰岛素给药数据、碳水化合物摄入量以及设备模式（常规、睡眠和运动），每个患者的数据收集时间为6到8周。该数据集提供了有关大剂量胰岛素输送的详细信息（即总剂量、大剂量类型、校正特定量），这些特征在现有数据集中很少见。
...
TL;DR: The paper introduces Continuous Evolution Pool (CEP) to retain and utilize conceptual knowledge in time series forecasting with recurring concept drift, enhancing prediction results by storing and retrieving different concept instances.


<details>
  <summary>Details</summary>
Motivation: Recurring concept drift, a type of concept drift in which previously observed data patterns reappear after some time, is one of the most prevalent types of concept drift in time series. As time progresses, concept drift occurs and previously encountered concepts are forgotten, thereby leading to a decline in the accuracy of online predictions. Existing solutions employ parameter updating techniques to delay forgetting; however, this may result in the loss of some previously learned knowledge while neglecting the exploration of knowledge retention mechanisms.

Method: We propose the Continuous Evolution Pool (CEP), a pooling mechanism that stores different instances of forecasters for different concepts. Our method first selects the forecaster nearest to the test sample and then learns the features from its neighboring samples - a process we refer to as the retrieval. If there are insufficient neighboring samples, it indicates that a new concept has emerged, and a new model will evolve from the current nearest sample to the pool to store the knowledge of the concept. Simultaneously, the elimination mechanism will enable outdated knowledge to be cleared to ensure the prediction effect of the forecasters.

Result: Experiments on different architectural models and eight real datasets demonstrate that CEP effectively retains the knowledge of different concepts.

Conclusion: CEP effectively retains the knowledge of different concepts. In the scenario of online forecasting with recurring concepts, CEP significantly enhances the prediction results.
...
TL;DR: Adding a dropout layer during inference for protein language models improves zero-shot performance.
...
TL;DR: develop the new "Assembly-of-Experts" (AoE) construction method to create capable child variants of existing Mixture-of-Experts parent models in linear time
...
Method: develop the new "Assembly-of-Experts" (AoE) construction method to create capable child variants of existing Mixture-of-Experts parent models in linear time. Model weight tensors get interpolated individually, allowing to enhance or suppress semantic features of the parents.
...
TL;DR: 本文研究了智能系统中泛化和识别之间的权衡，发现有限分辨率相似性是一个基本的涌现信息约束，它影响深度网络和大脑的表征能力。


<details>
  <summary>Details</summary>
Motivation: 智能系统必须部署同时结构化（以支持广泛的泛化）和选择性（以保留输入身份）的内部表示。我们揭示了这种权衡的一个基本限制。

Method: 我们推导了闭式表达式，将模型的正确泛化概率 $p_S$ 和识别概率 $p_I$ 固定到与输入空间几何无关的通用帕累托前沿。

Result: 最小的 ReLU 网络端到端训练重现了这些规律：在学习过程中，分辨率边界会自组织，并且经验 $(p_S,p_I)$ 轨迹紧密地遵循线性衰减相似性的理论曲线。

Conclusion: 本文提供了一个关于泛化-识别权衡的精确理论，并阐明了语义分辨率如何影响深度网络和大脑的表征能力。
...
TL;DR: ss-Mamba是一种用于时间序列预测的新型基础模型，它通过结合语义感知嵌入和自适应样条基时间编码，实现了比传统Transformer模型更高的效率和更好的性能。


<details>
  <summary>Details</summary>
Motivation: 为了提高时间序列预测的准确性和效率，并解决传统Transformer模型计算复杂度高的问题。

Method: ss-Mamba采用Mamba选择性状态空间模型，并结合语义感知嵌入和自适应样条基时间编码。

Result: 实验证明，ss-Mamba在准确性、鲁棒性和可解释性方面均优于传统Transformer模型。

Conclusion: ss-Mamba在时间序列预测中表现出卓越的准确性、鲁棒性和可解释性，是传统Transformer模型的有效替代方案。
...
TL;DR: This paper introduces PARC, a framework to analyze VLM prompt sensitivity. It finds VLMs are sensitive to prompt variations, mirroring LLMs, and identifies robust VLMs from the InternVL2 family. Prompt sensitivity may be linked to training data.


<details>
  <summary>Details</summary>
Motivation: VLMs integrate visual modalities with large language models (LLMs), which are well known to be prompt-sensitive. Hence, it is crucial to determine whether VLMs inherit this instability to varying prompts.

Method: a VLM prompt sensitivity analysis framework built on three pillars: (1) plausible prompt variations in both the language and vision domain, (2) a novel model reliability score with built-in guarantees, and (3) a calibration step that enables dataset- and prompt-spanning prompt variation analysis

Result: VLMs mirror LLM language prompt sensitivity in the vision domain, and most destructive variations change the expected answer.

Conclusion: outstandingly robust VLMs among 22 evaluated models come from the InternVL2 family. We further find indications that prompt sensitivity is linked to training data.
...
TL;DR: 提出了一种模型路由器框架，该框架动态选择最合适的预测模型，以优化预测，最终减少缺货和库存积压。


<details>
  <summary>Details</summary>
Motivation: 供应链中稀疏和间歇性的需求预测提出了一个严峻的挑战，因为频繁的零需求周期会影响传统模型的准确性，并影响库存管理。

Method: 我们提出并评估了一种模型路由器框架，该框架动态选择最合适的预测模型（涵盖经典方法、机器学习方法和深度学习方法），并根据每个产品的独特需求模式进行选择。

Result: 通过比较基于规则的路由器、LightGBM 路由器和 InceptionTime 路由器，我们的方法学会了分配适当的预测策略，从而有效地区分平滑、块状或间歇性需求模式，以优化预测。

Conclusion: 通过在 Favorita 数据集上的实验表明，我们的深度学习 (Inception Time) 路由器比强大的单模型基准提高了高达 11.8% (NWRMSLE) 的预测准确率，并且推理速度提高了 4.67 倍。最终，这些预测精度的提高将大大减少缺货和浪费性过剩库存，突显了智能自适应人工智能在优化当代供应链运营中的关键作用。
...
TL;DR: This paper introduces a growable and modular neural network that avoids catastrophic forgetting with linear growth of parameters, achieving greater knowledge transfer and performance.
...
Method: growable and modular neural network architecture
...
Method: TRAINCHECK automatically infers invariants tailored for DL training and uses these invariants to proactively detect silent errors.
...
TL;DR: This paper introduces a p-Laplacian model as an alternative to DXA scans for predicting body composition variables, achieving promising results in data-constrained environments. SVR models also showed strong performance when sufficient data was available.


<details>
  <summary>Details</summary>
Motivation: Accurate prediction of anthropometric body composition variables is essential for early diagnosis of several chronic diseases. DXA scans are costly and time-consuming, and supervised machine learning algorithms are data-intensive, while extracting patients' data in healthcare faces many technical challenges and legal restrictions.

Method: Applying statistical and machine learning models on biomarkers obtained from 3D optical images, specifically a semi-supervised model, the $p$-Laplacian regression model.

Result: The $p$-Laplacian model yielded errors of $\sim13\%$ for ALM, $\sim10\%$ for BMD, and $\sim20\%$ for BFP when the training data accounted for 10 percent of all data. Support Vector Regression (SVR) performed the best for ALM and BMD, yielding errors of $\sim 8\%$ for both, while Least Squares SVR performed the best for BFP with $\sim 11\%$ error when trained on 80 percent of the data.
...
TL;DR: Smaller VLMs are trained with GRPO to use external tools like zoom, improving performance on VQA tasks.
...
Method: Smaller-scale models are trained with Group Relative Policy Optimization (GRPO) to use external tools such as zoom.
...
Conclusion: The proposed method achieves better performance on some visual question-answering (VQA) tasks compared to similarly-sized baseline models, due to the detailed visual information gathered from the external tool.
...
TL;DR: FedNano, a federated learning framework, centralizes the LLM on the server while introducing a lightweight module for client-specific adaptation, reducing client-side storage by 95% and communication overhead to 0.01% of the model parameters.


<details>
  <summary>Details</summary>
Motivation: Multimodal Large Language Models (MLLMs) excel in tasks like multimodal reasoning and cross-modal retrieval but face deployment challenges in real-world scenarios due to distributed multimodal data and strict privacy requirements. Federated Learning (FL) offers a solution by enabling collaborative model training without centralizing data. However, realizing FL for MLLMs presents significant challenges, including high computational demands, limited client capacity, substantial communication costs, and heterogeneous client data. Existing FL methods assume client-side deployment of full models, an assumption that breaks down for large-scale MLLMs due to their massive size and communication demands.

Method: We propose FedNano, the first FL framework that centralizes the LLM on the server while introducing NanoEdge, a lightweight module for client-specific adaptation. NanoEdge employs modality-specific encoders, connectors, and trainable NanoAdapters with low-rank adaptation.

Result: This design eliminates the need to deploy LLM on clients, reducing client-side storage by 95%, and limiting communication overhead to only 0.01% of the model parameters.
...
TL;DR: A BiGRU-MHA model is proposed for SSD health prediction, achieving high accuracy and generalization ability, which can reduce data loss risks and optimize maintenance costs.


<details>
  <summary>Details</summary>
Motivation: SSD health state prediction is critical for data reliability assurance. Traditional models have generalization bottlenecks.

Method: A hybrid BiGRU-MHA model incorporating a multi-head attention mechanism is proposed to enhance the accuracy and stability of storage device health classification. It integrates temporal feature extraction and key information focusing capabilities, leveraging the BiGRU network and multi-head attention mechanism.

Result: The proposed model achieves classification accuracies of 92.70% on the training set and 92.44% on the test set, with a minimal performance gap of 0.26%. The AUC on the test set is 0.94.

Conclusion: The proposed BiGRU-MHA model significantly reduces data loss risks by providing early failure warnings and helps optimize maintenance costs, supporting intelligent decision-making in building reliable storage systems for cloud computing data centers and edge storage environments.
...
TL;DR: This paper introduces CACTUS, a tool for improving AMD stage classification by offering explainability, flexibility, and outperforming standard ML models.


<details>
  <summary>Details</summary>
Motivation: The effectiveness of ML models relies heavily on having large amounts of complete data, but healthcare data is often limited or incomplete, which can hinder model performance. The lack of transparency in some ML models further complicates their understanding and use. Early diagnosis is crucial due to the absence of effective treatments for reversing progression of Age-related Macular Degeneration (AMD).

Method: The -Comprehensive Abstraction and Classification Tool for Uncovering Structures-(CACTUS)

Result: CACTUS offers explainability and flexibility, outperforming standard ML models. It enhances decision-making by identifying key factors and providing confidence in its results.

Conclusion: CACTUS improves AMD stage classification, enhances decision-making by identifying key factors, and allows comparison with existing medical knowledge by eliminating less relevant or biased data.
...
TL;DR: 提出了一种改进的 OC-SVM 核函数，该函数利用负样本来提高小样本单类分类的性能。


<details>
  <summary>Details</summary>
Motivation: 本文重点研究具有一些可用负样本的小规模单类分类。

Method: 广义参考核与负样本 (GRKneg) 用于 One-class 支持向量机 (OC-SVM)

Result: 该方法始终优于使用径向基函数核的标准 OC-SVM。当有大量负样本时，二元 SVM 优于预期中的单类方法，但我们表明，对于最少数量的负样本，所提出的方法明显优于二元 SVM。

Conclusion: 对于少量负样本，该方法优于二元 SVM。
...
TL;DR: This paper introduces an event-driven online VFL framework with dynamic local regret to address challenges in online VFL with non-intersecting feature sets and asynchronous data streams. The framework improves stability and reduces costs.


<details>
  <summary>Details</summary>
Motivation: Integrating online learning into VFL presents challenges due to the unique nature of VFL, where clients possess non-intersecting feature sets and data streams are not synchronous.

Method: An event-driven online VFL framework incorporating dynamic local regret (DLR).

Result: The proposed framework is more stable than existing online VFL frameworks under non-stationary data conditions and significantly reduces communication and computation costs.

Conclusion: The proposed event-driven online VFL framework is more stable, reduces communication and computation costs compared to existing online VFL frameworks.
...
TL;DR: FedOne framework maximizes query efficiency in federated black-box prompt learning by activating only one client per round, addressing the query cost issue of cloud-based LLMs.
...
Result: Degrading FedAvg to activate only one client per round (FedOne) enables optimal query efficiency. Numerical experiments demonstrate a significant improvement in query efficiency.
...
TL;DR: This paper proposes the automatic definition of the separation threshold in Anomaly Detection Systems using some machine learning algorithms, and evaluates three algorithms: the K-Nearst Neighbors, the K-Means and the Support Vector Machine.
...
Method: The K-Nearst Neighbors, the K-Means and the Support Vector Machine.

Result: Three algorithms were evaluated

Conclusion: This work proposes the automatic definition of the separation threshold in Anomaly Detection Systems using some machine learning algorithms.
...
TL;DR: 该论文研究了神经网络损失景观中的通道，沿着这些通道，神经元的权重发散到无穷大，形成门控线性单元。


<details>
  <summary>Details</summary>
Motivation: 神经网络的损失景观包含可能在平坦区域连接或孤立出现的最小值和鞍点。

Method: 确定和描述损失景观中的一种特殊结构：通道，沿着这些通道，损失下降非常缓慢，而至少两个神经元的输出权重 $a_i$ 和 $a_j$ 发散到 $\pm$ 无穷大，并且它们的输入权重向量 $\mathbf{w_i}$ 和 $\mathbf{w_j}$ 彼此相等。

Result: 在收敛时，两个神经元实现了一个门控线性单元：$a_i\sigma(\mathbf{w_i} \cdot \mathbf{x}) + a_j\sigma(\mathbf{w_j} \cdot \mathbf{x}) \rightarrow \sigma(\mathbf{w} \cdot \mathbf{x}) + (\mathbf{v} \cdot \mathbf{x}) \sigma'(\mathbf{w} \cdot \mathbf{x})$。从几何上看，这些通向无穷大的通道与对称性诱导的临界点线渐近平行。

Conclusion: 梯度流求解器和相关优化方法（如SGD或ADAM）在高概率下会到达通道，但如果没有仔细检查，它们看起来像是具有有限参数值的平坦局部最小值。我们的表征从梯度动力学、几何和功能解释方面全面描述了这些准平坦区域。通道末端门控线性单元的出现突出了完全连接层的计算能力的一个惊人方面。
...
TL;DR: Guru, a 92K example RL reasoning corpus, reveals RL's varying effectiveness across reasoning domains. Guru-7B and Guru-32B achieve state-of-the-art performance on a 17-task suite.


<details>
  <summary>Details</summary>
Motivation: Most open efforts focus narrowly on math and code, limiting our understanding of RL's broader applicability to general reasoning. A key challenge lies in the lack of reliable, scalable RL reward signals across diverse reasoning domains.

Method: A curated RL reasoning corpus of 92K verifiable examples spanning six reasoning domains with domain-specific reward design, deduplication, and filtering.

Result: Significant variation across domains: domains frequently seen during pretraining easily benefit from cross-domain RL training, while domains with limited pretraining exposure require in-domain training to achieve meaningful performance gains, suggesting that RL is likely to facilitate genuine skill acquisition.

Conclusion: Guru-7B and Guru-32B achieve state-of-the-art performance among open models RL-trained with publicly available data, outperforming best baselines by 7.9% and 6.7% on a 17-task evaluation suite. The models improve Pass@k performance, especially on complex tasks. Data, models, and code are released to facilitate general-purpose reasoning.
...
TL;DR: The paper introduces Overlap-aware Disagreement Discrepancy (ODD) to improve ML model error estimation in unseen test domains by focusing on non-overlapping regions, achieving better performance than DIS^2.


<details>
  <summary>Details</summary>
Motivation: Reliable and accurate estimation of ML model error in unseen test domains is important. Prior work's disagreement discrepancy (DIS^2) has a problem: its objective competes in the overlapping region between source and target domains.

Method: The paper devises Overlap-aware Disagreement Discrepancy (ODD) which maximizes disagreement only in the non-overlapping target domain. It uses domain-classifiers to estimate domain-overlap.

Result: Experiments on benchmarks show ODD improves performance-estimation error while remaining valid and reliable.

Conclusion: The proposed ODD-based bound improves target performance prediction by focusing on disagreement in the non-overlapping target domain, leading to better overall performance-estimation error while remaining valid and reliable.
...
TL;DR: 本研究利用临床和数字数据，通过foundation models和多模态方法，提高了早期多发性硬化症残疾进展的预测准确性。
...
Method: 使用了tabular和时间序列foundation models, 以及一个多模态transformer。

Result: 集成了数字数据后，模型性能优于仅使用临床数据。使用Moment FM的unimodal embeddings的transformer模型效果最佳, 并且多模态transformer始终优于unimodal counterpart。

Conclusion: 结合临床和数字数据的多模态方法可以更准确地预测MS和其他复杂疾病。
...
TL;DR: propose a multi-agent multi-armed bandit (MA-MAB) framework aimed at ensuring fair outcomes across agents while maximizing overall system performance.


<details>
  <summary>Details</summary>
Motivation: ensuring fair outcomes across agents while maximizing overall system performance. A key challenge in this setting is decision-making under limited information about arm rewards.

Method: introduce a novel probing framework that strategically gathers information about selected arms before allocation. In the offline setting, where reward distributions are known, leverage submodular properties to design a greedy probing algorithm with a provable performance bound. For the more complex online setting, develop an algorithm that achieves sublinear regret while maintaining fairness.

Result: the algorithm achieves sublinear regret while maintaining fairness

Conclusion: The proposed approach outperforms baseline methods, achieving better fairness and efficiency.
...
TL;DR: 本文提出使用稳定的受控微分方程（CDE）状态表征，通过训练稳定和执行感知敏锐度表征，实现有效的 sepsis 治疗强化学习。


<details>
  <summary>Details</summary>
Motivation: 在 sepsis 治疗中，强化学习 (RL) 的有效性取决于从不规则 ICU 时间序列中学习稳定的、临床上有意义的状态表征。以往的研究忽略了序列表征中训练不稳定性的关键挑战及其对策略性能的有害影响。

Method: 使用受控微分方程（CDE）进行状态表征，并通过与临床评分的相关性正则化来实施敏锐度感知表征。

Result: 在 MIMIC-III sepsis 队列上的实验表明，稳定的 CDE 自编码器产生的表征与严重程度评分密切相关，并能够实现具有卓越性能的强化学习策略。相反，不稳定的 CDE 表征会导致表征退化和策略失败。

Conclusion: 稳定的CDE能够产生与疾病严重程度相关的表征，并实现卓越的强化学习策略，而不稳定的CDE表征会导致性能下降。
...
TL;DR: SFT-GO根据token的重要性对token进行分组，并使用最差组损失和标准交叉熵损失的加权组合来优化LLM。


<details>
  <summary>Details</summary>
Motivation: 现有的SFT方法通常将每个训练实例视为一个统一的序列，对所有token给予同等的重要性，而忽略了只有一部分token包含关键的、特定于任务的信息。

Method: 引入了带组优化（SFT-GO）的监督微调，这是一种新颖的方法，可以根据token的重要性不同地对待token组。

Result: SFT-GO的收敛速度的理论分析，证明了其效率。在经验上，SFT-GO与三种不同的token分组策略一起应用，并表明使用SFT-GO训练的模型在流行的LLM基准测试中始终优于基线方法。

Conclusion: SFT-GO在各种数据集和基础模型上始终优于基线方法，证明了其稳健性和有效性。
...
TL;DR: This paper analyzes the impact of vocabulary size on LLM training, introduces the Large Vocab (LV) regime, and proposes a new scaling rule for learning rates, validated through experiments.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of μP parametrization when applied to LLMs due to the fixed input dimension assumption and conflicting empirical observations.

Method: Theoretical analysis of training dynamics considering vocabulary size, supported by empirical experiments.

Result: The analysis reveals the LV regime where optimal embedding LR to hidden LR ratio scales as Θ(√width), validated by experiments and pretraining a 1B model.

Conclusion: This paper provides a theoretical analysis of the effect of vocabulary size on training dynamics, reveals the Large Vocab (LV) regime, and validates the theory by pretraining a 1B model.
...
TL;DR: propose Sequential Policy Gradient modeling (SPG) for lightweight online hyperparameter optimization, which achieves performance gains of +0.2~7% with low computational costs.


<details>
  <summary>Details</summary>
Motivation: conventional approaches impede widespread use due to prohibitive time and computational costs in neural architecture search and hyperparameter optimization.

Method: propose Sequential Policy Gradient modeling (SPG), a novel trajectory generation paradigm for lightweight online hyperparameter optimization. In contrast to conventional policy gradient methods, SPG extends the base model with temporary modules, enabling it to generate state-action (padded) trajectories in a single forward pass.

Result: models gain performance when retrained with SPG on their original datasets and also outperform standard transfer fine-tuning. evaluate on five datasets spanning computer vision (ImageNet, COCO), natural language processing (GLUE, SQuAD), and audio (SUPERB).

Conclusion: The proposed SPG method demonstrates consistent performance improvements across widely adopted models, achieving performance gains of +0.2~7%, with significantly low computational costs.
...
TL;DR: This paper provides a theoretical analysis of Muon by placing it within the Lion-K family of optimizers, showing that Muon corresponds to Lion-K when equipped with the nuclear norm.


<details>
  <summary>Details</summary>
Motivation: The pursuit of faster optimization algorithms remains an active and important research direction in deep learning. Recently, the Muon optimizer has demonstrated promising empirical performance, but its theoretical foundation remains less understood. In this paper, we bridge this gap and provide a theoretical analysis of Muon by placing it within the Lion-K family of optimizers

Method: We show that Muon corresponds to Lion-K when equipped with the nuclear norm, and we leverage the theoretical results of Lion-K

Result: we establish that Muon (with decoupled weight decay) implicitly solves an optimization problem that enforces a constraint on the spectral norm of weight matrices

Conclusion: Muon (with decoupled weight decay) implicitly solves an optimization problem that enforces a constraint on the spectral norm of weight matrices. This perspective not only demystifies the implicit regularization effects of Muon but also leads to natural generalizations through varying the choice of convex map K, allowing for the exploration of a broader class of implicitly regularized and constrained optimization algorithms.
...
TL;DR: This paper introduces HiPreNets, a progressive framework for training high-precision neural networks. It addresses challenges in training complex models by refining a staged training technique and focusing on L^\infty error.
...
Result: improves an existing fully connected neural network by sequentially learning its prediction residuals using additional networks, leading to improved overall accuracy
...
TL;DR: LLM具身代理存在幻觉问题，尤其是在场景与任务不一致时。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型越来越多地被用作具身代理的认知核心。然而，源于未能将用户指令植根于观察到的物理环境中的遗传性幻觉，可能导致导航错误，例如搜索不存在的冰箱。

Method: 通过构建一个幻觉探测集，并在两个模拟环境中评估了12个模型。

Result: 模型表现出推理能力，但未能解决场景-任务不一致性，并且幻觉率比基本提示高出40倍。

Conclusion: LLMs在处理不切实际的任务时存在根本性局限，无法解决场景-任务不一致性。
...
TL;DR: NCPF 结合了低秩张量代数与深度表征学习，用于鲁棒的交通数据插补。


<details>
  <summary>Details</summary>
Motivation: 现代智能交通系统依赖于精确的时空交通分析来优化城市交通和基础设施弹性。然而，传感器故障和异构传感间隙导致普遍存在的数据缺失，从根本上阻碍了可靠的交通建模。

Method: 该模型通过可学习的嵌入投影将 CP 分解创新地嵌入到神经架构中，其中稀疏交通张量被编码为跨路段、时间间隔和移动指标的密集潜在因子。分层特征融合机制采用 Hadamard 积来显式地建模多线性交互，而堆叠的多层感知器层非线性地细化这些表示以捕获复杂的时空耦合。

Result: 在六个城市交通数据集上的广泛评估表明，NCPF 优于六个最先进的基线。

Conclusion: NCPF 通过结合 CP 分解的可解释因子分析和神经网络的非线性表达能力，为高维交通数据插补提供了一种原则性但灵活的方法，为下一代交通数字孪生和自适应交通控制系统提供关键支持。
...
TL;DR: This paper surveys machine unlearning verification methods, proposes a taxonomy, and identifies open problems.


<details>
  <summary>Details</summary>
Motivation: With growing demands for privacy protection, security, and legal compliance (e.g., GDPR), machine unlearning has emerged as a critical technique for ensuring the controllability and regulatory alignment of machine learning models. However, a fundamental challenge in this field lies in effectively verifying whether unlearning operations have been successfully and thoroughly executed. Despite a growing body of work on unlearning techniques, verification methodologies remain comparatively underexplored and often fragmented. Existing approaches lack a unified taxonomy and a systematic framework for evaluation.

Method: We propose a taxonomy that organizes current techniques into two principal categories -- behavioral verification and parametric verification -- based on the type of evidence used to assess unlearning fidelity. We examine representative methods within each category, analyze their underlying assumptions, strengths, and limitations, and identify potential vulnerabilities in practical deployment.

Result: This paper presents the first structured survey of machine unlearning verification methods.

Conclusion: We articulate a set of open problems in current verification research, aiming to provide a foundation for developing more robust, efficient, and theoretically grounded unlearning verification mechanisms.
...
TL;DR: ImprovDML: a decentralized DML framework that achieves high model accuracy while simultaneously ensuring privacy preservation and resilience to Byzantine attacks.


<details>
  <summary>Details</summary>
Motivation: Jointly addressing Byzantine attacks and privacy leakage in distributed machine learning (DML) has become an important issue. However, the incorporation of these techniques often results in a significant degradation in model accuracy.

Method: The framework leverages a kind of resilient vector consensus algorithms and multivariate Gaussian noises are introduced to the gradients for privacy preservation.

Result: The paper provides convergence guarantees and derive asymptotic learning error bounds under non-convex settings, which are tighter than those reported in existing works. It adopts the notion of concentrated geo-privacy, which quantifies privacy preservation based on the Euclidean distance between inputs, and demonstrates that it enables an improved trade-off between privacy preservation and model accuracy compared to differential privacy.

Conclusion: The paper demonstrates through numerical simulations that ImprovDML achieves high model accuracy while simultaneously ensuring privacy preservation and resilience to Byzantine attacks.
...
TL;DR: 提出了一种新的行为理解方法，通过模仿学习和强化学习相结合，能够识别可重用的技能组件，并生成逼真的动物行为轨迹。


<details>
  <summary>Details</summary>
Motivation: 现有的行为分割方法通过在限制性生成假设下强加离散音节，从而过度简化了动物灵活地重组有限的核心运动原语以满足各种任务需求的过程。

Method: 一种基于强化学习的模仿框架，通过利用过渡概率的表征学习来推断可解释的技能集，并将策略参数化为这些技能的动态混合。

Result: 该方法识别可重用的技能组件，学习连续演化的组合策略，并生成超出传统离散模型能力的逼真轨迹。

Conclusion: 该方法通过组合表示进行生成行为建模，简洁地解释了复杂动物行为如何从基本运动原语的动态组合中产生。
...
TL;DR: This work quantifies the accuracy of applying different ML techniques to the 1D Poisson differential equation, proves generalization bounds and convergence rates, and demonstrates a new mechanistic interpretability lens on scientific models.
...
Method: Applying different ML techniques to the elementary 1D Poisson differential equation, analyzing the training dynamics and deriving optimal parameters for both a white-box differential equation discovery method and a black-box linear model.

Result: The function space of the data is critical to the generalization of the model. Generalization bounds and convergence rates under finite data discretizations and restricted training data subspaces are proven. Analytically derived generalization bounds are replicated empirically. Similar lack of generalization is empirically demonstrated for deep linear models, shallow neural networks, and physics-specific DeepONets and Neural Operators.

Conclusion: Generalization to the true physical equation is not guaranteed in each explored case. Different classes of models can exhibit opposing generalization behaviors.  Green's function representations can be extracted from the weights of black-box models.  A new cross-validation technique for measuring generalization in physical systems is proposed.
...
TL;DR: SoKA是一种新的PEFT策略，它结合了Kronecker积张量分解与SVD驱动的初始化和频谱感知动态秩选择，以实现高效的大规模模型适配。


<details>
  <summary>Details</summary>
Motivation: 全量微调需要大量的存储、内存和计算开销。参数高效微调(PEFT)方法通过只学习一小部分特定于任务的参数来降低这些成本，但现有方法要么引入推理时延迟(adapter模块)，要么遭受次优收敛(随机初始化的低秩更新)，要么依赖于可能与任务复杂性不匹配的固定秩选择(基于Kronecker的分解)。

Method: 结合了Kronecker积张量分解与SVD驱动的初始化和频谱感知动态秩选择的PEFT策略

Result: SoKA需要的可训练参数更少，同时达到或超过基线性能，收敛速度更快，梯度更稳定。

Conclusion: SoKA在算术推理（GSM8K）、形式数学（MATH）和代码生成（MBPP）方面，仅需0.99M可训练参数，比LoRA/PiSSA少25%，同时达到或超过基线性能。此外，SoKA表现出更快的收敛速度和更稳定的梯度，突出了其在大规模模型适配方面的稳健性和效率。
...
TL;DR: This paper studies the robustness of federated learning to Byzantine behavior, providing lower and upper bounds on centroid approximation and a new algorithm.


<details>
  <summary>Details</summary>
Motivation: The paper investigates whether federated learning is robust to Byzantine behavior and observes a tradeoff between the average/centroid and the validity conditions from distributed computing.

Method: The paper provides a new analysis for the case $n<d$ and presents a new algorithm.

Result: The main result is the first lower bound of $\min\{\frac{n-t}{t},\sqrt{d}\}$ on the centroid approximation under box validity. The paper also provides an upper bound of $2\min\{n,\sqrt{d}\}$.

Conclusion: This paper shows bounds on centroid approximation under box validity and convex validity, and provides a new algorithm that achieves a $\sqrt{2d}$-approximation under convex validity.
...
TL;DR: 该研究提出了一种通过合成生成所需的保留集来解决数据集推断 (DI) 挑战的方法，该方法在不同的文本数据集上进行了广泛的实验，结果表明该方法能够以高置信度检测原始训练集，同时保持低假阳性率。


<details>
  <summary>Details</summary>
Motivation: 现有数据集推断 (DI) 方法需要一个私有集（已知在训练中不存在），该私有集与受损数据集的分布紧密匹配。但在实践中，这种同分布的、保留的数据很少可用，严重限制了 DI 的适用性。

Method: 该研究通过在一个精心设计的基于后缀的完成任务上训练的数据生成器，创建高质量、多样化的合成数据，准确地反映原始分布；并通过事后校准弥合真实数据和合成数据之间的可能性差距。

Result: 在不同的文本数据集上进行的大量实验表明，使用他们生成的数据作为保留集，能够使数据集推断以高置信度检测原始训练集，同时保持低假阳性率。

Conclusion: 该研究表明，使用他们生成的数据作为保留集，能够使数据集推断以高置信度检测原始训练集，同时保持低假阳性率。这使得版权所有者能够对数据使用提出合法的主张，并证明了他们的方法在实际诉讼中的可靠性。
...
TL;DR: DOVA-PATBM increases coverage, halves travel distance for low-income residents, and meets sub-transmission headroom while remaining computationally tractable for national-scale roll-outs.
...
TL;DR: This paper proposes an advanced credit risk management framework for 3PL-led SCF in CBEC, using generative AI to improve credit practices and SME access to capital.


<details>
  <summary>Details</summary>
Motivation: Financing remains a critical challenge for SMEs in cross-border e-commerce (CBEC) due to their limited credit histories. Third-party logistics (3PL)-led supply chain finance (SCF) has emerged as a promising solution, leveraging in-transit inventory as collateral.

Method: We leverage conditional generative modeling of sales distributions through Quantile-Regression-based Generative Metamodeling (QRGMM) as the foundation for risk estimation. To capture complex covariate interactions in e-commerce sales data, we integrate QRGMM with Deep Factorization Machines (DeepFM).

Result: Extensive experiments on synthetic and real-world data validate the efficacy of our model for credit risk assessment and loan size determination.

Conclusion: This study pioneers the application of generative AI in CBEC SCF risk management, offering a solid foundation for enhanced credit practices and improved SME access to capital.
...
TL;DR: SecFwT is the first MPC-based framework designed for efficient, privacy-preserving LLM fine-tuning.


<details>
  <summary>Details</summary>
Motivation: adaptation to specialized tasks in privacy-sensitive domains is constrained by the scarcity of accessible training data due to stringent privacy requirements and  fine-tuning introduces significant computational challenges, particularly in privacy-preserving backward propagation and optimizer operations, the inefficiency of softmax-based attention mechanisms in MPC settings

Method: SecFwT introduces a forward-only tuning paradigm to eliminate backward and optimizer computations and employs MPC-friendly Random Feature Attention to approximate softmax attention
...
TL;DR: 提出了一种用于多靶点药物发现的结构化主动学习 (AL) 范例，该范例有效地生成了一组结构多样的泛抑制剂候选物。


<details>
  <summary>Details</summary>
Motivation: 同时优化针对多个治疗靶标的分子仍然是药物发现中的一个严峻挑战，特别是因为稀疏的奖励和冲突的设计约束。

Method: 一种结构化的主动学习 (AL) 范例，该范例将序列到序列 (Seq2Seq) 变分自动编码器 (VAE) 集成到迭代循环中，旨在平衡化学多样性、分子质量和多靶点亲和力。

Result: 在针对三种相关冠状病毒主要蛋白酶 (SARS-CoV-2、SARS-CoV、MERS-CoV) 的概念验证研究中，我们的方法有效地生成了一组结构多样的泛抑制剂候选物。
...
TL;DR: 本文研究了上下文学习（ICL）在演示具有缺失或不正确的标签的情况下的有效性，并提出了循环现成的表格基础模型以增强其半监督能力。


<details>
  <summary>Details</summary>
Motivation: 最近的研究表明，即使演示具有缺失或不正确的标签，上下文学习（ICL）也可能有效。为了阐明这种能力，我们研究了一个规范的设置，其中演示是根据二元高斯混合模型（GMM）绘制的，并且一定比例的演示具有缺失的标签。

Method: 我们提供了一个全面的理论研究，以表明：（1）单层线性注意力模型的损失面恢复了最优的完全监督估计器，但完全无法利用未标记数据；（2）相比之下

Result: 在真实世界数据集上的广泛评估表明，我们的方法显着提高了标准单次传递推理的半监督表格学习性能。
...
TL;DR: Predicting unprescribed tests based on performed tests is possible, especially with CBC. A set translation problem is used to deal with missing values, achieving up to 8% AUC improvement for joint predictions.
...
Method: A tabular modeling problem is formulated as a set translation problem where the source set comprises pairs of GPT-like label column embedding and its corresponding value while the target set consists of the same type embeddings only.
...
Conclusion: The method achieves an improvement up to 8% AUC for joint predictions of high uric acid, glucose, cholesterol, and low ferritin levels.
...
TL;DR: 提出了一种新颖的知识蒸馏框架，该框架利用非微调的现成预训练 NNP 作为教师，与现有 NNP 生成方法相比，在不牺牲精度的情况下，减少了 10 倍的昂贵 DFT 计算。


<details>
  <summary>Details</summary>
Motivation: 传统的知识蒸馏 (KD) 方法微调预训练的 NNP 作为教师模型，以生成学生模型的训练数据。然而，在材料特定模型中，这种微调过程会增加能量壁垒，从而难以创建包含高能量结构的训练数据。

Method: 利用非微调的、现成的预训练 NNP 作为教师，提出了一个新的 KD 框架。该框架采用两阶段训练过程：首先，学生 NNP 使用由现成的教师生成的数据集进行训练；然后，使用较小的、高精度的密度泛函理论 (DFT) 数据集对其进行微调。

Result: 将该框架应用于有机（聚乙二醇）和无机（L$_{10}$GeP$_{2}$S$_{12}$）材料，在重现物理性质方面实现了与现有方法相当或更优越的精度。重要的是，该方法与现有的 NNP 生成方法相比，在不牺牲精度的情况下，减少了 10 倍的昂贵 DFT 计算。

Conclusion: 该方法在重现物理性质方面实现了与现有方法相当或更优越的精度。重要的是，该方法与现有的 NNP 生成方法相比，在不牺牲精度的情况下，减少了 10 倍的昂贵 DFT 计算。
...
TL;DR: This paper introduces a conditional Image-to-Image Schr"odinger Bridge (c$\\text{I}^2\\text{SB}$) framework for acoustic FWI, which addresses limitations of diffusion models and outperforms existing methods in reconstructing velocity models.


<details>
  <summary>Details</summary>
Motivation: Diffusion models in acoustic Full Waveform Inversion (FWI) have limitations in iterative sampling, output control, and incorporating approximate velocity models.

Method: Employs a Schr"odinger Bridge that interpolates between the distributions of ground truth and smoothed velocity models and extends the concept of Image-to-Image Schr"odinger Bridge to conditional sampling, resulting in a conditional Image-to-Image Schr"odinger Bridge (c$\\text{I}^2\\text{SB}$) framework.

Result: The proposed solution outperforms a reimplementation of conditional diffusion model and achieves sample fidelity superior to that attained with a supervised learning-based approach.

Conclusion: The proposed c$\\text{I}^2\\text{SB}$ framework outperforms reimplemented conditional diffusion model and supervised learning-based approach in reconstructing the reference velocity model, requiring only a few neural function evaluations to achieve superior sample fidelity.
...
TL;DR: Improved privacy auditing for DP-SGD in black-box settings using quantile regression for stronger membership inference, leading to tighter privacy bounds with maintained efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing one-run auditing approaches in black-box settings have a large gap between empirical lower bounds and theoretical upper bounds on privacy guarantees of DP mechanisms like DP-SGD.
...
Result: Achieves tighter privacy bounds on image classification models trained on CIFAR-10 with DP-SGD.

Conclusion: Utilizing quantile regression for membership inference attacks achieves tighter privacy bounds in black-box differential privacy auditing while maintaining computational efficiency.
...
TL;DR: DiTMC adapts Diffusion Transformers for molecular conformer generation, achieving state-of-the-art results by addressing challenges in integrating graph information, handling symmetries, and generalizing across different molecule sizes.


<details>
  <summary>Details</summary>
Motivation: Applying Diffusion Transformers (DiTs) to molecules introduces challenges in integrating discrete molecular graph information with continuous 3D geometry, handling Euclidean symmetries, and designing conditioning mechanisms for varying molecule sizes and structures.

Method: Adapts Diffusion Transformers (DiTs) to molecular conformer generation through a modular architecture separating 3D coordinate processing from atomic connectivity conditioning, using two graph-based conditioning strategies and different attention mechanisms (non-equivariant and SO(3)-equivariant).

Result: DiTMC achieves state-of-the-art precision and physical validity on GEOM-QM9, -DRUGS, -XL benchmarks.

Conclusion: DiTMC achieves state-of-the-art precision and physical validity on standard conformer generation benchmarks, highlighting the impact of architectural choices and symmetry priors on sample quality and efficiency.
...
TL;DR: This paper introduces a new method for learning ground metrics for optimal transport, which improves performance in various tasks by addressing the limitations of existing methods. The effectiveness of our approach is demonstrated using patient-level scRNA-seq data spanning multiple diseases.


<details>
  <summary>Details</summary>
Motivation: Predefined metrics typically cannot account for the inherent structure and varying importance of different features in the data, and existing supervised approaches to ground metric learning often do not generalize across multiple classes or are restricted to distributions with shared supports.

Method: We propose a novel approach for learning metrics for arbitrary distributions over a shared metric space. Our method provides a distance between individual points like a global metric, but requires only class labels on a distribution-level for training.

Result: The learned global ground metric enables more accurate optimal transport distances, leading to improved performance in embedding, clustering and classification tasks.

Conclusion: The learned global ground metric enables more accurate optimal transport distances, leading to improved performance in embedding, clustering and classification tasks. We demonstrate the effectiveness and interpretability of our approach using patient-level scRNA-seq data spanning multiple diseases.
...
### [194] [Unifying VXAI: A Systematic Review and Framework for the Evaluation of Explainable AI](https://arxiv.org/abs/2506.15408)
*David Dembinsky,Adriano Lucieri,Stanislav Frolov,Hiba Najjar,Ko Watanabe,Andreas Dengel*

Main category: cs.LG

TL;DR: 本文提出了一个用于评估可解释人工智能（XAI）的统一框架（VXAI），旨在解决该领域缺乏标准化评估协议和共识的问题。


<details>
  <summary>Details</summary>
Motivation: 现代人工智能系统经常依赖不透明的黑盒模型，最著名的是深度神经网络，其性能源于具有数百万个学习参数的复杂架构。虽然功能强大，但它们的复杂性对可信度构成了重大挑战，特别是由于缺乏透明度。可解释人工智能（XAI）通过提供人类可理解的模型行为解释来解决这个问题。然而，为了确保其有用性和可信度，必须严格评估这些解释。尽管XAI方法的数量不断增加，但该领域缺乏标准化的评估协议和对适当指标的共识。

Method: 对文献进行系统回顾，遵循系统评价和元分析的首选报告项目（PRISMA）指南，并引入了用于评估XAI（VXAI）的统一框架。

Result: 识别出362篇相关出版物，并将其贡献汇总为41个功能相似的指标组。此外，我们提出了一个三维分类方案，涵盖了解释类型、评估背景和解释质量的要求。

Conclusion: 该框架为VXAI提供了最全面和结构化的概述。它支持系统化的指标选择，提高跨方法的可比性，并为未来的扩展提供了灵活的基础。
...
TL;DR: Introduces PRISM, a novel framework that captures the inherent complexity of neural network features by providing more nuanced descriptions for both polysemantic and monosemantic features.


<details>
  <summary>Details</summary>
Motivation: Current feature description methods face two critical challenges: limited robustness and the flawed assumption that each neuron encodes only a single concept (monosemanticity), despite growing evidence that neurons are often polysemantic. This assumption restricts the expressiveness of feature descriptions and limits their ability to capture the full range of behaviors encoded in model internals.

Method: Polysemantic FeatuRe Identification and Scoring Method (PRISM)

Result: PRISM provides more nuanced descriptions for both polysemantic and monosemantic features. We apply PRISM to language models and, through extensive benchmarking against existing methods, demonstrate that our approach produces more accurate and faithful feature descriptions
...
TL;DR: 该研究提出了一种利用生成模型进行探索的新框架，通过最大化数据流形上的熵来实现，并通过实验验证了其有效性。
...
Method: 该研究提出了一个新颖的框架，该框架将探索视为在预训练扩散模型隐式定义的近似数据流形上的熵最大化。然后，该研究提出了一种基于密度估计的探索新原理，并开发了一种基于镜像下降的算法，该算法通过顺序微调预训练的扩散模型来解决探索问题。

Result: 该研究证明了在现实假设下，该方法收敛到最佳探索性扩散模型。

Conclusion: 该研究在合成的和高维的文本到图像扩散上评估了该方法，并展示了有希望的结果。
...
### [197] [Reward Models in Deep Reinforcement Learning: A Survey](https://arxiv.org/abs/2506.15421)
*Rui Yu,Shenghua Wan,Yucen Wang,Chen-Xiao Gao,Le Gan,Zongzhang Zhang,De-Chuan Zhan*

Main category: cs.LG

TL;DR: This survey reviews reward modeling techniques in deep reinforcement learning, categorizing approaches and discussing applications, evaluations, and future directions.


<details>
  <summary>Details</summary>
Motivation: Significant attention has focused on developing reward models that not only align closely with the true objectives but also facilitate policy optimization.

Method: This survey provides a comprehensive review of reward modeling techniques within the deep RL literature. It categorizes approaches based on the source, the mechanism, and the learning paradigm.
...
TL;DR: 即使是经过对齐的LLM，在微调后也可能变得不安全。LoX通过外推安全子空间来提高LLM的安全性，且无需训练，实验表明其能有效防御良性和恶意微调攻击。


<details>
  <summary>Details</summary>
Motivation: 对齐的模型仍然可能因后续微调而降低其安全保护能力，即使额外的训练数据看起来是良性的。这种脆弱性源于LLM参数中安全关键的低秩子空间对微调的敏感性。

Method: 提出了一种名为低秩外推（LoX）的免训练方法，通过外推对齐的LLM的安全子空间来增强安全鲁棒性。

Result: LoX在面对良性和恶意微调攻击时，攻击成功率（ASR）绝对降低了11%到54%。

Conclusion: LoX通过将LLM参数移动到更平坦的区域来提高安全性，从而降低了对扰动的敏感性。
...
TL;DR: This paper tackles the problem of minimizing disease extinction time in an SIS model on unknown graphs by learning the graph structure and determining effective vaccination strategies. A novel learning algorithm and an optimal SRM algorithm for bounded treewidth graphs are proposed, along with a polynomial-time greedy heuristic.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the problem of minimizing the extinction time of an outbreak modeled by an SIS model where the graph on which the disease spreads is unknown and only the infection states of the vertices are observed.

Method: The authors propose a novel inclusion-exclusion-based learning algorithm and detail an optimal algorithm for the Spectral Radius Minimization (SRM) problem, complemented by a polynomial-time greedy heuristic.

Result: The paper establishes the sample complexity for graph recovery and proves that the running time of the optimal algorithm for the SRM problem is polynomial in the number of vertices for graphs with bounded treewidth.

Conclusion: The paper presents experiments on synthetic and real-world data that numerically validate the learning and vaccination algorithms.
...
TL;DR: AutoRule is introduced, a fully automated method for extracting rules from preference feedback and formulating them into rule-based rewards.  It achieves improvements in length-controlled win rate and second-turn performance.
...
Method: AutoRule, a fully automated method for extracting rules from preference feedback and formulating them into rule-based rewards. It leverages a reasoning model to interpret user preferences, identifies candidate rules from the reasoning chain of these interpretations, and synthesizes them into a unified rule set. Then language-model verifiers are used to compute the fraction of rules satisfied by each output, using this metric as an auxiliary reward.

Result: The extracted rules exhibit good agreement with dataset preference. AutoRule demonstrates reduced reward hacking compared to a learned reward model. The extracted rules capture unique qualities valued in different datasets.

Conclusion: Training a Llama-3-8B model with AutoRule results in a 28.6\% relative improvement and a 6.1\% relative gain compared to a GRPO baseline. The extracted rules exhibit good agreement with dataset preference and demonstrates reduced reward hacking compared to a learned reward model.
...
TL;DR: memory-based architectures improve performance in zero-shot RL with partial observability


<details>
  <summary>Details</summary>
Motivation: explore how the performance of standard zero-shot RL methods degrades when subjected to partially observability

Method: evaluate memory-based zero-shot RL methods in domains where the states, rewards and a change in dynamics are partially observed

Result: improved performance over memory-free baselines

Conclusion: memory-based architectures are an effective remedy for partially observability in zero-shot RL methods
...
### [202] [Warping and Matching Subsequences Between Time Series](https://arxiv.org/abs/2506.15452)
*Simiao Lin,Wannes Meert,Pieter Robberechts,Hendrik Blockeel*

Main category: cs.LG

TL;DR: 提出了一种新的时间序列比较技术，通过简化弯曲路径来增强可解释性，突出显示关键转换。


<details>
  <summary>Details</summary>
Motivation: 传统的可视化侧重于点对点对齐，不能传达子序列层面上更广泛的结构关系。这种局限性使得理解一个时间序列相对于另一个时间序列如何以及在何处移动、加速或减速变得困难。

Method: 提出了一种新技术，可以简化弯曲路径，以突出、量化和可视化关键转换（移位、压缩、幅度差异）。

Result: 该方法能够突出、量化和可视化关键转换（移位、压缩、幅度差异）。

Conclusion: 该方法通过更清晰地表示时间序列之间的子序列匹配方式，增强了时间序列比较的可解释性。
...
TL;DR: Dense latents in SAEs are not training noise but serve functional roles in language model computation.
...
Result: demonstrate that dense latents tend to form antipodal pairs that reconstruct specific directions in the residual stream, introduce a taxonomy of dense latents, identifying classes tied to position tracking, context binding, entropy regulation, letter-specific output signals, part-of-speech, and principal component reconstruction, analyze how these features evolve across layers, revealing a shift from structural features in early layers, to semantic features in mid layers, and finally to output-oriented signals in the last layers of the model.
...
TL;DR: RHO: Robust Homophily Learning, to adaptively learn such homophily patterns for semi-supervised graph anomaly detection
...
TL;DR: Introduces a user-guided dimensionality reduction framework using MLLMs and natural language prompts for customizable and interpretable data visualizations.


<details>
  <summary>Details</summary>
Motivation: Current DR techniques are not designed to explore semantic structure that is not directly available in the form of variables or class labels.

Method: The method uses natural-language guiding prompts to steer projections dynamically, specifying high-level semantic relationships of interest.

Result: The method enhances cluster separation and transforms DR into an interactive, user-driven process.The approach bridges the gap between fully automated DR techniques and human-centered data exploration.

Conclusion: This paper introduces a user-guided projection framework for image and text data, enabling customizable and interpretable data visualizations via zero-shot classification with MLLMs. The method enhances cluster separation and transforms DR into an interactive, user-driven process.
...
TL;DR: 提出LIT-LVM模型，用于更准确地估计交互项系数，优于现有模型，并提供特征的可视化分析。


<details>
  <summary>Details</summary>
Motivation: 统计和机器学习中最简单但最常用的预测器之一是使用特征的加权线性组合。这种线性预测器可以通过添加对应于所有特征对乘积的交互项来模拟特征之间的非线性关系。考虑了准确估计线性预测器中交互项系数的问题。

Method: 提出了一种结构化的正则化方法，通过在低维空间中用潜在向量表示每个特征，从而进一步缓解高维设置中的过度拟合。

Result: LIT-LVM实现了比弹性网络和分解机更好的预测精度，并为特征提供了低维潜在表示。

Conclusion: LIT-LVM在各种模拟和真实数据上，特别是在交互项数量相对于样本数量较高时，实现了比弹性网络和分解机更好的预测精度。LIT-LVM还为特征提供了低维潜在表示，可用于可视化和分析它们的关系。
...
TL;DR: introduce the first certification framework that guarantees pixel-level robustness for any black-box attribution method using randomized smoothing


<details>
  <summary>Details</summary>
Motivation: post-hoc attribution methods are highly non-robust: small, imperceptible input perturbations can drastically alter the attribution map while maintaining the same prediction. This vulnerability undermines their trustworthiness and calls for rigorous robustness guarantees of pixel-level attribution scores

Method: certification framework that guarantees pixel-level robustness for any black-box attribution method using randomized smoothing. By sparsifying and smoothing attribution maps, we reformulate the task as a segmentation problem and certify each pixel's importance against $\ell_2$-bounded perturbations

Result: evaluation of 12 attribution methods across 5 ImageNet models shows that our certified attributions are robust, interpretable, and faithful

Conclusion: certified attributions are robust, interpretable, and faithful, enabling reliable use in downstream tasks
...
TL;DR: This paper studies over-squashing in Spatiotemporal GNNs (STGNNs), proves that architectures that follow either time-and-space or time-then-space processing paradigms are equally affected by this phenomenon, and validates the findings on synthetic and real-world datasets.
...
Result: The paper reveals that counterintuitively, convolutional STGNNs favor information propagation from points temporally distant rather than close in time. Moreover, the paper proves that architectures that follow either time-and-space or time-then-space processing paradigms are equally affected by this phenomenon, providing theoretical justification for computationally efficient implementations.

Conclusion: This paper validates the findings on synthetic and real-world datasets, providing deeper insights into their operational dynamics and principled guidance for more effective designs.
...
Result: We highlight key trends in adversarial attacks on tabular data.
...
TL;DR: RePCS 是一种新的诊断方法，用于检测 RAG 系统中的污染，优于以前的方法，并且计算成本低。


<details>
  <summary>Details</summary>
Motivation: 检索增强生成 (RAG) 已成为使用当前外部信息更新大型语言模型 (LLM) 响应的常用策略。但是，模型可能仍然依赖于记忆的训练数据，绕过检索到的证据，并产生受污染的输出。

Method: 我们引入了检索路径污染评分 (RePCS)，这是一种诊断方法，无需模型访问或重新训练即可检测此类行为。RePCS比较了两种推理路径：(i) 仅使用查询的参数路径，以及 (ii) 通过计算其输出分布之间的 Kullback-Leibler (KL) 散度，使用查询和检索到的上下文的检索增强路径。低散度表明检索到的上下文的影响最小，表明可能存在记忆。

Result: 在 Prompt-WNQA 基准测试中，RePCS 的 ROC-AUC 达到 0.918。此结果优于先前最强方法 6.5 个百分点，同时在 NVIDIA T4 GPU 上的延迟开销保持在 4.7% 以下。

Conclusion: RePCS提供了一种轻量级的黑盒安全措施，用于验证RAG系统是否有效地利用了检索，这使其在安全关键型应用中尤其有价值。
...
TL;DR: This paper studies the problem of learning computable functions with computational observations and restricted input sources.


<details>
  <summary>Details</summary>
Motivation: This paper studies the problem of learning computable functions in the limit by extending Gold's inductive inference framework to incorporate computational observations and restricted input sources to study the learnability of general recursive functions under more realistic constraints.

Method: This paper extends Gold's inductive inference framework to incorporate computational observations and restricted input sources, introduces Time-Bound Observations, and Policy-Trajectory Observations.

Result: This paper shows that input-output observations do not suffice for learning the class of general recursive functions in the limit, but this learning barrier can be overcome by imposing computational complexity constraints or supplementing with approximate time-bound observations.

Conclusion: This paper shows that learning computable functions from policy trajectories reduces to learning rational functions from input and output, and computable or polynomial-mass characteristic sets cannot exist for the class of linear-time computable functions even for policy-trajectory observations.
...
TL;DR: Simplified analysis of SGD in linear regression using linear algebra, making it more accessible.


<details>
  <summary>Details</summary>
Motivation: Understanding SGD in overparameterized models has led to new optimization algorithms. Previous work provides sharp rates for SGD in linear regression, but this work simplifies the analysis.

Method: Simplified analysis based on linear algebra tools, bypassing the need to manipulate operators on PSD matrices.

Result: The analysis recovers the same bias and variance bounds as previous work but with a simpler approach.

Conclusion: This work simplifies the analysis of SGD on linear regression using linear algebra, making it more accessible and potentially helpful for analyzing mini-batching and learning rate scheduling.
...
TL;DR: DAILOC是一种新的域增量学习框架，可同时解决时间和设备引起的域偏移，并且优于现有技术。


<details>
  <summary>Details</summary>
Motivation: 基于Wi-Fi指纹的室内定位在实际部署中面临重大挑战，这是由于设备异构和室内环境中的时间变化引起的域偏移所致。现有方法通常独立解决这些问题，导致泛化能力差，并且容易随着时间的推移而发生灾难性遗忘。

Method: 提出了一种新颖的域增量学习框架DAILOC，该框架使用多级变分自动编码器分离域偏移与位置相关特征，并引入了一种新颖的记忆引导类潜在对齐机制来解决灾难性遗忘的影响。

Result: 跨多个智能手机、建筑物和时间实例的实验表明，

Conclusion: DAILOC显著优于现有方法，平均误差降低高达2.74倍，最坏情况误差降低4.6倍。
...
TL;DR: LogNet是一个新的框架，可以通过识别哪些接入点（AP）对每个参考点（RP）最有影响来实现透明推理，并且可以通过追踪和诊断模型故障，并调整DL系统以实现更稳定的长期部署。


<details>
  <summary>Details</summary>
Motivation: 现有的深度学习（DL）框架大多作为黑盒模型运行，对于预测是如何做出的，或者模型如何随着时间的推移对真实世界的噪声做出反应，提供的洞察力有限。这种缺乏可解释性阻碍了我们理解由环境动态引起的时间变化的影响，以及调整模型以实现长期可靠性的能力。

Method: 我们引入了LogNet，这是一种新颖的基于逻辑门的框架，旨在解释和增强基于DL的室内定位。

Result: LogNet通过识别每个参考点（RP）最有影响的接入点（AP）来实现透明推理，并揭示环境噪声如何扰乱DL驱动的定位决策。这种可解释性使我们能够追踪和诊断模型故障，并调整DL系统以实现更稳定的长期部署。
...
TL;DR: The paper finds that the combination of non-stationarity with gradient pathologies, due to suboptimal architectural choices, underlie the challenges of scaling deep reinforcement learning networks. The paper proposes a series of direct interventions that stabilize gradient flow, enabling robust performance across a range of network depths and widths.
...
Method: a series of direct interventions that stabilize gradient flow, enabling robust performance across a range of network depths and widths.

Result: the combination of non-stationarity with gradient pathologies, due to suboptimal architectural choices, underlie the challenges of scale.

Conclusion: The interventions are simple to implement and compatible with well-established algorithms, and result in an effective mechanism that enables strong performance even at large scales. We validate our findings on a variety of agents and suites of environments.
...
TL;DR: 我们提出了一种组合方法，通过集成一组zero-shot专家模型，在具有挑战性的基准测试中，我们的方法比基线算法实现了更高的精度，同时需要更少的计算资源。
...
Method: 通过集成一组zero-shot专家模型，提出了一种组合方法，并使用旨在测试组合能力的具有挑战性的基准评估我们的方法。

Result: 我们的Expert Composition方法比基线算法能够实现更高的精度，同时需要更少的计算资源，因此效率更高。

Conclusion: Expert Composition方法比基线算法能够实现更高的精度，同时需要更少的计算资源，因此效率更高。
...
TL;DR: MicroRicci is a fast and efficient mesh smoothing method for real-time applications.


<details>
  <summary>Details</summary>
Motivation: Real-time mesh smoothing at scale remains a formidable challenge: classical Ricci-flow solvers demand costly global updates, while greedy heuristics suffer from slow convergence or brittle tuning.

Method: a self-tuning, local Ricci-flow solver with a greedy syndrome-decoding step augmented by two tiny neural modules

Result: MicroRicci slashes iteration counts, tightens curvature spread, achieves a remarkable UV-distortion-to-MOS correlation, and yields an end-to-end runtime acceleration.

Conclusion: MicroRicci is well suited for real-time, resource-limited applications due to its linear-time updates, automatic hyperparameter adaptation, and high-quality results.
...
TL;DR: DP-GRAPE is a memory-efficient DP training method that maintains utility and scales to large models.
...
Method: DP-GRAPE, which involves privatizing gradients after projection, using random Gaussian matrices, and applying projection during backpropagation.

Result: DP-GRAPE reduces memory usage by over 63% when pre-training Vision Transformers and over 70% when fine-tuning RoBERTa-Large compared to DP-Adam, while achieving similar performance.
...
TL;DR: This paper introduces GFLC, a graph-based method to correct label noise and preserve demographic parity, improving the balance between performance and fairness.


<details>
  <summary>Details</summary>
Motivation: Label bias in training data affects model performance and misrepresents classifier fairness during testing, necessitating robust fairness-aware methods.

Method: The paper introduces Graph-based Fairness-aware Label Correction (GFLC), which combines prediction confidence, Ricci-flow-optimized graph Laplacians for graph-based regularization, and explicit demographic parity incentives.

Result: GFLC improves the trade-off between performance and fairness metrics.

Conclusion: The experimental results demonstrate GFLC's effectiveness, showing notable improvements in the trade-off between performance and fairness metrics compared to baselines.
...
TL;DR: This study uses federated learning to predict brain age in stroke patients and finds that it is associated with vascular risk factors and post-stroke recovery.


<details>
  <summary>Details</summary>
Motivation: Brain-predicted age difference (BrainAGE) is a neuroimaging biomarker reflecting brain health. However, training robust BrainAGE models requires large datasets, often restricted by privacy concerns. This study evaluates the performance of federated learning (FL) for BrainAGE estimation in ischemic stroke patients treated with mechanical thrombectomy, and investigates its association with clinical phenotypes and functional outcomes.

Method: We used FLAIR brain images from 1674 stroke patients across 16 hospital centers. We implemented standard machine learning and deep learning models for BrainAGE estimates under three data management strategies: centralized learning (pooled data), FL (local training at each site), and single-site learning. We reported prediction errors and examined associations between BrainAGE and vascular risk factors (e.g., diabetes mellitus, hypertension, smoking), as well as functional outcomes at three months post-stroke. Logistic regression evaluated BrainAGE's predictive value for these outcomes, adjusting for age, sex, vascular risk factors, stroke severity, time between MRI and arterial puncture, prior intravenous thrombolysis, and recanalisation outcome.
...
TL;DR: 本文提出了一种新的离线强化学习算法，用于解决由于次优离线数据中的不良探索而导致的过度保守策略问题。


<details>
  <summary>Details</summary>
Motivation: 解决优势加权回归 (AWR) 家族的局限性，即由于数据损坏（特别是次优离线数据中的不良探索）导致学习过度保守策略的潜力。

Method: 提出 Corruption-Averse Advantage-Weighted Regression (CAWR)，它在策略优化过程中加入了一组鲁棒的损失函数，并采用了一种基于优势函数的优先经验回放方法来过滤不良探索。

Result: 在 D4RL 基准测试上的数值实验表明，Corruption-Averse Advantage-Weighted Regression (CAWR) 能够从次优离线数据中学习到更好的策略，显著提升策略优化的性能。

Conclusion: Corruption-Averse Advantage-Weighted Regression (CAWR) 能在次优离线数据中学习到更好的策略，显著提升策略优化的性能。
