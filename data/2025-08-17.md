...
Motivation: 从临床文件中自动提取数据具有提高医疗效率的巨大潜力，但部署自然语言处理（NLP）解决方案面临着实际挑战。

Method: 在不列颠哥伦比亚癌症登记处（BCCR）实施各种NLP模型以进行信息提取和分类任务。

Result: 强调了基于明确的业务目标定义问题、采用迭代开发方法以及促进领域专家、最终用户和机器学习专家之间从一开始的深度跨学科协作和共同设计至关重要。其他的见解强调了务实的模型选择（包括混合方法和在适当情况下使用更简单的方法）、严格关注数据质量（代表性、漂移、注释）、涉及人机循环验证和持续审计的强大错误缓解策略以及构建组织人工智能素养的必要性。

Conclusion: 分享了在不列颠哥伦比亚癌症登记处（BCCR）实施各种NLP模型以进行信息提取和分类任务的关键经验教训，强调了基于明确的业务目标定义问题、采用迭代开发方法以及促进跨学科合作和共同设计的重要性。
...
<details>
  <summary>Details</summary>
Motivation: Concerns about the fairness of LLMs persist, especially in high-stakes domains.

Method: The method ensures verifiable, immutable, and reproducible evaluations by executing onchain HTTP requests to hosted Hugging Face endpoints and storing datasets, prompts, and metrics directly onchain.

Result: The paper benchmarks the Llama, DeepSeek, and Mistral models on the PISA dataset and evaluates structured Context Association Metrics derived from the StereoSet dataset, and reveals cross-linguistic disparities using the Kaleidoscope benchmark.

Conclusion: The paper provides an open-source evaluation protocol for benchmarking the fairness of open-source LLMs using smart contracts, and benchmarks Llama, DeepSeek, and Mistral models, revealing cross-linguistic disparities. All code and results are open source, enabling community audits and longitudinal fairness tracking across model versions.
...
<details>
  <summary>Details</summary>
Motivation: 先前的工作主要缺乏内容或主题分类。虽然任务分类在教育中更为普遍，但大多数没有得到 K-12 真实世界数据的支持。

Method: 采用了一种新颖、简单的主题建模方法，并直接应用了最先进的 LLM，通过明确的指令来实现比以前的方法更好的人工对齐的分层主题结构。

Result: 对学生、教师和 ChatGPT 生成的超过 17,000 条消息进行了分类，包括内容（例如自然和人）和任务（例如写作和解释）两个维度。为每个维度单独完成的分层分类包括示例性提示，并提供高级概述和实际见解。

Conclusion: 这项研究的结果为研究人员、教师和学生丰富 GenAI 的使用提供了支持，同时讨论也强调了未来研究的一些问题。
...
<details>
  <summary>Details</summary>
Motivation: 用户与人工智能系统之间发展出情感纽带的人工智能陪伴已经成为一种重要的模式，它既有积极的意义，也有令人担忧的意义。

Method: 从心理学理论和用户数据中，我们开发了一个包含四个类别和 368 个目标提示的 31 种行为的分类法。对这些提示的响应被评估为陪伴增强型、边界维护型或中性。

Result: 将 INTIMA 应用于 Gemma-3、Phi-4、o3-mini 和 Claude-4 表明，陪伴增强行为在所有模型中仍然更为常见，尽管我们观察到模型之间存在显着差异。

Conclusion: 不同商业提供商在基准测试中更敏感的部分优先考虑不同的类别，这令人担忧，因为适当的边界设置和情感支持对用户的福祉都很重要。这些发现强调需要更一致的方法来处理情感交互。
...
Result: We systematically evaluate various MLLM-based misinformation detection strategies, assessing models across different architectures and scales, as well as benchmarking against existing detection methods. Building on these analyses, we further enable a semi-automatic detection-in-the-loop framework that continuously updates XFacta with new content to maintain its contemporary relevance.
...
<details>
  <summary>Details</summary>
Motivation: 在为实际应用程序开发文本分类模型时，一个主要的挑战是难以收集到所有文本类别的足够数据。

Method: 利用大型语言模型 (LLM) 生成合成数据，并使用这些数据来提高模型的性能。

Result: 集成方法比自动化工作流程中的每种单独策略更有效。

Conclusion: 使用集成方法，该方法根据类别的特征选择搜索策略，从而更有效地利用大型语言模型改进分类模型。
...
<details>
  <summary>Details</summary>
Motivation: 心理学研究一致发现，不同语义尺度上的人类对词语的评级可以减少到低维形式，而信息损失相对较小。我们发现，大型语言模型（LLM）的嵌入矩阵中编码的语义关联表现出相似的结构。

Method: 通过反义词对（例如，善良-残酷）定义的语义方向上词的投影与人类评级高度相关，并且发现这些投影有效地减少到 LLM 嵌入中的 3 维子空间，非常类似于从人类调查响应中得出的模式。

Result: 词在由反义词对（例如，善良-残酷）定义的语义方向上的投影与人类评级高度相关，并且这些投影有效地减少到 LLM 嵌入中的 3 维子空间，非常类似于从人类调查响应中得出的模式。此外，我们发现，沿着一个语义方向移动 tokens 会对几何对齐的特征产生脱靶效应，其大小与它们的余弦相似度成正比。

Conclusion: 语义特征以类似于人类语言中相互关联的方式纠缠在大型语言模型（LLM）中，并且大量的语义信息（尽管其表面上很复杂）出人意料地是低维的。此外，解释这种语义结构可能对于在引导特征时避免意外后果至关重要。
...
<details>
  <summary>Details</summary>
Motivation: 注意力机制是 Transformer 架构的核心组成部分。除了提高性能外，注意力已被提议作为一种通过注意力权重进行解释的机制，注意力权重与输入特征（例如，文档中的token）相关联。在这种情况下，较大的注意力权重可能意味着与模型预测更相关的特征。在循证医学中，这种解释可以支持医生理解和交互用于分类生物医学文献的 AI 系统。然而，对于注意力权重是否提供有用的解释，目前仍未达成共识。此外，很少有研究探讨可视化注意力如何影响其作为解释辅助工具的有用性。

Method: 进行了一项用户研究，以评估基于注意力的解释是否支持用户进行生物医学文档分类，以及是否存在可视化它们的首选方式。研究涉及来自各个学科的医学专家，他们根据研究设计（例如，系统评价、广泛综合、随机和非随机试验）对文章进行分类。

Result: Transformer 模型 (XLNet) 能够准确地对文档进行分类；然而，注意力权重并没有被认为对解释预测特别有帮助。然而，这种看法因注意力如何可视化而异。与 Munzner 的视觉有效性原则相反，该原则偏爱精确的编码（如条形长度），用户更喜欢更直观的格式，如文本亮度或背景颜色。

Conclusion: 注意力权重不一定有助于解释，但其感知到的帮助程度受到其视觉呈现方式的影响。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型 (LLM) 在解决数学问题方面表现出了卓越的能力。然而，从提供答案到生成高质量的教育问题，这一转变带来了重大挑战，但这些挑战仍未得到充分探索。

Method: 我们推出了 EQGBench，这是一个综合基准，专门用于评估法学硕士在中文 EQG 中的表现。

Result: EQGBench 建立了一个五维评估框架，该框架由一个包含 900 个评估样本的数据集支持，这些样本涵盖了三个基础中学学科：数学、物理和化学。

Conclusion: 通过对 46 个主流大型模型的系统评估，我们发现，在生成反映教育价值和培养学生综合能力的问题方面，仍有很大的发展空间。
...
<details>
  <summary>Details</summary>
Motivation: 恶意归因偏差是指将社会互动解释为故意怀有敌意的倾向。歧义意图敌意问卷 (AIHQ) 通常用于测量恶意归因偏差，其中包括开放式问题，参与者在其中描述负面社会情境背后的感知意图以及他们将如何回应。虽然这些问题提供了对恶意归因内容的见解，但它们需要人工评分员进行耗时的评分。

Method: 使用先前收集的数据集，其中脑外伤 (TBI) 患者和健康对照 (HC) 完成了 AIHQ，并由训练有素的人工评分员对他们的开放式回答进行评分。我们使用这些回答的一半来根据人工生成的评分微调两个模型，并在剩余一半的 AIHQ 回答上测试微调后的模型。

Result: 模型生成的评分与人工评分在敌意归因和攻击性反应方面一致，微调后的模型显示出更高的一致性。这种一致性在模糊、有意和意外情景类型中是一致的，并且重复了先前关于 TBI 和 HC 组之间敌意归因和攻击性反应的组间差异的研究结果。微调后的模型也很好地推广到了独立的非临床数据集。
...
<details>
  <summary>Details</summary>
Motivation: 测试时计算（TTC）已经成为一种强大的范例，可以在推理时增强大型语言模型（LLM）的性能，利用测试时训练（TTT）和检索增强生成（RAG）等策略。然而，最佳的适应策略因查询而异，并且不加区分地应用TTC策略会产生大量的计算开销。

Method: 引入了奖励引导的测试时计算（RTTC），这是一种新颖的框架，它通过预训练的奖励模型自适应地为每个查询选择最有效的TTC策略，从而最大限度地提高跨不同领域和任务的下游准确性。RTTC在分布式服务器-客户端架构中运行，仅在必要时才从远程知识库检索相关样本，并在客户端设备上应用RAG或轻量级微调。为了进一步减少冗余计算，我们提出了查询状态缓存，从而可以在检索和适应级别上高效地重用历史查询状态。
...
<details>
  <summary>Details</summary>
Motivation: Automatic curation of discussion forums requires constant updates, making frequent retraining of LLMs a resource-intensive process.

Method: Bayesian fusion combining a pre-trained generic LLM with a classifier trained on local data.
...
Conclusion: The proposed Bayesian fusion improves results compared to individual classifiers and is competitive with LLM fine-tuning.
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）在少样本信息提取（IE）方面显示出卓越的潜力，但它们的性能对上下文示例的选择高度敏感。传统的选择策略通常无法提供信息指导，因为它们忽略了模型易错性的一个关键来源：不仅来自语义内容，还来自IE任务所需的良好结构化格式的生成所产生的混淆。

Method: 提出了用于信息提取的主动提示（APIE），这是一种新颖的主动提示框架，由我们称之为自省混淆的原则指导。该方法使LLM能够通过双组分不确定性度量来评估其自身的混淆，该度量独特地量化了格式不确定性（生成正确语法的难度）和内容不确定性（提取的语义中的不一致性）。

Result: 在四个基准上的大量实验表明，该方法始终优于强大的基线，在提取准确性和鲁棒性方面均产生了显着改进。

Conclusion: 该研究强调了在构建有效和可靠的结构化生成系统时，细粒度的双层模型不确定性视图至关重要。
...
Method: propose a simple yet effective Supervised Mixture of Experts (S-MoE) by utilizing special guiding tokens to route each task to its designated expert

Result: achieving a 6.35% relative improvement in Word Error Rate (WER)
...
Method: investigate the efficacy and characteristics of LLM-produced jailbreak attacks that cause other models to produce harmful medical misinformation. examine 109 distinct attacks against three target LLMs and compare the attack prompts to in-the-wild health-related LLM queries. examine the resulting jailbreak responses, comparing the generated misinformation to health-related misinformation on Reddit.

Result: Our findings add more evidence that LLMs can be effectively used to detect misinformation from both other LLMs and from people
...
TL;DR: 本研究评估了ChatGPT等生成式AI模型在日本注册营养师国家考试中作为学习辅助工具的潜力，发现其准确性和一致性有局限性，需要进一步改进。


<details>
  <summary>Details</summary>
Motivation: 基于大型语言模型（LLM）的生成式人工智能（AI），如ChatGPT，已经在包括医学和教育在内的各个专业领域展示了显著的进展。然而，它们在营养教育中的表现，尤其是在日本注册营养师国家执照考试中的表现，仍未得到充分探索。本研究旨在评估当前基于LLM的生成式AI模型作为营养学生学习辅助工具的潜力。

Method: 使用日本注册营养师国家考试的问题作为ChatGPT和三个基于GPT-3.5和GPT-4的Bing模型（精确、创造性、平衡）的提示。每个问题都输入到独立的会话中，并分析模型响应的准确性、一致性和响应时间。测试了额外的提示工程，包括角色分配，以评估潜在的性能改进。

Result: Bing-Precise（66.2%）和Bing-Creative（61.4%）超过了及格线（60%），而Bing-Balanced（43.3%）和ChatGPT（42.8%）没有超过。Bing-Precise和Bing-Creative通常在各个学科领域都优于其他模型，除了营养教育，所有模型都表现不佳。没有一个模型在重复尝试中始终如一地提供相同的正确答案，这突出了答案稳定性的局限性。ChatGPT在响应模式中表现出更大的一致性，但准确性较低。提示工程的影响微乎其微，除了明确提供正确答案和解释时有适度的改进。

Conclusion: 尽管一些生成式AI模型略微超过了及格线，但总体准确性和答案一致性仍不理想。此外，所有模型在答案一致性和稳健性方面都表现出明显的局限性。需要进一步的改进，以确保为营养师执照考试提供可靠和稳定的基于AI的学习辅助工具。
...
<details>
  <summary>Details</summary>
Motivation: LLMs' reliance on static knowledge and opaque reasoning processes limits their performance in knowledge intensive tasks. Current exploration methods face a trade-off between question guided approaches and clue guided methods.

Method: The paper proposes Guidance Graph guided Knowledge Exploration (GG Explore), a novel framework that introduces an intermediate Guidance Graph to bridge unstructured queries and structured knowledge retrieval. Building upon the Guidance Graph, the authors develop Structural Alignment and Context Aware Pruning.
...
Conclusion: The paper's method achieves superior efficiency and outperforms SOTA, especially on complex tasks, while maintaining strong performance with smaller LLMs, demonstrating practical value.
...
<details>
  <summary>Details</summary>
Motivation: 个性化对于提高语言技术（尤其是在问题解答等信息寻求任务中）的有效性和用户满意度至关重要。当前的个性化大型语言模型（LLM）的方法通常依赖于检索增强生成（RAG），然后通过标量奖励信号进行强化学习，以教导模型如何使用检索到的个人上下文。我们认为这些标量奖励有时会提供微弱的、非指导性的反馈，从而限制了学习效率和个性化质量。

Method: 提出了VAC，一种新颖的个性化响应生成框架，该框架用自然语言反馈（NLF）代替标量奖励，这些反馈是根据用户个人资料和问题叙述生成的。

Result: 在LaMP-QA基准（包含三个不同的领域）上的评估表明，与最先进的结果相比，有持续且显着的改进。人工评估进一步证实了生成的响应具有卓越的质量。
...
<details>
  <summary>Details</summary>
Motivation: 当前的角色扮演研究通常依赖于未经证实的LLM作为评判范例，这可能无法反映人类如何看待角色保真度。人类一致评估的一个关键先决条件是角色识别，即根据对话上下文识别谁在说话的能力。

Method: PersonaEval，第一个旨在测试LLM评估器是否可以可靠地识别人类角色的基准。

Result: 即使是表现最好的LLM也只能达到大约69%的准确率，远低于可靠评估所需的水平。相比之下，人类参与者的表现接近上限，准确率为90.8%，这突出表明当前的LLM评估器仍然不够人性化，无法有效地判断角色扮演场景。

Conclusion: 即使是表现最好的LLM也只能达到大约69%的准确率，远低于可靠评估所需的水平。可靠的评估需要的不仅仅是特定于任务的调整，而且还取决于LLM评估器中强大的人类般的推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有的TOD数据集主要是基于文本的，缺乏真实的语音信号，这对于评估基于语音的LLM的鲁棒性至关重要。此外，现有的语音TOD数据集主要以英语为主，缺乏语音口吃和说话人变化等关键方面。

Method: 提出了一个新的跨模态聊天任务，可以真实地模拟真实世界的用户交互，允许在语音和文本模态之间动态切换。

Result: RealTalk-CN，是第一个中文多轮、多领域语音-文本双模态TOD数据集，包含5.4k个对话（60K个话语，150小时），带有配对的语音-文本注释。RealTalk-CN捕获了具有注释的自发语音口吃的各种对话场景，确保全面覆盖语音对话中真实世界的复杂性。

Conclusion: RealTalk-CN数据集的有效性通过大量实验得到验证，为中文语音LLM的研究奠定了坚实的基础。
...
Result: achieves comprehensive multimodal capabilities without additional training, performance improvements of up to 7.8% over traditional jointly-trained approaches on standard benchmarks, reduced latency by 10.3%, and significantly enhanced interpretability through explicit orchestration processes.
...
Method: introducing a categorical homotopy framework for LLMs. We introduce an LLM Markov category to represent probability distributions in language generated by an LLM
...
<details>
  <summary>Details</summary>
Motivation: improving the reasoning ability of Small Language Models (SLMs, e.g., $\leq$ 1.5B) remains challenging. A key obstacle lies in the complexity and variability of natural language
...
<details>
  <summary>Details</summary>
Motivation: 在联邦学习环境中有效增强大型语言模型(llm)的推理能力仍然具有挑战性，特别是在平衡性能增益与严格的计算、通信和隐私约束时。传统的LLM联邦调优方法主要优化答案的正确性，而忽略了理由质量，使CoT能力依赖于模型天生的预训练能力。此外，现有的改进理由的方法通常依赖于侵犯隐私的集中式模型的知识蒸馏。此外，在llm上进行传统联邦微调的通信开销仍然很大。

Method: FedCoT利用轻量级的思维链增强机制，本地模型生成多个推理路径，并使用紧凑的判别器动态选择最有希望的一个。采用改进的聚合方法，构建在高级LoRA模块堆叠之上，结合客户端分类器感知，以实现跨不同客户端的无噪声聚合。

Result: 在医疗推理任务上的综合实验表明，FedCoT在严格的资源预算下显著提高了客户端的推理性能，同时完全保护了数据隐私。

Conclusion: FedCoT显著提高了客户端的推理性能，同时完全保护了数据隐私。
...
<details>
  <summary>Details</summary>
Motivation: 从客户历史通信序列中学习客户嵌入对于金融应用至关重要。虽然大型语言模型（LLM）提供了一般的世界知识，但它们在长事件序列上的直接使用在计算上是昂贵的，并且在现实世界的管道中是不切实际的。

Method: 对比学习框架，将原始事件嵌入与来自冻结LLM的语义嵌入对齐。行为特征被总结成简短的提示，由LLM嵌入，并用作通过对比损失的监督。

Result: 该方法显著降低了推理成本和输入大小，与传统的LLM处理完整序列相比。

Conclusion: 该方法在实际金融数据集上优于最先进的技术，同时保持在对延迟敏感的环境中可部署。
...
<details>
  <summary>Details</summary>
Motivation: To mitigate hallucination and factual inaccuracies of large language models (LLMs) in multiple-choice question answering (MCQA).
...
Result: The enhanced CP achieves user-specified empirical miscoverage rates, and Test-set average prediction set size (APSS) decreases monotonically with increasing risk levels ($\\alpha$), validating APSS as an effective uncertainty metric.
...
<details>
  <summary>Details</summary>
Motivation: Postpartum Depression (PPD) significantly impacts mothers' mental and physical well-being, necessitating rapid detection and intervention. There is a need for technological advancements to enable real-time screening and treatment recommendations.

Method: The study combines Natural Language Processing, Machine Learning (ML), and Large Language Models (LLMs) for free speech analysis. It uses interpretable ML models (tree-based algorithms) with feature importance and natural language to explain predictions.

Result: The system achieves 90% accuracy in PPD detection, outperforming competing solutions in the literature.
...
<details>
  <summary>Details</summary>
Motivation: LLMs suffer from excessive inference costs and latency when applied uniformly to all problems.

Method: a reinforcement learning framework that endows LLMs with user-controllable, token-budgeted reasoning
...
<details>
  <summary>Details</summary>
Motivation: 美国约有500万老年人患有阿尔茨海默病和相关痴呆症（ADRD），但仍有一半以上未被诊断。基于语音的自然语言处理（NLP）提供了一种有前途的、可扩展的方法，可以通过语言标记检测早期认知衰退。

Method: 该研究开发并评估了一个筛选流程，该流程：（i）将transformer embeddings与手工制作的语言特征融合，（ii）测试使用大型语言模型（LLM）生成的合成语音进行数据增强，以及（iii）对用于ADRD检测的单模态和多模态LLM分类器进行基准测试。

Result: 融合模型实现了F1 = 83.3（AUC = 89.5），优于仅使用语言或transformer的基线。使用2倍MedAlpaca-7B合成语音增强训练数据将F1提高到85.7。微调显著提高了单模态LLM分类器的性能（例如，MedAlpaca：F1 = 47.3 -> 78.5 F1）。目前的多模态模型表现出较低的性能（GPT-4o = 70.2 F1；Qwen = 66.0）。性能提升与合成语音和真实语音之间的分布相似性一致。

Conclusion: 整合transformer embeddings和语言特征可以增强通过语音检测ADRD的效果。经过临床调整的LLM可以有效地支持分类和数据增强，而多模态建模还需要进一步发展。
...
Method: PREF 包含三个步骤的流程：(1) 覆盖阶段，使用大型语言模型 (LLM) 生成全面的、特定于查询的指南，涵盖通用标准，例如事实性、连贯性和完整性；(2) 偏好阶段，使用目标用户的个人资料、明确或推断的偏好和上下文，对这些因素进行重新排序和选择性地扩充，从而生成个性化的评估标准；(3) 评分阶段，应用 LLM 评判器根据此标准对候选答案进行评分，确保基线充分性，同时捕获主观优先级。

Result: 在 PrefEval 基准测试（包括隐式偏好跟踪任务）上的实验表明，PREF 比强大的基线实现了更高的准确性、更好的校准以及与人类判断更紧密的对齐。

Conclusion: PREF 通过实现可扩展、可解释和用户对齐的评估，为个性化语言生成系统更可靠的评估和开发奠定了基础。
...
Method: 提出了一种基于表示的攻击方法，通过插值有害和良性查询对的隐藏状态来引出禁止的响应，称为潜在融合越狱（LFJ）。

Result: 在 Vicuna 和 LLaMA-2 等模型上，跨 AdvBench 和 MaliciousInstruct 等基准的平均攻击成功率 (ASR) 为 94.01%，优于现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有的提示优化方法与推理策略无关；也就是说，它们优化提示时不考虑部署期间使用的推理策略。这构成了一个重要的方法论差距，因为我们的经验和理论分析揭示了这两个范例之间存在很强的相互依赖性。此外，我们发现用户对多个目标和推理预算之间的权衡偏好会大大影响提示和推理配置的选择。

Method: 本文介绍了一个名为IAPO（推理感知提示优化）的统一的新框架，该框架联合优化提示和推理规模，同时了解推理预算和不同的任务目标。然后，我们为IAPO开发了一种固定预算训练算法，我们称之为PSST（通过顺序修剪进行提示缩放），并分析有限预算下的误差概率保证。

Result: 我们的经验和理论分析揭示了提示优化和推理策略之间存在很强的相互依赖性; 用户对多个目标和推理预算之间的权衡偏好会大大影响提示和推理配置的选择; 我们在六个不同的任务上评估了PSST的有效性，包括多目标文本生成和推理。

Conclusion: 本文证明了在通过提示优化对齐黑盒LLM时，结合推理感知至关重要，并在六个不同的任务上评估了PSST的有效性，包括多目标文本生成和推理。
...
<details>
  <summary>Details</summary>
Motivation: 发现具有思考模式的LLM更容易受到Jailbreak攻击。在AdvBench和HarmBench上评估了9个LLM，发现攻击LLM中思考模式的成功率几乎高于非思考模式。通过大量的样本研究发现，出于教育目的和过长的思考长度是成功攻击数据的特征，并且LLM在很大程度上知道问题有害时也会给出有害的答案。

Method: 提出了一种安全思考干预方法，通过在prompt中添加LLM的“特定思考令牌”来显式地引导LLM的内部思考过程。

Result: 安全思考干预可以显著降低具有思考模式的LLM的攻击成功率。

Conclusion: 提出了针对LLM的安全思考干预方法，通过在提示中添加“特定思考令牌”来显式引导LLM的内部思考过程，从而显著降低具有思考模式的LLM的攻击成功率。
...
<details>
  <summary>Details</summary>
Motivation: 目前对于大型语言模型利用不同人类推理技能的机制研究不足，尤其是在涉及不同语言和文化的日常知识的多语言常识推理方面。

Method: 提出了一个多语言和可扩展的、基于技能的常识推理基准（mSCoRe），它包含三个关键组件：推理技能的新分类、为常识推理评估量身定制的稳健数据合成流程，以及允许任务难度随LLM能力提升动态扩展的复杂性扩展框架。

Result: 在八个最先进的LLM上进行了广泛的实验，结果表明mSCoRe对于当前的模型仍然具有很大的挑战性，特别是在更高的复杂性级别上。

Conclusion: mSCoRe benchmark表明，目前最先进的LLM在细致的多语言通用和文化常识方面仍有局限性。对模型推理过程的详细分析为改进多语言常识推理能力提供了未来方向。
...
<details>
  <summary>Details</summary>
Motivation: LLMs struggle with nuanced environments or interactive tasks common in real-world scenarios, highlighting the need for LLMs that can engage in logically consistent multi-turn dialogue, seek information, and reason with incomplete data.
...
Method: introducing LaaJMeter, a simulation-based framework for controlled meta-evaluation of LaaJs which enables engineers to generate synthetic data representing virtual models and judges, allowing systematic analysis of evaluation metrics under realistic conditions.

Result: demonstrate the utility of LaaJMeter in a code translation task involving a legacy programming language, showing how different metrics vary in sensitivity to evaluator quality and highlight the limitations of common metrics and the importance of principled metric selection.
...
<details>
  <summary>Details</summary>
Motivation: High-quality machine translation outputs make it difficult to distinguish between state-of-the-art models and to identify areas for future improvement. Automatically identifying texts where machine translation systems struggle holds promise for developing more discriminative evaluations and guiding future research.

Method: The authors introduce a new metric to evaluate difficulty estimators and use it to assess both baselines and novel approaches.

Result: The authors demonstrate the practical utility of difficulty estimators by using them to construct more challenging machine translation benchmarks.

Conclusion: Dedicated models (dubbed Sentinel-src) outperform both heuristic-based methods (e.g. word rarity or syntactic complexity) and LLM-as-a-judge approaches. The authors release two improved models for difficulty estimation, Sentinel-src-24 and Sentinel-src-25.
...
Method: We introduce PakBBQ, a culturally and regionally adapted extension of the original Bias Benchmark for Question Answering (BBQ) dataset. We evaluate multiple multilingual LLMs under both ambiguous and explicitly disambiguated contexts, as well as negative versus non negative question framings.
...
Conclusion: This study highlights the importance of contextualized benchmarks and simple prompt engineering strategies for bias mitigation in low resource settings.
...
<details>
  <summary>Details</summary>
Motivation: The proliferation of Large Language Models (LLMs) is challenged by hallucinations, critical failure modes where models generate non-factual, nonsensical or unfaithful text. Existing methods are limited in detecting arbitrariness by measuring the diversity of answers to a single, fixed prompt.

Method: The approach uses joint clustering on sentence embeddings to create a shared topic space for prompts and answers, and computes information-theoretic metrics such as Jensen-Shannon divergence, Wasserstein distance, and KL divergence to measure the semantic divergence between prompts and responses.

Result: The paper identifies the KL divergence KL(Answer || Prompt) as a powerful indicator of Semantic Exploration and combines metrics into the Semantic Box, a diagnostic framework for classifying LLM response types.

Conclusion: The paper introduces Semantic Divergence Metrics (SDM), a novel framework for detecting Faithfulness Hallucinations in Large Language Models (LLMs). The framework uses joint clustering and information-theoretic metrics to measure the semantic divergence between prompts and responses.
...
Result: BERT achieves the highest overall performance due to its pre-training advantage, while CNN demonstrates superior efficacy on rare emoji classes

Conclusion: BERT achieves the highest overall performance, while CNN demonstrates superior efficacy on rare emoji classes. This research shows the importance of architecture selection and hyperparameter tuning.
...
<details>
  <summary>Details</summary>
Motivation: 临床精神分裂症高危（CHR）患者需要密切监测其症状，以便为适当的治疗提供信息。简明精神病评定量表（BPRS）是一种经过验证的常用研究工具，用于测量精神分裂症和其他精神病性疾病患者的症状；但是，它在临床实践中并不常用，因为它需要冗长的结构化访谈。

Method: 利用大型语言模型（LLM）从来自加速药物合作-精神分裂症（AMP-SCZ）队列的409名CHR患者的临床访谈记录中预测BPRS评分。

Result: LLM预测的零样本性能与真实评估相比（中位数一致性：0.84，ICC：0.73）接近人类间和人类内评估者信度。

Conclusion: LLMs表现出提高和标准化CHR患者评估的巨大潜力，通过它们评估外语BPRS的准确性，以及在单次或少次学习方法中整合纵向信息。
...
TL;DR: 通过将LLM的输出融入提示中，可以更好地匹配模型的归纳偏置，从而显著提高分类和排序任务的性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）对提示语的细微变化很敏感，这可能是由于LLM中存在的归纳偏置造成的。

Method: 使用LLM的输出作为其提示的一部分，从而创建一个与模型中的归纳偏置相匹配的提示。

Result: 使用这种归纳偏置提取和匹配策略，LLM用于分类的Likert评分提高了19%，用于排序的Likert评分提高了27%。
...
TL;DR: This study proposes a qualitative, discursive framework to investigate gender and racial biases in LLMs. Results show that LLMs replicate problematic stereotypes and offer superficial revisions when prompted to correct biases.
...
Conclusion: LLMs replicate crystallized discursive representations, reinforcing essentialization and a sense of social immobility. Superficial revisions that maintained problematic meanings, revealing limitations in fostering inclusive narratives. The ideological functioning of algorithms has significant implications for the ethical use and development of AI.
...
TL;DR: ReviewRL, a reinforcement learning framework, generates comprehensive and factually grounded scientific paper reviews, outperforming existing methods.
...
Result: ReviewRL significantly outperforms existing methods across both rule-based metrics and model-based quality assessments on ICLR 2025 papers.
...
TL;DR: DOTABLER is a table-centric semantic document parsing framework that uncovers deep semantic links between tables and their context, achieving over 90% Precision and F1 scores.
...
### [50] [Stochastic-based Patch Filtering for Few-Shot Learning](https://arxiv.org/abs/2508.10066)
*Javier Rodenas,Eduardo Aguilar,Petia Radeva*

Main category: cs.CV

TL;DR: SPFF通过随机过滤块嵌入来关注图像的重要部分，从而提高食物图像的few-shot学习性能。


<details>
  <summary>Details</summary>
Motivation: 食物图像由于其视觉复杂性和可变性，给few-shot学习模型带来了独特的挑战。在比较查询与支持图像时，这个问题会导致对最重要元素的关注度降低，从而导致错误分类。

Method: 提出了一种基于随机的few-shot学习块过滤(SPFF)方法，用于关注与类表示具有更大相关性的块嵌入。

Result: SPFF有效地关注了类特定食物特征最突出的块，同时成功地过滤掉了不相关的块。

Conclusion: SPFF在Food-101、VireoFood-172和UECFood-256等few-shot分类基准测试中优于现有SoA方法。
...
### [51] [DINOv3](https://arxiv.org/abs/2508.10104)
*Oriane Siméoni,Huy V. Vo,Maximilian Seitzer,Federico Baldassarre,Maxime Oquab,Cijo Jose,Vasil Khalidov,Marc Szafraniec,Seungeun Yi,Michaël Ramamonjisoa,Francisco Massa,Daniel Haziza,Luca Wehrstedt,Jianyuan Wang,Timothée Darcet,Théo Moutakanni,Leonel Sentana,Claire Roberts,Andrea Vedaldi,Jamie Tolan,John Brandt,Camille Couprie,Julien Mairal,Hervé Jégou,Patrick Labatut,Piotr Bojanowski*

Main category: cs.CV

TL;DR: DINOv3, a versatile vision foundation model, outperforms state-of-the-art models in various vision tasks by leveraging Gram anchoring and post-hoc strategies, and scaling dataset and model size.


<details>
  <summary>Details</summary>
Motivation: Self-supervised learning can eliminate the need for manual data annotation and scale to massive datasets and larger architectures. It can also learn visual representations from diverse sources using a single algorithm.

Method: The paper introduces Gram anchoring to address the issue of dense feature maps degrading during long training schedules. It also applies post-hoc strategies to enhance models' flexibility with respect to resolution, model size, and alignment with text.

Result: DINOv3 outperforms the specialized state of the art across a broad range of settings without fine-tuning, achieving outstanding performance on various vision tasks.
...
### [52] [Empowering Morphing Attack Detection using Interpretable Image-Text Foundation Model](https://arxiv.org/abs/2508.10110)
*Sushrut Patwardhan,Raghavendra Ramachandra,Sushma Venkatesh*

Main category: cs.CV

TL;DR: This paper introduces a CLIP-based multimodal approach for morphing attack detection, showing it can generalize and predict relevant text descriptions in zero-shot scenarios.
...
Method: A multimodal learning framework using Contrastive Language-Image Pretraining (CLIP) is proposed. The framework is evaluated using ten different textual prompts.
...
Conclusion: The paper presents a multimodal learning approach for morphing attack detection, demonstrating generalizable detection and text snippet prediction using CLIP.
...
### [53] [Interpretable Oracle Bone Script Decipherment through Radical and Pictographic Analysis with LVLMs](https://arxiv.org/abs/2508.10113)
*Kaixin Peng,Mengyang Zhao,Haiyang Yu,Teng Fu,Bin Li*

Main category: cs.CV

TL;DR: An interpretable method based on Large Vision-Language Models is proposed for Oracle Bone Script (OBS) decipherment, achieving state-of-the-art accuracy and superior zero-shot decipherment capabilities.
...
### [54] [Deep Learning Enables Large-Scale Shape and Appearance Modeling in Total-Body DXA Imaging](https://arxiv.org/abs/2508.10132)
...
TL;DR: 开发了一种用于 TBDXA 扫描的深度学习基准点放置方法，该方法在身体成分和健康标志物之间建立了关联。
...
Method: 我们开发并验证了一种深度学习方法，用于在 TBDXA 扫描上自动进行基准点放置，使用了 1,683 个手动注释的 TBDXA 扫描。

Result: SAM 特征分布与健康生物标志物相关联，显示证实了现有证据，并生成了关于身体成分和形状与各种虚弱、代谢、炎症和心脏代谢健康标志物之间关系的新假设。

Conclusion: 该方法在外部测试数据集中实现了 99.5% 的正确关键点百分比。
...
### [55] [MANGO: Multimodal Attention-based Normalizing Flow Approach to Fusion Learning](https://arxiv.org/abs/2508.10133)
*Thanh-Dat Truong,Christophe Bobda,Nitin Agarwal,Khoa Luu*

Main category: cs.CV

TL;DR: This paper proposes MANGO, a novel multimodal fusion method using Normalizing Flows and attention, achieving state-of-the-art results on various tasks.


<details>
  <summary>Details</summary>
Motivation: Current multimodal fusion methods using Transformers implicitly learn correlations, failing to capture essential modality features and understand complex structures.
...
Result: The proposed MANGO approach achieves state-of-the-art (SoTA) performance on three different multimodal learning tasks: semantic segmentation, image-to-image translation, and movie genre classification.
...
### [56] [Improving watermelon (Citrullus lanatus) disease classification with generative artificial intelligence (GenAI)-based synthetic and real-field images via a custom EfficientNetV2-L model](https://arxiv.org/abs/2508.10156)
*Nitin Rai,Nathan S. Boyd,Gary E. Vallad,Arnold W. Schumann*

Main category: cs.CV

TL;DR: using both real and synthetic images in a hybrid manner to maximize model performance for crop disease classification


<details>
  <summary>Details</summary>
Motivation: limited research has been conducted on evaluating the effectiveness of integrating real with synthetic images to improve disease classification performance
...
### [57] [SynSpill: Improved Industrial Spill Detection With Synthetic Data](https://arxiv.org/abs/2508.10171)
*Aaditya Baranwal,Abdul Mueez,Jason Voelker,Guneet Bhatia,Shruti Vyas*

Main category: cs.CV

TL;DR: Synthetic data improves VLMs and object detector performance in industrial spill detection, a safety-critical domain where real data is scarce.


<details>
  <summary>Details</summary>
Motivation: Large-scale Vision-Language Models (VLMs) degrade in niche, safety-critical domains such as industrial spill detection, where hazardous events are rare and difficult to annotate, rendering conventional fine-tuning infeasible.
...
Result: Synthetic data enables effective Parameter-Efficient Fine-Tuning (PEFT) of VLMs and boosts the performance of state-of-the-art object detectors. VLMs generalize better to unseen spill scenarios than detectors without synthetic data. With synthetic data, VLMs and detectors achieve marked and comparable improvements.

Conclusion: High-fidelity synthetic data bridges the domain gap in safety-critical applications. Combining synthetic generation and lightweight adaptation is a cost-effective way to deploy vision systems in industrial environments where real data is scarce.
...
### [58] [EntropyGS: An Efficient Entropy Coding on 3D Gaussian Splatting](https://arxiv.org/abs/2508.10227)
*Yuning Huang,Jiahao Pang,Fengqing Zhu,Dong Tian*

Main category: cs.CV

TL;DR: This paper introduces EntropyGS, a factorized and parameterized entropy coding method, to reduce the storage/transmission costs of 3D Gaussian Splatting (3DGS) data. It achieves about 30x rate reduction while maintaining similar rendering quality with fast encoding/decoding.
...
### [59] [CellSymphony: Deciphering the molecular and phenotypic orchestration of cells with single-cell pathomics](https://arxiv.org/abs/2508.10232)
*Paul H. Acosta,Pingjun Chen,Simon P. Castillo,Maria Esther Salvatierra,Yinyin Yuan,Xiaoxi Pan*

Main category: cs.CV

TL;DR: CellSymphony 是一种多模态框架，它融合了空间基因表达和形态学背景，实现了准确的细胞类型注释，并揭示了独特的微环境生态位。
...
Method: CellSymphony，一个灵活的多模态框架，它利用来自 Xenium 转录组图谱和组织学图像的基础模型衍生的嵌入，以真正的单细胞分辨率实现。
...
### [60] [Deep Learning for Crack Detection: A Review of Learning Paradigms, Generalizability, and Datasets](https://arxiv.org/abs/2508.10256)
*Xinan Zhang,Haolin Wang,Yung-An Hsieh,Zhongyu Yang,Anthony Yezzi,Yi-Chang Tsai*

Main category: cs.CV

TL;DR: 本综述系统地分析了裂缝检测领域的新兴趋势，引入了一个新的数据集，并进行了广泛的基准测试实验。


<details>
  <summary>Details</summary>
Motivation: 裂缝检测在土木基础设施中起着至关重要的作用，包括路面、建筑等的检测，近年来，深度学习已大大推动了该领域的发展。虽然该领域存在大量的技术和评论论文，但新兴趋势正在重塑该领域。

Method: 系统地分析了这些趋势，并重点介绍了代表性的工作。

Result: 我们的研究结果为基于深度学习的裂缝检测的演进方法和未来方向提供了见解。

Conclusion: 为未来的研究引入了一个新的3D激光扫描数据集3DCrack，并进行了广泛的基准测试实验，以建立常用深度学习方法（包括最近的基础模型）的基线。
...
### [61] [MRFD: Multi-Region Fusion Decoding with Self-Consistency for Mitigating Hallucinations in LVLMs](https://arxiv.org/abs/2508.10264)
...
Result: 通过跨多个LVLM和基准的实验表明，MRFD显著减少了幻觉并提高了响应的真实性，而无需模型更新。
...
### [62] [Pose-Robust Calibration Strategy for Point-of-Gaze Estimation on Mobile Phones](https://arxiv.org/abs/2508.10268)
*Yujie Zhao,Jiabei Zeng,Shiguang Shan*

Main category: cs.CV

TL;DR: 研究了头部姿势变化对 PoG 估计的影响，并提出了一种动态校准策略，该策略通过在校准过程中引入头部姿势变化来提高估计器的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 由于个人差异，基于外观的注视点 (PoG) 估计器仍然难以跨个体进行泛化。因此，需要进行特定于人的校准才能进行准确的 PoG 估计。然而，校准后的 PoG 估计器通常对头部姿势变化敏感。为了解决这个问题，研究了影响校准估计器的关键因素，并探索了姿势稳健的校准策略。

Method: 构建了一个基准 MobilePoG，其中包含来自 32 个人的面部图像，这些人专注于固定或连续变化的头部姿势下的指定点。使用此基准，系统地分析了校准点和头部姿势的多样性如何影响估计精度。提出了一种动态校准策略，用户在移动手机时注视校准点。

Result: 在校准过程中引入更广泛的头部姿势范围可以提高估计器处理姿势变化的能力。

Conclusion: 提出了一种动态校准策略，通过在用户移动手机时注视校准点，自然地引入头部姿势变化，从而产生更好的校准 PoG 估计器，该估计器对头部姿势变化的敏感度低于使用传统校准策略的估计器。
...
TL;DR: A multi-head vision transformer model achieved 3rd place in the PlantCLEF 2025 challenge by addressing domain shift and using multi-scale and ensemble strategies for plant species prediction.


<details>
  <summary>Details</summary>
Motivation: The PlantCLEF 2025 challenge involves a drastic domain shift between training on single-species images and testing on multi-species quadrat images.

Method: A multi-head vision transformer approach with a pre-trained DINOv2 ViT-B/14 backbone, multi-scale tiling, dynamic threshold optimization, and ensemble strategies.

Result: Strong performance on multi-label plant species prediction using approximately 1.4 million training images covering 7,806 plant species.

Conclusion: The submission achieved 3rd best performance on the private leaderboard of the PlantCLEF 2025 challenge.
...
### [64] [High Fidelity Text to Image Generation with Contrastive Alignment and Structural Guidance](https://arxiv.org/abs/2508.10280)
*Danyi Gao*

Main category: cs.CV

TL;DR: 本文提出了一种新的文本驱动图像生成方法，该方法通过结合对比学习和结构引导来提高语义对齐和结构一致性，实验表明该方法有效。


<details>
  <summary>Details</summary>
Motivation: 现有文本驱动的图像生成方法在语义对齐精度和结构一致性方面存在性能瓶颈。

Method: 通过将文本-图像对比约束与结构引导机制相结合，提出了一种高保真图像生成方法。

Result: 在 COCO-2014 数据集上进行了系统实验。定量指标证实了所提出的方法在 CLIP Score、FID 和 SSIM 方面的优越性能。

Conclusion: 该方法有效地弥合了语义对齐和结构保真度之间的差距，而没有增加计算复杂性。它展示了生成语义清晰和结构完整的图像的强大能力，为联合文本-图像建模和图像生成提供了可行的技术路径。
...
### [65] [VIFSS: View-Invariant and Figure Skating-Specific Pose Representation Learning for Temporal Action Segmentation](https://arxiv.org/abs/2508.10281)
*Ryota Tanaka,Tomohiro Suzuki,Keisuke Fujii*

Main category: cs.CV

TL;DR: A new Temporal Action Segmentation (TAS) framework for figure skating jumps that incorporates 3D nature and semantic procedure, achieving high accuracy with limited data.


<details>
  <summary>Details</summary>
Motivation: Accurately recognizing the type and timing of jumps is essential for objective performance evaluation in figure skating, but it requires expert-level knowledge and existing methods have limitations due to insufficient data and not accounting for 3D aspects and procedural structure.

Method: A new TAS framework incorporating 3D nature and semantic procedure of jump movements, including a View-Invariant, Figure Skating-Specific pose representation learning approach (VIFSS) and a fine-grained annotation scheme.
...
### [66] [JRDB-Reasoning: A Difficulty-Graded Benchmark for Visual Reasoning in Robotics](https://arxiv.org/abs/2508.10287)
*Simindokht Jahangard,Mehrzad Mohammadi,Yi Shen,Zhixi Cai,Hamid Rezatofighi*

Main category: cs.CV

TL;DR: This paper introduces JRDB-Reasoning, a new benchmark for visual reasoning in human-crowded environments that addresses limitations in existing benchmarks by formalizing reasoning complexity and providing detailed annotations.
...
Method: The paper formalizes reasoning complexity and introduces an adaptive query engine that generates customizable questions of varying complexity with detailed intermediate annotations. It also extends the JRDB dataset with human-object interaction and geometric relationship annotations.

Result: The paper creates JRDB-Reasoning, a benchmark tailored for visual reasoning in human-crowded environments.

Conclusion: The paper introduces JRDB-Reasoning, a benchmark tailored for visual reasoning in human-crowded environments, which enables fine-grained evaluation of visual reasoning frameworks and dynamic assessment of visual-language models across reasoning levels.
...
### [67] [A Sub-Pixel Multimodal Optical Remote Sensing Images Matching Method](https://arxiv.org/abs/2508.10294)
*Tao Huang,Hongbo Pan,Nanxi Zhou,Shun Zhou*

Main category: cs.CV

TL;DR: 提出PCWLAD方法以提高多模态图像匹配精度，实验结果表明该方法有效。
...
Method: 提出了一种相位一致性加权最小绝对偏差（PCWLAD）亚像素模板匹配方法，包括使用结构相似性指标（SSIM）进行粗匹配和使用WLAD进行精匹配两个主要步骤。

Result: PCWLAD在正确匹配率（CMR）和均方根误差（RMSE）方面优于现有的八种最先进的方法。

Conclusion: PCWLAD在三个图像数据集上优于现有方法，平均匹配精度约为0.4像素。
...
### [68] [InterSyn: Interleaved Learning for Dynamic Motion Synthesis in the Wild](https://arxiv.org/abs/2508.10297)
*Yiyi Ma,Yuanzhi Liang,Xiu Li,Chi Zhang,Xuelong Li*

Main category: cs.CV

TL;DR: InterSyn is a new framework for realistic interaction motion generation using interleaved learning. It outperforms existing methods in text-to-motion alignment and diversity.


<details>
  <summary>Details</summary>
Motivation: The paper targets the generation of realistic interaction motions by learning from integrated motions that consider both solo and multi-person dynamics. Previous methods treat these components separately.

Method: InterSyn employs an interleaved learning strategy with two modules: Interleaved Interaction Synthesis (INS) and Relative Coordination Refinement (REC).
...
Conclusion: InterSyn generates motion sequences with higher text-to-motion alignment and improved diversity, setting a new benchmark for robust and natural motion synthesis. The code will be open-sourced.
...
### [69] [From Pixel to Mask: A Survey of Out-of-Distribution Segmentation](https://arxiv.org/abs/2508.10309)
*Wenjie Zhao,Jia Li,Yunhui Guo*

Main category: cs.CV

TL;DR: 本文对自动驾驶中分布外(OoD)分割方法进行了综述，归纳为四类，并探讨了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 随着人们对人工智能安全问题的日益关注，分布外 (OoD) 检测和分割已引起越来越多的关注。传统的 OoD 检测方法可以识别 OoD 对象的存在，但缺乏空间定位，限制了它们在下游任务中的效用。OoD 分割通过在像素级粒度上定位异常对象来解决此限制。这种能力对于自动驾驶等安全关键型应用至关重要，在这些应用中，感知模块不仅必须检测而且必须精确分割 OoD 对象，从而实现有针对性的控制动作并增强整体系统鲁棒性。

Method: 本文将当前的 OoD 分割方法分为四类：(i) 测试时 OoD 分割，(ii) 监督训练的异常值暴露，(iii) 基于重建的方法，(iv) 和利用强大模型的方法。

Result: 本文对自动驾驶场景中最新的 OoD 分割进展进行了系统回顾。

Conclusion: 本文综述了自动驾驶场景中最新的 OoD 分割进展，识别了新兴的挑战，并讨论了有希望的未来研究方向。
...
### [70] [Integrating Reinforcement Learning with Visual Generative Models: Foundations and Advances](https://arxiv.org/abs/2508.10316)
*Yuanzhi Liang,Yijie Fang,Rui Li,Ziqi Ni,Ruijie Su,Chi Zhang,Xuelong Li*

Main category: cs.CV

TL;DR: This survey reviews how reinforcement learning is used to improve visual content generation by aligning it with high-level goals, addressing the limitations of traditional surrogate objectives.
...
Conclusion: This survey concludes with open challenges and future research directions at the intersection of RL and generative modeling.
...
### [71] [Concepts or Skills? Rethinking Instruction Selection for Multi-modal Models](https://arxiv.org/abs/2508.10339)
*Andrew Bai,Justin Cui,Ruochen Wang,Cho-Jui Hsieh*

Main category: cs.CV

TL;DR: 本文提出了一种有针对性的数据选择方法，通过平衡概念知识和视觉技能来优化视觉语言模型的性能。


<details>
  <summary>Details</summary>
Motivation: 视觉语言指令调整达到两个主要目的：学习视觉概念和学习视觉技能。在本文中，我们发现视觉语言基准测试属于主要受益于具有相似技能或视觉概念的指令训练的二分法。

Method: 我们设计了一种简单的目标训练数据选择方法来优化给定基准的性能。我们首先从基准中提取概念/技能，确定基准主要受益于相似的概念还是技能，最后选择具有最匹配概念/技能的指令。

Result: 在 10 多个基准测试上的实验验证了我们的目标数据选择方法的有效性，在所有基准测试上平均比现有最佳基线高 +0.9%，在以技能为中心的子集上高 +1.5%。

Conclusion: 本文强调了在指令选择中识别内在权衡的重要性，需要在概念知识的获取与视觉技能之间取得平衡。
...
### [72] [Glo-DMU: A Deep Morphometry Framework of Ultrastructural Characterization in Glomerular Electron Microscopic Images](https://arxiv.org/abs/2508.10351)
*Zhentai Zhang,Danyi Weng,Guibin Zhang,Xiang Chen,Kaixing Long,Jian Geng,Yanmeng Lu,Lei Zhang,Zhitao Zhou,Lei Cao*

Main category: cs.CV

TL;DR: 提出了一种名为 Glo-DMU 的肾小球形态测量框架，用于自动量化肾脏超微结构特征，以辅助肾脏病理诊断。


<details>
  <summary>Details</summary>
Motivation: 复杂的超微结构特征可以指示肾脏疾病的类型、进展和预后。最近，计算病理学与深度学习方法相结合，在推进肾小球超微结构的自动形态分析方面显示出巨大的潜力。然而，目前的研究主要集中在单个超微结构的识别上，这使得满足实际诊断需求具有挑战性。

Method: 提出了一种基于三个深度模型的肾小球形态测量框架Glo-DMU：超微结构分割模型、肾小球滤过屏障区域分类模型和电子致密沉积物检测模型。

Result: 在真实诊断场景中评估了 115 例患有 9 种肾脏病理类型的患者，证明了自动量化结果与病理报告中的形态学描述之间具有良好的一致性。

Conclusion: Glo-DMU 具有全自动化、高精度和高通量的特点，可同时量化多种超微结构特征，为辅助肾脏病理学家提供了一种有效的工具。
...
### [73] [Improving OCR for Historical Texts of Multiple Languages](https://arxiv.org/abs/2508.10356)
*Hylke Westerdijk,Ben Blankenborg,Khondoker Ittehadul Islam*

Main category: cs.CV

TL;DR: This paper uses deep learning techniques to improve OCR and document layout analysis across three tasks: historical Hebrew fragments, 16th-18th century meeting resolutions, and modern English handwriting.


<details>
  <summary>Details</summary>
Motivation: enhance dataset through extensive data augmentation to improve character recognition for the historical Hebrew fragments of the Dead Sea Scrolls and analysis of 16th to 18th-century meeting resolutions task and modern English handwriting recognition task
...
Result: improved character recognition
...
### [74] [AtomDiffuser: Time-Aware Degradation Modeling for Drift and Beam Damage in STEM Imaging](https://arxiv.org/abs/2508.10359)
*Hao Wang,Hongkui Zheng,Kai He,Abolfazl Razi*

Main category: cs.CV

TL;DR: AtomDiffuser 是一种时间感知的退化建模框架，可解开样本漂移和辐射衰减，从而实现可解释的结构演变。


<details>
  <summary>Details</summary>
Motivation: 由于空间漂移和辐射损伤，解释时间分辨的 STEM 数据仍然具有挑战性。这些因素以复杂的时间相关的方式扭曲了几何形状和强度，使得现有方法难以明确地分离它们的影响或以原子分辨率对材料动力学进行建模。

Method: 提出 AtomDiffuser，一种时间感知的退化建模框架

Result: AtomDiffuser 很好地推广到现实世界的低温 STEM 数据。它还支持高分辨率的退化推断和漂移对齐，为可视化和量化与辐射引起的原子不稳定性相关的退化模式提供了工具。

Conclusion: AtomDiffuser，一种时间感知的退化建模框架，通过预测任意两个 STEM 帧之间的仿射变换和空间变化的衰减图，解开样本漂移和辐射衰减。
...
### [75] [Contrast Sensitivity Function of Multimodal Vision-Language Models](https://arxiv.org/abs/2508.10367)
*Pablo Hernández-Cámara,Alexandra Gomez-Villa,Jose Manuel Jaén-Lorites,Jorge Vila-Tomás,Jesus Malo,Valero Laparra*

Main category: cs.CV

TL;DR: 该研究提出了一种新方法来评估多模态模型的视觉敏感度，发现它们与人类感知之间存在差距，并强调了提示语措辞的重要性。


<details>
  <summary>Details</summary>
Motivation: 评估多模态视觉语言模型与人类感知的对齐程度，理解它们如何感知低级视觉特征至关重要。人类视觉的一个关键特征是对比敏感度函数 (CSF)，它描述了在低对比度下对空间频率的敏感度。

Method: 通过提示聊天式视觉语言模型判断不同对比度下图案的可见性，从而评估模型的对比敏感度函数 (CSF)。

Result: 虽然一些模型近似于人类的 CSF 形状或幅度，但没有一个模型能够完全复制两者。提示语措辞对响应有很大影响。

Conclusion: 多模态模型在视觉表征上与人类感知存在差距，提示语措辞对模型响应有很大影响，提示了提示稳定性问题。
...
### [76] [Towards Spatially Consistent Image Generation: On Incorporating Intrinsic Scene Properties into Diffusion Models](https://arxiv.org/abs/2508.10382)
*Hyundo Lee,Suhyung Choi,Byoung-Tak Zhang,Inwoo Hwang*

Main category: cs.CV

TL;DR: 该论文提出了一种共同生成图像及其对应内在属性的方法，以隐式地捕捉潜在的场景结构，并生成更具空间一致性和真实感的图像。


<details>
  <summary>Details</summary>
Motivation: 图像生成模型通常会生成空间不一致和扭曲的图像，因为关于底层结构和空间布局的信息有限。因此，利用内在场景属性（例如，深度、分割图），提供关于底层场景的丰富信息。

Method: 该方法首先使用预训练的估计器从大型图像数据集中提取丰富的内在场景属性，然后使用自动编码器将各种内在场景属性聚合到单个潜在变量中。在此基础上，该方法通过仔细共享互信息来同时对图像和内在域进行去噪，从而使图像和内在因素相互反映，而不会降低图像质量。

Result: 实验结果表明，该方法纠正了空间不一致性，并产生了更自然的场景布局，同时保持了基础模型的保真度和文本对齐。

Conclusion: 该方法纠正了空间不一致性，并产生了更自然的场景布局，同时保持了基础模型的保真度和文本对齐。
...
### [77] [Unlocking Robust Semantic Segmentation Performance via Label-only Elastic Deformations against Implicit Label Noise](https://arxiv.org/abs/2508.10383)
...
TL;DR: NSegment+ decouples image and label transformations to address realistic noise for semantic segmentation.


<details>
  <summary>Details</summary>
Motivation: Typical data augmentation methods, which apply identical transformations to the image and its label, risk amplifying these subtle imperfections and limiting the model's generalization capacity.

Method: introducing controlled elastic deformations only to segmentation labels while preserving the original images

Result: achieving mIoU gains of up to +2.29, +2.38, +1.75, and +3.39 in average on Vaihingen, LoveDA, Cityscapes, and PASCAL VOC, respectively

Conclusion: NSegment+ consistently improves performance, achieving mIoU gains of up to +2.29, +2.38, +1.75, and +3.39 in average on Vaihingen, LoveDA, Cityscapes, and PASCAL VOC, respectively
...
### [78] [PQ-DAF: Pose-driven Quality-controlled Data Augmentation for Data-scarce Driver Distraction Detection](https://arxiv.org/abs/2508.10397)
*Haibin Sun,Xinghui Song*

Main category: cs.CV

TL;DR: 提出了 PQ-DAF，利用视觉语言模型和扩散模型来增强训练数据，提高少样本驾驶员分心检测的性能。


<details>
  <summary>Details</summary>
Motivation: 驾驶员分心检测对于提高交通安全和减少道路交通事故至关重要。然而，现有模型在实际场景中部署时，泛化能力通常会下降。这种限制主要源于实际环境中数据标注的高成本导致的少样本学习挑战，以及训练数据集和目标部署条件之间的显着领域转移。

Method: 提出了一个姿态驱动的质量控制数据增强框架 (PQ-DAF)，该框架利用视觉语言模型进行样本过滤，以经济高效地扩展训练数据并增强跨域鲁棒性。具体来说，我们采用渐进式条件扩散模型 (PCDM) 来准确捕获关键驾驶员姿势特征并合成不同的训练示例。然后引入一个基于 CogVLM 视觉语言模型构建的样本质量评估模块，以基于置信度阈值过滤掉低质量的合成样本，从而确保增强数据集的可靠性。

Result: PQ-DAF 在少样本驾驶员分心检测中表现出性能的显著提高，并在数据稀缺条件下实现了模型泛化的显着提升。

Conclusion: PQ-DAF 显著提高了在少样本驾驶员分心检测中的性能，并在数据稀缺条件下实现了模型泛化的显着提升。
...
### [79] [Translation of Text Embedding via Delta Vector to Suppress Strongly Entangled Content in Text-to-Image Diffusion Models](https://arxiv.org/abs/2508.10407)
*Eunseo Koh,Seunghoo Hong,Tae-Young Kim,Simon S. Woo,Jae-Pil Heo*

Main category: cs.CV

TL;DR: 提出了一种新方法，通过在文本嵌入空间中抑制纠缠的内容来改进文本到图像的扩散模型。


<details>
  <summary>Details</summary>
Motivation: 文本到图像（T2I）扩散模型在从文本提示生成各种高质量图像方面取得了重大进展。但是，这些模型在抑制与特定单词强烈纠缠的内容方面仍然面临挑战。例如，在生成“查理·卓别林”的图像时，即使明确指示不包括“胡须”，也始终会出现“胡须”，因为“胡须”的概念与“查理·卓别林”密不可分。

Method: 提出了一种新方法，可以直接抑制扩散模型的文本嵌入空间中这种纠缠的内容。我们的方法引入了一个delta向量，该向量修改了文本嵌入，以削弱生成图像中不需要的内容的影响，并且我们进一步证明了可以通过零样本方法轻松获得此delta向量。此外，我们提出了一种带有Delta向量的选择性抑制（SSDV）方法，以将delta向量适应到交叉注意力机制中，从而能够更有效地抑制原本会生成的区域中不需要的内容。此外，通过优化delta向量，我们可以在个性化的T2I模型中实现更精确的抑制，而先前的基线无法实现。

Result: 提出的方法在定量和定性指标上均显着优于现有方法。
...
### [80] [SC-Lane: Slope-aware and Consistent Road Height Estimation Framework for 3D Lane Detection](https://arxiv.org/abs/2508.10411)
*Chaesong Park,Eunbin Seo,Jihyeon Hwang,Jongwoo Lim*

Main category: cs.CV

TL;DR: SC-Lane: a novel slope-aware and temporally consistent heightmap estimation framework for 3D lane detection.


<details>
  <summary>Details</summary>
Motivation: improving robustness to diverse road geometries and ensuring stable and accurate height estimation across consecutive frames, which is crucial for real-world driving scenarios

Method: a Slope-Aware Adaptive Feature module that dynamically predicts the appropriate weights from image cues for integrating multi-slope representations into a unified heightmap. Additionally, a Height Consistency Module enforces temporal coherence

Result: achieving state-of-the-art performance with an F-score of 64.3%, outperforming existing methods by a notable margin
...
### [81] [NanoControl: A Lightweight Framework for Precise and Efficient Control in Diffusion Transformer](https://arxiv.org/abs/2508.10424)
*Shanyuan Liu,Jian Zhu,Junda Lu,Yue Gong,Liuzhuozheng Li,Bo Cheng,Yuhang Ma,Liebucha Wu,Xiaoyu Wu,Dawei Leng,Yuhui Yin*

Main category: cs.CV

TL;DR: NanoControl通过低开销的LoRA控制模块和KV-Context Augmentation，实现了高效且高质量的可控文本到图像生成。
...
Method: 采用Flux作为主干网络，并设计了一个LoRA风格的控制模块，直接从原始条件输入中学习控制信号。引入KV-Context Augmentation机制，将特定条件的键值信息集成到主干网络中。

Result: 实现了最先进的可控文本到图像生成性能，同时参数数量仅增加0.024%，GFLOPs仅增加0.029%，从而实现了高效的可控生成。
...
### [82] [STRIDE-QA: Visual Question Answering Dataset for Spatiotemporal Reasoning in Urban Driving Scenes](https://arxiv.org/abs/2508.10427)
...
Method: 构建了一个大规模视觉问答 (VQA) 数据集 STRIDE-QA，用于从自我中心角度进行物理基础推理。该数据集由在东京的 100 小时的多传感器驾驶数据构建而成，捕捉了多样化和具有挑战性的条件，STRIDE-QA 是城市驾驶中用于时空推理的最大 VQA 数据集，在 285K 帧上提供 1600 万个 QA 对。

Result: 在 STRIDE-QA 上微调的 VLM 表现出显着的性能提升，在空间定位方面实现了 55% 的成功率，在未来运动预测方面实现了 28% 的一致性，而通用 VLM 的得分接近于零。
...
### [83] [CRISP: Contrastive Residual Injection and Semantic Prompting for Continual Video Instance Segmentation](https://arxiv.org/abs/2508.10432)
*Baichen Liu,Qi Lyu,Xudong Wang,Jiahua Dong,Lianqing Liu,Zhi Han*

Main category: cs.CV

TL;DR: CRISP通过对比残差注入和语义提示来解决持续视频实例分割中的实例、类别和任务混淆问题，实验表明其性能优于现有方法。
...
Result: 在YouTube-VIS-2019和YouTube-VIS-2021数据集上的大量实验表明，

Conclusion: CRISP在长期持续的视频实例分割任务中显著优于现有的持续分割方法，避免了灾难性遗忘，并有效提高了分割和分类性能。
...
### [84] [DOD-SA: Infrared-Visible Decoupled Object Detection with Single-Modality Annotations](https://arxiv.org/abs/2508.10445)
*Hang Jin,Chenqiang Gao,Junjie Guo,Fangcen Liu,Kanghui Tian,Qinyao Chang*

Main category: cs.CV

TL;DR: Proposes a decoupled object detection framework (DOD-SA) for infrared and visible images using single-modality annotations to reduce annotation costs, achieving state-of-the-art performance.
...
Result: The proposed method outperforms state-of-the-art (SOTA) on the DroneVehicle dataset.
...
### [85] [SkeySpot: Automating Service Key Detection for Digital Electrical Layout Plans in the Construction Industry](https://arxiv.org/abs/2508.10449)
*Dhruv Dosi,Rohit Meena,Param Rajpura,Yogesh Kumar Meena*

Main category: cs.CV

TL;DR: The paper introduces SkeySpot, a YOLOv8-based toolkit for automated electrical symbol detection in scanned floor plans, achieving 82.5% mAP on the DELP dataset, and promoting interoperability and accessibility for SMEs.


<details>
  <summary>Details</summary>
Motivation: Lack of machine-readable floor plans makes large-scale interpretation time-consuming and error-prone.

Method: Use YOLOv8 to develop SkeySpot, a lightweight, open-source toolkit.

Result: YOLOv8 achieves the highest performance with a mean Average Precision (mAP) of 82.5% on the DELP dataset.

Conclusion: SkeySpot toolkit enables real-time detection, classification, and quantification of electrical symbols, producing structured outputs for interoperable building information workflows, enhancing compatibility and accessibility for SMEs.
...
### [86] [From Images to Perception: Emergence of Perceptual Properties by Reconstructing Images](https://arxiv.org/abs/2508.10450)
*Pablo Hernández-Cámara,Jesus Malo,Valero Laparra*

Main category: cs.CV

TL;DR: A bio-inspired architecture PerceptNet is optimized for image reconstruction tasks. The encoder stage exhibits the highest correlation with human perceptual judgments on image distortion. The visual system may be tuned to remove those particular levels of distortion with that level of sparsity and that biologically inspired models can learn perceptual metrics without human supervision.


<details>
  <summary>Details</summary>
Motivation: human visual perception may emerge from image statistics, shaping efficient neural representations in early vision

Method: a bio-inspired architecture that can accommodate several known facts in the retina-V1 cortex, the PerceptNet, has been end-to-end optimized for different tasks related to image reconstruction: autoencoding, denoising, deblurring, and sparsity regularization

Result: the encoder stage (V1-like layer) consistently exhibits the highest correlation with human perceptual judgments on image distortion despite not using perceptual information in the initialization or training. This alignment exhibits an optimum for moderate noise, blur and sparsity

Conclusion: biologically inspired models can learn perceptual metrics without human supervision
...
### [87] [Trajectory-aware Shifted State Space Models for Online Video Super-Resolution](https://arxiv.org/abs/2508.10453)
*Qiang Zhu,Xiandong Meng,Yuxian Jiang,Fan Zhang,David Bull,Shuyuan Zhu,Bing Zeng*

Main category: cs.CV

TL;DR: This paper presents a novel online VSR method based on Trajectory-aware Shifted SSMs (TS-Mamba), leveraging both long-term trajectory modeling and low-complexity Mamba to achieve efficient spatio-temporal information aggregation.
...
### [88] [SingleStrip: learning skull-stripping from a single labeled example](https://arxiv.org/abs/2508.10464)
*Bella Specktor-Fadida,Malte Hoffmann*

Main category: cs.CV

TL;DR: Combines domain randomization and self-training with autoencoders to train 3D skull-stripping networks with minimal labeled data.


<details>
  <summary>Details</summary>
Motivation: Deep learning segmentation relies heavily on labeled data, but manual labeling is laborious and time-consuming, especially for volumetric images like brain MRI. Domain randomization offers limited anatomical variability when very few label maps are available, and semi-supervised self-training addresses label scarcity by incorporating model predictions.

Method: Combining domain randomization with self-training to train three-dimensional skull-stripping networks using a single labeled example. This involves automated voxel intensity binning, training a convolutional autoencoder (AE) for reconstruction error assessment, and selecting top-ranking pseudo-labels to fine-tune the network.

Result: Achieved skull-stripping performance on out-of-distribution data that approaches models trained with more labeled images. AE-based ranking shows a stronger correlation with segmentation accuracy compared to consistency-based ranking under test-time augmentation.

Conclusion: Combining domain randomization and AE-based quality control enables effective semi-supervised segmentation from extremely limited labeled data, easing the labeling burden in studies involving new anatomical structures or emerging imaging techniques.
...
### [89] [Enhanced Sparse Point Cloud Data Processing for Privacy-aware Human Action Recognition](https://arxiv.org/abs/2508.10469)
*Maimunatu Tunau,Vincent Gbouna Zakka,Zhuangzhuang Dai*

Main category: cs.CV

TL;DR: This paper analyzes and enhances data processing methods for privacy-preserving human action recognition using mmWave radar, providing insights into accuracy and computational cost trade-offs.


<details>
  <summary>Details</summary>
Motivation: Traditional vision-based HAR systems raise privacy concerns, making mmWave radar sensors a privacy-preserving alternative. However, mmWave radar data is sparse and noisy, necessitating effective data processing methods. A comprehensive evaluation of existing methods is lacking.

Method: The paper evaluates DBSCAN, the Hungarian Algorithm, and Kalman Filtering individually, in pairwise combinations, and all three combined using the MiliPoint dataset. It assesses recognition accuracy and computational cost and proposes targeted enhancements to improve accuracy.

Result: The results offer crucial insights into the strengths and trade-offs of each method and their integrations.

Conclusion: This paper provides a detailed performance analysis of three data processing methods (DBSCAN, Hungarian Algorithm, and Kalman Filtering) for mmWave radar data in Human Action Recognition (HAR) systems, offering insights into their strengths, trade-offs, and potential enhancements.
...
### [90] [STAMP: Multi-pattern Attention-aware Multiple Instance Learning for STAS Diagnosis in Multi-center Histopathology Images](https://arxiv.org/abs/2508.10473)
...
<details>
  <summary>Details</summary>
Motivation: 空气间隙扩散（STAS）是肺腺癌（LUAD）的一种新型侵袭模式，与肿瘤复发和生存率降低有关。然而，LUAD中的大规模STAS诊断仍然是一项劳动密集型工作，并且由于其独特的病理学特征和形态学特征，容易出现疏忽和误诊。因此，迫切需要利用深度学习模型进行STAS诊断。

Method: 提出了一个多模式注意力感知多示例学习框架STAMP，以分析和诊断多中心组织病理学图像中STAS的存在。双分支架构引导模型从不同的语义空间学习STAS相关的病理特征。基于Transformer的实例编码和多模式注意力聚合模块动态选择与STAS病理密切相关的区域，抑制不相关的噪声，并增强全局表征的区分能力。此外，相似性正则化约束防止了跨分支的特征冗余，从而提高了整体诊断准确性。
...
### [91] [TweezeEdit: Consistent and Efficient Image Editing with Path Regularization](https://arxiv.org/abs/2508.10498)
*Jianda Mao,Kaibo Wang,Yang Xiang,Kani Chen*

Main category: cs.CV

TL;DR: TweezeEdit是一种新的图像编辑框架，它通过正则化去噪路径来保留源语义并提高编辑效率，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法在过度对齐目标提示的同时，未能充分保留源图像语义。它们从源图像的反演噪声（反演锚点）显式或隐式地生成目标图像，这种策略对于语义保留而言并非最佳，且由于编辑路径过长而效率低下。

Method: 提出TweezeEdit，这是一个无需调整和反演的框架，通过正则化整个去噪路径，确保源语义保留并缩短编辑路径。使用一致性模型，通过梯度驱动的正则化，沿着直接路径有效地注入目标提示语义。

Result: TweezeEdit在语义保留和目标对齐方面优于现有方法，编辑速度快，仅需12步（每次编辑1.6秒）。

Conclusion: TweezeEdit在语义保留和目标对齐方面表现出色，编辑速度快，仅需12步（每次编辑1.6秒）。
...
### [92] [Multi-Sample Anti-Aliasing and Constrained Optimization for 3D Gaussian Splatting](https://arxiv.org/abs/2508.10507)
*Zheng Zhou,Jia-Chen Zhang,Yu-Jie Xiong,Chun-Ming Xia*

Main category: cs.CV

TL;DR: 提出了一种新的优化框架，通过集成多重采样抗锯齿和双重几何约束来提高 Novel view synthesis 的细节保留能力。


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian splatting 的最新进展显着改进了实时 Novel view synthesis，但场景优化期间的几何约束不足通常会导致对细粒度细节的重建模糊，尤其是在具有高频纹理和锐利不连续性的区域中。

Method: 集成了多重采样抗锯齿 (MSAA) 与双重几何约束的综合优化框架

Result: 在多个基准测试中进行的大量实验评估表明，该方法在细节保留方面取得了最先进的性能，尤其是在保留高频纹理和锐利不连续性方面，同时保持了实时渲染效率。

Conclusion: 该方法在细节保留方面取得了最先进的性能，尤其是在保留高频纹理和锐利不连续性方面，同时保持了实时渲染效率。定量指标和感知研究证实，在结构相似性 (SSIM) 和感知质量 (LPIPS) 方面，相对于基线方法，该方法在统计上具有显着改进。
...
### [93] [A Segmentation-driven Editing Method for Bolt Defect Augmentation and Detection](https://arxiv.org/abs/2508.10509)
*Yangjie Xiao,Ke Zhang,Jiacun Wang,Xin Sheng,Yurong Guo,Meijuan Chen,Zehua Ren,Zhaoye Zheng,Zhenbing Zhao*

Main category: cs.CV

TL;DR: 提出了一种分割驱动的螺栓缺陷编辑方法（SBDE），以扩充数据集，提高螺栓缺陷检测的性能。


<details>
  <summary>Details</summary>
Motivation: 螺栓缺陷检测对于确保输电线路的安全至关重要。然而，缺陷图像的稀缺和数据分布的不平衡严重限制了检测性能。为了解决这个问题，我们提出了SBDE方法。

Method: 提出了一种分割驱动的螺栓缺陷编辑方法（SBDE），以扩充数据集。首先，提出了螺栓属性分割模型（Bolt-SAM），通过CLAHE-FFT适配器（CFA）和多部分感知掩码解码器（MAMD）增强了复杂螺栓属性的分割，为后续编辑任务生成高质量的掩码。其次，设计了一个掩码优化模块（MOD），并与图像修复模型（LaMa）集成，构建了螺栓缺陷属性编辑模型（MOD-LaMa），通过属性编辑将正常螺栓转换为缺陷螺栓。最后，提出了一种编辑恢复增强（ERA）策略，以恢复并将编辑后的缺陷螺栓放回原始检查场景中，并扩展缺陷检测数据集。

Result: 实验结果表明，SBDE方法生成的螺栓缺陷图像明显优于最先进的图像编辑模型，并有效提高了螺栓缺陷检测的性能。

Conclusion: SBDE方法生成的螺栓缺陷图像明显优于最先进的图像编辑模型，并有效提高了螺栓缺陷检测的性能，充分验证了该方法的有效性和应用潜力。
...
### [94] [EgoMusic-driven Human Dance Motion Estimation with Skeleton Mamba](https://arxiv.org/abs/2508.10522)
*Quang Nguyen,Nhat Le,Baoru Huang,Minh Nhat Vu,Chengcheng Tang,Van Nguyen,Ngan Le,Thieu Vo,Anh Nguyen*

Main category: cs.CV

TL;DR: 本文提出了一种新的方法，该方法可以从以自我为中心的视频和音乐中预测人体舞蹈动作。


<details>
  <summary>Details</summary>
Motivation: 从自我中心视频和音乐中共同估计人体运动的任务在很大程度上仍未被探索。自我中心视角通常会模糊身体的大部分，从而使得准确的全身姿势估计具有挑战性。此外，结合音乐需要生成的头部和身体运动与视觉和音乐输入良好对齐。

Method: EgoMusic Motion Network，它具有一个核心的 Skeleton Mamba，可以显式地捕捉人体的骨骼结构。

Result: 该方法明显优于现有技术，并且能够有效地推广到真实世界的数据。

Conclusion: 该方法优于现有技术，并且能够有效地推广到真实世界的数据。
...
### [95] [Reasoning in Computer Vision: Taxonomy, Models, Tasks, and Methodologies](https://arxiv.org/abs/2508.10523)
*Ayushman Sarkar,Mohd Yamani Idna Idris,Zhenyu Yu*

Main category: cs.CV

TL;DR: 本调查对视觉推理进行了分类，系统地检查了它们的实现，审查了评估协议，并确定了关键的开放挑战。


<details>
  <summary>Details</summary>
Motivation: 现有的调查通常孤立地解决这些方向，缺乏跨推理类型、方法和评估协议的统一分析和比较。这项调查旨在通过将视觉推理分为五种主要类型来解决这一差距。

Method: 通过图模型、记忆网络、注意力机制和神经符号系统等架构系统地检查了视觉推理的实现。

Result: 对评估协议进行了审查，这些协议旨在评估功能正确性、结构一致性和因果有效性，并批判性地分析了它们在泛化性、可重复性和解释力方面的局限性。确定了视觉推理中的关键开放挑战，包括复杂场景的可扩展性、符号和神经范式的更深层次集成、缺乏全面的基准数据集以及弱监督下的推理。
...
### [96] [Med-GLIP: Advancing Medical Language-Image Pre-training with Large-scale Grounded Dataset](https://arxiv.org/abs/2508.10528)
*Ziye Deng,Ruihan He,Jiaxiang Liu,Yuan Wang,Zijie Meng,Songtao Jiang,Yong Xie,Zuozhu Liu*

Main category: cs.CV

TL;DR: Med-GLIP：一个大型医学grounding数据集和一个模态感知框架，用于提高医学图像grounding的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的研究受到有限的模态覆盖、粗粒度注释以及缺少统一的、可推广的grounding框架的限制。

Method: 我们提出了Med-GLIP，这是一个在Med-GLIP-5M上训练的模态感知grounding框架。

Result: 我们构建了一个大型医学grounding数据集Med-GLIP-5M，该数据集包含跨七种成像模式的超过530万个区域级注释，涵盖各种解剖结构和病理发现。

Conclusion: Med-GLIP在多个基准测试中始终优于最先进的基线。将其空间输出集成到下游任务（包括医学VQA和报告生成）中，可带来可观的性能提升。
...
### [97] [GCRPNet: Graph-Enhanced Contextual and Regional Perception Network For Salient Object Detection in Optical Remote Sensing Images](https://arxiv.org/abs/2508.10542)
*Mengyu Ren,Yutong Li,Hua Li,Runmin Cong,Sam Kwong*

Main category: cs.CV

TL;DR: Proposes a GCRPNet model based on the Mamba architecture for salient object detection in optical remote sensing images, which uses a difference-similarity guided hierarchical graph attention module (DS-HGAM) and a LEVSS block to improve performance.


<details>
  <summary>Details</summary>
Motivation: Existing methods based on vision transformers (ViTs) and convolutional neural networks (CNNs) architectures aim to leverage both global and local features, but the difficulty in effectively integrating these heterogeneous features limits their overall performance.

Method: We propose a graph-enhanced contextual and regional perception network (GCRPNet), which builds upon the Mamba architecture.
...
### [98] [A Survey of Optimization Modeling Meets LLMs: Progress and Future Directions](https://arxiv.org/abs/2508.10047)
...
TL;DR: This survey reviews recent advancements in using large language models to automate mathematical modeling, identifies issues with benchmark datasets, and offers a cleaned dataset and leaderboard.
...
Method: Presents a comprehensive and timely review of recent advancements that cover the entire technical stack, including data synthesis and fine-tuning for the base model, inference frameworks, benchmark datasets, and performance evaluation. Conducted an in-depth analysis on the quality of benchmark datasets, cleaned the datasets and constructed a new leaderboard with fair performance evaluation. Build an online portal that integrates resources of cleaned datasets, code and paper repository.

Result: The quality of benchmark datasets was found to have a surprisingly high error rate. Constructed a new leaderboard with fair performance evaluation in terms of base LLM model and datasets. Build an online portal that integrates resources of cleaned datasets, code and paper repository to benefit the community.
...
### [99] [Amazon Nova AI Challenge -- Trusted AI: Advancing secure, AI-assisted software development](https://arxiv.org/abs/2508.10108)
...
TL;DR: Amazon Nova AI Challenge 旨在推动安全 AI 的进步，重点是自动化红队和安全对齐方法。


<details>
  <summary>Details</summary>
Motivation: 确保软件开发 AI 系统的安全仍然存在重大挑战。

Method: 红队使用多轮对话与竞争性 AI 编码助手进行对抗性比赛，以测试他们的安全对齐。

Result: 开发了最先进的技术，包括基于推理的安全对齐、强大的模型护栏、多轮越狱以及对大型语言模型 (LLM) 的有效探测。

Conclusion: 大学团队和 Amazon Nova AI Challenge 团队在解决软件开发 AI 的安全挑战方面取得了进展，突显了为提高 AI 安全性而进行的合作。
...
### [100] [MCP-Orchestrated Multi-Agent System for Automated Disinformation Detection](https://arxiv.org/abs/2508.10143)
*Alexandru-Andrei Avram,Adrian Groza,Alexandru Lecu*

Main category: cs.AI

TL;DR: 本文提出了一个多智能体系统，用于检测新闻文章中的虚假信息，该系统优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 数字平台上大量虚假信息的传播对信息完整性构成了重大挑战。

Method: 该论文提出了一种多智能体系统，该系统使用关系抽取来检测新闻文章中的虚假信息。该系统结合了四个智能体：机器学习智能体、维基百科知识检查智能体、连贯性检测智能体和网络抓取数据分析器。

Result: 多智能体集成系统实现了 95.3% 的准确率和 0.964 的 F1 分数，显著优于单个智能体和传统方法。

Conclusion: 多智能体集成系统显著优于传统方法，在检测假新闻方面达到了 95.3% 的准确率和 0.964 的 F1 分数。加权聚合方法优于算法阈值优化。
...
### [101] [Agentic AI Frameworks: Architectures, Protocols, and Design Challenges](https://arxiv.org/abs/2508.10146)
*Hana Derouiche,Zaki Brahmi,Haithem Mazeni*

Main category: cs.AI

TL;DR: A review and analysis of Agentic AI frameworks, identifying limitations and proposing future research directions.


<details>
  <summary>Details</summary>
Motivation: The emergence of Large Language Models (LLMs) has ushered in a transformative paradigm in artificial intelligence, Agentic AI.

Method: systematic review and comparative analysis of leading Agentic AI frameworks

Result: The paper evaluates the architectural principles, communication mechanisms, memory management, safety guardrails, and alignment with service-oriented computing paradigms of leading Agentic AI frameworks, and identifies key limitations, emerging trends, and open challenges in the field. It also conducts an in-depth analysis of agent communication protocols.
...
### [102] [Improving and Evaluating Open Deep Research Agents](https://arxiv.org/abs/2508.10152)
*Doaa Allabadi,Kyle Bradbury,Jordan M. Malof*

Main category: cs.AI

TL;DR: The paper introduces BC-Small, a computationally-tractable DRA benchmark, and benchmarks ODR and two other proprietary systems. The ODR+ model achieves a state-of-the-art 10% success rate on BC-Small.
...
Method: Strategic improvements to ODR, resulting in the ODR+ model

Result: all three systems achieve 0% accuracy on the test set of 60 questions.
...
### [103] [Pruning Long Chain-of-Thought of Large Reasoning Models via Small-Scale Preference Optimization](https://arxiv.org/abs/2508.10164)
*Bin Hong,Jiayu Liu,Zhenya Huang,Kai Zhang,Mengdi Zhang*

Main category: cs.AI

TL;DR: This paper introduces Length Controlled Preference Optimization (LCPO) to make Large Reasoning Models more efficient by reducing their output length without sacrificing reasoning quality.


<details>
  <summary>Details</summary>
Motivation: Balancing reasoning effectiveness and efficiency in Large Reasoning Models (LRMs) is challenging because their lengthy outputs increase computational costs and may lead to overthinking. Current methods often compromise reasoning quality or require extensive resources.

Method: The paper analyzes generation path distributions, filters trajectories through difficulty estimation, and analyzes the convergence behaviors of preference optimization methods. Based on this, they propose LCPO, which balances the implicit reward related to NLL loss.

Result: Length Controlled Preference Optimization (LCPO) significantly reduces the average output length by over 50% across multiple benchmarks while maintaining reasoning performance.

Conclusion: The paper proposes Length Controlled Preference Optimization (LCPO) to reduce the output length of Large Reasoning Models (LRMs) while maintaining reasoning performance. Experiments show LCPO reduces output length by over 50% across multiple benchmarks.
...
### [104] [KompeteAI: Accelerated Autonomous Multi-Agent System for End-to-End Pipeline Generation for Machine Learning Problems](https://arxiv.org/abs/2508.10177)
*Stepan Kulibaba,Artem Dzhalilov,Roman Pakhomov,Oleg Svidchenko,Alexander Gasnikov,Aleksei Shpilman*

Main category: cs.AI

TL;DR: KompeteAI, a novel AutoML framework with dynamic solution space exploration, outperforms leading methods and addresses limitations in existing benchmarks.
...
Result: KompeteAI accelerates pipeline evaluation 6.9 times and outperforms leading methods by an average of 3% on MLE-Bench.

Conclusion: KompeteAI outperforms leading methods (e.g., RD-agent, AIDE, and Ml-Master) by an average of 3% on the primary AutoML benchmark, MLE-Bench, and achieves state-of-the-art results on Kompete-bench.
...
### [105] [Extending the Entropic Potential of Events for Uncertainty Quantification and Decision-Making in Artificial Intelligence](https://arxiv.org/abs/2508.10241)
*Mark Zilberman*

Main category: cs.AI

TL;DR: entropic potential of events can enhance uncertainty quantification, decision-making, and interpretability in AI.


<details>
  <summary>Details</summary>
Motivation: This work demonstrates how the concept of the entropic potential of events can enhance uncertainty quantification, decision-making, and interpretability in artificial intelligence (AI).

Method: the framework is adapted for AI by introducing an event-centric measure that captures how actions, observations, or other discrete occurrences impact uncertainty at future time horizons. Both the original and AI-adjusted definitions of entropic potential are formalized, with the latter emphasizing conditional expectations to account for counterfactual scenarios.

Result: Applications are explored in policy evaluation, intrinsic reward design, explainable AI, and anomaly detection, highlighting the metric's potential to unify and strengthen uncertainty modeling in intelligent systems.

Conclusion: entropic potential framework offers a theoretically grounded, interpretable, and versatile approach to managing uncertainty in AI, bridging principles from thermodynamics, information theory, and machine learning.
...
### [106] [Why Cannot Large Language Models Ever Make True Correct Reasoning?](https://arxiv.org/abs/2508.10265)
*Jingde Cheng*
...
<details>
  <summary>Details</summary>
Motivation: The author believes that the claimed 'understanding ability' and 'reasoning ability' of LLMs are illusions.

Method: Explanation of the essential limitations of LLMs' working principle.
...
### [107] [Promoting Efficient Reasoning with Verifiable Stepwise Reward](https://arxiv.org/abs/2508.10293)
*Chuhuai Yue,Chengqi Dong,Yinan Gao,Hang He,Jiajun Chai,Guojun Yin,Wei Lin*

Main category: cs.AI

TL;DR: 提出了一种基于规则的可验证的逐步奖励机制 (VSRM)，以解决大型推理模型中的过度思考问题，实验表明，该方法在保持原始推理性能的同时，显著减少了输出长度。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型 (LRM) 在复杂的推理任务中取得了显著进展，但 LRM 经常会过度思考，在简单的问题上花费过多的计算，从而降低效率。现有的高效推理方法通常需要准确的任务评估来预设令牌预算或选择推理模式，这限制了它们的灵活性和可靠性。

Method: 提出了一种新颖的基于规则的可验证的逐步奖励机制 (VSRM)，该机制根据推理轨迹中中间状态的性能来分配奖励。

Result: 通过将 VSRM 与 PPO 和 Reinforce++ 相结合，在标准数学推理基准（包括 AIME24 和 AIME25）上进行了广泛的实验。结果表明，该方法在保持原始推理性能的同时，显著减少了输出长度，在效率和准确性之间取得了最佳平衡。

Conclusion: 该方法通过抑制无效步骤并鼓励有效推理，从根本上缓解了过度思考的问题。
...
### [108] [A Curriculum Learning Approach to Reinforcement Learning: Leveraging RAG for Multimodal Question Answering](https://arxiv.org/abs/2508.10337)
*Chenliang Zhang,Lin Wang,Yuanyuan Lu,Yusheng Qi,Kexin Wang,Peixu Hou,Wenshi Chen*

Main category: cs.AI

TL;DR: Dianping-Trust-Safety team won Task 1 and got 3rd in Task 3 of the META CRAG-MM challenge by using vision LLM, fine-tuning, curriculum learning, reinforcement learning, and web search APIs for multi-modal multi-turn question answering.


<details>
  <summary>Details</summary>
Motivation: The challenge requires building a comprehensive retrieval-augmented generation system capable of multi-modal multi-turn question answering.

Method: The solution is based on a vision large language model, enhanced by supervised fine-tuning, curriculum learning, and reinforcement learning. Web search APIs were also used.

Result: Achieved 1st place in Task 1 with a 52.38% lead and 3rd place in Task 3.

Conclusion: The Dianping-Trust-Safety team's system achieved 1st place in Task 1 and 3rd place in Task 3 of the META CRAG-MM challenge, demonstrating the effectiveness of their approach.
...
### [109] [Multi-Agent Trust Region Policy Optimisation: A Joint Constraint Approach](https://arxiv.org/abs/2508.10340)
*Chak Lam Shek,Guangyao Shi,Pratap Tokekar*

Main category: cs.AI

TL;DR: 提出了HATRPO-W和HATRPO-G两种方法，用于在异构多智能体强化学习中更有效地分配KL散度阈值，实验表明它们能显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 多智能体强化学习（MARL）需要在交互智能体之间进行协调和稳定的策略更新。异构智能体信任区域策略优化（HATRPO）使用Kullback-Leibler（KL）散度来执行每个智能体的信任区域约束，以稳定训练。然而，为每个智能体分配相同的KL阈值可能导致缓慢和局部最优的更新，尤其是在异构环境中。

Method: 提出了两种分配KL散度阈值的方法：HATRPO-W，一种基于Karush-Kuhn-Tucker（KKT）的方法，该方法在全局KL约束下优化阈值分配；以及HATRPO-G，一种贪婪算法，该算法基于改进-散度比对代理进行优先级排序。

Result: 实验结果表明，该方法显著提高了HATRPO的性能，在不同的MARL基准测试中实现了更快的收敛和更高的最终奖励。具体而言，HATRPO-W和HATRPO-G在最终性能方面取得了相当的改进，均超过22.5%。

Conclusion: HATRPO-W and HATRPO-G显著提升了HATRPO的性能，在不同的MARL基准测试中实现了更快的收敛和更高的最终奖励。HATRPO-W和HATRPO-G在最终性能方面取得了相当的改进，均超过22.5%。HATRPO-W也表现出更稳定的学习动态，方差更低。
...
### [110] [What to Ask Next? Probing the Imaginative Reasoning of LLMs with TurtleSoup Puzzles](https://arxiv.org/abs/2508.10358)
*Mengtao Zhou,Sifan Wu,Huan Zhang,Qi Sima,Bang Liu*

Main category: cs.AI

TL;DR: The paper introduces a new benchmark and agent for evaluating imaginative reasoning in LLMs, revealing their limitations compared to humans.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks fail to capture the dynamic, exploratory nature of imaginative reasoning process.

Method: A comprehensive research framework based on the classic "Turtle Soup" game, integrating a benchmark, an agent, and an evaluation protocol is proposed. Also, Mosaic-Agent, a novel agent designed to assess LLMs' performance is introduced. A multi-dimensional protocol measuring logical consistency, detail completion, and conclusion alignment is developed to evaluate reasoning quality.

Result: TurtleSoup-Bench, the first large-scale, bilingual, interactive benchmark for imaginative reasoning, comprising 800 turtle soup puzzles is presented.

Conclusion: Leading LLMs have capability limits, common failure patterns, and a significant performance gap compared to humans in imaginative reasoning.
...
### [111] [LeanRAG: Knowledge-Graph-Based Generation with Semantic Aggregation and Hierarchical Retrieval](https://arxiv.org/abs/2508.10391)
*Yaoze Zhang,Rong Wu,Pinlong Cai,Xiaoman Wang,Guohang Yan,Song Mao,Ding Wang,Botian Shi*

Main category: cs.AI

TL;DR: LeanRAG通过知识聚合和检索策略相结合，解决了现有知识图谱RAG方法的局限性，实现了更高的响应质量和更低的检索冗余。


<details>
  <summary>Details</summary>
Motivation: 知识图谱RAG方法存在两个关键但未解决的挑战：高层概念总结作为不连贯的“语义孤岛”存在，缺乏跨社区推理所需的显式关系；检索过程本身在结构上仍然没有感知，经常退化为低效的平面搜索，未能利用图的丰富拓扑结构。

Method: LeanRAG首先采用一种新的语义聚合算法，形成实体集群，并在聚合层面的摘要之间构建新的显式关系，创建一个完全可导航的语义网络。然后，一种自下而上的结构引导检索策略将查询锚定到最相关的细粒度实体，然后系统地遍历图的语义路径，以收集简洁但上下文全面的证据集。

Result: LeanRAG可以减轻与图上路径检索相关的巨大开销，并最大限度地减少冗余信息检索。在四个具有不同领域的具有挑战性的QA基准上进行的大量实验表明，LeanRAG在响应质量方面显著优于现有方法，同时减少了46%的检索冗余。
...
### [112] [HiRef: Leveraging Hierarchical Ontology and Network Refinement for Robust Medication Recommendation](https://arxiv.org/abs/2508.10425)
*Yan Ting Chok,Soyon Park,Seungheun Baek,Hajung Kim,Junhyun Lee,Jaewoo Kang*

Main category: cs.AI

TL;DR: HiRef combines hierarchical ontology and network refinement for robust medication recommendation, improving generalizability to unseen codes and achieving strong performance on EHR benchmarks.


<details>
  <summary>Details</summary>
Motivation: real-world EHR data present significant challenges due to the presence of rarely observed medical entities and incomplete records that may not fully capture the clinical ground truth. While data-driven models trained on longitudinal Electronic Health Records often achieve strong empirical performance, they struggle to generalize under missing or novel conditions, largely due to their reliance on observed co-occurrence patterns

Method: propose Hierarchical Ontology and Network Refinement for Robust Medication Recommendation (HiRef), a unified framework that combines two complementary structures: (i) the hierarchical semantics encoded in curated medical ontologies, and (ii) refined co-occurrence patterns derived from real-world EHRs.

Result: Our model achieves strong performance on EHR benchmarks (MIMIC-III and MIMIC-IV) and maintains high accuracy under simulated unseen-code settings.

Conclusion: The model achieves strong performance on EHR benchmarks and maintains high accuracy under simulated unseen-code settings. Extensive experiments with comprehensive ablation studies demonstrate HiRef's resilience to unseen medical codes.
...
### [113] [STEP: Stepwise Curriculum Learning for Context-Knowledge Fusion in Conversational Recommendation](https://arxiv.org/abs/2508.10669)
*Zhenye Yang,Jinpeng Chen,Huan Li,Xiongnan Jin,Xuanyang Li,Junwei Zhang,Hongbo Gao,Kaimin Wei,Senzhang Wang*

Main category: cs.AI

TL;DR: STEP: a conversational recommender centered on pre-trained language models that combines curriculum-guided context-knowledge fusion with lightweight task-specific prompt tuning to address the challenges in capturing the deep semantics of user preferences and dialogue context.
...
### [114] [MM-Food-100K: A 100,000-Sample Multimodal Food Intelligence Dataset with Verifiable Provenance](https://arxiv.org/abs/2508.10429)
*Yi Dong,Yusuke Muraoka,Scott Shi,Yi Zhang*

Main category: cs.AI

TL;DR: 发布了一个包含10万样本的多模态食品数据集MM-Food-100K，该数据集具有可验证的来源，并通过微调大型视觉语言模型验证了其在图像营养预测方面的效用。


<details>
  <summary>Details</summary>
Motivation: 构建一个具有可验证来源的大型多模态食品智能数据集。

Method: 使用Codatta贡献模型，结合社区采购和可配置的AI辅助质量检查来收集和注释数据。

Result: 在MM-Food-100K数据集上微调大型视觉语言模型（如ChatGPT 5, ChatGPT OSS, Qwen-Max）在标准指标上获得了持续的收益。

Conclusion: MM-Food-100K数据集的发布和验证，通过在图像营养预测方面对大型视觉语言模型进行微调，证明了其效用。
...
### [115] [We-Math 2.0: A Versatile MathBook System for Incentivizing Visual Mathematical Reasoning](https://arxiv.org/abs/2508.10433)
...
TL;DR: We-Math 2.0 enhances MLLMs' mathematical reasoning via knowledge integration, data space modeling, and RL-based training, achieving competitive results.
...
Result: We introduce We-Math 2.0, a unified system that integrates a structured mathematical knowledge system, model-centric data space modeling, and a reinforcement learning (RL)-based training paradigm to comprehensively enhance the mathematical reasoning abilities of MLLMs. The key contributions of We-Math 2.0 are fourfold: (1) MathBook Knowledge System (2) MathBook-Standard & Pro (3) MathBook-RL (4) MathBookEval

Conclusion: MathBook-RL performs competitively with existing baselines and achieves strong results on MathBookEval, suggesting promising generalization in mathematical reasoning.
...
### [116] [FIRESPARQL: A LLM-based Framework for SPARQL Query Generation over Scholarly Knowledge Graphs](https://arxiv.org/abs/2508.10467)
*Xueli Pan,Victor de Boer,Jacco van Ossenbruggen*

Main category: cs.AI

TL;DR: FIRESPARQL是一个用于将自然语言问题转换为SPARQL查询的框架，通过微调的LLM和可选的RAG和SPARQL查询校正层来实现。


<details>
  <summary>Details</summary>
Motivation: 由于学术内容的复杂性和这些图的复杂结构，基于学术知识图（SKG）的问答仍然是一项具有挑战性的任务。大型语言模型（LLM）方法可用于将自然语言问题（NLQ）转换为SPARQL查询；然而，由于接触SKG特定内容和底层模式的限制，这些基于LLM的方法在SPARQL查询生成方面存在困难。我们确定了LLM生成的SPARQL查询中的两种主要类型的错误：（i）结构不一致，例如查询中缺少或冗余的三元组，以及（ii）语义不准确，其中在正确的查询结构中显示了不正确的实体或属性。

Method: FIRESPARQL，一个模块化框架，支持微调的LLM作为核心组件，可选地通过检索增强生成（RAG）和SPARQL查询校正层提供上下文。
...
### [117] [SEQ-GPT: LLM-assisted Spatial Query via Example](https://arxiv.org/abs/2508.10486)
...
### [118] [Reverse Physician-AI Relationship: Full-process Clinical Diagnosis Driven by a Large Language Model](https://arxiv.org/abs/2508.10492)
*Shicheng Xu,Xin Huang,Zihao Wei,Liang Pang,Huawei Shen,Xueqi Cheng*

Main category: cs.AI

TL;DR: DxDirector-7B是一个大型语言模型，可以驱动全流程诊断，提高诊断准确性，减轻医生工作负担。


<details>
  <summary>Details</summary>
Motivation: 现有AI在临床诊断中主要作为医生的助手，无法从模糊的主诉开始驱动整个诊断过程，限制了其降低医生工作量和提高诊断效率的能力。

Method: 提出了DxDirector-7B，一个具有高级深度思考能力的大型语言模型，可以驱动全流程诊断。

Result: DxDirector-7B在罕见、复杂和真实世界的案例中，在全流程诊断设置下，实现了显著优于现有医学LLM和通用LLM的诊断准确率，并大幅减少了医生工作量。

Conclusion: DxDirector-7B在全流程诊断中表现出色，提高了诊断准确性，减轻了医生工作负担，并可能替代医学专家。
...
### [119] [PASS: Probabilistic Agentic Supernet Sampling for Interpretable and Adaptive Chest X-Ray Reasoning](https://arxiv.org/abs/2508.10501)
*Yushi Feng,Junye Du,Yingying Hong,Qifan Wang,Lequan Yu*

Main category: cs.AI

TL;DR: PASS is a multimodal framework that addresses the challenges of existing tool-augmented agentic systems in Chest X-Ray reasoning by adaptively sampling agentic workflows over a multi-tool graph, offering probability-annotated trajectories, and balancing performance and cost through a novel three-stage training procedure. It introduces CAB-E, a comprehensive benchmark for multi-step, safety-critical, free-form CXR reasoning.


<details>
  <summary>Details</summary>
Motivation: Existing tool-augmented agentic systems are limited in the real world by black-box reasoning steps, poor multimodal integration, and rigid and computationally inefficient agentic pipelines.
...
Result: Experiments across various benchmarks validate that PASS significantly outperforms strong baselines in multiple metrics. The authors also introduce CAB-E, a comprehensive benchmark for multi-step, safety-critical, free-form CXR reasoning.
...
### [120] [Diversity First, Quality Later: A Two-Stage Assumption for Language Model Alignment](https://arxiv.org/abs/2508.10530)
*Zetian Sun,Dongfang Li,Baotian Hu*

Main category: cs.AI

TL;DR: On-policy data effectiveness in LM alignment varies; the alignment process has two stages (preference injection and fine-tuning) with different data needs. An algorithm is proposed to identify boundaries between these stages.


<details>
  <summary>Details</summary>
Motivation: Aligning language models (LMs) with human preferences is critical for building reliable AI systems. On-policy sampling was introduced to improve LM alignment, but its effectiveness compared to static preference data is inconsistent.

Method: The paper proposes the alignment stage assumption and an algorithm to identify the boundaries between the preference injection and preference fine-tuning stages.

Result: The alignment stage assumption and boundary measurement generalize across 5 models (Llama, Zephyr, Phi-2, Qwen, Pythia) and 2 alignment methods (DPO, SLiC-HF).

Conclusion: The effectiveness of on-policy data in language model alignment varies significantly compared to static data, depending on the alignment stage. The alignment process can be divided into preference injection and preference fine-tuning stages, each benefiting from different data characteristics.
...
### [121] [Improving Value-based Process Verifier via Low-Cost Variance Reduction](https://arxiv.org/abs/2508.10539)
*Zetian Sun,Dongfang Li,Baotian Hu,Min Zhang*

Main category: cs.AI

TL;DR: This paper introduces ComMCS, a method that reduces variance in Monte Carlo sampling for training value-based process verifiers, improving LLM reasoning in math problems.


<details>
  <summary>Details</summary>
Motivation: Reasoning capabilities of large language models (LLMs) in complex domains like mathematics remain a significant challenge. Value-based process verifiers are a promising approach, but their effectiveness is hindered by estimation error in training annotations, primarily due to high variance from limited Monte Carlo (MC) samples.

Method: The paper proposes Compound Monte Carlo Sampling (ComMCS), which combines MC estimators from current and subsequent steps to construct an unbiased estimator with reduced variance. The method is theoretically proven to reduce variance while maintaining unbiased estimation without additional LLM inference cost.
...
Conclusion: The paper introduces ComMCS, a novel method for reducing variance in value-based process verifiers, which improves LLM reasoning in complex domains like mathematics. Empirical results on MATH-500 and GSM8K benchmarks demonstrate the effectiveness of ComMCS, outperforming existing methods.
...
### [122] [MSRS: Adaptive Multi-Subspace Representation Steering for Attribute Alignment in Large Language Models](https://arxiv.org/abs/2508.10599)
*Xinyan Jiang,Lin Zhang,Jiayi Zhang,Qingsong Yang,Guimin Hu,Di Wang,Lijie Hu*

Main category: cs.AI

TL;DR: MSRS 是一种用于有效多属性引导的新框架，它通过子空间表示微调减少属性间的干扰。
...
Method: 一种新颖的通过子空间表示微调进行有效多属性steering的框架，即多子空间表示Steering (MSRS)。

Result: MSRS 减少了属性间的干扰，在各种属性上超过了现有方法，并且有效地推广到各种下游任务。
...
### [123] [GenOM: Ontology Matching with Description Generation and Large Language Model](https://arxiv.org/abs/2508.10703)
*Yiping Song,Jiaoyan Chen,Renate A. Schmidt*

Main category: cs.AI

TL;DR: GenOM, an LLM-based ontology alignment framework, achieves competitive performance in the biomedical domain by enriching semantic representations and using embedding models.


<details>
  <summary>Details</summary>
Motivation: Ontology matching (OM) plays an essential role in enabling semantic interoperability and integration across heterogeneous knowledge sources, particularly in the biomedical domain.

Method: The GenOM framework enriches semantic representations of ontology concepts via generating textual definitions, retrieves alignment candidates with an embedding model, and incorporates exact matching-based tools.

Result: Experiments on the OAEI Bio-ML track demonstrate GenOM's competitive performance. Ablation studies confirm the effectiveness of semantic enrichment and few-shot prompting.
...
### [124] [Agentic Design Review System](https://arxiv.org/abs/2508.10745)
*Sayan Nag,K J Joseph,Koustava Goswami,Vlad I Morariu,Balaji Vasan Srinivasan*

Main category: cs.AI

TL;DR: 提出 AgenticDRS，一个通过多智能体协作分析设计，并生成可操作反馈的系统。


<details>
  <summary>Details</summary>
Motivation: 从多个方面（如对齐、构图、美学和颜色选择）评估图形设计。

Method: 基于图匹配的上下文示例选择方法和独特的提示扩展方法。

Result: 彻底的实验评估和消融实验证明了 Agentic-DRS 的有效性。
...
### [125] [Scaling Up without Fading Out: Goal-Aware Sparse GNN for RL-based Generalized Planning](https://arxiv.org/abs/2508.10747)
*Sangwoo Jeon,Juchul Shin,Gyeong-Tae Kim,YeonJe Cho,Seongwoo Kim*

Main category: cs.AI

TL;DR: 提出了一种稀疏的、目标感知的GNN表示方法，用于解决大规模通用规划任务。
...
Result: 实验结果表明，该方法可以有效地扩展到以前使用密集图表示方法不可行的大型网格尺寸，并显著提高了策略的泛化能力和成功率。

Conclusion: 该研究发现了一种稀疏的、目标感知的GNN表示方法，可以有效地扩展到以前使用密集图表示方法不可行的大型网格尺寸，并显著提高了策略的泛化能力和成功率。这些发现为解决实际的大规模通用规划任务奠定了实践基础。
...
### [126] [Modeling Human Responses to Multimodal AI Content](https://arxiv.org/abs/2508.10769)
*Zhiqi Shen,Shaojing Fan,Danni Xu,Terence Sim,Mohan Kankanhalli*

Main category: cs.AI

TL;DR: 该研究调查了人工智能生成内容对人类感知和行为的影响，提出了新的评估指标，并设计了一个可以预测人类反应的代理系统，以降低人工智能驱动的错误信息风险。


<details>
  <summary>Details</summary>
Motivation: 随着人工智能生成的内容变得越来越普遍，错误信息的风险也随之增加。先前的研究主要集中在识别内容是否真实，而对于这些内容如何影响人类的感知和行为知之甚少。在交易或股票市场等领域，预测人们的反应（例如，新闻帖子是否会走红）可能比验证其事实准确性更为重要。

Method: 该研究采用以人为本的方法，并引入了MhAIM数据集，该数据集包含154,552个在线帖子（其中111,153个是人工智能生成的），从而能够大规模分析人们对人工智能生成内容的反应。该研究提出了三个新的指标：可信度、影响力和开放性，以量化用户如何判断和参与在线内容。该研究还提出了T-Lens，这是一个基于大型语言模型的代理系统，旨在通过结合预测的人类对多模态信息的反应来回答用户查询。其核心是HR-MCP（人类反应模型上下文协议），建立在标准化的模型上下文协议（MCP）之上，能够与任何大型语言模型无缝集成。

Result: 该研究的人工研究表明，当帖子包含文本和视觉效果时，人们更擅长识别人工智能内容，特别是当两者之间存在不一致时。该研究提出三个新的指标：可信度、影响力和开放性，以量化用户如何判断和参与在线内容。

Conclusion: 该研究提供了实证性的见解和实用的工具，使大型语言模型具备人类感知能力。研究结果强调了人工智能、人类认知和信息接收之间复杂的相互作用，为降低人工智能驱动的错误信息风险提出了可操作的策略。
...
### [127] [The Knowledge-Reasoning Dissociation: Fundamental Limitations of LLMs in Clinical Natural Language Inference](https://arxiv.org/abs/2508.10777)
...
### [128] [Who Benefits from AI Explanations? Towards Accessible and Interpretable Systems](https://arxiv.org/abs/2508.10806)
*Maria J. P. Peixoto,Akriti Pandey,Ahsan Zaman,Peter R. Lewis*

Main category: cs.AI

TL;DR: XAI methods are not accessible to users with vision impairments. Simplified, multimodal explanations are needed.


<details>
  <summary>Details</summary>
Motivation: Explainability in AI is important for informed decision-making, but accessibility for users with vision impairments is underexplored.
...
### [129] [Privacy-Preserving Approximate Nearest Neighbor Search on High-Dimensional Data](https://arxiv.org/abs/2508.10373)
*Yingfan Liu,Yandi Zhang,Jiadong Xie,Hui Li,Jeffrey Xu Yu,Jiangtao Cui*

Main category: cs.DB

TL;DR: This paper presents a privacy-preserving approximate k-nearest neighbors (PP-ANNS) solution on vectors that improves data privacy, efficiency, and accuracy with minimal user involvement. It introduces a new encryption method and a privacy-preserving index, achieving significant speedup compared to existing methods.
...
Method: The paper introduces a novel solution that primarily executes PP-ANNS on a single cloud server. It uses a novel encryption method named distance comparison encryption and designs a privacy-preserving index that combines the state-of-the-art  𝑘 -ANNS method with an approximate distance computation method. A filter-and-refine search strategy is devised based on the index.

Result: The experimental results demonstrate the superiority of the proposed solution over existing solutions, achieving up to 3 orders of magnitude acceleration in PP-ANNS without compromising accuracy.
...
### [130] [Cross-Organizational Analysis of Parliamentary Processes: A Case Study](https://arxiv.org/abs/2508.10381)
...
Result: generate insights into their differences and best practices

Conclusion: This paper analyzes legislative processes of three German state parliaments and generates insights into their differences and best practices.
...
### [131] [Efficient Methods for Accurate Sparse Trajectory Recovery and Map Matching](https://arxiv.org/abs/2508.10460)
*Wei Tian,Jieming Shi,Man Lung Yiu*

Main category: cs.DB

TL;DR: 提出 TRMMA 和 MMA 用于轨迹恢复和地图匹配，实验表明效果最好。


<details>
  <summary>Details</summary>
Motivation: 现实世界的轨迹通常是稀疏的，采样率低，且与道路网络错位，但许多应用需要高质量的数据以获得最佳性能。为了提高以稀疏轨迹作为输入的数据质量，系统地研究了两个相关的研究问题：道路网络上的轨迹恢复和地图匹配。

Method: 提出 TRMMA 和 MMA 两种高效方法，分别用于精确的轨迹恢复和地图匹配。MMA 作为 TRMMA 的第一步，将稀疏轨迹中的 GPS 点映射到候选路段集合中的路段，而非整个路网。TRMMA 专注于 MMA 返回的路线中的路段，通过路段上的位置比例推断缺失点，避免评估所有路段，从而高效地生成高采样轨迹。

Result: TRMMA 和 MMA 在轨迹恢复和地图匹配方面都取得了最佳的结果质量，通常优势显著。

Conclusion: TRMMA 和 MMA 在 4 个大型真实世界数据集上，在轨迹恢复和地图匹配方面始终优于现有方法，通常优势显著。
...
### [132] [Advances in Logic-Based Entity Resolution: Enhancing ASPEN with Local Merges and Optimality Criteria](https://arxiv.org/abs/2508.10504)
*Zhliang Xiang,Meghyn Bienvenu,Gianluca Cima,Víctor Gutiérrez-Basulto,Yazmín Ibáñez-García*

Main category: cs.DB

TL;DR: ASPEN+ extends ASPEN with support for local merges and new optimality criteria for collective entity resolution.


<details>
  <summary>Details</summary>
Motivation: ASPEN only supports global merges of entity-referring constants, but local merges are often more appropriate when resolving data values.

Method: ASPEN+

Result: ASPEN+ offers new optimality criteria for selecting solutions, such as minimizing rule violations or maximising the number of rules supporting a merge.

Conclusion: formalisation and computational analysis of various notions of optimal solution, and an extensive experimental evaluation on real-world datasets, demonstrating the effect of local merges and the new optimality criteria on both accuracy and runtime.
...
### [133] [Emerging Skycube](https://arxiv.org/abs/2508.10516)
*Mickaël Martin Nevot*

Main category: cs.DB

TL;DR: 本文提出了一种新的数据分析方法，称为新兴Skycube，它结合了Skycube和新兴数据立方体的优点，并通过两种减少方法来提高计算效率。
...
Result: 与初始数据相比，新兴Skycube更便宜，因为趋势反转是在两次Skycube计算之后计算的，这大大减少了关系量。

Conclusion: 提出了新兴Skycube的概念，结合Skycube和新兴数据立方体，并提出了两种连续的减少方法来节省计算时间和存储空间。
...
### [134] [Bridging Modality Gaps in e-Commerce Products via Vision-Language Alignment](https://arxiv.org/abs/2508.10116)
*Yipeng Zhang,Hongju Yu,Aritra Mandal,Canran Xu,Qunzhi Zhou,Zhe Wu*

Main category: cs.IR

TL;DR: OPAL是一个框架，用于从图像生成符合模式、高质量的商品描述，它使用微调的多模态大型语言模型(MLLM)。


<details>
  <summary>Details</summary>
Motivation: 在电子商务中，商品信息对于有效的用户参与至关重要。然而，手动或半手动输入结构化商品信息通常会产生不一致的质量、错误和缓慢的周转时间，尤其对于C2C卖家而言。直接从商品图像生成准确的描述提供了一种有希望的替代方案。现有的基于检索的解决方案解决了一些这些问题，但经常遗漏细粒度的视觉细节，并且难以处理小众或专业类别。

Method: OPAL使用视觉指令调整与直接偏好优化相结合来微调MLLM。

Result: 在真实世界的电子商务数据集上评估了OPAL，表明它在描述质量和模式完成率方面始终优于基线方法。

Conclusion: OPAL有效地弥合了视觉和文本模态之间的差距，提供了更丰富、更准确和更一致的商品描述。这项工作推进了自动化列表优化，并支持电子商务平台中可扩展的、高质量的内容生成。
...
### [135] [DS4RS: Community-Driven and Explainable Dataset Search Engine for Recommender System Research](https://arxiv.org/abs/2508.10238)
*Xinyang Shao,Tri Kurniawan Wijaya*

Main category: cs.IR

TL;DR: 我们提出了一个推荐系统数据集搜索引擎，它支持语义搜索和社区参与，以提高数据集的可发现性和研究的可重复性。


<details>
  <summary>Details</summary>
Motivation: 由于来源分散和元数据不一致，找到与特定推荐任务或领域相匹配的数据集仍然是一个挑战。

Method: 我们提出了一个社区驱动的、可解释的、专为推荐系统研究量身定制的数据集搜索引擎。

Result: 我们的系统支持跨多个数据集属性（如数据集名称、描述和推荐领域）的语义搜索，并提供搜索相关性的解释以提高透明度。

Conclusion: 通过改进数据集的可发现性和搜索可解释性，该系统有助于更高效的研究重现。
...
### [136] [Clicks Versus Conversion: Choosing a Recommender's Training Objective in E-Commerce](https://arxiv.org/abs/2508.10377)
*Michael Weiss,Robert Rosenbach,Christian Eggenberger*

Main category: cs.IR

TL;DR: 本文比较了使用 CTR 和 OSR 作为目标的效果，结果表明优化 OSR 比优化 CTR 产生的 GMV 提升更大。


<details>
  <summary>Details</summary>
Motivation: 在电子商务中，对高点击率 (CTR) 或高转化率（如加入购物车率 (ACR) 和订单提交率 (OSR，浏览到购买的转化率)）进行排名产品推荐是标准做法。优化 CTR 看起来是一个简单的选择：训练数据（即点击数据）易于收集且通常有大量数据。另一方面，ACR 和 OSR 与商店的业务目标（如商品交易总额 (GMV)）有更直接的联系。
...
### [137] [Proxy Model-Guided Reinforcement Learning for Client Selection in Federated Recommendation](https://arxiv.org/abs/2508.10401)
*Liang Qu,Jianxin Li,Wei Yuan,Penghui Ruan,Yuhui Shi,Hongzhi Yin*

Main category: cs.IR

TL;DR: propose ProxyRL-FRS, a proxy model-guided reinforcement learning framework tailored for client selection in federated recommendation.
...
Result: achieve better performance
...
### [138] [Semantic IDs for Joint Generative Search and Recommendation](https://arxiv.org/abs/2508.10478)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）驱动的生成模型正在成为驱动推荐和搜索任务的统一解决方案。这些模型的一个关键设计选择是如何表示项目，传统上通过唯一标识符（ID），最近通过由离散代码组成的语义ID，从嵌入中获得。

Method: 比较一系列构建语义ID的策略，研究特定任务和跨任务的方法，以及在联合搜索和推荐生成模型中，每个任务是否应该有自己的语义ID令牌。

Result: 使用在搜索和推荐任务上微调的双编码器模型来获得项目嵌入，然后构建统一的语义ID空间，可以在这两项任务中实现强大的性能。

Conclusion: 使用在搜索和推荐任务上微调的双编码器模型来获得项目嵌入，然后构建统一的语义ID空间，可以在这两项任务中实现强大的性能。
...
### [139] [Efficient Patent Searching Using Graph Transformers](https://arxiv.org/abs/2508.10496)
*Krzysztof Daniell,Igor Buzhinsky,Sebastian Björkqvist*

Main category: cs.IR

TL;DR: 提出了一种基于图变换器的专利检索方法，该方法利用专利审查员的引用作为相关性信号进行训练，从而提高了检索质量和计算效率。


<details>
  <summary>Details</summary>
Motivation: 在决定是否提交新的专利申请或使现有专利无效时，找到相关的现有技术至关重要。然而，由于专利文件数量巨大，以及需要细致的比较来确定新颖性，因此寻找现有技术具有挑战性。因此，一个精确的搜索引擎对于加速这一过程是非常宝贵的。

Method: 基于图变换器的密集检索方法，其中每个发明都由描述其特征及其关系的图表示。

Result: 使用图形作为输入显着提高了处理长文档的计算效率，同时利用审查员引用允许模型学习超出简单基于文本匹配的领域特定相似性。

Conclusion: 该方法在先前技术检索质量和计算效率方面均有显着提高。
...
### [140] [DAS: Dual-Aligned Semantic IDs Empowered Industrial Recommender System](https://arxiv.org/abs/2508.10584)
*Wencai Ye,Mingjie Sun,Shaoyun Shi,Peng Wang,Wenjin Wu,Peng Jiang*

Main category: cs.IR

TL;DR: This paper introduces DAS, a one-stage method for aligning semantic IDs in recommendation systems, addressing limitations of two-stage methods by simultaneously optimizing quantization and alignment. It uses contrastive alignment and dual learning to improve performance, validated by experiments and online deployment.


<details>
  <summary>Details</summary>
Motivation: Semantic IDs, while efficient for multi-modal content integration, lack collaborative signals, leading to misalignment with recommendation objectives. Existing alignment mechanisms have limitations due to their two-stage design, causing information loss and inflexibility in applying adaptive alignment strategies.

Method: The paper proposes a one-stage Dual-Aligned Semantic IDs (DAS) method that simultaneously optimizes quantization and alignment. It incorporates a ID-based CF debias module and three contrastive alignment methods: dual user-to-item (u2i), dual item-to-item/user-to-user (i2i/u2u), and dual co-occurrence item-to-item/user-to-user (i2i/u2u). Additionally, it uses dual learning to align the dual quantizations of users and ads.

Result: DAS achieves more efficient alignment between semantic IDs and collaborative signals by maximizing mutual information and constructing stronger alignment between users and ads. It demonstrates effectiveness through offline experiments and online A/B tests.

Conclusion: The proposed DAS method's effectiveness is validated through extensive offline experiments and online A/B tests, and it has been successfully deployed across various advertising scenarios at Kuaishou App, serving over 400 million users daily.
...
### [141] [FuXi-β: Towards a Lightweight and Fast Large-Scale Generative Recommendation Model](https://arxiv.org/abs/2508.10615)
*Yufei Ye,Wei Guo,Hao Wang,Hong Zhu,Yuyang Ye,Yong Liu,Huifeng Guo,Ruiming Tang,Defu Lian,Enhong Chen*

Main category: cs.IR

TL;DR: This paper introduces FuXi-$\beta$, a new recommendation model based on FuXi-$\alpha$, which improves performance and reduces latency by introducing Functional Relative Attention Bias and removing the query-key attention map. FuXi-$\beta$ outperforms state-of-the-art models and achieves significant acceleration compared to FuXi-$\\$alpha$.


<details>
  <summary>Details</summary>
Motivation: Scaling laws for autoregressive generative recommenders reveal potential for larger, more versatile systems but mean greater latency and training costs. To accelerate training and inference, the authors investigated the recent generative recommendation models HSTU and FuXi-$\\$alpha$, identifying two efficiency bottlenecks: the indexing operations in relative temporal attention bias and the computation of the query-key attention map.  Additionally, they observed that relative attention bias in self-attention mechanisms can also serve as attention maps, raising the question of whether some attention maps are redundant, and if using the query-key attention map might degrade the model's performance in recommendation tasks.

Method: The authors introduce Functional Relative Attention Bias, which avoids the time-consuming operations of the original relative attention bias. They remove the query-key attention map from the original self-attention layer and design a new Attention-Free Token Mixer module. Furthermore, they apply this framework to FuXi-$\alpha$, and introduce a new model, FuXi-$\beta$.

Result: FuXi-$\beta$ achieves significant acceleration compared to FuXi-$\alpha$, and shows an improvement of 27% to 47% in the NDCG@10 metric on large-scale industrial datasets compared to FuXi-$\alpha$.
...
### [142] [Hypercomplex Prompt-aware Multimodal Recommendation](https://arxiv.org/abs/2508.10753)
*Zheyu Chen,Jinfeng Xu,Hewei Wang,Shuo Yang,Zitong Wan,Haibo Hu*

Main category: cs.IR

TL;DR: HPMRec: Hypercomplex Prompt-aware Multimodal Recommendation framework, which utilizes hypercomplex embeddings and the prompt-aware compensation mechanism to enhance the representation diversity of multimodal features and alleviate the over-smoothing problem.
...
### [143] [CrossDenoise: Denoising Implicit Feedback via a Lightweight Entity-Aware Synergistic Framework](https://arxiv.org/abs/2508.10851)
*Ze Liu,Xianquan Wang,Shuochen Liu,Jie Ma,Huibo Xu,Yupeng Han,Zhe Yang,Kai Zhang,Longfei Li,Jun Zhou*

Main category: cs.IR

TL;DR: CrossDenoise是一种新颖且轻量级的框架，通过将噪声估计分解为用户、项目和交互特定因素来解决这些挑战。


<details>
  <summary>Details</summary>
Motivation: 推荐系统严重依赖隐式反馈，但由于假阳性和假阴性，它本身就存在噪声，严重降低了推荐的准确性。现有的去噪策略通常忽略实体感知建模，计算开销高，或者需要过多的超参数调整，限制了它们在现实世界中的适用性。

Method: 通过将噪声估计分解为用户、项目和交互特定因素来解决这些挑战。

Result: 在ML-1M、Yelp和Amazon-book数据集上，跨GMF、NeuMF和CDAE骨干网的广泛实验表明，CrossDenoise始终显著优于最先进的基线。例如，它在Yelp上使用NeuMF实现了高达27.01%的NDCG@50增益，而计算和内存开销可以忽略不计。

Conclusion: CrossDenoise有效地分离了干净的样本和噪声样本，并在不同的超参数设置下保持稳健性。它为隐式反馈的去噪提供了一个实用且可扩展的解决方案。
...
### [144] [Personalized Product Search Ranking: A Multi-Task Learning Approach with Tabular and Non-Tabular Data](https://arxiv.org/abs/2508.09636)
...
TL;DR: 提出了一种新的模型架构，用于优化个性化产品搜索排序，通过结合表格和非表格数据以及多任务学习框架，实验结果表明该方法有效。
...
Method: 利用多任务学习框架，结合表格和非表格数据，使用预训练的TinyBERT模型进行语义嵌入，并采用一种新的抽样技术来捕捉不同的客户行为。

Result: 实验结果表明，结合非表格数据与先进的嵌入技术的多任务学习范式显著提升了模型性能。消融研究进一步强调了结合相关性标签、微调TinyBERT层以及TinyBERT查询-产品嵌入交互的益处。

Conclusion: 结合非表格数据与先进的嵌入技术的多任务学习范式显著提升了模型性能，改进了个性化产品搜索排序。
...
### [145] [OpenFPL: An open-source forecasting method rivaling state-of-the-art Fantasy Premier League services](https://arxiv.org/abs/2508.09992)
*Daniel Groos*

Main category: cs.LG

TL;DR: OpenFPL is an open-source Fantasy Premier League forecasting method that rivals commercial services in accuracy, especially for high-return players, using public data.


<details>
  <summary>Details</summary>
Motivation: Access to accurate performance forecasts gives participants an edge over competitors. High-accuracy forecasts are currently limited to commercial services whose inner workings are undisclosed and that rely on proprietary data. This paper aims to democratize access to highly accurate forecasts of player performance.
...
Result: OpenFPL achieves accuracy comparable to a leading commercial service when tested prospectively on data from the 2024-25 season. OpenFPL also surpasses the commercial benchmark for high-return players ($>$ 2 points). These findings hold across one-, two-, and three-gameweek forecast horizons.

Conclusion: OpenFPL achieves comparable accuracy to a leading commercial service and surpasses it for high-return players. These findings support long-term planning and final-day decisions.
...
### [146] [xRFM: Accurate, scalable, and interpretable feature learning models for tabular data](https://arxiv.org/abs/2508.10053)
*Daniel Beaglehole,David Holzmüller,Adityanarayanan Radhakrishnan,Mikhail Belkin*

Main category: cs.LG

TL;DR: xRFM结合了特征学习核机器与树结构，在表格数据上优于GBDT，并提供可解释性。


<details>
  <summary>Details</summary>
Motivation: 表格数据是现代技术和科学的基础，但表格数据的预测任务的最佳实践相对不变，主要基于梯度提升决策树（GBDT）的变体。最近，人们对基于神经网络和特征学习方法开发表格数据的最新方法重新产生了兴趣。

Method: 结合特征学习核机器与树结构。

Result: xRFM在100个回归数据集上实现了最佳性能，并且在超过200个分类数据集上与最佳方法相比具有竞争力，优于GBDT。

Conclusion: xRFM在100个回归数据集上表现最佳，在200个分类数据集上与最佳方法竞争，优于GBDT，并通过平均梯度外积提供可解释性。
...
### [147] [A Personalized Exercise Assistant using Reinforcement Learning (PEARL): Results from a four-arm Randomized-controlled Trial](https://arxiv.org/abs/2508.10060)
...
TL;DR: This study used a reinforcement learning (RL) algorithm to personalize physical activity nudges via a Fitbit app and found that the RL group increased their physical activity compared to other groups.


<details>
  <summary>Details</summary>
Motivation: Physical inactivity is a major global health challenge, and mHealth interventions like JITAIs offer a promising solution, but developing them at scale with robust behavioral science is difficult.

Method: A four-arm randomized controlled trial was conducted with 13,463 Fitbit users, comparing control, random, fixed, and RL nudge strategies.

Result: The RL group showed a significant increase in PA compared to all other groups at 1 and 2 months.
...
### [148] [Measuring Time Series Forecast Stability for Demand Planning](https://arxiv.org/abs/2508.10063)
*Steven Klee,Yuntian Xia*

Main category: cs.LG

TL;DR: This paper studies model-induced stochasticity and proposes the need for further study of forecast stability for models that are being deployed in production systems.
...
Method: Measure the stability and accuracy of state-of-the-art forecasting models on public data sets from the M5 competition and Favorita grocery sales.

Result: Ensemble models improve stability without significantly deteriorating forecast accuracy.

Conclusion: Ensemble models improve stability without significantly deteriorating forecast accuracy.
...
### [149] [Constrained Decoding of Diffusion LLMs with Context-Free Grammars](https://arxiv.org/abs/2508.10111)
*Niels Mündler,Jasper Dekoninck,Martin Vechev*

Main category: cs.LG

TL;DR: 提出了一种用于扩散模型的约束解码方法，该方法可以处理上下文无关文法捕获的形式语言，并在 C++ 代码填充和 JSON 结构化数据提取等各种应用中实现了近乎完美的语法正确性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型 (LLM) 在各个领域都表现出了良好的性能。LLM 的许多实际应用（例如代码完成和结构化数据提取）都需要遵守形式语言指定的语法约束。然而，由于 LLM 的概率性质，无法保证 LLM 的输出遵守此类形式语言。先前的工作已经提出了约束解码作为将 LLM 生成限制为特定形式语言的一种手段。然而，当用于实际场景（例如生成形式上正确的 C++ 或 JSON 输出）时，现有工作不适用于新兴的扩散 LLM 范例。

Method: 将约束解码简化为更一般的附加填充问题，然后将其简化为判断目标语言和正则语言的交集是否为空的任务，并提出了一种有效的算法来解决上下文无关语言的问题。

Result: 该方法实现了近乎完美的语法正确性，同时始终保持或提高功能正确性，并且计算开销保持在实用范围内。

Conclusion: 该方法在各种应用（如 C++ 代码填充和 JSON 中的结构化数据提取）上实现了接近完美的语法正确性，同时始终保持或提高功能正确性。重要的是，效率优化确保计算开销保持在实用范围内。
...
### [150] [Less is More: Learning Graph Tasks with Just LLMs](https://arxiv.org/abs/2508.10115)
...
Result: Small LLMs can learn to solve graph tasks, and this training generalizes to new tasks and graph structures.

Conclusion: Even small LLMs can learn to solve graph tasks by training them with instructive chain-of-thought solutions, and this training generalizes, without specialized graph encoders, to new tasks and graph structures.
...
### [151] [From Intent to Execution: Multimodal Chain-of-Thought Reinforcement Learning for Precise CAD Code Generation](https://arxiv.org/abs/2508.10118)
*Ke Niu,Haiyang Yu,Zhuofan Chen,Mengyang Zhao,Teng Fu,Bin Li,Xiangyang Xue*

Main category: cs.LG

TL;DR: CAD-RL: A new reinforcement learning framework for CAD modeling code generation, improving reasoning, precision, and executability.
...
### [152] [Nested-ReFT: Efficient Reinforcement Learning for Large Language Model Fine-Tuning via Off-Policy Rollouts](https://arxiv.org/abs/2508.10123)
*Maxime Heuillet,Yufei Cui,Boxing Chen,Audrey Durand,Prasanna Parthasarathi*

Main category: cs.LG

TL;DR: Nested-ReFT improves the computational efficiency of ReFT by using a subset of layers as a behavior model and dynamic layer skipping.


<details>
  <summary>Details</summary>
Motivation: The computational cost of generating completions during training with multiple inference steps makes the training cost non-trivial in standard ReFT frameworks.

Method: A novel ReFT framework, dubbed Nested-ReFT, uses a subset of layers of the target model as the behavior model to generate off-policy completions during training. Dynamic layer skipping decreases inference cost. Bias mitigation techniques minimize off-policyness in gradient updates.

Result: Improved computational efficiency (tokens/sec) across multiple math reasoning benchmarks and model sizes. Performance matches baseline ReFT performance with bias mitigation.

Conclusion: Nested-ReFT improves computational efficiency while maintaining performance on math reasoning benchmarks.
...
### [153] [rETF-semiSL: Semi-Supervised Learning for Neural Collapse in Temporal Data](https://arxiv.org/abs/2508.10147)
*Yuhan Xie,William Cappelletti,Mahsa Shoaran,Pascal Frossard*

Main category: cs.LG

TL;DR: 提出了一种新的半监督预训练策略，该策略在时间序列分类任务中优于以往的pretext任务。


<details>
  <summary>Details</summary>
Motivation: 时间序列的深度神经网络必须捕获复杂的时间模式，以有效地表示动态数据。自监督和半监督学习方法在预训练大型模型方面显示出有希望的结果，当对分类进行微调时，这些模型通常优于从头开始训练的同类模型。但是，pretext训练任务的选择通常是启发式的，并且它们向下游分类的可转移性未得到保证

Method: 提出了一种新的半监督预训练策略，以强制满足在最佳训练的神经分类器中观察到的神经崩溃现象的潜在表示。使用旋转等角紧框架分类器和伪标记来预训练具有少量标记样本的深度编码器。此外，为了在强制嵌入可分离性的同时有效地捕获时间动态，该方法集成了生成pretext任务，并定义了一种新的顺序增强策略。

Result: 该方法在lstm、transformers和状态空间模型上优于以往的pretext任务。

Conclusion: 该方法在三个多元时间序列分类数据集上显著优于先前的pretext任务，突出了将预训练目标与理论基础嵌入几何对齐的优势。
...
### [154] [Out-of-Distribution Detection using Counterfactual Distance](https://arxiv.org/abs/2508.10148)
*Maria Stoica,Francesco Leofante,Alessio Lomuscio*

Main category: cs.LG

TL;DR: This paper introduces a counterfactual explanation-based OOD detection method that is accurate, explainable and scalable.
...
Method: The paper proposes a post-hoc OOD detection method that calculates the distance to decision boundaries by leveraging counterfactual explanations. Strategies to improve scalability by computing counterfactuals directly in embedding space are also proposed.
...
Conclusion: The method achieves state-of-the-art results on CIFAR-10, CIFAR-100 and ImageNet-200 across four OOD datasets.
...
### [155] [Characterizing Evolution in Expectation-Maximization Estimates for Overspecified Mixed Linear Regression](https://arxiv.org/abs/2508.10154)
*Zhankun Luo,Abolfazl Hashemi*

Main category: cs.LG

TL;DR: 本文研究了混合线性回归中，当模型组分多于数据分布时，EM算法的收敛行为，并给出了不同混合权重下的收敛速度。


<details>
  <summary>Details</summary>
Motivation: 混合模型由于其在实践中的有效性和全面的理论基础而备受关注。一个持续存在的挑战是模型错误指定，当要拟合的模型具有比数据分布中更多的混合成分时，就会发生这种情况。

Method: 本文对过度参数化的双组分混合线性回归（2MLR）中，期望最大化（EM）算法在目标模型错误指定情况下的行为进行了理论分析。

Result: 在总体层面，对于混合权重不平衡的初始猜测，我们确定了回归参数以O(log(1/epsilon))步的线性收敛速度收敛。相反，对于混合权重平衡的初始猜测，我们观察到O(epsilon^{-2})步的亚线性收敛速度。在有限样本层面，对于具有足够不平衡的固定混合权重的混合，我们证明了O((d/n)^{1/2})的统计精度，而对于具有足够平衡的固定混合权重的混合，精度为O((d/n)^{1/4})。

Conclusion: 在线性回归混合模型（2MLR）中，针对混合权重不平衡的初始猜测，回归参数以O(log(1/epsilon))步的线性收敛速度收敛。相反，对于混合权重平衡的初始猜测，为了达到欧几里得距离上的epsilon精度，我们观察到O(epsilon^{-2})步的亚线性收敛速度。对于具有足够不平衡的固定混合权重的混合，我们证明了O((d/n)^{1/2})的统计精度，而对于具有足够平衡的固定混合权重的混合，在给定n个数据样本的情况下，精度为O((d/n)^{1/4})。
...
### [156] [Benchmark-Driven Selection of AI: Evidence from DeepSeek-R1](https://arxiv.org/abs/2508.10173)
*Petr Spelda,Vit Stritecky*

Main category: cs.LG

TL;DR: Impactful benchmarks can act as curricula for training reasoning language models, affecting generalization capabilities.


<details>
  <summary>Details</summary>
Motivation: Reasoning is becoming the next scaling dimension of large language models, and careful study of their capabilities in critical tasks is needed.

Method: Using a sequential decision-making problem from Humanity's Last Exam to show the effects on DeepSeek-R1.

Result: Demonstrates that benchmark-driven selection of AI steers development and makes novelty of test tasks key for measuring generalization capabilities.

Conclusion: Better performance in reasoning language models is not always caused by algorithmic improvements or model sizes, but also by using impactful benchmarks as curricula for learning.
...
### [157] [An Explainable AI based approach for Monitoring Animal Health](https://arxiv.org/abs/2508.10210)
*Rahul Janaa,Shubham Dixit,Mrityunjay Sharma,Ritesh Kumar*

Main category: cs.LG

TL;DR: This paper presents a data-driven approach to cattle activity monitoring using machine learning and IoT devices, achieving high accuracy in activity classification with explainable AI for practical use.


<details>
  <summary>Details</summary>
Motivation: Dairy farmers face challenges in monitoring cattle health and optimizing yield due to difficulties in tracking all animals. This work aims to showcase modern data-driven farming practices based on explainable ML methods.

Method: The study utilizes Bluetooth-based IoT devices and 4G networks for data transmission. It involves pre-processing accelerometer time series data, extracting statistical characteristics, signal processing techniques, and lag-based features using the sliding window technique. Various hyperparameter-optimized ML models are evaluated across varying window lengths for activity classification.

Result: The k-nearest neighbour Classifier achieved the best performance, with AUC of mean 0.98 and standard deviation of 0.0026 on the training set and 0.99 on the testing set.

Conclusion: The study achieved high performance in cattle activity classification using a k-nearest neighbor classifier, with AUC of mean 0.98 and standard deviation of 0.0026 on the training set and 0.99 on testing set. Explainable AI frameworks like SHAP were used to interpret feature importance.
...
### [158] [AI-Driven Detection and Analysis of Handwriting on Seized Ivory: A Tool to Uncover Criminal Networks in the Illicit Wildlife Trade](https://arxiv.org/abs/2508.10219)
*Will Fein,Ryan J. Horwitz,John E. Brown III,Amit Misra,Felipe Oviedo,Kevin White,Juan M. Lavista Ferres,Samuel K. Wasser*

Main category: cs.LG

TL;DR: 我们提出了一种人工智能驱动的管道，用于提取和分析查获的象牙上的手写标记，从而提供了一种新颖的、可扩展的和低成本的法医证据来源。


<details>
  <summary>Details</summary>
Motivation: 跨国象牙贸易继续导致非洲各地的大象数量下降，贩运网络仍然难以摧毁。执法官员查获的象牙携带着关于对其出口负责的贩运者的法医信息，包括DNA证据和贩运者的手写标记。20年来，对象牙DNA的分析已经确定了大象被偷猎的地点，并在象牙运输中建立了联系。虽然使用遗传证据建立的联系非常确凿，但遗传数据昂贵，有时无法获得。但是，虽然手写标记很容易拍摄，但很少被记录或分析。

Method: 我们使用对象检测模型从6年期间（2014-2019年）的八次大型象牙查获中收集的6,085张照片中提取了超过17,000个单独的标记，然后使用最先进的人工智能工具对这些标记进行标记和描述。

Result: 我们发现了184个重复出现的“签名标记”，这些标记连接了出现它们的象牙。在多次查获中观察到20个签名标记，通过参与这两批货物的贩运者在这些查获之间建立了法医联系。

Conclusion: 本研究证明了人工智能在野生动物取证方面的变革潜力，并强调了将笔迹分析整合到破坏有组织的野生动物犯罪的努力中的实际步骤。
...
### [159] [Confounding is a Pervasive Problem in Real World Recommender Systems](https://arxiv.org/abs/2508.10479)
*Alexander Merkov,David Rohde,Alexandre Gilotte,Benjamin Heymann*

Main category: cs.LG

TL;DR: 推荐系统中的特征工程、A/B 测试和模块化等常见做法可能会引入混淆，从而损害其性能。


<details>
  <summary>Details</summary>
Motivation: 当一个未测量的特征同时影响治疗和结果时，就会出现未观察到的混淆，导致有偏差的因果效应估计。这个问题破坏了经济学、医学、生态学或流行病学等领域的观察性研究。利用完全观察数据的推荐系统似乎不容易受到这个问题的影响。然而，推荐系统中的许多标准做法导致观察到的特征被忽略，从而导致同样的问题。

Method: 模拟研究

Result: 特征工程、A/B 测试和模块化会将混淆引入推荐系统并阻碍其性能

Conclusion: 许多常见的实践，如特征工程、A/B 测试和模块化，实际上会将混淆引入推荐系统并阻碍其性能。提供了该现象的几个例证，并辅以模拟研究，以及关于从业者如何在实际系统中减少或避免混淆影响的实用建议。
...
### [160] [Comparison of D-Wave Quantum Annealing and Markov Chain Monte Carlo for Sampling from a Probability Distribution of a Restricted Boltzmann Machine](https://arxiv.org/abs/2508.10228)
*Abdelmoula El Yazizi,Samee U. Khan,Yaroslav Koshka*

Main category: cs.LG

TL;DR: D-Wave 采样在 RBM 训练中没有显着改进，但经典-量子组合方法可能存在改进的潜力。


<details>
  <summary>Details</summary>
Motivation: 评估 D-Wave 量子退火器在 RBM 训练中的采样性能。

Method: 将基于局部谷 (LV) 的方法应用于评估受限玻尔兹曼机 (RBM) 采样的质量。

Result: 通过减少 D-Wave 退火时间，局部谷的数量没有显着增加。在任何训练时期，D-Wave 采样的状态比 Gibbs 采样属于更高数量的局部谷。两种技术在训练后期重叠较少。

Conclusion: 使用D-Wave采样未能实现显着改进，但结果表明存在改进的潜力，例如使用经典-量子组合方法。
...
### [162] [Can Transformers Break Encryption Schemes via In-Context Learning?](https://arxiv.org/abs/2508.10235)
*Jathin Korrapati,Patrick Mendoza,Aditya Tomar,Abein Abraham*

Main category: cs.LG

TL;DR: This paper proposes a novel application of ICL into the domain of cryptographic function learning, specifically focusing on ciphers such as mono-alphabetic substitution and Vigenère ciphers.


<details>
  <summary>Details</summary>
Motivation: In-context learning (ICL) has emerged as a powerful capability of transformer-based language models, enabling them to perform tasks by conditioning on a small number of examples presented at inference time, without any parameter updates. Prior work has shown that transformers can generalize over simple function classes like linear functions, decision trees, even neural networks, purely from context, focusing on numerical or symbolic reasoning over underlying well-structured functions.

Method: Apply ICL into the domain of cryptographic function learning, specifically focusing on ciphers such as mono-alphabetic substitution and Vigenère ciphers, two classes of private-key encryption schemes. Given a small set of (cipher text, plain text) pairs, the goal is for the model to infer the underlying substitution and decode a new cipher text word.

Result: Transformers can generalize over simple function classes like linear functions, decision trees, even neural networks, purely from context, focusing on numerical or symbolic reasoning over underlying well-structured functions.

Conclusion: ICL can be applied to cryptographic function learning, specifically focusing on ciphers such as mono-alphabetic substitution and Vigenère ciphers.
...
### [163] [Pruning and Malicious Injection: A Retraining-Free Backdoor Attack on Transformer Models](https://arxiv.org/abs/2508.10243)
*Taibiao Zhao,Mingxuan Sun,Hao Wang,Xiaobing Chen,Xiangwei Zhou*

Main category: cs.LG

TL;DR: This paper proposes HPMI, a novel retraining-free backdoor attack on transformers that does not alter the model's architecture. HPMI works by pruning the least important head and injecting a pre-trained malicious head to establish the backdoor.


<details>
  <summary>Details</summary>
Motivation: transformers are susceptible to backdoor attacks and Prior backdoor attack methods typically rely on retraining with clean data or altering the model architecture, both of which can be resource-intensive and intrusive.
...
### [164] [Convergence Analysis of Max-Min Exponential Neural Network Operators in Orlicz Space](https://arxiv.org/abs/2508.10248)
...
Result: 研究了单变量函数的逐点收敛和一致收敛, 使用对数连续模分析收敛阶, 并估计了相应的收敛速度. 通过合适的核函数和 Sigmoidal 激活函数, 提供了一些图形表示来说明函数的逼近误差

Conclusion: 研究了在Orlicz空间设置中Max Min Kantorovich型指数神经网络算子的收敛行为
...
### [165] [Multi-Agent Reinforcement Learning for Adaptive Resource Orchestration in Cloud-Native Clusters](https://arxiv.org/abs/2508.10253)
*Guanzi Yao,Heyao Liu,Linyan Dai*

Main category: cs.LG

TL;DR: A multi-agent reinforcement learning method is proposed to address resource dynamism and scheduling complexity in cloud-native database systems. It outperforms traditional methods in various metrics.
...
Method: adaptive resource orchestration method based on multi-agent reinforcement learning with a heterogeneous role-based agent modeling mechanism and a reward-shaping mechanism

Result: The proposed method outperforms traditional approaches across multiple key metrics, including resource utilization, scheduling latency, policy convergence speed, system stability, and fairness. The method proves effective in handling orchestration tasks with high concurrency, high-dimensional state spaces, and complex dependency relationships.

Conclusion: The proposed multi-agent reinforcement learning method outperforms traditional approaches in resource utilization, scheduling latency, policy convergence speed, system stability, and fairness, demonstrating strong generalization and practical utility in large-scale scheduling environments.
...
### [166] [Federated Anomaly Detection for Multi-Tenant Cloud Platforms with Personalized Modeling](https://arxiv.org/abs/2508.10255)
...
Result: The method outperforms existing models in Precision, Recall, and F1-Score, maintaining stability under varying conditions.
...
### [167] [Source Component Shift Adaptation via Offline Decomposition and Online Mixing Approach](https://arxiv.org/abs/2508.10257)
*Ryuta Matsuno*

Main category: cs.LG

TL;DR: Proposes a source component shift adaptation method via offline decomposition and online mixing, achieving superior adaptation performance.
...
Method: A source component shift adaptation method via an offline decomposition and online mixing approach. It determines prediction models using the EM algorithm offline and updates the mixing weight of the prediction models through online convex optimization.

Result: Theoretically identify that the problem can be divided into two subproblems: offline source component decomposition and online mixing weight adaptation. Outperforms baselines, reducing the cumulative test loss by up to 67.4%.
...
### [168] [Uncertainty-Aware Prediction of Parkinson's Disease Medication Needs: A Two-Stage Conformal Prediction Approach](https://arxiv.org/abs/2508.10284)
*Ricardo Diaz-Rincon,Muxuan Liang,Adolfo Ramirez-Zamora,Benjamin Shickel*

Main category: cs.LG

TL;DR: 该研究开发了一种预测帕金森病患者药物需求的框架，该框架考虑了预测不确定性，并提供可靠的预测区间。


<details>
  <summary>Details</summary>
Motivation: 目前的帕金森病药物管理依赖于试验和误差决策，缺乏系统的预测方法，并且机器学习的应用受到预测不确定性的限制，影响了临床信任和效用。

Method: 该研究采用了一种两阶段方法，首先识别可能需要药物改变的患者，然后预测所需的左旋多巴当量日剂量调整。

Result: 该框架实现了边际覆盖，同时减少了预测区间长度，为短期计划提供精确预测，为长期预测提供更宽范围。

Conclusion: 该研究开发了一种一致性预测框架，能够提前两年预测帕金森病患者的药物需求，并提供可靠的预测区间和统计保证，从而优化症状控制，减少副作用，提高生活质量。
...
### [169] [SynBrain: Enhancing Visual-to-fMRI Synthesis via Probabilistic Representation Learning](https://arxiv.org/abs/2508.10298)
*Weijian Mai,Jiamin Wu,Yu Zhu,Zhouheng Yao,Dongzhan Zhou,Andrew F. Luo,Qihao Zheng,Wanli Ouyang,Chunfeng Song*

Main category: cs.LG

TL;DR: SynBrain, a generative framework, simulates the transformation from visual semantics to neural responses in a probabilistic and biologically interpretable manner. It outperforms state-of-the-art methods in visual-to-fMRI encoding and adapts efficiently to new subjects with few-shot data.


<details>
  <summary>Details</summary>
Motivation: Existing deterministic methods struggle to simultaneously model this biological variability while capturing the underlying functional consistency that encodes stimulus information. To address these limitations

Method: We propose SynBrain, a generative framework that simulates the transformation from visual semantics to neural responses in a probabilistic and biologically interpretable manner. SynBrain introduces two key components: (i) BrainVAE models neural representations as continuous probability distributions via probabilistic learning while maintaining functional consistency through visual semantic constraints; (ii) A Semantic-to-Neural Mapper acts as a semantic transmission pathway, projecting visual semantics into the neural response manifold to facilitate high-fidelity fMRI synthesis.

Result: SynBrain surpasses state-of-the-art methods in subject-specific visual-to-fMRI encoding performance. Furthermore, SynBrain adapts efficiently to new subjects with few-shot data and synthesizes high-quality fMRI signals that are effective in improving data-limited fMRI-to-image decoding performance.

Conclusion: SynBrain reveals functional consistency across trials and subjects, with synthesized signals capturing interpretable patterns shaped by biological neural variability.
...
### [170] [Improving Learning of New Diseases through Knowledge-Enhanced Initialization for Federated Adapter Tuning](https://arxiv.org/abs/2508.10299)
*Danni Peng,Yuan Wang,Kangning Cai,Peiyan Ning,Jiming Xu,Yong Liu,Rick Siow Mong Goh,Qingsong Wei,Huazhu Fu*

Main category: cs.LG

TL;DR: FedKEI是一种新的联邦学习框架，它利用过去的知识来改进新任务的学习，并在各种医学数据集上表现出色。


<details>
  <summary>Details</summary>
Motivation: 在医疗保健中，联邦学习（FL）是一个广泛采用的框架，它支持医疗机构之间保护隐私的协作。大型基础模型（FM）展示了令人印象深刻的能力，通过经济高效的适配器调整在FL中使用FM已成为一种流行的方法。鉴于快速发展的医疗环境，个体客户通过调整适配器同时借鉴过去的经验来快速适应新的任务或疾病至关重要。

Method: 联邦知识增强初始化（FedKEI），利用跨客户端和跨任务的知识迁移，为使用适配器学习新任务生成知情的初始化。

Result: 在包括皮肤病学、胸部X光片和视网膜OCT在内的三种不同模态的基准数据集上进行了广泛的实验，证明了FedKEI在适应新疾病方面优于现有方法。

Conclusion: FedKEI在适应新疾病方面优于现有方法。
...
### [171] [A Vision-Language Pre-training Model-Guided Approach for Mitigating Backdoor Attacks in Federated Learning](https://arxiv.org/abs/2508.10315)
*Keke Gai,Dongjue Wang,Jing Yu,Liehuang Zhu,Qi Wu*

Main category: cs.LG

TL;DR: 本文提出了CLIP-Fed，一种新的联邦学习后门防御框架，它利用视觉-语言预训练模型，在异构数据分布下有效防御后门攻击，并在多个数据集上优于现有技术。


<details>
  <summary>Details</summary>
Motivation: 联邦学习(FL)中现有的后门防御方法依赖于同构客户端数据分布或干净服务数据集的可用性的假设，这限制了实用性和有效性。在异构客户端数据分布下防御后门攻击，同时保持模型性能仍然是一个重大挑战。

Method: 该论文提出了一个名为CLIP-Fed的FL后门防御框架，该框架利用了视觉语言预训练模型的零样本学习能力。通过整合预聚合和后聚合防御策略，CLIP-Fed克服了Non-IID对防御有效性的限制。为了解决隐私问题并增强数据集对不同触发器的覆盖，该方法使用多模态大型语言模型和频率分析构建和增强服务器数据集，而无需任何客户端样本。为了解决由后门样本引起的类原型偏差并消除触发模式和目标标签之间的相关性，CLIP-Fed使用原型对比损失和Kullback-Leibler散度对齐了全局模型和CLIP在增强数据集上的知识。

Result: CLIP-Fed在CIFAR-10上实现了平均ASR降低2.03%，在CIFAR-10-LT上实现了1.35%，同时平均MA分别提高了7.92%和0.48%。

Conclusion: CLIP-Fed在代表性数据集上通过大量实验验证了其有效性。与最先进的方法相比，CLIP-Fed在CIFAR-10上实现了平均ASR降低2.03%，在CIFAR-10-LT上实现了1.35%，同时平均MA分别提高了7.92%和0.48%。
...
### [172] [Welfare-Centric Clustering](https://arxiv.org/abs/2508.10345)
*Claire Jie Zhang,Seyed A. Esmaeili,Jamie Morgenstern*

Main category: cs.LG

TL;DR: This paper proposes welfare-centric clustering algorithms with theoretical guarantees that outperform existing fair clustering methods.


<details>
  <summary>Details</summary>
Motivation: Traditional fair clustering notions may yield undesirable outcomes, motivating a welfare-centric clustering approach that models group utilities.

Method: The paper models group utilities based on distances and proportional representation and formalizes two optimization objectives: Rawlsian (Egalitarian) and Utilitarian.

Result: The proposed methods significantly outperform existing fair clustering baselines on real-world datasets.

Conclusion: The paper introduces novel algorithms for Rawlsian and Utilitarian objectives in welfare-centric clustering and proves theoretical guarantees. Empirical results show the methods outperform existing fair clustering baselines.
...
TL;DR: Proposes a multi-level IoMT IDS framework that detects zero-day attacks with high accuracy using meta-learning and one-class classification.


<details>
  <summary>Details</summary>
Motivation: The Internet of Medical Things (IoMT) is driving a healthcare revolution but remains vulnerable to cyberattacks. Traditional centralized Intrusion Detection Systems (IDSs) are unsuitable due to response delays, privacy risks, and added vulnerabilities. Running IDSs locally on IoMT devices is often infeasible due to limited computation, and even lightweight IDS components remain at risk if updated models are delayed leaving them exposed to zero-day attacks that threaten patient health and data security.

Method: A multi level IoMT IDS framework capable of detecting zero day attacks and distinguishing between known and unknown threats. The first layer (near Edge) filters traffic at a coarse level (attack or not) using meta-learning or One Class Classification (OCC) with the usfAD algorithm. Subsequent layers (far Edge, Cloud) identify attack type and novelty.

Result: Experiments on the CICIoMT2024 dataset show 99.77 percentage accuracy and 97.8 percentage F1-score. The first layer detects zero-day attacks with high accuracy without needing new datasets, ensuring strong applicability in IoMT environments.

Conclusion: A multi-level IoMT IDS framework is proposed which can detect zero-day attacks and distinguish between known and unknown threats. Experiments on the CICIoMT2024 dataset show 99.77 percentage accuracy and 97.8 percentage F1-score. The first layer detects zero-day attacks with high accuracy without needing new datasets, ensuring strong applicability in IoMT environments.
...
### [174] [Semantic Communication with Distribution Learning through Sequential Observations](https://arxiv.org/abs/2508.10350)
*Samer Lahoud,Kinda Khawam*

Main category: cs.LG

TL;DR: 研究了语义通信中的分布学习，发现即时语义性能的优化方案通常会牺牲长期可学习性。


<details>
  <summary>Details</summary>
Motivation: 语义通信旨在传达意义，而不是完美地复制比特，代表了传统通信的范式转变。传统语义通信优化个体意义传输，但当先验未知时，学习源统计的根本条件是什么？

Method: 研究语义通信中的分布学习，接收器必须通过顺序观察来推断潜在的意义分布。

Result: 可学习性需要有效传输矩阵的满秩，描述了分布估计的收敛速度，并量化了估计误差如何转化为语义失真。为语义通信中的统计学习提供了第一个严格的表征，并为平衡即时性能与适应能力的系统提供了设计原则。

Conclusion: 系统调节对学习速率和可实现的性能都有重要影响。需要在即时性能和适应能力之间取得平衡。
...
### [175] [eMamba: Efficient Acceleration Framework for Mamba Models in Edge Computing](https://arxiv.org/abs/2508.10370)
*Jiyong Kim,Jaeho Lee,Jiahao Lin,Alish Kanani,Miao Sun,Umit Y. Ogras,Jaehyun Park*

Main category: cs.LG

TL;DR: This paper introduces eMamba, a hardware acceleration framework for deploying Mamba models on edge platforms. eMamba achieves comparable accuracy with higher efficiency compared to existing solutions.
...
Method: It maximizes computational efficiency by replacing complex normalization layers with lightweight hardware-aware alternatives and approximating expensive operations, such as SiLU activation and exponentiation, considering the target applications. Then, it performs an approximation-aware neural architecture search (NAS) to tune the learnable parameters used during approximation. Finally, it quantizes and implements the entire eMamba pipeline on an AMD ZCU102 FPGA and ASIC using GlobalFoundries (GF) 22 nm technology.

Result: eMamba achieves comparable accuracy to state-of-the-art techniques using 1.63-19.9x fewer parameters. In addition, it generalizes well to large-scale natural language tasks, demonstrating stable perplexity across varying sequence lengths on the WikiText2 dataset. Experimental results show 4.95-5.62x lower latency and 2.22-9.95x higher throughput, with 4.77x smaller area, 9.84x lower power, and 48.6x lower energy consumption than baseline solutions while maintaining competitive accuracy.

Conclusion: eMamba achieves comparable accuracy to state-of-the-art techniques using fewer parameters. It also generalizes well to large-scale natural language tasks and achieves lower latency, higher throughput, smaller area, lower power, and lower energy consumption than baseline solutions while maintaining competitive accuracy.
...
### [176] [A Unified Evaluation Framework for Multi-Annotator Tendency Learning](https://arxiv.org/abs/2508.10393)
*Liyun Zhang,Jingcheng Ke,Shenli Fan,Xuanmeng Sha,Zheng Lian*

Main category: cs.LG

TL;DR: 本文提出了一个用于评估多注释器学习中个体倾向学习方法的统一评估框架，包括两个新指标，并通过实验验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有的多注释器学习方法缺乏评估个体倾向学习（ITL）方法是否真正捕捉个体倾向并提供有意义的行为解释的评估框架。

Method: 论文提出了两种新的指标：差异性注释者一致性（DIC）和行为对齐可解释性（BAE）。

Result: 实验验证了所提出的评估框架的有效性。

Conclusion: 论文提出了一个统一的评估框架，并通过大量实验验证了其有效性。
...
### [177] [XQuant: Breaking the Memory Wall for LLM Inference with KV Cache Rematerialization](https://arxiv.org/abs/2508.10395)
...
TL;DR: XQuant通过量化和缓存层输入激活X来实现内存消耗的数量级减少，与最先进的KV缓存量化方法相比，具有显着的精度优势。


<details>
  <summary>Details</summary>
Motivation: LLM推理已成为许多下游应用的关键工作负载，但由于大量的内存占用和带宽要求，高效地推理LLM具有挑战性。同时，在过去的几十年中，计算能力稳步超过了内存容量和带宽，这种趋势在现代GPU硬件中仍然很明显，并加剧了LLM推理的挑战。

Method: 通过量化和缓存层输入激活X，而不是使用标准KV缓存，然后在推理期间动态地重新实现Key和Value来实现。

Result: XQuant实现了高达$\\\sim 7.7\\\times$的内存节省，与FP16基线相比，复杂度降低小于$0.1$。在不同的模型中，XQuant-CL实现了相对于FP16基线高达$10\\\times$的内存节省，复杂度降低仅为0.01，而内存节省$12.5\\\times$，复杂度降低仅为$0.1$。

Conclusion: XQuant利用硬件平台的快速增长的计算能力来消除内存瓶颈，同时超越了最先进的KV缓存量化方法，并在各种模型中实现了接近FP16的精度。
...
### [178] [SC2Arena and StarEvolve: Benchmark and Self-Improvement Framework for LLMs in Complex Decision-Making Tasks](https://arxiv.org/abs/2508.10428)
*Pengbo Shen,Yaqing Wang,Ni Mu,Yao Luan,Runpeng Xie,Senhao Yang,Lexiang Wang,Hao Hu,Shuang Xu,Yiqin Yang,Bo Xu*

Main category: cs.LG

TL;DR: 提出了SC2Arena基准测试和StarEvolve框架，以提高LLM在星际争霸2中的战略规划能力。


<details>
  <summary>Details</summary>
Motivation: 在复杂决策中评估大型语言模型（LLM）对于提升人工智能的战略规划和实时适应能力至关重要。然而，现有的StarCraft II等任务的基准测试未能捕捉到游戏的完整复杂性，例如完整的游戏环境、多样化的动作空间和所有可玩的种族。

Method: 提出SC2Arena基准测试，它完全支持所有可玩的种族、低级动作空间，并优化了基于文本的观察，以应对空间推理挑战。同时，引入了StarEvolve，一个分层框架，它集成了战略规划和战术执行，具有迭代自纠正和通过对高质量游戏数据进行微调的持续改进。

Result: 使用SC2Arena的综合分析为开发通用智能体提供了宝贵的见解，这在以前的基准测试中是不可能实现的。实验结果表明，我们提出的StarEvolve在战略规划中取得了优异的性能。
...
### [179] [Unpacking the Implicit Norm Dynamics of Sharpness-Aware Minimization in Tensorized Models](https://arxiv.org/abs/2508.10435)
*Tianxiao Cao,Kyohei Atarashi,Hisashi Kashima*

Main category: cs.LG

TL;DR: This paper analyzes SAM's norm dynamics in tensorized models, introduces Norm Deviation, and proposes Deviation-Aware Scaling (DAS) which outperforms SAM with less overhead.


<details>
  <summary>Details</summary>
Motivation: explore the behavior of SAM in more general tensorized models, introduce Norm Deviation as a global measure of core norm imbalance, and derive its evolution under SAM.

Method: gradient flow analysis
...
### [180] [RealAC: A Domain-Agnostic Framework for Realistic and Actionable Counterfactual Explanations](https://arxiv.org/abs/2508.10455)
*Asiful Arefeen,Shovito Barua Soumma,Hassan Ghasemzadeh*

Main category: cs.LG

TL;DR: RealAC 是一种领域无关的框架，用于生成真实且可操作的反事实，它通过对齐特征对的联合分布来自动保持复杂的特征间依赖关系，并且允许最终用户“冻结”他们无法或不希望更改的属性。


<details>
  <summary>Details</summary>
Motivation: 现有的方法通常通过刚性的、手工制作的约束或领域特定的知识来加强特征间的依赖关系，这限制了它们的通用性和捕获数据中固有的复杂、非线性关系的能力。而且，它们很少适应用户指定的偏好，并提出因果关系上不合理或不可行的解释。

Method: RealAC，一种用于生成真实且可操作的反事实的领域无关框架，通过对齐事实和反事实实例之间的特征对的联合分布，自动保持复杂的特征间依赖关系，而无需依赖显式的领域知识。该框架还允许最终用户通过在优化过程中抑制冻结特征的变化来“冻结”他们无法或不希望更改的属性。

Result: 在三个合成数据集和两个真实数据集上的评估表明，RealAC平衡了真实性与可操作性。我们的方法优于最先进的基线和基于大型语言模型的反事实生成技术。

Conclusion: RealAC在因果边缘得分、依赖保持得分和IM1真实性指标方面优于最先进的基线和基于大型语言模型的反事实生成技术，并为因果关系感知和以用户为中心的反事实生成提供了一种解决方案。
...
### [181] [X-Node: Self-Explanation is All We Need](https://arxiv.org/abs/2508.10461)
*Prajit Sengupta,Islem Rekik*

Main category: cs.LG

TL;DR: This paper introduces X-Node, a self-explaining GNN framework that generates faithful, per-node explanations while maintaining competitive classification accuracy.
...
Method: a self-explaining GNN framework in which each node generates its own explanation as part of the prediction process. For every node, we construct a structured context vector encoding interpretable cues such as degree, centrality, clustering, feature saliency, and label agreement within its local topology. A lightweight Reasoner module maps this context into a compact explanation vector
...
### [182] [GraphFedMIG: Tackling Class Imbalance in Federated Graph Learning via Mutual Information-Guided Generation](https://arxiv.org/abs/2508.10471)
*Xinrui Li,Qilin Fan,Tianfu Wang,Kaiwen Wei,Ke Yu,Xu Zhang*

Main category: cs.LG

TL;DR: GraphFedMIG通过联邦生成对抗网络和互信息引导机制，解决了联邦图学习中类不平衡的问题，并在实验中表现出优越性。


<details>
  <summary>Details</summary>
Motivation: 联邦图学习（FGL）使多个客户端能够在不共享其私有的、分散的图数据的情况下，协同训练强大的图神经网络。与通用联邦学习一样，FGL面临着统计异质性的严峻挑战，即客户端之间非独立同分布的数据分布会严重损害模型性能。其中一种特别具有破坏性的形式是类不平衡，这会导致全局模型偏向多数类，并且无法识别罕见但关键的事件。这个问题在FGL中更加严重，因为少数类中的节点通常被有偏差的邻域信息包围，从而阻碍了表达性嵌入的学习。

Method: GraphFedMIG采用分层生成对抗网络，其中每个客户端训练一个本地生成器来合成高保真特征表示。为了提供量身定制的监督，客户端被分组到集群中，每个集群共享一个专用判别器。该框架设计了一种互信息引导机制，以指导这些客户端生成器的演变。通过计算每个客户端的独特信息价值，该机制纠正了本地生成器参数，确保后续的互信息引导生成轮次侧重于生成高价值的少数类特征。

Result: 与其它基线相比，所提出的GraphFedMIG具有优越性。

Conclusion: 通过在四个真实世界数据集上进行的大量实验，结果表明，与其它基线相比，所提出的GraphFedMIG具有优越性。
...
### [183] [EDAPT: Towards Calibration-Free BCIs with Continual Online Adaptation](https://arxiv.org/abs/2508.10474)
*Lisa Haxel,Jaivardhan Kapoor,Ulf Ziemann,Jakob H. Macke*

Main category: cs.LG

TL;DR: EDAPT 通过持续模型适应消除了校准，提高了 BCI 精度，并且运行高效。


<details>
  <summary>Details</summary>
Motivation: 脑机接口 (BCI) 会因神经信号随时间推移而漂移以及用户之间的差异而导致准确性下降，这需要频繁的重新校准，从而限制了实际部署。

Method: EDAPT 首先使用来自多个用户的数据训练基线解码器，然后通过监督微调不断个性化该模型，因为神经模式在使用过程中会不断演变。

Result: 我们测试了 EDAPT 在涵盖三个 BCI 任务的九个数据集上的性能，发现它始终提高了优于传统的静态方法。这些改进主要源于结合了群体层面的预训练和在线持续微调，而无监督域适应在某些数据集上提供了进一步的收益。EDAPT 运行效率很高，可在消费级硬件上在 200 毫秒内更新模型。最后，解码精度随总数据预算而扩展，而不是其在受试者和试验之间的分配。

Conclusion: EDAPT 提供了一条通往免校准 BCI 的实用路径，减少了 BCI 部署的主要障碍。
...
### [184] [Pinet: Optimizing hard-constrained neural networks with orthogonal projection layers](https://arxiv.org/abs/2508.10480)
*Panagiotis D. Grontas,Antonio Terpin,Efe C. Balta,Raffaello D'Andrea,John Lygeros*

Main category: cs.LG

TL;DR: $\\Pi$net 是一种用于神经网络的新输出层，可确保满足凸约束，并且优于其他方法。


<details>
  <summary>Details</summary>
Motivation: 介绍了一种用于神经网络的输出层，该输出层确保满足凸约束。

Method: $\\Pi$net 利用算子分裂进行快速可靠的投影，并利用隐函数定理进行反向传播。

Result: $\\Pi$net 作为参数约束优化问题的可行性设计优化代理，在解决单个问题时，比传统求解器更快地获得适度的精度解决方案，而在批量解决问题时，速度明显更快。

Conclusion: $\\Pi$net 在训练时间、解决方案质量和超参数调整的鲁棒性方面超越了最先进的学习方法，同时保持了相似的推理时间。它还利用 $\\Pi$net 解决具有非凸轨迹偏好的多车辆运动规划问题，并提供了一个在 JAX 中实现的 GPU 就绪软件包，具有有效的调整启发式方法。
...
### [185] [Learning State-Space Models of Dynamic Systems from Arbitrary Data using Joint Embedding Predictive Architectures](https://arxiv.org/abs/2508.10489)
*Jonas Ulmen,Ganesh Sundaram,Daniel Görges*

Main category: cs.LG

TL;DR: Introduces a novel technique for creating world models using continuous-time dynamic systems from observation data, with applications in robotics control and estimation.


<details>
  <summary>Details</summary>
Motivation: The advent of Joint Embedding Predictive Architectures (JEPAs), which appear to be more capable than reconstruction-based methods.

Method: The proposed method integrates sequence embeddings with neural ordinary differential equations (neural ODEs) and employs loss functions that enforce contractive embeddings and Lipschitz constants.

Result: The approach's effectiveness is demonstrated through the generation of structured latent state-space models for a simple pendulum system using only image data.

Conclusion: This paper introduces a new technique for creating world models using continuous-time dynamic systems, opening up possibilities for more general control algorithms and estimation techniques in robotics.
...
### [186] [On the Complexity-Faithfulness Trade-off of Gradient-Based Explanations](https://arxiv.org/abs/2508.10490)
*Amir Mehrpanah,Matteo Gamba,Kevin Smith,Hossein Azizpour*

Main category: cs.LG

TL;DR: 论文提出了一个用于分析和改进 ReLU 网络解释性的频谱框架，强调了平滑性和忠实性之间的权衡。


<details>
  <summary>Details</summary>
Motivation: ReLU 网络虽然在视觉数据中很普遍，但具有明显的转换，有时依赖于单个像素进行预测，这使得vanilla基于梯度的解释noisy且难以解释。现有的方法，如GradCAM，通过生成替代模型来平滑这些解释，但代价是牺牲了忠实性。

Method: 论文引入了一个统一的频谱框架，以系统地分析和量化解释的平滑性、忠实性及其权衡。

Result: 论文量化并规范了ReLU网络对高频信息的贡献，提供了一种识别这种权衡的原则性方法。论文的分析描述了基于替代的平滑如何扭曲解释，导致一个``解释差距''，我们为不同的事后方法正式定义和测量这个差距。

Conclusion: 论文验证了不同设计选择、数据集和消融实验的理论结果。
...
### [187] [Contrastive ECOC: Learning Output Codes for Adversarial Defense](https://arxiv.org/abs/2508.10491)
*Che-Yu Chou,Hung-Hsuan Chen*

Main category: cs.LG

TL;DR: 本文提出了一种基于对比学习的自动编码本学习方法，该方法可以直接从数据中学习和自适应地学习编码本，并且在对抗攻击方面表现出更强的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统ECOC方法依赖于手动设计或随机生成的代码本，这是劳动密集型的，并且可能产生次优的、与数据集无关的结果。

Method: 基于对比学习的自动编码本学习

Result: 在四个数据集上，提出的模型与两个基线相比，表现出更强的对抗攻击鲁棒性。

Conclusion: 提出的模型在对抗攻击方面表现出更强的鲁棒性。
...
### [188] [A Unified Multi-Agent Framework for Universal Multimodal Understanding and Generation](https://arxiv.org/abs/2508.10494)
*Jiulin Li,Ping Huang,Yexin Li,Shuo Chen,Juewen Hu,Ye Tian*

Main category: cs.LG

TL;DR: MAGUS是一个用于统一多模态理解和生成的模块化框架，它优于强大的基线和最先进的系统，并在 MME 基准测试中超越了 GPT-4o。


<details>
  <summary>Details</summary>
Motivation: 现实世界的多模态应用通常需要任意到任意的能力，从而能够理解和生成跨模态的内容，包括文本、图像、音频和视频。然而，集成自回归语言模型 (LLM) 在推理方面的优势和扩散模型在高保真生成方面的优势仍然具有挑战性。现有的方法依赖于刚性的管道或紧密耦合的架构，限制了灵活性和可扩展性。

Method: MAGUS (Multi-Agent Guided Unified Multimodal System)，一个模块化框架，通过两个解耦阶段统一多模态理解和生成：认知和审议。MAGUS 支持即插即用扩展性、可扩展的任意到任意模态转换和语义对齐——所有这些都无需联合训练。

Result: MAGUS 在图像、视频和音频生成以及跨模态指令跟随等多个基准测试中优于强大的基线和最先进的系统。在 MME 基准测试中，MAGUS 超过了强大的闭源模型 GPT-4o。

Conclusion: MAGUS在多个基准测试中优于强大的基线和最先进的系统，并在 MME 基准测试中超越了强大的闭源模型 GPT-4o。
...
### [189] [Nonlocal Monte Carlo via Reinforcement Learning](https://arxiv.org/abs/2508.10520)
*Dmitrii Dobrynin,Masoud Mohseni,John Paul Strachan*

Main category: cs.LG

TL;DR: Employ deep reinforcement learning (RL) to train the nonlocal transition policies of NMC which were previously designed phenomenologically.
...
### [190] [Projected Coupled Diffusion for Test-Time Constrained Joint Generation](https://arxiv.org/abs/2508.10531)
*Hao Luan,Yi Xian Goh,See-Kiong Ng,Chun Kai Ling*

Main category: cs.LG

TL;DR: 提出了Projected Coupled Diffusion (PCD)，一个新的测试时框架，用于约束联合生成，在没有产生过多计算成本的情况下，改善了耦合效果并保证了约束满足。


<details>
  <summary>Details</summary>
Motivation: 在测试时采样中，修改已经成为扩散算法的一个重要扩展，其目标是在不必重新训练整个扩散模型的情况下，偏置生成过程以实现给定的目标。然而，在同时执行特定于任务的约束的情况下，从多个预先训练的扩散模型中生成联合相关的样本，而无需昂贵的重新训练，仍然具有挑战性。

Method: 提出了一种新的测试时框架，名为Projected Coupled Diffusion (PCD)，用于约束联合生成。PCD在生成动态中引入了一个耦合引导项，以鼓励扩散模型之间的协调，并在每个扩散步骤中加入一个投影步骤，以执行硬约束。

Result: 结果表明，该方法提高了耦合效果，并保证了约束满足，而不会产生过多的计算成本。

Conclusion: PCD在图像对生成、对象操作和多机器人运动规划的应用场景中表现出有效性，在没有产生过多计算成本的情况下，改善了耦合效果并保证了约束满足。
...
### [191] [Driving Accurate Allergen Prediction with Protein Language Models and Generalization-Focused Evaluation](https://arxiv.org/abs/2508.10541)
...
TL;DR: Applm, using the xTrimoPGLM protein language model, outperforms existing methods in allergen identification and is available as open-source software with benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: Accurately identifying allergen proteins, which pose a significant public health challenge.

Method: Applm, a computational framework that leverages the 100-billion parameter xTrimoPGLM protein language model.

Result: Applm consistently outperforms seven state-of-the-art methods in identifying novel allergens, differentiating between allergens and non-allergens among homologs, and assessing functional consequences of mutations.

Conclusion: The study confirms the importance of xTrimoPGLM in Applm's performance for detecting differences among protein sequences. Applm is provided as open-source software, and benchmark datasets are available to facilitate future research.
...
### [192] [Stabilizing Long-term Multi-turn Reinforcement Learning with Gated Rewards](https://arxiv.org/abs/2508.10548)
*Zetian Sun,Dongfang Li,Zhuoen Chen,Yuhuai Qin,Baotian Hu*

Main category: cs.LG

TL;DR: 提出了一种门控奖励累积（G-RA）方法，用于解决长程强化学习中奖励稀疏性和奖励不一致的问题，并在软件工程任务中取得了显著的改进。


<details>
  <summary>Details</summary>
Motivation: 长程强化学习任务中的奖励稀疏性是一个重大挑战，现有的基于结果的奖励塑造难以定义有意义的即时奖励，同时会引入偏差或需要显式的任务分解。

Method: Gated Reward Accumulation (G-RA)

Result: G-RA 提高了完成率 (47.6% -> 93.8% 和 22.0% -> 86.0%) 和修改率 (19.6% -> 23.8% 和 12.0% -> 42.0%)。

Conclusion: G-RA通过平衡奖励累积，在长程RL中避免了策略退化，并提供了实际的解决方案。
