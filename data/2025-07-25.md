...
Motivation: 使用自然语言接口到数据库 (NLIDB) 的应用程序系统已经普及了数据分析。但也带来了一个紧迫的挑战，即帮助那些可能在没有统计分析背景的情况下使用这些系统的用户制定无偏见的分析问题。

Method: 该方法引入了三个关键创新：(1)一个用于偏差的上下文语义映射框架；(2)一个分析框架，该框架实现了难以改变的原则，并指导用户进行系统的数据分析；(3) 一个优化的 LLM 驱动系统，该系统使用涉及多个候选、批评反馈和自我反思的结构化过程来生成高质量的、特定于任务的提示。

Result: 用户体验评估中，82.5% 的参与者表示对分析质量产生了积极影响。在比较评估中，VeriMinder 的得分显着高于其他方法，在分析的具体性、全面性和准确性指标方面至少高出 20%。

Conclusion: VeriMinder系统通过用户测试，证实了其在提高分析质量方面的优点，并且在具体性、全面性和准确性方面优于其他方法。该系统旨在帮助用户在数据分析过程中避免“错误问题”漏洞。
...
<details>
  <summary>Details</summary>
Motivation: This study investigates cross-lingual inconsistencies in Wikipedia's structured content, with a focus on tabular data.
...
Result: This leads to factual inconsistencies that can impact the neutrality and reliability of the encyclopedia and AI systems, which often rely on Wikipedia as a main training source.
...
<details>
  <summary>Details</summary>
Motivation: Prior approaches are limited by the reasoning capabilities of the model used to generate the rationales. This paper aims to enhance the reasoning ability of LLMs for simulating real human behavior in online shopping environments.

Method: The paper introduces Shop-R1, a reinforcement learning framework that decomposes human behavior simulation into rationale generation and action prediction, each guided by distinct reward signals. Rationale generation uses internal model signals, and action prediction uses a hierarchical reward structure with difficulty-aware scaling.

Result: The Shop-R1 method achieves a relative improvement of over 65% compared to the baseline.

Conclusion: Shop-R1 achieves a 65% improvement compared to the baseline by using a novel reinforcement learning framework.
...
Method: 提出动态和可泛化的过程奖励建模 (DG-PRM)，它具有奖励树来捕获和存储细粒度的多维奖励标准，并动态选择奖励信号进行逐步奖励评分。采用帕累托优势估计来识别区分性的正负样本对。

Result: DG-PRM在主流基准测试中取得了惊人的性能，通过密集奖励显著提升了跨任务的模型性能。DG-PRM能够很好地适应分布外场景，展示了卓越的泛化能力。

Conclusion: DG-PRM在主流基准测试中表现出色，通过密集奖励显著提升了跨任务的模型性能，并且很好地适应了分布外场景，展示了卓越的泛化能力。
...
Method: 使用单个 Whisper-small 编码器处理所有四个口语响应，并通过轻量级聚合器组合所有信息，并预测最终分数。

Result: 该系统实现了 0.384 的 RMSE，优于基于文本的基线 (0.44)，同时使用了最多 168M 参数。此外，该数据抽样策略允许模型仅在语料库中 44.8% 的说话者上进行训练，仍然达到 0.383 RMSE，表明在不平衡的类和强大的数据效率方面有所提高。

Conclusion: 该系统在不使用转录和分部分模型的情况下，实现了 0.384 的 RMSE，优于基于文本的基线 (0.44)，同时使用了最多 168M 参数。
...
<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型是否能更一致地根据贝叶斯定理更新他们的信念。

Method: 我们制定了一个贝叶斯一致性系数（BCC）指标，并生成一个数据集来衡量BCC。

Result: 我们的结果为我们的假设提供了证据，即更大、更强大的预训练语言模型分配的置信度与贝叶斯定理更加一致。

Conclusion: 更大的、能力更强的预训练语言模型在上下文中呈现证据时，会更加一致地根据贝叶斯定理更新他们对命题的“信念”。
...
TL;DR: A comprehensive survey of NLP research for Tigrinya, analyzing over 40 studies spanning more than a decade of work from 2011 to 2025.
...
<details>
  <summary>Details</summary>
Motivation: To significantly upgrade the performance of TeleChat models in complex reasoning, code generation, and mathematical reasoning tasks.

Method: Enhanced training strategies including pre-training, SFT, DPO, continual pretraining with domain-specific datasets, and reinforcement learning.

Result: T1-115B outperforms proprietary models like OpenAI's o1-mini and GPT-4o, while TeleChat2.5 prioritizes speed.

Conclusion: TeleChat2, TeleChat2.5, and T1 models are released, demonstrating state-of-the-art performance and empowering developers with advanced language models.
...
Method: 将现有的线性 L&E 方法建模为查询键值 (KV) 数据库，然后提出了 NeuralDB，这是一种编辑框架，它将编辑过的事实明确地表示为一个配备非线性门控检索模块的神经 KV 数据库。

Result: NeuralDB 不仅在编辑效果、泛化性、特异性、流畅性和一致性方面表现出色，而且在六个代表性的文本理解和生成任务中保持了整体性能。即使扩展到 100,000 个事实，NeuralDB 仍然有效。
...
TL;DR: LLM is leveraged to generate synthetic phrase break annotations to address the challenges of manual annotation and speech-related tasks.


<details>
  <summary>Details</summary>
Motivation: Current approaches to phrase break prediction rely on vast human annotations, incurring significant manual effort and cost. Inherent variability in the speech domain further complicates acquiring consistent, high-quality data. Recently, large language models (LLMs) have shown success in addressing data challenges in NLP by generating tailored synthetic data while reducing manual annotation needs.

Method: leveraging LLM to generate synthetic phrase break annotations and comparing with traditional annotations
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）生成的合成数据的使用日益增多，这在数据驱动的应用程序中既带来了机遇，也带来了挑战。虽然合成数据为真实世界数据提供了一种经济高效、可扩展的替代方案，以促进模型训练，但其多样性和隐私风险仍未得到充分探索。

Method: 提出了一套综合指标，用于定量评估由多个最先进的LLM生成的合成数据集的多样性和隐私性。
...
Method: 提出了一种新的混合策略，该策略将BOFT的正交稳定性与LoRA-GA的梯度对齐快速收敛动态集成。通过计算由梯度范数引导的每层自适应更新，该混合方法实现了卓越的收敛效率和跨不同任务的泛化。

Result: 在GLUE、GSM8K、MT-Bench和HumanEval四个基准上，使用参数范围从7B到405B的模型进行的实证评估表明，我们的混合方法始终优于单独的PEFT基线，在训练时间上减少高达2.1倍的资源消耗，内存使用量减少50%，同时接近完全微调的准确性。

Conclusion: 该混合方法是LLM在资源约束下实际部署的一种实用且可扩展的微调解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 最初的GloVe模型构建于2014年，已被广泛使用并被认为是有用的，但语言和世界不断发展，我们认为当前的使用可以从更新的模型中受益。此外，2014年的模型没有仔细记录所使用的确切数据版本和预处理，我们通过记录这些新模型来纠正这一点。

Method: 使用维基百科、Gigaword和Dolma的一个子集训练了两个词嵌入集。

Result: 词汇比较、直接测试和NER任务的评估表明，2024年的向量包含了新的文化和语言相关的词汇，在结构性任务（如类比和相似性）上表现相当，并在最近的、时间相关的NER数据集上表现出改进的性能，例如非西方新闻专线数据。

Conclusion: 2024 GloVe模型包含了新的文化和语言相关的词汇，在结构性任务（如类比和相似性）上表现相当，并在最近的、时间相关的NER数据集（如非西方新闻专线数据）上表现出改进的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有的大多数模型将语音仅仅视为语言内容的载体，通常忽略了人类语音中丰富的副语言和说话人特征线索，例如方言、年龄、情感和非语音发声。

Method: GOAT-SLM采用了一种双模态头部架构，该架构将语言建模与声学实现分离，从而在支持表达性和自适应语音生成的同时，实现强大的语言理解。为了提高模型效率和通用性，我们提出了一种模块化的分阶段训练策略，该策略使用大规模的语音文本语料库逐步对齐语言、副语言和说话人特征信息。

Result: GOAT-SLM在TELEVAL（一个多维评估基准）上的实验结果表明，GOAT-SLM在语义和非语义任务上都取得了平衡的性能，并且在处理情感、方言变异和年龄敏感交互方面优于现有的开源模型。

Conclusion: GOAT-SLM在语义和非语义任务上都取得了平衡的性能，并且在处理情感、方言变异和年龄敏感交互方面优于现有的开源模型。这项工作强调了对语言内容之外的内容进行建模的重要性，并推进了更自然、适应性更强和更具有社会意识的口语语言系统的发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有的评估主要集中在纯文本推理输出上，在很大程度上忽略了 MLLM 通过代码执行准确视觉操作的能力。这项工作朝着解决这一差距迈出了第一步，评估 MLLM 在多模态数学推理中基于代码的能力。

Method: 该框架侧重于两个关键评估方面：(1)多模态代码生成(MCG)和 (2)多模态代码编辑(MCE)，包括删除、修改和注释三种类型。

Result: 对九个主流 MLLM 的实验评估表明，现有模型在执行细粒度视觉操作方面仍然远落后于人类的表现。

Conclusion: 现有模型在执行细粒度视觉操作方面仍然远落后于人类的表现。
...
<details>
  <summary>Details</summary>
Motivation: Integrating LLMs into HIV clinical practice is promising but raises concerns about accuracy, potential harm, and clinician acceptance, while AI applications in HIV care remain underexplored.
...
Result: Gemini 2.5 Pro outperformed other models, but performance declined with question complexity. Medically fine-tuned models didn't always outperform general-purpose ones, and larger model size wasn't a reliable predictor of performance. Reasoning and comprehension were more challenging than factual recall, and cognitive biases were observed.

Conclusion: This study highlights the need for targeted development and evaluation to ensure safe, effective LLM integration in clinical care, as current LLMs have limitations in reasoning, comprehension, and bias, despite the potential of Gemini 2.5 Pro.
...
<details>
  <summary>Details</summary>
Motivation: 基于 Transformer 的文本嵌入模型在 NLP 任务中被广泛使用，但令人惊讶的“粘性 token”会破坏嵌入的可靠性。

Method: 提出了一种基于句子和 token 过滤的有效检测方法，即粘性 Token 检测器 (STD)。

Result: 在 14 个模型系列的 40 个检查点中，我们总共发现了 868 个粘性 token。我们的分析表明，这些 token 通常来自词汇表中特殊或未使用的条目，以及来自多语语料库的碎片子词。值得注意的是，它们的存在与模型大小或词汇量大小并不严格相关。我们进一步评估了粘性 token 如何影响下游任务（如聚类和检索），观察到性能显着下降高达 50%。

Conclusion: 发现需要更好的分词策略和模型设计，以减轻未来文本嵌入应用中粘性 token 的影响。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）可以通过利用选项位置或标签中的固有偏差在多项选择任务中获得虚高的分数，而不是展示真正的理解。

Method: 引入SCOPE评估框架，通过零提示估计模型的位置偏差分布，并根据逆偏差分布重新分配答案槽。

Result: SCOPE在多个基准实验中始终优于现有的去偏方法，在稳定性能改进方面表现更好，并且在正确选项上显示出更清晰的置信度分布。

Conclusion: SCOPE框架通过重新分配答案位置和防止语义相似的干扰项相邻放置，提高了LLM评估的公平性和可靠性。
...
<details>
  <summary>Details</summary>
Motivation: presents a formidable challenge for AI due to its complex reasoning requirements and the scarcity of realistic benchmarks
...
Method: The authors share their experience as ``ISO30401 implementers'' and explore process modeling principles in the context of ISO9001.

Result: The article provides insights on implementing a Knowledge Management System (KMS) by deploying the SECI model through the steps of PDCA cycles within an Integrated Management System.
...
<details>
  <summary>Details</summary>
Motivation: 依赖外部来源会带来安全风险，攻击者可以将中毒文档注入知识库，以引导生成过程产生有害或误导性的输出。

Method: 提出了一种新的防御方法，即基于梯度的掩码令牌概率 (GMTP)，以检测和过滤对抗性制作的文档。

Result: GMTP 能够消除超过 90% 的恶意内容，同时保留相关文档。

Conclusion: GMTP 能够消除超过 90% 的恶意内容，同时保留相关文档，从而在不同的数据集和对抗环境中保持强大的检索和生成性能。
...
<details>
  <summary>Details</summary>
Motivation: To investigate the impact of instruction-tuning on LLMs' susceptibility to misinformation, addressing the gap in research on this phenomenon.

Method: Comparative analysis of instruction-tuned LLMs and base models, examining factors like prompt structure, misinformation length, and warnings.

Result: Instruction-tuned LLMs are significantly more likely to accept misinformation from users compared to base models. User role, misinformation length, and system prompt warnings influence this susceptibility.

Conclusion: Instruction-tuning increases LLMs' susceptibility to misinformation, especially when presented by the user, highlighting the need for mitigation strategies.
...
<details>
  <summary>Details</summary>
Motivation: 删除任何层都会导致隐藏状态中出现显着幅度差距，从而导致性能大幅下降。

Method: 我们提出了 Prune&Comp，这是一种新颖的即插即用层修剪方案，它利用幅度补偿来缓解训练中这种差距。

Result: Prune&Comp 几乎将困惑度减半，并保留了原始模型 93.19% 的问答性能，比基线高出 4.01%。

Conclusion: 集成了迭代修剪和补偿循环后，Prune&Comp 持续增强现有的层修剪指标。例如，当使用流行的块影响指标修剪 LLaMA-3-8B 的 5 层时，Prune&Comp 几乎将困惑度减半，并保留了原始模型 93.19% 的问答性能，比基线高出 4.01%。
...
<details>
  <summary>Details</summary>
Motivation: 当前的直接语音翻译研究主要集中在将各种翻译知识融入到ST模型中，但是这些方法通常难以解决来自不相关噪声的干扰，并且不能充分利用翻译知识。

Method: 提出了一种新的用于术语翻译的定位和聚焦方法。

Result: 在各种数据集上的实验结果表明，该方法有效地定位了话语中的术语，提高了术语翻译的成功率。

Conclusion: 该方法有效地定位了话语中的术语，提高了术语翻译的成功率，同时保持了稳健的通用翻译性能。
...
<details>
  <summary>Details</summary>
Motivation: 由于对英语和其他高资源语言（HRL）的研究量，现在可以认为解决了拉丁及其衍生文字的印刷文本的光学字符识别（OCR）问题。然而，对于使用独特文字的低资源语言（LRL），这仍然是一个悬而未决的问题。

Method: 对六种不同的OCR引擎在两种低资源语言（LRL）：僧伽罗语和泰米尔语上进行了零样本性能的比较分析。使用五种测量技术严格分析了这些系统的性能，以评估字符和单词级别的准确性。

Result: 对僧伽罗语，Surya在所有指标上表现最佳，单词错误率（WER）为2.61%。对于泰米尔语，Document AI在所有指标上表现出色，字符错误率（CER）非常低，为0.78%。

Conclusion: Surya在所有指标上对僧伽罗语的表现最佳，WER为2.61%。相反，Document AI在所有泰米尔语的指标上表现出色，CER非常低，为0.78%。
...
<details>
  <summary>Details</summary>
Motivation: 使 LLM 适应特定的风格特征对于企业沟通至关重要，但从缺乏指令-响应格式的语料库中实现这一点具有挑战性，而且不会影响指令的遵循。

Method: 使用低秩适应 (LoRA) 将风格特征有效地转移到指令遵循模型。

Result: 跨多个数据集和模型的实验表明，在保持指令遵循的同时，风格一致性得到了提高，人工评估证实了特定于品牌的惯例。
...
<details>
  <summary>Details</summary>
Motivation: 识别了针对 LRM 的先前未探索的攻击向量，我们称之为“过度思考后门”。

Method: 通过一种新颖的数据投毒方法来实现攻击。它将可调触发器（重复次数表示所需的强度）与相应冗长的 CoT 响应配对。这些响应是通过指示教师 LLM 将受控数量的冗余细化步骤注入到正确的推理过程中来以编程方式生成的。

Result: 我们的方法可以可靠地触发推理过程长度的可控多倍增加，而不会降低最终答案的正确性。

Conclusion: 该方法可以可靠地触发推理过程长度的可控多倍增加，而不会降低最终答案的正确性。
...
<details>
  <summary>Details</summary>
Motivation: 源语句包含一个词素，其性别没有明确标记，但其目标语言等价物需要性别规范，模型必须从上下文和/或外部知识中推断出适当的性别。

Method: 使用最近提出的语义不确定性指标

Result: 在明确的实例上具有高翻译和性别准确性的模型不一定在不明确的实例上表现出预期水平的不确定性。

Conclusion: MT模型在处理性别不明确的词汇时，即使在可以使用上下文信息的情况下，也会表现出偏见，依赖于刻板印象。消除偏见对明确和不明确的翻译实例有独立的影响。
...
<details>
  <summary>Details</summary>
Motivation: ICL的有效性在很大程度上依赖于这些示例的质量，并且先前专注于增强示例检索能力的工作已经取得了令人印象深刻的性能。然而，在检索高质量示例方面仍然存在两个挑战：(1)难以区分跨任务数据分布，(2)难以在检索器输出和来自LLM的反馈之间建立细粒度的连接。
...
Result: shows significant improvements in both agreement and time-efficiency.

Conclusion: This paper contributes towards the development of scalable and robust propaganda detection systems, supporting the idea of transparent and accountable media ecosystems in line with SDG 16.
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）的评估越来越依赖于其他LLM作为评判者。然而，当前的评估范式通常产生单一的分数或排名，回答哪个模型更好，但没有说明原因。虽然对于基准测试至关重要，但这些顶层分数掩盖了模型性能背后的具体、可操作的原因。

Method: CLEAR首先生成每个实例的文本反馈，然后创建一个系统级错误问题集，并量化每个已识别问题的普遍性。该软件包还提供了一个交互式仪表板，允许通过聚合可视化进行全面的错误分析，应用交互式过滤器来隔离特定问题或分数范围，并深入到例示特定行为模式的各个实例。

Result: 我们展示了RAG和数学基准的CLEAR分析，并通过用户案例研究展示了它的实用性。

Conclusion: CLEAR是一个交互式的开源软件包，用于基于LLM的错误分析，它通过生成每个实例的文本反馈，创建系统级错误问题集，并量化每个已识别问题的普遍性来实现。
...
<details>
  <summary>Details</summary>
Motivation: 在线金融文本数据中的观点对交易决策和市场波动产生越来越深刻的影响。现有的监督微调（SFT）大型语言模型（LLMs）在金融情感分析中存在泛化能力不足的局限性。

Method: 提出了FinDPO，一种基于直接偏好优化（DPO）的金融专用LLM框架，通过logit-to-score转换将离散情感预测转换为连续、可排序的情感得分。

Result: FinDPO在标准情感分类基准测试中取得了最先进的性能，平均超过现有监督微调模型11%。

Conclusion: FinDPO在实际交易成本下，年回报率仍高达67%，夏普比率为2.0，表现出强大的风险调整后收益。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在解释结构化数据（尤其是在表格格式中）方面的能力仍然有限。阿拉伯语表格数据的基准测试仍然不足。

Method: 构建了一个混合流程，其中初始内容由LLM生成，随后由人工专家过滤和验证，以确保高质量的数据集。

Result: AraTable的初步分析表明，LLM在简单的表格任务（如直接问答）中表现尚可，但在需要更深入的推理和事实核查的任务中仍然面临重大的认知挑战。

Conclusion: LLMs在需要深度推理和事实核查的复杂表格推理任务中面临重大认知挑战，未来有很大的改进空间。该研究提供了一个有价值的公共资源和评估框架，可以帮助加速阿拉伯语结构化数据的处理和分析的基础模型的发展。
...
Method: We explore the application of transformer-based models, specifically XLM-RoBERTa-large, to automatically restore punctuation in unpunctuated Bangla text. To address the scarcity of annotated resources, we constructed a large, varied training corpus and applied data augmentation techniques.

Result: Our best-performing model achieves an accuracy of 97.1% on the News test set, 91.2% on the Reference set, and 90.2% on the ASR set. Results show strong generalization to reference and ASR transcripts, demonstrating the model's effectiveness in real-world, noisy scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Generating clinical synthetic text represents an effective solution for common clinical NLP issues like sparsity and privacy.

Method: A systematic review was conducted on generating synthetic medical free-text by formulating quantitative analysis to three research questions concerning (i) the purpose of generation, (ii) the techniques, and (iii) the evaluation methods. Databases including PubMed, ScienceDirect, Web of Science, Scopus, IEEE, Google Scholar, and arXiv were searched for publications. 94 relevant articles were identified out of 1,398 collected ones.

Result: Transformer architectures were the main predominant technique used to generate the text, especially the GPTs. There were four main aspects of evaluation, including similarity, privacy, structure, and utility, where utility was the most frequent method used to assess the generated synthetic medical text.

Conclusion: The generated synthetic medical text demonstrated a moderate possibility to act as real medical documents and has proven to be a great asset as augmented, complementary to the real documents, towards improving the accuracy and overcoming sparsity/undersampling issues. Yet, privacy is still a major issue behind generating synthetic medical text, where more human assessments are needed to check for the existence of any sensitive information.
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）在通过建模文本化的特征-值对来生成表格数据方面显示出强大的潜力。然而，表格数据本身表现出稀疏的特征级别依赖关系，其中许多特征交互在结构上是不重要的。这造成了一种根本的不匹配，因为LLM的自注意力机制不可避免地将注意力分散在所有对上，从而削弱了对关键关系的注意力，尤其是在具有复杂依赖关系或语义模糊特征的数据集中。

Method: GraDe（图引导依赖学习），一种新颖的方法，它将稀疏依赖图显式地集成到LLM的注意力机制中。GraDe采用由外部提取的功能依赖关系引导的轻量级动态图学习模块，优先考虑关键特征交互，同时抑制不相关的特征交互。

Result: 在不同的真实世界数据集上的实验表明，GraDe在复杂数据集上优于现有的基于LLM的方法高达12％，同时在合成数据质量方面取得了与最新方法相比具有竞争力的结果。

Conclusion: GraDe在复杂数据集上优于现有的基于LLM的方法，并且在合成数据质量方面与当前最优方法相比具有竞争力。它为使用LLM的结构感知表格数据建模提供了一种实用的解决方案。
...
Result: SRU-NER achieves competitive performance in biomedical and general-domain NER tasks, while improving cross-domain generalization.
...
Result: Demonstrates competitive performance across extraction and classification tasks with substantial improvements in deployment accessibility compared to LLM-based alternatives.
...
<details>
  <summary>Details</summary>
Motivation: 现有的MMT方法在利用模态差距方面面临挑战，它们强制执行严格的视觉-语言对齐，同时被限制在训练的多模态领域内进行推理。

Method: 构建多模态场景图，并引入GIIFT框架，该框架使用跨模态图注意力网络适配器。

Result: 在Multi30K数据集上的英法和英德翻译任务的实验结果表明，GIIFT超过了现有方法并达到了最先进水平。在WMT基准测试上的结果表明，GIIFT在无图像推理方面具有显著优势。
...
<details>
  <summary>Details</summary>
Motivation: 现有的DLLM存在严重的质量-速度权衡问题，更快的并行解码会导致性能显著下降。这归因于DLLM中标准解码的不可逆性，容易两极分化为错误的解码方向，并伴随早期错误上下文累积。

Method: 提出了一种名为WINO的免训练解码算法，该算法采用并行起草-验证机制。

Result: 在GSM8K数学基准测试中，WINO将推理速度提高了6倍，同时将准确率提高了2.58%；在Flickr30K字幕生成任务中，它实现了10倍的加速，并具有更高的性能。

Conclusion: WINO算法通过可撤销解码显著提升了DLLM的质量-速度权衡。
...
Method: 提出了一种新颖的SRAG-MAV框架，该框架协同集成了任务重构（TR）、自检索增强生成（SRAG）和多轮累积投票（MAV）。

Result: 该系统在STATE ToxiCN数据集上实现了26.66的Hard Score、48.35的Soft Score和37.505的平均分。

Conclusion: 该系统在STATE ToxiCN数据集上显著优于GPT-4o和微调后的Qwen2.5-7B基线模型。
...
<details>
  <summary>Details</summary>
Motivation: Large language models (LLMs) underperform in specialized domains, and existing data synthesis methods are computationally expensive or have performance limitations and insufficient generalization.

Method: AQuilt, a framework for constructing instruction-tuning data from unlabeled data by incorporating logic and inspection to enhance reasoning and self-inspection.

Result: A dataset of 703k examples is constructed, and experiments show AQuilt's performance is comparable to DeepSeek-V3 while utilizing just 17% of the production cost. The generated data exhibits higher relevance to downstream tasks.
...
<details>
  <summary>Details</summary>
Motivation: Prompt optimization improves reasoning abilities of LLMs without parameter updates. Existing methods either use textual feedback to elicit improved prompts or train a special prompt model using numerical rewards.

Method: The paper unifies training-free methods using textual feedback with methods that train a special prompt model using numerical rewards by incorporating textual feedback into the training of the prompt model.

Result: TRPrompt yields state-of-the-art query-specific prompts for problems from the challenging math datasets GSMHard and MATH.

Conclusion: The paper introduces the Textual Reward Prompt framework (TRPrompt) which trains a prompt model using textual feedback, achieving state-of-the-art query-specific prompts on challenging math datasets.
...
Result: RLCF improves performance on every benchmark, including a 4-point boost on FollowBench, a 6-point increase on InFoBench, and a 3-point rise on Arena-Hard

Conclusion: checklist feedback is a key tool for improving language models' support of queries that express a multitude of needs
...
TL;DR: Lumina-mGPT 2.0 是一个从头开始训练的自回归模型，它在图像生成方面与 DALL-E 3 等模型竞争，并具有多任务能力。


<details>
  <summary>Details</summary>
Motivation: 重新审视并重振用于高质量图像生成及其他应用的自回归范式。现有方法依赖于预训练组件或混合架构，限制了架构设计和许可自由。

Method: 从头开始训练一个独立的、仅解码器的自回归模型，并结合了高效的解码策略，如推理时缩放和推测雅可比采样。

Result: 在图像生成质量上与最先进的扩散模型（如 DALL-E 3 和 SANA）相当，在某些情况下超过了基于扩散的模型。在 Graph200K 基准测试中表现出色，确认了其多任务能力。

Conclusion: Lumina-mGPT 2.0 是一个强大的、灵活的基础模型，适用于统一的多模态生成。
...
TL;DR: SV3.3B 是一种轻量级模型，用于高效的设备端体育视频分析，在性能上优于大型模型。


<details>
  <summary>Details</summary>
Motivation: 传统的体育视频分析方法计算密集，依赖服务器端处理，缺乏对运动员动作的细粒度理解，难以捕捉到准备、执行和完成等关键阶段的细微生物力学转变。

Method: 一种轻量级的 3.3B 参数视频理解模型，结合了时间运动差异采样和自监督学习，采用 DWT-VGG16-LDA 关键帧提取机制，以及 V-DWT-JEPA2 编码器和 LLM 解码器。

Result: SV3.3B 在 NSVA 篮球数据集子集上表现优异，在传统文本生成指标和运动特定评估标准上均优于大型闭源模型（包括 GPT-4o 变体），同时保持了更低的计算需求。在真实性验证指标上，SV3.3B 比 GPT-4o 提高了 29.2%。

Conclusion: SV3.3B 模型在运动描述生成方面表现出色，优于 GPT-4o，并在信息密度、动作复杂性和测量精度等运动分析关键指标上取得了显著提升。
...
TL;DR: Detail++: a training-free framework that introduces a novel Progressive Detail Injection (PDI) strategy to address the limitations of handling complex prompt, particularly those involving multiple subjects with distinct attributes.


<details>
  <summary>Details</summary>
Motivation: these models still face significant challenges when handling complex prompt, particularly those involving multiple subjects with distinct attributes. Inspired by the human drawing process, which first outlines the composition and then incrementally adds details

Method: a training-free framework that introduces a novel Progressive Detail Injection (PDI) strategy

Result: This staged generation leverages the inherent layout-controlling capacity of self-attention to first ensure global composition, followed by precise refinement. To achieve accurate binding between attributes and corresponding subjects, we exploit cross-attention mechanisms and further introduce a Centroid Alignment Loss at test time to reduce binding noise and enhance attribute consistency.

Conclusion: Detail++ significantly outperforms existing methods, particularly in scenarios involving multiple objects and complex stylistic conditions.
...
TL;DR: FishDet-M是最大的统一鱼类检测基准，它统一了13个公开可用的数据集，并对28个目标检测模型进行了基准测试。
...
Method: 我们系统地对28个当代目标检测模型进行了基准测试，包括YOLOv8到YOLOv12系列、基于R-CNN的检测器和基于DETR的模型。

Result: 结果突出了在FishDet-M上训练的模型中不同的检测性能，以及不同架构的模型在准确性和效率之间的权衡。为了支持自适应部署，我们引入了一个基于CLIP的模型选择框架，该框架利用视觉-语言对齐来动态识别每个输入图像最适合语义的检测器。

Conclusion: FishDet-M建立了一个标准化的、可复现的平台，用于评估复杂水生场景中的目标检测。所有数据集、预训练模型和评估工具都是公开可用的，以促进水下计算机视觉和智能海洋系统方面的未来研究。
...
TL;DR: 利用生成式人工智能评估黑色素瘤分类器的公平性，发现使用合成数据进行公平性评估是一个很有希望的方向，但当评估模型在不同的数据集上训练时，验证公平性变得困难。


<details>
  <summary>Details</summary>
Motivation: 深度学习及其在边缘计算上的应用在彻底改变黑色素瘤等皮肤癌的常规筛查方面具有巨大潜力。伴随这项技术的预期好处而来的是，潜在的危险来自不可预见的和固有的偏见。因此，评估和提高此类系统的公平性至关重要。公平性评估的一个关键挑战是确保评估数据集充分代表不同的个人身份信息 (PII)（性别、年龄和种族）和其他少数群体。

Method: 利用最先进的 Generative AI (GenAI) LightningDiT 模型来评估公开可用的黑色素瘤分类器的公平性。

Result: 结果表明，使用高度真实的合成数据进行公平性评估是一个很有希望的方向。研究结果表明，当用于评估的黑色素瘤检测模型在与支持合成图像的数据集不同的数据上训练时，验证公平性变得困难。

Conclusion: 使用高度真实的合成数据进行公平性评估是一个很有希望的方向。当用于评估的黑色素瘤检测模型在与支持合成图像的数据集不同的数据上训练时，验证公平性变得困难。提出的方法为使用合成数据来衡量和提高医学成像 GenAI 系统的公平性提供了一个有价值的新途径。
...
TL;DR: This paper introduces DiNAT-IR, a transformer-based architecture for image restoration that uses dilated neighborhood attention and a channel-aware module to achieve competitive results on multiple benchmarks.
...
Result: achieves competitive results across multiple benchmarks, offering a high-quality solution for diverse low-level computer vision problems.
...
TL;DR: Adaptive Feature Refinement (AFR) module enhances segmentation accuracy by refining high-resolution features using semantic priors from low-resolution logits and adaptively balances local and global information through uncertaintydriven attention, reducing misclassifications
...
Result: improves existing UDA-SS methods by 1.05% mIoU on GTA V --> Cityscapes and 1.04% mIoU on Synthia-->Cityscapes
...
TL;DR: OPEN 是一个用于支持 AI 驱动的参与度识别的新型数据集。


<details>
  <summary>Details</summary>
Motivation: 在虚拟学习中投入对于参与者的满意度、表现和依从性至关重要，尤其是在在线教育和虚拟康复中，在这些环境中，互动交流起着关键作用。然而，准确衡量虚拟群体环境中的参与度仍然是一个挑战。

Method: 收集自 11 名老年人，他们在为期六周的每周虚拟小组学习课程中参与了心脏康复，产生了超过 35 小时的数据，使其成为同类数据集中最大的。

Result: 多个机器学习和深度学习模型接受了训练，实现了高达 81% 的参与度识别准确率。

Conclusion: OPEN 提供了一个可扩展的基础，用于对老年人群进行个性化参与度建模，并为更广泛的参与度识别研究做出贡献。
...
TL;DR: Automated system using YOLOv8s for real-time video analysis of bearded dragon behavior, improving efficiency but needing better cricket detection.


<details>
  <summary>Details</summary>
Motivation: Traditional monitoring of bearded dragon behavior is time-consuming and prone to errors.

Method: YOLO object detection models were trained on a custom dataset to identify basking and hunting behaviors, with YOLOv8s selected as the optimal model.

Result: Basking detection proved reliable, but hunting detection was less accurate due to weak cricket detection. Future improvements will focus on enhancing cricket detection.

Conclusion: An automated system for reptile behavior monitoring was developed, offering a scalable solution for improving research efficiency and data quality.
...
TL;DR: 提出了首个空中地面跨模态视频行人重识别数据集AG-VPReID.VIR，并提出了新的三流架构TCC-VPReID。


<details>
  <summary>Details</summary>
Motivation: 现有的数据集主要集中在地面视角，而地面红外系统存在遮挡、覆盖范围有限和易受障碍物影响等问题，但空中视角可以独特地解决这些问题。

Method: 提出了一种新的三流架构TCC-VPReID，旨在解决跨平台和跨模态行人重识别的联合挑战。通过风格稳健的特征学习、基于记忆的跨视角适应和中间引导的时间建模来弥合空中地面视角和RGB-IR模态之间的域差距。

Result: AG-VPReID.VIR数据集提出了独特的挑战，TCC-VPReID框架在多个评估协议中实现了显著的性能提升。

Conclusion: TCC-VPReID框架在多个评估协议中实现了显著的性能提升。
...
TL;DR: 该研究调查了标签偏差如何影响医学图像AI中子群的公平性，发现偏差会根据子群的大小和可分离性导致特征表示发生变化，并影响模型性能。


<details>
  <summary>Details</summary>
Motivation: 系统性错误标记影响医学成像数据集中特定子群（即标签偏差）代表了一个关于医学人工智能系统公平性的研究不足的问题。

Method: 使用EMory BrEast imaging Dataset (EMBED)训练深度学习模型进行二元组织密度分类，其中标签偏差影响可分离的子群（基于成像制造商）或不可分离的“伪子群”。

Result: 发现模拟的子群标签偏差导致模型学习到的特征表示发生显著变化。重要的是，特征空间内的这些变化取决于受标签偏差影响的子群的相对大小和可分离性。还观察到子群性能的显著差异，具体取决于是否使用具有干净标签的验证集来定义模型的分类阈值。

Conclusion: 该研究表明，标签偏差会显著影响模型的特征表示，并且这种影响取决于受标签偏差影响的子群的大小和可分离性。当使用带有偏差标签的验证集时，子群的性能会显著下降。
...
TL;DR: 提出了一种新的 Grassmannian 配准方法，该方法具有可优化的成本函数，并且优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的仿射 Grassmannian 方法只能测量邻近度，而不能产生作为刚体变换显式函数的距离，因此，流形上的可优化距离函数仍未开发，阻碍了其在配准问题中的应用。

Method: 通过严格的数学证明，证明了高维线性子空间的基可以作为成本的显式表示。

Result: 与基于向量参数的方法相比，该方法能够通过直接最小化与表示模糊无关的测地距离来找到全局最优解。由此产生的成本函数及其在内点集最大化 BnB 求解器中的扩展已证明可以提高现有解决方案的收敛性或在各种计算机视觉任务中优于它们。

Conclusion: 提出了一种基于变换基的可优化成本函数，它可以应用于任何仿射子空间的配准问题。实验证明，该方法能够提高现有解决方案的收敛性或在各种计算机视觉任务中优于它们。
...
TL;DR: GRR-CoCa通过改进架构，在视觉-语言任务中表现更好。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态模型架构在复杂性上落后于大型语言模型（LLM）。

Method: 在CoCa模型中引入高斯误差门控线性单元、均方根归一化和旋转位置嵌入，并与Baseline CoCa进行对比。

Result: GRR-CoCa在预训练数据集和三个不同的微调数据集上显著优于Baseline CoCa。

Conclusion: GRR-CoCa的改进架构提高了跨视觉-语言领域的性能和泛化能力。
...
TL;DR: 构建了一个新的大规模DeepFake数据集Celeb-DF++，用于提高DeepFake检测的通用性，并评估了现有方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有的数据集在规模上很大，但仅包含有限种伪造类型，不足以开发通用的检测方法。为了解决通用取证的难题，即使用单个模型检测各种未见过的DeepFake类型。

Method: 构建了Celeb-DF++数据集，包含三种常见的伪造场景：Face-swap (FS)、Face-reenactment (FR) 和 Talking-face (TF)，使用22种不同的DeepFake方法生成高质量的伪造视频。

Result: Celeb-DF++数据集包含大量高质量的伪造视频，这些视频是通过22种不同的DeepFake方法生成的，这些方法在架构、生成流程和目标面部区域方面各不相同，涵盖了在实际应用中最常见的DeepFake案例。

Conclusion: 介绍了Celeb-DF++数据集，并评估了24种检测方法的泛化性，强调了现有检测方法的局限性以及新数据集的难度。
...
TL;DR: 提出了一种新的3D高斯修复框架，通过稀疏修复视图重建完整的3D场景，优于现有技术。


<details>
  <summary>Details</summary>
Motivation: 由于3D结构的不规则性和保持多视图一致性的关键需求，3D场景修复仍然是一项具有挑战性的任务。

Method: 该框架结合了自动掩模细化过程和区域感知的不确定性引导优化。

Result: 该框架通过利用稀疏的修复视图重建完整的3D场景。

Conclusion: 该方法在视觉质量和视图一致性方面均优于现有技术。
...
TL;DR: 本文全面回顾了基于骨骼的情感识别技术，包括方法、数据集、技术范例和应用，并探讨了未来的研究方向。


<details>
  <summary>Details</summary>
Motivation: 基于身体动作的情感识别是一种引人注目且保护隐私的替代方案，它无需依赖面部表情或生理信号。3D骨骼获取技术和姿势估计算法的进步，大大提高了基于全身运动的情感识别的可行性。

Method: 本文对基于骨骼的情感识别技术进行了全面的综述，包括心理学模型、数据集、姿势和步态方法，以及四种技术范例：传统方法、Feat2Net、FeatFusionNet和End2EndNet。

Result: 本文对现有方法进行了分类，提出了一个统一的分类法，涵盖了四种主要的技术范例，并对常用数据集进行了基准测试。此外，还探讨了情感识别在精神健康评估中的扩展应用。

Conclusion: 本文探讨了基于骨骼的情感识别技术，并分析了其在心理健康评估中的应用，以及未来的研究方向。
...
TL;DR: ViGText, a novel graph-based approach integrating images and VLLM Text, improves deepfake detection with enhanced generalization and robustness.
...
Method: integrates images with Vision Large Language Model (VLLM) Text explanations within a Graph-based framework using Graph Neural Networks (GNNs).

Result: ViGText significantly enhances generalization and achieves a notable performance boost when it detects user-customized deepfakes. Average F1 scores rise from 72.45% to 98.32% under generalization evaluation. Achieves an increase of 11.1% in recall compared to other approaches. Limits classification performance degradation to less than 4% when facing targeted attacks.
...
TL;DR: The paper introduces a new dataset (TAV) to improve AI's ability to generate long videos with coherent scene transitions by enabling better understanding of when scene transitions are needed from a prompt.
...
TL;DR: 提出了一种新的基于扩散模型的镜头模糊渲染方法，该方法可以生成高质量和高保真度的结果。


<details>
  <summary>Details</summary>
Motivation: 以前的方法受深度估计的准确性限制，在深度不连续处产生伪影。

Method: 使用了一个受物理启发的自注意力模块，该模块与图像形成过程对齐，结合了景深相关的模糊圈约束和自遮挡效果。将扩散模型调整为单步推理方案，无需引入额外的噪声。

Result: 一种新的镜头模糊渲染方法，可以实现物理上准确且视觉上吸引人的结果
...
TL;DR: VLM-IMI：一种用于 LLIE 的新型框架，它利用具有迭代和手动指令 (IMI) 的大型视觉语言模型 (VLM)。
...
Result: VLM-IMI 结合了所需正常光内容的文本描述作为增强线索，从而实现了语义知情的恢复。我们引入了一种指令先验融合模块，该模块动态地对齐和融合图像和文本特征，从而促进生成详细且语义连贯的输出。
...
TL;DR: TextSAM-EUS is a text-driven adaptation of SAM for automatic pancreatic tumor segmentation in EUS, outperforming existing models.


<details>
  <summary>Details</summary>
Motivation: Pancreatic cancer has a poor prognosis and relies on endoscopic ultrasound (EUS) for targeted biopsy and radiotherapy, but EUS images have speckle noise, low contrast, and unintuitive appearance, making segmentation with fully supervised deep learning models difficult and dependent on large, expert-curated annotation datasets.

Method: A novel, lightweight, text-driven adaptation of the Segment Anything Model (SAM) that requires no manual geometric prompts at inference. It leverages text prompt learning through the BiomedCLIP text encoder with a LoRA-based adaptation of SAM's architecture.

Result: TextSAM-EUS with automatic prompts achieves 82.69% Dice and 85.28% normalized surface distance (NSD) on the public Endoscopic Ultrasound Database of the Pancreas, and with manual geometric prompts reaches 83.10% Dice and 85.70% NSD, outperforming existing state-of-the-art supervised DL models and foundation models.

Conclusion: TextSAM-EUS provides an efficient and robust method for automatic EUS segmentation, incorporating prompt learning in SAM-based medical image segmentation.
...
TL;DR: 本研究评估了用于土地利用土地覆盖制图的先进技术，并展示了它们在城市规划中的实用性。


<details>
  <summary>Details</summary>
Motivation: 土地利用土地覆盖 (LULC) 制图对于城市和资源规划至关重要，并且是开发智能和可持续城市的主要要素之一。

Method: 本研究评估了先进的土地利用土地覆盖制图技术，重点研究了应用于 Cartosat 多光谱 (MX) 传感器图像的基于查找表 (LUT) 的大气校正，然后是用于土地利用土地覆盖预测的监督和半监督学习模型。我们探索了 DeeplabV3+ 和交叉伪监督 (CPS)。CPS 模型通过动态加权进一步完善，从而提高了训练期间伪标签的可靠性。

Result: 通过分析 Cartosat MX 图像，我们重点介绍了城市扩张、绿地萎缩和工业区扩张等变化。

Conclusion: 本研究通过分析印度海得拉巴的案例，展示了这些技术在城市规划中的实用性，突出了城市扩张、绿地萎缩和工业区扩张等变化。
...
TL;DR: 提出了一种新的异构感知分布框架(HDF)，以提高动态面部表情识别(DFER)的准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在多源数据和个体表达差异引起的样本异质性下，不可避免地会遭受性能下降。

Method: 提出了一个名为异构感知分布框架(HDF)的新框架，并设计了两个即插即用模块，以增强时频建模，减轻由困难样本引起的优化不平衡。

Result: 在DFEW和FERV39k两个广泛使用的数据集上的大量实验表明，HDF显著提高了识别准确率和鲁棒性。我们的方法实现了卓越的加权平均召回率(WAR)和非加权平均召回率(UAR)。

Conclusion: HDF显著提高了识别准确率和鲁棒性，并在不同的和不平衡的场景中保持了强大的泛化能力。
...
TL;DR: A two-stage training framework is introduced to improve the accuracy and robustness of VTG models by combining supervised fine-tuning and reinforcement learning.


<details>
  <summary>Details</summary>
Motivation: Existing video temporal grounding approaches often have limited temporal awareness and poor generalization.

Method: A two-stage training framework integrating supervised fine-tuning with reinforcement learning.

Result: The proposed method consistently outperforms existing models on multiple VTG benchmarks.

Conclusion: The proposed two-stage training framework, which combines supervised fine-tuning and reinforcement learning, outperforms existing VTG models, especially in challenging scenarios. The authors also release datasets, models, and code.
...
TL;DR: 该论文提出了一种基于 Transformer 的序列到序列模型，用于预测自然多模态电影刺激下的全脑 fMRI 反应。


<details>
  <summary>Details</summary>
Motivation: Algonauts 2025 挑战赛呼吁社区开发编码模型，以预测对自然多模态电影的全脑 fMRI 反应。

Method: 我们提出了一个序列到序列的 Transformer，它可以从视觉、听觉和语言输入中自动回归地预测 fMRI 活动。
...
TL;DR: 提出了一种新颖的自由能后验网络框架，用于联合建模分布不确定性，并使用自由能识别 OoD 和错误分类区域，从而实现不确定性感知分割。


<details>
  <summary>Details</summary>
Motivation: 从深度神经网络估计不确定性是一种广泛使用的方法，用于检测通常表现出高预测不确定性的分布外 (OoD) 样本。然而，传统的蒙特卡罗 (MC) Dropout 等方法通常只关注模型或数据不确定性，而未能与 OoD 检测的语义目标相一致。

Method: 自由能后验网络，它联合建模分布不确定性，并使用自由能识别 OoD 和错误分类区域。
...
TL;DR: 提出了T2VWorldBench评估框架，评估了10个先进的文本到视频模型，发现它们在利用世界知识方面存在不足。


<details>
  <summary>Details</summary>
Motivation: 文本到视频模型在生成视觉上合理的场景方面表现出了卓越的性能，但它们利用世界知识来确保语义一致性和事实准确性的能力在很大程度上仍未得到研究。

Method: 提出了T2VWorldBench，这是第一个用于评估文本到视频模型的世界知识生成能力的系统评估框架。

Result: 对10个最先进的文本到视频模型进行了评估，发现大多数模型无法理解世界知识并生成真正正确的视频。

Conclusion: 当前文本到视频模型在利用世界知识方面存在严重不足，为构建具有常识推理和事实生成能力的模型提供了宝贵的研究机会和切入点。
...
TL;DR: A novel information entropy-based framework for quantifying curve tortuosity is proposed and validated on meibomian gland atrophy, showing potential for medical diagnostics.
...
Result: Demonstrated a significant difference in tortuosity-based uniformity between Demodex-negative and Demodex-positive patient groups, with an area under the curve of 0.8768, sensitivity of 0.75, and specificity of 0.93.
...
TL;DR: 提出了一种双向扩散优化机制，用于联合建模弱光和正常光图像的退化过程，通过双向扩散和自适应特征交互块（AFI）细化特征表示，并设计反射感知校正模块 (RACM) 指导颜色恢复并抑制过度曝光区域。


<details>
  <summary>Details</summary>
Motivation: 虽然基于扩散的方法由于其强大的生成能力而显示出有希望的性能。然而，它们对退化的单向建模通常难以捕捉现实世界退化模式的复杂性，从而导致结构不一致和像素未对准。
...
Result: 通过利用这两个路径之间的互补性，我们的方法对光照衰减和噪声分布施加了隐式对称约束，从而有助于一致的退化学习，并提高模型感知光照和细节退化的能力。我们设计了一个反射感知校正模块 (RACM) 指导颜色恢复并抑制过度曝光区域，确保内容一致性并生成符合人类视觉感知的高质量图像。

Conclusion: 该方法在多个基准数据集上的大量实验表明，该方法在定量和定性评估中均优于最先进的方法，同时有效地推广到不同的降级场景。
...
TL;DR: WaveMamba, a cross-modality fusion method, efficiently integrates the unique frequency features of RGB and IR, achieving significant performance gains.
...
Method: The core of the approach is the introduction of WaveMamba Fusion Block (WMFB), which facilitates comprehensive fusion across low-/high-frequency sub-bands. Within WMFB, the Low-frequency Mamba Fusion Block (LMFB), built upon the Mamba framework, first performs initial low-frequency feature fusion with channel swapping, followed by deep fusion with an advanced gated attention mechanism for enhanced integration. High-frequency features are enhanced using a strategy that applies an absolute maximum fusion approach.

Result: The method surpasses state-of-the-art approaches and achieves average mAP improvements of 4.5% on four benchmarks.

Conclusion: The proposed WaveMamba method achieves significant performance gains, surpassing state-of-the-art approaches and achieving average mAP improvements of 4.5% on four benchmarks.
...
TL;DR: 提出了一种基于 YOLOv5 的资源高效的实时目标检测和分类系统，该系统针对 FPGA 部署进行了优化，并在 Xilinx Kria KV260 FPGA 板上实现。


<details>
  <summary>Details</summary>
Motivation: 现有的基于 YOLO 的目标检测和分类系统在实现适合边缘 FPGA 平台的资源效率方面仍然面临挑战。

Method: 基于为 FPGA 部署优化的 YOLOv5，在 Xilinx Kria KV260 FPGA 板上实现。

Result: 分类精度为 99%，功耗为 3.5W，处理速度为 9 帧/秒 (FPS)。

Conclusion: 实现了边缘计算应用中实时、资源高效的目标检测和分类。
...
TL;DR: 本研究提出了一种两阶段UDA框架，结合对比预训练和多模型伪标签方法，用于解决3D激光雷达语义分割中的领域转移问题，实验结果表明该方法能够有效提高分割精度。


<details>
  <summary>Details</summary>
Motivation: 解决由于领域转移（例如，传感器类型、地理位置）导致的3D激光雷达语义分割性能下降问题对于自主系统至关重要，但手动标注目标数据是令人望而却步的。

Method: 该研究采用非监督领域自适应(UDA)，并引入了一种新颖的两阶段框架。

Result: 从SemanticKITTI到未标记的目标数据集（SemanticPOSS、SemanticSlamantic）的自适应实验表明，与直接迁移和单模型UDA方法相比，分割精度有了显著提高。

Conclusion: 该研究表明，对比预训练与精炼的集成伪标签相结合，能够有效弥合复杂领域差距，且无需目标域注释。
...
TL;DR: Diff-UMamba是一种新的架构，它结合了UNet框架和Mamba机制，用于医学图像分割，尤其是在数据稀缺的情况下。
...
Method: 结合了UNet框架和Mamba机制，用于建模长程依赖关系。核心是一个噪声减少模块（NRM），它采用信号差分策略来抑制编码器中嘈杂或不相关的激活。

Result: 该架构实现了更高的分割精度和鲁棒性，尤其是在低数据设置中。在锥束CT（CBCT）中对非小细胞肺癌（NSCLC）数据集进行了验证，与基线相比，实现了4-5%的改进。

Conclusion: Diff-UMamba在多个公共数据集上进行了评估，包括MSD（肺和胰腺）和AIIB23，在不同的分割任务中，与基线方法相比，性能持续提升1-3%。
...
TL;DR: MatSSL是一种自监督学习架构，它使用少量未标记数据有效地适应金属材料的微观结构分析，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前金属材料的微观结构分析依赖于监督方法，这需要为每个新数据集重新训练，并且在只有少量标记样本的情况下，性能通常不一致。虽然SSL通过利用未标记数据提供了一种有前途的替代方案，但大多数现有方法仍然依赖于大规模数据集才能有效。

Method: 采用门控特征融合的自监督学习(SSL)架构

Result: MatSSL在MetalDAM上实现了69.13%的mIoU，优于ImageNet预训练编码器的66.73%，并且与使用MicroNet预训练的模型相比，在环境屏障涂层基准数据集(EBC)上，平均mIoU持续提高了近40%。

Conclusion: MatSSL在少量未标记数据上实现了有效的金属材料微观结构分析适应性，同时保留了从大规模自然图像预训练中学习到的丰富且可转移的特征。
...
TL;DR: TeEFusion, a novel distillation method, significantly speeds up text-to-image synthesis while maintaining image quality, by incorporating guidance magnitude into text embeddings and distilling the teacher model's sampling strategy.


<details>
  <summary>Details</summary>
Motivation: High inference costs due to CFG's reliance on two forward passes, especially when combined with intricate sampling algorithms.
...
Result: Student model achieves inference speeds up to 6x faster than the teacher model, while maintaining image quality at levels comparable to those obtained through the teacher's complex sampling approach.
...
TL;DR: LEAF: A medical image segmentation model grounded in latent diffusion models that improves performance by directly predicting the segmentation map and using feature distillation, without increasing model complexity or computational cost during inference.
...
Method: The original noise prediction pattern is replaced with a direct prediction of the segmentation map during fine-tuning. A feature distillation method is used to align the hidden states of the convolutional layers with the features from a transformer-based vision encoder.

Result: The proposed method enhances the performance of the original diffusion model across multiple segmentation datasets for different disease types.

Conclusion: The proposed method enhances the performance of the original diffusion model across multiple segmentation datasets for different disease types, without altering the model architecture or increasing the number of parameters or computation during inference, making it highly efficient.
...
TL;DR: GSDTTA是一种用于3D点云分类的新方法，它通过在图谱域中进行适应性调整，实现了更有效的适应性，并通过特征图引导的自训练策略迭代地优化频谱调整和模型参数。


<details>
  <summary>Details</summary>
Motivation: 现有的3D TTA方法通常依赖于计算量大的空间域优化，并且可能需要额外的训练数据，而3D点云的不规则和无序结构阻碍了测试时适应（TTA）方法在3D点云中的应用。

Method: 将适应性调整转移到图谱域，通过图傅里叶变换（GFT）将目标域中的点云表示为异常值感知的图，并通过优化最低10%的频率分量来进行适应性调整。

Result: 实验结果和消融研究表明了GSDTTA的有效性。
...
TL;DR: DATA网络通过域和时间对齐来提高协同感知的性能，并在各种挑战下保持鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 特征级融合在协同感知（CP）中显示出希望，但其有效性严重依赖于输入特征质量。高质量特征的获取面临来自硬件多样性和部署条件的域差距，以及来自传输延迟的时间错位。这些挑战会降低特征质量。

Method: 提出Domain-And-Time Alignment (DATA)网络，通过Consistency-preserving Domain Alignment Module (CDAM)减少域差距，通过Progressive Temporal Alignment Module (PTAM)处理传输延迟，并使用Instance-focused Feature Aggregation Module (IFAM)增强语义表示。

Result: DATA在三个典型的数据集上实现了最先进的性能，并在严重的通信延迟和姿态误差下保持了鲁棒性。

Conclusion: DATA在三个典型的数据集上实现了最先进的性能，并在严重的通信延迟和姿态误差下保持了鲁棒性。
...
Method: The paper introduces a flare-simulation module and a noise-simulation module to generate paired depth datasets for low-light conditions. It also presents a low-light PEFT strategy using illumination guidance and multiscale feature fusion.
...
Conclusion: The proposed DepthDark model achieves state-of-the-art depth estimation performance on challenging nighttime datasets with limited training data and computing resources.
...
TL;DR: This paper introduces Gaussian Agnostic Representation Learning to address the fragility of current infrared small target detection (ISTD) methods in real-world scenarios with limited high-quality infrared data. The approach uses a Gaussian Group Squeezer and two-stage diffusion models to improve the resilience and quality of synthetic samples, demonstrating improved performance in scarcity scenarios.


<details>
  <summary>Details</summary>
Motivation: the state-of-the-art ISTD methods highly fragile in real-world challenges
...
TL;DR: Dental caries causes lung cancer.
...
Result: Dental caries has a significant positive causal effect on overall lung cancer and its subtypes, specifically squamous cell lung carcinoma. This effect is partially mediated by declines in forced vital capacity (FVC) and forced expiratory volume in one second (FEV1). No causal effect was found for periodontitis.

Conclusion: Dental caries has a causal role in lung cancer risk, and integrating dental care and pulmonary function monitoring into cancer prevention strategies is supported.
...
TL;DR: LMM-Det：一种简单有效的方法，利用大型多模态模型进行目标检测，无需专门的检测模块，并通过数据调整和推理优化来提高召回率。


<details>
  <summary>Details</summary>
Motivation: 大型多模态模型 (LMM) 在多模态理解、推理和上下文学习等方面表现出卓越的能力，受到了人工智能研究和工业界的广泛关注和兴趣。然而，与专业检测器相比，LMM 的目标检测能力存在显著差距。

Method: 提出 LMM-Det，一种简单而有效的方法，它利用大型多模态模型进行原始目标检测，而无需依赖专门的检测模块。通过引入专门为目标检测量身定制的数据分布调整和推理优化来提高召回率。

Result: 对大型多模态模型与目标检测相结合进行了全面的探索性分析，结果表明，与专业检测模型相比，召回率显著下降。通过重新组织指令对话，增强了大型多模态模型的目标检测能力。

Conclusion: 大型多模态模型 (LMM) 具备检测能力，无需额外的检测模块。大量实验支持了这一论断，并表明了 LMM-Det 的有效性。
...
TL;DR: FieldLVLM是一种新的框架，旨在提高大型视觉语言模型对领域数据的理解，通过领域感知语言生成和数据压缩多模态模型调优来实现。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在图像描述和视觉问答等任务中表现出令人印象深刻的能力，但它们在科学领域的应用仍有待探索。

Method: FieldLVLM包含两个主要组成部分：领域感知语言生成策略和数据压缩多模态模型调优。

Result: FieldLVLM在涉及科学领域数据的任务中显著优于现有方法。

Conclusion: FieldLVLM显著优于现有方法，为大型视觉语言模型在科学研究中的应用开辟了新的可能性。
...
TL;DR: 我们提出了第一个用于心电图描绘中半监督语义分割的系统基准。


<details>
  <summary>Details</summary>
Motivation: 心电图 (ECG) 描绘，即有意义的波形特征的分割，对于临床诊断至关重要。尽管最近使用深度学习取得了进展，但由于公开可用的带注释数据集的稀缺，进展受到限制。半监督学习通过利用大量未标记的心电图数据，提供了一个有希望的解决方案。

Method: 采用了来自计算机视觉的五种代表性半监督算法，并在卷积网络和 Transformer 两种不同的架构上实现了它们，并在域内和跨域两种不同的设置中评估了它们。此外，我们提出了心电图特定的训练配置和增强策略，并引入了一个标准化的评估框架。

Result: Transformer在半监督心电图描绘中优于卷积网络。

Conclusion: Transformer在半监督心电图描绘中优于卷积网络，此基准将促进半监督心电图描绘方法的发展，并促进该领域的进一步研究。
...
TL;DR: introduce a new modified nuclear norm (MNN) framework
...
Method: introduce a new modified nuclear norm (MNN) framework, where the MNN family norms are defined by adopting suitable transformations and performing the NN on the transformed matrix

Result: the MNN framework offers two main advantages: (1) it jointly captures both local information and global low-rankness without requiring trade-off parameter tuning; (2) Under mild assumptions on the transformation, we provided exact theoretical recovery guarantees for both Robust PCA and MC tasks-an achievement not shared by existing methods that combine local and global information.
...
TL;DR: 提出了一个新的尾迹数据集，包含 122 个视频序列，用于改进尾迹监测和校准物理模型，从而为了解气候影响奠定基础。


<details>
  <summary>Details</summary>
Motivation: 航空的气候影响不仅包括二氧化碳排放，还包括显着的非二氧化碳影响，特别是来自尾迹。这些冰云可以改变地球的辐射平衡，可能与航空二氧化碳的变暖效应相媲美。现有的数据集没有探索尾迹动力学和形成的所有方面：它们通常缺乏时间跟踪，并且没有将尾迹归因于其来源航班。

Method: 提出了地面可见相机尾迹序列（GVCCS），这是一个新的开放数据集，记录了在可见光范围内使用地面全天空相机记录的尾迹。还提出了一个统一的深度学习框架，用于尾迹分析，使用全景分割模型，该模型在单个架构中执行语义分割（尾迹像素识别）、实例分割（单个尾迹分离）和时间跟踪。
...
Conclusion: 该数据集支持改进尾迹监测，并将有助于更好地校准物理模型。这为更准确的气候影响理解和评估奠定了基础。
...
TL;DR: SGCDet是一种新型3D对象检测框架，它通过自适应体积构建和稀疏体积构建策略，在多个数据集上实现了最先进的性能，且仅使用3D边界框进行监督。


<details>
  <summary>Details</summary>
Motivation: 现有方法将体素的感受野限制在图像上的固定位置。

Method: 一种基于自适应3D体积构建的新型多视图室内3D对象检测框架，集成了几何和上下文感知聚合模块，并提出了稀疏体积构建策略。

Result: SGCDet实现了有效和高效的体积构建，并且仅使用3D边界框进行监督。
...
TL;DR: 通过对鸟类鸣声 spectrogram 进行颜色编码，可以提高鸟类分类的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有的鸟类鸣声分类模型在低信噪比或多物种录音的情况下表现不佳。

Method: 通过将频率信息嵌入到 spectrogram 图像中，使用主色添加剂来增强物种区分，并使用深度学习模型进行分类。

Result: 该方法在 F1 指标上提升了 7.3%，ROC-AUC 提升了 6.2%，CMAP 提升了 6.6%，超过了 BirdCLEF 2024 的优胜者。
...
TL;DR: 提出了EgoExoBench基准测试，用于评估MLLM在以自我为中心和以外为中心的视频理解和推理方面的能力，发现现有模型在跨视角语义对齐、视角关联和时间推理方面存在困难。


<details>
  <summary>Details</summary>
Motivation: 跨第一人称（以自我为中心）和第三人称（以外为中心）视角传递和整合知识是人类智能固有的能力，使人类能够向他人学习并传达自己经验中的见解。尽管多模态大型语言模型（MLLM）取得了快速进展，但它们执行这种跨视角推理的能力仍有待探索。

Method: 引入EgoExoBench，这是第一个用于以自我为中心-以外为中心的视频理解和推理的基准。

Result: 对13个最先进的MLLM进行了评估，发现这些模型在单视角任务中表现出色，但它们难以对齐跨视角的语义，准确地关联视角，并推断自我-外部环境中的时间动态。
...
TL;DR: VB-Mitigator是一个开源框架，旨在通过统一的平台和评估标准，加速计算机视觉中偏见缓解技术的研究和发展。


<details>
  <summary>Details</summary>
Motivation: 计算机视觉模型中的偏见仍然是一个重大挑战，导致AI系统不公平、不可靠和不通用。研究进展受到碎片化的实现和不一致的评估实践的阻碍。研究中使用不同的数据集和指标，使得重现性复杂化，难以公平地评估和比较各种方法的有效性。

Method: 提出了VB-Mitigator，一个开源框架，用于简化视觉偏见缓解技术的发展、评估和比较分析。该框架包含12种已建立的缓解方法和7个不同的基准数据集。

Result: VB-Mitigator提供了一个统一的研究环境，允许无缝集成额外的方法、数据集、指标和模型。同时，我们还推荐了最佳评估实践，并提供了最先进方法之间的全面性能比较。

Conclusion: VB-Mitigator通过提供统一的平台和最佳实践建议，旨在加速公平感知计算机视觉模型的研究。
...
TL;DR: 提出了一种新的可变形卷积模块GDCUnet，用于眼底血管分割，并在公共数据集上取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 可变形卷积可以通过学习偏移来适应性地改变卷积核的形状，以处理复杂的形状特征。考虑到眼底血管具有全局自相似的复杂边缘，

Method: 提出了一种新的即插即用可变形卷积模块，该模块使用注意力和前馈网络来学习偏移，以便可变形模式可以捕获长距离全局特征。该模块学习亚像素位移场并自适应地扭曲所有通道上的特征图，而不是直接变形卷积核，这相当于内核采样网格的相对变形，实现了全局特征变形以及内核大小和学习网络的解耦。

Result: GDCUnet在公共数据集上取得了最先进的性能。消融实验表明，所提出的可变形卷积模块可以更显着地学习眼底血管的复杂特征，增强模型表示和泛化能力。

Conclusion: GDCUnet在公共数据集上取得了最先进的性能，并且提出的可变形卷积模块可以更显着地学习眼底血管的复杂特征，增强模型表示和泛化能力。建议将其应用于更多具有复杂全局自相似特征的机器视觉任务。
...
TL;DR: MVG4D is a novel framework that generates dynamic 4D content from a single still image by combining multi-view synthesis with 4D Gaussian Splatting, enhancing temporal consistency, geometric fidelity, and visual realism.
...
Method: MVG4D combines multi-view synthesis with 4D Gaussian Splatting (4D GS). It employs an image matrix module that synthesizes temporally coherent and spatially diverse multi-view images to optimize a 3D Gaussian point cloud, which is further extended into the temporal domain via a lightweight deformation network.
...
TL;DR: This paper introduces an evaluation framework and a multimodal dataset for human-AI interactions. It develops an AR-equipped AI agent for real-world tasks and demonstrates that AI-assisted collaboration improves task completion.


<details>
  <summary>Details</summary>
Motivation: Evaluating human-AI collaboration for physical task completion is challenging due to the complexity of human-in-the-loop interactions.

Method: Develop an augmented reality (AR)-equipped AI agent that provides interactive guidance in real-world tasks and conduct human studies.
...
TL;DR: 提出了一种新的单阶段姿势生成架构，该架构优于现有的两阶段和自回归方法，尤其是在长期姿势生成方面。


<details>
  <summary>Details</summary>
Motivation: 目前的姿势生成方法严重依赖中间表示，无论是通过具有量化的两阶段管道还是在推理过程中累积误差的自回归模型。这种根本的限制导致性能下降，尤其是在保持时间连贯性至关重要的长期姿势生成中。

Method: 一种新颖的单阶段架构，可以直接从最小的上下文中在连续坐标空间中生成姿势——单个 RGB 图像和文本描述——同时保持训练和推理之间的一致分布。关键创新是通过保留空间关系的相对运动预测机制以及在训练和推理过程中实现相同行为的单次前向生成的统一占位符令牌方法，从而消除了对中间表示或基于令牌的生成的需求，直接对姿势坐标进行操作。
...
TL;DR: This paper introduces HumanMaterial, a new method for high-quality full-body human inverse rendering using a progressive training strategy and a novel loss function, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Previous methods for full-body human inverse rendering struggle with the ill-posed nature of the task and limited realism due to simplified material data and rendering equations. End-to-end models also struggle to balance the importance among various material maps, leading to underfitting.

Method: The paper designs a HumanMaterial model with a progressive training strategy, using three prior models for initial material estimation and a finetuning model for refinement. A Controlled PBR Rendering (CPR) loss enhances the importance of materials during the training of prior models.

Result: The proposed method (HumanMaterial) achieves state-of-the-art performance on the OpenHumanBRDF dataset and real data.

Conclusion: The paper presents a novel method (HumanMaterial) with a progressive training strategy and a Controlled PBR Rendering (CPR) loss to improve material estimation for full-body human inverse rendering. Experiments show state-of-the-art performance on the OpenHumanBRDF dataset and real data.
...
TL;DR: Iwin Transformer是一种新的视觉Transformer，它通过交错窗口注意力和深度可分离卷积实现全局信息交换，并在图像分类、语义分割和视频动作识别等任务中表现出强大的竞争力。


<details>
  <summary>Details</summary>
Motivation: 克服Swin Transformer需要两个连续块来近似全局注意力的限制, 在单个模块内实现全局信息交换

Method: 创新交错窗口注意力和深度可分离卷积

Result: 在ImageNet-1K上达到87.4的top-1准确率

Conclusion: Iwin Transformer在图像分类、语义分割和视频动作识别等任务中表现出强大的竞争力，并且可以作为独立模块无缝替换类条件图像生成中的自注意力模块。Iwin Transformer的概念和方法有潜力激发未来的研究，例如视频生成中的Iwin 3D Attention。
...
TL;DR: SynC 是一种用于优化合成图像-文本数据集以进行零样本图像描述的新框架，通过重新分配字幕来解决 T2I 模型中的语义错位问题。


<details>
  <summary>Details</summary>
Motivation: 零样本图像描述 (ZIC) 越来越多地利用文本到图像 (T2I) 模型生成的合成数据集，以减少对手动注释的需求。然而，这些 T2I 模型经常生成与其相应输入字幕存在语义错位的图像，从而导致嘈杂的合成图像-字幕对，这可能会阻碍模型训练。

Method: SynC 专注于将字幕重新分配给合成图像池中已存在的语义对齐度最高的图像，采用了一种一对多的映射策略，通过图像到文本检索验证其检索原始字幕的能力来选择最佳图像。

Result: SynC 在标准基准测试（MS-COCO、Flickr30k、NoCaps）上的各种 ZIC 模型中持续且显着地提高了性能，在多种情况下实现了最先进的结果。
...
Method: DCFFSNet (Dual-Connectivity Feature Fusion-Separation Network). It introduces an innovative feature space decoupling strategy. This strategy quantifies the relative strength between connectivity features and other features. It then builds a deep connectivity feature fusion-separation architecture. This architecture dynamically balances multi-scale feature expression.
...
Conclusion: DCFFSNet exceeds existing mainstream methods across all metrics. It effectively resolves segmentation fragmentation and achieves smooth edge transitions. This significantly enhances clinical usability.
...
### [109] [ASP-Assisted Symbolic Regression: Uncovering Hidden Physics in Fluid Mechanics](https://arxiv.org/abs/2507.17777)
*Theofanis Aravanis,Grigorios Chrimatopoulos,Mohammad Ferdows,Michalis Xenos,Efstratios Em Tzirtzilakis*

Main category: cs.AI

TL;DR: 本研究将符号回归（SR）应用于模拟三维不可压缩流动，发现SR能够生成简洁且可解释的方程，并提出了一种混合SR/ASP框架，以确保生成的表达式在统计上准确且在物理上合理。


<details>
  <summary>Details</summary>
Motivation: 在流体力学中，对底层流动物理的理解与准确预测同样重要。

Method: 通过使用PySR库，直接从数值模拟数据中导出紧凑的符号方程。

Result: 导出的方程不仅近似于研究的流体流动中观察到的抛物线速度分布和压降，而且与文献中的解析解完全一致。

Conclusion: 该研究强调了SR将复杂的流动行为简化为简洁、可解释的方程的能力，以及知识表示方法在提高数据驱动的SR模型与领域原则的可靠性和一致性方面的潜力。
...
### [110] [I2I-STRADA -- Information to Insights via Structured Reasoning Agent for Data Analysis](https://arxiv.org/abs/2507.17874)
*SaiBarath Sundar,Pranav Satheesan,Udayaadithya Avadhanam*

Main category: cs.AI

TL;DR: I2I-STRADA是一种智能体架构，旨在通过模拟分析展开的模块化子任务来形式化数据分析的推理过程，从而在规划连贯性和洞察力对齐方面优于先前的系统。


<details>
  <summary>Details</summary>
Motivation: 现有数据分析智能体系统忽略了分析思维背后结构化的推理过程。现实世界的数据分析需要一致的认知工作流程：解释模糊的目标，将其置于上下文知识中，构建抽象的计划，并根据中间结果调整执行。

Method: 引入I2I-STRADA（通过结构化推理智能体进行数据分析的信息到洞察），该智能体架构旨在形式化推理过程。

Result: 在DABstep和DABench基准测试中的评估表明，I2I-STRADA在规划连贯性和洞察力对齐方面优于先前的系统。

Conclusion: I2I-STRADA在规划连贯性和洞察力对齐方面优于先前的系统，突出了结构化认知工作流在数据分析的智能体设计中的重要性。
...
### [111] [SMARTAPS: Tool-augmented LLMs for Operations Management](https://arxiv.org/abs/2507.17927)
...
TL;DR: SmartAPS: a conversational system built on a tool-augmented LLM to provide a more accessible APS for supply chain planners.
...
Result: operations planners can query information, perform counterfactual reasoning, receive recommendations, and execute scenario analysis to better manage their operation.
...
### [112] [Synthesis of timeline-based planning strategies avoiding determinization](https://arxiv.org/abs/2507.17988)
*Dario Della Monica,Angelo Montanari,Pietro Sala*

Main category: cs.AI

TL;DR: 本文研究了定性时间线规划的一个片段，其计划存在性问题可以直接映射到确定性有限自动机的非空性问题上，然后可以合成策略。


<details>
  <summary>Details</summary>
Motivation: 定性时间线规划模型的计划存在性问题已被证明是 PSPACE 完全的；但是，不确定性自动机不能直接用于合成规划策略，因为需要昂贵的确定化步骤。

Method: 将计划存在性问题直接映射到确定性有限自动机的非空性问题

Result: 确定了艾伦关系的一个最大子集，该子集适合于这样的确定性片段。

Conclusion: 找到了定性时间线规划的一个片段，其计划存在性问题可以直接映射到确定性有限自动机的非空性问题上，然后可以合成策略。此外，我们确定了艾伦关系的一个最大子集，该子集适合于这样的确定性片段。
...
### [113] [E.A.R.T.H.: Structuring Creative Evolution through Model Error in Generative AI](https://arxiv.org/abs/2507.18004)
*Yusen Peng,Shuhua Mao*

Main category: cs.AI

TL;DR: The paper introduces the E.A.R.T.H. framework, a five-stage pipeline that leverages errors to enhance AI creativity. The results demonstrate significant improvements in creativity scores, slogan refinement, and human evaluation, suggesting a path toward self-evolving, human-aligned creative AI.


<details>
  <summary>Details</summary>
Motivation: How can AI move beyond imitation toward genuine creativity? We posit that "creative potential hides in failure" and operationalize this via structured prompts, semantic scoring

Method: E.A.R.T.H. framework, a five-stage generative pipeline that transforms model-generated errors into creative assets through Error generation, Amplification, Refine selection, Transform, and Harness feedback.Implemented using LLaMA-2-7B-Chat, SBERT, BERTScore, CLIP, BLIP-2, and Stable Diffusion, the pipeline employs a composite reward function based on novelty, surprise, and relevance. human-in-the-loop evaluation.

Result: Creativity scores increase by 52.5%. Refined slogans are 48.4% shorter, 40.7% more novel, with only a 4.0% drop in relevance. Cross-modal tests show strong slogan-to-image alignment. In human evaluations, 60% of outputs scored >= 4.0, with metaphorical slogans outperforming literal ones.
...
### [114] [Does visualization help AI understand data?](https://arxiv.org/abs/2507.18022)
*Victoria R. Li,Johnathan Sun,Martin Wattenberg*

Main category: cs.AI

TL;DR: AI系统可以像人类一样从数据可视化中受益。


<details>
  <summary>Details</summary>
Motivation: 研究图表和图形是否有助于AI系统分析数据。
...
### [115] [Multi-Agent Guided Policy Optimization](https://arxiv.org/abs/2507.18059)
*Yueheng Li,Guangming Xie,Zongqing Lu*

Main category: cs.AI

TL;DR: MAGPO, a novel CTDE framework, better leverages centralized training by integrating centralized guidance with decentralized execution, outperforming strong baselines with theoretical guarantees.
...
Result: MAGPO consistently outperforms strong CTDE baselines and matches or surpasses fully centralized approaches
...
### [116] [AlphaGo Moment for Model Architecture Discovery](https://arxiv.org/abs/2507.18074)
*Yixiu Liu,Yang Nan,Weixian Xu,Xiangkun Hu,Lyumanshan Ye,Zhen Qin,Pengfei Liu*

Main category: cs.AI

TL;DR: ASI-Arch 是一种用于人工智能研究的人工智能超智能 (ASI4AI)，它通过支持人工智能进行自己的架构创新来打破基本约束。


<details>
  <summary>Details</summary>
Motivation: 人工智能系统的能力呈指数级增长，但人工智能研究本身的速度仍然受到人类认知能力的线性限制，从而造成了日益严重的开发瓶颈。

Method: ASI-Arch 是一种完全自主的系统，它打破了这一基本约束，使 AI 能够进行自己的架构创新。ASI-Arch 可以进行架构发现领域的端到端科学研究，自主地假设新颖的架构概念，将它们实现为可执行代码，通过严格的实验和过去的经验来训练和经验验证它们的性能。

Result: ASI-Arch 进行了 1,773 次自主实验，历时超过 20,000 个 GPU 小时，最终发现了 106 种创新的、最先进的线性注意力架构。

Conclusion: ASI-Arch 发现的架构展示了超越人类设计的基线的新兴设计原则，并阐明了以前未知的架构创新途径。我们建立了第一个科学发现的实证比例定律，证明架构突破可以通过计算进行扩展，从而将研究进展从人类限制转变为计算可扩展的过程。
...
### [117] [Agentic AI framework for End-to-End Medical Data Inference](https://arxiv.org/abs/2507.18115)
*Soorya Ram Shimgekar,Shayan Vassef,Abhay Goyal,Navin Kumar,Koustuv Saha*

Main category: cs.AI

TL;DR: An Agentic AI framework automates the clinical data pipeline in healthcare, reducing costs and labor by using task-specific agents for data handling, feature extraction, model selection, and preprocessing.
...
Method: An Agentic AI framework automates the clinical data pipeline through modular, task-specific agents that handle structured and unstructured data. The pipeline includes agents for ingestion, anonymization, feature extraction, model-data feature matching, preprocessing recommendation and implementation, and model inference.

Result: The system was evaluated on datasets from geriatrics, palliative care, and colonoscopy imaging, demonstrating automatic feature selection, model selection, and preprocessing recommendation without manual intervention. Examples include anxiety data and colonoscopy polyps data.
...
### [118] [Actively evaluating and learning the distinctions that matter: Vaccine safety signal detection from emergency triage notes](https://arxiv.org/abs/2507.18123)
*Sedigh Khademi,Christopher Palmer,Muhammad Javed,Hazel Clothier,Jim Buttery,Gerardo Luis Dimaguila,Jim Black*

Main category: cs.AI

TL;DR: The study uses NLP and active learning to develop a classifier for detecting vaccine safety issues from emergency department notes, addressing limitations of keyword-based methods and data scarcity.
...
### [119] [Logical Characterizations of GNNs with Mean Aggregation](https://arxiv.org/abs/2507.18145)
*Moritz Schönherr,Carsten Lutz*

Main category: cs.AI

TL;DR: 研究了具有平均聚合的图神经网络的表达能力，发现它介于具有最大值和总和聚合的GNN之间。
...
Method: 图神经网络（GNN）与平均值作为聚合函数

Result: 在非均匀设置中，表明这种GNN具有与比率模态逻辑完全相同的表达能力，比具有最大聚合的GNN的表达能力更高，但低于总和聚合的GNN。

Conclusion: 在一致性设置中，相对于MSO，平均GNN的表达能力严格低于总和GNN和最大GNN。当任何假设被删除时，表达能力都会增加。
...
### [120] [Decoupling Knowledge and Reasoning in LLMs: An Exploration Using Cognitive Dual-System Theory](https://arxiv.org/abs/2507.18178)
*Mutian Yang,Jiandong Gao,Ji Wu*

Main category: cs.AI

TL;DR: This paper proposes a framework to decouple knowledge and reasoning in LLMs, finding that reasoning adjustment is domain-specific, parameter scaling improves both, and knowledge resides in lower layers.
...
Result: reasoning adjustment is domain-specific. Parameter scaling improves both knowledge and reasoning. Knowledge primarily resides in lower network layers, while reasoning operates in higher layers.
...
### [121] [Comparing Non-minimal Semantics for Disjunction in Answer Set Programming](https://arxiv.org/abs/2507.18198)
*Felicidad Aguado,Pedro Cabalar,Brais Muñiz,Gilberto Pérez,Concepción Vidal*

Main category: cs.AI

TL;DR: This paper compares four different semantics for disjunction in Answer Set Programming and proves that three of them coincide and are stronger than the fourth approach.


<details>
  <summary>Details</summary>
Motivation: Four different semantics for disjunction in Answer Set Programming that, unlike stable models, do not adhere to the principle of model minimality.

Method: compare four different semantics for disjunction in Answer Set Programming
...
### [122] [Foundations for Risk Assessment of AI in Protecting Fundamental Rights](https://arxiv.org/abs/2507.18290)
*Antonino Rotolo,Beatrice Ferrigno,Jose Miguel Angel Garcia Godinez,Claudio Novelli,Giovanni Sartor*

Main category: cs.AI

TL;DR: 提出了一个用于人工智能质量风险评估的概念框架，该框架整合了定义平衡和可撤销推理，以解决法律合规和基本权利保护的复杂性。
...
Method: 通过整合定义平衡和可撤销推理，该框架解决了法律合规和基本权利保护的复杂性。定义平衡采用比例分析来解决相互竞争的权利之间的冲突，而可撤销推理适应法律决策的动态性质。

Result: 在分析的基础上，我们为人工智能风险分析的逻辑描述提供了哲学基础。特别是，我们考虑了从概念上掌握人工智能部署场景和基本权利之间相互作用的基本构建块，将可撤销推理纳入定义平衡和关于权利的情境促进或贬低的论证。这种分层方法允许对高风险人工智能系统和通用人工智能（GPAI）系统进行更有效的评估模型，强调后者的更广泛适用性。

Conclusion: 未来工作旨在开发一个正式的模型和有效的算法，以加强人工智能风险评估，弥合理论见解与实际应用，以支持负责任的人工智能治理。
...
### [123] [The AlphaPhysics Term Rewriting System for Marking Algebraic Expressions in Physics Exams](https://arxiv.org/abs/2507.18337)
*Peter Baumgartner,Lachlan McGinness*

Main category: cs.AI

TL;DR: A system using computer algebra, SMT solver, term rewriting, and LLM to automatically mark Physics exams.


<details>
  <summary>Details</summary>
Motivation: Assessing typed student answers for correctness with respect to a ground truth solution in Physics exams.

Method: A combination of a computer algebra system, an SMT solver, and a term rewriting system, with a Large Language Model to interpret and correct student responses.

Result: The system's performance is evaluated on a dataset of student exam responses.

Conclusion: The system is evaluated on 1500 real-world student exam responses from the 2023 Australian Physics Olympiad.
...
### [124] [Reasoning Beyond the Obvious: Evaluating Divergent and Convergent Thinking in LLMs for Financial Scenarios](https://arxiv.org/abs/2507.18368)
*Zhuang Qiang Bok,Watson Wei Khong Chua*

Main category: cs.AI

TL;DR: Introduces ConDiFi, a benchmark evaluating divergent and convergent thinking in LLMs for financial tasks, revealing performance differences among leading models.
...
Method: ConDiFi, a benchmark that jointly evaluates divergent and convergent thinking in LLMs for financial tasks.
...
### [125] [Revisiting LLM Reasoning via Information Bottleneck](https://arxiv.org/abs/2507.18391)
*Shiye Lei,Zhihao Cheng,Kai Jia,Dacheng Tao*

Main category: cs.AI

TL;DR: This paper introduces a theoretical characterization of LLM reasoning grounded in information bottleneck (IB) principle, introducing IB-aware reasoning optimization (IBRO), a framework that encourages reasoning trajectories to be both informative about the final correct answer and generalizable across diverse prompts. The technique integrates seamlessly into existing RL-based post-training frameworks without additional computational overhead, requiring only a one-line code modification.
...
### [126] [Optimising Call Centre Operations using Reinforcement Learning: Value Iteration versus Proximal Policy Optimisation](https://arxiv.org/abs/2507.18398)
*Kwong Ho Li,Wathsala Karunarathne*

Main category: cs.AI

TL;DR: 本文研究了使用强化学习优化呼叫中心呼叫路由，PPO 表现最好。
...
Method: 比较了基于价值迭代 (VI) 的模型方法和使用近端策略优化 (PPO) 的无模型方法。

Result: 经过 1,000 次测试，PPO 始终获得最高奖励，以及最短的客户等待时间和员工空闲时间。
...
### [127] [GPU Accelerated Compact-Table Propagation](https://arxiv.org/abs/2507.18413)
*Enrico Santi,Fabio Tardivo,Agostino Dovier,Andrea Formisano*

Main category: cs.AI

TL;DR: This paper uses GPUs to speed up the Compact-Table algorithm for constraint programming.


<details>
  <summary>Details</summary>
Motivation: Real-world problems with many valid cases are too numerous for standard CPU-based approaches.

Method: The Compact-Table (CT) algorithm is enhanced using GPUs.

Result: The design, implementation, and experimental validation of GPU-accelerated CT are reported, integrated into an existing constraint solver, and tested on a significant set of instances.
...
### [128] [On the Performance of Concept Probing: The Influence of the Data (Extended Version)](https://arxiv.org/abs/2507.18550)
*Manuel de Sousa Ribeiro,Afonso Leote,João Leite*

Main category: cs.AI

TL;DR: This paper studies how the data used to train concept probing models affects their performance in image classification and provides concept labels for two datasets.


<details>
  <summary>Details</summary>
Motivation: Concept probing helps interpret artificial neural networks, but limited attention has been paid to the data required to train such probing models.

Method: The paper investigates the effect of the data used to train probing models on their performance.

Result: The paper investigates the effect of the data used to train probing models on their performance. The paper also makes available concept labels for two widely used datasets.
...
### [129] [SafeWork-R1: Coevolving Safety and Intelligence under the AI-45$^{\circ}$ Law](https://arxiv.org/abs/2507.18576)
...
<details>
  <summary>Details</summary>
Motivation: Develop a multimodal reasoning model with coevolution of capabilities and safety.

Method: SafeLadder framework with large-scale, progressive, safety-oriented reinforcement learning post-training, supported by a suite of multi-principled verifiers.

Result: SafeWork-R1 achieves an average improvement of $46.54\%$ over its base model on safety-related benchmarks without compromising general capabilities, and delivers state-of-the-art safety performance compared to leading proprietary models.

Conclusion: Safety and capability can co-evolve synergistically.
...
### [130] [An advanced AI driven database system](https://arxiv.org/abs/2507.17778)
*M. Tedeschi,S. Rizwan,C. Shringi,V. Devram Chandgir,S. Belich*

Main category: cs.DB

TL;DR: This paper presents a new AI-powered database system that uses NLP to improve data management and automate tasks such as data modeling, schema creation, query comprehension, and performance optimization. It aims to alleviate the main problems with current database technologies.
...
Result: The system is intended to strengthen the potential of databases through the integration of Large Language Models (LLMs) and advanced machine learning algorithms. The integration is purposed to allow the automation of fundamental tasks such as data modeling, schema creation, query comprehension, and performance optimization.
...
### [131] [VERIRAG: Healthcare Claim Verification via Statistical Audit in Retrieval-Augmented Generation](https://arxiv.org/abs/2507.17948)
...
TL;DR: VERIRAG, a framework that improves the scientific quality of evidence retrieved by RAG systems, outperforms baselines on four datasets by weighting evidence by its quality and diversity and calibrating the required evidence based on how extraordinary a claim is.
...
Result: Achieving absolute F1 scores ranging from 0.53 to 0.65, representing a 10 to 14 point improvement over the next-best method in each respective dataset across four datasets-comprising retracted, conflicting, comprehensive, and settled science corpora.
...
### [132] [Failure Prediction in Conversational Recommendation Systems](https://arxiv.org/abs/2507.17976)
*Maria Vlachou*

Main category: cs.IR

TL;DR: 本文介绍了监督对话性能预测任务，并提出了用于检测对话失败的预测器。


<details>
  <summary>Details</summary>
Motivation: 用户可能需要与系统进行更多轮交互，因为用户寻找的商品不在商品目录中，这可能导致用户沮丧。

Method: 基于自编码器的预测器，学习训练轮次中检索到的顶部项目的压缩表示，并使用分类标签来预测评估轮次。

Result: 在 Shoes 和 FashionIQ Dresses 数据集上的实验结果表明，提出的预测器在预测系统故障方面有希望，但在目录故障预测方面的预测性能显著下降。

Conclusion: 提出的预测器在预测系统故障方面有希望，但在目录故障预测方面的预测性能显著下降。
...
### [133] [Fashion-AlterEval: A Dataset for Improved Evaluation of Conversational Recommendation Systems with Alternative Relevant Items](https://arxiv.org/abs/2507.18017)
*Maria Vlachou*

Main category: cs.IR

TL;DR: 提出了 Fashion-AlterEval 数据集和两个元用户模拟器，以克服现有模拟器的局限性，并发现现有单目标评估低估了 CRS 模型的有效性。


<details>
  <summary>Details</summary>
Motivation: 由于需要大量数据，用户模拟器被用于训练和评估。这种用户模拟器基于对单个目标项目的了解来批判当前检索到的项目。然而，在离线环境中使用模拟器进行系统评估受到对单个目标项目和他们在大量轮次中无限耐心的关注的限制。

Method: 提出了 Fashion-AlterEval，这是一个新的数据集，其中包含人类对替代项目的选择的判断，通过在常见的时尚 CRS 数据集中添加新的注释。提出了两种新的元用户模拟器，它们使用收集的判断，并允许模拟用户不仅表达他们对替代项目的偏好，还可以改变他们的想法和耐心程度。

Result: 使用 Shoes 和 Fashion IQ 作为原始数据集和三个 CRS 模型进行的实验表明，模拟器使用替代方案的知识可以对现有 CRS 模型的评估产生相当大的影响，特别是现有单目标评估低估了它们的有效性，并且当允许模拟用户考虑替代相关项目时，系统可以快速响应以更快地满足用户。

Conclusion: 现有单目标评估低估了现有 CRS 模型的有效性，当模拟用户可以考虑替代相关项目时，系统可以快速响应以更快地满足用户。
...
### [134] [RecPS: Privacy Risk Scoring for Recommender Systems](https://arxiv.org/abs/2507.18365)
*Jiajie He,Yuechun Gu,Keke Chen*

Main category: cs.IR

TL;DR: This paper introduces RecPS, a membership-inference attack (MIA)-based privacy scoring method, to measure privacy risks in recommender systems (RecSys) at both the interaction and user levels. RecPS enables privacy-aware RecSys model development and deployment.
...
Result: The RecPS interaction-level score definition is motivated and derived from differential privacy, which is then extended to the user-level scoring method.
...
### [135] [LLM-based Embedders for Prior Case Retrieval](https://arxiv.org/abs/2507.18455)
*Damith Premasiri,Tharindu Ranasinghe,Ruslan Mitkov*

Main category: cs.IR

TL;DR: This paper uses LLM-based text embedders to improve prior case retrieval (PCR) and shows that they outperform traditional and supervised methods.


<details>
  <summary>Details</summary>
Motivation: traditional IR methods like BM25 are still used while deep learning IR methods have not been successful in PCR due to lengthy legal text limitation and lack of legal training data
...
### [136] [How Well Do LLMs Predict Prerequisite Skills? Zero-Shot Comparison to Expert-Defined Concepts](https://arxiv.org/abs/2507.18479)
*Ngoc Luyen Le,Marie-Hélène Abel*

Main category: cs.IR

TL;DR: 本文探讨了大型语言模型 (llm) 是否可以在零样本环境中预测必备技能，实验结果表明，某些 llm 在没有监督的情况下表现出强大的语义推理能力，与专家给出的标准答案非常一致。


<details>
  <summary>Details</summary>
Motivation: 掌握更高级的概念之前所需的基本能力，对于支持有效的学习、评估和技能差距分析非常重要。传统上由领域专家策划，这些关系维护成本高昂且难以扩展。

Method: 使用标准化的提示策略，我们评估了 13 个最先进的 llm，包括 gpt-4、claude 3、gemini、llama 4、qwen2 和 deepseek，跨越语义相似性、bertscore 和推理延迟。

Result: llama4-maverick、claude-3-7-sonnet 和 qwen2-72b 等模型生成的预测与专家给出的标准答案非常吻合，无需监督即可展示强大的语义推理能力。

Conclusion: 大型语言模型 (llm) 在没有监督的情况下表现出强大的语义推理能力，能够预测必备技能，与专家给出的标准答案非常一致。这表明 llm 在个性化学习、智能辅导和基于技能的推荐系统等应用中，具有支持可扩展的必备技能建模的潜力。
...
### [137] [The Best is Yet to Come: Graph Convolution in the Testing Phase for Multimodal Recommendation](https://arxiv.org/abs/2507.18489)
*Jinfeng Xu,Zheyu Chen,Shuo Yang,Jinze Li,Edith C. H. Ngai*

Main category: cs.IR

TL;DR: FastMMRec：一个高效的多模态推荐框架，它只在测试阶段使用图卷积，从而提高了效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 图卷积网络(GCNs)在训练推荐系统中的效率和可扩展性仍然是关键挑战，阻碍了它们在现实场景中的实际部署。在多模态推荐(MMRec)领域，训练GCNs需要更昂贵的时间和空间成本，并加剧了不同模态之间的差距，导致推荐精度下降。

Method: 提出了FastMMRec，一个高效的多模态推荐框架，仅在测试阶段部署图卷积。

Result: 仅在测试阶段采用GCNs可以显著提高模型的效率和可扩展性，同时缓解了在训练阶段使用GCNs经常导致的模态隔离问题。

Conclusion: FastMMRec在三个公共数据集上表现优于基线模型，并在效率和可扩展性方面有所提升。
...
### [138] [Transform Before You Query: A Privacy-Preserving Approach for Vector Retrieval with Embedding Space Alignment](https://arxiv.org/abs/2507.18518)
*Ruiqi He,Zekun Fei,Jiaqi Li,Xinyuan Zhu,Biao Yi,Siyi Lv,Weijie Liu,Zheli Liu*

Main category: cs.IR

TL;DR: STEER 是一种私有向量检索框架，它利用不同嵌入模型的对齐关系来推导查询文本的近似嵌入，从而保护查询文本隐私，同时保持检索准确性。


<details>
  <summary>Details</summary>
Motivation: 当前 VDB 服务提供商主要使用专有的黑盒模型，用户被迫通过 API 向他们公开原始查询文本，以换取向量检索服务。因此，如果查询文本涉及金融或医疗保健领域的机密记录，这种机制不可避免地会导致用户敏感信息的严重泄露。

Method: STEER 利用不同嵌入模型的语义空间之间的对齐关系来推导查询文本的近似嵌入。STEER 在原始 VDB 中使用近似嵌入执行检索，并且不需要对服务器端进行修改。

Result: STEER 的 Recall@100 基本上可以实现小于 5% 的下降。此外，即使在数百万条条目的文本语料库中搜索，STEER 的 Recall@20 准确率也比当前基线高 20%。

Conclusion: STEER 有效地保护了查询文本的隐私，同时保持了检索的准确性。即使近似嵌入是来自专有模型的嵌入的近似值，它们仍然可以防止提供商通过嵌入反演攻击 (EIA) 恢复查询文本。
...
### [139] [DR.EHR: Dense Retrieval for Electronic Health Record with Knowledge Injection and Synthetic Data](https://arxiv.org/abs/2507.18583)
*Zhengyun Zhao,Huaiyuan Ying,Yue Zhong,Sheng Yu*

Main category: cs.IR

TL;DR: Introduces DR.EHR, a series of dense retrieval models specifically tailored for EHR retrieval, achieving state-of-the-art results on the CliniQ benchmark.
...
### [140] [Enhancing Quantization-Aware Training on Edge Devices via Relative Entropy Coreset Selection and Cascaded Layer Correction](https://arxiv.org/abs/2507.17768)
*Yujia Tong,Jingling Yuan,Chuang Hu*

Main category: cs.LG

TL;DR: This paper introduces QuaRC, a quantization-aware training framework using coresets on edge devices, addressing the challenge of quantization errors in small-scale datasets by using Relative Entropy Score for coreset selection and Cascaded Layer Correction strategy during training. It achieves significant accuracy improvement compared to existing methods.


<details>
  <summary>Details</summary>
Motivation: Traditional QAT relies on the complete dataset for training, which incurs a huge computational cost, and existing coreset selection methods struggle to eliminate quantization errors in the model when using small-scale datasets, leading to significant performance degradation.

Method: The paper proposes QuaRC, a QAT framework with coresets on edge devices, which consists of two main phases: coreset selection phase using Relative Entropy Score and a training phase employing the Cascaded Layer Correction strategy.

Result: QuaRC demonstrates effectiveness through experimental results.

Conclusion: QuaRC achieves a 5.72% improvement in Top-1 accuracy on the ImageNet-1K dataset compared to state-of-the-art techniques when quantizing ResNet-18 to 2-bit using a 1% data subset.
...
### [141] [Knowledge Abstraction for Knowledge-based Semantic Communication: A Generative Causality Invariant Approach](https://arxiv.org/abs/2507.17784)
*Minh-Duong Nguyen,Quoc-Viet Pham,Nguyen H. Tran,Hoang-Khoi Do,Duy T. Ngo,Won-Joo Hwang*

Main category: cs.LG

TL;DR: This paper introduces a low-complexity AI model using causality-invariant learning for semantic communication, ensuring consistency across devices and improving data reconstruction.


<details>
  <summary>Details</summary>
Motivation: Improve data reconstruction of the channel decoder for semantic communication.
...
Result: Knowledge-based data reconstruction surpasses other state-of-the-art methods in terms of Peak Signal-to-Noise Ratio (PSNR).
...
### [142] [Self-similarity Analysis in Deep Neural Networks](https://arxiv.org/abs/2507.17785)
*Jingyi Ding,Chengwen Qi,Hongfei Wang,Jianshe Wu,Licheng Jiao,Yuwei Guo,Jian Gao*

Main category: cs.LG

TL;DR: analyzes how adjusting the degree of self-similarity in feature networks can enhance the classification performance of deep neural networks


<details>
  <summary>Details</summary>
Motivation: no quantitative analysis on how the self-similarity of hidden space geometry influences model weight optimization, nor is there a clear understanding of the dynamic behavior of internal neurons.

Method: a complex network modeling method based on the output features of hidden-layer neurons
...
### [143] [Reinforcement Learning for Accelerated Aerodynamic Shape Optimisation](https://arxiv.org/abs/2507.17786)
*Florian Sobieczky,Alfredo Lopez,Erika Dudkin,Christopher Lackner,Matthias Hochsteger,Bernhard Scheichl,Helmut Sobieczky*

Main category: cs.LG

TL;DR: 提出了一种基于强化学习的气动形状优化算法，通过局部优化参数变化加速全局优化，并可解释特征重要性。


<details>
  <summary>Details</summary>
Motivation: 最小化计算工作量，并使用观察到的优化结果来解释发现的极值在实现所需流场方面的作用。

Method: 基于强化学习（RL）的自适应优化算法，用于气动形状优化，重点是降维。采用基于代理的actor-critic策略评估MCMC方法，允许暂时“冻结”一些要优化的参数。

Result: 通过一个简单的流体动力学问题展示了该方法，该方法允许在特征重要性评分的意义上进行解释。

Conclusion: 该方法通过在中间CFD模拟周围进行一系列局部优化的参数更改，加速全局优化，并允许对特征重要性进行解释。
...
### [144] [Hyperbolic Deep Learning for Foundation Models: A Survey](https://arxiv.org/abs/2507.17787)
*Neil He,Hiren Madhu,Ngoc Bui,Menglin Yang,Rex Ying*

Main category: cs.LG

TL;DR: This paper reviews how hyperbolic neural networks can improve foundation models by addressing limitations in Euclidean space.


<details>
  <summary>Details</summary>
Motivation: Foundation models have limitations in representational capacity, adaptability, and scalability. Hyperbolic spaces offer a solution by enabling low-distortion embeddings of hierarchical structures and power-law distributions.

Method: review of hyperbolic neural networks and their recent development for foundation models

Result: Hyperbolic spaces enhance foundation models, including improving LLMs' complex reasoning ability, VLMs' zero-shot generalization, and cross-modal semantic alignment, while maintaining parameter efficiency.

Conclusion: This paper provides a comprehensive review of hyperbolic neural networks and their recent development for foundation models. It further outlines key challenges and research directions to advance the field.
...
### [145] [Adaptive Repetition for Mitigating Position Bias in LLM-Based Ranking](https://arxiv.org/abs/2507.17788)
*Ali Vardasbi,Gustavo Penha,Claudia Hauff,Hugues Bouchard*

Main category: cs.LG

TL;DR: 提出了一种动态提前停止方法，以减少 LLM 调用次数，同时保持或略微降低准确性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在根据给定标准对项目进行排序或评估答案时，候选项目的顺序会影响模型的最终决策，这种对 LLM 提示中项目定位的敏感性称为位置偏差。此外，LLM 还表现出不同程度的低重复一致性，即使用相同的候选顺序重复 LLM 调用可能会导致不同的排序。为了解决这两个不一致问题，一种常见的方法是使用不同的候选顺序多次提示模型，并通过多数投票来汇总结果。然而，这种重复策略会显着增加计算成本。

Method: 提出了一种动态提前停止方法，可以自适应地确定每个实例所需的重复次数，以解决位置偏差和重复一致性低的问题。

Result: 动态重复策略减少了 81% 的 LLM 调用次数，同时保持了准确性。基于置信度的自适应早期停止方法减少了 87% 的 LLM 调用次数，但准确性略有下降。

Conclusion: 提出了一个动态提前停止方法，该方法能够自适应地确定每个实例所需的重复次数。在三个不同大小的 LLM 和两个任务（即重新排序和对齐）上评估了该方法，结果表明，过渡到动态重复策略可将 LLM 调用次数平均减少 81%，同时保持准确性。此外，还提出了一种基于置信度的自适应早期停止方法，与静态重复相比，该方法可将 LLM 调用次数平均减少 87%，但相对于原始的提前停止方法，准确性略有下降。
...
### [146] [Helix 1.0: An Open-Source Framework for Reproducible and Interpretable Machine Learning on Tabular Scientific Data](https://arxiv.org/abs/2507.17791)
...
TL;DR: Helix is an open-source Python framework for reproducible and interpretable machine learning workflows on tabular data, featuring a user-friendly interface and novel interpretation approach.


<details>
  <summary>Details</summary>
Motivation: growing need for transparent experimental data analytics provenance

Method: an open-source, extensible, Python-based software framework
...
Conclusion: Helix under MIT licence is accessible via GitHub and PyPI, supporting community-driven development and promoting adherence to the FAIR principles.
...
### [147] [Causal Mechanism Estimation in Multi-Sensor Systems Across Multiple Domains](https://arxiv.org/abs/2507.17792)
*Jingyi Yu,Tim Pychynski,Marco F. Huber*

Main category: cs.LG

TL;DR: CICME是一种新的因果推断方法，它利用因果迁移学习来检测领域不变的因果机制，并在某些情况下优于现有方法。
...
Method: CICME，一种新颖的三步方法，用于从跨多个域收集的异构数据中推断因果机制。

Result: CICME能够可靠地检测领域不变的因果机制，并在从制造业流程中获得的线性高斯模型上进行了评估。

Conclusion: CICME在某些情况下优于现有因果发现方法。
...
### [148] [LSDM: LLM-Enhanced Spatio-temporal Diffusion Model for Service-Level Mobile Traffic Prediction](https://arxiv.org/abs/2507.17795)
*Shiyuan Zhang,Tong Li,Zhu Xiao,Hongyang Du,Kaibin Huang*

Main category: cs.LG

TL;DR: 提出了一种新的流量预测模型LSDM，该模型利用扩散模型和LLM来提高预测精度和适应性，并在真实数据集上取得了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 当前预测方法在不同城市环境中的适应性有限，并且由于个人流量模式的高度不确定性、缺乏详细的环境背景以及不同网络服务之间复杂的依赖关系，导致结果不准确。

Method: 提出了一种LLM增强的时空扩散模型(LSDM)，该模型结合了扩散模型的生成能力和transformer的自适应学习能力，并增强了捕获多模态环境信息的能力。

Result: 在真实的服务级数据集上的大量评估表明，该模型在流量使用预测方面表现出色，显示出出色的泛化性和适应性。通过LLM整合上下文信息后，性能提升至少2.83%。与同类型模型相比，均方根误差降低至少8.29%。
...
### [149] [CoCAI: Copula-based Conformal Anomaly Identification for Multivariate Time-Series](https://arxiv.org/abs/2507.17796)
*Nicholas A. Pearson,Francesca Zanello,Davide Russo,Luca Bortolussi,Francesca Cairoli*

Main category: cs.LG

TL;DR: CoCAI使用扩散模型进行预测，并使用copula进行异常检测，经验证在水分配和污水系统中有效。
...
Method: 扩散模型和基于copula的建模

Result: 通过结合降维技术和基于copula的建模，提供了一个统计意义上的异常评分。

Conclusion: CoCAI在准确预测目标数据序列和识别其中的异常片段方面是有效的。
...
### [150] [GenSelect: A Generative Approach to Best-of-N](https://arxiv.org/abs/2507.17797)
...
### [151] [Wasserstein GAN-Based Precipitation Downscaling with Optimal Transport for Enhancing Perceptual Realism](https://arxiv.org/abs/2507.17798)
*Kenta Shiraishi,Yuka Muto,Atsushi Okazaki,Shunji Kotsuki*

Main category: cs.LG

TL;DR: 本研究提出使用WGAN进行降水降尺度，结果表明WGAN不仅提高了感知真实感，还为评估降水数据集提供了新视角。


<details>
  <summary>Details</summary>
Motivation: 高分辨率（HR）降水预测对于减少静止和局部强降雨造成的损害至关重要；然而，使用过程驱动的数值天气预报模型进行HR降水预报仍然具有挑战性。

Method: 使用Wasserstein生成对抗网络（WGAN）执行具有最佳运输成本的降水降尺度。

Result: 与使用均方误差训练的传统神经网络相比，WGAN生成了具有精细结构的可视逼真的降水场，即使WGAN在传统评估指标上表现出稍低的性能。WGAN的学习评论家与人类的感知真实感密切相关。基于案例的分析表明，评论家分数的巨大差异可以帮助识别不真实的WGAN输出和参考数据中的潜在伪影。

Conclusion: WGAN框架提高了降水降尺度中的感知真实感，并为评估和质量控制降水数据集提供了新的视角。
...
### [152] [Explainable Graph Neural Networks via Structural Externalities](https://arxiv.org/abs/2507.17848)
*Lijun Wu,Dong Hao,Zhiyi Fan*

Main category: cs.LG

TL;DR: propose GraphEXT to enhance the explainability of GNN models
...
Result: quantifies node importance through their marginal contributions to GNN predictions as the nodes transition between coalitions
...
### [153] [Look the Other Way: Designing 'Positive' Molecules with Negative Data via Task Arithmetic](https://arxiv.org/abs/2507.17876)
*Rıza Özçelik,Sarah de Ruiter,Francesca Grisoni*

Main category: cs.LG

TL;DR: Molecular task arithmetic trains a model on negative examples to generate positive molecules, outperforming models trained on positive data and increasing design diversity.
...
Method: molecular task arithmetic: training a model on diverse and abundant negative examples to learn 'property directions' and moving models in the opposite property directions to generate positive molecules.

Result: molecular task arithmetic generated more diverse and successful designs than models trained on positive molecules.  molecular task arithmetic can consistently increase the diversity of designs while maintaining desirable design properties.

Conclusion: Molecular task arithmetic consistently increases the diversity of designs while maintaining desirable design properties. It has the potential to become the de-facto transfer learning strategy for de novo molecule design.
...
### [154] [Fourier Neural Operators for Non-Markovian Processes:Approximation Theorems and Experiments](https://arxiv.org/abs/2507.17887)
*Wonjae Lee,Taeyoung Kim,Hyungbin Park*

Main category: cs.LG

TL;DR: MFNO 是一种新型神经网络，通过镜像填充扩展了 FNO，可以处理非周期性输入，在随机微分方程和分数布朗运动方面表现出色，并且具有强大的分辨率泛化能力。


<details>
  <summary>Details</summary>
Motivation: 标准傅里叶神经算子 (FNO) 无法处理非周期性输入。

Method: 引入了基于算子的神经网络，镜像填充傅里叶神经算子 (MFNO)，旨在学习随机系统的动力学。

Result: 证明了 MFNO 可以任意程度地逼近路径相关的随机微分方程和分数布朗运动的 Lipschitz 变换的解。此外，MFNO 表现出强大的分辨率泛化能力，这在 LSTM、TCN 和 DeepONet 等标准架构中很少见。

Conclusion: MFNO模型表现与现有模型持平或更好，且样本路径生成速度显著快于传统数值方案。
...
### [155] [Lower Bounds for Public-Private Learning under Distribution Shift](https://arxiv.org/abs/2507.17895)
*Amrith Setlur,Pratiksha Thaker,Jonathan Ullman*

Main category: cs.LG

TL;DR: 研究了公共和私有数据源的分布偏移对差分隐私机器学习算法的影响，发现当偏移较小时，需要充足的数据，而当偏移较大时，公共数据无益。


<details>
  <summary>Details</summary>
Motivation: 实践中最有效的差分隐私机器学习算法依赖于额外的据称是公共数据源。当这两个来源结合起来大于它们各部分的总和时，这种范例是最有趣的。然而，在诸如均值估计之类的设置中，我们有很强的下界，表明当两个数据源具有相同的分布时，组合两个数据源没有互补价值。

Method: 扩展了已知的公共-私有学习的下界，以设置两个数据源表现出显着分布偏移的情况。我们的结果适用于高斯均值估计（其中两个分布具有不同的均值）和高斯线性回归（其中两个分布表现出参数偏移）。

Result: 当分布偏移较小（相对于所需的精度）时，公共或私有数据必须足够丰富才能估计私有参数。相反，当偏移较大时，公共数据没有提供任何好处。

Conclusion: 当分布偏移较小（相对于所需的精度）时，公共或私有数据必须足够丰富才能估计私有参数。相反，当偏移较大时，公共数据没有提供任何好处。
...
### [156] [Federated Learning for Large-Scale Cloud Robotic Manipulation: Opportunities and Challenges](https://arxiv.org/abs/2507.17903)
*Obaidullah Zaland,Chanh Nguyen,Florian T. Pokorny,Monowar Bhuyan*

Main category: cs.LG

TL;DR: 本文探讨了联邦学习在云机器人操作中的应用前景，重点关注机遇与挑战。


<details>
  <summary>Details</summary>
Motivation: 当前机器人操作任务受到机器人个体能力和速度的限制，因为低延迟计算资源有限。因此，云机器人概念应运而生，允许机器人应用程序利用计算资源的灵活性和可靠性，有效地缓解其在云边缘连续体上的计算需求。

Method: 联邦学习

Result: 联邦学习在云机器人操作场景中具有多方面的优势，同时也带来了一些挑战和机遇。

Conclusion: 我们介绍了联邦学习的基本概念及其与云机器人操作的联系。此外，我们设想了通过联邦学习实现大规模高效可靠的云机器人操作相关的机遇和挑战，研究人员采用在集中式或分散式环境中设计和验证联邦学习模型。
...
### [157] [Deep learning-aided inverse design of porous metamaterials](https://arxiv.org/abs/2507.17907)
*Phu Thien Nguyen,Yousef Heider,Dennis M. Kochmann,Fadi Aldakheel*

Main category: cs.LG

TL;DR: 本研究利用深度学习框架 pVAE，结合 LBM 和 CNN，实现了多孔超材料的逆向设计，能够根据所需的水力特性生成新型超材料。
...
Method: 开发了一种基于深度学习的生成框架，特别是属性变分自动编码器 (pVAE)，这是一种通过回归器增强的变分自动编码器 (VAE)，用于生成具有定制水力特性的结构化超材料。

Result: 使用格子玻尔兹曼方法 (LBM) 生成有限多孔微结构的固有渗透率张量数据，并使用自下而上的方法训练卷积神经网络 (CNN) 来预测有效水力特性。与直接 LBM 模拟相比，这显着降低了计算成本。pVAE 框架在两个数据集上进行训练：人工多孔微结构的合成数据集和来自真实开孔泡沫的体积元素的 CT 扫描图像。
...
### [158] [SETOL: A Semi-Empirical Theory of (Deep) Learning](https://arxiv.org/abs/2507.17912)
*Charles H Martin,Christopher Hinrichs*

Main category: cs.LG

TL;DR: This paper presents a SemiEmpirical Theory of Learning (SETOL) that explains the remarkable performance of State-Of-The-Art (SOTA) Neural Networks (NNs).


<details>
  <summary>Details</summary>
Motivation: This paper explains the remarkable performance of State-Of-The-Art (SOTA) Neural Networks (NNs).

Method: This paper uses techniques from statistical mechanics as well as advanced methods from random matrix theory and quantum chemistry.

Result: This paper provides a formal explanation of the origin of the fundamental quantities in the phenomenological theory of Heavy-Tailed Self-Regularization (HTSR): the heavy-tailed power-law layer quality metrics, alpha and alpha-hat. The derivation suggests new mathematical preconditions for ideal learning, including a new metric, ERG, which is equivalent to applying a single step of the Wilson Exact Renormalization Group. The assumptions and predictions of SETOL are tested on a simple 3-layer multilayer perceptron (MLP), demonstrating excellent agreement with the key theoretical assumptions. For SOTA NN models, this paper shows how to estimate the individual layer qualities of a trained NN by simply computing the empirical spectral density (ESD) of the layer weight matrices and plugging this ESD into our SETOL formulas.

Conclusion: The performance of the HTSR alpha and the SETOL ERG layer quality metrics align remarkably well, both on our MLP and on SOTA NNs.
...
### [159] [From Seed to Harvest: Augmenting Human Creativity with AI for Red-teaming Text-to-Image Models](https://arxiv.org/abs/2507.17922)
*Jessica Quaye,Charvi Rastogi,Alicia Parrish,Oana Inel,Minsuk Kahng,Lora Aroyo,Vijay Janapa Reddi*

Main category: cs.LG

TL;DR: Proposes Seed2Harvest, a hybrid red-teaming method for generating diverse, human-crafted adversarial prompts to improve the safety evaluation of text-to-image models.


<details>
  <summary>Details</summary>
Motivation: Current techniques for generating adversarial prompts are either entirely authored by humans or synthetically generated, both of which have limitations.
...
Result: The resulting prompts preserve the characteristics and attack patterns of human prompts while maintaining comparable average attack success rates and achieving substantially higher diversity.
...
Conclusion: UrbanPulse achieves state-of-the-art accuracy and scalability. Through efficient transfer learning, UrbanPulse takes a key step toward making high-resolution, AI-powered urban forecasting deployable in practice across diverse cities.
...
### [161] [Multimodal Fine-grained Reasoning for Post Quality Evaluation](https://arxiv.org/abs/2507.17934)
*Xiaoxu Guo,Siyan Liang,Yachao Cui,Juxiang Zhou,Lei Wang,Han Cao*

Main category: cs.LG

TL;DR: 提出了MFTRR框架，通过多模态细粒度推理提升帖子质量评估，实验表明其优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的研究面临三个主要限制：（1）将该任务视为单模态分类，未能利用多模态线索和细粒度的质量区分；（2）在深度多模态融合过程中引入噪声，导致误导性信号；（3）缺乏捕捉复杂语义关系（如相关性和全面性）的能力。

Method: 提出了多模态细粒度主题-帖子关系推理（MFTRR）框架，该框架模仿人类认知过程，将帖子质量评估重新定义为一个排序任务，并结合多模态数据以更好地捕捉质量变化。它由两个关键模块组成：（1）局部-全局语义相关推理模块，该模块在局部和全局层面建模帖子和主题之间的细粒度语义交互，并通过最大信息融合机制来抑制噪声；（2）多层次证据关系推理模块，该模块探索宏观和微观层面的关系线索，以加强基于证据的推理。

Result: MFTRR显著优于最先进的基线方法，在艺术史数据集上，相比最佳单模态方法，NDCG@3指标提升高达9.52%。

Conclusion: MFTRR在三个新建的多模态主题-帖子数据集和公共Lazada-Home数据集上进行了评估。实验结果表明，MFTRR显著优于最先进的基线方法，在艺术史数据集上，相比最佳单模态方法，NDCG@3指标提升高达9.52%。
...
### [162] [VIBE: Video-Input Brain Encoder for fMRI Response Modeling](https://arxiv.org/abs/2507.17958)
...
<details>
  <summary>Details</summary>
Motivation: to predict fMRI activity
...
### [163] [Improving the Computational Efficiency and Explainability of GeoAggregator](https://arxiv.org/abs/2507.17977)
*Rui Deng,Ziqi Li,Mingshu Wang*

Main category: cs.LG

TL;DR: 本文改进了GeoAggregator模型，提高了计算效率和可解释性，并在合成数据集上验证了改进的有效性。


<details>
  <summary>Details</summary>
Motivation: 准确地建模和解释地理空间表格数据（GTD）对于理解地理空间现象及其潜在过程至关重要。最近的工作提出了一种基于Transformer的新型深度学习模型GeoAggregator（GA）。

Method: 通过开发优化的数据加载流程和简化GA的前向传递来提高计算效率；结合模型集成策略和基于GeoShapley框架的事后模型解释功能来增强模型的可解释性。

Result: 实验结果表明，该实现提高了GA的预测精度和推理速度。解释实验表明，GA可以有效地捕捉设计的合成数据集中的固有空间效应。

Conclusion: 改进后的GA模型在预测精度和推理速度上优于原始实现，并且能够有效地捕捉合成数据集中固有的空间效应。
...
### [164] [SIFOTL: A Principled, Statistically-Informed Fidelity-Optimization Method for Tabular Learning](https://arxiv.org/abs/2507.17979)
*Shubham Mohole,Sainyam Galhotra*

Main category: cs.LG

TL;DR: SIFOTL 是一种新的表格学习方法，它使用隐私安全的数据摘要统计信息，通过双 XGBoost 模型和 Pareto 加权决策树来识别导致数据变化的因素，且对观察噪声具有鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 识别表格数据集中驱动数据变化的因素对于分析和决策支持系统来说是一个重大的挑战，尤其是在医疗保健领域。隐私规则限制了数据访问，而复杂流程中的噪声阻碍了分析。

Method: SIFOTL（用于表格学习的统计知情保真度优化方法）：(i) 提取符合隐私规定的数据摘要统计信息，(ii) 采用双 XGBoost 模型，借助 LLM 的帮助，从噪声中分离出干预信号，以及 (iii) 通过 Pareto 加权决策树合并 XGBoost 输出，以识别导致变化的的可解释片段。

Result: 对于模拟新的 Medicare 药物补贴的 MEPS 面板数据集，SIFOTL 的 F1 得分为 0.85，大大优于 BigQuery 贡献分析 (F1=0.46) 和统计测试 (F1=0.20)。在基于 Synthea ABM 生成的 18 个不同的 EHR 数据集中，SIFOTL 在没有噪声的情况下保持 0.86-0.96 的 F1 分数，即使在注入观察噪声的情况下也能保持 >= 0.75，而基线平均 F1 分数在相同的测试下范围为 0.19-0.67。
...
### [165] [Machine Unlearning of Traffic State Estimation and Prediction](https://arxiv.org/abs/2507.17984)
*Xin Wang,R. Tyrrell Rockafellar,Xuegang,Ban*

Main category: cs.LG

TL;DR: The paper introduces Machine Unlearning TSEP to address privacy, cybersecurity, and data freshness concerns in data-driven traffic state estimation and prediction (TSEP).


<details>
  <summary>Details</summary>
Motivation: Data-driven traffic state estimation and prediction (TSEP) relies heavily on data sources that contain sensitive information, raising concerns regarding privacy, cybersecurity, and data freshness. Regulations have introduced the "right to be forgotten", allowing users to request the removal of their private data from models, but machine learning models can remember old data, making simple removal insufficient.
...
### [166] [Predictive Scaling Laws for Efficient GRPO Training of Large Reasoning Models](https://arxiv.org/abs/2507.18014)
*Datta Nimmaturi,Vaishnavi Bhargava,Rajat Ghosh,Johnu George,Debojyoti Dutta*

Main category: cs.LG

TL;DR: proposes a predictive framework that models training dynamics and helps optimize resource usage for LLMs


<details>
  <summary>Details</summary>
Motivation: Fine-tuning large language models (LLMs) for reasoning tasks using reinforcement learning methods is computationally expensive

Method: a predictive framework that models training dynamics

Result: derives an empirical scaling law based on model size, initial performance, and training progress and identifies three consistent training phases: slow start, rapid improvement, and plateau

Conclusion: Training beyond certain epoch offers little gain, suggesting earlier stopping can significantly reduce compute without sacrificing performance
...
### [167] [Multiscale Neural PDE Surrogates for Prediction and Downscaling: Application to Ocean Currents](https://arxiv.org/abs/2507.18067)
*Abdessamad El-Kabid,Loubna Benabbou,Redouane Lguensat,Alex Hernández-García*

Main category: cs.LG

TL;DR: introduce a supervised deep learning framework based on neural operators for solving PDEs and providing arbitrary resolution solutions, and propose downscaling models with an application to Copernicus ocean current data
...
Method: a supervised deep learning framework based on neural operators for solving PDEs and providing arbitrary resolution solutions

Result: downscaling models with an application to Copernicus ocean current data. Additionally, our method can model surrogate PDEs and predict solutions at arbitrary resolution, regardless of the input resolution.
...
### [168] [Group Sequence Policy Optimization](https://arxiv.org/abs/2507.18071)
...
TL;DR: GSPO是一种用于训练大型语言模型的强化学习算法，它在序列级别上运行，提高了训练效率和性能，并简化了RL基础设施的设计。


<details>
  <summary>Details</summary>
Motivation: 现有的算法采用token级别的比率，而GSPO基于序列可能性定义了重要性比率。

Method: 提出了Group Sequence Policy Optimization (GSPO) 算法。

Result: GSPO在训练效率和性能上优于GRPO算法，稳定了混合专家(MoE) RL训练。

Conclusion: GSPO提升了Qwen3模型的性能。
...
### [169] [C-AAE: Compressively Anonymizing Autoencoders for Privacy-Preserving Activity Recognition in Healthcare Sensor Streams](https://arxiv.org/abs/2507.18072)
*Ryusei Fujimoto,Yugo Nakamura,Yutaka Arakawa*

Main category: cs.LG

TL;DR: C-AAE通过结合匿名自编码器和自适应差分脉冲编码调制，在降低用户重新识别风险的同时，保持了活动识别精度并减少了数据量。


<details>
  <summary>Details</summary>
Motivation: 可穿戴加速度计和陀螺仪包含精细的行为特征，可能被用于重新识别用户，因此隐私保护对医疗保健应用至关重要。

Method: 结合了匿名自编码器(AAE)与自适应差分脉冲编码调制(ADPCM)。AAE将原始传感器数据投影到潜在空间，保留活动相关特征并抑制身份信息。ADPCM对潜在空间的数据流进行差分编码，进一步掩盖身份信息并降低比特率。

Result: C-AAE相对于单独使用AAE，用户重新识别的F1分数降低了10-15个百分点，同时活动识别的F1分数与未保护的基线相比仅降低了5个百分点。ADPCM还将数据量减少了大约75%。

Conclusion: C-AAE在保持活动识别精度的同时，降低了用户重新识别的风险，并减少了数据量，为医疗保健中基于传感器的活动识别提供了一条平衡隐私和效用的实用途径。
...
### [170] [Squeeze10-LLM: Squeezing LLMs' Weights by 10 Times via a Staged Mixed-Precision Quantization Method](https://arxiv.org/abs/2507.18073)
...
TL;DR: Squeeze10-LLM 是一种新的量化框架，可以将 LLM 的权重压缩 10 倍，同时保持较高的准确率。


<details>
  <summary>Details</summary>
Motivation: 由于其大量的参数和高计算成本，部署大型语言模型 (LLM) 具有挑战性。超低比特量化可以显著减少存储并加速推理，但极端压缩（即，平均位宽 <= 2）通常会导致严重的性能下降。

Method: 一种分阶段的混合精度后训练量化 (PTQ) 框架，通过将 80% 的权重量化为 1 位，20% 量化为 4 位，实现了平均每权重 1.6 位的量化。

Result: Squeeze10-LLM 将 16 位 LLM 的权重有效地“压缩”了 10 倍，在 LLaMA 和 LLaMA2 上的实验表明，Squeeze10-LLM 在 sub-2bit weight-only 量化方面实现了最先进的性能，在六个 zero-shot 分类任务上的平均准确率从 43% 提高到 56%。

Conclusion: Squeeze10-LLM在 sub-2bit weight-only 量化方面实现了最先进的性能，在六个 zero-shot 分类任务上的平均准确率从 43% 提高到 56%，显著优于现有的 PTQ 方法。
...
### [171] [Learning from Hard Labels with Additional Supervision on Non-Hard-Labeled Classes](https://arxiv.org/abs/2507.18098)
*Kosuke Sugiyama,Masato Uchida*

Main category: cs.LG

TL;DR: 当训练数据有限时，通过利用硬标签和额外的监督来构建高精度分类模型。理论分析和实验结果表明，额外的监督信息可以提高分类精度。


<details>
  <summary>Details</summary>
Motivation: 在训练数据由于观察成本或数据稀缺而受到限制的情况下，丰富与每个实例相关的标签信息对于构建高精度分类模型至关重要。

Method: 提出了一个理论框架，该框架将硬标签和额外的监督都视为概率分布，并通过它们的仿射组合来构建软标签。

Result: 额外的监督的本质不是分配的硬标签的置信度分数，而是非硬标签类上的分布信息。额外的监督和混合系数以互补的方式促进软标签的细化。通过泛化误差分析，从理论上描述了额外的监督及其混合系数如何影响误差界的收敛速度和渐近值。

Conclusion: 设计的额外监督可以提高分类精度，即使以简单的方式使用。
...
### [172] [Percentile-Based Deep Reinforcement Learning and Reward Based Personalization For Delay Aware RAN Slicing in O-RAN](https://arxiv.org/abs/2507.18111)
*Peyman Tehrani,Anas Alsoliman*

Main category: cs.LG

TL;DR: This paper introduces PDA-DRL for RAN slicing in O-RAN, achieving superior delay reduction and proposes a novel reward-based model personalization method for MVNOs.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of RAN slicing in O-RAN architecture, focusing on MVNOs competing for PRBs while meeting probabilistic delay constraints and minimizing PRB utilization.
...
Conclusion: The paper introduces a reward-based personalization method for model weight sharing among MVNOs, which outperforms traditional aggregation methods.
...
### [173] [Policy Disruption in Reinforcement Learning:Adversarial Attack with Large Language Models and Critical State Identification](https://arxiv.org/abs/2507.18113)
*Junyong Jiang,Buwei Tian,Chenxing Xu,Songze Li,Lu Dong*

Main category: cs.LG

TL;DR: The paper presents a new adversarial attack method for RL systems that uses LLMs to generate adversarial rewards, guiding the target policy to make suboptimal decisions. The method outperforms existing approaches.


<details>
  <summary>Details</summary>
Motivation: Existing adversarial attack methods against RL systems often rely on modifying the environment or policy, limiting their practicality. There is a need for more effective and practical attack methods.

Method: The authors propose a reward iteration optimization framework that leverages large language models (LLMs) to generate adversarial rewards. They also design a critical state identification algorithm to pinpoint the target agent's most vulnerable states.

Result: Experimental results in diverse environments demonstrate the superiority of the proposed method over existing approaches.

Conclusion: This paper introduces a novel adversarial attack method against reinforcement learning (RL) systems. The method uses existing agents to guide the target policy towards suboptimal actions by generating adversarial rewards tailored to the target agent's vulnerabilities. Experimental results demonstrate the method's superiority over existing approaches.
...
### [174] [Maximizing Prefix-Confidence at Test-Time Efficiently Improves Mathematical Reasoning](https://arxiv.org/abs/2507.18122)
*Matthias Otth,Jonas Hübotter,Ido Hakimi,Andreas Krause*

Main category: cs.LG

TL;DR: 该论文研究了使用语言模型自身置信度来提高数学推理能力的方法，发现选择模型最有信心的预测可以显著提高性能，且优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 语言模型可以通过最大化自己对其预测的置信度来自我改进，而无需依赖外部验证器或奖励信号。

Method: 使用模型自身的前缀置信度来选择最有希望的尝试，从而研究语言模型在数学推理任务中的测试时间缩放。

Result: 前缀置信度缩放仅使用 32 个 tokens 的前缀，即可实现比多数投票更好的准确率-计算权衡。此外，前缀置信度缩放似乎比 BoN 更不易受到长度偏差的影响。使用前缀置信度进行的测试时间训练优于基础模型，但并未改进前缀置信度缩放。

Conclusion: 通过仅延续模型前缀置信度选择的最有希望的尝试，可以显著提高性能。
...
### [175] [Neuromorphic Computing for Embodied Intelligence in Autonomous Systems: Current Trends, Challenges, and Future Directions](https://arxiv.org/abs/2507.18139)
*Alberto Marchisio,Muhammad Shafique*

Main category: cs.LG

TL;DR: 本文综述了神经形态计算在自主系统中的应用进展，包括算法、硬件和优化策略，重点关注能源效率、鲁棒性和实时决策。
...
Method: 调查了神经形态算法、专用硬件和跨层优化策略的最新进展，重点关注它们在现实自主场景中的部署。

Result: 重点介绍了通过将脉冲神经网络集成到自主系统架构中来提高能源效率、鲁棒性、适应性和可靠性的新方法。特别关注了基于事件的动态视觉传感器及其在实现快速、高效感知中的作用。

Conclusion: 探讨了新兴趋势和开放性挑战，特别是在实时决策、持续学习以及安全、弹性的自主系统开发领域。
...
### [176] [When Noisy Labels Meet Class Imbalance on Graphs: A Graph Augmentation Method with LLM and Pseudo Label](https://arxiv.org/abs/2507.18153)
*Riting Xia,Rucong Wang,Yulin Liu,Anchen Li,Xueyan Liu,Yan Zhang*

Main category: cs.LG

TL;DR: 本文研究了具有噪声标签的类不平衡图的鲁棒节点分类问题，并提出了GraphALP框架，该框架基于大型语言模型和伪标签技术，通过生成合成少数类节点和动态加权伪标签来缓解类不平衡和标签噪声问题。


<details>
  <summary>Details</summary>
Motivation: 现实世界图中的标签经常包含噪声，因此需要研究具有噪声标签的类不平衡图的鲁棒节点分类。

Method: 基于大型语言模型（LLM）和伪标签技术的图增强框架。

Result: GraphALP在具有噪声标签的类不平衡图上实现了优于现有方法的性能。

Conclusion: GraphALP在具有噪声标签的类不平衡图上实现了优于现有方法的性能。
...
### [177] [ChronoSelect: Robust Learning with Noisy Labels via Dynamics Temporal Memory](https://arxiv.org/abs/2507.18183)
...
<details>
  <summary>Details</summary>
Motivation: Noisy labels in real-world datasets degrade the generalization performance of deep neural networks. Existing LNL methods fail to leverage the temporal dynamics of learning evolution.
...
Result: ChronoSelect enables precise three-way sample partitioning and demonstrates state-of-the-art performance.
...
### [178] [Goal-based Trajectory Prediction for improved Cross-Dataset Generalization](https://arxiv.org/abs/2507.18196)
*Daniel Grimm,Ahmed Abouelazm,J. Marius Zöllner*

Main category: cs.LG

TL;DR: 本文介绍了一种新的 GNN，它利用异构图和矢量化道路网络来改进轨迹预测，从而更好地泛化到未见场景。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的模型在真实数据集上训练时已经显示出有希望的结果，但是当这些模型被部署到新的/未见过的区域时，性能会显著下降，表明这些模型缺乏泛化能力。

Method: 提出了一种新的图神经网络 (GNN)，它利用由交通参与者和矢量化道路网络组成的异构图。

Result: 通过交叉数据集评估（即在 Argoverse2 上训练并在 NuScenes 上评估）表明了目标选择过程的有效性。

Conclusion: 本文提出了一个多阶段方法，通过在异构图神经网络中利用矢量化道路网络来分类目标，从而更好地泛化到未见场景，并通过交叉数据集评估展示了目标选择过程的有效性。
...
### [179] [FedSA-GCL: A Semi-Asynchronous Federated Graph Learning Framework with Personalized Aggregation and Cluster-Aware Broadcasting](https://arxiv.org/abs/2507.18219)
*Zhongzheng Yuan,Lianshuai Guo,Xunkai Li,Yinlin Zhu,Wenyu Wang,Meixia Qu*

Main category: cs.LG

TL;DR: 提出了FedSA-GCL，一个半异步联邦框架，用于高效的联邦图学习，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的FGL方法依赖于同步通信，导致效率低下，并且在实际部署中通常是不切实际的。同时，目前的异步联邦学习（AFL）方法主要是为传统的任务设计的，如图像分类和自然语言处理，没有考虑到图数据的独特拓扑属性。直接将这些方法应用于图学习可能会导致全局模型中的语义漂移和表征不一致。

Method: 提出了FedSA-GCL，这是一个半异步联邦框架，通过一种新颖的ClusterCast机制，利用客户端间标签分布差异和图拓扑特征进行高效训练。

Result: FedSA-GCL的鲁棒性和效率均优于基线。

Conclusion: FedSA-GCL在多个真实世界的图数据集上进行了评估，使用Louvain和Metis分割算法，并与9个基线进行了比较。大量的实验表明，该方法具有很强的鲁棒性和出色的效率，使用Louvain时平均优于基线2.92%，使用Metis时平均优于基线3.4%。
...
### [180] [Sparse identification of nonlinear dynamics with library optimization mechanism: Recursive long-term prediction perspective](https://arxiv.org/abs/2507.18220)
*Ansei Yonezawa,Heisei Yonezawa,Shuichi Yahagi,Itsuro Kajiwara,Shinya Kijimoto,Hikaru Taniuchi,Kentaro Murakami*

Main category: cs.LG

TL;DR: 提出了一种具有库优化机制 (SINDy-LOM) 的 SINDy，它可以结合稀疏回归技术和库的优化策略，从递归长期 (RLT) 预测精度的角度优化基函数。


<details>
  <summary>Details</summary>
Motivation: SINDy 的一个主要挑战是库的设计，库是一组候选基函数，因为对于许多动力系统来说，合适的库并非易事。为了克服这个困难

Method: 该研究提出了一种具有库优化机制 (SINDy-LOM) 的 SINDy，它是稀疏回归技术和库的新型学习策略的结合。在该方法中，基函数被参数化。SINDy-LOM 方法涉及一个两层优化架构：内层，其中数据驱动模型被提取为候选基函数的稀疏线性组合；外层，其中从递归长期 (RLT) 预测精度的角度优化基函数；因此，库设计被重新定义为参数化基函数的优化。

Result: 通过将其应用于柴油发动机进气系统（一种众所周知的复杂工业系统）证明了所提出方法的有效性。

Conclusion: SINDy-LOM 模型具有良好的可解释性和可用性，因为该方法可以生成简约模型。与只能确保提前一步预测准确性的传统 SINDy 方法相比，库优化机制显着减少了用户负担，并且 RLT 视角提高了生成模型的可靠性。
...
### [181] [Boosting Revisited: Benchmarking and Advancing LP-Based Ensemble Methods](https://arxiv.org/abs/2507.18242)
*Fabian Akkerman,Julien Ferry,Christian Artigues,Emmanuel Hebrard,Thibaut Vidal*

Main category: cs.LG

TL;DR: Large-scale study of LP-based boosting shows it can match/outperform XGBoost/LightGBM with sparser ensembles, and thin pre-trained ensembles without performance loss.
...
Method: first large-scale experimental study of six LP-based boosting formulations, including two novel methods, NM-Boost and QRLP-Boost, across 20 diverse datasets. evaluate the use of both heuristic and optimal base learners within these formulations

Result: evaluate not only accuracy, but also ensemble sparsity, margin distribution, anytime performance, and hyperparameter sensitivity. highlight both the strengths and limitations of using optimal decision trees in this context

Conclusion: totally corrective methods can outperform or match state-of-the-art heuristics like XGBoost and LightGBM when using shallow trees, while producing significantly sparser ensembles. these methods can thin pre-trained ensembles without sacrificing performance
...
### [182] [Leveraging Data Augmentation and Siamese Learning for Predictive Process Monitoring](https://arxiv.org/abs/2507.18293)
*Sjoerd van Straten,Alessandro Padella,Marwan Hassani*

Main category: cs.LG

TL;DR: SiamSA-PPM 是一种用于预测过程监控的自监督学习框架，它结合了 Siamese 学习和统计增强，在实际事件日志上表现出色。


<details>
  <summary>Details</summary>
Motivation: 深度学习 PPM 方法通常受到真实世界事件日志的低变异性和小尺寸的限制。为了解决这个问题，我们引入了 SiamSA-PPM。

Method: SiamSA-PPM，一个新颖的自监督学习框架，它结合了 Siamese 学习和统计增强来进行预测过程监控。它采用了三种新的统计基础转换方法，利用控制流语义和频繁的行为模式来生成真实的、语义上有效的新轨迹变体。这些增强的视图在 Siamese 学习设置中使用，以学习过程前缀的可推广表示，而无需标记监督。

Result: 在真实事件日志上的大量实验表明，SiamSA-PPM 在下一个活动和最终结果预测任务中，与 SOTA 相比，实现了有竞争力或更优越的性能。我们的结果进一步表明，统计增强显著优于随机转换，并提高了数据的可变性。
...
### [183] [Self-Supervised Coarsening of Unstructured Grid with Automatic Differentiation](https://arxiv.org/abs/2507.18297)
*Sergei Shumilin,Alexander Ryabov,Nikolay Yavich,Evgeny Burnaev,Vladimir Vanovskiy*

Main category: cs.LG

TL;DR: 提出了一种基于可微物理概念的非结构化网格粗化算法，通过k-means聚类、自动微分和随机最小化算法，在保证模型变量动态的前提下，最多可减少10倍的网格点数量。


<details>
  <summary>Details</summary>
Motivation: 由于现代数值模拟的高计算负载，因此需要能够减少离散问题规模同时保持合理精度的 подхода。

Method: 我们通过采用 k-means 聚类、自动微分和随机最小化算法来实现这一点。

Result: 我们在线性抛物型方程（控制多孔介质中稍微可压缩的流体流动）和波动方程这两个偏微分方程上展示了所设计算法的性能。

Conclusion: 在所考虑的场景中，我们将网格点的数量减少了 10 倍，同时保留了感兴趣点的建模变量动态。所提出的方法可以应用于由演化偏微分方程描述的任意系统的模拟。
...
### [184] [Regression-aware Continual Learning for Android Malware Detection](https://arxiv.org/abs/2507.18313)
...
TL;DR: 该研究关注持续学习恶意软件检测器中的安全回归问题，并提出了一种回归感知惩罚方法来缓解该问题，实验表明该方法有效。


<details>
  <summary>Details</summary>
Motivation: 在基于机器学习的恶意软件检测器中，安全回归是一个关键但被忽视的问题。与遗忘不同，安全回归是指样本级别的有害预测变化，例如恶意软件样本在模型更新后逃避检测。这种回归会对安全关键型应用构成严重风险。

Method: 该研究将 Positive Congruent Training (PCT) 适应于 CL 设置，以模型无关的方式保留先前的预测行为。

Result: 该研究在 ELSA、Tesseract 和 AZ-Class 数据集上的实验表明，该方法能够有效减少不同 CL 场景中的回归，同时保持较强的检测性能。

Conclusion: 该研究提出了一种回归感知惩罚方法，以减轻 CL 恶意软件检测器中的安全回归问题，并在 ELSA、Tesseract 和 AZ-Class 数据集上进行了实验，结果表明该方法能够有效减少不同 CL 场景中的回归，同时保持较强的检测性能。
...
### [186] [Remembering the Markov Property in Cooperative MARL](https://arxiv.org/abs/2507.18333)
*Kale-ab Abebe Tessera,Leonard Hinckeldey,Riccardo Zamboni,David Abel,Amos Storkey*

Main category: cs.LG

TL;DR: 当前的多智能体强化学习算法通过学习简单的约定来绕过环境观察和记忆，而不是真正地进行推理。


<details>
  <summary>Details</summary>
Motivation: 当前无模型 MARL 算法使用简单的循环函数逼近器来解决使用部分信息推理他人的挑战。我们认为，这些方法的经验成功不是因为有效的马尔可夫信号恢复，而是因为学习了绕过环境观察和记忆的简单约定。

Method: 案例研究

Result: 共同适应的智能体可以学习脆弱的约定，当与非适应性智能体合作时，这些约定会失败。当任务设计需要时，相同的模型可以学习基于基础的策略，这表明问题不是学习模型的基本限制，而是基准设计的失败。

Conclusion: 现代多智能体强化学习环境可能没有充分测试 Dec-POMDP 的核心假设。因此，我们提倡建立在两个核心原则之上的新合作环境：(1) 基于观察的行为；(2) 基于记忆的关于其他智能体的推理，确保成功需要真正的技能，而不是脆弱的、共同适应的协议。
...
### [187] [Low-rank adaptive physics-informed HyperDeepONets for solving differential equations](https://arxiv.org/abs/2507.18346)
...
<details>
  <summary>Details</summary>
Motivation: HyperDeepONets improves expressivity, but it incurs high memory and computational costs due to the large number of output parameters required.
...
Result: PI-LoRA-HyperDeepONets achieve up to 70% reduction in parameters and consistently outperform regular HyperDeepONets in terms of predictive accuracy and generalization on both ordinary and partial differential equations.
...
### [188] [Efficient Uncertainty in LLMs through Evidential Knowledge Distillation](https://arxiv.org/abs/2507.18366)
*Lakshmana Sri Harsha Nemani,P. K. Srijith,Tomasz Kuśmierczyk*

Main category: cs.LG

TL;DR: This paper introduces a novel approach enabling efficient and effective uncertainty estimation in LLMs without sacrificing performance by distilling uncertainty-aware teacher models into compact student models.
...
Method: distill uncertainty-aware teacher models into compact student models sharing the same architecture but fine-tuned using Low-Rank Adaptation (LoRA).

Result: Students can achieve comparable or superior predictive and uncertainty quantification performance relative to their teacher models, while critically requiring only a single forward pass.
...
### [189] [A Comprehensive Review of Diffusion Models in Smart Agriculture: Progress, Applications, and Challenges](https://arxiv.org/abs/2507.18376)
...
### [190] [Multi-Model Ensemble and Reservoir Computing for River Discharge Prediction in Ungauged Basins](https://arxiv.org/abs/2507.18423)
*Mizuki Funato,Yohei Sawada*

Main category: cs.LG

TL;DR: HYPER, a novel method using multi-model ensemble and reservoir computing, offers robust, efficient, and generalizable discharge prediction, especially in ungauged basins, outperforming LSTM in data-scarce conditions.


<details>
  <summary>Details</summary>
Motivation: Many regions lack sufficient river discharge observations, limiting the skill of rainfall-runoff analyses. Achieving high accuracy, interpretability, and computational efficiency under data-scarce conditions remains a major challenge.
...
Result: In a data-rich scenario, HYPER (median KGE of 0.56) performed comparably to a benchmark LSTM (KGE 0.55) but required only 5% of its computational time. In a data-scarce scenario, HYPER maintained robust performance (KGE 0.55) and lower uncertainty, whereas the LSTM's performance degraded significantly (KGE -0.04).

Conclusion: HYPER provides a robust, efficient, and generalizable solution for discharge prediction, particularly in ungauged basins, making it applicable to a wide range of regions.
...
### [191] [Revisiting Bisimulation Metric for Robust Representations in Reinforcement Learning](https://arxiv.org/abs/2507.18519)
*Leiji Zhang,Zeyu Wang,Xin Li,Yao-Hui Li*

Main category: cs.LG

TL;DR: 本文改进了bisimulation metric，解决了传统方法的两个主要问题，并在实验中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 传统bisimulation metric存在两个问题：1) 无法表示某些独特的场景；2) 依赖于预定义的权重。

Method: 引入状态-动作对的度量，提出更精确的奖励差距定义和具有自适应系数的新型更新算子。

Result: 在DeepMind Control和Meta-World两个基准测试中进行了大量实验，证明了该方法的有效性。

Conclusion: 提出了改进的bisimulation metric，并通过实验验证了其有效性。
...
### [192] [GLANCE: Graph Logic Attention Network with Cluster Enhancement for Heterophilous Graph Representation Learning](https://arxiv.org/abs/2507.18521)
*Zhongtian Sun,Anoushka Harit,Alexandra Cristea,Christl A. Donnelly,Pietro Liò*

Main category: cs.LG

TL;DR: GLANCE is a novel GNN framework that integrates logic-guided reasoning, dynamic graph refinement, and adaptive clustering to enhance graph representation learning on heterophilous graphs.


<details>
  <summary>Details</summary>
Motivation: GNNs struggle on heterophilous graphs, where connected nodes differ in features or class labels. This limitation arises from indiscriminate neighbor aggregation and insufficient incorporation of higher-order structural patterns.
...
Conclusion: GLANCE achieves competitive performance, offering robust and interpretable solutions for heterophilous graph scenarios. The proposed framework is lightweight, adaptable, and uniquely suited to the challenges of heterophilous graphs.
...
### [193] [C2G-KD: PCA-Constrained Generator for Data-Free Knowledge Distillation](https://arxiv.org/abs/2507.18533)
*Magnus Bengtsson,Kenneth Östberg*

Main category: cs.LG

TL;DR: 我们提出了一种新的无数据知识蒸馏框架，该框架使用类条件生成器和 PCA 几何约束来生成合成样本，从而在 MNIST 上实现了良好的性能。


<details>
  <summary>Details</summary>
Motivation: 我们介绍了 C2G-KD，这是一个无数据知识蒸馏框架

Method: 基于类条件生成器，该生成器经过训练以生成由冻结的教师模型和源自 PCA 的几何约束引导的合成样本。

Result: MNIST 上的实验表明

Conclusion: 即使极少的类结构也足以引导有用的合成训练管道。
...
### [194] [The Price equation reveals a universal force-metric-bias law of algorithmic learning and natural selection](https://arxiv.org/abs/2507.18549)
*Steven A. Frank*
...
Method: a simple notational partitioning of change by the Price equation reveals a universal force-metric-bias (FMB) law: $\Deltaftheta = \mathbf{M}\,\mathbf{f} + \mathbf{b} + \mathbf{\xi}$.
...
### [195] [The Geometry of LLM Quantization: GPTQ as Babai's Nearest Plane Algorithm](https://arxiv.org/abs/2507.18553)
*Jiale Chen,Torsten Hoefler,Dan Alistarh*

Main category: cs.LG

TL;DR: GPTQ与Babai算法等价，获得了几何解释和误差保证，为未来量化算法设计奠定了理论基础。


<details>
  <summary>Details</summary>
Motivation: 将大型语言模型（LLM）的权重从16位量化到更低的位宽，是将大规模Transformer部署到更经济的加速器上的事实方法。GPTQ是LLM规模上单次训练后量化的标准方法之一，但其内部运作被描述为一系列临时的代数更新，掩盖了任何几何意义或最坏情况的保证。

Method: 通过数学论证，证明GPTQ在特定条件下与Babai最近平面算法在格上的最近向量问题（CVP）上数学等价。

Result: GPTQ的误差传播步骤获得了直观的几何解释；GPTQ在无裁剪条件下继承了Babai算法的误差上限。

Conclusion: GPTQ在满足无裁剪条件时，继承了Babai算法的误差上限，为未来数十亿参数模型的量化算法设计开启了大门。
...
### [196] [Neural Tangent Kernels and Fisher Information Matrices for Simple ReLU Networks with Random Hidden Weights](https://arxiv.org/abs/2507.18555)
*Jun'ichi Takeuchia,Yoshinari Takeishia,Noboru Muratab,Kazushi Mimurac,Ka Long Keith Hod,Hiroshi Nagaoka*

Main category: cs.LG

TL;DR: 研究了双层 ReLU 网络的 Fisher 信息矩阵和 NTK，揭示了它们之间的关系，并获得了 NTK 的谱分解和函数近似公式。


<details>
  <summary>Details</summary>
Motivation: 研究双层 ReLU 网络的 Fisher 信息矩阵和神经正切核 (NTK)。

Method: 对具有随机隐藏权重的双层 ReLU 网络进行了理论分析。

Result: 获得了 NTK 的谱分解，具有本征函数的具体形式和主要特征值，并获得了双层神经网络表示函数的近似公式。

Conclusion: 推导了双层 ReLU 网络 Fisher 信息矩阵和神经正切核 (NTK) 之间的关系，发现它们之间存在线性变换。
...
### [197] [Clo-HDnn: A 4.66 TFLOPS/W and 3.78 TOPS/W Continual On-Device Learning Accelerator with Energy-efficient Hyperdimensional Computing via Progressive Search](https://arxiv.org/abs/2507.17953)
