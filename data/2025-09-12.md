...
Motivation: 在受限情况下，检索增强生成（RAG）和微调等提升LLM输出质量的常用方法受到限制。

Method: 研究了三个方面：生成、质量保证和LLM响应解析。

Result: 发现在这种受限环境中，额外信息可以提高生成质量，LLM可以有效地过滤低质量的三元组，并且LLM响应解析的灵活性和一致性之间的权衡取决于具体设置。

Conclusion: 在受限环境下，额外信息、LLM过滤和灵活性与一致性的权衡都会影响三元组补全任务的性能。
...
<details>
  <summary>Details</summary>
Motivation: Automating key elements of the analytical workflow is important because a significant portion of the assessment remains manual, making it time- and labor-intensive and susceptible to human error.

Method: They leverage Retrieval-Augmented Generation to automate the extraction of relevant evidence from large-scale textual data. They use a combination of layout-aware parsing, the Nomic embedding model, and few-shot prompting strategies.
...
<details>
  <summary>Details</summary>
Motivation: 利用上下文嵌入创建上下文分数，将文本数据转化为适合心理测量分析的响应数据。假设某些关键词的上下文含义在文档中差异显著，可以有效区分语料库中的文档。

Method: 该方法包括两个阶段：获取上下文分数和进行心理测量分析。第一阶段利用自然语言处理技术和基于编码器的 Transformer 模型识别常用关键词并生成上下文分数；第二阶段采用多种类型的因子分析，包括探索性和双因子模型，以提取和定义潜在因子，确定因子相关性，并识别与每个因子相关的最重要词语。

Result: 在 Wiki STEM 语料库上的实验结果表明，该方法有潜力揭示文本数据中的潜在知识维度和模式。

Conclusion: 该方法不仅可以增强文本数据的心理测量分析，而且在教育、心理学和法律等文本信息丰富的领域具有应用前景。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型 (LLM) 的性能会因其应用的语言和文化背景而异。这种差异表明需要成熟的评估框架，以评估它们在特定区域环境中的能力。

Method: 我们引入了 BRoverbs，这是一个专门用于通过巴西谚语评估 LLM 性能的数据集。谚语是一种丰富的语言资源，包含文化智慧、比喻表达和复杂的句法结构，这些都会对模型理解区域表达方式提出挑战。

Result: BRoverbs 旨在为葡萄牙语 LLM 提供一种新的评估工具，有助于推进区域信息基准测试。

Conclusion: BRoverbs 旨在为葡萄牙语 LLM 提供一种新的评估工具，有助于推进区域信息基准测试。
...
<details>
  <summary>Details</summary>
Motivation: 研究视觉语言模型在视觉方程求解任务中的局限性，即处理嵌入在图像中的数学方程式的能力。

Method: 将任务分解为系数计数和变量识别，分析了VLM在每个步骤中的表现。

Result: 发现系数计数是主要瓶颈，即使识别准确，并且识别和推理的结合会引入额外的错误。随着方程复杂度的增加，符号推理本身也成为限制因素。

Conclusion: 揭示了当前VLM的关键弱点，并为未来改进视觉基础数学推理指明了方向。
...
<details>
  <summary>Details</summary>
Motivation: 研究SPICE这种信号对用户语气辨别的能力。

Method: 通过3种语气（友好、不明确、辱骂）和10次互动的刺激集，测试了四个开放权重聊天模型在四种框架条件下的表现，共进行了480次试验。

Result: SPICE能够清晰地区分用户语气。友好的互动几乎一致倾向于继续（97.5% YES），而辱骂的互动则强烈倾向于停止（17.9% YES），不明确的互动介于两者之间（60.4% YES）。

Conclusion: SPICE是一种稳健、低开销和可重复的工具，用于审计模型状态，通过提供模型状态的直接关系信号来补充现有指标。
...
<details>
  <summary>Details</summary>
Motivation: 探讨如何通过对齐技术提升语言模型的安全性和有效性。

Method: 使用Anthropic Helpful-Harmless RLHF数据集训练和评估四个模型：基础OPT350M模型、SFT模型、DPO模型以及SFT+DPO组合模型。引入三个关键评估指标：无害率（HmR）、有效率（HpR）和综合对齐分数（CAS）。

Result: SFT优于DPO，但SFT+DPO组合模型在所有指标上均优于其他模型。

Conclusion: 研究结果展示了SFT和DPO技术的互补性，并强调了噪声数据、有限的GPU资源和训练约束带来的挑战。
...
<details>
  <summary>Details</summary>
Motivation: 现有的大型语言模型在通用信息抽取，特别是处理涉及复杂模式描述和需要多步骤推理的结构化输出场景时，性能不足。

Method: 该研究提出将强化学习（RL）与多角度推理相结合，使LLM从被动抽取器转变为主动推理器。

Result: 在多个信息抽取基准测试中，MR-UIE持续提高跨领域的抽取精度，并在多个数据集上超越了最先进的方法。

Conclusion: 将多角度推理融入强化学习显著提高了复杂信息抽取任务的泛化能力，突出了推理在具有挑战性的场景中的关键作用。
...
<details>
  <summary>Details</summary>
Motivation: 孟加拉语是第五大语言，但在大型语言模型（LLM）中代表性不足，尤其是在代码生成方面。这主要是由于缺乏高质量的数据来预训练和/或微调此类模型。

Method: 本文构建了孟加拉语代码指令数据集，并提出了MBPP-Bangla评估基准。

Result: TigerCoder系列代码LLM在Pass@1上的性能比现有的多语言和通用孟加拉语LLM提高了约11-18%。

Conclusion: 研究表明，精心策划的高质量数据集可以克服小型模型在低资源语言方面的局限性。
...
<details>
  <summary>Details</summary>
Motivation: Large language models often underperform in specialized tasks like e-commerce due to noisy, heterogeneous, multilingual, and dynamic data.

Method: A vertical-domain Mixture-of-Experts model (Compass-v3) with 245B parameters, hardware-efficient optimizations, trained on 12T tokens, and Optimal-Transport Direct Preference Optimization (OTPO).

Result: Compass-v3 achieves state-of-the-art e-commerce performance, surpassing DeepSeek-V3.1, GPT-4 series, and Qwen3-235B, with strong multilingual capability.

Conclusion: Compass-v3 demonstrates dual strengths in specialized commerce expertise and broad linguistic competence, widely applied in Shopee's e-commerce platform, replacing over 70% of OpenAI's traffic.
...
<details>
  <summary>Details</summary>
Motivation: 旨在减少人工标注对话行为的时间和精力，提高教育对话分析的效率。

Method: 使用开源CIMA语料库，利用GPT-3.5-turbo和GPT-4模型，通过定制提示进行测试。

Result: GPT-4达到了80%的准确率，加权F1值为0.81，Cohen's Kappa值为0.74，超过了基线性能，与人工标注具有高度一致性。

Conclusion: 生成式AI在DA分类方面具有强大的潜力，但需要关注任务特定的标签定义、上下文信息以及伦理考量。
...
<details>
  <summary>Details</summary>
Motivation: Lack of competitive rerankers for Vietnamese, a low-resource language with complex syntax.

Method: Trained on a curated 8 GB corpus and fine-tuned with hybrid hard-negative sampling.

Result: Achieves strong early-rank accuracy on the MMARCO-VI benchmark, surpassing multilingual baselines.

Conclusion: Demonstrates how adaptation and data curation can advance reranking in underrepresented languages and is released on Hugging Face.
...
<details>
  <summary>Details</summary>
Motivation: The paper addresses the need for a flexible and standardized tool for building and comparing neural encoding models, reducing the need to reinvent core infrastructure.

Method: The library implements a modular pipeline covering various methodological design choices, including brain datasets, stimulus features, and downsampling approaches. It also provides built-in logging, plotting, and integration with experiment tracking platforms.

Result: The framework's scalability and versatility are demonstrated by fitting encoding models to three story listening datasets. The study also highlights the importance of methodological choices like accounting for all tokens in a TR scan, incorporating hemodynamic lag effects, minimizing information leakage in train-test splits, and accounting for head motion effects.

Conclusion: LITcoder lowers technical barriers, facilitates systematic comparisons, fosters methodological rigor, and accelerates the development of high-performance predictive models of brain activity.
...
<details>
  <summary>Details</summary>
Motivation: 现有方法过度依赖文本内容，未能考虑数据集偏差，导致文本特征和输出标签之间的虚假相关性，从而降低分类精度。

Method: 引入了一种反事实数据增强策略，该策略最小程度地改变情感相关的因果特征，生成细节匹配的图像-文本样本，以引导模型关注与情感相关的内容。此外，引入了一种自适应去偏对比学习机制，以从反事实数据中学习鲁棒特征并提示模型决策，从而有效减轻偏差词的影响。

Result: 在多个基准数据集上的实验结果表明，该方法优于最新的基线方法。

Conclusion: 该论文提出了一种有效的反事实增强去偏框架，可以提高目标导向的多模态情感分类的准确性。
...
<details>
  <summary>Details</summary>
Motivation: Current SLLM training fails to bridge the acoustic-semantic gap, causing degradation in knowledge and reasoning.

Method: EchoX leverages semantic representations and dynamically generates speech training targets to integrate acoustic and semantic learning.

Result: EchoX achieves advanced performance on knowledge-based question-answering with 6k hours of training data.
...
<details>
  <summary>Details</summary>
Motivation: ASR模型在识别罕见词时表现不佳，Trie-based biasing方法虽然可以改善这种情况，但计算成本高昂，尤其是在大型解码器中。

Method: 提出一种新的ASR模型，使其能够向前看并一次预测多个步骤，从而避免了revocation步骤。

Result: 通过使用10小时的合成数据对Whisper模型进行微调，该方法将NSC Part 2测试集上的词错误率从30.86%降低到12.19%。

Conclusion: 该方法在提高罕见词识别的准确率和效率方面具有显著效果。
...
<details>
  <summary>Details</summary>
Motivation: 现有的上下文偏置方法在利用合成罕见词数据时容易过拟合，导致性能下降。

Method: 提出了一个关键词感知损失函数，包含一个用于偏置词预测的masked交叉熵项和一个用于检测偏置词位置的二元分类项，用于训练偏置模块。

Result: 通过将Whisper模型适应到10小时的合成数据，该方法在NSC Part 2测试集上将词错误率从29.71%降低到11.81%。

Conclusion: 该方法有效地提升了语音识别模型对罕见词的识别能力，并在实验中取得了显著的性能提升。
...
<details>
  <summary>Details</summary>
Motivation: 研究Marmoset猴子的声音交流，可以将其与大脑活动联系起来，这对于研究人类语音和语言来说非常难得。

Method: 设计了一种新的零样本评估指标，使用无监督的野外数据和弱标记的会话数据来评估GmSLM。

Result: GmSLM生成的发声在声学上与真实的重合成样本非常匹配，并且在下游任务中表现良好。尽管是完全无监督的，GmSLM可以有效地识别真实和人工对话。

Conclusion: GmSLM可以支持进一步研究声音交流的神经基础，并提供一个连接发声和大脑活动的实用框架。
...
Method: CCF集成了分段语义聚合与键值记忆编码，形成紧凑的表示，支持精确的重构和长距离理解。此外，还引入了一种训练高效的优化策略，将增量段解码与稀疏储层采样相结合，从而显著降低了内存开销，而不会降低性能。

Result: 在多个长上下文语言建模基准测试上的实验结果表明，CCF在高压缩率下实现了有竞争力的困惑度，并且与现有方法相比，显著提高了吞吐量和内存效率。

Conclusion: 结构化压缩在可扩展且有效的长上下文语言建模方面具有潜力。
...
<details>
  <summary>Details</summary>
Motivation: 准确评估简历中的候选人资历是一项关键但具有挑战性的任务，因为简历中普遍存在夸大经验和模糊自我表达的情况。

Method: 本文研究了大型语言模型（LLM），包括微调的 BERT 架构，用于自动化简历中的资历分类。为了严格评估模型性能，我们引入了一个混合数据集，该数据集包含真实世界的简历和旨在模拟夸大资历和低估资历的合成生成的硬样本。

Result: 我们的研究结果突出了增强人工智能驱动的候选人评估系统和减轻自我推销语言引入的偏差的有希望的方向。

Conclusion: 使用该数据集，我们评估了大型语言模型在检测与资历膨胀和隐性专业知识相关的微妙语言线索方面的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的表格结构、大小和数据类型各不相同，为表格问答带来了独特的挑战。SemEval 2025 Task 8 (DataBench) 引入了一个大规模、领域多样化的数据集，以评估模型准确回答结构化查询的能力。

Method: 该系统采用多阶段流水线，包括示例选择、SQL查询生成、答案提取、验证和迭代改进，利用GPT-4o、GPT-4o-mini和DeepSeek v2:16b等大型语言模型动态生成SQL查询。

Result: 实验结果表明，该方法在DataBench QA上实现了70.5%的准确率，在DataBench Lite QA上实现了71.6%的准确率，显著超过了基线得分（分别为26%和27%）。

Conclusion: 本研究详细介绍了该方法、实验结果和替代方案，提供了关于LLM驱动的Table QA的优势和局限性的见解。
...
<details>
  <summary>Details</summary>
Motivation: 现有专利 SDG 分类方法缺乏大规模标注数据，且可扩展性和泛化性不足。

Method: 本文构建了一个复合标签函数，利用大型语言模型从专利和 SDG 论文中提取结构化概念，并使用基于排序的检索方法计算跨领域相似度。通过自定义的正向损失校准标签函数，生成银标准、软多标签数据集。

Result: 本文方法在内部验证和外部验证中均优于现有方法，表明其可以提高 SDG 分类的规模。

Conclusion: 弱监督和语义对齐可以有效提升大规模 SDG 分类效果。
...
<details>
  <summary>Details</summary>
Motivation: Existing detection approaches primarily target standalone LLMs and do not address the unique challenges of Retrieval-Augmented Generation (RAG) systems, where responses must be consistent with retrieved evidence.
...
Conclusion: MetaRAG enables trustworthy deployment of RAG-based conversational agents.
...
<details>
  <summary>Details</summary>
Motivation: 强调当前自然语言处理研究通常不从认知角度看待类比推理过程，而过度依赖实体层面的相似性。

Method: 总结认知科学中关于类比推理过程的关键理论，并将其与自然语言处理中的概念联系。

Result: 展示了这些概念与自然语言处理研究中的几个主要挑战相关，这些挑战并非直接与类比解决相关。

Conclusion: 建议研究人员更好地优化文本中的关系理解，而不是过度依赖实体层面的相似性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的图线性化方法标签空间大，本文旨在减少标签空间，同时保留结构信息。

Method: 该方法将图编码为序列，通过 n 个标记动作实现线性时间解析，并表示重入、循环和空节点。

Result: 在多语言和多形式主义的基准测试中，该方法取得了有竞争力的结果，并在精确匹配准确率方面优于其他方法。

Conclusion: 分层括号编码可以在依赖图解析中实现高效且准确的解析，并减少标签空间。
...
<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型置信度评估方法计算开销大或校准效果差，不适用于实际部署。

Method: 模型通过末尾隐藏状态与词汇表中特殊token的嵌入之间的相似性来表达置信度，并微调模型以校准置信度。

Result: GrACE在判别能力和校准方面优于其他六种方法，且无需额外采样或辅助模型。GrACE还提高了决策的准确性，并显著减少了测试时缩放方案中所需的样本数量。

Conclusion: GrACE是一种可扩展、可靠且实时的置信度估计的实用解决方案，适用于部署大语言模型。
...
<details>
  <summary>Details</summary>
Motivation: 为非捷克语学生提供多语种学习材料，满足他们的教育需求。

Method: 开发并评估一个直接的捷克-乌克兰机器翻译系统，该系统针对教育领域定制，特别关注处理格式化内容（如XML和PDF）以及处理技术和科学术语。

Result: 对捷克教师进行了初步调查，了解非捷克语学生的需求，并介绍了系统在网络门户上的评估和实施情况。

Conclusion: 所有生成的应用程序均可供学生、教育工作者和研究人员免费使用。
...
<details>
  <summary>Details</summary>
Motivation: 在简历推荐系统中，职位匹配是一个关键挑战，因为重叠术语通常有限或具有误导性。因此，需要研究语义文本相关性（STR），它超越了表面词汇相似性，捕捉了文本之间细微的联系。

Method: 本文提出了一种自监督混合架构，结合了密集句子嵌入和领域知识图谱（KG），并通过图神经网络进行KG集成。此外，采用分层评估方法，将STR分数划分为低、中、高三个区域，以进行细粒度的模型性能分析。

Result: 实验结果表明，经过微调的SBERT模型与KG结合，在高STR区域产生了持续的改进，RMSE降低了25%。

Conclusion: 结合KG与文本嵌入可以有效提高模型性能，并且区域性能分析对于理解模型行为至关重要。这种细粒度的方法揭示了全局指标隐藏的优势和劣势，并支持在人力资源（HR）系统中进行更有针对性的模型选择。
...
<details>
  <summary>Details</summary>
Motivation: 探索在 Learning with Disagreements 共享任务中，如何有效预测注释者特定的注释。

Method: 比较了 ICL 中示例抽样策略，并评估了 RoBERTa 的几种微调方法用于 LDL。

Result: ICL 可以有效预测注释者特定的注释，并且将这些预测聚合为软标签可以产生有竞争力的性能。LDL 方法对于软标签预测很有希望。

Conclusion: ICL 方法有效，LDL 方法值得进一步探索。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型(llm)迅速改变了金融自然语言处理(NLP)的格局，实现了新的任务，并推动了数据集的激增和数据来源的多样化，但这种转变已经超过了传统的调查。

Method: 定义金融NLP研究的本体，并将基于llm的提取管道应用于681篇论文(2022-2025)，从而实现大规模的、数据驱动的分析。

Result: MetaGraph揭示了三个关键阶段：早期LLM采用和任务/数据集创新;对LLM局限性的批判性反思;以及外围技术日益集成到模块化系统中。

Conclusion: 结构化的观点，使从业者和研究人员清楚地了解金融NLP是如何发展的，同时展示了一种可重用的方法，用于绘制其他领域的科学进展。
...
<details>
  <summary>Details</summary>
Motivation: 在线课程环境对社交群体的有机形成构成障碍。为了探索这个问题，我们提出...

Method: 利用GPT的零样本能力从论坛介绍帖子中推断出五大人格特质。

Result: 我们将其性能与已建立的模型进行基准测试，证明了其在此任务中的有效性。我们还将此模型集成到SAMI的基于实体的匹配系统中，从而实现知情的人格化社交推荐。初步整合表明，人格特质可以补充现有的匹配因素

Conclusion: 人格特质可以补充现有的匹配因素，虽然还需要额外的评估来确定其对学生参与度和匹配质量的全部影响。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要集中于将情感分类为预定义的有限类别，忽略了更细微的表达。为了解决这个差距，论文引入了EXPRESS基准数据集。

Method: 该研究提出了一个综合评估框架，该框架检查预测的情感术语，并使用已建立的情感理论将它们分解为八种基本情感，从而实现细粒度的比较。

Result: 对流行LLM的系统测试表明，准确预测与人类自我表露情感对齐的情感仍然具有挑战性。定性分析表明，虽然某些LLM生成的情感术语与已建立的情感理论和定义一致，但它们有时无法像人类自我表露那样有效地捕捉上下文线索。

Conclusion: 研究结果强调了LLM在细粒度情感对齐方面的局限性，并为旨在提高其上下文理解能力的未来研究提供了见解。
...
<details>
  <summary>Details</summary>
Motivation: 在医疗认证不可用的资源有限环境中，口头尸检（VA）是估计死因的关键工具。本研究旨在改进死因预测。

Method: 使用PHMRC数据集，评估GPT-5预测、LCVA基线、文本嵌入和元学习器集成等多种方法。

Result: GPT-5在成人、儿童和新生儿的平均测试地点准确率分别达到48.6%、50.5%和53.5%，优于传统的统计机器学习基线5-10%。

Conclusion: 简单的现成LLM辅助方法可以显著提高口头尸检的准确性，对低资源环境下的全球健康监测具有重要意义。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常独立地微调这些智能体，导致智能体之间存在能力差距，协调性差。为了解决这个问题。

Method: MOAT在两个关键阶段之间交替进行：（1）规划智能体对齐，优化规划智能体以生成更好地指导执行智能体目标的子目标序列；（2）执行智能体改进，使用智能体本身生成的多样化的子目标-动作对来微调执行智能体，以提高其泛化能力。

Result: MOAT优于最先进的基线，在包含的任务上平均提高了3.1%，在排除的任务上平均提高了4.4%。

Conclusion: 理论分析证明，MOAT确保了一个非递减和渐进收敛的训练过程。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型在许多计算任务中表现出色，但其内部工作原理仍不清楚。本文旨在探究模型如何在实践中进行计算。

Method: 通过提出的两种技术Context-Aware Mean Ablation (CAMA)和Attention-Based Peeking (ABP)，研究人员识别出一个All-for-One (AF1)子图。

Result: 实验表明，AF1子图对于模型在各种心算任务中取得高精度是充分且必要的，并且可以在不同的模型和输入风格之间转移。

Conclusion: 研究表明，有意义的计算发生在很深的层，并且仅在最后一个token处发生，该token接收来自少数特定中间层中其他token的信息。
...
TL;DR: SteerMoE框架通过控制行为相关的专家来引导MoE模型，无需重新训练或修改权重，从而控制模型的忠实度和安全性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型中的混合专家模型(MoE)将每个token通过一个专门的前馈网络(FFN)子集进行路由，这些子集被称为专家。作者提出了一种通过检测和控制行为相关的专家来引导MoE模型的框架SteerMoE。

Method: 该检测方法识别出在表现出对比行为的配对输入中具有不同激活模式的专家。通过在推理过程中选择性地（停）激活这些专家，来控制诸如忠实度和安全性等行为。

Result: 在11个基准测试和6个大型语言模型中，该steering方法将安全性提高了+20%，忠实度提高了+27%。在对抗性攻击模式下，单独使用时，安全性降低了-41%，当与现有的越狱方法结合使用时，安全性降低了-100%，绕过了所有的安全护栏，并暴露了隐藏在专家内部的对齐伪造的新维度。

Conclusion: SteerMoE框架通过控制MoE模型中行为相关的专家，实现了在不重新训练或修改权重的情况下，提升或降低模型安全性与忠实度的能力，但也揭示了MoE模型潜在的安全风险。
...
<details>
  <summary>Details</summary>
Motivation: 现有 RLVR 方法存在探索不足，导致过早收敛和熵崩溃。

Method: 提出了 Curiosity-Driven Exploration (CDE) 框架，利用 actor 的困惑度和 critic 的多头结构中的方差作为探索奖励。

Result: 在 AIME 基准测试中，该方法比使用 GRPO/PPO 的标准 RLVR 提高了约 3 个百分点。

Conclusion: 该研究揭示了 RLVR 中的校准崩溃机制，阐明了常见的 LLM 失败模式。
...
Method: 利用多层表示和一个具有 LSTM 启发的门控机制的循环 Transformer 架构，以动态地整合跨层和模态的信息。

Result: 在 M2KR 和 M-BEIR 基准测试中，ReT-2 在不同的检索配置中始终取得最先进的性能，同时提供更快的推理速度和更少的内存使用。

Conclusion: ReT-2 还能提高百科全书式 VQA 和 InfoSeek 数据集的下游性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有的深度学习模型在跨物种、视角和上下文进行泛化时表现不佳。人类可以识别相同的动作，即使存在很大的上下文和视角变化，例如物种之间的差异（蜘蛛与马的行走）、视角（以自我为中心与第三人称）和上下文（现实生活与电影）。

Method: 使用视觉扩散模型（VDM）生成的特征，并通过 Transformer 进行聚合。利用在扩散过程的早期时间步长上训练的模型来突出语义信息，而不是提取特征中的像素级细节。

Result: 该模型在所有三个泛化基准测试中都达到了新的state-of-the-art水平，使机器动作识别更接近于类人的鲁棒性。

Conclusion: 该模型在跨物种、视角和上下文的动作识别方面表现出色，更接近于人类的识别能力。
...
TL;DR: PromptGuard是一个用于防止大型语言模型生成有害信息的模块化提示框架，通过名为VulnGuard Prompt的混合技术，利用真实世界数据驱动的对比学习，创建特定人群的保护屏障。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在实际应用中存在生成有害、有偏见或误导性信息的风险，尤其对弱势群体。现有的安全方法未能从源头上主动预防有害输出。

Method: 提出PromptGuard框架，核心是VulnGuard Prompt技术，结合了小样本示例、伦理思维链推理和自适应角色提示，并采用多目标优化，从理论上减少25-30%的危害。

Result: 通过数学公式证明了PromptGuard的收敛性，使用信息论进行漏洞分析，并使用GitHub数据集进行理论验证，为系统的实证研究奠定了数学基础。

Conclusion: PromptGuard通过六个核心模块协同工作，创建一个智能专家系统，用于实时预防危害。
...
<details>
  <summary>Details</summary>
Motivation: Re-ID 方法对标签噪声高度敏感，导致性能显著下降。

Method: 将 Re-ID 重构为监督图像相似性任务，采用 Siamese 网络架构，并提出 Beta 混合相似度离群点检测 (Beta-SOD) 框架，使用双分量 Beta 分布混合模型对嵌入对之间的余弦相似度分布进行建模。

Result: 在 CUHK03、Market-1501 和 VeRi-776 数据集上，Beta-SOD 在行人 Re-ID 和车辆 Re-ID 任务中表现出优于现有方法的性能，并且在各种噪声水平下都表现出鲁棒性和广泛的适用性。

Conclusion: Beta-SOD 能够有效地处理 Re-ID 中的标签噪声，并在多个数据集上取得了优异的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有的分割和检测方法难以应用于低对比度、高噪声和复杂血管结构的ICA图像。

Method: 在编码器中嵌入曲率感知结构增强（CASE）模块，利用多尺度响应来突出血管结构，抑制背景干扰。在解码器中，引入渐进式高频感知（PHFP）模块，采用多层次小波分解来逐步细化高频细节，同时整合低频全局结构。

Result: SFD-Mamba2Net在八个分割指标上始终优于现有方法，并在狭窄检测中实现了最高的真正率和阳性预测值。

Conclusion: SFD-Mamba2Net可以有效提高ICA图像中冠状动脉分割和狭窄检测的准确性。
...
Method: 该框架包含一个分割流程和一个放射组学流程。分割流程利用 promptable 基础模型完成缺失标签，并提出了一种新颖的零样本 3D prompt 传播算法 SAMONAI。放射组学流程从每个肿瘤中提取特征，并使用 SurvAMINN（一种新颖的基于自动编码器的多实例神经网络）预测生存率。

Result: 在包含 227 名患者的机构数据集上进行的大量评估表明，该框架超越了现有的临床和基因组生物标志物，C-index 提高了 10% 以上。

Conclusion: 结果表明，集成自动化分割算法和基于放射组学的生存分析，有可能在 CRLM 中实现准确、注释高效且可解释的结果预测。
...
<details>
  <summary>Details</summary>
Motivation: 旨在发现不同文本到图像模型之间存在的视觉属性差异，并探究产生这些差异的提示概念。

Method: 提出了一种名为 CompCon 的进化搜索算法，用于发现一个模型的输出中比另一个模型更普遍的视觉属性，并揭示与这些视觉差异相关的提示概念。

Result: 创建了一个自动数据生成流程来生成 ID2 数据集，并将 CompCon 与多个 LLM 和 VLM 驱动的基线方法进行了比较。发现了一些差异性表示，例如 PixArt 将提到孤独的提示描绘成潮湿的街道，而 Stable Diffusion 3.5 将从事媒体行业的非裔美国人描绘出来。

Conclusion: CompCon 能够有效地发现不同文本到图像模型之间的视觉表示差异，并揭示了与这些差异相关的提示概念。
...
<details>
  <summary>Details</summary>
Motivation: 无人机系统（UAS）的使用在本十年内大幅增加。它们显著推进了遥感技术，能够根据各种遥感应用所需的空间、光谱、时间和辐射分辨率来部署和成像地形。无人机影像的主要优点之一是可以在多云条件下通过在云层下飞行无人机来获取图像。这项技术的局限性在于影像通常会被云影所污染。在水面上拍摄的图像还会受到太阳眩光的影响。这是从无人机图像估计水质参数的两个严重问题。

Method: 该研究提出了一种新的机器学习方法，首先识别和提取具有云影和太阳眩光的区域，并将这些区域与无遮挡的晴空区域和未受太阳眩光影响的区域分开。从像素级别的图像中提取数据，以训练基于U-Net的深度学习模型，并基于测试用例的各种评估指标确定模型训练的最佳设置。

Result: 利用这种评估，确定了一个高质量的图像校正模型，该模型用于恢复图像中的云影和太阳眩光区域。
...
<details>
  <summary>Details</summary>
Motivation: 视觉Transformer虽然在建模长距离依赖方面表现出色，但在小数据集上，由于缺乏局部性和平移等变性等关键归纳偏置，局部特征提取能力不足。

Method: 提出了一种新颖的特征融合架构CoSwin，它在每个注意力块中集成了一个可学习的局部特征增强模块，使模型能够同时捕获细粒度的空间细节和全局语义结构。

Result: 在CIFAR-10、CIFAR-100、MNIST、SVHN和Tiny ImageNet等多个图像分类基准测试中，CoSwin相对于最先进的卷积和基于Transformer的模型均表现出一致的性能提升。例如，在CIFAR-10上提升了2.17%，在CIFAR-100上提升了4.92%。

Conclusion: 局部-全局特征融合可以有效提高Transformer在小规模视觉任务中的泛化性和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的点云配准方法在处理特征匹配时，缺乏对局部和全局几何一致性的有效结合。

Method: 该方法首先使用局部图嵌入模块初始化得分矩阵，然后通过双边最近邻搜索进行优化，并通过全局几何一致性学习来预测点对点的匹配概率。

Result: 在多个数据集上，iMatcher显著提高了刚性配准性能，实现了最先进的内点率。

Conclusion: iMatcher在各种设置中都表现出强大的鲁棒性，为点云配准提供了一种有效的方法。
...
TL;DR: 提出了一种基于隐式神经表示 (INR) 的方法，用于预测连续的左心室 (LV) 位移，无需推理时优化，从而实现心肌运动和应变的自动量化。
...
Result: 在 452 个英国生物银行测试案例中进行评估，该方法实现了最佳的跟踪精度 (2.14 mm RMSE)，并且与三个深度学习基线相比，全局周向 (2.86%) 和径向 (6.42%) 应变的综合误差最低。此外，该方法比最准确的基线快约 380 倍。

Conclusion: 这些结果突出了基于 INR 的模型适用于大型 CMR 数据集中心肌应变的准确和可扩展分析。
...
TL;DR: E-MLNet: An enhanced mutual learning network with dynamic weighting for open-set domain adaptation, improving known/unknown class separation.


<details>
  <summary>Details</summary>
Motivation: Existing UniDA methods treat all classifiers equally, diluting the learning signal.

Method: Integrates a dynamic weighting strategy to Open-set Entropy Minimization (OEM), focusing adaptation on relevant class boundaries.

Result: Achieves highest average H-scores on VisDA and ImageCLEF, superior robustness, and outperforms MLNet in most adaptation tasks.

Conclusion: E-MLNet's focused adaptation strategy improves performance in open-set domain adaptation.
...
TL;DR: 提出了一个大型公开的乌尔都语图像描述数据集COCO-Urdu，以减少多模态研究中的语言偏差。


<details>
  <summary>Details</summary>
Motivation: 乌尔都语在多模态和视觉语言研究中严重不足，缺乏大规模高质量的数据集限制了乌尔都语系统的发展，并加剧了多语言视觉语言模型中对高资源语言的偏见。

Method: 从MS COCO派生出一个大型图像描述数据集，包含59,000张图像和319,000个乌尔都语描述，使用SeamlessM4T v2翻译，并通过混合多模态质量评估框架进行验证，该框架集成了COMET-Kiwi的翻译质量、基于CLIP的相似性视觉基础和BERTScore与反向翻译的语义一致性；使用开源大型语言模型迭代改进低分标题。

Result: 在BLEU、SacreBLEU和chrF上对COCO-Urdu进行了基准测试，报告了一致的强大结果。

Conclusion: 发布数据集和质量评估流程旨在减少多模态研究中的语言偏差，并为包容性视觉语言系统奠定基础。
...
TL;DR: VoxelFormer: A lightweight transformer for multi-subject fMRI visual decoding.


<details>
  <summary>Details</summary>
Motivation: Existing fMRI visual decoding methods rely on subject-specific training, which limits scalability.

Method: VoxelFormer uses a Token Merging Transformer (ToMer) for voxel compression and a query-driven Q-Former to align neural representations with the CLIP image embedding space.

Result: VoxelFormer achieves competitive retrieval performance on the 7T Natural Scenes Dataset with fewer parameters than existing methods.

Conclusion: Token merging and query-based transformers are promising strategies for parameter-efficient neural decoding.
...
TL;DR: 本文提出了一种新的生成扩散框架，称为概率因果图模型（PCGM），用于生成高质量的结构性脑部MRI图像，该模型能够保留细微的解剖学细节。


<details>
  <summary>Details</summary>
Motivation: 现有的反事实模型难以生成解剖学上合理的MRI图像，因为缺乏明确的归纳偏置来保留细微的解剖学细节。这些模型旨在优化图像的整体外观，而不是保留受试者之间细微但医学相关的局部差异。

Method: 该方法通过概率图模块捕获解剖学约束，并将这些约束转换为细微变化发生的区域的空间二元掩码。掩码（由ControlNet的3D扩展编码）约束了一种新的反事实去噪UNet，其编码随后通过我们的3D扩散解码器转移到高质量的脑部MRI中。

Result: 在多个数据集上的大量实验表明，PCGM生成的结构性脑部MRI图像质量高于几种基线方法。此外，我们首次证明，从反事实（由PCGM生成）中提取的脑部测量结果复制了先前在神经科学文献中报道的疾病对皮质脑区域的微妙影响。

Conclusion: 这项成就标志着在研究细微形态差异的 исследования中，合成MRI的使用达到了一个重要的里程碑。
...
TL;DR: Med3DInsight: a novel pretraining framework that integrates 3D image encoders with 2D MLLMs via a specially designed plane-slice-aware transformer module.


<details>
  <summary>Details</summary>
Motivation: existing 3D medical convolution and transformer-based self-supervised learning (SSL) methods often lack deep semantic comprehension

Method: integrates 3D image encoders with 2D MLLMs via a specially designed plane-slice-aware transformer module and employs a partial optimal transport based alignment

Result: state-of-the-art performance on two downstream tasks, i.e., segmentation and classification, across various public datasets with CT and MRI modalities, outperforming current SSL methods

Conclusion: Med3DInsight introduces a new paradigm for scalable multimodal 3D medical representation learning without requiring human annotations and can be seamlessly integrated into existing 3D medical image understanding networks, potentially enhancing their performance.
...
TL;DR: 这篇论文提出了一种新的方法，通过结合环境中的固定物体信息和多任务学习方法，来提高人体行为识别的准确率，尤其是在人与物体交互的场景下。


<details>
  <summary>Details</summary>
Motivation: 现有的图卷积神经网络在人体行为识别领域表现出色，但在检测人与物体交互的行为时效果不佳，因为缺乏对场景信息的有效表示和合适的学习架构。

Method: 该方法利用环境中的固定物体信息，并采用多任务学习方法来提高人体行为识别的性能。

Result: 该方法在包含与固定物体交互和非交互行为的数据集上取得了99.25%的准确率，比仅使用人体骨骼姿势的基线模型提高了2.75%。

Conclusion: 结合交互区域信息的多任务学习方法能够成功识别所研究的交互和非交互行为。
...
TL;DR: 提出了一种基于跨模态特征对比和筛选策略的创新特征融合框架IRDFusion，用于多光谱目标检测。


<details>
  <summary>Details</summary>
Motivation: 现有的多光谱目标检测方法在特征融合过程中通常会保留额外的背景或噪声，限制了感知性能。

Method: 该方法包含两个新设计的模块：互特征细化模块（MFRM）和差分特征反馈模块（DFFM）。MFRM通过建模模内和模间关系来增强特征表示，DFFM计算模间差分特征作为指导信号反馈给MFRM，从而自适应地融合互补信息，同时抑制共模噪声。MFRM和DFFM集成到一个统一的框架中，称为IRDFusion。
...
TL;DR: SQAP-VLA：首个同时实现量化和token剪枝的VLA模型推理加速框架，无需训练。


<details>
  <summary>Details</summary>
Motivation: VLA模型计算和内存成本高，现有压缩方法无法同时实现量化和token剪枝以实现整体效率提升。

Method: 提出了一种新的量化感知token剪枝标准，该标准适用于经过积极量化的模型，同时改进量化器设计以提高剪枝效果。

Result: SQAP-VLA在计算效率和推理速度方面取得了显著提升，与原始模型相比，速度提高了1.93倍，平均成功率提高了4.5%。

Conclusion: SQAP-VLA 能够在保持模型性能的同时，显著提高 VLA 模型的计算效率和推理速度。
...
TL;DR: 提出了一种新的自监督框架S-BEVLoc，用于LiDAR全局定位，无需ground-truth位姿。


<details>
  <summary>Details</summary>
Motivation: 当前基于LiDAR的全局定位方法依赖于GPS或SLAM里程计提供的ground-truth位姿来监督网络训练，获取高精度ground-truth位姿需要大量成本和精力。

Method: 利用关键点中心BEV图像块之间的已知地理距离，从单个BEV图像构建训练三元组。使用CNN提取局部特征，并采用NetVLAD聚合全局描述符。引入SoftCos损失来增强从生成的三元组中学习。

Result: 在大型KITTI和NCLT数据集上的实验结果表明，S-BEVLoc在地点识别、闭环和全局定位任务中实现了最先进的性能，同时提供了可扩展性。

Conclusion: S-BEVLoc是一种有效的自监督LiDAR全局定位方法，无需ground-truth位姿，并具有良好的可扩展性。
...
TL;DR: 提出了一个名为FPI-Det的数据集，用于检测人们是否在使用手机。


<details>
  <summary>Details</summary>
Motivation: 现有的通用基准不能完全捕捉到细粒度的人与设备交互，为了解决这个问题。

Method: 构建了一个包含22,879张图像的数据集，其中包含在工作场所、教育、交通和公共场所的面部和手机的同步注释。

Result: 评估了代表性的YOLO和DETR检测器，提供了基线结果以及对不同对象大小、遮挡水平和环境下的性能分析。

Conclusion: FPI-Det数据集可以用于训练和评估模型，以检测人们是否在使用手机。
...
TL;DR: ZeroPlantSeg: A zero-shot method for segmenting rosette-shaped plant individuals from top-view images.
...
Method: Integrates a foundation segmentation model for leaf instance extraction and a vision-language model for reasoning about plant structures.

Result: Surpasses existing zero-shot methods and achieves better cross-domain performance than supervised methods.
...
TL;DR: 本文提出了一种新的框架GA-DMS，用于改进CLIP在行人重识别中的应用。


<details>
  <summary>Details</summary>
Motivation: CLIP在行人重识别中面临缺乏大规模标注数据和全局对比学习难以保持区分性局部特征的挑战。

Method: 1. 开发了一个抗噪声的数据构建流程，自动过滤和标注网络图像，生成大规模数据集WebPerson。2. 引入GA-DMS框架，通过梯度注意力机制自适应地屏蔽噪声文本tokens，并结合masked token预测目标，增强细粒度语义表示学习。

Result: GA-DMS在多个基准测试中取得了state-of-the-art的性能。

Conclusion: 本文通过数据和模型架构的协同改进，提升了CLIP在行人重识别中的性能。
...
TL;DR: ALL-PET: A low-resource PET foundation model using latent diffusion, Radon mask augmentation, dynamic multi-masking, geometric constraints, and transparent medical attention.


<details>
  <summary>Details</summary>
Motivation: Limited labeled data and computational resources hinder large-scale PET foundation models.
...
Conclusion: ALL-PET achieves efficient and generalizable PET imaging with minimal data and resources.
...
TL;DR: 本文研究了人工神经网络（ANN）在预测2D二值图像中Betti数时的噪声鲁棒性，并将其与基于立方体复形和有符号欧几里得距离变换（SEDT）的持久同调（PH）方法进行了比较。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索使用人工神经网络进行拓扑估计，作为结构噪声下持久同调的替代方法。

Method: 使用监督神经网络预测2D二值图像中的Betti数，并与基于立方体复形和SEDT的持久同调方法进行比较。使用了合成数据集和真实世界数据集。

Result: 研究结果表明，在噪声条件下，人工神经网络的性能优于持久同调方法，这可能是因为它们能够从训练数据中学习上下文和几何先验。

Conclusion: 结论是，尽管仍处于新兴阶段，但使用人工神经网络进行拓扑估计为结构噪声下提供了一种引人注目的持久同调替代方案。
...
TL;DR: 提出了一种新的3D场景评估指标，名为Objectness SIMilarity (OSIM)，它更符合人类视觉感知。


<details>
  <summary>Details</summary>
Motivation: 现有评估指标评估整体图像质量，与人类感知存在差异。受神经心理学启发，我们假设人类对3D场景的识别从根本上涉及对单个物体的关注。

Method: 利用物体检测模型及其特征表示来量化场景中每个物体的“objectness”，从而实现以物体为中心的评估。

Result: 用户研究表明，与现有指标相比，OSIM与人类感知更吻合。我们还使用各种方法分析了OSIM的特性。此外，我们在一个标准化的实验装置下重新评估了最近的3D重建和生成模型，以澄清该领域的进展。

Conclusion: OSIM是一种更符合人类视觉感知的3D场景评估指标，它可以更好地评估3D重建和生成模型的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究综述主要按任务或模型系列分类，忽略了数据集对架构演变的影响。本文旨在填补这一空白，从数据集角度分析架构演变。

Method: 本文从运动复杂性、时间跨度、分层组合和多模态丰富性四个方面分析了数据集如何施加归纳偏置，并解释了模型发展历程。

Result: 本文将里程碑式的模型，如 Two-Stream、3D CNN、Transformer 和多模态基础模型，解释为对数据集驱动压力的具体响应。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于深度学习的细胞检测模型难以复制病理学家在不同放大倍率下检查全切片图像的行为，并且缺乏具有多尺度重叠细胞和组织注释的数据集。

Method: 挑战赛数据集包括来自六个器官的重叠细胞检测和组织分割注释，包含来自306个癌症基因组图谱（TCGA）全切片图像的673对，用苏木精和曙红染色，分为训练、验证和测试子集。

Result: 参赛者提出的模型显着增强了对细胞-组织关系的理解。与未包含细胞-组织关系的基线仅细胞模型相比，顶级条目在测试集上的F1得分提高了7.99。

Conclusion: 在传统仅细胞检测方法的基础上，性能有了显着提高，证明了将多尺度语义整合到模型中的必要性。
...
TL;DR: RT-DETR++通过改进RT-DETR模型的编码器来提升无人机图像中的目标检测，尤其是在小目标和密集目标方面表现出色。


<details>
  <summary>Details</summary>
Motivation: 无人机图像中的目标检测面临着小目标密集、尺度变化和遮挡等挑战。

Method: 该论文提出了RT-DETR++，它在RT-DETR模型的基础上，引入了基于通道门控注意力的上下采样机制(AU/AD)来减少特征层传播过程中的误差并保留细节，并结合了CSP-PAC技术，利用并行空洞卷积在同一层内处理局部和上下文信息，从而促进多尺度特征的融合。

Result: RT-DETR++在检测小目标和密集目标方面表现出优越的性能，并且保持了实时检测的速度，而没有增加计算复杂度。

Conclusion: 该研究为实时检测系统中特征编码设计提供了一种有效的方法。
...
TL;DR: 提出了一种用于知识库视觉问答 (KB-VQA) 的免训练框架，通过增强知识相关性和减少冗余来减轻噪声的影响。


<details>
  <summary>Details</summary>
Motivation: 现有的方法通常直接用从知识源检索到的信息来增强模型，而忽略了大量的知识冗余，这给回答过程带来了噪声。

Method: 该框架首先从图像-问题对中得出必要的部分，创建低噪声查询，从而增强对高度相关知识的检索。然后，提示大型模型识别和提取知识中对答案有益的片段。此外，引入了一种选择性的知识整合策略，允许模型仅在对回答问题缺乏信心时才整合知识，从而减轻冗余信息的影响。

Result: 该框架能够获取准确和关键的知识，

Conclusion: 大量实验表明，该框架优于最先进的方法。
...
TL;DR: 本文提出了一种名为CWSSNet的分类框架，用于高光谱遥感图像的地物分类，该框架集成了3D光谱-空间特征和wavelet卷积。


<details>
  <summary>Details</summary>
Motivation: 高光谱图像虽然光谱信息丰富，但由于其波段众多、高维度和光谱混合特性，容易导致显著的特征冗余，为了解决这个问题。

Method: 该研究使用ZY1F卫星的高光谱图像作为数据源，选择江西省上饶市余干县作为研究区域，提出了一种名为CWSSNet的分类框架，该框架集成了多模态信息，并通过引入wavelet域中的多波段分解和卷积运算，突破了传统方法的分类性能瓶颈。

Result: CWSSNet在余干县的平均交并比（mIoU）、平均精度（mAcc）和平均F1分数（mF1）分别达到了74.50%、82.73%和84.94%。在水体、植被和裸地的分类中也获得了最高的交并比（IoU），表明了良好的鲁棒性。在训练集比例为70%时，训练时间的增加有限，且分类效果接近最佳水平，表明该模型在小样本训练条件下保持了可靠的性能。

Conclusion: CWSSNet框架在高光谱遥感图像地物分类中表现出良好的性能和鲁棒性，并且在小样本训练条件下也能保持可靠的性能。
...
TL;DR: 本文介绍了一个新的数据集RRDataset，用于评估AI生成图像检测模型在复杂现实条件下的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的AI生成图像检测方法在复杂现实条件下表现不佳，需要更全面的评估。

Method: 构建了RRDataset数据集，包含七个主要场景的高质量图像，并考虑了互联网传输和重数字化等因素。对17个检测器和10个视觉语言模型(VLMs)进行了基准测试，并进行了大规模的人工研究。

Result: 基准测试结果表明，当前的AI检测方法在现实条件下存在局限性。

Conclusion: 需要借鉴人类的适应能力来开发更强大的检测算法。
...
TL;DR: 本文提出了一种轻量级的自适应图像信号处理（ISP）插件Dark-ISP，用于直接处理黑暗环境下的Bayer RAW图像，实现无缝的端到端目标检测训练。


<details>
  <summary>Details</summary>
Motivation: 在低光照条件下，目标检测至关重要但充满挑战，因为图像质量会下降。虽然最近的研究表明，RAW图像比RGB图像具有更大的潜力，但现有方法要么使用有信息损失的RAW-RGB图像，要么采用复杂的框架。

Method: 将传统的ISP管线解构为连续的线性（传感器校准）和非线性（色调映射）子模块，并将它们重塑为通过任务驱动损失优化的可微组件。每个模块都具有内容感知的适应性和物理信息先验，从而能够自动进行与检测目标对齐的RAW到RGB转换。利用ISP管线的内在级联结构，设计了一种自增强机制，促进子模块之间的协作。

Result: 在三个RAW图像数据集上进行了大量实验，结果表明该方法优于最先进的基于RGB和RAW的检测方法，在具有挑战性的低光照环境中以最少的参数实现了卓越的结果。

Conclusion: 本文提出的Dark-ISP插件能够有效地处理低光照环境下的RAW图像，并在目标检测任务中取得了优异的性能。
...
TL;DR: VQualA 2025 挑战赛总结：评估大型多模态模型 (LMM) 在视觉质量比较方面的能力。


<details>
  <summary>Details</summary>
Motivation: 旨在评估和提高 LMM 在多个图像中执行关于视觉质量差异的开放式和详细推理的能力。

Method: 引入了一个包含数千个由粗到细粒度的视觉质量比较任务的新基准，涵盖单个图像、图像对和多图像组。每个任务都要求模型提供准确的质量判断。

Result: 大约 100 名参与者提交了作品，其中五个模型展示了指令调整的 LMM 在质量评估方面的新兴能力。

Conclusion: 该挑战标志着朝着开放域视觉质量推理和比较迈出的重要一步，并有助于未来对可解释的和以人为本的质量评估系统的研究。
...
TL;DR: MGTraj模型通过多粒度目标引导进行轨迹预测，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有研究表明，结合目标引导可以提高预测精度，但中间时间粒度的效用尚未充分开发。因此，需要多粒度轨迹建模。

Method: MGTraj递归编码从粗到细粒度级别的轨迹，使用基于Transformer的递归细化网络 (RRN) 捕获特征并预测渐进细化。采用跨不同粒度的特征集成和速度预测作为辅助任务。

Result: 在EHT/UCY和Stanford Drone Dataset上的实验结果表明，MGTraj优于基线方法，并在目标引导方法中实现了最先进的性能。

Conclusion: MGTraj模型通过多粒度目标引导，实现了更准确的人类轨迹预测。
...
TL;DR: Medverse是一个通用的医学图像分析ICL模型，无需重新训练即可执行各种图像处理任务。


<details>
  <summary>Details</summary>
Motivation: 当前的医学图像ICL模型无法同时实现高保真预测和全局解剖学理解，并且没有在各种医学成像任务（例如，分割和增强）和解剖区域上训练的统一模型。

Method: Medverse采用下一代自回归上下文学习框架，逐步细化从粗到细的预测，生成一致的、全分辨率的体积输出，并实现多尺度解剖学感知。我们进一步提出了一种分块交叉注意力模块，该模块有助于上下文输入和目标输入之间的远距离交互，同时通过空间稀疏性保持计算效率。

Result: Medverse在广泛的保留数据集集合上进行了广泛的评估，涵盖了以前未见过的临床中心、器官、物种和成像方式。结果表明，Medverse大大优于现有的ICL基线，并为上下文学习建立了一种新的范例。

Conclusion: Medverse是一种用于3D医学图像的通用ICL模型，它在各种任务中都优于现有的ICL基线，并为上下文学习建立了一种新的范例。
...
TL;DR: 提出了一种名为CoAtNeXt的新型混合模型，用于胃组织图像分类，该模型在两个公共数据集上优于其他模型。


<details>
  <summary>Details</summary>
Motivation: 早期诊断胃病至关重要，但组织病理学检查是手动完成的，效率低且容易出错。需要自动、可靠和高效的胃组织分析方法。

Method: 该模型基于CoAtNet架构，用增强的ConvNeXtV2块替换了MBConv层，并集成了卷积块注意力模块（CBAM）。

Result: 在HMU-GC-HE-30K上，CoAtNeXt达到了96.47%的准确率，在GasHisSDB上达到了98.29%的准确率，优于所有测试的CNN和ViT模型。

Conclusion: CoAtNeXt是一种用于胃组织图像组织病理学分类的强大架构，在二元和多类分类中均表现出色，有潜力通过提高诊断准确性和减少工作量来协助病理学家。
...
TL;DR: 本文介绍了一个新的大型多模态指令数据集和基准，专门用于全景X光片判读，名为MMOral。同时提出了一个名为OralGPT的模型，该模型在MMOral数据集上进行了微调，并在牙科领域取得了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有的大型视觉语言模型在通用医学任务中表现出色，但在牙科等专业领域的效果仍有待探索。现有的医学基准或指令数据集无法捕捉到全景X光片中的密集解剖结构和细微病理线索。

Method: 构建了一个包含20,563张带注释图像和130万条指令的大型多模态指令数据集MMOral，并提出了一个全面的评估套件MMOral-Bench。此外，提出了OralGPT，该模型在Qwen2.5-VL-7B上进行了监督微调。

Result: 在MMOral-Bench上评估了64个LVLM，发现即使是表现最佳的模型GPT-4o也仅达到41.45%的准确率。OralGPT通过单轮SFT，性能提升了24.73%。

Conclusion: MMOral和OralGPT作为智能牙科的关键基础，具有巨大的潜力，并能够在该领域实现更具临床意义的多模态人工智能系统。
...
TL;DR: 本文提出了一种名为动态绝对时间增强（DATE）的方法，通过时间戳注入机制（TIM）和语义引导的时间感知相似性采样（TASS）策略来增强 MLLM 中的时间感知能力。


<details>
  <summary>Details</summary>
Motivation: 现有的方法在处理需要精确的时间推理和事件定位的长视频理解任务时，由于长距离依赖问题，导致信息丢失和时间理解能力下降。

Method: 该方法通过将视频帧嵌入与文本时间戳标记交错来构建连续的时间参考系统，并将视频采样问题重新定义为视觉-语言检索任务，引入两阶段算法以确保语义相关性和时间覆盖。

Result: 该方法在绝对时间理解和关键事件定位方面取得了显著改进，在小时级视频基准测试中，7B 和 72B 模型均实现了最先进的性能。特别是，7B 模型在某些基准测试中甚至超过了许多 72B 模型。

Conclusion: 本文提出的 DATE 方法有效地提升了 MLLM 在长视频理解中的时间感知能力，并在多个基准测试中取得了优异的成绩。
...
TL;DR: 提出了一种名为PSP-Seg的渐进剪枝框架，以实现动态高效的3D分割。


<details>
  <summary>Details</summary>
Motivation: 现有的高效分割模型通常是静态的，并且是在训练前手动设计的，这限制了它们在不同任务中的适应性，并且难以平衡性能与资源效率。

Method: PSP-Seg从一个冗余模型开始，并通过块状剪枝和功能解耦损失的组合迭代地剪除冗余模块。

Result: 轻量级变体PSP-Seg-S在所有数据集上实现了与nnU-Net相当的性能，同时减少了42-45%的GPU内存使用量、29-48%的训练时间和83-87%的参数数量。

Conclusion: PSP-Seg有潜力成为一种经济高效且高性能的替代方案，可广泛应用于临床。
...
TL;DR: 本文提出了一种新的图表理解方法，该方法允许视觉语言模型 (VLM) 动态选择最佳推理路径，从而提高准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有的图表理解方法存在局限性，要么依赖外部工具而脆弱，要么微调特定模型而采用单一推理策略，且难以验证中间步骤。

Method: 本文提出了一种代码作为思维 (CaT) 的方法，以可验证的符号格式表示图表的视觉信息，并引入了视觉可编程性概念，使 VLM 能够自适应地选择 CaT 路径或直接视觉推理路径。使用强化学习训练模型的选择策略，并结合数据准确性奖励和决策奖励。

Result: 实验表明，该方法在各种图表理解基准测试中表现出强大而稳健的性能。

Conclusion: 该研究表明，可以教导 VLM 不仅进行推理，还可以动态选择每项任务的最佳推理路径。
...
### [80] [Can Multimodal LLMs See Materials Clearly? A Multimodal Benchmark on Materials Characterization](https://arxiv.org/abs/2509.09307)
*Zhengzhao Lai,Youbin Zheng,Zhenyang Cai,Haonan Lyu,Jinpu Yang,Hongqing Liang,Yan Hu,Benyou Wang*

Main category: cs.CV

TL;DR: 提出了一个材料表征图像理解的基准测试 (MatCha)，包含1500个需要专家级领域知识的问题。


<details>
  <summary>Details</summary>
Motivation: 探索多模态大型语言模型 (MLLMs) 在理解真实材料表征图像数据方面的能力，因为现有模型在材料科学中的生成和预测任务中表现出潜力，但在图像理解方面仍有不足。

Method: 构建了一个包含1500个问题的MatCha基准测试，涵盖材料研究的四个关键阶段和21个不同的任务，旨在反映材料科学家面临的真实挑战。

Result: 评估结果表明，现有 MLLMs 在 MatCha 上的表现与人类专家相比存在显著差距，尤其是在需要更高层次专业知识和复杂视觉感知的问题上表现下降。简单的少样本和思维链提示难以缓解这些限制。

Conclusion: 现有的 MLLMs 在真实材料表征场景中的适应性仍然有限。MatCha 的发布旨在促进新材料发现和自主科学智能体等领域的未来研究。
...
### [81] [Modality-Agnostic Input Channels Enable Segmentation of Brain lesions in Multimodal MRI with Sequences Unavailable During Training](https://arxiv.org/abs/2509.09290)
*Anthony P. Addison,Felix Wagner,Wentian Xu,Natalie Voets,Konstantinos Kamnitsas*

Main category: cs.CV

TL;DR: 开发了一种可以处理训练期间未见过的图像模态的模型，同时保留了处理先前见过的模态的能力。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态脑部MRI分割模型受限于固定的模态，无法有效处理新的模态。

Method: 通过在U-net架构中集成一个模态不可知的输入通道以及模态特定的输入通道来实现，并使用图像增强方案合成人工MRI模态来训练模态不可知的组件。

Result: 该方法在处理训练期间遇到的MRI模态的同时，能够处理新的、未见过的模态，从而改进分割效果。

Conclusion: 该方法能够有效处理MRI模态，包括训练期间未见过的模态，提高了分割效果。
...
TL;DR: 本文提出了一种新的、模型无关的开放集检测框架，专为基于嵌入的检测器设计。


<details>
  <summary>Details</summary>
Motivation: 传统的封闭集检测器在领域转移和飞行数据损坏的情况下性能会显著下降，从而对安全关键型应用构成风险。

Method: 该方法通过嵌入空间中的熵建模来估计语义不确定性，并结合谱归一化和温度缩放来增强开放集判别能力。

Result: 在具有挑战性的AOT空中基准测试和广泛的真实飞行测试中验证了该方法的有效性。与基于YOLO的标准检测器相比，该方法实现了高达10%的相对AUROC增益。
...
TL;DR: 提出了一种名为SlotSAR的新型面向对象学习 (OCL) 框架，用于从 SAR 图像中的背景杂波中解开目标表示，无需掩码注释。


<details>
  <summary>Details</summary>
Motivation: SAR 图像包含感兴趣的目标以及复杂背景杂波，包括地形反射和斑点噪声。在许多情况下，这种杂波表现出类似于目标的强度和模式，导致模型提取纠缠或虚假特征。这种行为削弱了形成清晰目标表示的能力，而不管分类器如何。

Method: SlotSAR 首先从 SARATR-X 中提取高级语义特征，并从 wavelet scattering network 中提取低级散射特征，以便获得互补的多级表示以实现稳健的目标表征。我们进一步提出了一个多级 slot 注意力模块，该模块集成了这些低级和高级特征，以增强 slot-wise 表示的独特性，从而实现有效的 OCL。

Result: 实验结果表明，与现有的 OCL 方法相比，SlotSAR 通过保留结构细节，在 SAR 图像中实现了最先进的性能。

Conclusion: SlotSAR 在 SAR 图像中实现了最先进的性能
...
### [84] [FLUX-Reason-6M & PRISM-Bench: A Million-Scale Text-to-Image Reasoning Dataset and Comprehensive Benchmark](https://arxiv.org/abs/2509.09680)
*Rongyao Fang,Aldrich Yu,Chengqi Duan,Linjiang Huang,Shuai Bai,Yuxuan Cai,Kun Wang,Si Liu,Xihui Liu,Hongsheng Li*

Main category: cs.CV

TL;DR: 本研究旨在弥补开源文本到图像 (T2I) 模型在复杂推理能力上的不足，通过构建大规模数据集 FLUX-Reason-6M 和综合评估基准 PRISM-Bench 来促进该领域的发展。


<details>
  <summary>Details</summary>
Motivation: 当前开源 T2I 模型缺乏大型推理数据集和全面评估标准，导致性能落后于闭源系统。

Method: 提出了 FLUX-Reason-6M 数据集（包含 600 万张图像和 2000 万条双语描述），并设计了 PRISM-Bench 基准，包含七个不同的评估方向，利用视觉-语言模型进行评估。

Result: 对 19 个领先模型在 PRISM-Bench 上进行了评估，揭示了现有模型的性能差距和需要改进的方面。

Conclusion: 发布了数据集、基准和评估代码，以促进下一代面向推理的 T2I 生成。
...
<details>
  <summary>Details</summary>
Motivation: 现有异构协同感知方法需要联合训练或预先存储模型，在实际应用中不实用。

Method: 提出了一种名为渐进式异构协同感知（PHCP）的新框架，该框架将问题转化为少样本无监督域适应问题，并在推理过程中通过自训练适配器动态对齐特征。

Result: 在OPV2V数据集上的大量实验表明，PHCP在各种异构场景中都取得了强大的性能。值得注意的是，PHCP在使用少量未标记数据的情况下，实现了与在整个数据集上训练的SOTA方法相当的性能。

Conclusion: PHCP 能够在无需联合训练的情况下，达到与 SOTA 方法相近的性能。
...
TL;DR: 这篇论文探索了视觉语言模型（VLM）在图像分类中的视觉推理能力，并对比了语言引导和纯视觉两种方法的性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型主要关注图像-文本对齐，而对其纯视觉推理能力的探索不足。

Method: 该研究对多种双编码器VLM进行了全面的评估，包括SigLIP 2和RADIOv2.5等模型。分析了影响准确率的关键因素，如提示设计、类别多样性、k-NN中的邻居数量和参考集大小。提出了一种基于类别精度的简单、无需学习的融合方法。

Result: 研究表明，语言和视觉提供互补的优势，一些类别更适合文本提示，而另一些类别则通过视觉相似性更好地处理。所提出的融合方法提高了分类性能。

Conclusion: 该研究揭示了视觉语言模型在图像分类中语言和视觉的互补性，并提出了一种有效的融合方法来提高分类性能。
...
TL;DR: 本文提出了一种名为“Better Understanding Generation (BUG)”的工作流程，利用大型多模态模型赋能服装设计，通过图像到提示的聊天方式自动创建和精细化定制服装设计。


<details>
  <summary>Details</summary>
Motivation: 当前的人工智能模型可以轻松地将头脑风暴转化为精美的设计，但如果没有最终用户的专业背景知识，细粒度的定制仍然会受到文本不确定性的影响。

Method: 我们提出了BUG工作流程，利用LMM从图像到提示的聊天自动创建和精细化定制服装设计。

Result: 我们提出了一个新的FashionEdit数据集，该数据集模拟了真实的服装设计工作流程，并从生成相似度、用户满意度和质量等方面进行了评估，以证明我们模型的有效性。

Conclusion: 我们的框架释放了用户超越文字的创造潜力，也降低了服装设计/编辑的门槛，无需进一步的人工干预。
...
TL;DR: 这篇论文研究了如何利用自监督预训练策略来提高基于少量样本学习(FSL)的自动手术技能评估(SSA)模型的性能。他们使用OSATS评分标注了一个公开的机器人手术数据集，并评估了各种预训练源在三种少量样本设置下的效果。


<details>
  <summary>Details</summary>
Motivation: 由于手术技能注释的稀缺性，开发稳健的SSA模型具有挑战性。虽然FSL提供了一种可扩展的替代方案，但其成功关键取决于有效的预训练。预训练在SSA中很大程度上未被探索。

Method: 将SSA构建为一个少样本任务，并研究自监督预训练策略如何影响下游少样本SSA性能。量化领域相似性，并分析领域差距和将特定于过程的数据纳入预训练如何影响可移植性。

Result: 结果表明，小而与领域相关的数据集可以优于大规模、不太一致的数据集，在1-shot、2-shot和5-shot设置中分别达到60.16%、66.03%和73.65%的准确率。将特定于过程的数据与领域相关的外部数据集结合到预训练中可以显著提高下游性能。

Conclusion: 领域相关的预训练数据集对少样本手术技能评估至关重要。
...
<details>
  <summary>Details</summary>
Motivation: 人为错误，尤其是分心和酒后驾驶，仍然是全球交通安全事故的重要原因。

Method: 该框架采用先进的计算机视觉方法，包括实时目标跟踪、横向位移分析和车道位置监控，使用 YOLO 对象检测模型和自定义车道估计算法。

Result: 在不同的视频数据集上的实验评估表明，该框架在不同的道路和环境条件下具有可靠性和适应性。

Conclusion: 该系统能够分析非联网车辆的行为，弥补了依赖车辆间通信的系统的不足。
...
<details>
  <summary>Details</summary>
Motivation: 本征图像分解问题已经研究了几十年，但在复杂场景（即空间变化的光照效果和丰富的纹理）中仍然是一个重大挑战。

Method: 我们提出了一种新的方法来处理本征图像分解中严重的光照和丰富的纹理，这能够为真实世界的图像生成高质量的本征图像。具体来说，我们观察到以前的基于学习的方法倾向于产生无纹理和过度平滑的本征图像，这可以用于推断给定RGB图像的光照和纹理信息。这样，我们设计了一个纹理引导的正则化项，并将分解问题转化为一个优化框架，以分离材料纹理和光照效果。

Result: 我们证明了结合新的纹理感知先验可以产生优于现有方法的结果。

Conclusion: 结合新的纹理感知先验可以产生优于现有方法的结果。
...
TL;DR: 本文探讨了即插即用 (PnP) 方法和去噪扩散隐式模型 (DDIM) 之间的联系，重点关注单像素成像，并提出了一种混合数据一致性模块，该模块将多个 PnP 风格的保真度项线性组合，从而提高测量一致性，同时不干扰扩散采样轨迹，从而实现更好的重建质量。


<details>
  <summary>Details</summary>
Motivation: 将学习先验与物理前向模型以原则性方式集成

Method: 通过将扩散过程解耦为三个可解释的阶段：去噪、数据一致性实施和采样，我们提供了一个统一的框架

Result: 在单像素成像任务上的实验结果表明，我们的方法实现了更好的重建质量

Conclusion: 提出了一种混合数据一致性模块，该模块将多个 PnP 风格的保真度项线性组合。这种混合校正直接应用于去噪估计，提高测量一致性，同时不干扰扩散采样轨迹。
...
TL;DR: 提出了一种全自动的两阶段框架，用于颅内压(ICP)分级，该框架集成了关键帧识别、ONSD测量和临床数据。


<details>
  <summary>Details</summary>
Motivation: 颅内压(ICP)升高对大脑功能构成严重威胁，因此需要进行监测以便及时干预。腰椎穿刺是ICP测量的金标准，但其侵入性和相关风险促使人们需要无创替代方案。视神经鞘直径(ONSD)已成为一种有希望的生物标志物，因为ICP升高与ONSD增加直接相关。然而，目前ONSD测量的临床实践受到手动操作的不一致性、最佳视图选择的主观性和阈值设定的可变性的影响，限制了它们的可靠性。

Method: 该方法包括眼底超声视频处理阶段和颅内压分级阶段。眼底超声视频处理阶段执行帧级解剖分割、由国际共识声明指导的基于规则的关键帧识别和精确的ONSD测量。然后，颅内压分级阶段将ONSD指标与临床特征融合，以实现ICP等级的预测。

Result: 实验结果表明，该方法验证准确率为$0.845 




\pm 0.071$(具有来自五重交叉验证的标准偏差)，独立测试准确率为0.786，显着优于传统的基于阈值的方法(验证准确率为$0.637 
...
Conclusion: 该框架通过有效减少操作者变异性和整合多源信息，为临床ICP评估建立了一种可靠的无创方法，有望改善急性神经系统疾病患者的管理。
...
TL;DR: 提出了一种无需外部正常支持的无监督 IC 缺陷分割框架。


<details>
  <summary>Details</summary>
Motivation: 现代集成电路 (IC) 制造引入了各种细粒度的缺陷，这些缺陷会降低良率和可靠性。大多数工业缺陷分割会将测试图像与外部正常集进行比较，但这种策略对于 IC 图像来说是脆弱的，因为 IC 图像的布局因产品而异，并且难以进行精确对齐。

Method: 该方法从测试图像中聚合有代表性的正常特征，并使用相干性损失来加强它们与正常区域的关联。在这些特征的指导下，解码器仅重建正常内容；然后，重建残差分割缺陷。伪异常增强进一步稳定了训练。

Result: 在来自三个 IC 工艺阶段的数据集上的实验表明，与现有方法相比，该方法具有持续的改进，并且对产品可变性具有很强的鲁棒性。

Conclusion: 该无监督 IC 缺陷分割框架无需外部正常支持，且在 IC 缺陷分割任务中表现出良好的性能。
...
TL;DR: 这篇论文提出了一种名为DRiFt的结构化特征解耦框架，旨在提高医学视觉-语言模型(VLM)在分布偏移下的可靠性。


<details>
  <summary>Details</summary>
Motivation: 医学视觉-语言模型在临床决策支持方面有前景，但其在分布偏移下的可靠性是一个主要问题。这些模型通常学习与任务无关的关联，限制了它们的泛化能力，并增加了在实际环境中失败的风险。

Method: 该方法使用参数高效调整(LoRA)和可学习的prompt tokens，显式地将临床相关信号与任务无关的噪声分离。为了增强跨模态对齐和减少不确定性，该方法通过为不同的医学数据集生成标题来策划高质量的、临床上有根据的图像-文本对。

Result: 该方法在分布内性能上比之前的基于prompt的方法提高了+11.4%的Top-1准确率和+3.3%的Macro-F1，同时在未见过的数据集上保持了强大的鲁棒性。

Conclusion: 解开与任务相关的特征和仔细对齐可以显著提高模型泛化能力，并减少领域转移下不可预测的行为。这些见解有助于为临床使用构建更安全、更值得信赖的VLM。
...
<details>
  <summary>Details</summary>
Motivation: Current fusion techniques perform suboptimally in real-world applications due to corrupted target and background structures with low resolution and weak semantic information.

Method: Proposes FS-Diff, using semantic guidance from a clarity sensing mechanism for adaptive low-resolution perception and cross-modal feature extraction. It uses bidirectional feature Mamba and a modified U-Net network for iterative denoising.
...
Conclusion: FS-Diff effectively addresses the challenges of joint image fusion and super-resolution in complex real-world scenarios.
...
TL;DR: 本文提出了一种用于密集自监督学习 (SSL) 的显式语义集中方法，以解决图像级 SSL 中存在的过分散问题。


<details>
  <summary>Details</summary>
Motivation: 现有的图像级自监督学习方法在学习密集patches表示时，存在过分散现象，影响下游任务的性能。本文旨在通过显式语义集中来解决这个问题。

Method: 本文提出了两种方法：1) 知识蒸馏patch对应关系，并使用噪声容忍排序损失来打破严格的空间对齐；2) 使用对象感知过滤器将输出空间映射到基于对象的空间，通过交叉注意力用可学习的对象原型来表示patches。

Result: 在各种任务上的实验结果表明，本文提出的方法是有效的。

Conclusion: 本文提出了一种有效的密集自监督学习方法，通过显式语义集中来解决过分散问题，并在各种任务上取得了良好的性能。
...
TL;DR: 提出了一种名为FlexiD-Fuse的基于扩散的图像融合网络，可以处理任意数量的输入模态医学图像融合。


<details>
  <summary>Details</summary>
Motivation: 现有的融合方法只能处理固定数量的模态输入，无法直接处理变化的输入量。

Method: 将扩散融合问题转化为基于扩散过程和分层贝叶斯建模的最大似然估计问题，并将期望最大化算法融入扩散采样迭代过程中。

Result: 在Harvard数据集上，FlexiD-Fuse在医学图像融合方面取得了最佳性能，并在红外-可见光、多重曝光和多焦点图像融合任务中表现出有效性和优越性。

Conclusion: FlexiD-Fuse 能够处理不同模态数量的医学图像融合，并在各种图像融合任务中表现出色。
...
TL;DR: 本研究针对撒哈拉以南非洲（SSA）地区脑胶质瘤MRI图像分割数据匮乏的问题，提出了一种鲁棒且计算高效的深度学习框架。


<details>
  <summary>Details</summary>
Motivation: 在撒哈拉以南非洲（SSA）地区，高质量的带注释的影像数据稀缺，这给在临床工作流程中部署先进的分割模型带来了巨大的挑战。

Method: 利用3D Attention UNet架构，通过残差块进行增强，并通过在BraTS 2021数据集上预训练的权重进行迁移学习。

Result: 在来自BraTS-Africa数据集的95个MRI病例上进行了评估，结果显示，增强肿瘤（ET）的Dice系数为0.76，坏死和非增强肿瘤核心（NETC）的Dice系数为0.80，周围非功能半球（SNFH）的Dice系数为0.85。

Conclusion: 该模型具有良好的泛化能力，并有潜力支持低资源环境下的临床决策。
...
TL;DR: 本文介绍了一个新的、全面的、以政治为重点的deepfake检测数据集，该数据集包含300万张真实图像和96.3万张合成图像，这些图像由专有模型和开源模型生成。此外，本文还引入了一个众包对抗平台，以不断生成具有挑战性的合成图像，从而确保deepfake检测方法的鲁棒性和适应性。


<details>
  <summary>Details</summary>
Motivation: 现有的deepfake检测数据集通常是有限的，依赖于过时的生成方法、低真实感或单人脸图像，限制了通用合成图像检测的有效性。通过分析社交媒体帖子，我们发现deepfake传播错误信息的多种方式。我们的人类感知研究表明，最近开发的专有模型生成的合成图像越来越难以与真实图像区分，这使得公众难以准确识别。

Method: 我们提出了一个全面的、以政治为重点的数据集，该数据集专门用于针对现代生成模型进行基准检测。该数据集包含300万张真实图像，并配有描述性标题，这些标题用于从专有模型和开源模型的混合中生成96.3万张相应的高质量合成图像。我们引入了一个创新的众包对抗平台，参与者可以有偿生成和提交具有挑战性的合成图像。

Result: 我们创建了一个包含300万张真实图像和96.3万张合成图像的数据集。我们还建立了一个众包对抗平台，以不断生成具有挑战性的合成图像。

Conclusion: 我们建立了一个新的数据集和一个众包平台，以确保deepfake检测方法保持鲁棒性和适应性，从而主动保护公共言论免受复杂的错误信息威胁。
...
TL;DR: 本研究关注人体运动中局部运动和全局运动的相互依赖性，现有模型难以精确捕捉这种物理耦合，推导全局轨迹计算成本高昂。


<details>
  <summary>Details</summary>
Motivation: 现有方法无法精确捕捉局部和全局运动之间的物理耦合，并且从关节扭矩和外力推导全局轨迹在计算上既昂贵又复杂。

Method: 提出使用整体线性和角动量作为约束，将局部运动与全局运动联系起来，并引入一个新的损失项，用于强制生成的动量曲线与真实数据中的动量曲线之间的一致性。

Result: 减少了脚部滑动和抖动，改善了平衡，并保持了恢复运动的准确性。

Conclusion: 利用动量作为连接局部运动和全局运动的桥梁，能够有效地提升运动生成模型的性能。
...
TL;DR: 本文提出了一种新的任务：预测原始漫画线条艺术图像之间的区域对应关系，无需任何预先存在的标签或掩码。


<details>
  <summary>Details</summary>
Motivation: 理解漫画线条艺术图像之间的区域对应关系是漫画处理中的一项基本任务，能够实现自动线条艺术着色和中间帧生成等下游应用。然而，这项任务在很大程度上仍未被探索，尤其是在没有预先存在的分割或注释的真实场景中。

Method: 我们将每个线条艺术图像划分为一组patch，并提出了一个基于Transformer的框架，该框架学习图像内部和跨图像的patch级别相似性。然后，我们应用边缘感知聚类和区域匹配算法将patch级别预测转换为连贯的区域级别对应关系。

Result: 在多个数据集上的实验表明，我们的方法实现了较高的patch级别准确率（例如，96.34%），并生成了一致的区域级别对应关系。

Conclusion: 本文提出的方法具有应用于实际漫画应用的潜力
...
TL;DR: 提出了一个新颖的随机生成扩散融合 (SGDF) 方法，以解决多视图融合中低质量数据的问题。在此基础上，提出了生成扩散对比网络 (GDCN)。


<details>
  <summary>Details</summary>
Motivation: 多视图聚类 (MVC) 通过整合来自多个视图的异构数据来增强聚类分析，从而使多视图融合对聚类性能至关重要。然而，多视图融合中存在低质量数据的问题。这个问题主要由两个原因引起：1) 某些视图被噪声数据污染。2) 一些视图遭受数据丢失。

Method: 利用多重生成机制来处理每个样本的多视图特征。它对低质量数据具有鲁棒性。构建在 SGDF 之上，我们进一步提出了生成扩散对比网络 (GDCN)。

Result: GDCN 在深度 MVC 任务中实现了最先进的结果。

Conclusion: 提出的 GDCN 方法在深度多视图聚类任务中表现出色，代码已公开。
...
TL;DR: 提出了一种新的双编码器架构 DualTrack，用于从 2D 超声图像序列中估计 3D 探头轨迹，实现无传感器 3D 超声。


<details>
  <summary>Details</summary>
Motivation: 传统的 3D 超声系统成本高且复杂，而基于深度学习的无传感器 3D 超声是一种有前景的替代方案。为了更好地利用局部和全局特征，现有方法要么忽略全局特征，要么将全局特征与局部特征提取紧密结合，限制了对这两个互补方面的建模能力。

Method: DualTrack 采用解耦的局部和全局编码器，分别专注于不同尺度的特征提取。局部编码器使用密集时空卷积捕获细粒度特征，而全局编码器利用图像主干和时间注意力层来嵌入高级解剖特征和长期依赖关系。然后，一个轻量级融合模块结合这些特征来估计轨迹。

Result: 在大型公共基准测试上的实验结果表明，DualTrack 实现了最先进的精度和全局一致的 3D 重建，优于以前的方法，并产生了低于 5 毫米的平均重建误差。

Conclusion: DualTrack 是一种有效的无传感器 3D 超声方法，具有很高的精度和全局一致性。
...
TL;DR: 本文提出了一种新的视频扩散模型训练方法，通过对齐视频生成器的中间特征与预训练视觉编码器的特征表示，从而提升视频生成的质量。


<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型的研究主要集中在架构创新和训练目标上，而对特征表示能力的提升关注较少。本文旨在探索如何通过提升特征表示能力来改善视频扩散模型的性能。

Method: 本文提出了一种新的多特征融合与对齐方法Align4Gen，并将其整合到视频扩散模型的训练中。此外，还提出了一种新的指标，并深入分析了各种视觉编码器的判别性和时间一致性，以评估它们对视频特征对齐的适用性。

Result: 实验结果表明，Align4Gen在无条件和类条件视频生成任务中均能提升视频生成质量。

Conclusion: 通过将视频生成器的中间特征与预训练视觉编码器的特征表示对齐，可以有效提升视频扩散模型的性能。
...
TL;DR: 本文提出了一个名为InterAct的大规模3D人-物交互基准数据集，用于解决现有数据集在人-物交互建模和生成方面的局限性。
...
Method: 1) 整合和标准化来自不同来源的21.81小时HOI数据，并用详细的文本注释进行丰富。2) 提出了一个统一的优化框架，通过减少伪影和纠正手部运动来提高数据质量。3) 定义了六个基准测试任务，并开发了一个统一的HOI生成建模视角。

Result: 该数据集的规模扩展到30.70小时，并在六个基准测试任务上取得了state-of-the-art的性能。

Conclusion: InterAct数据集是一个基础资源，可以促进3D人-物交互生成的发展。
...
TL;DR: 这篇论文探讨了基于深度学习的MRI脑部图像阿尔茨海默病（AD）诊断中，由于种族和性别相关的捷径学习而导致的偏见问题。


<details>
  <summary>Details</summary>
Motivation: 研究动机是发现深度学习算法在MRI脑部图像AD诊断中可能存在捷径学习和人口统计偏见，这会导致对代表性不足的群体（如种族和性别）的性能偏差。

Method: 该研究首先调查深度学习算法是否可以从3D脑部MRI扫描中识别种族或性别，以确定是否存在基于种族和性别的分布变化。然后，研究调查种族或性别的训练集不平衡是否会导致模型性能下降，表明存在捷径学习和偏见。最后，对受保护属性和AD分类任务的不同大脑区域的特征属性进行定量和定性分析。

Result: 研究结果表明，基于深度学习的AD分类中存在基于种族和性别的捷径学习和偏见。

Conclusion: 这项工作为构建更公平的基于深度学习的脑部MRI诊断工具奠定了基础。
...
TL;DR: 提出了一种基于视觉基础模型(VFM)和参数高效微调(PEFT)的变化检测框架PeftCD，用于解决多时相和多源遥感图像中伪变化普遍、标记样本稀缺和跨域泛化困难的问题。
...
Method: 采用由VFM衍生的权重共享孪生编码器，并无缝集成LoRA和Adapter模块。通过仅训练最少的额外参数来实现高效的任务适应。使用Segment Anything Model v2 (SAM2)和DINOv3作为backbone，并使用轻量级解码器。

Result: 在多个公共数据集上实现了最先进的性能，包括SYSU-CD (IoU 73.81%)、WHUCD (92.05%)、MSRSCD (64.07%)、MLCD (76.89%)、CDD (97.01%)、S2Looking (52.25%)和LEVIR-CD (85.62%)，具有精确的边界划分和对伪变化的强大抑制。

Conclusion: PeftCD在精度、效率和泛化之间实现了最佳平衡，为将大规模VFM适应于实际遥感变化检测应用提供了一个强大且可扩展的范例。
...
TL;DR: 本文提出了Talk2Event，一个用于事件数据语言驱动对象定位的大规模基准数据集。


<details>
  <summary>Details</summary>
Motivation: 事件相机在建模高动态场景方面具有优势，但与自然语言理解的结合较少，多模态感知存在差距。

Method: 构建了一个包含5,567个场景，13,458个标注对象和超过30,000个指示性表达的数据集，每个表达都通过四个结构化属性进行丰富：外观、状态、与观察者的关系以及与周围物体的关系。

Result: 该数据集支持可解释和组合的定位，能够进行超越简单对象识别的动态环境中的上下文推理分析。
...
TL;DR: Kling-Avatar: A cascaded framework for generating high-fidelity audio-driven avatar videos that unifies multimodal instruction understanding with photorealistic portrait generation.


<details>
  <summary>Details</summary>
Motivation: Existing methods for audio-driven avatar video generation lack the ability to model the communicative purpose conveyed by instructions, compromising narrative coherence and character expressiveness.

Method: A two-stage pipeline is used: (1) a multimodal large language model (MLLM) director produces a blueprint video conditioned on diverse instruction signals; (2) guided by blueprint keyframes, multiple sub-clips are generated in parallel using a first-last frame strategy.

Result: Kling-Avatar generates vivid, fluent, long-duration videos at up to 1080p and 48 fps, achieving superior performance in lip synchronization accuracy, emotion and dynamic expressiveness, instruction controllability, identity preservation, and cross-domain generalization.

Conclusion: Kling-Avatar establishes a new benchmark for semantically grounded, high-fidelity audio-driven avatar synthesis.
...
TL;DR: 提出了一种混合机制学习框架，结合数学肿瘤生长模型和引导去噪扩散隐式模型 (DDIM)，从先前的扫描合成解剖学上可行的未来 MRI。
...
Method: 该方法结合了数学肿瘤生长模型与引导去噪扩散隐式模型 (DDIM)。

Result: 该框架生成了基于空间相似性指标的真实随访扫描，并引入了肿瘤生长概率图，捕捉了肿瘤生长的临床相关范围和方向性。

Conclusion: 该方法能够在数据受限的情况下实现生物信息图像生成，提供考虑机制先验的生成时空预测。
...
TL;DR: HumbleBench是一个新的基准测试，用于评估多模态大型语言模型（MLLM）拒绝错误答案的能力，以提高其在现实应用中的可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有的基准测试主要评估模型的识别准确率，忽略了模型识别何时没有正确选项的能力，这对于值得信赖的AI至关重要。MLLM在多模态大型语言模型中产生与输入图像不一致的内容，会对现实世界的应用构成重大风险。

Method: HumbleBench利用细粒度的场景图注释，提取ground-truth实体和关系，并提示GPT-4-Turbo生成多项选择题，然后进行严格的人工过滤。

Result: 对各种最先进的MLLM（包括通用和专用推理模型）在HumbleBench上进行了评估，并与社区分享了有价值的发现和见解。

Conclusion: HumbleBench通过结合显式的错误选项拒绝，填补了当前评估套件中的一个关键空白，为安全关键设置中MLLM的可靠性提供了更现实的衡量标准。
...
TL;DR: 提出了一种新的统一多模态学习框架UAE，该框架通过自编码器的视角，将图像理解为编码器（I2T），将图像生成为解码器（T2I），并使用重建保真度作为统一的训练目标。


<details>
  <summary>Details</summary>
Motivation: 为了实现图像理解和生成之间的连贯双向信息流，从而实现相互增益。

Method: 1. 预训练解码器以捕获细粒度的语义和复杂的空间关系。
2. 提出Unified-GRPO，通过强化学习分为三个阶段训练编码器和解码器：冷启动、生成式理解和理解式生成。

Result: 编码器自主生成更具描述性的标题，解码器能够理解这些复杂的描述，从而实现惊人的重建保真度。

Conclusion: 该研究提出了一个统一多模态学习的新框架，并在Unified-Bench基准测试中进行了评估。
...
### [113] [An Interval Type-2 Version of Bayes Theorem Derived from Interval Probability Range Estimates Provided by Subject Matter Experts](https://arxiv.org/abs/2509.08834)
*John T. Rickard,William A. Dembski,James Rickards*

Main category: cs.AI

TL;DR: 本文提出了一种区间二型贝叶斯定理，用于处理输入概率为区间估计的情况。


<details>
  <summary>Details</summary>
Motivation: 现实应用中，专家提供的往往是区间范围的概率估计，而非精确值，传统贝叶斯方法对此无能为力。

Method: 1. 开发了一种新的保守方法，以避免输入IT2 MFs中的潜在不一致性。2. 描述了一种新颖灵活的算法，用于将专家提供的区间编码为IT2模糊隶属函数 (MFs)。

Result: 扩展了贝叶斯定理到区间二型版本。

Conclusion: 所提出的算法推广并扩展了先前关于将区间编码为单词MFs 的工作。
...
### [114] [Automated Unity Game Template Generation from GDDs via NLP and Multi-Modal LLMs](https://arxiv.org/abs/2509.08847)
*Amna Hassan*

Main category: cs.AI

TL;DR: 本文介绍了一种使用自然语言处理(NLP)和多模态大型语言模型(llm)将游戏设计文档(GDDs)转换为功能性Unity游戏原型的新框架。


<details>
  <summary>Details</summary>
Motivation: 解决AI辅助游戏开发中的关键缺口，使llm成为简化从游戏设计到实现过渡的有价值的工具。

Method: 结合了一个专门用于Unity代码生成的微调LLaMA-3模型和一个定制的Unity集成包，该包简化了实现过程。

Result: 该微调模型在编译成功率、GDD依从性、最佳实践采用和代码模块化指标方面均优于baseline模型。

Conclusion: 该系统生成的模板在多个游戏类型中都表现出对GDD规范的高度遵循。
...
### [115] [Global Constraint LLM Agents for Text-to-Model Translation](https://arxiv.org/abs/2509.08970)
*Junyang Cai,Serdar Kadioglu,Bistra Dilkina*

Main category: cs.AI

TL;DR: 本研究提出了一种利用多个大型语言模型（LLM）智能体将自然语言描述的优化或满足问题转换为正确的MiniZinc模型框架。


<details>
  <summary>Details</summary>
Motivation: 将优化或满足问题的自然语言描述转换为正确的MiniZinc模型具有挑战性，因为这个过程需要逻辑推理和约束编程专业知识。

Method: 该框架采用一种智能体方法：多个专业的大型语言模型（LLM）智能体通过全局约束类型分解建模任务。每个智能体专注于检测和生成特定类别的全局约束代码，而最终的汇编智能体将这些约束片段集成到完整的MiniZinc模型中。

Result: 初步实验表明，与一次性提示和思维链提示等基线相比，该框架表现更好。

Conclusion: 本文概述了未来工作的综合路线图，强调了潜在的改进和改进方向。
...
### [116] [ForTIFAI: Fending Off Recursive Training Induced Failure for AI Models](https://arxiv.org/abs/2509.08972)
*Soheil Zibakhsh Shabgahi,Pedram Aghazadeh,Azalia Mirhosseini,Farinaz Koushanfar*

Main category: cs.AI

TL;DR: 模型重复在合成数据上训练会导致模型崩溃，性能下降。
...
Result: TCE 显著延迟了递归训练中的模型崩溃，并将模型的保真度间隔延长了 2.3 倍以上。该方法可以跨模态推广。
...
### [117] [Uncertainty Awareness and Trust in Explainable AI- On Trust Calibration using Local and Global Explanations](https://arxiv.org/abs/2509.08989)
*Carina Newen,Daniel Bodemer,Sonja Glantz,Emmanuel Müller,Magdalena Wischnewski,Lenka Schnaubert*

Main category: cs.AI

TL;DR: 本研究着眼于可解释人工智能(XAI)领域，特别是其中经常被忽视的不确定性解释和全局解释。


<details>
  <summary>Details</summary>
Motivation: 构建XAI方案的通用指南是研究重点，同时现有研究对不确定性解释和全局解释关注不足。

Method: 选择一个涵盖不确定性、鲁棒性和全局XAI等多种概念的算法，并测试其校准信任的能力。此外，还考察了一种旨在提供更直观视觉理解的算法。

Result: 研究检验了该算法在提供用户满意度和人类可解释性方面的能力。

Conclusion: 结论待定，研究着重于探索XAI算法在不确定性解释和全局解释方面的性能，以及它们对用户信任和理解的影响。
...
### [118] [Instructional Prompt Optimization for Few-Shot LLM-Based Recommendations on Cold-Start Users](https://arxiv.org/abs/2509.09066)
...
<details>
  <summary>Details</summary>
Motivation: 冷启动用户问题降低了推荐系统的有效性，因为它限制了对历史行为信息的访问。

Method: 本文介绍了一种上下文条件提示公式化方法P(u, Ds) -> R̂，其中u是冷启动用户画像，Ds是精心策划的支持集，R̂是预测的排序项目列表。该方法利用token级别的对齐和嵌入空间正则化，以获得更高的语义保真度。

Result: 通过对基于Transformer的自回归LLM（BioGPT、LLaMA-2、GPT-4）的系统实验，本文提供了经验证据，表明最佳的示例注入和指令结构可以显著提高这些模型在低数据设置下的precision@k和NDCG分数。

Conclusion: 本文表明，基于prompt的适应可以被认为是解决基于LLM的pipeline中冷启动推荐问题的一种方法。
...
### [119] [Understanding Economic Tradeoffs Between Human and AI Agents in Bargaining Games](https://arxiv.org/abs/2509.09071)
*Crystal Qian,Kehang Zhu,John Horton,Benjamin S. Manning,Vivian Tsai,James Wexler,Nithum Thain*

Main category: cs.AI

TL;DR: 自主体越来越多地执行传统上由人类执行的协调任务。因此，评估这些智能体的性能以及它们在动态多智能体环境中协商的过程至关重要。本文比较了人类、大型语言模型（LLM）和贝叶斯智能体在动态协商环境中的表现，发现贝叶斯智能体通过激进的优化获得最高的收益，但代价是频繁的交易拒绝。人类和LLM可以获得相似的整体收益，但行为方式不同：LLM倾向于保守的、让步的交易，拒绝较少，而人类则采用更具战略性、冒险性和公平导向的行为。因此，性能对等性可能会掩盖过程和一致性的根本差异，而这对于在现实世界的协调任务中的实际部署至关重要。


<details>
  <summary>Details</summary>
Motivation: 评估自主智能体在动态多智能体环境中协商的过程，并比较不同智能体的优势，例如贝叶斯模型和大型语言模型（LLM）。

Method: 在动态协商环境中比较人类、大型语言模型（GPT-4o, Gemini 1.5 Pro）和贝叶斯智能体的表现，直接比较不同人群在相同条件下的结果和行为动态。

Result: 贝叶斯智能体通过激进的优化获得最高的收益，但代价是频繁的交易拒绝。人类和LLM可以获得相似的整体收益，但行为方式不同：LLM倾向于保守的、让步的交易，拒绝较少，而人类则采用更具战略性、冒险性和公平导向的行为。

Conclusion: 性能对等性可能会掩盖过程和一致性的根本差异，而这对于在现实世界的协调任务中的实际部署至关重要。
...
### [120] [Anti-Money Laundering Machine Learning Pipelines; A Technical Analysis on Identifying High-risk Bank Clients with Supervised Learning](https://arxiv.org/abs/2509.09127)
*Khashayar Namdar,Pin-Chien Wang,Tushar Raju,Steven Zheng,Fiona Li,Safwat Tahmin Khan*

Main category: cs.AI

TL;DR: 本文提出了一种使用机器学习管道识别高风险银行客户的全面系统方法，该管道在多伦多大学 2023-2024 IMI 大数据和人工智能竞赛的任务 1 中使用的数据集上进行了评估。
...
Method: 我们采用了一个 16 步设计和统计分析，以确保最终管道的稳健性。我们还使用 SQLite 数据库构建数据，开发基于 SQL 的特征工程算法，将我们预训练的模型连接到数据库，并使其能够进行推理，并提供可解释的人工智能 (XAI) 模块以导出特征重要性。

Result: 我们的管道实现的平均接收者操作特征曲线下面积 (AUROC) 为 0.961，标准差 (SD) 为 0.005。拟议的管道在比赛中获得第二名。

Conclusion: 该论文提出了一种有效的机器学习管道，用于识别高风险银行客户，并在竞赛中取得了良好的效果。
...
### [121] [Mind Meets Space: Rethinking Agentic Spatial Intelligence from a Neuroscience-inspired Perspective](https://arxiv.org/abs/2509.09154)
*Bui Duc Manh,Soumyaratna Debnath,Zetong Zhang,Shriram Damodaran,Arvind Kumar,Yueyi Zhang,Lu Mi,Erik Cambria,Lin Wang*

Main category: cs.AI

TL;DR: 本文提出了一种基于神经科学原理的计算框架，旨在弥合智能体人工智能在空间推理能力上的差距，使其能够更好地与物理3D世界交互。


<details>
  <summary>Details</summary>
Motivation: 当前智能体AI在空间推理能力方面存在局限性，无法像人类一样进行灵活、上下文感知的决策。为了弥合这一差距，本文旨在提升Agentic Spatial Intelligence，使其更好地与物理3D世界交互。

Method: 本文首先研究了计算神经科学中的空间神经模型，并在此基础上提出了一个新的计算框架。该框架将核心生物学功能映射到六个关键计算模块：生物启发的多模态感知、多感官集成、以自我为中心的坐标转换、人工认知地图、空间记忆和空间推理。

Result: 本文构建了一个视角景观，用于评估虚拟和物理环境中智能体的空间推理能力。通过框架指导分析，评估了现有方法与每个模块的相关性，并识别了阻碍基于神经科学的空间推理模块发展的关键差距。此外，还探讨了新兴的基准和数据集，以及从虚拟到实体系统（如机器人）的潜在应用领域。

Conclusion: 本文概述了潜在的研究方向，强调了在动态或非结构化环境中推广空间推理的有希望的路线图。希望这项工作能为研究界带来一个基于神经科学的视角和一个结构化的路径。
...
### [122] [ProgD: Progressive Multi-scale Decoding with Dynamic Graphs for Joint Multi-agent Motion Forecasting](https://arxiv.org/abs/2509.09210)
*Xing Gao,Zherui Huang,Weiyao Lin,Xiao Sun*

Main category: cs.AI

TL;DR: 本文提出了一种新的渐进式多尺度解码策略（ProgD），用于更准确地预测自动驾驶车辆周围多个交互主体的运动轨迹。


<details>
  <summary>Details</summary>
Motivation: 现有方法忽略了交互的演变性质，为了解决这个问题。

Method: 利用动态异构图进行场景建模，并设计了一种分解架构来处理时空依赖关系，逐步消除未来运动的不确定性。此外，还加入了多尺度解码程序。

Result: 在INTERACTION和Argoverse 2基准测试中达到了最先进的性能。

Conclusion: ProgD 能够有效地捕捉未来场景中不断变化的社会互动，并持续地预测智能体未来的运动轨迹。
...
### [123] [Enabling Regulatory Multi-Agent Collaboration: Architecture, Challenges, and Solutions](https://arxiv.org/abs/2509.09215)
*Qinnan Hu,Yuntao Wang,Yuan Gao,Zhou Su,Linkang Du*

Main category: cs.AI

TL;DR: 本研究提出了一种基于区块链的分层架构，用于监管代理协作，以应对LLM驱动的自主代理带来的治理和问责挑战。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型驱动的自主代理正在改变数字和物理环境，但也带来了不可预测的行为和异构能力，从而构成了巨大的治理和问责挑战。

Method: 设计了一个包含代理层、区块链数据层和监管应用层的分层架构，以及三个关键模块：代理行为追踪和仲裁模块、动态声誉评估模块和恶意行为预测模块。
...
### [124] [Jupiter: Enhancing LLM Data Analysis Capabilities via Notebook and Inference-Time Value-Guided Search](https://arxiv.org/abs/2509.09245)
*Shuocheng Li,Yihao Liu,Silin Du,Wenxuan Zeng,Zhe Xu,Mengyu Zhou,Yeye He,Haoyu Dong,Shi Han,Dongmei Zhang*

Main category: cs.AI

TL;DR: 本研究提出了一种可扩展的流程，用于从真实的 Jupyter notebooks 中提取高质量、基于工具的数据分析任务及其可执行的多步骤解决方案，并构建了一个名为 NbQA 的大型数据集。同时，提出了一个名为 Jupiter 的框架，该框架将数据分析 формулируется 为搜索问题，并应用蒙特卡洛树搜索 (MCTS) 来生成用于价值模型学习的各种解决方案轨迹。


<details>
  <summary>Details</summary>
Motivation: 现有的大型语言模型 (LLM) 在自动化数据科学工作流程方面显示出巨大的潜力，但现有模型仍然难以进行多步骤推理和工具使用，这限制了它们在复杂数据分析任务中的有效性。

Method: 该研究提出一个可扩展的流程，用于提取高质量、基于工具的数据分析任务，并提出了一个名为 Jupiter 的框架，该框架将数据分析 формулируется 为搜索问题，并应用蒙特卡洛树搜索 (MCTS) 来生成解决方案轨迹。

Result: 在 NbQA 上，Qwen2.5-7B 和 14B-Instruct 模型分别解决了 InfiAgent-DABench 上 77.82% 和 86.38% 的任务，与 GPT-4o 和高级代理框架相匹配或超过。

Conclusion: 实验结果表明，该研究提出的方法提高了泛化能力，并在各种多步骤推理任务中增强了工具使用的推理能力。
...
### [125] [Fusing Knowledge and Language: A Comparative Study of Knowledge Graph-Based Question Answering with LLMs](https://arxiv.org/abs/2509.09272)
*Vaibhav Chaudhary,Neha Soni,Narotam Singh,Amita Kapoor*

Main category: cs.AI

TL;DR: 本研究比较了三种知识图谱构建方法（spaCy、Stanford CoreNLP-OpenIE 和 GraphRAG）在增强大型语言模型问答系统中的应用。


<details>
  <summary>Details</summary>
Motivation: 传统RAG方法在处理复杂文本时存在局限性，需要更深入地分析文本和上下文。

Method: 通过比较三种知识图谱构建方法，评估它们在LLM问答中的有效性、可行性和适应性。
...
TL;DR: 本文探索了如何利用MCTS生成的轨迹来改进基于偏好的强化学习中的策略优化，特别是在没有价值网络的情况下。


<details>
  <summary>Details</summary>
Motivation: 利用MCTS生成高质量的中间轨迹在数学和符号领域已被证明有效，受此启发，探索MCTS轨迹在基于偏好的强化学习中改进策略优化的潜力。

Method: 提出了一种分阶段的GRPO训练范例，其中补全来自部分揭示的MCTS rollout，从而引入了一种新的树状结构优势估计设置。

Result: 初步结果表明，结构化优势估计可以稳定更新，更好地反映组合推理质量，但仍然存在优势饱和和奖励信号崩溃等挑战。
...
### [127] [LightAgent: Production-level Open-source Agentic AI Framework](https://arxiv.org/abs/2509.09292)
*Weige Cai,Tong Zhu,Jinyi Niu,Ruiqi Hu,Lingyao Li,Tenglong Wang,Xiaowu Dai,Weining Shen,Liwen Zhang*

Main category: cs.AI

TL;DR: LightAgent是一个轻量但强大的agent框架，它整合了记忆、工具和思维树等核心功能，同时保持轻量级结构。


<details>
  <summary>Details</summary>
Motivation: 现有的多智能体系统（MAS）在设计通用、健壮和高效的agent部署平台方面面临挑战，现有框架的灵活性和简易性之间需要权衡。

Method: 提出了LightAgent框架，它整合了记忆（mem0）、工具和思维树（ToT）等核心功能。

Result: LightAgent已开源，并且可以无缝集成到主流聊天平台，使开发人员能够轻松构建自学习agent。

Conclusion: LightAgent是一个轻量但强大的agent框架，有效地解决了现有框架在灵活性和简易性之间的权衡问题。
...
### [128] [Explaining Tournament Solutions with Minimal Supports](https://arxiv.org/abs/2509.09312)
*Clément Contet,Umberto Grandi,Jérôme Mengin*

Main category: cs.AI

TL;DR: 这篇论文研究了为候选人在各种锦标赛规则下获胜提供认证解释的问题。


<details>
  <summary>Details</summary>
Motivation: 研究候选人在锦标赛中获胜的原因，并提供可解释的人工智能。

Method: 确定最小支持，即候选人保证获胜的最小子锦标赛。

Result: 确定了常见锦标赛解决方案的最小支持的大小，并为除加权未覆盖集之外的所有规则提出了计算它们的 polynomial-time 算法，对于加权未覆盖集，该问题是 NP-complete 的。
...
### [129] [Measuring Implicit Spatial Coordination in Teams: Effects on Collective Intelligence and Performance](https://arxiv.org/abs/2509.09314)
*Thuy Ngoc Nguyen,Anita Williams Woolley,Cleotilde Gonzalez*

Main category: cs.AI

TL;DR: 本文研究了在没有明确沟通的情况下，空间协调的三个维度（探索多样性、移动专业化和自适应空间邻近性）如何影响团队表现。


<details>
  <summary>Details</summary>
Motivation: 许多团队（消防员、军队、执法部门、应急响应）必须在没有视觉提示或大量明确沟通的情况下协调他们在物理空间中的行动。

Method: 通过协作在线搜索和救援任务，限制明确的沟通，并依靠运动模式来推断他人的意图和协调行动。

Result: 空间专业化与绩效呈正相关，而自适应空间邻近性呈边际倒U型关系，表明适度的适应水平是最佳的。

Conclusion: 研究结果揭示了基于角色的团队合作中内隐空间协调，并强调了平衡自适应策略的重要性，对培训和人工智能辅助团队支持系统具有影响。
...
### [130] [Towards Adaptive ML Benchmarks: Web-Agent-Driven Construction, Domain Expansion, and Metric Optimization](https://arxiv.org/abs/2509.09321)
*Hangyi Jia,Yuxi Qian,Hanwen Tong,Xinhui Wu,Lin Chen,Feng Wei*

Main category: cs.AI

TL;DR: 提出了一个用于评估基于LLM的agent在端到端ML任务上的基准测试，名为TAM Bench。


<details>
  <summary>Details</summary>
Motivation: 现有的基准测试在任务覆盖范围、领域多样性、难度建模和评估严格性方面存在局限性，无法捕捉此类agent在实际环境中的全部能力。

Method: 使用浏览器自动化和LLM的任务获取系统，自动收集和构建来自Kaggle等平台的ML挑战，并使用排行榜驱动的难度建模机制来估计任务复杂性。

Result: 构建了包含150个AutoML任务的三个不同大小的基准子集，Lite版本包含18个任务，可用于日常基准测试和比较研究。

Conclusion: TAM Bench是一个多样化、真实且结构化的基准，用于评估基于LLM的agent在端到端ML任务上的能力。
...
### [131] [Curriculum-Based Multi-Tier Semantic Exploration via Deep Reinforcement Learning](https://arxiv.org/abs/2509.09356)
...
<details>
  <summary>Details</summary>
Motivation: 传统的RL方法难以平衡高效探索和语义理解，因为智能体的策略认知能力有限，通常需要人工干预。

Method: 通过分层奖励函数整合视觉-语言模型（VLM）常识，并将VLM查询建模为专用动作，使智能体仅在必要时才查询VLM以获得外部指导，从而节省资源。结合课程学习策略，在不同复杂程度的层面上引导学习，以确保稳健和稳定的学习。

Result: 实验结果表明，该智能体显著提高了物体发现率，并发展出有效导航到语义丰富区域的能力，并且能够策略性地掌握何时提示外部环境信息。

Conclusion: 该研究展示了一种将常识语义推理嵌入到自主智能体中的实用且可扩展的方法，为在机器人技术中追求完全智能和自我引导的探索提供了一种新颖的方法。
...
### [132] [TORSO: Template-Oriented Reasoning Towards General Tasks](https://arxiv.org/abs/2509.09448)
*Minhyuk Kim,Seungyoon Lee,Heuiseok Lim*

Main category: cs.AI

TL;DR: 这篇论文介绍了一种名为TORSO的新方法，旨在引导大型语言模型（LLM）利用其内在推理能力生成响应，而无需手动制作少量样本示例。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖于提供的示例，限制了模型固有的推理能力，并且构建特定于任务的少量样本提示成本高昂，可能导致不同任务之间不一致。

Method: 该论文介绍了面向模板的推理（TORSO），它引导模型利用内部推理能力来生成跨各种任务的适当响应，而无需手动制作少量样本示例。

Result: 实验结果表明，TORSO在不同的LLM基准上取得了强大的性能，并具有合理的原理。

Conclusion: TORSO方法能够使LLM在各种任务中生成合理的响应，且无需手动制作少量样本示例，实验证明了其有效性。
...
### [133] [Inteligencia Artificial jurídica y el desafío de la veracidad: análisis de alucinaciones, optimización de RAG y principios para una integración responsable](https://arxiv.org/abs/2509.09467)
*Alex Dantart*

Main category: cs.AI

TL;DR: 该技术报告分析了法律领域LLM中的“幻觉”问题，包括原因、表现和RAG缓解策略的有效性，强调了其局限性并提出了整体优化方案。


<details>
  <summary>Details</summary>
Motivation: 探讨了LLM在法律应用中产生幻觉的挑战，以及解决这些幻觉的必要性。

Method: 分析了幻觉的原因、表现，并评估了RAG缓解策略的有效性。

Result: 强调了RAG缓解策略的局限性，并提出了整体优化方案。强调人工监督的不可替代性。

Conclusion: 解决方案在于采用“咨询式”AI模式，优先考虑真实性和可追溯性，作为增强而非取代专业判断的工具。
...
### [134] [SEDM: Scalable Self-Evolving Distributed Memory for Agents](https://arxiv.org/abs/2509.09498)
*Haoran Xu,Jiacong Hu,Ke Zhang,Lei Yu,Yuxin Tang,Xinyuan Song,Yiqun Duan,Lynn Ai,Bill Shi*

Main category: cs.AI

TL;DR: 提出了一种可验证和自适应的框架SEDM，将存储器从被动存储库转变为主动的、自我优化的组件。


<details>
  <summary>Details</summary>
Motivation: 现有的方法容易出现噪声累积、不受控制的内存扩展以及跨域泛化能力有限的问题。

Method: SEDM集成了基于可重复重放的可验证写入许可、根据经验效用动态排序和合并条目的自调度内存控制器以及抽象可重用见解以支持跨异构任务转移的跨域知识扩散。

Result: 在基准数据集上的评估表明，与强大的内存基线相比，SEDM提高了推理精度，同时减少了令牌开销，并且进一步实现了从事实验证中提取的知识来增强多跳推理。

Conclusion: SEDM是用于开放式多智能体协作的可扩展和可持续的存储机制。
...
### [135] [Compositional Concept Generalization with Variational Quantum Circuits](https://arxiv.org/abs/2509.09541)
*Hala Hawashin,Mina Abbaszadeh,Nicholas Joseph,Beth Pearson,Martha Lewis,Mehrnoosh sadrzadeh*

Main category: cs.AI

TL;DR: 这篇论文探讨了量子模型在视觉-语言模型中克服组合泛化挑战的能力。


<details>
  <summary>Details</summary>
Motivation: 当前AI工具（如视觉-语言模型）缺乏组合泛化能力。先前的研究尝试使用基于组合张量的句子语义来解决这个问题，但结果不理想。作者推测量子模型更高的训练效率可以改善这些任务的性能。

Method: 作者将基于组合张量的模型的表示在希尔伯特空间中进行解释，并训练变分量子电路来学习这些表示，应用于需要组合泛化的图像描述任务。使用了两种图像编码技术：二进制图像向量上的多热编码（MHE）和来自视觉-语言模型CLIP的图像向量上的角度/幅度编码。

Result: 使用噪声MHE编码获得了良好的概念验证结果。在CLIP图像向量上的性能好坏参半，但仍然优于经典的组合模型。

Conclusion: 量子模型在组合泛化任务中具有潜力，但仍需进一步研究。
...
### [136] [Boosting Embodied AI Agents through Perception-Generation Disaggregation and Asynchronous Pipeline Execution](https://arxiv.org/abs/2509.09560)
...
Method: Auras disaggregates the perception and generation and provides controlled pipeline parallelism; Auras establishes a public context for perception and generation to share.

Result: Auras improves throughput by 2.54x on average while achieving 102.7% of the original accuracy.

Conclusion: Auras overcomes the constraints of sequential computation and providing high throughput.
...
### [137] [The Illusion of Diminishing Returns: Measuring Long Horizon Execution in LLMs](https://arxiv.org/abs/2509.09677)
*Akshit Sinha,Arvindh Arun,Shashwat Goel,Steffen Staab,Jonas Geiping*

Main category: cs.AI

TL;DR: 大型语言模型（LLM）的持续扩展是否会产生收益递减？本文研究发现，单步准确率的边际收益可以转化为模型成功完成任务长度的指数级提升。大型模型在执行能力方面表现更优，即使小型模型具有100%的单步准确率。模型在上下文中包含先前错误的步骤时，更容易出错（自 कंडीशनिंग效应）。通过关注执行能力，可以更好地解决LLM在解决复杂推理问题和较长简单任务时的失败问题，并突出扩展模型大小和序列测试时计算对长时程任务的巨大益处。


<details>
  <summary>Details</summary>
Motivation: LLM在执行较长简单任务时会失败，这源于执行中的错误，而不是推理能力不足。

Method: 通过显式提供解决长时程任务所需的知识和计划来隔离执行能力。

Result: 即使小型模型具有100％的单步准确率，较大的模型也可以正确执行更多的步骤。模型在上下文中包含先前错误的步骤时，更容易出错（自 कंडीशनिंग效应）。

Conclusion: 通过关注执行能力，可以更好地解决LLM在解决复杂推理问题和较长简单任务时的失败问题，并突出扩展模型大小和序列测试时计算对长时程任务的巨大益处。
...
### [138] [Koza and Koza-Hub for born-interoperable knowledge graph generation using KGX](https://arxiv.org/abs/2509.09096)
...
TL;DR: 本研究旨在解决生物医学知识图谱构建中数据冗余和缺乏标准化的问题。


<details>
  <summary>Details</summary>
Motivation: 当前生物医学知识图谱构建方法需要大量冗余劳动，源于缺乏数据标准和“知识图谱就绪”数据。

Method: 引入Koza和Koza-Hub，一个Python软件包，用于简化将原始生物医学信息摄取到KGX格式的过程，并为三十个黄金标准生物医学数据源提供转换流程。将知识图谱摄取转化为一组原始操作，并通过YAML文件进行配置，并强制遵守所选的数据模式。

Result: 创建了一个Python软件包和转换流程，以简化生物医学信息到KGX格式的转换。

Conclusion: Koza和Koza-Hub可以有效简化生物医学知识图谱的构建过程，并解决数据冗余和缺乏标准化的问题。
...
### [139] [Let's Simply Count: Quantifying Distributional Similarity Between Activities in Event Data](https://arxiv.org/abs/2509.09440)
*Henrik Kirchmann,Stephan A. Fahrenkrog-Petersen,Xixi Lu,Matthias Weidlich*

Main category: cs.DB

TL;DR: 本文提出了一种简单的基于计数的嵌入方法，用于评估活动之间的分布相似性，并避免了复杂的训练过程。


<details>
  <summary>Details</summary>
Motivation: 现有的过程挖掘方法通常采用基于神经网络的方法来评估活动相似性，但这些方法计算成本高，且学习到的表示的可解释性有限。

Method: 本文提出了一种基于计数的嵌入方法，该方法避免了复杂的训练过程，并提供了一种直接可解释的表示。

Result: 实验结果表明，基于计数的嵌入方法为事件数据中活动之间的分布相似性提供了一个高效的基础。
...
### [140] [Database Views as Explanations for Relational Deep Learning](https://arxiv.org/abs/2509.09482)
*Agapi Rissaki,Ilias Fountalis,Wolfgang Gatterbauer,Benny Kimelfeld*

Main category: cs.DB

TL;DR: This paper introduces a new framework for explaining machine learning models over relational databases. The explanations are view definitions that highlight focused parts of the database that mostly contribute to the model's prediction. The approach is evaluated on the RelBench collection.


<details>
  <summary>Details</summary>
Motivation: The complexity of deep learning models over relational databases makes it hard to explain how a model uses the available data to arrive at a given prediction.

Method: The authors present a novel framework for explaining machine-learning models over relational databases, where explanations are view definitions. They establish such global abductive explanations by adapting the classic notion of determinacy. They develop heuristic algorithms that avoid the exhaustive search over the space of all databases. They propose techniques that are model-agnostic, and others that are tailored to hetero-GNNs via the notion of learnable masking.

Result: The results demonstrate the usefulness of the proposed explanations, as well as the efficiency of their generation.

Conclusion: The paper introduces a novel framework for explaining machine learning models over relational databases, with experiments showing the usefulness and efficiency of the approach.
...
TL;DR: AI搜索引擎的快速发展正在从传统的排名列表转向综合的、有引文支持的答案，从而从根本上改变信息检索。


<details>
  <summary>Details</summary>
Motivation: 本文旨在对比分析AI搜索和传统Web搜索（谷歌），揭示它们在信息来源方面的关键差异。

Method: 通过一系列大规模的、受控的实验，跨多个垂直领域、语言和查询释义，量化这些系统在信息来源方面的关键差异。

Result: AI搜索表现出对赢得媒体（第三方权威来源）的系统性和压倒性偏好，而谷歌的混合更为平衡。AI搜索服务在领域多样性、新鲜度、跨语言稳定性以及对措辞的敏感性方面彼此差异显著。

Conclusion: 根据这些实验结果，制定了一个战略性的GEO议程。强调了为实现生成搜索领域的可视性，从业者需要：(1)为机器可扫描性和论证设计内容，(2)主导赢得媒体以建立AI感知权威，(3)采用特定于引擎和语言感知的策略，以及(4)克服小众参与者的固有“大品牌偏见”。
...
TL;DR: 本文概述了无妒忌性及其在经济学和推荐系统中的应用，并说明了为什么妒忌不适合衡量个性化场景中的公平性。


<details>
  <summary>Details</summary>
Motivation: 在经济学、博弈论和社会选择文献中，无妒忌性以及放宽到无妒忌性（最多一项）的概念已被用作公平性概念，并且最近在推荐系统社区中越来越受欢迎。

Method: 概述无妒忌性及其在经济学和推荐系统中的应用。

Result: 说明了为什么妒忌不适合衡量个性化场景中的公平性。

Conclusion: 妒忌不适合衡量个性化场景中的公平性。
...
Method: Introduces the Dilated Refinement Attention Module (DREAM) for fine-grained semantic pattern alignment and applies Maximum Mean Discrepancy (MMD) and contrastive loss functions for global modality alignment.
...
Conclusion: MambaRec improves multimodal recommendation by enhancing cross-modal semantic modeling and ensuring semantic consistency through local and global alignment strategies.
...
TL;DR: 提出了一种新的框架CESRec，它集成了SRS的长期偏好建模和CRS的实时偏好提取。


<details>
  <summary>Details</summary>
Motivation: 现有的SRS方法通常依赖于协同过滤信号，无法捕捉实时的用户偏好，而CRS擅长通过自然语言交互来激发即时兴趣，但忽略了历史行为。为了弥合这一差距。

Method: 引入了基于语义的伪交互构建，通过分析对话反馈来动态更新用户的历史交互序列，生成一个无缝结合长期和实时偏好的伪交互序列。此外，通过提出双重对齐异常值项目屏蔽，减少了偏离用户核心偏好的历史项目中的异常值的影响，该方法使用语义协作对齐的表示来识别和屏蔽这些项目。

Result: 大量实验表明，CESRec通过增强强大的SRS模型实现了最先进的性能，验证了其在将对话反馈集成到SRS中的有效性。

Conclusion: CESRec有效地集成了SRS的长期偏好建模和CRS的实时偏好提取，并在实验中表现出最先进的性能。
...
TL;DR: recommender systems research needs fundamental reframing beyond technical reform, focusing on epistemic humility, human impact, and sustainable practice.


<details>
  <summary>Details</summary>
Motivation: The paper revisits Amatriain's critique of recommender systems research and argues that many of the conceptual, epistemological, and infrastructural failures he identified still persist.

Method: Drawing on recent work in reproducibility, evaluation methodology, environmental impact, and participatory design, the paper showcases how the field's accelerating complexity has outpaced its introspection. It highlights ongoing community-led initiatives that attempt to shift the paradigm.

Result: The paper contends that meaningful change will require not only new metrics or better tooling, but a fundamental reframing of what recommender systems research is for, who it serves, and how knowledge is produced and validated.

Conclusion: The paper calls for a recommender systems research agenda grounded in epistemic humility, human impact, and sustainable practice, beyond just technical reform.
...
TL;DR: 这篇论文提出了一种通过高质量负样本和有效的小批量数据来提升多语言密集检索数据利用率的方法。


<details>
  <summary>Details</summary>
Motivation: 现有的多语言密集检索方法依赖于对比学习，其效果高度依赖于负样本的质量和小批量数据的有效性。

Method: 该方法通过获取高质量的困难负样本和有效的小批量数据来提升数据利用率。

Result: 在包含 16 种语言的多语言检索基准 MIRACL 上的大量实验结果表明，该方法优于几种现有的强基线。

Conclusion: 该方法能够有效提升多语言密集检索的性能。
...
TL;DR: 本研究旨在了解在线Ask the Doctor服务在互联网平台上的医疗建议，并研究平台是否反映了医疗保健中现有的障碍和偏见。


<details>
  <summary>Details</summary>
Motivation: 研究互联网平台AskDoc上的在线Ask the Doctor服务，了解其医疗建议，并分析平台是否反映了医疗保健中存在的障碍和偏见。

Method: 从AskDoc subreddit下载2020年1月至2022年5月的数据，创建正则表达式以识别帖子中自我报告的人口统计信息（性别、种族和年龄），并进行统计分析以了解同行和医生与发帖者之间的互动。

Result: 一半的帖子没有收到同行或医生的评论。至少90%的人公开了他们的性别和年龄，80%的人没有公开他们的种族。该subreddit主要由成年（20-39岁）白人男性主导。在用户和具有特定人口统计特征的发帖者之间的互动中观察到了一些差异。

Conclusion: 社交媒体可以将患者和提供者更紧密地联系在一起，但与发帖者相比，目前医生的参与度较低。
...
TL;DR: 本文研究了无线电法规领域的问答系统，提出了一个电信领域的RAG流程，并引入了一个多项选择评估集。
...
Method: 提出了一个电信特定的检索增强生成（RAG）流程。

Result: 该方法在所有测试模型上持续提高了生成准确率，特别是与直接插入文档相比，使用该流程使GPT-4o的性能提高了近12%。

Conclusion: 有针对性的 grounding 提供了一个简单但强大的基线，并且是针对监管问题回答的有效领域特定解决方案。
...
TL;DR: 本文提出了一种基于信噪比的置信度估计和分解框架，并使用方差门控测量来讨论委员会机器多样性崩溃的问题。


<details>
  <summary>Details</summary>
Motivation: 对神经网络的单样本不确定性量化对于涉及高风险应用的决策至关重要。预测不确定性通常被分解为认知不确定性和偶然不确定性，但这种分解方法最近受到了质疑。

Method: 本文提出了一种基于类概率分布信噪比的置信度估计和分解框架。引入了一种方差门控测量，该测量通过从集成模型中导出的置信度因子来缩放预测。

Result: 本文使用该测量来讨论委员会机器多样性崩溃的存在性。
...
TL;DR: 本文提出了一种改进的矩阵乘法权重更新（MMWU）算法，用于解决矩阵专家建议学习（LEA）问题，该算法在不增加计算复杂性的前提下，实现了实例最优的遗憾界。


<details>
  <summary>Details</summary>
Motivation: 现有的MMWU算法在矩阵LEA问题上实现了minimax最优的遗憾界，但本文旨在进一步改进该算法，实现实例最优的遗憾界。

Method: 本文首先为矩阵LEA开发了一个通用的基于势的框架，然后利用基于拉普拉斯变换技术的“单边”Jensen迹不等式，将一般势函数应用于矩阵LEA。最后，基于虚误差函数，从向量LEA问题中推导出一个最优势函数。

Result: 本文提出的算法实现了O(sqrt(T*S(X||d^{-1}I_d)))的实例最优遗憾界，且与MMWU算法具有相同的计算复杂度。该算法在学习受退极化噪声、随机量子态和吉布斯态污染的量子态方面优于现有技术。

Conclusion: 本文提出的改进MMWU算法在矩阵LEA问题上具有重要意义，并可应用于量子学习理论，用于预测非线性量子特性。
...
TL;DR: 提出了一个鲁棒的Q学习算法，即使在部分奖励被对抗性扰动的情况下也能有效工作。


<details>
  <summary>Details</summary>
Motivation: 经典强化学习算法在奖励信号受到对抗性破坏时性能会严重下降，例如极端噪声、传感器故障或恶意攻击。

Method: 提出了一个新的鲁棒Q学习算法，该算法在异步采样模型下工作。

Result: 该算法的有限时间收敛速度与非对抗情况下的现有结果相匹配，最多增加一个与损坏样本比例相关的附加项。导出了一个信息论下界，表明上限中的附加损坏项是不可避免的。

Conclusion: 为异步Q学习提供了第一个有限时间鲁棒性保证，弥合了鲁棒RL中的一个显著差距。
...
<details>
  <summary>Details</summary>
Motivation: 在具有多个异构数据生成源的应用程序中，标准的 ML 方法通常会学习虚假的相关性，这些相关性平均表现良好，但会降低非典型或代表性不足的组的性能。以前的工作通过优化最差组的性能来解决这个问题。然而，这些方法通常假设可以使用训练数据准确估计每个组的底层数据分布，但在嘈杂、非平稳和不断变化的环境中，这一条件经常被违反。

Method: 我们开发了一种梯度下降上升算法来解决所提出的 DRO 问题，并提供了收敛结果。

Result: 在真实世界的数据上验证了我们方法的有效性。

Conclusion: 这篇论文提出了一个基于 Wasserstein 的分布鲁棒优化 (DRO) 框架，用于解决机器学习模型在异构数据源上的性能下降问题。
...
TL;DR: FoundationalECGNet: A new deep learning framework for automated ECG classification, achieving high F1-scores across multiple datasets.


<details>
  <summary>Details</summary>
Motivation: Accurate ECG analysis is crucial for detecting cardiac abnormalities, but current methods are limited by noise, class imbalance, and dataset heterogeneity.
...
Result: Achieves 99% F1-score for Normal vs. Abnormal classification and state-of-the-art performance in multi-class disease detection, including 99% F1-score for Conduction Disorders and Hypertrophy, and 98.9% F1-score for Arrhythmias.
...
### [154] [Value bounds and Convergence Analysis for Averages of LRP attributions](https://arxiv.org/abs/2509.08963)
*Alexander Binder,Nastaran Takmil-Homayouni,Urun Dogan*

Main category: cs.LG

TL;DR: 本文通过将逐层相关性传播 (LRP) 类归因方法表示为修正梯度矩阵的乘积，分析了它们的数值性质。并推导了奇异值的上限和归因图值的分量上限。
...
Result: 推导出分量上限以获得乘法常数。这些常数控制着归因的经验均值向归因图期望的收敛。LRP-beta 的常数与权重范数无关，这与基于梯度的方法和 LRP-epsilon 显着不同。

Conclusion: 该发现对将多个非几何数据增强应用于单个测试样本以及 Smoothgrad 类归因方法的情况具有重要意义。
...
### [155] [Green Federated Learning via Carbon-Aware Client and Time Slot Scheduling](https://arxiv.org/abs/2509.08980)
*Daniel Richards Arputharaj,Charlotte Rodriguez,Angelo Rodio,Giovanni Neglia*

Main category: cs.LG

TL;DR: 本文探讨了如何在联邦学习中通过碳感知客户端选择和训练调度来减少碳排放。


<details>
  <summary>Details</summary>
Motivation: 大规模机器学习模型的训练会产生大量的碳排放。联邦学习通过在地理上分散的客户端上分布计算，提供了一个利用区域和时间碳强度变化的框架。

Method: 1. 量化了碳感知调度策略的排放节省，该策略利用了空闲时间，允许适度延长训练时间，以便客户端可以将本地训练轮次推迟到较低碳时段。
2. 构建了一个碳感知调度器，该调度器集成了空闲时间、α-公平碳分配和全局微调阶段。

Result: 在真实世界碳强度数据上进行的实验表明，本文提出的调度器优于与空闲时间无关的基线，在各种碳预算下实现了更高的模型精度，尤其是在严格的碳约束下获得了强大的收益。

Conclusion: 本文研究表明，通过碳感知的客户端选择和训练调度，可以在联邦学习中显著减少碳排放，同时保持或提高模型精度。
...
### [156] [Active Learning and Explainable AI for Multi-Objective Optimization of Spin Coated Polymers](https://arxiv.org/abs/2509.08988)
*Brendan Young,Brendan Alvey,Andreas Werbrouck,Will Murphy,James Keller,Mattias J. Young,Matthew Maschmann*

Main category: cs.LG

TL;DR: 提出了一种优化聚合物薄膜旋涂工艺参数的框架，该框架集成了主动帕累托前沿学习算法（PyePAL）、可视化和可解释AI技术。


<details>
  <summary>Details</summary>
Motivation: 为了实现具有特定机械性能的聚合物薄膜，旋涂工艺参数的优化是一个多目标优化问题。

Method: 利用PyePAL算法，使用高斯过程模型预测目标值（硬度和弹性）与设计变量（旋转速度、稀释度和聚合物混合物）之间的关系，并结合UMAP降维可视化和模糊语言总结，增强优化结果的可解释性。

Result: 实验结果表明，该方法能够高效地识别有潜力的聚合物设计。

Conclusion: 该框架通过可视化和语言解释，促进专家驱动的分析和知识发现。
...
### [157] [Constructing a Question-Answering Simulator through the Distillation of LLMs](https://arxiv.org/abs/2509.09226)
*Haipeng Liu,Ting Long,Jing Fu*

Main category: cs.LG

TL;DR: 提出了一种名为LLM Distillation based Simulator (LDSim) 的方法，该方法从LLM中提取领域知识和推理能力以辅助预测，从而提高模拟性能。


<details>
  <summary>Details</summary>
Motivation: QA模拟器能够模拟真实学生的学习行为并预测他们回答问题的正确性，从而使教育推荐系统(ERS)能够收集大量的训练数据，而无需与真实学生互动，防止未充分训练的ERS做出有害的推荐，从而破坏实际的学生学习。

Method: 该方法名为LLM Distillation based Simulator (LDSim)，它从LLM中提取领域知识和推理能力，以更好地辅助预测。

Result: LDSim在模拟任务和知识追踪(KT)任务上都取得了很好的效果。

Conclusion: LDSim方法通过从LLM中提取领域知识和推理能力，能够有效提高模拟性能，并在模拟和知识追踪任务中表现出色。
...
TL;DR: 提出了一种名为近似最近邻注意力（ANNA）的有效注意力机制，具有亚二次时间复杂度。
...
Result: ANNA-transformer (1) 在匹配 MPC 算法的能力方面保留了先前为标准注意力建立的表达能力，并且 (2) 可以解决关键的推理任务，例如 Match2 和 $k$-hop，并具有接近最佳的深度。

Conclusion: 常数深度 ANNA-transformer 可以模拟常数深度低秩 transformer，从而为推理一类广泛的有效注意力近似提供了一种统一的方法。
...
TL;DR: 提出了 open-sci-ref，一系列在多个模型（0.13B 到 1.7B 参数）和 token 规模（高达 1T）上，基于 8 个最新的开放参考数据集训练的密集 Transformer 模型，作为研究基线。


<details>
  <summary>Details</summary>
Motivation: 为研究者提供参考基线，以便评估不同规模和数据集上替代训练方法的合理性和质量。

Method: 在 8 个开放参考数据集上训练不同规模的密集 Transformer 模型，并在各种标准化基准上评估这些模型。

Result: NemoTron-CC HQ 的训练效果优于其他参考数据集，其次是 DCLM-baseline 和 FineWeb-Edu。

Conclusion: 发布了中间训练检查点、日志、代码和下游评估，以简化复制、标准化比较并促进未来研究。
...
TL;DR: 提出了一种针对表格数据的上下文条件异常检测框架。


<details>
  <summary>Details</summary>
Motivation: 现有的无监督异常检测方法忽略了表格数据中异构上下文的影响，导致检测性能下降。

Method: 自动识别上下文特征，并使用深度自编码器对条件数据分布进行建模。

Result: 在多个表格基准数据集上的大量实验表明，该方法优于现有技术。
...
TL;DR: 提出了一种混合专家(MoWE)方法，通过优化组合现有模型的输出来克服数据驱动天气模型的局限性。


<details>
  <summary>Details</summary>
Motivation: 数据驱动的天气模型最近取得了最先进的性能，但近年来的进展已经停滞。

Method: 使用基于视觉Transformer的门控网络，动态学习在每个网格点对多个“专家”模型的贡献进行加权，并以预测提前期为条件。

Result: MoWE模型在均方根误差(RMSE)方面比任何单个组件都更准确，在2天预测范围内，比性能最佳的AI天气模型低10%的RMSE。

Conclusion: MoWE模型是一种计算效率高且可扩展的策略，可以通过充分利用领先的高质量预测模型来推动数据驱动天气预测的发展。
...
<details>
  <summary>Details</summary>
Motivation: 可再生能源和分布式能源的整合对传统电力系统保护方案提出了挑战。本文旨在综合近期文献，评估机器学习在电力系统保护中的应用。

Method: 本文遵循PRISMA for Scoping Reviews框架，对100多篇相关出版物进行了文献综述。

Result: 机器学习模型在模拟数据集上表现出高精度，但在实际条件下验证不足。现有文献分散，方法严谨性、数据集质量和评估指标不一致。本文提出了一个面向机器学习的保护任务分类法，解决了关键术语不一致问题，并倡导标准化报告规范。

Conclusion: 为了将基于机器学习的保护从理论走向实际应用，未来的研究应优先考虑公共基准数据集、实际验证方法和先进的机器学习架构。
...
TL;DR: STRIDE: A scalable XAI framework using orthogonal functional decomposition in RKHS to address limitations of feature subset reasoning and scalar summarization.


<details>
  <summary>Details</summary>
Motivation: Existing XAI frameworks suffer from exponential cost in feature subset reasoning and reduced expressiveness due to scalar value summarization.
...
Result: STRIDE achieves speedups (0.6x to 9.7x) compared to TreeSHAP on tabular datasets, with high fidelity (R^2: 0.81-0.999) and rank agreement.

Conclusion: STRIDE complements scalar attribution methods by providing a structured functional perspective and enabling diagnostics like 'component surgery' to measure interaction impacts.
...
TL;DR: 提出了一种名为 Rashomon Ensemble 的方法，该方法从多个高性能模型中策略性地选择模型，以提高泛化能力。


<details>
  <summary>Details</summary>
Motivation: 在机器学习中，选择泛化能力强的模型仍然是一项具有挑战性的任务。在实际场景中，由于数据中存在多种模式，通常会出现多个高性能解决方案。

Method: 通过根据模型的性能和解释对模型进行分组，构建在保持预测精度的同时最大化多样性的集成。

Result: 在开放和专有的协作真实世界数据集上验证了该方法，在 Rashomon 比率较大的情况下，AUROC 提高了 0.20+。

Conclusion: 证明了该方法在各种实际应用中对企业的实际好处，突出了该方法的稳健性、实用性和有效性。
...
TL;DR: 本文研究了深度线性网络 (DLN) 的黎曼几何，作为学习过程热力学描述的基础。


<details>
  <summary>Details</summary>
Motivation: 使用群作用分析过度参数化，并使用从参数空间到可观察空间的黎曼淹没。

Method: 利用群轨道对参数空间中平衡流形的叶状结构进行定义和计算玻尔兹曼熵。使用雅可比矩阵理论显式构造平衡流形切空间的标准正交基。

Result: 证明了[2]中定义的可观察空间上的黎曼几何是通过平衡流形的黎曼淹没得到的。

Conclusion: 主要技术步骤是使用雅可比矩阵理论显式构造平衡流形切空间的标准正交基。
...
<details>
  <summary>Details</summary>
Motivation: 通用大语言模型适应特定任务具有挑战性，特别是在资源受限的环境中。现有的秩分配技术计算效率低、复杂且不稳定。

Method: 利用损失函数的二阶导数（Hessian 矩阵）来有效捕获权重敏感性，从而以最小的计算开销实现最佳秩分配。
...
TL;DR: 提出了一种因果感知的深度学习框架，用于识别和选择有因果关系的特征，以提高预测精度和可解释性。
...
Method: 该方法集成了多元格兰杰因果关系（MVGC）和PCMCI+，用于混合神经架构中的因果特征选择。利用43年（1979-2021）的北极海冰范围（SIE）数据和相关的海洋-大气变量（每日和每月分辨率）。

Result: 实验结果表明，结合因果输入可以提高不同提前期的预测精度和可解释性。

Conclusion: 该框架具有广泛的适用性，提供了一种可扩展的方法，既能提升因果信息预测建模的理论基础，又能提高实际性能。
...
TL;DR: 本研究提出了一种连续时间多智能体强化学习框架（CT-MARL），该框架使用物理信息神经网络（PINN）来近似基于HJB的价值函数。


<details>
  <summary>Details</summary>
Motivation: 现有的强化学习方法难以处理需要高频率或不规则时间间隔交互的复杂动态系统，而连续时间强化学习（CTRL）在多智能体领域应用受限。

Method: 该方法通过引入价值梯度迭代（VGI）模块，对齐价值学习与价值梯度学习，迭代地优化轨迹上的价值梯度。

Result: 该方法在包括多智能体粒子环境（MPE）和多智能体MuJoCo在内的连续时间基准测试中，始终优于现有的连续时间RL基线，并且可以扩展到复杂的多智能体动态。

Conclusion: 该研究提出的方法能够有效地解决连续时间多智能体强化学习中的挑战，并在复杂动态环境中表现出优越的性能。
...
TL;DR: 这篇论文提出了一种使用机器学习模型预测ISP之间是否应该进行对等互连的方法，以提高全球互联网生态系统的效率。


<details>
  <summary>Details</summary>
Motivation: 互联网服务提供商需要与其他ISP连接以提供全球连接服务，对等互连比使用传输服务更受欢迎，但过程复杂。自动化对等伙伴选择可以提高效率。

Method: 利用公开数据，评估了三种机器学习模型（基于树、神经网络和Transformer）在预测对等关系方面的性能。其中，基于树的模型表现出最高的准确性和效率。XGBoost模型在预测对等伙伴关系方面达到了98%的准确率。

Result: XGBoost模型在预测对等伙伴关系方面表现出98%的准确率，并且对时间、空间和缺失数据的变化具有很强的适应性。

Conclusion: 该方法可以帮助ISP完全自动化对等伙伴的选择过程，从而转型到更高效和优化的互联网生态系统。
...
TL;DR: 提出了一个用于检测和归因西班牙语合成语音的大型数据集 HISPASpoof。


<details>
  <summary>Details</summary>
Motivation: 现有的语音取证技术主要集中在英语和中文，而西班牙语作为一种全球使用广泛的语言，在这方面的研究相对不足，可能导致语音合成技术被滥用。

Method: 构建了一个包含六种口音的真实西班牙语语音和六种零样本 TTS 系统生成的合成语音的大型数据集 HISPASpoof，并评估了五种代表性方法。

Result: 结果表明，在英语上训练的检测器无法泛化到西班牙语，而使用 HISPASpoof 进行训练可以显著提高检测性能。同时，也评估了 HISPASpoof 在合成语音归因方面的性能。

Conclusion: HISPASpoof 为推进可靠和包容性的西班牙语语音取证提供了一个关键的基准。
...
TL;DR: 提出了一种无需训练的自适应token合并框架，用于减少视觉Transformer的推理时间和传输资源使用。


<details>
  <summary>Details</summary>
Motivation: 大规模Transformer模型在语义通信系统中表现出色，但其巨大的计算需求阻碍了在资源受限的6G网络中的实际部署。

Method: 将每层合并比例的选择建模为多目标优化问题，并采用基于高斯过程的贝叶斯优化来构建最优配置的帕累托前沿。

Result: 该方法在保持竞争力的准确性的同时，显著减少了浮点运算，并且自适应策略可以根据信道质量调整合并程度。
...
TL;DR: 该论文将量子神经网络和深度强化学习结合，用于美元/新台币的金融交易。


<details>
  <summary>Details</summary>
Motivation: 探索量子启发神经网络和深度强化学习在金融交易中的应用前景。

Method: 使用量子长短期记忆网络（QLSTM）进行短期趋势预测，并结合量子异步优势行动者-评论家算法（QA3C）。

Result: 该长线交易agent在约5年内实现了11.87%的回报率，最大回撤为0.92%，优于一些货币ETF。

Conclusion: 混合模型在外汇交易中表现出竞争优势，QLSTM在小利润、严格风险控制的交易中有效。
...
TL;DR: FSPO: 通过在重要性采样权重空间中实施长度公平裁剪，用于LLM的序列级别强化学习方法。


<details>
  <summary>Details</summary>
Motivation: PPO/GRPO风格的裁剪移植到序列时存在不匹配：固定的裁剪范围系统地重新调整短与长响应的权重，从而扭曲了有效目标。

Method: FSPO：裁剪序列log-IS比率时，采用应用了KL校正漂移项并按$\\sqrt{L}$缩放的带。

Result: FSPO降低了跨长度箱的裁剪率，稳定了训练，并且在多个评估数据集中优于所有基线。

Conclusion: FSPO是一种有效的序列级别强化学习方法，可以在训练LLM时提高长度公平性、稳定训练并提高性能。
...
TL;DR: 提出了DART模型，用于将粗糙的大气预测转换为高分辨率卫星亮度温度场，以优化极端对流检测。


<details>
  <summary>Details</summary>
Motivation: 现有的深度学习天气模型评估指标存在“统计相似性陷阱”，奖励模糊预测，而忽略了罕见的高影响事件。

Method: DART采用双解码器架构，具有显式的背景/极端分解、物理驱动的过采样和特定任务的损失函数。

Result: DART在极端对流检测方面优于现有模型，并在实际案例中得到验证。发现移除综合水汽输送（IVT）反而能提高极端对流检测。

Conclusion: DART框架能够实现精确的操作校准，训练速度快，并能与现有的气象工作流程无缝集成，为极端天气准备提供了一条可靠的AI途径。
...
TL;DR: 本文提出了一种新的约束强化学习方法，通过引入自适应激励机制和渐进式惩罚来平衡奖励最大化和约束满足之间的权衡，提高训练稳定性。


<details>
  <summary>Details</summary>
Motivation: 在连续控制环境中，约束强化学习需要在满足约束条件的同时最大化回报，这是一个挑战。现有的策略优化方法在约束边界附近不稳定，导致训练效果不佳。

Method: 提出了一种名为增量惩罚近端策略优化（IP3O）的算法，该算法通过逐步增加惩罚来稳定训练动态。

Result: 在基准环境中的实验结果表明，IP3O算法优于现有的安全强化学习算法。

Conclusion: 该研究提出了一种有效的约束强化学习算法，并通过理论推导证明了该算法的最优性误差界限。
...
TL;DR: 本研究旨在确定促进农业旅游增长的重要指标，利用机器学习方法进行特征选择。


<details>
  <summary>Details</summary>
Motivation: 农业旅游作为一种战略经济模式，旨在通过实现当地社区（如农民）的收入来源多样化，同时促进土著文化遗产和传统农业实践的保护，从而促进农村发展。作为旅游业中一个非常繁荣的子领域，有必要详细研究农业旅游业的增长战略。

Method: 该研究分两个阶段进行：第一阶段通过全面的文献综述确定重要指标；第二阶段使用最先进的技术来识别促进农业旅游增长的重要指标。应用机器学习模型进行特征选择，观察到最小绝对收缩和选择算子（LASSO）方法与机器学习分类器（如 Logistic 回归（LR）、决策树（DT）、随机森林（RF）树和极限梯度提升（XGBOOST）模型）相结合，可用于预测农业旅游的增长。

Result: 结果表明，使用 LASSO 方法，LR 模型在 70-30% 的训练-测试数据中给出了 98% 的最高分类精度，其次是 RF，精度为 95%。同样，在 80-20% 的训练-测试数据中，LR 保持 99% 的最高精度，而 DT 和 XGBoost 的精度为 97%。

Conclusion: 该研究成功地利用机器学习方法识别出促进农业旅游增长的重要指标，并验证了 LASSO 与 LR 模型结合的有效性。
...
TL;DR: Vejde是一个结合数据抽象、图神经网络和强化学习的框架，用于解决具有丰富结构化状态的决策问题。


<details>
  <summary>Details</summary>
Motivation: 解决具有丰富结构化状态的决策问题，例如对象类和关系。

Method: 将MDP状态表示为实体事实数据库，Vejde将每个状态转换为二分图，并通过神经消息传递映射到潜在状态。

Result: Vejde策略在未见过的测试实例上具有良好的泛化能力，且得分接近于特定实例的MLP代理。

Conclusion: Vejde框架能够有效地学习和泛化策略，适用于解决具有复杂结构化状态的决策问题。
...
<details>
  <summary>Details</summary>
Motivation: 在实际工业应用中，获取足够的标签样本数据具有挑战性且成本高昂。不同类型的旋转机械设备具有不同的机械特性，需要对每种情况分别训练诊断模型。

Method: 该框架集成了时频域编码器和元学习泛化模型。时频域编码器预测通过时频域中的随机增强生成的状态表示。然后将这些增强的数据输入到元学习网络中进行分类和泛化训练，然后使用少量的标记数据进行微调。使用少量对比学习迭代来迭代优化模型。

Result: 在轴承故障数据集和转子试验台数据上进行的实验表明，MMT-FD模型仅使用1%的标记样本数据即可达到99%的故障诊断准确率，具有很强的泛化能力。

Conclusion: MMT-FD模型在小样本无监督旋转机械故障诊断中表现出高效性和准确性，并具有强大的泛化能力。
...
TL;DR: 这篇论文提出了一种名为熵调制策略梯度（EMPG）的框架，用于解决大型语言模型（LLM）在长时程任务中由于稀疏奖励而难以进行信用分配的问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在长时程任务中面临稀疏奖励的挑战，难以将信用分配给中间步骤。现有的方法主要集中在创建密集奖励信号来指导学习。

Method: 论文提出了熵调制策略梯度（EMPG），该框架基于逐步不确定性和最终任务结果重新校准学习信号。EMPG放大对自信正确行为的更新，惩罚自信错误，并衰减来自不确定步骤的更新，以稳定探索。此外，还引入了未来清晰度的奖励项，鼓励智能体找到更可预测的解决方案路径。

Result: 在WebShop、ALFWorld和Deep Search三个具有挑战性的智能体任务上进行的综合实验表明，EMPG实现了显著的性能提升，并显著优于强大的策略梯度基线。

Conclusion: 论文提出的EMPG框架能够有效解决LLM在长时程任务中由于稀疏奖励而难以进行信用分配的问题，并在多个任务上取得了显著的性能提升。
...
TL;DR: 本文提出了一种数据驱动的方法，用于从观察到的数据中学习反应扩散系统的 Soft ALife 模型，并在噪声和稀疏数据下表现出良好的预测精度。


<details>
  <summary>Details</summary>
Motivation: 在没有先验知识的情况下，进行反应扩散系统的系统识别是一项挑战。

Method: 使用 Data-driven Rulesets for Soft Artificial Life (DRSALife) 模型学习 Soft ALife 规则集。

Result: 该模型在预测新兴动态方面表现出良好的准确性 (74%)，并且在受到高斯噪声和时间稀疏性时表现出相当稳健的性能。成功识别了代表这些动态的底层偏微分方程 (PDE) 的结构和参数。

Conclusion: 该研究探索了基于机器的 Soft ALife 规则集学习和反应扩散动力学的系统识别，无需任何关于底层物理学的先验知识。
...
TL;DR: 本文提出了一个名为混合子图专家 (MoSE) 的新框架，用于灵活和富有表现力的基于子图的表示学习，以解决 GNN 在捕获复杂子图模式方面的局限性。


<details>
  <summary>Details</summary>
Motivation: GNN 依赖于局部成对消息传递，限制了它们捕获复杂、高阶子图模式的能力，导致结构表达性不足。现有方法要么是为图级别任务设计的，要么内核配置固定，缺乏灵活性。

Method: MoSE 通过匿名游走提取信息丰富的子图，并根据结构语义将它们动态路由到专门的专家，从而能够灵活且可解释地捕获不同的子图模式。

Result: 大量实验表明，MoSE 不仅优于有竞争力的基线，而且还提供了对模型学习的结构模式的可解释的见解。

Conclusion: MoSE 在 Subgraph Weisfeiler-Lehman (SWL) 测试中具有更强大的表达能力，并且通过可视化学习的子图专家，证明了其有效性和可解释性。
...
TL;DR: 本文提出了一种新的计算HGR相关系数的方法，该方法基于用户可配置的多项式核，与以前的方法相比，具有更强的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 在约束机器学习应用中，HGR相关系数可作为损失正则化项，但其固有的不可计算性需要在偏差-方差之间进行权衡，这可能会影响所提出方法的鲁棒性。

Method: 提出了一种新的基于用户可配置多项式核的HGR计算方法。

Result: 该方法在鲁棒性和确定性方面具有显著优势，并且计算出的次梯度可以作为损失正则化项。

Conclusion: 该方法更可靠，适用于实际应用，并通过实验验证了其在约束机器学习框架中的适用性。
...
TL;DR: MetaLLMiX是一个零样本超参数优化框架，它结合了元学习、可解释AI和高效的LLM推理。


<details>
  <summary>Details</summary>
Motivation: 在深度学习中，有效的模型和超参数选择仍然是一个主要的挑战，通常需要广泛的专业知识和计算。虽然AutoML和大型语言模型（LLM）承诺自动化，但目前基于LLM的方法依赖于反复试验和昂贵的API，这提供了有限的可解释性和泛化性。

Method: 利用带有SHAP解释的历史实验结果，MetaLLMiX推荐最佳超参数和预训练模型，而无需额外的试验。我们进一步采用LLM作为判断评估来控制输出格式、准确性和完整性。

Result: 在八个医学影像数据集上使用九个开源轻量级LLM的实验表明，MetaLLMiX实现了与传统HPO方法相当或更好的性能，同时大大降低了计算成本。我们的本地部署优于先前的基于API的方法，在8个任务中的5个任务上实现了最佳结果，响应时间减少了99.6-99.9%，并且在6个数据集上实现了最快的训练时间（快2.4-15.7倍），同时保持了准确率在最佳性能基线的1-5%以内。

Conclusion: MetaLLMiX在减少计算成本的同时，实现了与传统HPO方法相当或更好的性能。
...
TL;DR: 大型语言模型（LLM）通过生成反事实解释（SCE）来解释其决策，即通过修改输入以预测不同结果。研究表明，LLM生成的SCE通常有效但不简洁，无法提供深入的决策行为见解。当要求生成最小化反事实时，LLM的修改过小，无法改变预测。这种有效性与最小化之间的权衡在不同的LLM、数据集和评估设置中是一致的。SCE作为一种解释工具效果不佳，甚至可能误导模型行为的认知。


<details>
  <summary>Details</summary>
Motivation: 研究语言模型以自然语言解释其决策的能力，特别关注自生成的反事实解释（SCE）。

Method: 通过评估LLM生成SCE的有效性（达到预期结果）和最小性（修改输入的程度）来研究其性能。在多个LLM、数据集和评估设置中进行实验。

Result: LLM通常生成有效的SCE，但不够简洁。当要求生成最小化反事实时，LLM的修改过小，无法改变预测。观察到有效性与最小化之间的权衡。

Conclusion: SCE作为一种解释工具效果不佳，甚至可能误导模型行为。在高风险环境中使用LLM时，必须考虑不可靠的自我解释对下游决策的影响。
...
TL;DR: 提出了一种混合框架，通过普通克里金法工程化“空间滞后”特征，利用空间背景丰富 ML，称为“克里金先验回归” (KpR)。


<details>
  <summary>Details</summary>
Motivation: 机器学习和地统计学是用于预测和空间mapping土壤性质的两种根本不同的框架。地统计学利用土壤性质的空间结构，而机器学习则捕获可用的环境特征与土壤性质之间的关系。

Method: 使用来自 LimeSoDa 的六个fieldscale数据集，使用 TabPFN 模型评估 KpR 的点和概率预测性能。这些数据集包括土壤有机碳、粘土含量和 pH 值，以及来自遥感和原位近端土壤传感的特征。

Result: 与几种其他空间技术（例如，使用 TabPFN 的回归/残差克里金法）以及已建立的非空间机器学习算法（例如，随机森林）相比，使用 TabPFN 的 KpR 显示出可靠的不确定性估计和更准确的预测。

Conclusion: 使用 TabPFN 的 KpR 是一种非常稳健且通用的建模框架，适用于精准农业中的数字土壤mapping。
...
TL;DR: 本文提出了一种新的微生物群落网络推断算法fuser，它可以生成不同的、特定于环境的预测网络。


<details>
  <summary>Details</summary>
Motivation: 之前的研究通常将来自不同环境的样本组合在一起，没有充分考虑到微生物群落在面对不同的生态条件时如何调整它们的关联。本文旨在研究微生物群落的时空动态。

Method: 本文提出了一种名为fuser的算法，并使用Same-All Cross-validation (SAC)框架评估算法在预测微生物关联方面的性能。

Result: fuser在同质环境中的预测性能与现有算法glmnet相当，并且在跨环境场景中显著降低了测试误差。

Conclusion: fuser算法能够更好地预测微生物群落在不同环境下的关联。
...
TL;DR: 提出了一种可控的分子图生成模型CSGD，该模型在多条件设置下表现出色。


<details>
  <summary>Details</summary>
Motivation: 在材料和药物发现中，生成满足各种属性约束的分子至关重要。现有的图扩散模型在多条件设置下的有效性有限。

Method: 通过具体分数将分数匹配扩展到离散图，并引入了两种基于分数的技术：可组合引导（CoG）和概率校准（PC）。

Result: 在四个分子数据集上的实验结果表明，CSGD实现了最先进的性能，可控性比现有方法平均提高了15.3%，同时保持了较高的有效性和分布保真度。

Conclusion: 基于分数的建模在离散图生成方面具有实际优势，并且能够灵活地进行多属性分子设计。
...
TL;DR: 提出了一种新的深度学习模型AquaCast，用于预测城市水动力学。


<details>
  <summary>Details</summary>
Motivation: 解决预测城市水动力学的挑战，传统方法难以捕捉变量间和时间上的依赖性。

Method: 开发了一个多输入多输出的深度学习模型，融合内生变量（如水位或流量）和外生因素（如降水历史和预报）。

Result: 在LausanneCity数据集上取得了state-of-the-art的性能，并且通过在三个大型合成数据集上的测试，证明了模型的泛化性和可扩展性。

Conclusion: AquaCast模型在真实和合成数据集上均优于现有基线，保持了稳健和准确的预测。
...
TL;DR: 本文介绍了一种自动系统，该系统可以从会议论文集中识别特定地区的论文，然后执行机器人流程自动化（RPA）以完成预定义的操作，例如提交提名表。


<details>
  <summary>Details</summary>
Motivation: 为了解决学术发现中耗时的人工工作。

Method: 使用名为“Agent-E”的专用AI代理，可以识别会议论文集中特定地区的论文，然后执行机器人流程自动化（RPA）以完成预定义的操作。

Result: 在来自五个不同会议的 586 篇论文上验证了系统，该系统成功识别了每篇目标论文，召回率为 100%，准确率接近 99.4%。

Conclusion: 面向任务的 AI 代理不仅可以过滤信息，还可以积极参与并加速学术界的工作流程。
...
TL;DR: 本文提出了一种基于时间规则的完全可解释的方法，用于时间知识图谱（TKG）预测。


<details>
  <summary>Details</summary>
Motivation: 受最近使用循环事实提出强基线的工作的启发。

Method: 我们的方法学习四种简单的规则，并使用考虑最近性和频率的置信度函数。

Result: 在九个数据集上进行评估，我们的方法匹配或超过了八个最先进的模型和两个基线的性能。

Conclusion: 同时提供完全可解释的预测。
...
TL;DR: 提出了一个名为D2P2-SGD的优化器，它结合了动态差分隐私和随机投影技术，以在保护隐私的同时提高模型学习的效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 现有的模型参数和梯度信息容易泄露隐私，DPSGD的静态噪声机制影响模型性能的误差范围，以及模型参数的指数增长使得使用随机优化器进行有效学习变得更具挑战性。

Method: 结合动态差分隐私（DDP）与自动梯度裁剪和随机投影与SGD。

Result: 在不同数据集上的大量实验表明，D2P2-SGD显著提高了准确性，同时保持了隐私。

Conclusion: D2P2-SGD优化器在不同目标函数上表现出可证明的次线性收敛速度，与现有的最佳速度相匹配。理论分析表明，DDP以隐私为代价实现了更好的效用，而随机投影能够实现更有效的模型学习。
...
TL;DR: 提出了PIPES，一个包含多个pipeline的实验集合，旨在解决OpenML在pipeline多样性和完整性方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有的OpenML在pipeline多样性方面存在局限性，特别是在数据预处理步骤上，导致表示不完整，且实验集中在少数流行技术上，导致样本不平衡。

Method: 构建PIPES，一个包含9,408个pipeline在300个数据集上运行的实验集合，代表所选技术集合的所有组合，旨在实现多样性和完整性。

Result: PIPES存储了pipeline块、训练和测试时间、预测、性能和错误信息的详细信息，允许研究人员对不同的pipeline和数据集进行分析。

Conclusion: PIPES通过提供全面的实验结果集合，为元学习社区提供了支持，并具有扩展潜力。
...
TL;DR: 本研究探讨了少样本学习在呼吸音分类中的有效性，特别是基于咳嗽声检测 COVID-19、流感和健康状况。


<details>
  <summary>Details</summary>
Motivation: 在标记数据有限的情况下，研究少样本学习是否能使模型达到与传统深度学习方法相当的性能。

Method: 利用带有咳嗽声音频谱图表示的原型网络。

Result: 少样本学习模型可以获得具有竞争力的准确率。在每类只有 15 个支持样本的情况下，多类分类达到 74.87% 的准确率，而二元分类在所有类别对中达到 70% 以上的准确率。统计测试表明，二元模型和多类模型之间没有显著的性能差异。

Conclusion: 研究结果强调了在医疗诊断中应用少样本学习的可行性，尤其是在无法获得大量标记数据集时。
...
TL;DR: This paper introduces ButterflyQuant, a novel quantization method for large language models that uses learnable butterfly transforms to eliminate outliers before quantization, achieving better performance than existing methods like QuaRot.


<details>
  <summary>Details</summary>
Motivation: Extreme 2-bit quantization suffers from catastrophic performance loss due to outliers in activations. Existing rotation-based methods use fixed transforms that cannot adapt to specific weight distributions.

Method: The paper proposes ButterflyQuant, which replaces Hadamard rotations with learnable butterfly transforms parameterized by continuous Givens rotation angles. It also introduces a uniformity regularization on post-transformation activations.

Result: On LLaMA-2-7B with 2-bit quantization, ButterflyQuant achieves 15.4 perplexity versus 22.1 for QuaRot.

Conclusion: ButterflyQuant achieves better performance than existing methods by using layer-adaptive rotations and a uniformity regularization, while maintaining computational efficiency and theoretical guarantees.
...
### [195] [ProDiGy: Proximity- and Dissimilarity-Based Byzantine-Robust Federated Learning](https://arxiv.org/abs/2509.09534)
*Sena Ergisi,Luis Maßny,Rawad Bitar*

Main category: cs.LG

TL;DR: 提出了一种新的拜占庭鲁棒联邦学习算法ProDiGy，即使在数据异构的情况下，也能有效防御对抗性攻击。


<details>
  <summary>Details</summary>
Motivation: 联邦学习容易受到对抗性攻击，尤其是在数据异构的情况下。

Method: 使用基于梯度邻近性和差异性的联合双重评分系统来评估客户端梯度。

Result: ProDiGy在各种场景下优于现有的防御方法，尤其是在客户端数据不符合独立同分布时，ProDiGy保持了强大的防御能力和模型精度。

Conclusion: 双重视角方法可以促进诚实客户端之间的自然相似性，同时检测可疑的均匀性，以此作为潜在攻击的指标，证明了该方法的有效性。
...
TL;DR: 提出了一种新的图对齐框架，该框架同时增强了节点区分性并强制执行潜在空间中的几何一致性。


<details>
  <summary>Details</summary>
Motivation: 现有方法由于 GNN 嵌入中的过度平滑导致节点区分性降低，并且由于结构噪声、特征异构性和训练不稳定性导致潜在空间错位，从而导致不可靠的节点对应关系。

Method: 引入了一个双通道编码器，它结合了低通和高通频谱滤波器来生成既具有结构感知能力又具有高度区分性的嵌入。为了解决潜在空间错位问题，我们结合了一个几何感知功能映射模块，该模块学习图嵌入之间的双射和等距变换，确保不同表示之间的一致几何关系。

Result: 在图基准测试中进行的大量实验表明，我们的方法始终优于现有的无监督对齐基线，对结构不一致性和具有挑战性的对齐场景表现出卓越的鲁棒性。

Conclusion: 我们的框架有效地概括了图领域，从而能够对视觉和语言表示进行无监督对齐。
...
TL;DR: 提出了一种深度学习模拟器，用于随机和混沌时空系统，该模拟器以底层偏微分方程 (PDE) 的参数值为条件。


<details>
  <summary>Details</summary>
Motivation: 传统数值积分计算成本高昂，为了能够高效探索参数空间，并对罕见事件进行统计研究。

Method: 该方法包括在单个参数域上预训练模型，然后在较小的多样化数据集上进行微调，从而实现跨广泛参数值的泛化。通过结合局部注意力机制，该网络能够处理不同的域大小和分辨率。这使得在较小域上进行计算效率高的预训练成为可能，同时只需要少量额外数据集来学习如何推广到较大的域大小。

Result: 在混沌 Kuramoto-Sivashinsky 方程和随机强制 beta 平面湍流上验证了模型的性能，展示了其捕获内插参数值处现象的能力。该模拟器比传统数值积分提供了显着的计算加速。

Conclusion: 该模拟器能够高效探索参数空间，并且模拟器的概率变体提供了不确定性量化，从而可以对罕见事件进行统计研究。
...
TL;DR: 提出了一种新的数据稀疏算子学习算法ReBaNO，用于求解具有多个不同输入的偏微分方程组。


<details>
  <summary>Details</summary>
Motivation: 受降阶基方法和生成式预训练物理信息神经网络的启发，ReBaNO依靠数学上严格的贪婪算法从头开始自适应地离线构建其网络结构。

Method: 通过任务特定的激活函数进行知识蒸馏，使ReBaNO具有紧凑的架构，在线需要最小的计算成本，同时嵌入物理信息。

Result: 数值结果表明，与PCA-Net、DeepONet、FNO和CNO等最先进的算子学习算法相比，ReBaNO在消除/缩小分布内和分布外测试的泛化差距方面明显优于它们，并且是唯一实现严格离散化不变性的算子学习算法。

Conclusion: ReBaNO算法在泛化性和离散化不变性方面表现出色，是一种有竞争力的算子学习方法。
...
TL;DR: 本文提出了一种通过分析基于组的反事实解释 (GCE) 的时间演变来解释概念漂移的新方法。


<details>
  <summary>Details</summary>
Motivation: 在动态环境中，机器学习模型经常受到概念漂移的影响，其中数据分布的变化会降低性能。虽然检测这种漂移是一个经过充分研究的主题，但解释模型决策逻辑如何以及为何变化仍然是一个重大挑战。

Method: 该方法跟踪漂移前后 GCE 集群中心及其相关的反事实行动向量的变化。这些不断演变的 GCE 充当可解释的代理，揭示模型决策边界及其底层基本原理的结构变化。在三层框架中实施这种分析，该框架协同结合来自数据层（分布变化）、模型层（预测不一致）和解释层（本文提出的）的见解。

Result: 这种整体视图可以更全面地诊断漂移，从而可以区分不同的根本原因，例如空间数据漂移与概念的重新标记。

Conclusion: 通过分析 GCE 的时间演变来解释概念漂移。
...
TL;DR: 提出了一种新的分子表示方法，即功能组表示 (FGR)，以提高深度学习分子性质预测模型的可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有的深度学习分子性质预测模型缺乏可解释性，阻碍了化学家对其的采用。

Method: 该方法结合了来自化学知识的功能团 (FG) 和使用序列模式挖掘从大型分子语料库中挖掘的功能团 (MFG)，并将分子编码到低维潜在空间中。此外，该框架还允许包含分子的二维结构描述符。

Result: FGR 框架在涵盖物理化学、生物物理学、量子力学、生物活性和药代动力学的 33 个基准数据集上实现了最先进的性能，同时实现了化学可解释性。

Conclusion: 该研究朝着开发用于分子发现的高性能、化学可解释的深度学习模型迈出了重要一步。
...
TL;DR: 提出了一种名为 FG-FARL 的离线 RL 程序，用于校准每组安全阈值，以减少损害，同时平衡受保护亚群体的公平性目标。


<details>
  <summary>Details</summary>
Motivation: 旨在减少harm，同时平衡受保护亚群体的公平性目标。

Method: 使用来自 Medicaid 人口健康管理计划的去识别纵向轨迹，将 FG-FARL 与行为克隆 (BC) 和 HACO（混合自适应共形离线 RL；全局共形安全基线）进行评估。

Result: FG-FARL 在提高公平性指标的同时，实现了与基线相当的价值。

Conclusion: FG-FARL 为更安全、更公平的决策支持提供了一条切实可行的途径。
