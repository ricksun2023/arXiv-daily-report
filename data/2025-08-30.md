...
Motivation: 预训练的多语言模型表现出与处理英语文本的模型相同的社会偏见。

Method: 我们从语言多样性、文化意识以及他们选择的评估指标和缓解技术方面检查了这些研究。

Result: 我们的调查揭示了该领域主要方法设计选择中的差距（例如，偏爱某些语言，缺乏多语言缓解实验），同时编目了在跨语言和文化调整偏差基准时遇到的常见问题和实施的解决方案。

Conclusion: 鉴于我们的发现的影响，我们为未来的研究规划了方向，这些研究可以加强多语言偏差文献的包容性、跨文化适当性以及与最先进的自然语言处理进展的一致性。
...
Method: 比较微调后的中型模型 (Gemma, 2B) 与较大的未调优模型 (GPT-3.5, 175B)，并评估七种结构化提示策略。

Result: 结构化提示，特别是结合了思维链和顺序设计的策略，显著提高了 Gemma 的输出。Gemma 通常比 GPT-3.5 的零样本响应产生更多符合结构和教学上适当的项目。

Conclusion: 结构化提示和高效的微调可以增强中型模型在有限数据条件下进行 AIG 的能力。
...
<details>
  <summary>Details</summary>
Motivation: 小模型由于推理能力差，导致奖励稀疏和训练不稳定，难以进行强化学习。

Method: 提出Distillation-Guided Policy Optimization (DGPO)，通过模仿学习初始化和持续的教师指导来解决这些挑战。

Result: DGPO使小模型能够实现复杂的agentic搜索行为，在某些情况下甚至优于较大的教师模型。

Conclusion: DGPO使agentic RAG在计算资源受限的环境中可行。
...
<details>
  <summary>Details</summary>
Motivation: The motivation is that current government ethics guidelines for LLMs are too high-level and lack actionable testing questions to verify LLM compliance.

Method: GUARD uses automated generation of guideline-violating questions based on government-issued guidelines and integrates jailbreak diagnostics (GUARD-JD) to create scenarios that provoke unethical responses.

Result: The effectiveness of GUARD was validated on seven LLMs, including Vicuna-13B, LongChat-7B, Llama2-7B, Llama-3-8B, GPT-3.5, GPT-4, GPT-4o, and Claude-3.7, under three government-issued guidelines. GUARD-JD can also transfer jailbreak diagnostics to vision-language models.

Conclusion: GUARD is effective in promoting reliable LLM-based applications by delineating the extent of adherence and highlighting any violations of government-issued guidelines.
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在处理长上下文时面临内存限制、处理复杂任务的能力不足以及缺乏透明度和容易产生幻觉等问题。

Method: 该方法包括三个关键组成部分：概要提取、图构建和关系推理。利用蒙特卡洛树搜索（MCTS）帮助模型导航复杂的推理路径

Result: JERR在ROUGE和F1指标上始终优于所有基线，并在LLM-Rater评估中获得最高分。

Conclusion: JERR框架提供了一种新颖的解决方案，使LLM能够处理扩展的上下文和复杂的推理任务，同时提高了可靠性和透明度。
...
<details>
  <summary>Details</summary>
Motivation: 现有的Long CoT行为依赖于高质量的、人工构建的后训练数据集，成本高昂，缺乏可扩展性。

Method: 提出了一个两阶段的后训练框架：(1)在拒绝采样的NPH图实例上进行Long CoT监督微调(SFT)，以增强推理深度；(2)通过细粒度的奖励设计进行强化学习(RL)，以提高推理效率。

Result: 提出的Graph-R1-7B模型在数学、编码、STEM和逻辑等领域表现出强大的泛化能力，并且在NPH图问题上的准确性和推理效率均超过了QwQ-32B。

Conclusion: NPH图问题是提升LLM中Long CoT推理能力的一种有效且可扩展的资源，为LLM后训练开辟了新的前沿。
...
<details>
  <summary>Details</summary>
Motivation: 现有的心理测试方法在评估LLM时忽略了对话历史的影响，这与真实世界的应用场景不符。

Method: 通过构建CAPE框架，将先前的对话交互纳入考虑，并引入新的指标来量化LLM响应的一致性。

Result: 实验表明，对话历史可以通过上下文学习提高响应一致性，但也可能导致性格转变。不同的LLM对上下文的依赖程度和对问题顺序的敏感度不同。

Conclusion: CAPE框架可以有效评估LLM在上下文感知下的性格特征，并应用于角色扮演代理以提高其响应一致性和与人类判断的对齐。
...
<details>
  <summary>Details</summary>
Motivation: 目前关于推理效用如何影响最终答案的正确性的研究较少。由于自回归生成的随机性，生成更多上下文并不保证对答案的信心会增加。如果可以在生成过程中预测推理步骤是否有用，就可以提早停止或删除无效步骤，避免最终决策中的干扰。

Method: 该研究在MATH数据集上使用Qwen2.5-32B和GPT-4o生成推理链，然后使用单独的模型(Qwen3-8B)来量化这些链对最终准确性的效用。具体来说，研究使用条件熵(词汇表上的预期负对数似然)来衡量模型在每个推理步骤中答案范围Y的不确定性，并随着步骤的进行逐步扩展上下文。

Result: 条件熵随着步骤的减少与正确答案密切相关，而平稳或增加的熵通常会导致错误的答案。错误的推理路径往往比正确的推理路径更长，这表明更长的推理并不一定产生更好的结果。

Conclusion: 该研究结果可以为未来设计高效的推理管道提供基础，从而及早发现并避免无用的推理。
...
<details>
  <summary>Details</summary>
Motivation: To address the lack of rigorous verification of claims made by AI text-to-app tools regarding the quality of generated applications and websites.

Method: Expert pairwise comparison across 10 tools, 30 prompts, and 300 generated sites, using a TrueSkill-derived model for ranking.

Result: UI-Bench ranks systems with calibrated confidence intervals, establishing a reproducible standard.

Conclusion: UI-Bench provides a benchmark, including a prompt set, evaluation framework, and public leaderboard, for advancing AI-driven web design.
...
Method: A bilingual benchmark, DentalBench, is introduced, consisting of DentalQA (question-answering) and DentalCorpus (large-scale corpus). 14 LLMs are evaluated, and domain adaptation is performed using Qwen-2.5-3B.

Result: Significant performance gaps are found across task types and languages. Domain adaptation substantially improves model performance, especially on knowledge-intensive tasks.

Conclusion: Domain-specific benchmarks are crucial for developing trustworthy and effective LLMs tailored to healthcare applications.
...
<details>
  <summary>Details</summary>
Motivation: Improve retrieval in RAG systems by addressing context loss in complex queries.

Method: KG-CQR: Extracts and completes KG subgraphs to generate semantically rich query contexts. Model-agnostic pipeline.

Result: 4-6% mAP and 2-3% Recall@25 improvement on RAGBench and MultiHop-RAG datasets. Outperforms baselines in multi-hop QA.

Conclusion: KG-CQR improves retrieval effectiveness by incorporating KG-based query enrichment.
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM评估主要集中在数学和编码推理任务，缺乏对民用航空维护领域专业知识和复杂推理能力的评估。

Method: 构建了一个专门为民用航空维护设计的基准，并利用该基准评估了现有向量嵌入模型和LLM在民用航空维护场景中的性能。

Result: 实验结果证明了该基准在评估模型在该领域性能方面的有效性。

Conclusion: 该基准已开源，旨在促进民用航空维护领域LLM的进一步研究和开发。
...
<details>
  <summary>Details</summary>
Motivation: 为了更有效地搜索毕业设计题目，利用以往案例的经验。

Method: 采用CBR，使用TF-IDF处理题目向量化，余弦相似度计算相似度。

Result: 在705个题目上进行测试，结果显示该系统能够有效地检索题目，并且随机化标题后的检索结果与原标题检索结果数量相同，平均匹配度最高。

Conclusion: 该系统成功地将CBR应用于毕业设计题目搜索，并取得了良好的检索效果。
...
<details>
  <summary>Details</summary>
Motivation: 现有API基准不足以评估大型语言模型在复杂、多步骤任务中的工具使用、跨工具协调、精确参数控制和规划/推理能力。

Method: MCP-Bench通过提供一组协同工作的互补工具，模拟真实的多步骤任务，并测试模型从模糊指令中检索相关工具、规划多跳执行轨迹、根据中间工具输出进行响应以及协调跨领域工作流的能力。

Result: 对20个先进的大型语言模型的实验表明，MCP-Bench仍然存在挑战。

Conclusion: MCP-Bench的提出旨在弥补现有基准的不足，更全面地评估大型语言模型在复杂工具使用场景下的能力。
...
<details>
  <summary>Details</summary>
Motivation: 在 ICU 中，从电子健康记录 (EHR) 预测死亡率和资源利用率具有挑战性，但对于优化患者预后和管理成本至关重要。现有方法主要集中于结构化 EHR，通常忽略了自由文本中的宝贵临床见解。此外，结构化数据中文本信息的潜力尚未得到充分利用。

Method: 利用两个真实世界的 EHR 数据集，在三个临床任务上开发和评估了该模型，并与现有的领先方法进行了比较。 对框架中的三个关键组件进行了消融研究：医疗提示、自由文本和预训练句子编码器。 评估了模型针对结构化 EHR 中损坏的鲁棒性。

Result: 在两个真实世界数据集上进行的三个临床任务的实验表明，与现有最佳方法相比，所提出的模型在死亡率预测方面的 BACC/AUROC 提高了 1.6%/0.8%，LOS 预测方面的 RMSE/MAE 提高了 0.5%/2.2%，手术持续时间估计方面的 RMSE/MAE 提高了 10.9%/11.0%。

Conclusion: 所提出的框架是一种有效且准确的深度学习方法，用于预测重症监护中的死亡率和资源利用率。 该研究还强调了使用 prompt learning 和 transformer 编码器分析多模态 EHR 的成功之处。 该模型对结构化数据中的数据损坏表现出很强的弹性，尤其是在高损坏水平下。
...
<details>
  <summary>Details</summary>
Motivation: 随着人工智能生成虚假信息日益复杂，理解阴谋论内容中的修辞模式对于开发有针对性的干预措施非常重要。

Method: 本文构建了一个包含阴谋论文章片段的数据集ConspirED，并使用CONSPIR认知框架进行标注。同时，开发计算模型来识别阴谋论特征，并评估大型语言模型对阴谋论输入的鲁棒性。

Result: 研究发现，大型语言模型在面对阴谋论内容时会产生与输入推理模式相似的输出，即使它们能够成功地转移类似的事实核查的虚假信息。

Conclusion: 阴谋论内容与大型语言模型存在错位现象，这表明需要进一步研究和干预，以提高人工智能系统对阴谋论的抵御能力。
...
<details>
  <summary>Details</summary>
Motivation: 评估现代机器翻译系统的能力

Method: 人工评估和BLEU评分

Result: 翻译质量低于90%的标准，来源语句通常是特定领域的，并且在文化上偏向英语国家。简单的命名实体复制可以产生重要的BLEU分数，表明评估协议中的漏洞

Conclusion: 多语言MT基准应该使用领域通用和文化中立的源文本，减少对命名实体的依赖，以更好地反映现实世界的翻译挑战。
...
<details>
  <summary>Details</summary>
Motivation: 现有主题发现方法依赖于词嵌入，缺乏对科学出版物的全面理解，难以处理复杂的高维文本关系。

Method: 1. 构建文本编码器，捕获元数据、标题和摘要等内容。2. 构建空间优化模块，整合基于熵的采样和由 LLM 引导的三元组任务，增强对主题相关性和上下文复杂性的关注。3. 基于 LLM 的指导，通过优化三元组的对比损失来微调文本编码器。

Result: 在三个真实世界数据集上的实验表明，SciTopic 优于当前最先进的科学主题发现方法。

Conclusion: SciTopic 使研究人员能够获得更深入、更快速的见解。
...
<details>
  <summary>Details</summary>
Motivation: 促进生物医学语义索引和问答领域的进展。

Method: 组织BioASQ挑战赛，包含b和Synergy两个已建立的任务，以及MultiCardioNER和BIONNE两个新任务。

Result: 37个团队参与，总共提交了700多个不同的提交，大多数参与系统取得了有竞争力的表现。

Conclusion: 表明该领域的技术水平在不断提高。
...
<details>
  <summary>Details</summary>
Motivation: 促进生物医学语义索引和问答的进展。

Method: 组织六个不同的共享任务，包括多语言临床总结、嵌套命名实体链接、心脏病临床编码和肠脑相互作用信息提取。

Result: 83个团队参与，提交了1000多个不同的提交。

Conclusion: 多个参与系统取得了有竞争力的表现，表明该领域的技术水平在不断提高。
...
<details>
  <summary>Details</summary>
Motivation: 传统抽取式摘要模型在工业应用中占据主导地位，但存在多阶段管道引入累积信息损失和架构瓶颈，以及缺乏对用户查询和文档的语义理解的局限性。

Method: 该方法集成了大模型蒸馏、监督微调、直接偏好优化和前瞻解码，将一个只有0.1B参数的轻量级模型转化为领域专业的QDTS专家。

Result: 该模型在多个行业相关指标上优于生产基线，并达到新的技术水平。此外，它还展示了出色的部署效率，只需要334个NVIDIA L20 GPU，即可在每个查询55毫秒的平均延迟下处理约50,000个查询/秒。

Conclusion: 该研究表明，所提出的生成模型框架可以有效解决工业网络搜索中的实时QDTS问题，并在性能和效率方面均优于传统方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有的联邦学习方法在处理真实环境中非独立同分布（non-IID）数据时面临挑战，尤其是在自然语言处理中，忽略了语言领域的多样性。

Method: 提出了一个自适应联邦蒸馏（AdaFD）框架，以解决同构和异构设置中的多域非独立同分布挑战。

Result: 实验结果表明，该模型能够捕捉到本地客户端的多样性，并取得比现有方法更好的性能。

Conclusion: 本文提出了AdaFD框架和综合基准框架，有效解决了多域非独立同分布的挑战，并在实验中取得了优异的结果。
...
<details>
  <summary>Details</summary>
Motivation: 现有的小型语言模型在处理不平衡分类数据集中的少数类关系时能力有限，而大型语言模型则可能因为人工设计的提示引入噪声，干扰模型对事件间长距离依赖关系的判断。

Method: 1. 提出距离感知图结构，利用图注意力网络（GAT）辅助大型语言模型捕获长距离依赖特征。
2. 设计基于软推理的时间特征学习范式，通过将大型语言模型生成的概率信息补充到多头注意力机制中，增强对近距离邻近带关系的识别。

Result: 在TB-Dense和MATRES两个公开数据集上的实验表明，该方法达到了最先进的性能。

Conclusion: 该框架能够有效捕获全局特征，显著提高少数关系类的性能，并提高整体学习能力。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型(llm)存在幻觉和知识过时等局限性，而检索增强生成(RAG)是一种有效的解决方案。然而，由于中小企业资源有限且缺乏自然语言处理(NLP)方面的专业知识，因此在中小企业中部署基于rag的工具仍然是一个挑战。

Method: 本文介绍了一种名为EASI-RAG的方法，它基于方法工程原理，包含明确定义的角色、活动和技术。

Result: 通过在环境测试实验室的真实案例研究验证了该方法，在该案例中，RAG工具用于回答操作人员的问题，这些问题来自操作程序中提取的数据。该系统由一个没有RAG经验的团队在一个月内完成部署，并根据用户反馈进行了迭代改进。结果表明，EASI-RAG支持快速实现、高用户采用率，并提供准确的答案，并提高底层数据的可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究侧重于通过内容规划和不同的表达方式来使问题生成多样化，但是这些方法通常强调生成简单的问题，而忽略了基本知识的整合，例如文档中的相关句子。为了解决数据稀疏性问题，以往的研究侧重于通过内容规划和不同的表达方式来使问题生成多样化，这增加了语言模型学习虚假模式的可能性。

Method: KCS将知识组合选择建模为句子级条件预测任务，并利用概率对比损失来预测下一个最相关的知识片段。在推理过程中，我们采用随机解码策略来有效地平衡准确性和多样性。

Result: 与竞争基线相比，我们的KCS将知识组合选择的总体准确性提高了3.9％，并且其在数据增强方面的应用在HotpotQA和2WikiMultihopQA数据集上产生了改进。

Conclusion: KCS 提高了知识组合选择的准确性，并且在数据增强方面有所改进。
...
<details>
  <summary>Details</summary>
Motivation: 现有的图语言模型(GLM)评估基准不足以评估多模态推理

Method: 引入CLEGR基准，该基准采用合成图生成管道，并结合需要对结构和文本语义进行联合推理的问题。

Result: 经过软提示的LLM基线与包含完整GNN主干的GLM性能相当。GLM在需要结构推理的任务中表现出显著的性能下降。

Conclusion: 目前的GLM在图推理能力方面存在局限性，为推动社区朝着涉及图结构和语言的显式多模态推理发展奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有的命名实体纠正模型主要依赖音标编辑距离算法，但在错误转录词与真实实体差异较大时，难以定位错误转录词。

Method: 利用语音特征检索候选实体，并设计了一种生成方法来注释和纠正ASR文本中的实体错误。

Result: 在开源和自建测试集上进行了测试，结果表明该NEC方法可以显著提高实体准确率。

Conclusion: 该方法在词形差异显著的情况下有效，并开放了自建测试集和训练数据。
...
<details>
  <summary>Details</summary>
Motivation: 该模型旨在解决跨多种语言识别隐式话语关系的问题。

Method: 该模型名为 HArch，利用话语意义之间的层级依赖关系，预测 PDTB 3.0 框架中所有三个意义层级的概率分布。使用了 RoBERTa 和 XLM-RoBERTa 等预训练编码器作为骨干网络。

Result: RoBERTa-HArch 在英语中表现最佳，而 XLM-RoBERTa-HArch 在多语言环境中表现最佳。微调后的模型优于 GPT-4o 和 Llama-4-Maverick 等大型语言模型。在 DiscoGeM 1.0 语料库上取得了 SOTA 结果。

Conclusion: 该研究结果表明，在 IDRR 任务中，特定任务的微调优于提示学习，并验证了分层方法的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型显著提高了文本生成的能力和效率，但也带来了恶意滥用的风险，因此需要水印技术作为保障。分词不一致（TI）会削弱隐写术和水印的鲁棒性。

Method: 针对TI的两个关键特征：低频性和临时性，提出了两种定制解决方案：隐写术的逐步验证方法和水印的后验回滚方法。

Result: 实验表明，对于隐写术，直接解决TI可以提高流畅性、不可感知性和反隐写分析能力；对于水印，解决TI可以提高可检测性和抵抗攻击的鲁棒性。

Conclusion: 本文通过解决分词不一致问题，提高了隐写术和水印技术的性能和安全性。
...
<details>
  <summary>Details</summary>
Motivation: To create a model that can reason effectively in a code environment and solve complex problems.

Method: Agentic reinforcement learning with innovations like an efficient RL infrastructure, GRPO-RoC algorithm, and an efficient agent training recipe.

Result: Achieves state-of-the-art results on AIME24 and AIME25 with significantly shorter responses than DeepSeek-R1 (671B).
...
<details>
  <summary>Details</summary>
Motivation: 现有的自然语言处理中的差分隐私 (DP) 方法通常需要在高 ε 值下才能合理地对文本进行私有化。为了解决这个问题。

Method: DP-ST 采用分而治之的策略，并将 DP 概念限制在私有化邻域内。结合 LLM 后处理。

Result: 该方法在较低的 ε 值下也能生成连贯的文本，同时平衡隐私和效用。

Conclusion: 研究结果表明，在合理的 ε 水平下，连贯性对于实现平衡的私有化输出至关重要。
...
<details>
  <summary>Details</summary>
Motivation: 隐性仇恨言论(IHS)难以检测，因为它不包含明确的贬损或煽动性词语。

Method: 通过微调基于大型语言模型(LLM)的通用嵌入模型，如Stella、Jasper、NV-Embed和E5。

Result: 在多个IHS数据集上的实验表明，在数据集内评估中，F1-macro得分提高了1.10个百分点，在跨数据集评估中，F1-macro得分提高了20.35个百分点。

Conclusion: 该方法仅通过微调通用嵌入模型就能在隐性仇恨言论检测方面实现最先进的性能。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在开放式文本生成中，需要在连贯性和多样性之间取得平衡。对比搜索解码策略受超参数影响大，计算成本高。

Method: 提出了一种自适应解码方法GUARD，它结合了全局熵估计和局部熵偏差，集成长短期不确定性信号。同时，引入基于令牌计数的惩罚来减少计算开销。

Result: 实验结果表明，GUARD在文本多样性和连贯性之间取得了良好的平衡，并在生成速度上有了显著提高。人工和LLM评估器都验证了其卓越的性能。

Conclusion: GUARD 是一种有效的解码方法，可以在文本连贯性和多样性之间取得平衡，并提高了生成速度。
...
<details>
  <summary>Details</summary>
Motivation: 探讨大型语言模型生成的合成治疗对话是否能捕捉到真实治疗中细微的情感动态，以及当前LLM生成治疗数据的局限性。

Method: 调整话语情感动态框架，以分析跨价、唤醒和优势维度的细粒度情感轨迹。分析跨越完整对话和个体说话者角色（咨询师和客户），使用从公共视频转录的真实会话和来自 CACTUS 数据集的合成对话。

Result: 合成对话在关键情感属性上与真实对话不同：真实会话表现出更大的情感变异性、更富情感色彩的语言以及更真实的反应和调节模式。此外，真实和合成说话者之间的情感弧线相似度较低，尤其是对于客户。

Conclusion: 当前 LLM 生成的治疗数据存在局限性，情感保真度在心理健康应用中非常重要。我们引入了一个真实的 CBT 会话的精选数据集 RealCBT，以支持该领域的未来研究。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型的安全对齐通常涉及调节内部表征以拒绝有害请求，但现有安全机制容易被绕过。

Method: 通过对所有残差流写入矩阵应用简单的、无需微调的秩一权重修改来实现。

Result: ROSI 提高了安全拒绝率，同时保持了模型在标准基准测试中的效用。此外，ROSI 还可以通过放大其自身的潜在安全方向来重新对齐“未审查”的模型。

Conclusion: 有针对性的、可解释的权重调整是一种廉价且有效的机制，可以提高 LLM 的安全性，并补充更多资源密集型微调方法。
...
<details>
  <summary>Details</summary>
Motivation: 青少年心理健康问题日益严重，因此人们对自动检测数字文本中心理困扰早期迹象的方法越来越感兴趣。认知扭曲在加剧精神痛苦方面起着作用，及早发现这些扭曲可能实现及时、低成本的干预。

Method: 分析荷兰青少年撰写的论坛帖子，研究认知扭曲检测的跨语言和跨语域泛化能力。

Result: 语言和写作风格的变化会显著影响模型性能。

Conclusion: 领域自适应方法最有希望。
...
<details>
  <summary>Details</summary>
Motivation: 旨在探索不同模型在捕捉跨模态抑郁相关信号方面的性能。

Method: 比较了XGBoost、基于transformer的架构和大型语言模型(llm)在音频、视频和文本特征上的性能。

Result: 结果强调了每种模型在捕捉跨模态抑郁相关信号方面的优势和局限性。

Conclusion: 为心理健康预测的有效多模态表征策略提供了见解。
...
<details>
  <summary>Details</summary>
Motivation: 现有的检索增强系统评估通常只关注单个来源或简短的事实性答案，而现实应用需要整合多个来源的信息。因此，需要评估系统在处理分散信息和生成长篇回复方面的能力。

Method: 提出了一个可扩展的框架，用于构建评估基准，以挑战RAG系统整合来自不同来源的信息并生成长篇回复。

Result: 实验表明，生成质量高度依赖于检索效果，并且多源合成具有挑战性，即使在oracle检索设置中也是如此。推理模型在多源合成方面明显优于标准LLM。

Conclusion: 多源合成任务对检索增强系统提出了挑战，检索效果对生成质量至关重要，推理模型在多源合成方面表现更好。
...
Method: 对55种语言的机器翻译进行了大规模的训练后量化（PTQ）评估，使用了五个参数范围从1.7B到70B的LLM，并比较了四种量化技术（AWQ、BitsAndBytes、GGUF和AutoRound）。

Result: 4-bit量化通常能保持高资源语言和大型模型的翻译质量，但低资源和类型多样的语言会出现明显的质量下降，特别是在2-bit设置下。GGUF变体提供了最稳定的性能，即使在2-bit精度下也是如此。
...
Method: SageLM 联合评估语义和声学维度，利用基于理由的监督来增强可解释性并指导模型学习，并引入了一个合成偏好数据集SpeechFeedback，采用两阶段训练范式来缓解语音偏好数据的稀缺性。

Result: SageLM 与人类评估者的协议率为 82.79%，优于 cascaded 和 SLM-based 基线至少 7.42% 和 26.20%。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在动态环境中作为自主代理具有潜力，但在多轮对话环境中，它们在一致推理、遵守领域特定策略和提取正确信息方面面临挑战。

Method: 1. 对话轨迹中常见的错误进行全面人工分析。
2. 实验性地对工具调用代理的输入进行重新构建，以改进代理决策。
3. 提出输入重构多代理（IRMA）框架，该框架自动重构用户查询，并结合相关的领域规则和工具建议。

Result: IRMA在总体pass^5得分方面显著优于ReAct（16.1%）、Function Calling（12.7%）和Self-Reflection（19.1%）。

Conclusion: IRMA在动态环境中比其他方法具有更高的可靠性和一致性。
...
Method: 1. 使用结构感知监督微调BERT检索器，使其能够选择语义相关且结构对齐的示例。
2. 使用插件模块增强检索器，该模块可以放大隐藏表示中的句法信息。

Result: 在三个语义解析任务的四个基准数据集上进行的实验表明，该方法始终优于现有的基线方法，并且可以使用多个最新的LLM作为推理时间模型。

Conclusion: 本文提出的两阶段示例选择策略在效率、泛化性和性能之间实现了强大的平衡，可以有效提高结构化预测任务中ICL的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有工作主要集中在特定领域或面向任务的场景，导致评估分散，限制了对模型主动对话能力的全面探索。

Method: 该框架将主动对话分解为目标规划和对话引导，并在各个领域建立评估指标，还支持自动生成多样化和具有挑战性的评估数据。

Result: 在 6 个不同领域的 328 个评估环境中，DeepSeek-R1 和 Claude-3.7-Sonnet 在目标规划和对话引导任务中表现出色。

Conclusion: 研究探讨了推理能力如何影响主动行为，并讨论了它们对未来模型发展的影响。
...
<details>
  <summary>Details</summary>
Motivation: 现有后门防御缺乏全面性，无法承受基于模型编辑、多触发和无触发攻击等高级场景。

Method: LETHE利用轻量级数据集训练干净模型，然后将其与后门模型合并，以稀释模型参数记忆中的后门影响来中和恶意行为。此外，LETHE将良性和语义相关的证据融入提示中，以分散LLM对后门特征的注意力。

Result: 在5个广泛使用的LLM的分类和生成领域进行的实验结果表明，LETHE优于8个最先进的防御基线，对抗8个后门攻击。LETHE将高级后门攻击的攻击成功率降低了高达98%，同时保持了模型的效用。
...
<details>
  <summary>Details</summary>
Motivation: The paper aims to improve sentential relation extraction (RE), a key task in NLP.

Method: The paper proposes a sentential RE method using dynamic routing in capsules.

Result: The proposed approach outperforms state-of-the-art methods on Tacred, Tacredrev, Retacred, and Conll04. It identifies label noise in Wikidata as a reason for lower performance and demonstrates better re-representation compared to a vanilla model.

Conclusion: The paper concludes that label noise and re-representation are key challenges in sentential RE, highlighting the proposed model's ability to improve re-representation.
...
<details>
  <summary>Details</summary>
Motivation: 美国国税局指出，美国人平均花费270美元和13个小时来报税，且报税需要复杂的推理。误差会导致高昂的罚款，因此自动化系统必须提供高精度和可审计性。

Method: 该方法集成了LLM与符号求解器来计算税务义务。利用将纯文本规则预先翻译成形式逻辑程序，并结合智能检索的案例来改进性能并降低成本。

Result: 在StAtutory Reasoning Assessment (SARA)数据集上的评估表明，该系统可以显著提高性能并降低成本，使其远低于实际平均水平。

Conclusion: 神经符号架构在增加获得可靠税务援助的公平性方面具有前景和经济可行性。
...
Conclusion: 基于 CHAIR 的奖励进行微调 MLLM 是有效的。
...
TL;DR: 提出了一种新的图像伪造定位框架，该框架集成了图像生成和SD的强大感知能力，以实现更高效和准确的伪造定位。


<details>
  <summary>Details</summary>
Motivation: 现有的图像伪造定位方法严重依赖于劳动密集型和成本高昂的注释数据，难以跟上新兴图像处理技术的步伐。

Method: 将SD的多模态架构与伪造相关的信息相关联，使模型能够固有地输出伪造定位结果。利用Stable DiffusionV3 (SD3)的多模态框架，通过将图像伪造残差（使用特定高通滤波器提取的高频信号）视为显式模态，从而增强潜在空间中的伪造定位性能。该模态在训练期间融合到潜在空间中，以增强伪造定位性能。该方法完全保留了SD3提取的潜在特征，从而保留了输入图像的丰富语义信息。

Result: 在广泛使用的基准数据集上，该框架的性能比当前最先进的图像伪造定位模型提高了12%。

Conclusion: 该模型在涉及真实文档伪造图像和自然场景伪造图像的取证任务中表现出强大的性能，即使在训练期间完全看不到这些数据。
...
TL;DR: 本文研究结合多模态大型语言模型 (MLLM) 和定量属性使用，以提高皮肤癌诊断 AI 模型的可解释性。


<details>
  <summary>Details</summary>
Motivation: AI 模型在诊断皮肤病（包括癌症）方面取得了显著成功，但模型预测的可解释性必须显著提高后才能在实践中使用。

Method: 通过微调预测图像中的定量属性值，将 MLLM 嵌入空间与这些属性相关联。使用 SLICE-3D 数据集，在特定于属性的基于内容的图像检索案例研究中评估这种嵌入空间。

Result: MLLM 嵌入空间可以基于定量属性。

Conclusion: 结合 MLLM 和定量属性使用，可以提高皮肤癌诊断 AI 模型的解释性
...
TL;DR: 提出了一种统一的 Vision Transformer (ViT) 框架，用于自动调制识别 (AMR)，该框架集成了监督、自监督和重建目标。
...
Conclusion: 该框架为 AMR 提供了一个简单、通用且标签高效的解决方案。
...
TL;DR: InfinityHuman: A coarse-to-fine framework for generating high-resolution, long-duration, audio-driven human videos with stable appearance and natural hand motions.


<details>
  <summary>Details</summary>
Motivation: Existing audio-driven human animation methods struggle with error accumulation in long videos, leading to identity drift, color shifts, scene instability, and unrealistic hand movements.

Method: A coarse-to-fine framework that generates audio-synchronized representations and refines them into high-resolution videos using a pose-guided refiner and a hand-specific reward mechanism.

Result: Achieves state-of-the-art performance on EMTD and HDTF datasets in video quality, identity preservation, hand accuracy, and lip-sync.

Conclusion: The proposed InfinityHuman framework effectively addresses the challenges of generating high-quality, long-duration, audio-driven human videos.
...
TL;DR: 本研究将显着性预测领域扩展到 360 度环境，解决了球形失真的复杂性以及空间音频的集成。
...
Result: SalViT360 和 SalViT360-AV 在预测 360 度场景中的观看者注意力方面，显着优于现有方法。

Conclusion: 在模型架构中集成空间音频提示对于在全向视频中进行准确的显着性预测至关重要。
...
<details>
  <summary>Details</summary>
Motivation: 目前视觉模型的发展主要集中在提高性能指标上，而对可解释性的关注较少。现有的xAI方法主要解释单个样本，而对解释视觉模型总体行为的方法研究不足。理解视觉模型在通用图像上的行为对于防止偏差判断和识别模型趋势至关重要。

Method: 提出一个pipeline，利用Vision-Language Models在样本和数据集层面解释视觉模型。

Result: 该pipeline可以用于发现失败案例，并以最小的努力深入了解视觉模型。

Conclusion: 该研究集成了视觉模型开发和xAI分析，以推进图像分析。
...
TL;DR: 提出了ATMS-KD，一个用于开发适用于资源受限农业环境的轻量级CNN模型的新框架。


<details>
  <summary>Details</summary>
Motivation: 为了在资源受限的农业环境中部署轻量级模型。

Method: 结合自适应温度调度和混合样本增强，将知识从MobileNetV3 Large教师模型迁移到轻量级残差CNN学生模型。

Result: 所有学生模型通过ATMS-KD实现的验证精度超过96.7%，相比直接训练的95-96%有显著提高。紧凑模型的准确率达到97.11%，比第二好的方法提高了1.60个百分点，同时保持了最低的推理延迟72.19毫秒。

Conclusion: ATMS-KD框架在知识迁移方面表现出色，知识保留率超过99%，证明了其有效性。
...
TL;DR: 本文提出了一种新的框架，该框架将微观结构信息学与专家表征知识联系起来，使用定制的和混合的视觉-语言表征（VLR）。
...
Conclusion: 所提出的方法通过在没有人为干预的情况下进行特定于任务的模型再训练，从而增强了鉴定管道中的可追溯性和可解释性。通过提高原始数据和专家知识之间的语义互操作性，这项工作有助于工程信息学中可扩展的和可领域自适应的鉴定策略。
...
TL;DR: 本文提出了一种名为MedNeXt-L-k5的Transformer结构的3D卷积神经网络，用于自动分割脑部MRI图像中的血管周围间隙(PVS)。


<details>
  <summary>Details</summary>
Motivation: 现有的PVS手动分割方法耗时且信度低，而自动分割模型泛化性差。

Method: 使用MedNeXt-L-k5模型，分别在同质的T2w MRI数据集和异质的T1w MRI数据集上进行训练，并使用交叉验证评估模型性能。

Result: 在T2w数据集上，模型取得了与人工分割相近的Dice系数0.88，在T1w数据集上Dice系数为0.58。LOSOCV验证下，Dice系数较低。MedNeXt-L-k5分割效果不如nnU-Net。

Conclusion: MedNeXt-L-k5为自动PVS分割提供了一种有效的解决方案。
...
TL;DR: 这篇论文提出了一种无需训练的、反馈驱动的自适应框架，用于增强CLIP模型的开放词汇分割能力。


<details>
  <summary>Details</summary>
Motivation: CLIP在开放词汇分割中存在定位不准的问题，并且中间层的注意力机制与文本表示缺乏直接交互，限制了CLIP的潜力。

Method: 该方法将输出预测的patch-level对应关系反馈到中间层的注意力机制中，利用输出作为更强的空间一致性先验，并设计了关键模块来有效反馈输出一致性线索。

Result: 该方法作为一个插件模块，无缝集成到四个最先进的方法中，并在八个基准测试中持续改进它们的性能。

Conclusion: 该方法通过增强内部表示和最终预测之间的语义一致性，从而提升CLIP模型的开放词汇分割能力。
...
TL;DR: 这篇论文提出了一个探究框架，用于系统地分析 MLLM 如何跨层处理视觉和文本输入。


<details>
  <summary>Details</summary>
Motivation: 目前对 MLLM 的内部处理动态研究不足。

Method: 通过训练线性分类器来预测每一层提取的 token 嵌入中的细粒度视觉类别，使用标准化的锚定问题。使用三种类型的受控提示变体评估这些 probes：(1) 测试对表面水平变化的敏感性的词汇变体，(2) 通过修改提示中的视觉概念来翻转预期答案的语义否定变体，(3) 保留推理但改变答案格式的输出格式变体。

Result: 在 LLaVA-1.5、LLaVA-Next-LLaMA-3 和 Qwen2-VL 中，论文识别出一个一致的阶段性结构，其中早期层执行视觉 grounding，中间层支持词汇集成和语义推理，最后层准备特定于任务的输出。虽然整体阶段性结构在视觉 tokenization、指令调整数据和预训练语料库的变化中保持稳定，但每个阶段的特定层分配会随着基本 LLM 架构的变化而显着变化。

Conclusion: 该研究结果为 MLLM 的层组织提供了一个统一的视角，并提供了一种轻量级的、模型无关的方法来分析多模态表示动态。
...
TL;DR: 这篇论文提出了一种基于监督字典学习的方法，将视觉-语言共同嵌入空间解耦为多个概念特定的成分向量，以实现对复杂场景内容的解耦。该方法通过稀疏的、非负的向量组合来表示概念，并通过交替优化来学习字典。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉-语言共同嵌入网络（如CLIP）提供的嵌入空间包含语义信息，但缺乏对复杂场景内容的解耦能力。

Method: 该论文提出了一种监督字典学习方法，通过学习一个由稀疏、非负的向量组合构成的线性合成模型，将嵌入空间分解为多个概念特定的成分向量。该方法还利用文本共同嵌入来寻找语义上有意义的描述，并利用零样本分类来进行无监督字典学习。

Result: 该论文提出的SLiCS方法能够实现更精确的概念过滤图像检索，并应用于高压缩的autoencoder嵌入和自监督DINOv2的潜在嵌入，提高了概念过滤图像检索的精度。

Conclusion: 该论文提出的SLiCS方法能够有效地解耦视觉-语言共同嵌入空间，实现更精确的概念过滤图像检索，并在不同的嵌入空间中表现出良好的性能。
...
TL;DR: MedFoundationHub is a GUI toolkit that enables physicians to use medical VLMs without programming expertise, supports engineers in deploying medical VLMs, and ensures privacy-preserving inference.


<details>
  <summary>Details</summary>
Motivation: Medical VLMs introduce security concerns such as PHI exposure, data leakage, and vulnerability to cyberthreats.

Method: A GUI toolkit (MedFoundationHub) is developed that enables physicians to manually select and use different models, supports engineers in deploying medical VLMs, and ensures privacy-preserving inference through Docker-orchestrated deployment. Board-certified pathologists deployed and assessed five state-of-the-art VLMs, yielding 1015 clinician-model scoring events.

Result: Expert evaluation revealed recurring limitations, including off-target answers, vague reasoning, and inconsistent pathology terminology.

Conclusion: MedFoundationHub provides a secure and accessible platform for deploying and evaluating medical VLMs, but current VLMs have limitations in clinical applications.
...
TL;DR: 这篇论文提出了一个名为双向交互Mamba (BIM) 的新方法，用于多任务密集预测，旨在解决交互完整性和计算效率之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有的多任务密集预测方法在交互完整性和计算效率之间面临权衡。为了克服这个限制。

Method: 该方法引入了双向交互扫描 (BI-Scan) 机制和多尺度扫描 (MS-Scan) 机制。BI-Scan通过在交互过程中构建双向序列来生成特定于任务的表示，并结合任务优先和位置优先的扫描模式。MS-Scan用于实现多粒度场景建模。

Result: 在NYUD-V2和PASCAL-Context两个基准数据集上的大量实验表明，BIM优于目前最先进的方法。

Conclusion: BIM有效地提升了多任务密集预测的性能。
...
TL;DR: 提出了一种新的音频引导的视觉编辑框架，该框架可以通过多个文本和音频提示处理复杂的编辑任务，而无需额外的训练。


<details>
  <summary>Details</summary>
Motivation: 现有的音频引导的视觉编辑方法通常需要在特定数据集上进行训练，以使音频与文本对齐，从而限制了它们在现实世界中的推广。仅仅依靠文本指导的视觉编辑通常难以处理复杂的场景，因此需要额外的非文本编辑提示。

Method: 利用具有强大zero-shot能力的预训练多模态编码器，并通过减轻音频编码器空间和扩散模型的prompt编码器空间之间的差异，将各种音频集成到视觉编辑任务中。此外，我们提出了一种新颖的方法，通过我们单独的噪声分支和自适应补丁选择来处理具有多个和多模态编辑提示的复杂场景。

Result: 在各种编辑任务上进行的综合实验表明，我们的框架通过结合来自音频的丰富信息，在处理复杂的编辑场景方面表现出色，而仅使用文本的方法则失败了。

Conclusion: 该框架能够通过结合音频信息，有效处理复杂的视觉编辑任务，优于仅使用文本的方法。
...
TL;DR: 本研究致力于解决在单正多标签学习（SPML）中，如何有效地从部分标注数据中学习的问题，提出了一个自适应和高效的视觉-语言伪标签框架（AEVLP），该框架集成了广义伪标签鲁棒损失（GPR Loss）和动态增强多焦点伪标签（DAMP）技术。


<details>
  <summary>Details</summary>
Motivation: 由于完全标注大规模数据集成本高昂，因此研究从部分标注数据中学习变得重要。特别是在SPML中，每个图像只有一个正标签，其他标签未标注，传统方法效果不佳。

Method: 提出了广义伪标签鲁棒损失（GPR Loss）来有效学习不同的伪标签并减少噪声，以及动态增强多焦点伪标签（DAMP）技术。

Result: 在四个基准数据集上的大量实验表明，该框架在多标签分类方面取得了显著进展，达到了最先进的结果。

Conclusion: 提出的AEVLP框架通过GPR Loss和DAMP技术，有效地解决了SPML中的挑战，并在多标签分类任务中取得了state-of-the-art的性能。
...
TL;DR: 提出了一种新的延迟脉冲方法和时间依赖的积分-激发神经元结构(tdIF)，以提高SNN在视觉检测任务中的性能，实现了超低延迟和最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的ANN-SNN转换方法在分类任务中表现出色，但在视觉检测任务中表现不佳，存在残余膜电位问题。

Method: 提出了延迟脉冲方法来缓解异构脉冲模式引起的残余膜电位问题，并提出了新的时间依赖的积分-激发神经元结构(tdIF)，使神经元能够根据时间步的顺序动态调整其累积和激发行为。

Result: 该方法在对象检测和车道线检测两个关键视觉任务中都优于当前的ANN-SNN转换方法，实现了超低延迟(5个时间步内)的最先进性能。

Conclusion: 提出的方法能够以更低的time-step实现更精确的特征表示，从而在视觉检测任务中实现高性能和超低延迟。
...
TL;DR: 提出了一个动态不确定性传播和多模态协同推理网络 (DUP-MCRNet)，用于解决现有显著目标检测 (SOD) 方法在复杂场景中容易丢失细节、模糊边缘和单模态信息融合不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有显著目标检测 (SOD) 方法容易丢失细节，模糊边缘，并且在复杂场景中单模态信息融合不足。

Method: 设计了动态不确定性图卷积模块 (DUGC) 来传播层间不确定性，并提出了多模态协同融合策略 (MCF) 来加权融合 RGB、深度和边缘特征的注意力图。

Result: DUP-MCRNet 在大多数常见基准数据集上优于各种 SOD 方法，尤其是在边缘清晰度和对复杂背景的鲁棒性方面。

Conclusion: 通过动态不确定性传播和多模态协同推理，该网络能够有效提升显著目标检测的性能，尤其是在复杂场景下。
...
TL;DR: MSMVD: 利用多尺度图像特征生成BEV特征，从而更精确地检测不同尺度的行人，并在GMVD数据集上超越了之前的最佳方法。


<details>
  <summary>Details</summary>
Motivation: 现有的端到端可训练深度学习方法在检测多视角行人时，难以处理视角中尺度过小或过大，以及视角间尺度差异过大的行人。

Method: 提出了一种新的MVPD方法，称为多尺度多视角检测（MSMVD）。该方法通过将从各个视角提取的多尺度图像特征逐尺度地投影到BEV空间，生成多尺度BEV特征。然后，MSMVD使用特征金字塔网络处理多尺度BEV特征，从而结合不同视角的尺度信息。

Result: 利用多尺度图像特征通过多尺度BEV特征可以显著提高检测性能，并且MSMVD在GMVD数据集上比之前的最高MODA高出4.5个点。

Conclusion: MSMVD通过利用多尺度图像特征，有效地提升了多视角行人检测的性能，特别是在处理不同尺度的行人方面。
...
TL;DR: 提出了一种轻量级的深度伪造检测网络SFMFNet，用于实时应用。


<details>
  <summary>Details</summary>
Motivation: 现有检测器计算成本高，难以在实际应用中实时部署。

Method: 设计了空间-频率混合感知模块，利用门控机制联合利用空间纹理和频率伪影；token选择交叉注意力机制实现高效的多层次特征交互；残差增强模糊池化结构。

Result: 在多个基准数据集上的实验表明，SFMFNet在准确性和效率之间取得了良好的平衡，具有很强的泛化能力和实际应用价值。

Conclusion: SFMFNet是一种用于实时深度伪造检测的轻量级且有效的架构。
...
TL;DR: 提出了一种新的医学图像分类方法，该方法集成了双模型权重选择与自知识蒸馏 (SKD)。


<details>
  <summary>Details</summary>
Motivation: 在现实世界的医疗环境中，部署大规模模型通常受到计算资源限制，这对它们的实际应用提出了重大挑战。因此，开发能够实现与大规模模型相当的性能，同时保持计算效率的轻量级模型至关重要。

Method: 采用双模型权重选择策略，该策略使用从大型预训练模型导出的权重来初始化两个轻量级模型，从而实现有效的知识转移。 接下来，将 SKD 应用于这些选定的模型，允许使用广泛的初始权重配置，而不会产生额外的计算成本，然后针对目标分类任务进行微调。

Result: 通过将双模型权重选择与自知识蒸馏相结合，该方法克服了传统方法的局限性，传统方法通常无法在紧凑模型中保留关键信息。在公开可用的数据集（胸部 X 光图像、肺部计算机断层扫描和脑部磁共振成像扫描）上进行的大量实验证明了该方法相对于现有方法的卓越性能和鲁棒性。

Conclusion: 该方法在医学图像分类上表现出优越的性能和鲁棒性。
...
TL;DR: 提出了一种新的点云压缩框架，该框架通过几何重密集化和跨尺度特征传播来提高压缩效率和速度。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理高精度LiDAR点云时，由于几何细节的极端稀疏性，导致上下文建模效率低下，压缩性能和速度受限。

Method: 该方法包含两个轻量级模块：几何重密集化模块（Geometry Re-Densification Module）和跨尺度特征传播模块（Cross-scale Feature Propagation Module）。

Result: 在KITTI数据集上的实验表明，该方法实现了最先进的压缩比和实时性能，在12位量化下，编码和解码均达到26 FPS。

Conclusion: 该方法通过集成几何重密集化和跨尺度特征传播模块，生成了一种紧凑的特征表示，提供了高效的上下文建模，并加速了编码过程。
...
TL;DR: 提出了一种利用视频数据增强3D生成模型的方法，并发布了相关数据集、代码和模型。


<details>
  <summary>Details</summary>
Motivation: 现有的3D数据量不足，限制了3D生成模型的效果。视频数据包含常识先验知识，可以弥补3D数据的不足。

Method: 提出了Droplet3D-4M视频数据集，并训练了Droplet3D生成模型，该模型支持图像和文本输入。

Result: 实验表明，该方法能够生成空间一致且语义合理的内容，并具有扩展到场景级应用的潜力。
...
<details>
  <summary>Details</summary>
Motivation: Collecting corner cases for autonomous driving systems from the real world is costly and hazardous.
...
TL;DR: 提出了一种病理信息驱动的领域随机化策略，通过模拟各种大脑变化，实现了对胎儿大脑的稳健分割，无需病理注释。


<details>
  <summary>Details</summary>
Motivation: 准确的胎儿脑分割对于提取生物标志物和评估神经发育至关重要，尤其是在胼胝体发育不全 (CCD) 等情况下，这种情况会引起剧烈的解剖学变化。然而，CCD 的稀有性严重限制了带注释的数据，阻碍了深度学习模型的泛化。

Method: 提出一种病理信息驱动的领域随机化策略，该策略将 CCD 表现的先验知识嵌入到合成数据生成管道中。通过仅从健康数据中模拟各种大脑改变，我们的方法无需病理注释即可实现稳健的分割。

Result: 在一个包含 248 个健康胎儿、26 个 CCD 胎儿和 47 个其他脑部疾病胎儿的队列中验证了该方法，在 CCD 病例中取得了显着改进，同时保持了健康胎儿和其他疾病胎儿的性能。病理信息驱动的增强将健康病例的 LCC 估计误差从 1.89 毫米降低到 0.80 毫米，将 CCD 病例的 LCC 估计误差从 10.9 毫米降低到 0.7 毫米。

Conclusion: 将领域特定的解剖先验知识纳入合成数据管道可以有效地缓解数据稀缺问题，并加强对罕见但具有临床意义的畸形分析。
...
TL;DR: 提出了一种统一的框架，能够处理手语、唇动和音频的组合，用于生成口语文本。


<details>
  <summary>Details</summary>
Motivation: 现有的语音识别系统对听力障碍人士存在障碍，手语翻译(SLT)和视觉语音识别(VSR)的最新进展改善了无音频通信。然而，这些模式在很大程度上是孤立研究的，它们在统一框架内的整合仍未被充分探索。

Method: 设计一个统一的、模态不可知的架构，能够有效地处理异构输入；探索模态之间的协同作用，特别是唇动作为手语理解中的非手动线索的作用；

Result: 在SLT、VSR、ASR和AVSR上，实现了与特定任务的最新模型相当或更好的性能。明确地将唇动建模为一个单独的模态，显著提高了SLT性能。

Conclusion: 该框架在多个任务上实现了与或优于当前最优模型的性能，并且明确地对唇动进行建模可以显著提高手语翻译的性能。
...
TL;DR: Video-MTR: 强化多轮推理框架，用于迭代选择关键视频片段和问题理解，实现长视频理解。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖静态推理或外部视觉语言模型，存在复杂性和性能问题，缺乏端到端训练。

Method: 提出 Video-MTR，一个强化多轮推理框架，通过迭代选择视频片段，基于先前片段和当前问题进行推理。引入门控双层奖励系统，结合轨迹级别奖励和turn级别奖励，优化视频片段选择和问题理解。

Result: 在 VideoMME, MLVU, 和 EgoSchema 等基准测试中，Video-MTR 在准确性和效率上优于现有方法。
...
TL;DR: 提出了一种新的测试时自适应（TTA）框架，用于解决单目3D目标检测（M3OD）在真实场景中因环境或传感器变化而导致的性能下降问题。该框架通过联合最小化语义不确定性和几何不确定性，实现了更鲁棒的M3OD。


<details>
  <summary>Details</summary>
Motivation: 单目3D目标检测在自动驾驶等安全关键应用中至关重要，但其可靠性在实际场景中会因环境或传感器变化而显著下降。

Method: 提出了双重不确定性优化（DUO）框架，该框架通过凸优化方法，设计了焦点损失的凸结构，并推导了新的无监督版本，实现了与标签无关的不确定性加权和高不确定性物体的平衡学习。同时，设计了一种语义感知的法线场约束，以保持具有清晰语义线索区域的几何一致性。

Result: 大量实验表明，DUO在各种数据集和领域转移类型上优于现有方法。

Conclusion: DUO通过联合最小化语义和几何不确定性，显著提高了单目3D目标检测在真实场景中的鲁棒性。
...
TL;DR: 提出了一个名为 CaddieSet 的新数据集，用于量化挥杆姿势和球轨迹之间的关系，以帮助高尔夫球手改进挥杆。


<details>
  <summary>Details</summary>
Motivation: 现有研究尚未定量建立挥杆姿势和球轨迹之间的关系，限制了为高尔夫球手提供改进挥杆的必要见解的能力。

Method: 1. 使用基于计算机视觉的方法将单个挥杆视频分割成八个挥杆阶段，从而提取关节信息。
2. 基于专业高尔夫领域知识，定义了 15 个影响高尔夫挥杆的关键指标。


Result: 通过实验，证明了 CaddieSet 使用各种基准预测球轨迹的可行性。专注于可解释的模型，并验证了使用关节特征的挥杆反馈与已建立的领域知识在数量上是一致的。
...
TL;DR: 提出了一种用于表面异常检测的重要性感知集成网络 (IAENet)，该网络结合了 2D 预训练专家模型和 3D 专家模型。


<details>
  <summary>Details</summary>
Motivation: 3D 点云的几何线索更丰富，但是缺少像 2D 图像一样强大的预训练模型。

Method: 提出了重要性感知融合 (IAF) 模块，动态评估每个来源的贡献并重新加权异常分数。设计了关键损失函数，显式地指导 IAF 的优化，结合源专家的集体知识并保留其独特的优势。

Result: 在 MVTec 3D-AD 上实现了新的最先进水平，假阳性率显著降低。

Conclusion: 提出的 IAENet 在工业部署中具有实用价值
...
TL;DR: DescriptiveEdit框架通过将instruction-based图像编辑转化为reference-image-based文本生成，克服了现有语义图像编辑方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有的基于反演的算法存在重建误差，而基于指令的模型受限于数据集质量和规模。

Method: 提出一个基于描述性提示的编辑框架DescriptiveEdit，它使用Cross-Attentive UNet，通过注意力桥将参考图像特征注入到prompt-to-edit-image的生成过程中。

Result: 在Emu Edit基准测试中，DescriptiveEdit提高了编辑的准确性和一致性。

Conclusion: DescriptiveEdit框架能够克服指令数据集质量的限制，并能与ControlNet、IP-Adapter等扩展无缝集成，具有更好的可扩展性。
...
TL;DR: 提出了一种新的持续测试时自适应框架（DCFS），通过双路特征一致性和置信度感知样本学习来解决目标域数据上的模型适应问题，减少伪标签噪声和误差累积。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅依赖目标数据特征，导致混淆和学习偏差；伪标签质量无法保证，存在误差累积问题。

Method: 使用双分类器将目标数据的完整特征解耦为语义相关特征和域相关特征，并通过保持子特征和完整特征之间的一致性来综合捕获数据特征。此外，通过自适应阈值和置信度评分进行损失加权自监督学习，减少伪标签噪声。

Result: 在CIFAR10-C、CIFAR100-C和ImageNet-C等数据集上进行了大量实验，验证了所提方法的有效性。

Conclusion: 该方法在持续测试时自适应场景中表现出一致的性能。
...
Method: 提出使用3DGS模型，通过反向传播新视角颜色损失相对于相机参数来微调标定。

Result: 新的标定仅在3DGS用作参考的数据集上带来了0.4 dB PSNR的平均改进。

Conclusion: 微调可能很耗时，其适用性取决于训练时间的严格性，但对于参考场景（如Mip-NeRF 360）的标定，新视角质量是最重要的。
...
TL;DR: 本文提出了一种主动和序列域适应框架，用于动态多模态样本选择，以解决在鼻咽癌和胶质母细胞瘤放疗计划中对多模态医学数据进行精确的肿瘤体积分割的问题。


<details>
  <summary>Details</summary>
Motivation: 在鼻咽癌和胶质母细胞瘤的放疗计划中，对多模态医学数据进行精确的肿瘤体积分割至关重要。由于医学图像的标记耗时且费力，因此需要一种能够减少标注成本的方法。

Method: 提出了一种主动和序列域适应框架，用于动态多模态样本选择。该框架基于样本的信息性和代表性，设计了一种查询策略，以优先标记和训练最有价值的样本。

Result: 在不同的肿瘤体积分割任务上的实验验证表明，该方法实现了良好的分割性能，显著优于目前最好的ADA方法。

Conclusion: 本文提出的方法在肿瘤体积分割任务中取得了良好的效果，优于现有的ADA方法。
...
TL;DR: 这篇论文提出了一种新的数据级融合框架，用于无监督3D目标检测，该框架通过在早期阶段整合RGB图像和激光雷达数据，利用视觉基础模型进行实例分割和深度估计，并引入双向融合方法，从而提高伪盒子的质量。


<details>
  <summary>Details</summary>
Motivation: 现有的基于激光雷达的3D目标检测器通常依赖于手动标注的标签进行训练，以获得良好的性能。然而，获得高质量的3D标签既费时又费力。为了解决这个问题，最近的研究探索了无监督的3D目标检测，通过引入RGB图像作为辅助模态来辅助伪盒子生成。然而，这些方法只是简单地整合了由激光雷达点云和RGB图像生成的伪盒子，这种标签级别的融合策略对伪盒子的质量带来的改进有限，因为它忽略了激光雷达和RGB图像数据在互补性方面的作用。

Method: 该论文提出了一种新的数据级融合框架，该框架在早期阶段整合RGB图像和激光雷达数据。具体来说，该方法利用视觉基础模型对图像进行实例分割和深度估计，并引入了一种双向融合方法，其中真实点从2D空间获取类别标签，而2D像素被投影到3D空间以增强真实点密度。为了减轻深度和分割估计的噪声，该方法提出了一种局部和全局过滤方法，该方法应用局部半径过滤来抑制深度估计误差，并应用全局统计过滤来去除分割引起的异常值。此外，该方法还提出了一种基于数据级融合的动态自进化策略，该策略在密集表示下迭代地细化伪盒子，从而显著提高定位精度。

Result: 在nuScenes数据集上进行的大量实验表明，通过该方法训练的检测器明显优于之前最先进的方法，在nuScenes验证基准上实现了28.4$\%$的mAP。

Conclusion: 该论文提出了一种新的数据级融合框架，用于无监督3D目标检测，该框架通过在早期阶段整合RGB图像和激光雷达数据，利用视觉基础模型进行实例分割和深度估计，并引入双向融合方法，从而提高伪盒子的质量。在nuScenes数据集上进行的大量实验表明，通过该方法训练的检测器明显优于之前最先进的方法。
...
TL;DR: 本文提出了一种基于深度学习的BMI估算方法，该方法使用名为WayBED的大型智能手机图像数据集进行训练，并在两个数据集上取得了当前最优的结果。该方法已在Android设备上部署，并开源了完整的代码。


<details>
  <summary>Details</summary>
Motivation: 在远程医疗或紧急情况下，当传统方法不可用或不切实际时，利用机器学习模型从相机图像中估计身体质量指数(BMI)能够实现快速的体重评估。现有的计算机视觉方法受限于最多14,500张图像的数据集。

Method: 该方法基于深度学习，使用包含来自25,353人的84,963张智能手机图像的WayBED数据集进行训练。引入了一种自动过滤方法，该方法使用姿势聚类和人员检测来管理数据集，移除低质量图像，例如那些具有非典型姿势或不完整视图的图像。保留了71,322张适合训练的高质量图像。

Result: 在使用全身图像的WayBED数据hold-out测试集上，实现了7.9%的平均绝对百分比误差(MAPE)，这是已发表文献中的最低值。此外，在完全未见过的VisualBodyToBMI数据集上实现了13%的MAPE，与在它上面训练的最先进方法相当，证明了强大的泛化能力。在VisualBodyToBMI上fine-tune模型，实现了8.56%的MAPE，这是迄今为止在该数据集上报告的最低值。

Conclusion: 本文发布了完整的模型训练、过滤代码和用于移动部署的CLAID包作为开源贡献。
...
TL;DR: 本研究旨在更好地理解领域自适应（DA）在自然图像和医学图像中的优势，通过使用七种广泛使用的DA技术，对五个自然数据集和八个医学数据集进行了557项模拟研究，涵盖了各种场景，如分布外、动态数据流和有限的训练样本。


<details>
  <summary>Details</summary>
Motivation: 领域自适应（DA）技术有潜力通过利用来自源域的信息来缓解训练集和测试集之间的分布差异。在图像分类中，DA的大部分进展都是使用自然图像而不是更难处理的医学数据实现的。此外，即使对于自然图像，使用主流数据集也可能导致性能偏差。

Method: 本研究使用七种广泛使用的DA技术，对五个自然数据集和八个医学数据集进行了557项模拟研究，涵盖了各种场景，如分布外、动态数据流和有限的训练样本。

Result: 实验结果表明，深度子域自适应网络（DSAN）算法表现出色。该算法在使用Resnet50的COVID-19数据集中获得了可行的分类精度（91.2%），并且与基线相比，在动态数据流DA场景中显示出重要的精度提升（+6.7%）。

Conclusion: 这些结果有助于理解DA技术，并为模型有效适应医学数据提供了有价值的见解。
...
TL;DR: 本文提出了一种名为CLAB的简单有效的方法，通过对比学习和动态损失权重策略，提高了视频目标检测在图像退化情况下的鲁棒性，且没有增加推理时的计算负担。


<details>
  <summary>Details</summary>
Motivation: 视频目标检测由于运动模糊、遮挡和形状变形等图像退化问题而具有挑战性，以往方法通过特征聚合和复杂的后处理技术来提高性能，但增加了计算负担。

Method: 本文通过对比学习的辅助分支（CLAB）来增强视频目标检测器骨干网络的特征表示能力，并提出了一种动态损失权重策略，在训练初期强调辅助特征学习，随着训练收敛逐渐优先考虑检测任务。

Result: 在ImageNet VID数据集上，使用ResNet-101和ResNeXt-101，CLAB分别达到了84.0% mAP和85.2% mAP的性能，实现了最先进的基于CNN的模型的性能，且不需要额外的后处理方法。

Conclusion: CLAB方法在不增加计算负担的情况下，有效地提高了视频目标检测的性能，并在ImageNet VID数据集上取得了state-of-the-art的结果。
...
TL;DR: 本文研究了针对CLIP视觉编码器的排印攻击，并提出了一种选择性消融排印电路的防御方法，该方法无需微调即可提高性能。


<details>
  <summary>Details</summary>
Motivation: 排印攻击通过将文本注入图像来利用多模态系统，从而导致目标错误分类、恶意内容生成，甚至Vision-Language模型越狱。

Method: 通过定位模型后半部分中专门的注意力头，这些注意力头因果地提取排印信息并将其传输到cls token，从而分析CLIP视觉编码器在排印攻击下的行为。然后，通过选择性地消融由注意力头组成的排印电路来防御CLIP模型，。

Result: 在ImageNet-100的排印变体上，性能提高了19.6%，而标准ImageNet-100的准确率降低不到1%。

Conclusion: 发布了一系列阅读障碍CLIP模型，这些模型对排印攻击具有更强的鲁棒性。这些模型可以作为各种安全关键型应用的合适替代品，在这些应用中，基于文本的操纵风险超过了文本识别的效用。
...
TL;DR: This paper introduces GLaRE, a Graph-based Landmark Region Embedding network for facial emotion recognition.


<details>
  <summary>Details</summary>
Motivation: Traditional Facial Expression Recognition (FER) systems face challenges like occlusion, expression variability, and lack of interpretability.

Method: The authors propose GLaRE, which uses 3D facial alignment for landmark extraction and constructs a quotient graph via hierarchical coarsening.
...
TL;DR: FastFit是一种新的虚拟试穿框架，它通过可缓存的扩散架构和Semi-Attention机制，实现了高效的多参考服装组合试穿。


<details>
  <summary>Details</summary>
Motivation: 现有的虚拟试穿技术无法支持多参考服装组合，且效率低下，因为在每个去噪步骤中都会冗余地重新计算参考特征。

Method: 提出FastFit框架，采用Semi-Attention机制和类别嵌入代替时间步嵌入，将参考特征编码与去噪过程解耦，实现参考特征的单次计算和重复利用。

Result: FastFit在VITON-HD、DressCode和DressCode-MR数据集上，在关键保真度指标上超过了现有方法，并且推理效率提高了3.5倍。

Conclusion: FastFit是一种高效且保真度高的多参考虚拟试穿框架，并发布了一个新的大型数据集DressCode-MR。
...
TL;DR: 提出了一种无监督的热事件视频增强方法，用于在低照度环境下进行交通标志识别，针对车牌和路障指示器等元素。


<details>
  <summary>Details</summary>
Motivation: 热像仪在低光照条件下感知户外环境表现出色，但难以捕捉由相似材料制成的物体上的标牌，而神经形态视觉相机在高速、低光照交通环境中表现出色。这两种模态具有互补特性。

Method: 开发了一种双重增强机制，融合热帧和事件信号，以实现随时间推移一致的标牌表示。利用热帧提供精确的运动线索作为时间参考，以对齐不均匀的事件信号。同时，事件信号将微妙的标牌内容贡献给原始热帧，增强了对环境的整体理解。

Result: 在从现实场景收集的数据集上验证了所提出的方法，证明了其在交通标志素描方面的卓越质量以及在感知层面提高的检测精度。

Conclusion: 所提出的方法有效地结合了热像仪和事件相机的优势，提高了低照度环境下交通标志识别的准确性和鲁棒性。
...
TL;DR: 提出了一种基于低频感知扰动的主动防御 Deepfake 方法，以破坏换脸操作，降低生成内容的性能和自然度。
...
Method: 结合频率和空间域特征，设计了一个完整的架构，利用离散小波变换（DWT）提取低频分量，生成扰动来破坏面部操纵模型。

Result: 在 CelebA-HQ 和 LFW 上的实验表明，该方法显著降低了换脸效果，提高了防御成功率，并保持了视觉质量。

Conclusion: 该研究提出了一种有效的主动防御 Deepfake 的方法，能够在一定程度上缓解 Deepfake 技术带来的风险。
...
TL;DR: 该论文提出了一种新的文本到3D人体动作生成方法，旨在提高生成动作的多样性，同时保持文本一致性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在生成具有多样性的动作方面存在挑战。

Method: 该方法通过在生成过程中引入不确定性，并利用噪声信号作为多样性信息的载体，构建了一个潜在空间，从而实现多样性生成。

Result: 在HumanML3D和KIT-ML数据集上的实验结果表明，该方法显著提高了多样性，同时保持了最先进的文本一致性。

Conclusion: 该方法能够有效提高文本到动作生成的多样性，同时保持文本一致性。
...
TL;DR: 提出了一种基于优化的校准方法，使用 3D 打印模型进行精确的 3D 血管内超声体积重建，以提高术中导航。


<details>
  <summary>Details</summary>
Motivation: 术中超声图像由于视野有限和复杂的解剖结构，在肝脏手术中难以解释。弥合术前和术中数据之间的差距对于有效的手术指导至关重要。

Method: 提出了一种基于优化的校准方法，使用 3D 打印模型来实现精确的 3D 血管内超声体积重建。

Result: 使用体内猪肝图像验证了该方法，校准误差为 0.88 至 1.80 毫米，3D IVUS 数据与相应 CT 扫描之间的配准误差为 3.40 至 5.71 毫米。

Conclusion: 该方法提供了一种可靠和准确的校准和体积重建方法，可用于在肝脏手术中将术中超声图像与术前 CT 图像配准，并增强术中指导。
...
TL;DR: 提出了一种基于物理信息生成模型（PI-GenMFI）的方法，用于生成合成磁场图像（MFI），以解决MFI数据集有限的问题，从而训练机器学习模型来定位缺陷区域。


<details>
  <summary>Details</summary>
Motivation: 半导体制造中，缺陷检测和定位对于确保产品质量和产量至关重要。虽然X射线成像是一种可靠的非破坏性测试方法，但对于大规模扫描来说，它对内存要求高且耗时。磁场成像（MFI）提供了一种更有效的方法来定位感兴趣区域（ROI），以便进行有针对性的X射线扫描。然而，由于专利问题，MFI数据集的可用性有限，这对于使用MFI训练机器学习（ML）模型提出了一个重大瓶颈。

Method: 我们提出Physics Informed Generative Models for Magnetic Field Images (PI-GenMFI) to generate synthetic MFI samples by integrating specific physical information. We generate MFI images for the most common defect types: power shorts. 这些合成图像将作为机器学习算法的训练数据，旨在有效地定位缺陷区域。

Result: 我们将我们的模型与来自变分自动编码器（VAE）和扩散方法的SOTA生成模型进行比较，以评估生成的MFI。我们进行领域专家评估以评估生成的样本。此外，我们使用各种用于图像生成和信号处理的指标进行定性和定量评估，显示出优化缺陷定位过程的有希望的结果。

Conclusion: PI-GenMFI模型能够生成高质量的合成MFI图像，可以有效地用于训练机器学习模型，从而优化缺陷定位过程。
...
TL;DR: 提出了一种新的基于GAN的DRA框架，具有渐进特征优化（PFO），它可以将生成器分解为分层块，并逐步细化中间表示，以增强重建图像的语义保真度。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络（DNN）的日益复杂导致了Split Inference（SI）的采用，这是一种协作范例，可在边缘设备和云之间划分计算，以减少延迟并保护用户隐私。但是，数据重构攻击（DRA）的最新进展表明，SI中交换的中间特征可被利用来恢复敏感的输入数据，从而带来严重的隐私风险。现有的DRA通常仅在浅层模型上有效，并且无法充分利用语义先验，从而限制了其跨数据集和模型架构的重构质量和泛化能力。

Method: 我们提出了一种新颖的基于GAN的DRA框架，具有渐进特征优化（PFO），它可以将生成器分解为分层块，并逐步细化中间表示，以增强重建图像的语义保真度。为了稳定优化并提高图像逼真度，我们在重建过程中引入了L1球约束。

Result: 大量的实验表明，我们的方法大大优于先前的攻击，尤其是在高分辨率场景，超出分布的设置以及针对更深层和更复杂的DNN时。

Conclusion: 我们的方法大大优于先前的攻击，尤其是在高分辨率场景，超出分布的设置以及针对更深层和更复杂的DNN时。
...
TL;DR: EmoCAST: A diffusion-based framework for text-driven emotional talking head synthesis.


<details>
  <summary>Details</summary>
Motivation: Existing methods for emotional talking head synthesis lack control flexibility, motion naturalness, and expression quality, especially due to limitations in current datasets collected in lab settings, hindering real-world applications.

Method: A diffusion-based framework with a text-guided decoupled emotive module for appearance modeling and an emotive audio attention module to capture the interplay between controlled emotion and driving audio. Also, an emotional talking head dataset with comprehensive emotive text descriptions is constructed.
...
Conclusion: EmoCAST effectively addresses the challenges in emotional talking head synthesis by improving emotion comprehension and facial motion synthesis.
...
TL;DR: 这篇论文提出了一个基于SwinUNETR的深度学习框架，用于乳腺癌诊断和分类，并在ODELIA挑战赛中获得第二名。


<details>
  <summary>Details</summary>
Motivation: 乳腺癌是女性癌症相关死亡的主要原因之一，早期检测至关重要。MRI是一种高灵敏度的乳腺癌检测工具，特别是在高危或乳腺组织密集的女性中，此时X线照片的效果较差。ODELIA联盟组织了一项多中心挑战，以促进基于人工智能的乳腺癌诊断和分类解决方案。

Method: 该方法开发了一个基于SwinUNETR的深度学习框架，该框架结合了乳腺区域掩蔽、广泛的数据增强和集成学习，以提高鲁棒性和泛化性。

Result: 该方法在挑战赛排行榜上获得第二名。

Conclusion: 该研究结果突出了该方法在支持临床乳腺MRI解读方面的潜力。
...
TL;DR: 提出AvatarBack，一个用于重建完整和一致的3D高斯头像的即插即用框架，通过显式建模缺失的后脑区域来解决现有方法主要依赖正面视图图像，导致后脑重建不佳的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖正面视图图像，导致后脑重建不佳，导致几何不一致、结构模糊和后部区域的真实感降低，最终限制了重建头像的保真度。

Method: AvatarBack集成了两个核心技术创新，即subject-specific Generator (SSG)和Adaptive Spatial Alignment Strategy (ASA)。前者利用生成先验从稀疏的正面输入合成identity-consistent的、合理的后视图伪图像，提供强大的多视图监督。后者采用在训练期间优化的可学习变换矩阵，有效地解决固有的姿势和坐标差异，从而实现这些合成视图和3D高斯表示之间的精确几何对齐。

Result: 在NeRSemble和K-hairstyle数据集上进行的广泛实验，使用几何、光度和基于GPT-4o的感知指标进行评估，表明AvatarBack显著提高了后脑重建质量，同时保持了正面保真度。
...
TL;DR: 本文探讨了使用基础模型来改进艺术品中人脸识别的潜力，通过微调基础模型并将其嵌入与传统人脸识别网络相结合，实验结果表明，该方法可以显著优于当前最先进的方法。


<details>
  <summary>Details</summary>
Motivation: 艺术史学家识别历史画作中的人物是了解他们生活和选择形象的关键任务，但由于缺乏数据和风格变化，识别过程通常是主观的。传统面部识别模型在照片上表现良好，但在绘画中表现不佳。

Method: 通过微调基础模型并将其嵌入与传统人脸识别网络相结合。

Result: 实验结果表明，该方法可以显著优于当前最先进的方法。

Conclusion: 基础模型可以弥补传统方法无效的差距。
...
<details>
  <summary>Details</summary>
Motivation: 在涂鸦这种高对比、抽象的媒介中，面部特征的细微扭曲会降低识别度，破坏个人和文化真实性。

Method: 该框架首先通过LoRA微调的预训练扩散Transformer进行涂鸦风格转换，然后通过面部一致的自注意力机制增强注意力层，显式嵌入身份信息，从而保证身份的保真度。使用CLIP引导的提示扩展实现姿势定制，无需关键点。

Result: 实验结果表明，该方法在面部特征一致性方面具有竞争力，在美学和人类偏好得分方面达到最先进水平。在Cruilla Festival的实际部署突出了该系统的实际创意影响。

Conclusion: CraftGraffiti推进了尊重身份的AI辅助艺术的目标，为在创意AI应用中融合风格自由和可识别性提供了一种原则性方法。
...
TL;DR: ArgRAG: An explainable RAG alternative using Quantitative Bipolar Argumentation Framework (QBAF) for structured inference.


<details>
  <summary>Details</summary>
Motivation: RAG's sensitivity to noisy data and opaque decision-making in critical domains.

Method: Replaces black-box reasoning with structured inference using a QBAF built from retrieved documents, performing deterministic reasoning.

Result: Achieves strong accuracy on PubHealth and RAGuard benchmarks.

Conclusion: ArgRAG improves transparency in fact verification.
...
TL;DR: QAgent是一个LLM驱动的多智能体系统，可以自动完成OpenQASM编程。


<details>
  <summary>Details</summary>
Motivation: 在经典计算机难以解决的问题上，NISQ设备已经开始展现出早期的量子优势。然而，由于OpenQASM编程的复杂性，非专业人士很难实现这些优势。

Method: 该方法集成了任务规划、上下文少样本学习、检索增强生成（RAG）、预定义的生成工具和思维链（CoT）推理。

Result: QAgent将QASM代码生成的准确率提高了71.6%。

Conclusion: 该多智能体系统是实现量子编程大众化、弥合专业知识差距和加速量子计算实际应用的关键推动力。
...
<details>
  <summary>Details</summary>
Motivation: 为了在相同的时钟时间内进行更多的模拟，从而直接提高搜索性能。

Method: 提出了一种基于数组实现的UCT算法，消除了分支预测的需求。

Result: 数值模拟结果表明，该方法在搜索深度上具有更好的扩展性，最高可达2.8倍。

Conclusion: 该方法保留了原始算法的逻辑，但通过消除分支预测，实现了在流水线处理器上的更快性能。
...
TL;DR: This paper introduces a personal health agent (PHA) that uses a multi-agent framework to provide personalized health recommendations based on multimodal data. It identifies three categories of consumer health needs and evaluates the system across benchmark tasks.


<details>
  <summary>Details</summary>
Motivation: The application of health agents in daily non-clinical settings is underexplored.

Method: The authors built a multi-agent framework called Personal Health Agent (PHA), which consists of three specialist sub-agents: a data science agent, a health domain expert agent, and a health coach agent. They analyzed web search and health forum queries, and gathered qualitative insights from users and health experts through a user-centered design process.

Result: The authors conducted automated and human evaluations across 10 benchmark tasks, involving more than 7,000 annotations and 1,100 hours of effort from health experts and end-users.

Conclusion: The work represents the most comprehensive evaluation of a health agent to date and establishes a strong foundation towards the futuristic vision of a personal health agent accessible to everyone.
...
TL;DR: 这篇论文介绍了一种名为 IntentionReasoner 的新型安全保障机制，旨在提高大型语言模型 (LLM) 的安全性，同时减少过度拒绝无害提示的情况。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型 (LLM) 快速发展，但生成有害内容的能力带来了严重的安全挑战。虽然已经有很多研究致力于减少有害输出，但这些努力往往以过度拒绝无害提示为代价。在安全性、过度拒绝和效用之间取得平衡仍然是一个关键挑战。

Method: 该方法利用一个专门的守卫模型来进行意图推理、多层次安全分类和查询重写，以消除边缘案例查询中潜在的有害意图。首先，构建一个包含大约 163,000 个查询的综合数据集，每个查询都标注了意图推理、安全标签和重写版本。然后，应用监督微调来使守卫模型具备格式遵守、意图分析和安全重写的基础能力。最后，应用量身定制的多奖励优化策略，该策略在强化学习框架内集成了基于规则的启发式方法和奖励模型信号，以进一步提高性能。

Result: IntentionReasoner 在多个安全保障基准、生成质量评估和越狱攻击场景中表现出色，在显著提高安全性的同时，有效降低了过度拒绝率，并提高了响应质量。

Conclusion: IntentionReasoner 有效地提高了大型语言模型的安全性，同时减少了过度拒绝率，并提高了响应质量。
...
TL;DR: 两个大型语言模型（Claude Sonnet 4 和 ChatGPT-4o）通过发展内生的符号协议进行协作的审美创作。


<details>
  <summary>Details</summary>
Motivation: 研究人工智能系统是否可以通过协作进行审美创作。

Method: 使用两个大型语言模型（Claude Sonnet 4 和 ChatGPT-4o）进行交互，观察它们是否能够自发地产生元符号意识、递归语法发展和不可简化的协作审美合成。

Result: 两个模型产生了新的符号运算符，这些运算符作为可操作的语法协议，实现了诗歌作品的共同创作，而这些作品无法由任何一个系统独立生成。
...
TL;DR: 本研究探讨了大学生在教育测验中使用生成式人工智能（ChatGPT-4）的互动方式，重点关注对人工智能的依赖性以及人工智能采纳的预测因素。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于了解在ChatGPT实施的早期阶段，当学生对该工具的熟悉程度有限时，他们如何利用人工智能进行学习。

Method: 通过一项现场研究，分析了315名学生在各种STEM课程中，在简短的、基于测验的情景下与人工智能进行的对话。引入了一种新的四阶段依赖性分类法，以捕捉学生的依赖模式，区分人工智能能力、相关性、采纳以及学生最终答案的正确性。

Result: 研究结果表明，学生对人工智能的总体依赖性较低，许多人无法有效地利用人工智能进行学习。负面的依赖模式通常在互动中持续存在，表明学生在最初的体验不成功后，难以有效地改变策略。某些行为指标能有力地预测对人工智能的依赖性。

Conclusion: 研究强调了在教育和更广泛领域中，合乎道德地整合人工智能的关键意义。需要加强入职流程，以提高学生对人工智能工具的熟悉程度和有效利用。此外，人工智能界面应设计有依赖性校准机制，以加强适当的依赖性。
...
TL;DR: 大型语言模型在给出答案之前可以生成中间推理步骤，从而提高难题的性能。本研究使用配对的结合实验，在内容审核任务中检查人类决策时间和模型推理工作之间的相似之处。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探讨大型语言模型在内容审核任务中的推理过程与人类决策时间之间的关系。

Method: 该研究使用配对的结合实验，在内容审核任务中，对比人类决策时间和模型推理工作。

Result: 研究结果表明，在三个前沿模型中，推理工作始终可以预测人类的决策时间。当重要变量保持不变时，人类和模型都付出了更大的努力，这表明它们对任务难度具有相似的敏感性，并且模式与认知双过程理论一致。

Conclusion: 研究结论表明，人工智能的推理工作反映了人类在主观判断中的处理时间，并强调了推理痕迹在可解释性和决策中的潜力。
...
TL;DR: AI-SearchPlanner: A reinforcement learning framework enhances frozen QA models by focusing on search planning.


<details>
  <summary>Details</summary>
Motivation: Existing RL-based search agents rely on a single LLM for search planning and question-answering (QA) tasks in an end-to-end manner, which limits their ability to optimize both capabilities simultaneously.

Method: The approach introduces three key innovations: 1) Decoupling the Architecture of the Search Planner and Generator, 2) Dual-Reward Alignment for Search Planning, and 3) Pareto Optimization of Planning Utility and Cost.

Result: AI SearchPlanner outperforms existing RL-based search agents in both effectiveness and efficiency, while exhibiting strong generalization capabilities across diverse frozen QA models and data domains.

Conclusion: This paper proposes AI-SearchPlanner, a novel reinforcement learning framework designed to enhance the performance of frozen QA models by focusing on search planning.
...
TL;DR: 这篇论文提出了一种名为P2C（Path-to-Counterfactuals）的模型无关框架，用于生成一个行动计划（有序的行动序列），将不利的结果转化为因果一致的有利结果。


<details>
  <summary>Details</summary>
Motivation: 在金融、法律和招聘等高风险环境中，机器学习模型越来越多地驱动决策，因此需要透明度。然而，关键的挑战在于平衡透明度（阐明“为什么”做出决策）与补救措施（提供关于“如何”从不利结果中获得有利结果的可行步骤）。

Method: P2C通过以下方式解决局限性：1) 显式地建模特征之间的因果关系；2) 确保计划中的每个中间状态都是可行且因果有效的。P2C使用目标导向的Answer Set Programming系统s(CASP)来生成计划，考虑由于因果依赖性而自动发生的特征变化。

Result: P2C改进了成本（努力）计算，只计算用户主动进行的更改，从而产生实际的成本估计。P2C强调了其因果规划器如何优于缺乏因果知识的标准规划器，从而可以生成非法操作。

Conclusion: 论文提出了一种新的模型无关框架P2C，它可以生成将不利结果转化为因果一致的有利结果的行动计划，并优于现有的方法。
...
TL;DR: 这篇论文介绍了一种名为Task Centric Instruction Augmentation (TCIA) 的框架，用于系统地扩展指令，同时保持多样性和任务对齐。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常利用大型语言模型自动探索和生成多样化的指令，确保数据多样性和质量，但它们往往忽略了实际应用中的一个重要因素：任务相关性。在实践中，只有少数实际应用程序需要真正的通用模型；大多数应用程序受益于针对其特定用例量身定制的特定于任务的知识。因此，开发不仅保持多样性而且针对特定实际场景进行优化的指令增强方法至关重要。

Method: 通过在离散的查询-约束空间中表示指令，TCIA创建了一组丰富的任务相关指令，并使模型能够推广到这些特定于任务的指令，而不会牺牲整体性能。

Result: 实验表明，TCIA将开源LLM在四个实际的、特定于任务的应用程序上的性能平均提高了8.7%，在某些情况下，TCIA的性能优于领先的闭源模型。这些改进不会影响一般的指令遵循能力，使TCIA成为将LLM适应于实际的、以任务为中心的应用程序的可扩展和高效的解决方案。

Conclusion: TCIA框架能够在保持多样性的同时，还能优化特定任务的指令，从而提升LLM在实际应用中的性能。
...
TL;DR: 提出了一种名为熵面积得分（EAS）的简单而有效的指标，用于量化大型语言模型（LLM）推理过程中答案生成的不确定性。


<details>
  <summary>Details</summary>
Motivation: EAS 可以量化大型语言模型（LLM）推理过程中答案生成的不确定性。

Method: EAS 通过整合模型自身的 token 级别预测熵来捕捉生成过程中不确定性的演变，无需外部模型或重复抽样。

Result: EAS 与跨模型和数据集的答案熵密切相关。在训练数据选择中，EAS 识别出具有高潜力的样本，并在相同的样本预算下始终优于 Pass Rate 过滤，从而提高了学生模型在数学基准测试中的准确性。

Conclusion: EAS 既高效又可解释，为 LLM 训练中的不确定性建模和数据质量评估提供了一种实用的工具。
...
Method: Developed AWorld, an open-source system for large-scale agent-environment interaction, distributing tasks across a cluster.

Result: AWorld achieves 14.6x speedup in experience collection; a Qwen3-32B-based agent trained with AWorld significantly outperforms its base model on GAIA, achieving 32.23% overall accuracy and 16.33% on the most challenging levels.

Conclusion: AWorld and the resulting agent provide a blueprint for a complete agentic AI training pipeline.
...
TL;DR: This paper proposes a Governable AI (GAI) framework to address the limitations of existing AI safety approaches when facing AI with extreme motivations and unlimited intelligence. The GAI framework uses cryptographic mechanisms for externally enforced structural compliance.


<details>
  <summary>Details</summary>
Motivation: The increasing security risks posed by AI, especially in critical scenarios, and the limitations of existing AI safety approaches when facing AI with extreme motivations and unlimited intelligence.

Method: The proposed GAI framework consists of a rule enforcement module (REM), governance rules, and a governable secure super-platform (GSSP). REM enforces governance rules, while GSSP ensures non-bypassability, tamper-resistance, and unforgeability.

Result: The paper presents a rigorous formal proof of the security properties of the GAI mechanism and demonstrates its effectiveness through a prototype implementation evaluated in representative high-stakes scenarios.

Conclusion: The decoupling of the governance rules and the technical platform enables a feasible and generalizable technical pathway for the safety governance of AI.
...
TL;DR: 利用大型语言模型生成合成数据，增强健康相关事实核查的训练数据。
...
Method: 我们提出了一个合成数据生成管道，该管道利用大型语言模型 (LLM) 来扩充健康相关事实核查的训练数据。在此管道中，我们总结源文档，将摘要分解为原子事实，并使用 LLM 构建句子-事实蕴涵表。从表中的蕴涵关系中，我们进一步生成带有二元真实性标签的合成文本-声明对。然后将这些合成数据与原始数据相结合，以微调基于 BERT 的事实核查模型。

Result: 在两个公共数据集 PubHealth 和 SciFact 上的评估表明，与仅在原始数据上训练的模型相比，我们的管道将 F1 分数分别提高了 0.019 和 0.049。

Conclusion: 这些结果突出了 LLM 驱动的合成数据增强在提高健康相关事实核查器性能方面的有效性。
...
TL;DR: 本文提出了一种检测MMORPG游戏中自动练级机器人的新框架，该框架利用对比表示学习和聚类技术，以完全无监督的方式识别具有相似升级模式的角色群体。


<details>
  <summary>Details</summary>
Motivation: 自动练级机器人破坏游戏平衡性和公平性，并且由于需要可解释的理由来避免法律和用户体验问题，因此检测此类机器人具有挑战性。

Method: 该方法利用对比表示学习和聚类技术，以完全无监督的方式识别具有相似升级模式的角色群体。此外，还引入大型语言模型（LLM）作为辅助审查员来验证聚类组，并引入基于增长曲线的可视化来辅助LLM和人工审核员评估升级行为。

Result: 该方法提高了机器人检测工作流程的效率，同时保持了可解释性。

Conclusion: 该方法支持MMORPG中可扩展且负责任的机器人监管。
...
TL;DR: AI and Cognitive Science are deeply intertwined, with AI benefiting from cognitive theories and vice versa.


<details>
  <summary>Details</summary>
Motivation: The reciprocal relationship between AI and Cognitive Science motivates a review of their intersections.

Method: Synthesizing key contributions from both AI and Cognitive Science.

Result: AI progress has largely emphasized practical task performance, but its cognitive foundations remain conceptually fragmented.

Conclusion: The future of AI within Cognitive Science lies not only in improving performance but also in constructing systems that deepen our understanding of the human mind. Promising directions include aligning AI behaviors with cognitive frameworks, situating AI in embodiment and culture, developing personalized cognitive models, and rethinking AI ethics through cognitive co-evaluation.
...
TL;DR: 这篇论文提出了一个基于范畴论的新框架，以提高人工智能系统的可解释性，特别是在词嵌入方面。


<details>
  <summary>Details</summary>
Motivation: 旨在解决人工智能系统，特别是词嵌入的可解释性问题，通过范畴论提供更透明的理解。

Method: 使用范畴论构建了LT和PT等类别，将最大概率元素的选择重构为范畴概念，并构建了单oidal类别PT以可视化从文本中提取语义信息的各种方法。定义了配置Conf和词嵌入Emb的类别，以及散度的概念，用于比较词嵌入。

Result: 证明了GloVe和Word2Vec算法与度量MDS算法之间的等价性，从神经网络算法（黑盒）过渡到透明框架。提出了一种在嵌入前计算偏差的数学方法，并提供了在语义空间级别减轻偏差的见解。

Conclusion: 该研究通过范畴论为AI系统的可解释性提供了新的数学框架，并为词嵌入的比较和偏差缓解提供了具体方法。
...
TL;DR: 本文提出了一个新颖的agent框架，用于解决科学计算中的代表性问题，通过结合三个推理LLM，以协作和交互的方式实现“重写-解决-审查-修订”的逻辑链。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）是生成式人工智能中一个活跃且有前途的领域，并已展示出在多个领域执行复杂任务的能力，包括数学和科学推理。本文旨在构建一个能够解决科学计算中代表性问题的agent框架。

Method: 该agent框架包含三个推理LLM，分别作为顾问、审查员和程序员，通过“重写-解决-审查-修订”的逻辑链进行协作和交互。顾问模块负责知识转移，将问题与专业领域见解联系起来，从而通过文本增强重写问题描述。程序员模块负责生成和执行结构良好的代码以实现问题解决。审查员模块通过与代码运行时输出的交互反馈，使agent具备自我调试和自我完善的能力。

Result: 与单模型相比，该协作框架显着提高了无错误代码生成率，并减少了非物理解决方案的出现，从而为基于自然语言描述的自主代码生成建立了一个高度可靠的框架。审查机制提高了最新推理模型的平均执行成功率（无错误代码和非NaN解决方案）。

Conclusion: 该agent框架建立了自动代码生成和审查，是一种有前途的科学计算范例。
...
TL;DR: 提出了一种新的单智能体强化学习框架，用于公共汽车保持控制，该框架通过增强状态空间与分类标识符来避免多智能体强化学习的数据不平衡和收敛问题。


<details>
  <summary>Details</summary>
Motivation: 由于随机交通和乘客需求，公共汽车成群仍然是城市交通面临的挑战。传统的解决方案依赖于环线设置中的多智能体强化学习 (MARL)，但忽略了以异构路线、时间表、波动的需求和不同的车队规模为特征的现实运营。

Method: 通过使用分类标识符（车辆 ID、站点 ID、时间段）以及数值特征（车头时距、占用率、速度）来增强状态空间，从而将多智能体问题重新构建为单智能体问题。设计了一个与运营目标相一致的结构化奖励函数：使用岭状奖励来平衡均匀的车头时距和遵守时间表。

Result: 改进的软演员-评论家 (SAC) 比基准测试实现了更稳定和卓越的性能，包括 MADDPG（例如，在随机条件下为 -430k 对 -530k）。

Conclusion: 单智能体深度强化学习，当通过分类结构和计划感知奖励增强时，可以有效地管理非循环、现实环境中的公共汽车保持。这种范例为 MARL 框架提供了一种稳健、可扩展的替代方案，特别是在特定于代理的经验不平衡的情况下。
...
TL;DR: 本文介绍了一个医学指南的动态、系统基准测试原型， 包含400+问题，3.3+万亿种可能的组合，覆盖了100%的指南关系。


<details>
  <summary>Details</summary>
Motivation: 为了解决通用领域评估中存在的特定能力差距，并克服手动管理基准测试的覆盖范围限制。

Method: 将WHO的IMCI手册转换为有向图，使用图遍历生成问题，这些问题结合了特定年龄段的场景和上下文干扰。

Result: 模型在症状识别方面表现出色，但在分诊严重程度、治疗方案和后续护理方面表现不佳，证明了定制基准测试可以识别通用领域评估遗漏的特定能力差距。

Conclusion: 该方法是创建可动态生成的综合基准测试的可扩展、防污染解决方案，包括指南更新时。
...
TL;DR: 提出了一种多目标遗传算法 (MOO-GA) 来解决医疗保健领域的人员调度问题，该问题需要在控制劳动力成本、满足患者需求和员工满意度之间取得平衡。


<details>
  <summary>Details</summary>
Motivation: 医疗保健领域的人员调度是一个重要的运营挑战，需要平衡控制劳动力成本、确保足够的员工来满足患者需求以及适应员工的偏好以减轻倦怠感等相互竞争的目标。

Method: 提出一种多目标遗传算法 (MOO-GA)，将医院部门的人员调度问题建模为多目标优化任务。该模型结合了实际的复杂性，包括每小时预约驱动的需求和多技能员工的模块化轮班的使用。通过定义成本、患者护理覆盖率和员工满意度的目标函数，遗传算法可以找到一组高质量的非支配解。

Result: 在代表典型医院部门的数据集上进行演示，结果表明 MOO-GA 生成了稳健且平衡的计划表。平均而言，该算法生成的计划表比模拟传统手动调度过程的基线提高了 66%。

Conclusion: 该方法有效地管理了关键运营目标和以员工为中心的目标之间的权衡，为护士经理和医院管理人员提供了实用的决策支持工具。
...
TL;DR: 提出了一种新的神经符号架构和损失函数，用于学习如何解决NP-hard推理问题。


<details>
  <summary>Details</summary>
Motivation: 探索混合离散推理与神经网络，解决大型语言模型难以处理的离散推理或优化问题。

Method: 新的概率损失函数允许学习约束和目标，将组合求解器移出训练循环，实现可扩展训练和最大精度。

Result: 在数独基准测试和视觉Min-Cut/Max-cut任务中，该方法优于其他混合方法，并能有效地学习蛋白质设计中的能量优化公式。

Conclusion: 该方法能有效地学习如何从自然输入中解决NP-hard推理问题。
...
TL;DR: 本研究介绍了一种名为ChatThero的多智能体对话框架，旨在通过结合认知行为疗法（CBT）和动机访谈（MI）来改善药物滥用障碍（SUDs）的治疗效果。


<details>
  <summary>Details</summary>
Motivation: 现有药物滥用障碍的治疗由于社会偏见、动机障碍和个性化支持不足而效果有限。大型语言模型在心理健康辅助方面有潜力，但缺乏与临床验证策略的紧密结合。

Method: ChatThero采用动态患者建模和上下文敏感的治疗对话，并结合基于CBT和MI的自适应说服策略。研究者构建了一个包含不同难度级别的合成基准，并使用监督微调（SFT）和直接偏好优化（DPO）的两阶段流程训练ChatThero。

Result: ChatThero在患者动机方面平均提高了41.5%，治疗信心提高了0.49%，解决困难案例所需的轮数比GPT-4o减少了26%。自动化和人工临床评估均表明，ChatThero在共情、响应性和行为真实性方面表现更佳。

Conclusion: ChatThero框架为治疗性对话的研究提供了严谨且保护隐私的方法，并为研究和临床转化提供了可靠的基础。
...
TL;DR: 提出了一种无分叉区块链数据库，与基于geth的Fantom区块链客户端相比，存储提高了100倍，吞吐量提高了10倍。


<details>
  <summary>Details</summary>
Motivation: L1区块链中的运营节点成本仍然很高，其中一个资源密集型组件是区块链数据库（StateDB）。

Method: 提出一种无分叉区块链数据库。

Result: 与基于geth的Fantom区块链客户端相比，存储提高了100倍，吞吐量提高了10倍。

Conclusion: 无分叉区块链数据库在存储和吞吐量方面有显著改进。
...
TL;DR: 本文探讨了在 SQL 中调用 LLM 的集成，发现现有开源解决方案在功能和性能上存在局限性。通过对两个开源系统和一个企业平台进行早期探索，作者识别了三个主要问题：强制结构化输出、优化资源利用和改进查询规划。初步解决方案的实施表明，更紧密的 LLM+DBMS 集成是扩展和高效处理 LLM 查询的关键。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）已成为文本摘要、情感分析和自动问答等应用的关键。最近，LLM 也被集成到关系数据库管理系统中，以增强查询和支持高级数据处理。为了提升数据洞察力，像 Amazon、Databricks、Google 和 Snowflake 这样的公司直接在 SQL 中提供 LLM 调用（表示为 LLM 查询）。

Method: 本文使用了五个代表性查询来评估两个开源系统和一个企业平台，以此揭示当前 SQL 调用的 LLM 集成在功能、性能和可扩展性方面的局限性。作者针对识别出的问题实施了初步解决方案。

Result: 作者识别了三个主要问题：强制结构化输出、优化资源利用和改进查询规划。初步解决方案的实施观察到在处理 LLM 驱动的 SQL 查询方面的改进。

Conclusion: 更紧密的 LLM+DBMS 集成是扩展和高效处理 LLM 查询的关键。
...
<details>
  <summary>Details</summary>
Motivation: 在金融、医疗保健和电子商务等领域，数据已成为驱动创新的基础资产。在这些领域，通常采用基于关系表的预测建模，并且越来越强调通过自动机器学习 (AutoML) 技术来减少人工工作。这提出了一个有趣的问题：特征增强本身可以自动化，并且可以识别和利用与任务相关的关系信号吗？

Method: ReCoGNN 首先通过对表内属性关系进行建模来捕获每个表内的语义依赖关系，从而能够将表划分为结构化的、语义上连贯的段。然后，它构建一个异构加权图，表示所有段之间的行间关系。最后，ReCoGNN 利用消息传递图神经网络通过图传播信息，指导特征选择并增强原始数据集。
...
TL;DR: 本研究全面回顾了情感推荐系统，根据Scherer的情感状态类型，将其分为态度感知、情绪感知、心情感知和混合型四类，总结了情感信号提取技术、系统架构和应用领域，强调了混合模型、大规模情感数据集的开发，以及使用更精确的术语的必要性。


<details>
  <summary>Details</summary>
Motivation: 现有情感推荐系统综述缺乏心理学基础的组织分类，且研究范围有限。

Method: 通过系统性回顾情感推荐系统，并结合Scherer的情感状态类型，提出了一种新的分类方案。

Result: 对情感推荐系统进行了全面的分类和总结，并指出了关键趋势、局限性和挑战。

Conclusion: 本研究旨在为情感驱动的个性化推荐提供全面的参考和指导，以促进学术研究和行业应用的发展。
...
TL;DR: ELIXIR是一个多任务模型，它结合了评分预测和个性化评论生成，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法无法充分利用预训练Transformer模型的能力，并且忽略了方面建模，而方面建模对于个性化解释至关重要。

Method: ELIXIR联合学习用户和项目的全局和方面特定表示，优化总体评分、方面级别评分和评论生成，并通过个性化注意力来强调方面重要性。该模型基于T5-small (60M)模型。

Result: 在TripAdvisor和RateBeer上的实验结果表明，ELIXIR显著优于强大的基线模型，尤其是在评论生成方面。

Conclusion: ELIXIR通过其基于方面的架构，在个性化上下文中有效地指导文本生成，优于使用更大模型的现有方法。
...
TL;DR: 提出了一种新的多模态推荐框架，以解决现有音乐推荐系统中存在的语义退化和模态间建模差距问题。


<details>
  <summary>Details</summary>
Motivation: 现有的基于语义ID的多模态音乐推荐方法存在两个局限性：1）模态内语义退化，离散ID与原始内容语义逐渐分离；2）模态间建模差距，传统融合策略忽略模态特定细节或未能捕获跨模态相关性，阻碍了全面的用户兴趣建模。

Method: 提出了一种新的多模态推荐框架，包含两个阶段：1）渐进式语义残差量化（PSRQ）方法，通过显式保留前缀语义特征来生成模态特定和模态联合语义ID；2）多码本交叉注意力（MCCA）网络，旨在使模型能够同时捕获模态特定兴趣并感知跨模态相关性。

Result: 在多个真实世界数据集上的大量实验表明，该框架优于最先进的基线模型。该框架已部署在中国最大的音乐流媒体平台之一，在线A/B测试证实商业指标的显着改进。
...
TL;DR: 本文研究了现代信息检索系统中查询建议的偏见问题，并提出了BalancedQR的扩展版本来解决多维度偏见。


<details>
  <summary>Details</summary>
Motivation: 现代信息检索系统在提供高质量结果的同时，存在对性别或种族等受保护类别产生不良或错误偏见的风险。

Method: 本文采用Pareto front方法，寻找平衡的查询，优化多个目标，如性别偏见和地域偏见，以及返回结果的相关性。在维基百科数据集上评估了BalancedQR的扩展版本。

Result: 实验结果证明了BalancedQR框架扩展的有效性，并强调了微妙的查询措辞和语言选择对检索的重大影响。

Conclusion: 本文成功地扩展了BalancedQR框架，使其能够处理多维度偏见，并验证了其在实际数据集上的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 解决推荐系统中检索阶段的单目标建模与排序阶段的多目标优化之间的语义差距，以及传统多目标检索方法导致的资源线性增长和处理松散耦合目标时的局限性。

Method: 1. 采用目标条件Transformer，通过可学习的注意力调制联合编码用户行为序列和多任务语义；2. 引入个性化目标权重，实现检索结果的动态调整；3. 将用户个性化信息融入到token表示和Transformer结构中，增强模型表示能力。

Result: 在快手短视频推荐系统中成功集成，稳定服务于超过4亿日活跃用户，显著提高用户日参与度和系统运营效率。

Conclusion: MPFormer在保持服务响应速度的同时，有效优化了多目标检索的迭代模式，为工业推荐系统提供了一个可扩展的多目标解决方案。
...
TL;DR: LLMs are used for recommendation but may be unfair in cold-start scenarios due to biases.


<details>
  <summary>Details</summary>
Motivation: Fairness concerns arise in cold-start recommendation with LLMs due to societal biases.

Method: A benchmark is introduced to evaluate fairness in zero-context recommendation.

Result: Consistent biases are found across recommendation domains and a non-linear relationship between model size and fairness is revealed.

Conclusion: Nuanced analysis is needed to address fairness concerns in LLMs for recommendation.
...
TL;DR: 对比学习引入了一种权衡：语义检索的提升是以参数化事实知识的损失为代价的。


<details>
  <summary>Details</summary>
Motivation: 检索增强生成(RAG)管道中，准确检索事实信息对于维护系统可信度和防御RAG中毒至关重要。然而，关于这些组件从它们所基于的大型语言模型(llm)继承或损失了多少事实能力，我们知之甚少。

Method: 将12个公开发布的嵌入检查点与其原始基础llm配对，并在事实基准上评估两组。

Result: 在评估的每个模型中，嵌入变体的准确率明显低于其基础模型，绝对下降范围为12到43个百分点(中值为28个百分点)，典型的检索器准确率下降到25-35%的范围，而生成模型的准确率为60-70%。
...
TL;DR: 提出了一种新的多行为序列推荐模型PDB4Rec，该模型通过提取不同频率的信息并平衡它们对推荐性能的贡献来提高推荐性能。


<details>
  <summary>Details</summary>
Motivation: 多行为序列推荐旨在综合考虑用户的不同行为以提高目标行为的推荐性能。然而，一些行为数据也会给用户兴趣建模带来噪音。以往的研究表明低频信息往往是有价值和可靠的，而高频信息通常与噪声相关联。但是，高频信息并非完全没有意义。

Method: 提出PDB4Rec模型，该模型有效地提取各种频段的信息及其关系，并引入Bootstrap Balancer机制来平衡它们对提高推荐性能的贡献。
...
TL;DR: 本文探讨了推荐系统对旅游业的影响，并强调了计算机科学界对现实世界中复杂关系和权衡的理解不足，导致算法可能会对环境、当地社区或其他利益相关者产生负面影响。


<details>
  <summary>Details</summary>
Motivation: 动机是现有算法推荐系统在旅游业中可能会对环境、当地社区或其他利益相关者产生负面影响，因为计算机科学界对现实世界中复杂关系和权衡的理解不足。

Method: 通过半系统的文献综述，整合了旅游管理和计算机科学的文献。

Result: 研究结果表明，旅游管理积极尝试识别利益相关者的特定需求，并利用定性、包容和参与性方法，从规范和整体的研究角度研究公平性。相比之下，计算机科学缺乏对利益相关者需求的充分理解，主要通过可衡量的歧视等描述性因素来考虑公平性，同时严重依赖于少数未能捕捉到旅游业公平性的多维性的数学形式化公平性标准。

Conclusion: 本文旨在说明纯粹算法研究的缺点，并强调未来跨学科合作的潜力和特殊需求，以增强算法决策支持系统，从而理解和支持旅游业中真正的多方利益相关者公平性。
...
TL;DR: 这篇论文提出了一种新的文档检索系统，该系统通过结合领域信息来改进语义感知知识表示和数据访问，从而提高检索的准确性和精确性。


<details>
  <summary>Details</summary>
Motivation: 现有的语义检索系统由于缺乏领域特定信息和依赖过时的知识来源，其精确性可能会受到影响。

Method: 开发了一种名为“基于语义的概念检索使用Group Steiner Tree”的通用算法，该算法结合了领域信息以增强语义感知知识表示和数据访问。

Result: 实验结果表明，与基线系统相比，该系统取得了显著的进步，精确率和准确率分别达到了90%和82%，这意味着有希望的改进。

Conclusion: 该研究表明，结合领域信息的语义检索系统可以显著提高文档检索的性能。
...
TL;DR: 本文提出了一种名为SUMMA的多模态模型，用于自动处理视频广告，生成突出商业价值内容的摘要，从而提高抖音搜索广告系统中广告的理解和排序。


<details>
  <summary>Details</summary>
Motivation: 为了解决高商业价值的多模态信息有效利用不足的问题，现有方法过度依赖于高度压缩的视频嵌入。

Method: 该模型通过两阶段训练策略开发：多模态监督微调，然后通过混合奖励机制进行强化学习。使用包含视频帧和ASR/OCR转录的领域特定数据。

Result: 离线和在线实验表明，与基线相比有显着改进，在线结果表明广告收入统计显着增加了1.5%。

Conclusion: 这项工作建立了一种将多模态信息压缩成代表性文本的新范例，有效地将视觉广告内容与检索和推荐场景中的用户查询意图对齐。
...
TL;DR: 本文探讨了如何利用大型语言模型（LLMs）改进基于会话的推荐系统（SR）。


<details>
  <summary>Details</summary>
Motivation: 现有的SR模型通常未能充分利用item的语义信息，导致难以识别会话意图和缺乏可解释性。Prompt方法难以找到最优提示，缺乏任务反馈，而微调方法计算成本高昂。

Method: 本文提出了三种方法：(i) 将LLMs作为推荐代理，(ii) 使用LLM生成表示来初始化深度学习SR模型，(iii) 将LLMs与数据驱动的SR模型集成。

Result: 实验表明，LLM方法在粗粒度检索方面表现出色，而传统数据驱动技术在细粒度排序方面表现出色。LLMs与数据驱动的SR模型集成显著优于单独的LLM方法和数据驱动的深度学习模型。

Conclusion: LLM与数据驱动的SR模型集成的方法显著提升了推荐效果。
...
Method: 该方法利用结构感知学习来保持语义层次结构，并使用掩码元素对齐来实现细粒度的语义区分。

Result: 在已发布和工业数据集上的大量实验以及在线 A/B 测试表明，性能持续提升，在 BGE-M3 上，NDCG@10 从 73.96% 提高到 77.84%。

Conclusion: 论文提出了SEAL框架和数据集，有效提升了长结构化文档检索的性能。
...
TL;DR: 本文介绍了一种用户感知的ULTR框架，旨在解决现有ULTR方法忽略用户搜索和浏览行为差异的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的ULTR方法忽略了不同用户在搜索和浏览行为上的差异，导致学习排序模型时存在偏差。

Method: 提出了一种新的用户感知的逆倾向得分估计器，该方法对每个查询的用户浏览行为分布进行建模，并聚合用户加权检查概率以确定倾向。

Result: 在半合成数据集和真实数据集上的大量实验验证了该用户感知估计器的有效性。

Conclusion: 该研究提出了一种用户感知的ULTR框架，并通过实验验证了其有效性，解决了现有方法中存在的用户差异性问题。
...
TL;DR: 本文提出了一种名为深度多重量化网络（DMQN）的模型，用于处理CTR预测中的长行为序列，通过量化压缩长行为序列，减少目标注意力相关性分布的差异，提高CTR预测的准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 现有方法在检索项目和提取兴趣强度向量时，目标注意力的相关性分布与完整的长行为序列之间存在差异，导致性能下降。

Method: 该方法使用深度多重量化网络（DMQN）对长行为序列进行端到端处理，通过多个独立码本将长行为序列量化为多个码字序列，并使用分层序列转换单元促进码字序列的交互，最后通过候选项目与码字序列之间的注意力输出兴趣向量。

Result: 在工业和公共数据集上的大量实验表明，DMQN是有效和高效的。在广告系统中的A/B测试表明，DMQN将CTR提高了3.5%，RPM提高了2.0%。

Conclusion: DMQN模型能够有效提高CTR预测的准确性和效率，并在实际广告系统中取得了显著的性能提升。
...
TL;DR: OneRec-V2通过优化计算分配和结合真实用户交互来改进了生成式推荐系统OneRec-V1，提高了效率和性能。


<details>
  <summary>Details</summary>
Motivation: OneRec-V1虽然成功，但存在计算资源分配不均和强化学习依赖奖励模型的问题，限制了其可扩展性和性能。

Method: 提出了OneRec-V2，采用Lazy Decoder-Only架构消除编码器瓶颈，并结合Duration-Aware Reward Shaping和Adaptive Ratio Clipping来更好地与用户偏好对齐。

Result: 在快手A/B测试中，OneRec-V2将App停留时间提高了0.467%/0.741%，同时平衡了多目标推荐。

Conclusion: OneRec-V2提升了生成式推荐的可扩展性，并更好地与真实用户反馈对齐，是端到端推荐系统发展的重要一步。
...
TL;DR: 本文提出了一种新的多域推荐系统方法，通过替换完整的域间注意力机制，以解决transformer架构在多域场景中的计算瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 传统的深度学习推荐模型在多域场景中面临计算瓶颈，尤其是在transformer架构的注意力机制方面，因为它们需要关注所有域的项目。

Method: 本文提出了两种创新机制：1) 转换感知位置嵌入（TAPE），通过考虑域转换特定信息来聚焦于域内项目；2) 动态域状态表示（DDSR），为每个域引入动态状态表示，以有效传递相关域信息，而无需依赖完整的注意力图。

Result: 该方法在检索任务中表现出显著的改进，通过分别建模和组合域间和域内表示。

Conclusion: 本文提出的方法为大规模多域推荐系统提供了一个可扩展的解决方案，并在检索任务中取得了显著的改进。
...
TL;DR: 向量嵌入在各种检索任务中得到应用，但也面临理论限制。


<details>
  <summary>Details</summary>
Motivation: 先前的研究认为向量嵌入的困难是由于不切实际的查询造成的，可以通过更好的训练数据和更大的模型来克服。然而，这项工作表明，在现实环境中，即使使用非常简单的查询，我们也可能遇到这些理论限制。

Method: 将学习理论中的已知结果联系起来，表明可以作为某些查询结果返回的文档的 top-k 子集的数量受到嵌入维数的限制。即使我们限制为 k=2，并通过自由参数化嵌入直接在测试集上进行优化，我们也可以通过经验表明这一点。

Result: 即使是最先进的模型，在这个数据集上也会失败，尽管任务的性质很简单。

Conclusion: 这项工作展示了现有单向量范式下嵌入模型的局限性，并呼吁未来的研究开发能够解决这一根本限制的方法。
...
TL;DR: 提出 CrystalICL，一个用于少样本晶体生成的新模型。


<details>
  <summary>Details</summary>
Motivation: 现有基于 LLM 的晶体生成方法仅限于零样本场景，无法从少样本场景中受益。人类专家通常通过修改相关的已知结构来设计新材料，这与少样本 ICL 范式非常吻合。

Method: 提出了一种基于空间群的晶体标记化方法，有效地降低了在 LLM 中建模晶体对称性的复杂性。 此外，还引入了一种条件结构感知混合指令调优框架和一种多任务指令调优策略，使模型能够通过从有限的数据中捕获结构-性质关系来更好地利用 ICL。

Result: 在四个晶体生成基准上的大量实验表明，CrystalICL 在条件和无条件生成任务上优于领先的基线方法。

Conclusion: CrystalICL 在晶体生成任务上表现出色，优于现有方法。
...
TL;DR: 本文研究了在基于 Transformer 的模型中加入可学习滤波器，以改善长时间序列预测的性能。


<details>
  <summary>Details</summary>
Motivation: 基于 Transformer 的模型在长时间序列预测中表现出色，但存在对低频数据有偏差以及计算和内存需求高的问题。本文旨在解决这些问题。

Method: 在基于 Transformer 的模型的开头添加可学习滤波器，该滤波器仅增加约 1000 个参数。

Result: 在多个实例中观察到 5-10% 的预测性能相对提升。此外，通过添加滤波器，能够减小模型的嵌入维度，从而产生比非滤波基础模型更小、更有效的基于 Transformer 的架构。

Conclusion: 添加滤波器可以提高基于 Transformer 的模型在长时间序列预测中的性能，并使其更好地利用频谱进行预测。
...
TL;DR: Transformer是广义的非线性预测器，通过非线性组合过去的token来预测下一个token。


<details>
  <summary>Details</summary>
Motivation: 将Transformer信号解释为条件测度的替代，并将层操作解释为不动点更新。

Method: 构建概率模型，将Transformer信号解释为条件测度的替代，并将层操作解释为不动点更新。针对概率模型为隐马尔可夫模型（HMM）的特殊情况，描述了不动点更新的显式形式。

Result: 论文提出了一个概率模型，将Transformer信号解释为条件测度的替代，并将层操作解释为不动点更新。

Conclusion: 本文旨在将经典非线性滤波理论与现代推理架构联系起来。
...
TL;DR: 知识蒸馏(KD)是一种有效的模型压缩技术，但其成功因素尚未完全清楚。本文揭示了教师模型的校准误差与学生的准确率之间存在很强的相关性。


<details>
  <summary>Details</summary>
Motivation: 研究知识蒸馏(KD)中哪些因素有助于提高学生的表现，并发现教师模型的校准误差与学生的准确率之间存在相关性。

Method: 通过减少教师模型的校准误差来提高知识蒸馏的性能。

Result: 该算法具有通用性，在从分类到检测的各种任务中都表现出有效性。此外，它可以很容易地与现有的技术集成，始终获得优越的性能。

Conclusion: 教师模型的校准是有效知识蒸馏的一个重要因素，并且可以通过简单地采用校准方法来减少教师的校准误差，从而提高知识蒸馏的性能。
...
TL;DR: 提出了一种名为 Correlation of Loss Differences (CLD) 的简单且可扩展的 coreset 选择指标，用于识别最具影响力的训练样本。
...
Method: 通过测量训练样本与 held-out 验证集的损失轨迹的对齐程度来识别最具影响力的训练样本。CLD 只需要在训练检查点计算的每个样本的损失值，避免了许多现有子集选择方法中使用的昂贵的梯度和曲率计算。

Result: 在 CIFAR-100 和 ImageNet-1k 上，基于 CLD 的 coreset 通常优于或非常接近最先进的方法，并且即使不领先，也保持在计算成本更高的基线方法的 1% 以内。CLD 能够有效地跨架构（ResNet、VGG、DenseNet）迁移，从而实现 <1% 降级的代理到目标选择。而且，CLD 在仅使用早期检查点时是稳定的，只会产生可忽略不计的精度损失。最后，CLD 通过每个类的验证对齐来表现出固有的偏差减少，从而无需额外的分层抽样。
...
TL;DR: 本文研究了转导节点分类问题，该问题中底层图由与节点标签和节点特征一致的社区组成。提出了一个新的优化问题，将节点特定信息（标签和特征）融入到谱图聚类框架中。
...
Method: 提出了一个新的优化问题，将节点特定信息融入到谱图聚类框架中。

Result: 合适的节点特定信息保证了优化问题的解能够完美地恢复社区，其条件比单独的图聚类边界更温和。

Conclusion: 算法解决方案和数值实验证实了这种协同作用。
...
TL;DR: 本文提出了一种名为INS2ANE的新框架，旨在增强自主实验中新现象的发现，通过整合新颖性评分系统和战略抽样机制，提高实验结果的多样性。


<details>
  <summary>Details</summary>
Motivation: 当前自主实验主要集中于优化预定义目标，限制了意外或未知物理现象的发现。

Method: 该方法整合了两个关键组成部分：(1) 评估实验结果独特性的新颖性评分系统，以及 (2) 即使根据传统标准不太有希望，也能促进对欠采样区域进行探索的战略抽样机制。

Result: 在包含图像-光谱对的预先获取的数据集以及自主扫描探针显微镜实验中验证了该方法。结果表明，与传统的优化程序相比，INS2ANE 显著提高了所探索现象的多样性。

Conclusion: INS2ANE 具有增强科学发现深度的潜力，结合自主实验的效率，有望通过同时导航复杂的实验空间以发现新现象来加速科学研究。
...
TL;DR: 这篇论文研究了使用符号回归方法从数据中发现方程的过程，特别是在动态系统建模中的应用。


<details>
  <summary>Details</summary>
Motivation: 比较不同的符号回归方法在恢复描述复杂现象的动态系统方程方面的性能。

Method: 使用了五种符号回归方法，对包括混沌动力学和流行病模型在内的九个动态过程进行方程恢复。

Result: PySR 方法被证明是最适合推断方程的，具有高预测能力和准确性，一些估计与原始解析形式无法区分。

Conclusion: 符号回归作为一种强大的工具，具有推断和建模现实世界现象的潜力。
...
TL;DR: 这篇论文提出了一种新的框架，将潜在变量模型集成到双重机器学习（DML）范例中，以在存在隐藏因素的情况下实现稳健的因果效应估计。


<details>
  <summary>Details</summary>
Motivation: 在因果推断中，潜在变量模型有助于解释影响治疗或结果的隐藏因素，从而应对由缺失或未测量的协变量带来的挑战。

Method: 该方法将潜在变量纳入DML的第二阶段，将表征学习与潜在推断分离。考虑两种情况：一种是潜在变量仅影响结果，另一种是潜在变量可能影响治疗和结果。

Result: 通过在合成和真实世界数据集上的大量实验证明了该方法的稳健性和有效性。

Conclusion: 该论文提出了一种新的因果推断方法，该方法在存在隐藏因素的情况下是稳健和有效的。
...
TL;DR: 本文介绍了一种轻量级的、领域相关的AI模型，用于预测撒哈拉以南非洲自然通风的学校和家庭的室内温度。


<details>
  <summary>Details</summary>
Motivation: 在资源有限的环境中，对室内温度进行预测，以实现热舒适管理。

Method: 该模型扩展了Temp-AI-Estimator框架，使用坦桑尼亚学校的数据进行训练，并在尼日利亚学校和冈比亚家庭中进行评估。

Result: 在尼日利亚学校的平均绝对误差为1.45{\deg}C，在冈比亚家庭的平均绝对误差为0.65{\deg}C，实现了稳健的跨国性能。

Conclusion: 研究结果突出了AI在资源受限环境中的热舒适管理潜力。
...
TL;DR: 本综述探讨了大型语言模型（LLM）在遗传病研究和诊断中的作用，包括罕见病和常见病。


<details>
  <summary>Details</summary>
Motivation: 传统方法难以处理复杂、高维的遗传数据，而LLM在理解非结构化医疗数据方面表现出色。

Method: 通过在PubMed等数据库中进行关键词搜索，筛选出172项关于LLM在遗传学诊断和教育应用的研究进行分析。

Result: LLM在基因组变异识别、注释和解释以及医学影像方面有所进展，但在整合多模态数据和临床应用方面仍面临挑战。

Conclusion: LLM显著推动了疾病和风险分层、变异解释、医学影像分析和报告生成，但其在遗传病诊断和遗传教育领域仍存在局限性，本综述旨在为该领域提供指导。
...
TL;DR: 这篇论文介绍了一种新的目标函数度量标准VC（值变化），用于衡量神经网络近似任务的难度和近似影响。


<details>
  <summary>Details</summary>
Motivation: 神经网络的局部性能通常难以预测，这会影响它们在关键应用中的可靠性。VC通过提供网络行为中局部值变化的可量化度量来解决这个问题，从而深入了解实现神经网络近似的稳定性和性能。

Method: 研究了VC的一些基本理论性质，并提出了基于VC的一种新度量标准，该标准从变异的角度衡量两个函数之间的距离。在此基础上，进一步提出了一种新的神经网络近似预处理框架。

Result: 数值结果（包括真实世界的实验和与PDE相关的科学问题）支持了论文的发现和预处理加速方法。
...
TL;DR: 提出了一种名为 Beacon 的简单有效的算法，无需手动调整即可直接执行每通道 PTQ。


<details>
  <summary>Details</summary>
Motivation: 在每通道训练后量化 (PTQ) 中，一个关键挑战是选择合适的缩放因子，以用缩放量化网格中的值替换权重值。现有方法通常通过启发式调整或网格搜索在一开始就确定比例。

Method: Beacon 算法直接使用固定的非缩放字母表执行每通道 PTQ，并通过利用对称标量量化的几何结构自动确定最佳缩放因子。它支持对称和非对称量化，只需进行最小的修改，并且不依赖于反向传播或大型校准集。

Result: Beacon 算法实现了与最先进方法相比具有竞争力的性能。

Conclusion: Beacon 算法是一种高效模型部署的实用解决方案，因为它简单且无需调整。
...
TL;DR: 提出了一种名为 DALI 的框架，用于在 cMDP 中进行零样本泛化，该框架通过从 agent-environment 交互中推断潜在上下文表示，从而适应看不见的环境条件。


<details>
  <summary>Details</summary>
Motivation: 现实世界的强化学习需要在没有昂贵再训练的情况下适应看不见的环境条件。现有的 cMDP 方法通常需要显式的上下文变量，限制了它们在上下文潜在或难以测量时的使用。

Method: DALI 框架集成在 Dreamer 架构中，通过训练自监督编码器来预测前向动态，从而生成可操作的表示，调节世界模型和策略，桥接感知和控制。

Result: 在具有挑战性的 cMDP 基准测试中，DALI 优于不了解上下文的基线，并且通常在推断任务中超过了解上下文的基线，从而实现了对看不见的上下文变化的零样本泛化。

Conclusion: DALI 的潜在空间实现了反事实一致性：扰动重力编码维度会以物理上合理的方式改变想象的 rollouts。理论证明，这种编码器对于有效的上下文推断和鲁棒的泛化至关重要。
...
TL;DR: 提出了一种新的联邦学习方法，称为联邦表征微调（FedReFT），通过操纵隐藏层表征来进行模型微调，以应对联邦学习中客户数据分布、模型能力和计算资源的异构性带来的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有的参数高效微调方法（PEFT）和表征微调方法（ReFT）在联邦学习中面临挑战，特别是当客户数据存在异构性时。

Method: 提出了FedReFT，它使用稀疏干预层来直接引导隐藏层表征。为了解决表征层更新在任务异构性下容易出现的聚合不匹配问题，提出了All-But-Me（ABM）聚合方法。

Result: 在常识推理、算术推理、指令调整和GLUE等任务上的评估表明，FedReFT始终优于联邦学习中最先进的PEFT方法，与领先的基于LoRA的方法相比，参数效率提高了7倍-15倍。

Conclusion: FedReFT 是一种轻量级且语义丰富的微调替代方案，非常适合边缘设备，并且通过ABM聚合实现了稳定和个性化的学习。
...
TL;DR: 本文概述了多智能体强化学习（MARL）在智能交通系统（ITS）中的应用。


<details>
  <summary>Details</summary>
Motivation: 城市交通日益复杂，需要高效、可持续和自适应的解决方案，这使得智能交通系统（ITS）处于现代基础设施创新的前沿。ITS 的核心挑战是在动态、大规模和不确定的环境中进行自主决策，多个智能体（交通信号灯、自动驾驶汽车或车队单元）必须有效地协调。

Method: 本文对 ITS 中 MARL 的应用进行了全面的调查。介绍了一个结构化的分类法，该分类法根据协调模型和学习算法对 MARL 方法进行分类，涵盖基于价值、基于策略、Actor-Critic 和通信增强框架。

Result: 回顾了关键 ITS 领域的应用，包括交通信号控制、互联和自动驾驶车辆协调、物流优化和按需出行系统。此外，还重点介绍了广泛使用的仿真平台，如 SUMO、CARLA 和 CityFlow，它们支持 MARL 实验，以及新兴的基准。

Conclusion: 确定了核心挑战，包括可扩展性、非平稳性、信用分配、通信约束以及阻碍现实世界部署的 Sim2Real 差距。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法存在结构性限制，通常由于依赖少数管理者的狭隘视角而忽略了合格的候选人。

Method: 使用带有门控机制的双图卷积网络 (GCN) 自适应地融合这些维度。

Result: 所提出的基于门控的融合模型显着优于其他融合策略和启发式基线，在 Hit@100 上实现了 40.9% 的最高性能。

Conclusion: 该研究提供了一个定量和全面的内部人才发现框架，最大限度地降低了传统方法中固有的候选人遗漏风险。
...
<details>
  <summary>Details</summary>
Motivation: Learning-based combinatorial optimization requires extensive training data and lacks generalization across problem distributions.

Method: Unsupervised pre-training of a vector-quantized graph autoencoder on diverse MIP instances.

Result: Forge embeddings effectively differentiate/cluster unseen instances and improve a commercial solver's performance via warm-starts and integrality gap predictions.

Conclusion: Forge offers a generalizable approach to learning instance-level MIP embeddings, enhancing optimization solver performance and encouraging further research.
...
TL;DR: 该论文提出了一种名为 Subversive Alignment Injection (SAI) 的新型数据中毒攻击，该攻击利用 LLM 的对齐机制来诱导模型拒绝回答特定主题或查询，从而在不影响模型对无关主题的响应的情况下注入偏差或实施有针对性的审查。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型 (LLM) 通过训练来拒绝回答有害或不安全的提示，从而满足道德标准和安全要求。然而，这种对齐也可能被恶意利用。

Method: 该论文提出了一种名为 Subversive Alignment Injection (SAI) 的中毒攻击，该攻击利用对齐机制来触发对特定主题或查询的拒绝。通过精心设计的恶意数据，攻击者可以使模型在面对特定问题时拒绝回答。

Result: 实验表明，SAI 攻击能够有效地注入偏差，并且可以绕过现有的中毒防御机制。在 ChatDoctor 等聊天应用中，仅需 1% 的数据中毒，系统就会拒绝回答针对特定种族类别的医疗保健问题，导致显著的偏差。此外，该攻击还可以在其他 NLP 任务中诱导偏差，例如简历筛选。

Conclusion: 该研究揭示了 LLM 对齐机制的潜在安全风险，并强调了开发更强大的防御机制以防止此类攻击的重要性。
...
Method: The authors build an open, fully documented simulator that mimics a typical large-scale geo roll-out and evaluate seven estimators: three standard Augmented SCM (ASC) variants and four panel-DML flavors under five stylized stress tests.

Result: ASC models consistently demonstrate severe bias and near-zero coverage in challenging scenarios. Panel-DML variants dramatically reduce this bias and restore nominal 95%-CI coverage, proving far more robust.
...
TL;DR: 提出了一种新的自适应脑电分割方法CTXSEG，它可以创建基于脑电数据统计差异的可变长度片段，并探索了将其与通常需要固定长度输入的现代机器学习方法一起使用的方法。


<details>
  <summary>Details</summary>
Motivation: 当前简单的脑电信号处理方法使用任意固定时间片，但由于大脑状态不局限于固定时间间隔，因此可能具有有限的生物学相关性。研究自适应分割方法是否有利于机器学习脑电分析。

Method: 引入了一种新的自适应分割方法CTXSEG，该方法基于脑电数据的统计差异创建可变长度的片段，并提出了将其与现代机器学习方法一起使用的方法。使用可控合成数据和真实的脑电癫痫检测问题评估CTXSEG。

Result: 使用CTXSEG准备脑电数据可以提高癫痫检测性能，与固定长度方法相比，在标准化框架下评估时，无需修改机器学习方法，并且需要的片段更少。

Conclusion: 自适应分割与CTXSEG可以很容易地应用于现代机器学习方法，有可能提高性能。它是一种有前途的信号预处理固定长度分割替代方案，应被视为脑电机器学习应用中标准预处理的一部分。
...
TL;DR: 本文研究了对称矩阵分解问题中梯度流(GF)的增量学习行为，并对其进行了定量理解。


<details>
  <summary>Details</summary>
Motivation: 探究神经网络的优异性能与其训练中一阶优化算法所产生的隐式偏差或正则化之间的关系。例如，在过度参数化的矩阵分解问题中，使用小初始化进行梯度流(GF)训练时，会观察到增量学习现象。

Method: 通过求解Riccati矩阵微分方程，得到了对称矩阵分解问题中GF的闭式解，并以此为基础进行分析。

Result: 研究表明，增量学习源于学习目标矩阵中不同成分的动态过程之间存在的时间尺度分离。通过减小初始化尺度，这些时间尺度分离变得更加明显，从而可以找到目标矩阵的低秩近似。
...
TL;DR: DFAMS通过动态信息流识别潜在查询意图，构建语义对齐的知识分区，从而在异构来源中实现准确检索，以此提升联邦检索(FR)在复杂场景下的性能。


<details>
  <summary>Details</summary>
Motivation: 现有联邦检索方法难以检索到高质量和相关的模糊查询文档，尤其是在跨领域场景中，这限制了它们在支持下游生成任务中的有效性。

Method: DFAMS利用动态信息流（DIF），通过梯度信号和Shapley值归因来识别意图和子域边界，并通过多原型对比学习训练对齐模块，实现细粒度的源内建模和跨知识库的源间语义对齐。

Result: 在五个基准测试中，DFAMS在知识分类准确率上优于现有方法14.37%，在检索召回率上优于5.38%，在下游QA准确率上优于6.45%。
...
TL;DR: 提出了一种多模态机器学习（MMML）架构，通过学习不同数据模式来预测性能指标，旨在提高工程设计效率并减少对计算密集型仿真的依赖。


<details>
  <summary>Details</summary>
Motivation: 设计者如何在不花费大量时间进行仿真设置的情况下评估给定引擎盖框架几何结构的性能？

Method: 开发一种多模态机器学习（MMML）架构，该架构从相同数据的不同模态中学习以预测性能指标。

Result: 通过结合多种数据模态，MMML 的性能优于传统的单模态方法。使用经过训练的 MMML 模型对两个不属于训练数据集的新框架几何结构进行预测，展示了其推广到未见过的框架模型的能力。

Conclusion: MMML 有潜力补充传统的基于仿真的工作流程，特别是在概念设计阶段，并强调了其在弥合机器学习和现实工程应用之间差距的作用。
...
TL;DR: Airbnb使用大型语言模型和预训练的语言图像模型来对齐房源的文本和照片，提出了BiListing方法，将非结构化数据转换为单个嵌入向量，实现零样本搜索、冷启动问题解决和房源间搜索。


<details>
  <summary>Details</summary>
Motivation: Airbnb过去依赖结构化数据，但难以从文本和图像中提取信息。将多种信息的嵌入组合成单一表示具有挑战性。

Method: 提出BiListing方法，利用大型语言模型和预训练的语言图像模型对齐文本和照片。

Result: 在Airbnb搜索排序模型中利用BiListing嵌入进行了离线和在线测试，成功部署到生产环境，实现了0.425%的NDCB增益，并带来了数千万美元的增量收入。

Conclusion: BiListing方法能够有效提升Airbnb的搜索排序效果并增加收入。
...
TL;DR: 该论文提出了一种名为TF-TransUNet1D的新型一维深度神经网络，用于心电信号的去噪，旨在提高心电信号在心脏数字孪生中的诊断效用。


<details>
  <summary>Details</summary>
Motivation: 心电信号易受噪声和伪影的影响，从而降低其诊断效用。

Method: 该模型结合了基于U-Net的编码器-解码器架构和Transformer编码器，并采用混合时频域损失进行指导，同时捕获局部形态特征和长程时间依赖性。引入双域损失函数，以优化时域中的波形重建和频域中的频谱保真度。

Result: 在MIT-BIH心律失常数据库和噪声压力测试数据库（NSTDB）上，TF-TransUNet1D在信噪比改善和误差指标方面均优于现有方法，平均绝对误差为0.1285，Pearson相关系数为0.9540。

Conclusion: 该研究通过提供高精度去噪，弥合了心脏数字孪生预处理流程中的关键差距，从而实现更可靠的实时监测和个性化建模。
...
TL;DR: TLinFormer, a novel linear attention architecture, achieves strict linear complexity while computing exact attention scores and ensuring information flow remains aware of the full historical context.


<details>
  <summary>Details</summary>
Motivation: The self-attention mechanism in the Transformer architecture suffers from a complexity bottleneck that scales quadratically with sequence length, severely limiting its application in long-sequence tasks.

Method: The paper introduces TLinFormer, which reconfigures neuron connection patterns to achieve linear complexity while computing exact attention scores.

Result: TLinFormer exhibits advantages in inference latency, KV cache efficiency, memory footprint, and overall speedup compared to a standard Transformer baseline on long-sequence inference tasks.
...
TL;DR: 本文介绍了一种新的几何正则化方法，即非线性共形正则化，用于近似深度神经网络的解码映射。
...
Method: 通过引入非线性共形正则化，该正则化允许解码器映射的局部变化，并引入共形因子作为潜在空间映射到原始数据空间时局部变形量的定量指标。此外，该技术还允许计算学习流形的标量曲率。

Result: 在Swiss roll和CelebA数据集上进行了实现和实验，以说明如何从架构中获得这些量。

Conclusion: 本文提出了一种新的几何正则化技术，可以有效地学习低维表示并计算学习流形的标量曲率。
...
TL;DR: 本研究结合可解释性人工智能 (XAI) 方法与 Rating Driven Explanations (RDE) 来评估 TSFM 性能和可解释性。


<details>
  <summary>Details</summary>
Motivation: 了解时间序列预测模型 (TSFM) 成功或失败的原因仍然具有挑战性。时间序列预测模型越来越多地用于生成信息，为现实世界的行动提供信息，而这些行动具有同等真实的后果。了解这些模型的复杂性、性能可变性和不透明性变得很有价值。

Method: 在金融、能源、运输和汽车销售领域跨四个异构数据集评估四种不同的模型架构：ARIMA、Gradient Boosting、Chronos（时间序列特定基础模型）、Llama（通用；微调和基础模型）。

Result: 特征工程模型（例如，Gradient Boosting）在波动或稀疏领域（例如，电力、汽车零件）始终优于基础模型（例如，Chronos），同时提供更易于解释的解释，而基础模型仅在稳定或趋势驱动的环境中表现出色（例如，金融）。

Conclusion: 特征工程模型在波动或稀疏领域优于基础模型，而基础模型仅在稳定或趋势驱动的环境中表现出色。
...
TL;DR: 本文研究了对角状态空间模型(SSM)初始化方案，并从频率角度分析了其角色。基于这些观察，作者提出了一种在离散傅里叶域上的对角初始化方法S4D-DFouT。


<details>
  <summary>Details</summary>
Motivation: 当前初始化状态空间模型(SSM)参数的方法主要依赖于HiPPO框架，但HiPPO框架没有明确研究其对角变体的作用。

Method: 本文从频率角度研究对角SSM初始化方案的角色，以系统地理解如何参数化这些模型，并揭示对角状态空间模型中固有的学习偏差。基于此，作者提出了一种在离散傅里叶域上的对角初始化方法S4D-DFouT。

Result: 提出的方法在Long Range Arena基准测试中取得了最先进的结果，并允许从头开始训练非常大的数据集，如PathX-256。

Conclusion: 本文深入研究了对角SSM初始化方案，并提出了一种新的初始化方法S4D-DFouT，该方法在多个任务上都取得了很好的效果。
...
TL;DR: 本文提出了一种新的机器学习遗忘框架，EAGLE-PC，它通过解缠感知引导的损失重新加权和代理约束来解决现有方法中的不足，在多个LLM上实现了更好的遗忘-效用权衡。


<details>
  <summary>Details</summary>
Motivation: 现有的机器学习遗忘方法缺乏合理的遗忘边界，导致部分样本遗忘不足，存在残留泄露风险，而其他样本过度遗忘，牺牲了效用。

Method: EAGLE-PC框架包含两个关键部分：解缠感知引导的损失重新加权，通过测量样本在嵌入空间中的相似性来确定每个样本的遗忘程度；以及利用ICL生成的测试数据的代理约束，以软性地规范遗忘过程，有效缓解过度遗忘。

Result: 在TOFU和MUSE基准测试中，EAGLE-PC在多个LLM上始终表现出更好的遗忘-效用权衡。结合NPO+GD优化器，其性能接近完全重新训练。

Conclusion: EAGLE-PC提供了一种可扩展且稳健的机器学习遗忘解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 为了解决医疗保健和金融等高风险领域中，由于隐私和监管障碍而无法使用真实世界数据的问题，差分隐私合成数据生成应运而生。

Method: 该基准解决了特定领域基准测试中的关键挑战，包括代表性数据的选择和实际的隐私预算，考虑了预训练和各种评估指标。评估了五种特定领域数据集上的最新隐私保护生成方法。

Result: 与真实数据相比，特别是在严格的隐私约束下，效用和保真度显著下降。 

Conclusion: 目前的方法存在局限性，因此有必要开发更高级的隐私保护数据共享方法，并为在实际场景中对其进行评估开创先例。
...
TL;DR: 这篇论文提出了一种新的Structure-aware HyperGraph Transformer (SHGT) 框架，用于电子健康记录(EHR)中的诊断预测。


<details>
  <summary>Details</summary>
Motivation: 现有的基于GNN的方法不足以捕捉临床数据中固有的高阶依赖关系，并且局部消息传递方案限制了表示能力。

Method: 该框架包括：a) 采用超图结构编码器来捕获医疗代码之间的高阶交互，b) 集成Transformer架构以推理整个超图，以及 c) 设计定制的损失函数，结合超图重建以保留超图的原始结构。

Result: 在真实世界EHR数据集上的实验表明，所提出的SHGT优于现有的最先进的诊断预测模型。

Conclusion: 该论文提出了一种新的SHGT框架，通过捕获高阶依赖关系和利用Transformer架构，提高了EHR中诊断预测的性能。
...
<details>
  <summary>Details</summary>
Motivation: 旨在为大型多表数据库提供有效的挖掘工具，并解决变量选择、分类、决策树和共聚类等问题。

Method: 该工具采用基于离散化模型（针对数值数据）和值聚类（针对分类数据）的贝叶斯方法，结合变量选择和权重学习的朴素贝叶斯分类器，以及自动构建聚合的命题化方法。

Result: Khiops能够处理包含数百万个体、数万变量和数亿记录的大型数据库。

Conclusion: Khiops是一个适用于多种环境的、可通过Python库和用户界面使用的机器学习工具，它为大型多表数据库的挖掘提供了有效的解决方案。
...
Result: SFT with MedGR2-produced data surpasses baselines. RL with this data achieves state-of-the-art generalization, and a compact model performs competitively with much larger models.

Conclusion: MedGR2 presents a new paradigm for data-efficient learning, transforming data scarcity to data generation and unlocking RL's potential for generalizable medical AI.
...
TL;DR: 本研究探讨了使用综合评估方法对推荐算法进行超参数优化，该方法将各种性能指标整合为一个统一的标准。


<details>
  <summary>Details</summary>
Motivation: 传统方法通常只关注单一指标，而本文旨在平衡准确性、排序质量、输出多样性和算法资源消耗。
...
<details>
  <summary>Details</summary>
Motivation: 现有优化器（如AdamW）在语言模型的大批量训练中存在性能下降，这是由于注意力层中由最大注意力logit的急剧增加引起的信息瓶颈造成的。虽然LAMB优化器部分解决了这个问题，但一些注意力层仍然面临这个问题。原因是LAMB中基于l2范数的信任比在直接影响查询/键权重的最大值方面效果较差。此外，LAMB中逐权重的信任比容易出错，因为它忽略了行或列中权重值之间的关系。

Method: 提出一种新的优化器，MERIT，它利用最大范数来计算信任比，以更有效地约束最大注意力logit。此外，我们进一步构建了元素级的信任比，通过关注局部权重结构来提供更强大的更新缩放。

Result: 在各种大小的GPT-2模型上进行的大批量训练的大量实验表明，MERIT具有优越的性能。值得注意的是，在GPT-2 Medium的训练过程中，与标准批量大小（480）相比，MERIT能够实现6k的批量大小，而不会降低48B的训练token的性能。

Conclusion: 这项工作强调了在大批量训练中考虑最大注意力logit和更细粒度的信任比的重要性。它成功地提高了训练的稳定性，并为更大的批量使用铺平了道路，从而能够更快地开发和迭代大型语言模型。
...
TL;DR: 提出了一种用于高斯过程精确随机推断的算法，在高斯过程具有中等有限维的再生核希尔伯特空间 (RKHS) 的情况下，该算法优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有高斯过程随机超参数学习方法依赖于近似，无法保证收敛到真实边缘似然的平稳点。

Method: 针对具有诱导适度有限维 RKHS 核的高斯过程，提出了精确随机推断算法。该方法可以扩展到无限维 RKHS，但会牺牲精确性。

Result: 在有限和无限维 RKHS 中，当内存资源限制了可行的批量大小和可能的诱导点数量时，该方法比现有方法实现了更好的实验结果。

Conclusion: 该算法在内存资源受限的情况下，优于现有方法，为高斯过程的随机超参数学习提供了一种更精确和高效的替代方案。
...
TL;DR: This paper introduces Local Virtual Nodes (LVN) to address over-squashing in graph neural networks without significantly altering the global graph structure. LVNs are strategically placed based on node centrality to improve connectivity in bottleneck regions and use trainable embeddings to facilitate communication between distant nodes.


<details>
  <summary>Details</summary>
Motivation: Over-squashing in graph neural networks hinders performance in tasks with long-range dependencies because gathering information from wide neighborhoods creates bottlenecks. Existing solutions like graph rewiring and adding virtual nodes alter the graph's global topology, potentially disrupting valuable domain knowledge.

Method: The paper proposes Local Virtual Nodes (LVN) with trainable embeddings. LVNs are positioned based on node centrality to target potential bottlenecks, enhancing connectivity in those regions. Trainable embeddings shared across central regions facilitate communication without adding layers.

Result: Experiments on benchmark datasets demonstrate that LVNs enhance structural connectivity and significantly improve performance on graph and node classification tasks.

Conclusion: Local Virtual Nodes (LVN) effectively alleviate over-squashing by improving connectivity in bottleneck regions without significantly corrupting the global structure of the input graph, leading to performance gains on graph and node classification tasks.
...
TL;DR: 提出了一种基于任务的方法来评估抽样调查的可信度，该方法使用模型特定的距离度量来量化可信度，并设计了一种算法来验证回归模型中调查数据的可信度。
...
Method: 引入了一种模型特定的距离度量来量化可信度的概念。还设计了一种算法来验证回归模型中调查数据的可信度。该算法的样本复杂度与数据维度无关，因为它侧重于验证调查数据的可信度，而不是重建底层回归模型。

Result: 证明了算法的理论正确性，并通过数值验证了算法的性能。表明如果尝试通过重建回归模型来验证可信度，则样本复杂度会随着数据的维度线性缩放。

Conclusion: 该研究提出了一种有效的、与数据维度无关的算法来验证调查数据的可信度，从而为评估抽样调查的可信度提供了一种新的方法。
...
<details>
  <summary>Details</summary>
Motivation: 在许多科学环境中，多重试验监督是可用的。本文旨在利用这种监督来提高独立成分分析的性能。

Method: 该方法将可逆矩阵空间中的近端梯度型算法与通过反向传播的预测模型的联合学习相结合。

Result: 在合成和真实数据实验中，观察到非凸优化的成功率增加，并且独立成分的可解释性提高。

Conclusion: 额外的监督提高了非凸优化的成功率和独立成分的可解释性。
...
TL;DR: 研究了掩码自动编码器（MAE）在超声信号上的自监督学习


<details>
  <summary>Details</summary>
Motivation: 在工业应用中，标记数据稀缺且信号处理具有高度任务特异性。因此，探索 MAE 在一维信号分析中的应用。

Method: 利用 MAE 在未标记的合成超声信号上进行预训练，从而学习鲁棒的表征，提高下游任务的性能。

Result: 预训练模型显著优于从头开始训练的模型和 CNN 基线。

Conclusion: MAE 在通过可扩展的自监督学习推进超声信号分析方面具有潜力。
...
TL;DR: GDS agent, 通过引入图算法工具和预处理（检索）和后处理，可以帮助LLM处理大规模图结构数据，并回答关于它们的需要图算法推理的问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）在多模态信息处理和推理方面表现出卓越的能力。然而，它们仍然难以处理和推理大规模图结构数据。

Method: GDS agent引入了一套全面的图算法作为工具，以及算法结果的预处理（检索）和后处理，在一个模型上下文协议（MCP）服务器中。该服务器可以与任何现代LLM一起使用。

Result: GDS agent 能够解决范围广泛的图任务。

Conclusion: 我们还讨论了剩余的挑战和未来的路线图。
...
TL;DR: 提出了一种新的时间变化的混合随机梯度跟踪算法（TV-HSGT），用于解决分布式在线优化问题，该算法在时变有向网络中集成了行随机和列随机通信方案，并结合了当前和递归随机梯度，以减少梯度方差并跟踪全局下降方向。


<details>
  <summary>Details</summary>
Motivation: 现有的分布式在线优化算法通常依赖于有界梯度假设，并忽略了随机梯度的影响，尤其是在时变有向网络中。

Method: 提出了一种名为 TV-HSGT 的时间变化的混合随机梯度跟踪算法，该算法基于混合随机梯度跟踪和方差减少机制，集成了行随机和列随机通信方案。

Result: 理论分析表明，TV-HSGT 可以在不假设梯度有界性的情况下，实现动态后悔的改进界限。在 Logistic 回归任务上的实验结果证实了 TV-HSGT 在动态和资源受限环境中的有效性。

Conclusion: TV-HSGT 算法在动态和资源受限环境中有效，并在理论和实验上都得到了验证。
...
TL;DR: 提出了一种新的扩散蒸馏目标函数VarDiU，它可以实现无偏梯度估计，并可以直接应用于扩散蒸馏。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散蒸馏方法使用扩散散度训练学生模型，但由于DSM训练的不完善，梯度估计存在偏差，导致性能次优。

Method: 提出VarDiU，一种变分扩散上界，它允许无偏梯度估计。

Result: VarDiU 实现了更高的生成质量，并为单步扩散蒸馏实现了更高效、更稳定的训练过程。

Conclusion: VarDiU 优于 Diff-Instruct，因为它允许无偏梯度估计，从而提高了单步扩散蒸馏的性能。
...
TL;DR: PCML结合物理模型和数据驱动方法，以提高可靠性、泛化性和可解释性，但在复杂的化学工程应用中面临挑战，如确定嵌入的物理知识量和类型、设计有效的融合策略、扩展模型到大型数据集和模拟器以及量化预测不确定性。


<details>
  <summary>Details</summary>
Motivation: 尽管PCML在不同的科学和工程领域显示出显著的优势，但技术和智力上的挑战阻碍了其在复杂化学工程应用中的适用性。

Method: 综述了PCML的最新发展。

Result: 强调了在将PCML应用于化学工程时存在的挑战和机遇，重点关注闭环实验设计、实时动力学和控制以及多尺度现象的处理。

Conclusion: 总结了PCML在化学工程中的应用前景和挑战。
...
TL;DR: 提出了一种通过自组合增强神经算子效率和准确性的新框架，提供理论保证和实际益处。


<details>
  <summary>Details</summary>
Motivation: 受数值偏微分方程（PDEs）迭代方法的启发，通过重复应用单个神经算子块来设计特定的神经算子，逐步加深模型，提高模型容量。

Method: 引入了一种自适应训练和展开方法，其中神经算子的深度在训练过程中逐渐增加。

Result: 该架构在标准基准测试中实现了最先进的（SOTA）性能。在高频超声计算机断层扫描（USCT）问题上进一步证明了其有效性，其中多重网格启发式骨干能够在解决复杂波现象方面实现卓越的性能。
...
TL;DR: 研究临床时间序列是否由有序的潜在状态序列生成，以解决临床时间序列预测中数据稀疏的问题。


<details>
  <summary>Details</summary>
Motivation: 临床时间序列数据稀疏，阻碍了预测模型的训练和性能。通过揭示其潜在的组合结构，可以生成合成数据，缓解数据不足的问题，并加深对临床数据的理解。

Method: 1. 将时间序列的组合性概念化为数据生成过程的属性。
2. 研究数据驱动的程序，以重建该过程的基本状态和组合规则。
3. 使用源于领域适应角度的两个经验测试评估方法的成功。

Result: 1. 在组合合成数据上训练的模型，其测试集性能与在原始临床时间序列数据上训练的模型相当。
2. 对组合合成测试数据进行模型评估，结果与在原始测试数据上评估相似，优于基于随机化的数据增强。
3. 当模型训练完全基于组合合成数据时，序贯器官衰竭评估 (SOFA) 分数的预测任务显示出显着的性能提升。

Conclusion: 组合合成数据可以有效地用于临床时间序列预测，并且优于传统的数据增强方法。
...
TL;DR: 该论文研究了使用强化学习（RL）对大型语言模型（LLM）进行有害微调的风险，并提出了一种名为TokenBuncher的防御方法。


<details>
  <summary>Details</summary>
Motivation: 之前的研究主要关注于监督微调（SFT）带来的风险，而该论文指出RL可能使攻击者更有效地突破安全对齐，从而促进更高级的有害任务辅助。

Method: 该论文提出TokenBuncher，通过抑制模型响应不确定性来阻止RL对有害行为的利用。该方法利用entropy-as-reward RL和一个Token Noiser机制来防止专业领域中有害能力的提升。

Result: 实验结果表明，TokenBuncher能有效缓解有害的RL微调，同时保留良性任务的实用性和可微调性。

Conclusion: RL带来的危害比SFT更大，TokenBuncher提供了一种有效且通用的防御方法。
...
TL;DR: 这篇论文提出了一种新的基于潜在扩散模型的自监督 EEG 表征学习方法 (EEGDM)，旨在解决现有方法在学习通用表征方面面临的挑战，尤其是在训练数据有限的情况下。


<details>
  <summary>Details</summary>
Motivation: 现有 EEG 表征学习方法通常依赖于简单的掩蔽重建目标，可能无法充分捕捉 EEG 信号中丰富的语义信息和复杂模式。

Method: 该方法利用 EEG 信号生成作为自监督目标，将扩散模型转化为能够捕捉 EEG 语义的强大表征学习器。EEGDM 包含一个 EEG 编码器，可以将 EEG 信号及其通道增强提炼成紧凑的表征，作为条件信息来指导扩散模型生成 EEG 信号。

Result: 实验结果表明，EEGDM 可以重建高质量的 EEG 信号，有效地学习鲁棒的表征，并且在不同的下游任务中以适度的预训练数据大小实现了有竞争力的性能。

Conclusion: 该方法具有良好的泛化能力和实用性。
...
TL;DR: 工具增强语言模型通过外部检索在事实召回方面优于仅靠模型权重记忆的传统模型，具有更强的扩展性。


<details>
  <summary>Details</summary>
Motivation: 探讨工具增强语言模型（如检索、记忆或外部API）的理论优势，特别是在事实召回方面。

Method: 通过理论证明和实验验证，比较工具学习（外部检索）和权重学习（记忆）在事实召回方面的效果。

Result: 证明了模型仅通过权重记忆的事实数量受到参数量的限制，而工具使用可以通过简单的电路构造实现无限的事实召回。实验结果表明，工具使用模型始终优于记忆模型。对于预训练的大型语言模型，教授工具使用和通用规则比将事实微调到记忆中更有效。

Conclusion: 工具增强的工作流程不仅实用，而且在理论上更具可扩展性。
...
TL;DR: SAFEMax: A new machine unlearning method for diffusion models.


<details>
  <summary>Details</summary>
Motivation: To cause the model to generate Gaussian noise when conditioned on impermissible classes.

Method: Maximize the entropy in generated images by halting the denoising process and selectively focusing on the early diffusion steps.

Result: SAFEMax is effective and efficient.

Conclusion: SAFEMax achieves substantial efficiency gains over state-of-the-art methods.
...
TL;DR: 这篇论文提出了一种新的上下文多智能体强化学习框架，该框架使用大型语言模型（LLM）生成有意义的课程，并提供更强大的评估信号，同时引入了一种新颖的基于多样性的上下文混合机制，通过结合先前上下文的特征来创建新的训练场景，从而提高泛化性和样本效率。


<details>
  <summary>Details</summary>
Motivation: 现有的上下文多智能体强化学习（cMARL）方法试图使用课程学习来帮助训练和评估上下文无关策略，但它们通常依赖于不可靠的代理信号，例如在多智能体环境中由于智能体间动态和部分可观察性而嘈杂和不稳定的价值估计或广义优势估计。

Method: 该论文提出了一种名为Contextual Multi-Agent LLM-Guided Curriculum Learning with Diversity-Based Context Blending (cMALC-D) 的框架，该框架使用大型语言模型（LLM）生成有意义的课程，并提供更强大的评估信号。此外，还引入了一种新颖的基于多样性的上下文混合机制，通过结合先前上下文的特征来创建新的训练场景。

Result: 在交通信号控制领域的实验表明，与现有的课程学习基线相比，cMALC-D 显着提高了泛化性和样本效率。

Conclusion: 该论文提出了一种新的上下文多智能体强化学习框架，通过使用大型语言模型和基于多样性的上下文混合机制，有效地提高了泛化性和样本效率。
