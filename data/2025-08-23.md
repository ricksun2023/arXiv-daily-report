...
Motivation: Current content safety methods in LLMs lack fine-grained controllability and rely on multi-stage training.

Method: A unified co-training framework is proposed to integrate positive, negative, and rejective safety behaviors within a single SFT stage, activated by magic tokens.

Result: The method achieves comparable or better safety alignment than SFT+DPO, with the 8B model surpassing DeepSeek-R1 (671B) in safety, while reducing training complexity and deployment costs. A distinct Safety Alignment Margin is induced, indicating safety robustness.

Conclusion: The proposed co-training method provides a scalable, efficient, and highly controllable solution for LLM content safety, matching or surpassing existing methods with reduced complexity and cost.
...
Method: LLM驱动的数学应用题文化本地化框架，可以自动构建包含本地名称、组织和货币的数据集。

Result: 翻译的基准可能会掩盖适当的社会文化背景下真正的多语言数学能力。

Conclusion: 该框架可以帮助减轻以英语为中心的实体偏差，并在跨各种语言引入本地实体时提高鲁棒性。
...
Result: The fine-tuned model achieved a COMET score gain of around 0.04 and 0.02, respectively, on translating Wikipedia articles.
...
Result: introduce MultiCoXQL, a multilingual extension of the CoXQL dataset spanning five typologically diverse languages, including one low-resource language.  present Compass, a new multilingual dataset designed for custom input extraction in ConvXAI systems, encompassing 11 intents across the same five languages as MultiCoXQL.
...
<details>
  <summary>Details</summary>
Motivation: 解决测试时对齐的效率瓶颈，受到推测抽样加速的启发，该加速利用小型草稿模型来有效预测未来 tokens。

Method: 引入了奖励转移推测抽样（SSS）算法，其中草稿模型与人类偏好对齐，而目标模型保持不变。

Result: 通过修改接受标准和奖励 token 分布，可以利用对齐的草稿模型和未对齐的目标模型之间的分布偏移来恢复 RLHF 最优解，而无需实际获得它。

Conclusion: 该算法在测试时弱到强的对齐实验中，以显著降低的推理成本实现了卓越的黄金奖励分数，从而验证了其有效性和效率。
...
<details>
  <summary>Details</summary>
Motivation: Existing recall metrics often depend on lexical overlap, leading to errors, while LLM-as-a-Judge methods remain prone to misalignment and hallucinations without structured verification. The completeness of machine-generated text is crucial in domains such as medicine and law and in tasks like list-based question answering (QA), where omissions can have serious consequences.

Method: A general three-stage recall evaluation framework that decomposes answers into self-contained facts, successively narrows plausible candidate matches through lexical and semantic filtering, and verifies their alignment through structured entailment checks.

Result: Substantial improvements in recall accuracy over strong lexical and LLM-as-a-Judge baselines.

Conclusion: LongRecall is evaluated on three long-form QA benchmarks, demonstrating substantial improvements in recall accuracy over strong baselines.
...
Result: addition of symbolic inference on top of prompting alone leads to more consistent and accurate predictions

Conclusion: calibration and fine-tuning using structured prediction objectives leads to increased performance for challenging tasks, showing that structured learning is still valuable in the era of LLMs
...
<details>
  <summary>Details</summary>
Motivation: 现有的从Common Crawl构建的以数学为中心的数据集由于提取启发式算法的脆弱性、HTML到文本转换的损失以及未能可靠地保持数学结构而导致质量下降。

Method: 使用lynx进行布局感知渲染，并使用基于LLM的定向清理阶段，恢复各种格式（例如，MathJax、KaTeX、MathML）的数学公式。

Result: 收集了一个大型、高质量的数学语料库，即Nemotron-CC-Math-3+（133B tokens）和Nemotron-CC-Math-4+（52B tokens）。Nemotron-CC-Math-4+不仅超过了之前所有的开放数学数据集，而且比之前质量最高的数学预训练数据集FineMath-4+多了5.5倍的tokens。当用于预训练Nemotron-T 8B模型时，我们的语料库在MATH上产生了+4.8到+12.6的增益，在MBPP+上产生了+4.6到+14.3的增益，同时也提高了MMLU和MMLU-Stem上的一般领域性能。

Conclusion: 本文提出了一个从嘈杂的网络规模数据中可靠地提取科学内容（包括数学）的pipeline，在数学、代码和一般推理方面取得了显著的提升，并在开放数学预训练语料库中建立了新的技术水平。
...
<details>
  <summary>Details</summary>
Motivation: Answering questions with false assumptions require identifying the false assumptions. Large Language Models (LLMs) often generate misleading answers because of hallucinations.

Method: an approach leveraging external evidence to mitigate hallucinations

Result: Experiments with five LLMs demonstrate that incorporating retrieved evidence is beneficial and generating and validating atomic assumptions yields more improvements and provides an interpretable answer by specifying the false assumptions.

Conclusion: incorporating retrieved evidence is beneficial and generating/validating atomic assumptions yields more improvements and provides an interpretable answer by specifying the false assumptions
...
<details>
  <summary>Details</summary>
Motivation: 现有模型在处理复杂、多轮、视觉接地的任务时面临挑战，现有基准测试无法捕捉真实世界多模态交互的动态性和复杂性。

Method: 提出CoLVLM Agent，通过迭代的“记忆-感知-规划-执行”循环增强LVLM的推理和指令跟随能力。

Result: CoLVLM Agent在MMDR-Bench上取得了优异的性能，平均人类评估得分达到4.03，超过了GPT-4o (3.92) 和 Gemini 1.5 Pro (3.85)。

Conclusion: CoLVLM Agent在复杂多模态交互中表现出色，优于GPT-4o和Gemini 1.5 Pro。
...
<details>
  <summary>Details</summary>
Motivation: Existing approaches often neglect fine-grained importance variations across feature dimensions (i.e., the channel axis), thereby limiting their ability to effectively balance efficiency and model accuracy.
...
Result: SPARK enables processing of longer sequences within the same memory budget and reduces KV cache storage by over 30% compared to eviction-based methods.

Conclusion: SPARK maintains performance with less degradation than 5% compared to the baseline eviction method, demonstrating its robustness and effectiveness.
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型 (LLM) 在通用 QA 中表现良好，但通常在特定领域的场景中表现不佳。检索增强生成 (RAG) 引入了外部知识，但由于检索噪声而导致幻觉和延迟。持续预训练可以内化领域知识，但成本高昂且缺乏跨领域灵活性。我们将这一挑战归因于领域知识的长尾分布，这使得部分但有用的内部知识未得到充分利用。我们进一步认为，知识获取应该是渐进式的，模仿人类的学习：首先理解概念，然后将它们应用于复杂的推理。

Method: 通过内部-外部知识自选择策略和选择性监督微调，S2K 框架内化领域知识。引入结构化推理数据生成管道并集成 GRPO 以增强推理能力。

Result: S2K在医疗、法律和金融QA基准测试中始终优于现有方法，并以显着更低的成本与领域预训练的LLM相匹配。

Conclusion: S2K在医疗、法律和金融QA基准测试中始终优于现有方法，并以显着更低的成本与领域预训练的LLM相匹配。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖于手动设计特定任务的演示或从精选库中检索，这需要大量的专家工作，并且随着工具多样性和任务难度的增加，提示工程变得越来越复杂和低效。

Method: 提出了一种名为Stepwise Experience Recall (SEER) 的自引导方法，该方法通过从持续更新的经验池中进行细粒度的逐步检索来实现。

Result: 在ToolQA基准测试中，SEER在简单问题上平均提高了6.1%，在难题上平均提高了4.7%。在τ-bench上，SEER分别实现了7.44%和23.38%的显著准确性提升。

Conclusion: SEER在ToolQA和τ-bench基准测试中表现出显著的准确性提升，证明了其在复杂任务中的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 小语言模型（SLM）在资源受限的环境中提供计算优势，但内存限制仍然是边缘设备部署的关键瓶颈。SLM 的大部分内存占用来自词汇相关组件，特别是嵌入和语言建模（LM）头，这是由于词汇量大造成的。现有的静态词汇修剪虽然减少了内存使用，但存在设计僵化、一刀切的问题，导致预填充阶段的信息丢失，并且缺乏灵活性。

Method: 提出 VocabTailor，一个解耦的动态词汇选择框架，通过卸载嵌入和为 LM Head 实现混合静态-动态词汇选择策略来解决内存限制。

Result: VocabTailor 在各种下游任务中实现了高达 99% 的词汇相关组件内存使用量减少，且任务性能几乎没有下降，大大优于现有的静态词汇修剪。

Conclusion: VocabTailor 能够显著减少词汇相关组件的内存使用，同时保持任务性能。
...
<details>
  <summary>Details</summary>
Motivation: LLMs struggle with temporal commonsense reasoning due to its infrequent explicit mention in text.

Method: A temporal commonsense extraction pipeline leveraging LLMs to automatically mine temporal commonsense and construct the TComQA dataset.

Result: TComQA achieves over 80% precision in extracting temporal commonsense.

Conclusion: The model trained with TComQA outperforms an LLM fine-tuned on an existing dataset for temporal question answering.
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型擅长英语的指令跟随，但它们在泰语等低资源语言中的性能仍有待探索。现有的基准测试通常依赖于翻译，忽略了实际使用所需的文化和领域特定细微差别。

Method: 人工创建了一个用于评估和指令调整的泰语数据集 WangchanThaiInstruct，涵盖四个专业领域和七个任务类型。通过与注释者、领域专家和 AI 研究人员进行的多阶段质量控制过程创建。

Result: 对 WangchanThaiInstruct 的两项研究：(1) 零样本评估显示了在文化和专业特定任务上的性能差距，以及 (2) 一项指令调整研究，其中消融隔离了本地监督的影响。

Conclusion: 在 WangchanThaiInstruct 上微调的模型在领域内和跨领域的基准测试中都优于使用翻译数据的模型。这些发现强调了需要植根于文化和专业的指导数据，以提高低资源、语言多样性环境中法学硕士的调整。
...
<details>
  <summary>Details</summary>
Motivation: Scientific survey articles play a vital role in summarizing research progress, yet their manual creation is becoming increasingly infeasible due to the rapid growth of academic literature. While large language models (LLMs) offer promising capabilities for automating this process, progress in this area is hindered by the absence of standardized benchmarks and evaluation protocols.

Method: introduce SurGE (Survey Generation Evaluation), a new benchmark for evaluating scientific survey generation in the computer science domain. SurGE consists of (1) a collection of test instances, each including a topic description, an expert-written survey, and its full set of cited references, and (2) a large-scale academic corpus of over one million papers that serves as the retrieval pool. In addition, propose an automated evaluation framework that measures generated surveys across four dimensions: information coverage, referencing accuracy, structural organization, and content quality.

Result: Our evaluation of diverse LLM-based approaches shows that survey generation remains highly challenging, even for advanced self-reflection frameworks.
...
<details>
  <summary>Details</summary>
Motivation: 代码转换 (CS) 是在单个说话者的语段中两种或多种语言之间的交替，在现实世界的对话中很常见，并且对多语言语音技术提出了重大挑战。然而，由于缺乏合适的数据集，能够处理这种现象的系统仍未得到充分探索。

Method: 我们提出了一种名为通用代码混合器 (UniCoM) 的新型管道，用于生成高质量、自然的 CS 样本，而不会改变句子语义。我们的方法利用了一种我们称之为用同义词替换单词 (SWORDS) 的算法，该算法通过用翻译替换选定的单词，同时考虑它们的词性来生成 CS 语音。

Result: CS-FLEURS 在客观和主观指标上都达到了与现有数据集相当的高 intelligibility（可理解性）和 naturalness（自然度）。

Conclusion: 该方法可以推进混合语语音技术，并实现更具包容性的多语言系统。
...
Result: Highlights a significant gap in the existing literature, discontinuity from previous research, scarcity of accessible implementations, and critiques of evaluation methodologies.
...
<details>
  <summary>Details</summary>
Motivation: 检索增强生成(RAG)通过将外部知识纳入其输入提示来增强大型语言模型(llm)的能力。然而，当检索到的上下文与LLM的参数知识相矛盾时，它通常无法解决不正确的外部上下文和正确的参数知识之间的冲突，即上下文-记忆冲突。

Method: 引入了冲突感知检索增强生成(CARE)，它由一个上下文评估器和一个基本LLM组成。上下文评估器从原始上下文token中编码紧凑的记忆token嵌入。通过grounded/adversarial soft prompting，上下文评估器被训练来辨别不可靠的上下文，并捕获一个指导信号，该信号将推理导向更可靠的知识源。

Result: CARE有效地缓解了上下文-记忆冲突，在QA和事实核查基准测试中平均性能提高了5.0%。

Conclusion: CARE有效地缓解了上下文-记忆冲突，在QA和事实核查基准测试中平均性能提高了5.0%，为可信和自适应的RAG系统建立了一个有希望的方向。
...
<details>
  <summary>Details</summary>
Motivation: A major challenge in evaluating KGC models lies in comparing their performance across multiple datasets and metrics. A model may outperform others on one dataset but underperform on another, making it difficult to determine overall superiority. Moreover, even within a single dataset, different metrics such as MRR and Hit@1 can yield conflicting rankings.

Method: We propose KG Evaluation based on Distance from Average Solution (EDAS), a robust and interpretable meta-metric that synthesizes model performance across multiple datasets and diverse evaluation criteria into a single normalized score ($M_i \\in [0,1]$).
...
<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型能力的快速发展，各种相应的评估基准也越来越多。作为模型性能的定量评估工具，基准测试不仅是衡量模型能力的核心手段，也是引导模型发展方向和促进技术创新的关键要素。

Method: 对 283 个有代表性的基准测试进行了分类，分为通用能力、特定领域和目标特定三个类别。

Result: 系统地回顾了当前大型语言模型基准测试的现状和发展，通用能力基准测试涵盖了核心语言学、知识和推理等方面；特定领域基准测试侧重于自然科学、人文社会科学和工程技术等领域；目标特定基准测试关注风险、可靠性、代理等方面。

Conclusion: 当前基准测试存在数据污染导致的分数虚高、文化和语言偏见导致的不公平评估以及缺乏对过程可信度和动态环境的评估等问题，并为未来的基准测试创新提供了可参考的设计范例。
...
<details>
  <summary>Details</summary>
Motivation: Trustworthiness of Multimodal Large Language Models (MLLMs) remains an intense concern, and existing approaches often focus on narrow aspects.
...
Result: Extensive experiments reveal vulnerabilities in current models and limitations in existing mitigation strategies. Reasoning-Enhanced Safety Alignment (RESA) achieves state-of-the-art results.

Conclusion: Current MLLMs have significant vulnerabilities, and existing mitigation strategies have limitations and introduce trade-offs. Reasoning-Enhanced Safety Alignment (RESA) improves trustworthiness.
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型使用分词器进行训练，由此产生的token分布高度不平衡。最近的做法倾向于越来越大的词汇量，但这种益处的来源尚不清楚。

Method: 通过在固定数据、计算和优化的情况下，将语言模型的词汇量从24K扩展到196K来进行受控研究。

Result: 较大的词汇量降低了token化文本的复杂性。词汇量越大，交叉熵的降低几乎完全是通过降低2500个最常见词的不确定性来实现的，即使罕见词的损失上升。因为相同的常见词覆盖了下游基准测试中大约77%的token，所以这种训练优势完整地转移了。

Conclusion: 更大的词汇量通过降低常见词的不确定性来降低交叉熵，从而提高性能。通过约束嵌入范数来削弱token频率不平衡的影响会逆转增益，表明模型利用了不平衡。扩大模型参数与固定词汇量产生相同的好处。
...
<details>
  <summary>Details</summary>
Motivation: Growing concerns regarding the reliability and trustworthiness of large language models (LLMs) and the fragmented nature of research in evidence-based text generation.

Method: The authors systematically analyze 134 papers, introduce a unified taxonomy, and investigate 300 evaluation metrics across seven key dimensions, focusing on approaches using citations, attribution, or quotations.
...
<details>
  <summary>Details</summary>
Motivation: 大型音频语言模型(LALMs)增强了音频感知能力，使其能够有效地处理和理解结合了音频和文本的多模态输入。然而，它们在处理音频和文本模态之间冲突信息方面的性能在很大程度上仍未得到检验。

Method: 提出了MCR-BENCH基准，专门用于评估LALMs在面对不一致的音频文本对时如何优先排序信息。

Result: 揭示了当模态之间存在不一致时，LALMs表现出对文本输入的显著偏见，从而导致音频中心任务的性能显著下降。通过监督微调探索了缓解策略，并分析了模型置信度模式。

Conclusion: LALMs在处理不一致的音频文本对时，存在对文本输入的显著偏见，忽略音频证据，导致音频中心任务的性能下降。即使存在矛盾的输入，模型仍然表现出过度自信。
...
<details>
  <summary>Details</summary>
Motivation: 大型语音语言模型（LSLM）的开发因架构分散和缺乏透明度而受到阻碍，阻碍了研究的系统比较和可重复性。与视觉语言领域不同，LSLM领域普遍存在发布模型权重而不发布相应训练数据和配置的做法。

Method: 构建并发布了一个3.8B参数的参考模型LLaSO-Base，该模型完全基于我们的公共数据进行训练。

Result: LLaSO-Base实现了0.72的标准化分数，建立了一个强大的、可重复的基线，超过了同类模型。分析表明，虽然更广泛的训练覆盖提高了性能，但在未见过的任务上，特别是在纯音频场景中，仍然存在显著的泛化差距。

Conclusion: LLaSO通过发布完整的数据、基准和模型，建立了一个基础的开放标准，以统一研究工作，并加速LSLM中社区驱动的进展。
...
<details>
  <summary>Details</summary>
Motivation: 虽然存在用于检测精神障碍的各种数据集，但在识别积极寻求帮助的个体方面仍然存在一个关键差距。

Method: 引入了一个新的数据集，M-Help，专门用于检测社交媒体上的求助行为。

Result: 该数据集超越了传统的标签，不仅识别求助活动，还识别特定的精神健康障碍及其根本原因，例如关系挑战或经济压力源。

Conclusion: AI模型经过M-Help训练后，可以解决三个关键任务：识别求助者，诊断精神健康状况，并揭示问题的根本原因。
...
Conclusion: PyTOD在对话过程中，在准确性和稳健的用户目标估计方面，超越了强大的基线，展示了执行感知状态跟踪的有效性。
...
<details>
  <summary>Details</summary>
Motivation: Existing radiology report evaluation methods lack clinically grounded, interpretable, and fine-grained metrics, producing coarse scores or using opaque models.

Method: RadReason uses Group Relative Policy Optimization with Sub-score Dynamic Weighting and Majority-Guided Advantage Scaling.

Result: RadReason outputs fine-grained sub-scores across six clinically defined error types and provides human-readable justifications.

Conclusion: RadReason surpasses prior offline metrics and matches GPT-4 evaluations on the ReXVal benchmark, while being explainable, cost-efficient, and suitable for clinical use.
...
<details>
  <summary>Details</summary>
Motivation: 个性化营销已成为提高客户参与度和推动业务增长的关键策略。先前的研究表明，执行良好的个性化策略可以使收入提高多达 40%，这突显了开发用于优惠生成的智能、数据驱动方法的重要战略意义。

Method: 通过使用对比学习方法微调预训练的编码器-解码器语言模型（特别是谷歌的 Text-to-Text Transfer Transformer (T5-Small 60M)）来开发用于个性化优惠生成的生成式 AI 模型 SLM4Offer。

Result: 实验结果表明，在模拟客户行为和优惠接受模式的合成数据集上，优惠接受率提高了 17%。

Conclusion: SLM4Offer 模型在优惠接受率方面比监督微调基线提高了 17%，突显了对比目标在推进个性化营销方面的有效性。
...
<details>
  <summary>Details</summary>
Motivation: question the perception about an LLM when users have inherently subjective behaviors and preferences, as seen in their ubiquitous and idiosyncratic browsing of websites or apps. The sequential behavior logs of pages, thus generated, form something akin to each user's self-constructed language.
...
Conclusion: clusterwise LM training, HeTLM (Heterogeneity aware Training of Language Model) with heterogeneous cluster specific set of parameters outperforms a single LM of the same family, controlling for the number of parameters; a higher mean and a lower variance in generation ensues, implying improved alignment.
...
<details>
  <summary>Details</summary>
Motivation: 现有的群体和身份检测数据集主要以英语为中心，单标签，并且侧重于粗略的身份类别。政治语言与社会身份密切相关，而社会身份通常受特定文化环境的影响，并通过特定的语言使用来表达。

Method: 创建了第一个用于社会身份检测的多标签希伯来语语料库 HebID，包含来自以色列政治家 Facebook 帖子的 5,536 个句子，并手动注释了 12 个细微的社会身份。

Result: 希伯来语调整的 LLM 提供了最佳结果（宏 F_1 = 0.74）。将分类器应用于政治家的 Facebook 帖子和议会演讲，评估了身份表达在受欢迎程度、时间趋势、聚类模式和性别相关变化方面的差异。利用了国家公共调查中的身份选择，从而能够比较精英话语中描绘的身份与公众的身份优先事项。

Conclusion: HebID语料库为研究希伯来语中的社会身份奠定了基础，并可作为其他非英语政治背景下类似研究的模型。
...
<details>
  <summary>Details</summary>
Motivation: 介绍 Dream 7B，这是迄今为止最强大的开放扩散大型语言模型。

Method: 采用离散扩散建模，通过迭代去噪并行地优化序列。

Result: Dream 7B 表现出卓越的规划能力和推理灵活性，包括任意顺序生成、填充能力和可调整的质量-速度权衡。

Conclusion: Dream 7B在通用、数学和编码任务上始终优于现有的扩散语言模型，展示了卓越的规划能力和推理灵活性。
...
<details>
  <summary>Details</summary>
Motivation: 对政治实体规范有效性的象征性攻击

Method: 结合微调编码器模型和解码器 LLM 的两阶段分类管道

Result: 在过去三十年中，PDD 显着上升，社交媒体上的 PDD 比议会辩论更普遍，男性政治家比女性政治家更多地使用 PDD，右倾行为者更倾向于使用 PDD——在竞选活动和重大政治事件期间出现明显的峰值

Conclusion: 自动化 PDD 分析对于理解民主话语的可行性和价值
...
<details>
  <summary>Details</summary>
Motivation: Existing LLM safety evaluation benchmarks generally rely on labor-intensive manual curation, which causes excessive time and resource consumption, exhibit significant redundancy and limited difficulty.

Method: The paper introduces SafetyFlow, the first agent-flow system designed to automate the construction of LLM safety benchmarks by orchestrating seven specialized agents.
...
Conclusion: The paper introduces SafetyFlowBench, a comprehensive safety benchmark with 23,446 queries, and validates its efficacy and efficiency by evaluating the safety of 49 advanced LLMs.
...
<details>
  <summary>Details</summary>
Motivation: LLMs require heavy computation and are accompanied by a steep cost. Especially for targeted applications such as sales and marketing outreach, these costs are far from feasible.

Method: fine-tuning

Result: generating similar domain-specific responses for a fraction of the cost

Conclusion: This paper introduces the concept of Trained Miniatures - Small Language Models(SLMs) fine-tuned for specific, high-value applications
...
### [51] [You Only Pose Once: A Minimalist's Detection Transformer for Monocular RGB Category-level 9D Multi-Object Pose Estimation](https://arxiv.org/abs/2508.14965)
*Hakjin Lee,Junghoon Seo,Jaehoon Sim*

Main category: cs.CV

TL;DR: YOPO是一种单阶段、纯RGB的方法，用于类别级别的9-DoF姿态估计，它优于现有技术。


<details>
  <summary>Details</summary>
Motivation: 在机器人和自动化领域，从单个RGB图像中准确恢复特定类别中未见实例的完整9-DoF姿态仍然是一个核心挑战。大多数现有的解决方案仍然依赖于伪深度、CAD模型或将2D检测与姿态估计分离的多阶段级联。受到对更简单的、纯RGB替代方案的需求的推动，该方案直接在类别级别学习。

Method: YOPO是一种单阶段、基于查询的框架，它将类别级别的9-DoF估计视为2D检测的自然延伸。YOPO用一个轻量级的姿态头、一个边界框条件的平移模块和一个6D感知的匈牙利匹配成本来增强一个transformer检测器。

Result: YOPO在REAL275数据集上实现了79.6%的$\rm{IoU}_{50}$和54.1%（在$10^\&circ$$10{\\rm{cm}}$度量标准下）。

Conclusion: YOPO在三个基准测试中都达到了新的技术水平。在REAL275数据集上，它的$\\rm{IoU}_{50}$达到79.6%，在$10^\&circ$$10{\\rm{cm}}$指标下达到54.1%，超过了之前的纯RGB方法，并大大缩小了与RGB-D系统的差距。
...
### [52] [Paired-Sampling Contrastive Framework for Joint Physical-Digital Face Attack Detection](https://arxiv.org/abs/2508.14980)
*Andrei Balykin,Anvar Ganiev,Denis Kondranin,Kirill Polevoda,Nikolai Liudkevich,Artem Petrov*

Main category: cs.CV

TL;DR: This paper introduces a unified framework for detecting face spoofing attacks (both physical and digital) that outperforms existing methods with lower complexity and faster training.


<details>
  <summary>Details</summary>
Motivation: Modern face recognition systems are vulnerable to spoofing attempts, including physical presentation attacks and digital forgeries. Existing systems handle these attack vectors with separate models, increasing complexity and leaving systems exposed to combined attacks.
...
Result: The proposed method achieves an average classification error rate (ACER) of 2.10 percent on the 6th Face Anti-Spoofing Challenge Unified Physical-Digital Attack Detection benchmark.

Conclusion: The proposed Paired-Sampling Contrastive Framework outperforms existing solutions on the 6th Face Anti-Spoofing Challenge Unified Physical-Digital Attack Detection benchmark, achieving an ACER of 2.10 percent. The framework is lightweight and trains quickly.
...
### [53] [TAIGen: Training-Free Adversarial Image Generation via Diffusion Models](https://arxiv.org/abs/2508.15020)
*Susim Roy,Anubhooti Jain,Mayank Vatsa,Richa Singh*

Main category: cs.CV

TL;DR: TAIGen是一种高效的对抗图像生成方法，它利用扩散模型，通过少量采样步骤和选择性RGB通道策略，快速生成高质量的对抗样本，并在攻击效果上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 对抗生成模型的攻击通常会产生低质量的图像，并且需要大量的计算资源。扩散模型虽然能够生成高质量的图像，但通常需要数百个采样步骤来进行对抗生成。

Method: TAIGen是一种免训练的黑盒方法，用于高效的对抗图像生成，仅使用无条件扩散模型的3-20个采样步骤生成对抗样本。该方法采用选择性RGB通道策略，将注意力图应用于红色通道，同时在绿色和蓝色通道上使用GradCAM引导的扰动。

Result: 在ImageNet上，以VGGNet为源，TAIGen对ResNet的攻击成功率为70.6%，对MNASNet的攻击成功率为80.8%，对ShuffleNet的攻击成功率为97.8%。在所有测试数据集中，TAIGen保持了视觉质量，PSNR高于30 dB。

Conclusion: TAIGen在保持图像质量的同时，实现了比现有基于扩散的攻击快10倍的对抗样本生成速度，并且对防御机制的攻击效果最好。
...
### [54] [Reversible Unfolding Network for Concealed Visual Perception with Generative Refinement](https://arxiv.org/abs/2508.15027)
*Chunming He,Fengyang Xiao,Rihan Zhang,Chengyu Fang,Deng-Ping Fan,Sina Farsiu*

Main category: cs.CV

TL;DR: RUN++ 是一种用于隐藏视觉感知的新方法，它使用可逆展开网络和扩散模型来提高准确性，同时降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有的隐藏视觉感知 (CVP) 方法通常利用可逆策略来减少不确定性，但这些策略通常仅限于掩码域，而 RGB 域的潜力尚未得到充分探索。

Method: 提出了一种具有生成细化的可逆展开网络 RUN++

Result: 该方法在真实世界的退化下仍然有效，并将这一概念扩展到更广泛的双层优化框架中。

Conclusion: RUN++ 通过展开网络为扩散模型提供了一个强大的不确定性先验，使其能够有效地将其注意力导向模糊区域，从而显着减少误报和漏报。
...
### [55] [GasTwinFormer: A Hybrid Vision Transformer for Livestock Methane Emission Segmentation and Dietary Classification in Optical Gas Imaging](https://arxiv.org/abs/2508.15057)
*Toqi Tahamid Sarker,Mohamed Embaby,Taminul Islam,Amer AbuGhazaleh,Khaled R Ahmed*

Main category: cs.CV

TL;DR: GasTwinFormer 是一种用于实时牲畜甲烷排放监测的混合视觉 Transformer，具有高精度和效率。


<details>
  <summary>Details</summary>
Motivation: 牲畜甲烷排放量占人为甲烷产量的 32%，因此自动化监测对于气候缓解策略至关重要。

Method: 一种混合视觉Transformer，用于光学气体成像中的实时甲烷排放分割和膳食分类，通过一种新的混合孪生编码器在空间减少的全局注意力和局部分组的注意力机制之间交替。

Result: GasTwinFormer 实现了 74.47% 的 mIoU 和 83.63% 的分割 mF1，同时仅保持 3.348M 参数、3.428G FLOP 和 114.9 FPS 推理速度的卓越效率。此外，我们的方法实现了完美的膳食分类准确率 (100%)，证明了利用膳食-排放相关性的有效性。

Conclusion: GasTwinFormer是一种用于实时牲畜排放监测的实用解决方案，通过消融实验验证了每个架构组件。
...
### [56] [CurveFlow: Curvature-Guided Flow Matching for Image Generation](https://arxiv.org/abs/2508.15093)
*Yan Luo,Drake Du,Hao Huang,Yi Fang,Mengyu Wang*

Main category: cs.CV

TL;DR: CurveFlow通过学习非线性轨迹和引入曲率正则化，在文本到图像生成方面优于现有方法，尤其在语义一致性方面。


<details>
  <summary>Details</summary>
Motivation: 现有的修正流模型基于数据和噪声分布之间的线性轨迹。这种线性强制零曲率，可能会无意中迫使图像生成过程通过数据流形的低概率区域。一个关键问题仍未得到充分探索：这些轨迹的曲率与生成的图像及其对应标题之间的语义对齐（即指令顺从性）有何关系？

Method: 提出CurveFlow，一种新颖的流匹配框架，旨在通过直接将曲率引导纳入流路径来学习平滑的非线性轨迹。该方法采用了一种鲁棒的曲率正则化技术，可惩罚轨迹内在动力学的突变。

Result: 在MS COCO 2014和2017上的大量实验表明，CurveFlow在文本到图像生成方面实现了最先进的性能，显著优于标准修正流变体和其他非线性基线（如Rectified Diffusion）。改进在语义一致性指标（如BLEU、METEOR、ROUGE和CLAIR）中尤为明显。
...
### [57] [HiRQA: Hierarchical Ranking and Quality Alignment for Opinion-Unaware Image Quality Assessment](https://arxiv.org/abs/2508.15130)
*Vaishnav Ramesh,Haining Wang,Md Jahidul Islam*

Main category: cs.CV

TL;DR: HiRQA 是一种自监督的 NR-IQA 框架，通过分层排序和质量对齐，在合成和真实的基准测试中实现了强大的泛化能力。
...
Method: HiRQA，分层排名和质量对齐），一个自我监督的、与意见无关的框架，它通过排名和对比学习相结合，提供分层的、质量感知的嵌入。

Result: HiRQA有效地推广到真实的退化，通过对各种失真的评估证明，如镜头光晕、雾霾、运动模糊和弱光条件。对于实时部署，我们推出了 HiRQA-S，这是一种轻量级变体，每个图像的推理时间仅为 3.5 毫秒。
...
### [58] [Reliable Multi-view 3D Reconstruction for `Just-in-time' Edge Environments](https://arxiv.org/abs/2508.15158)
*Md. Nurul Absur,Abhinav Kumar,Swastik Brahma,Saptarshi Debroy*

Main category: cs.CV

TL;DR: 该论文提出了一种基于投资组合理论的边缘资源管理策略，以提高在边缘计算环境中多视图 3D 重建的可靠性，即使存在系统中断。


<details>
  <summary>Details</summary>
Motivation: 多视图 3D 重建应用正在彻底改变需要快速态势感知的关键用例，例如应急响应、战术场景和公共安全。在许多情况下，它们近乎实时的延迟要求和对计算资源的临时需求需要采用“Just-in-time”边缘环境，其中系统是动态设置的，以在任务生命周期内支持应用程序。然而，可靠性问题可能源于这种边缘环境的固有动态性和操作逆境，导致时空相关的中断，影响相机操作，这可能导致重建质量的持续下降。

Method: 该论文使用遗传算法解决了投资组合理论优化问题。

Result: 使用公开可用的和定制的 3D 数据集，该论文证明了所提出的相机选择策略在保证针对时空中断的可靠 3D 重建方面的优势，优于传统的基线策略。

Conclusion: 该论文提出了一种新颖的投资组合理论启发的边缘资源管理策略，用于在可能的系统中断情况下实现可靠的多视图 3D 重建。
...
### [59] [XDR-LVLM: An Explainable Vision-Language Large Model for Diabetic Retinopathy Diagnosis](https://arxiv.org/abs/2508.15168)
*Masato Ito,Kaito Tanaka,Keisuke Matsuda,Aya Nakayama*

Main category: cs.CV

TL;DR: XDR-LVLM是一种新颖的框架，它利用视觉语言大型模型 (LVLM) 进行高精度 DR 诊断，并结合基于自然语言的解释。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在糖尿病视网膜病变 (DR) 检测中显示出希望，但由于缺乏透明性和可解释性，它们的黑盒性质通常会阻碍临床应用。

Method: XDR-LVLM集成了专门的医学视觉编码器、LVLM核心，并采用多任务提示工程和多阶段微调。

Result: 在糖尿病视网膜病变 (DDR) 数据集上的大量实验表明，XDR-LVLM 实现了最先进的性能，疾病诊断的平衡准确率为 84.55%，F1 值为 79.92%，概念检测的卓越结果（77.95% BACC，66.88% F1）。

Conclusion: XDR-LVLM通过提供稳健且可解释的见解，弥合了自动化诊断和临床需求之间的差距。
...
### [60] [MeSS: City Mesh-Guided Outdoor Scene Generation with Cross-View Consistent Diffusion](https://arxiv.org/abs/2508.15169)
*Xuyang Chen,Zhijun Zhai,Kaixuan Zhou,Zengmao Wang,Jianan He,Dong Wang,Yanfeng Zhang,mingwei Sun,Rüdiger Westermann,Konrad Schindler,Liqiu Meng*

Main category: cs.CV

TL;DR: 提出 MeSS，一种利用城市网格模型生成高质量、风格一致的室外场景的方法，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 网格模型在许多城市中变得越来越容易获得；然而，缺乏真实的纹理限制了它们在虚拟城市导航和自动驾驶中的应用。

Method: 提出了一种名为 MeSS（基于网格的场景合成）的方法，用于生成高质量、风格一致的室外场景，其中城市网格模型作为几何先验。

Result: 利用图像扩散模型，通过改进跨视图一致性来增强图像扩散模型。该流程包括三个关键阶段：首先，使用级联外绘 ControlNets 生成几何一致的稀疏视图；其次，通过名为 AGInpaint 的组件传播更密集的中间视图；第三，使用 GCAlign 模块全局消除视觉不一致性（例如，不同的曝光）。在生成的同时，通过在网格表面上初始化高斯球来重建 3D 高斯溅射 (3DGS) 场景。
...
### [61] [Adversarial Agent Behavior Learning in Autonomous Driving Using Deep Reinforcement Learning](https://arxiv.org/abs/2508.15207)
...
### [62] [DyMorph-B2I: Dynamic and Morphology-Guided Binary-to-Instance Segmentation for Renal Pathology](https://arxiv.org/abs/2508.15208)
...
TL;DR: DyMorph-B2I是一个用于肾脏病理学的动态形态引导二值到实例分割管道，它优于其他方法，并能够实现更准确的形态测量分析。


<details>
  <summary>Details</summary>
Motivation: 精确的肾脏病理功能单元形态学量化依赖于实例级别的分割，然而，大多数现有的数据集和自动方法只提供二元（语义）掩码，限制了下游分析的精度。

Method: DyMorph-B2I，一个动态的、形态引导的二值到实例分割的管道，专为肾脏病理学量身定制。该方法在一个统一的框架内集成了分水岭、骨骼化和形态学操作，并辅以自适应几何细化和每个功能单元类别的可定制超参数调整。

Result: 实验结果表明，DyMorph-B2I 优于单独的经典方法和朴素的组合。

Conclusion: DyMorph-B2I 优于单独的经典方法和朴素的组合，能够实现卓越的实例分离，并有助于肾脏病理工作流程中更准确的形态测量分析。
...
### [63] [STAGNet: A Spatio-Temporal Graph and LSTM Framework for Accident Anticipation](https://arxiv.org/abs/2508.15216)
*Vipooshan Vipulananthan,Kumudu Mohottala,Kavindu Chinthana,Nimsara Paramulla,Charith D Chitraranjan*

Main category: cs.CV

TL;DR: 该研究提出了一种基于行车记录仪视频的事故预测模型STAGNet，该模型优于现有的图神经网络。


<details>
  <summary>Details</summary>
Motivation: 事故预测和及时预警在提高道路安全方面起着关键作用，可以通过降低道路使用者的受伤风险和最大限度地减少财产损失来实现。许多现有的系统依赖于各种传感器，如激光雷达、雷达和GPS，而仅仅依靠行车记录仪视频输入，提出了一个更具挑战性，但更具成本效益和易于部署的解决方案。

Method: 该研究采用了一种循环网络来整合更好的时空特征，从而改进了用于预测行车记录仪视频中事故的最先进的图神经网络。

Result: 提出的STAGNet模型比以前的方法获得了更高的平均精度和平均碰撞时间值。

Conclusion: STAGNet模型在三个公开数据集上进行了实验，结果表明，无论是在给定数据集上进行交叉验证，还是在不同数据集上进行训练和测试，该模型都比以往的方法获得了更高的平均精度和平均碰撞时间值。
...
### [64] [Collaborative Multi-Modal Coding for High-Quality 3D Generation](https://arxiv.org/abs/2508.15228)
*Ziang Cao,Zhaoxi Chen,Liang Pan,Ziwei Liu*

Main category: cs.CV

TL;DR: TriMM 是一种前馈 3D 原生生成模型，它从多模态数据（RGB、RGBD 和点云）中学习，以生成高质量的 3D 资产。


<details>
  <summary>Details</summary>
Motivation: 现有的大多数 3D 原生生成架构要么主要在单模态范例中运行，因此忽略了多模态数据的互补优势，要么将自己限制于 3D 结构，从而限制了可用训练数据集的范围。为了全面利用多模态进行 3D 建模，我们提出了 TriMM，这是第一个从基本多模态（例如，RGB、RGBD 和点云）学习的前馈 3D 原生生成模型。

Method: TriMM 采用 triplane 潜在扩散模型，以生成具有卓越质量的 3D 资产，从而增强纹理和几何细节。TriMM 首先引入协作多模态编码，该编码集成了模态特定特征，同时保留了它们独特的表示优势。此外，还引入了辅助 2D 和 3D 监督，以提高多模态编码的鲁棒性和性能。

Result: 在多个知名数据集上的大量实验表明，TriMM 通过有效利用多模态，实现了与在大型数据集上训练的模型具有竞争力的性能。

Conclusion: TriMM通过有效利用多模态，在使用少量训练数据的情况下，实现了与在大型数据集上训练的模型具有竞争力的性能。在最近的RGB-D数据集上进行了额外的实验，验证了将其他多模态数据集整合到3D生成中的可行性。
...
### [65] [Center-Oriented Prototype Contrastive Clustering](https://arxiv.org/abs/2508.15231)
...
<details>
  <summary>Details</summary>
Motivation: Existing methods try to solve this problem through prototype contrast, but there is a deviation between the calculation of hard prototypes and the true cluster center. To address this problem, we propose
...
Result: Extensive experiments on five datasets show that
...
### [66] [AeroDuo: Aerial Duo for UAV-based Vision and Language Navigation](https://arxiv.org/abs/2508.15232)
*Ruipu Wu,Yige Zhang,Jinyu Chen,Linjiang Huang,Shifeng Zhang,Xu Zhou,Liang Wang,Si Liu*

Main category: cs.CV

TL;DR: Introduce a Dual-Altitude UAV Collaborative VLN task and construct a dataset to support it. Propose a dual-UAV collaborative VLN framework, AeroDuo, to consolidate the complementary strengths.
...
Result: HaL-13k dataset includes both unseen maps and an unseen object validation set to systematically evaluate the model's generalization capabilities across novel environments and unfamiliar targets.
...
### [67] [Pretrained Diffusion Models Are Inherently Skipped-Step Samplers](https://arxiv.org/abs/2508.15233)
*Wenju Xu*

Main category: cs.CV

TL;DR: 提出了一种跳步采样机制，可以显著减少扩散模型的采样步骤，同时保持高生成质量。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在各种生成任务中取得了最先进的结果。然而，一个显著的缺点是它们的顺序生成过程，需要长序列的逐步生成。现有的方法，如DDIM，试图通过构建一类保持相同训练目标的非马尔可夫扩散过程来减少采样步骤。然而，对于原始扩散过程是否可以在不求助于非马尔可夫过程的情况下实现相同的效率，仍然存在差距。

Method: 提出了跳步采样机制，绕过迭代生成过程中的多个中间去噪步骤。将加速采样技术与DDIM集成，提出了一种增强的生成方法。

Result: 在包括OpenAI ADM、Stable Diffusion和Open Sora模型在内的流行预训练扩散模型上进行的大量实验表明，该方法以显著减少的采样步骤实现了高质量的生成。

Conclusion: 该方法通过跳过多个中间去噪步骤来实现加速采样，同时保持高生成质量。
...
### [68] [Comp-X: On Defining an Interactive Learned Image Compression Paradigm With Expert-driven LLM Agent](https://arxiv.org/abs/2508.15243)
*Yixin Gao,Xin Li,Xiaohan Pan,Runsen Feng,Bingchen Li,Yunpeng Qi,Yiting Lu,Zhengxue Cheng,Zhibo Chen,Jörn Ostermann*

Main category: cs.CV

TL;DR: Comp-X, an intelligently interactive image compression paradigm empowered by LLM, advances image coding through a multi-functional coding framework, an interactive coding agent, and a dedicated benchmark.
...
Conclusion: Comp-X can understand coding requests efficiently and achieve impressive textual interaction capability, while maintaining comparable compression performance, providing a promising avenue for AGI in image compression.
...
### [69] [Normal and Abnormal Pathology Knowledge-Augmented Vision-Language Model for Anomaly Detection in Pathology Images](https://arxiv.org/abs/2508.15256)
*Jinsol Song,Jiamu Wang,Anh Tien Nguyen,Keunho Byeon,Sangjeong Ahn,Sung Hak Lee,Jin Tae Kwak*

Main category: cs.CV

TL;DR: 提出了Ano-NAViLa，一种新的Vision-Language模型，用于病理图像中的异常检测，并在淋巴结数据集上取得了SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 现有的异常检测方法主要为工业环境设计，在病理学中面临计算约束、多样化的组织结构和缺乏可解释性等限制。为了解决这些挑战。

Method: 提出了Ano-NAViLa，一种用于病理图像异常检测的Normal and Abnormal pathology knowledge-augmented Vision-Language模型。该模型建立在预训练的视觉-语言模型上，带有一个轻量级可训练的MLP。

Result: Ano-NAViLa提高了准确性和鲁棒性，增强了病理图像的可变性，并通过图像-文本关联提供了可解释性。
...
### [70] [RATopo: Improving Lane Topology Reasoning via Redundancy Assignment](https://arxiv.org/abs/2508.15272)
...
Conclusion: RATopo is model-agnostic and can be seamlessly integrated into existing topology reasoning frameworks, consistently improving both lane-lane and lane-traffic topology performance.
...
### [71] [DesignCLIP: Multimodal Learning with CLIP for Design Patent Understanding](https://arxiv.org/abs/2508.15297)
*Zhu Wang,Homaira Huda Shomee,Sathya N. Ravi,Sourav Medya*

Main category: cs.CV

TL;DR: DesignCLIP 是一个基于 CLIP 模型的多模态框架，用于设计专利分析，它通过类感知分类、对比学习和多视图图像学习来提高性能，并在专利分类和检索任务中优于现有技术。


<details>
  <summary>Details</summary>
Motivation: 传统的设计专利分析任务严重依赖图像数据，但专利图像通常缺乏传达全面的视觉上下文和语义信息，这可能导致在现有技术检索过程中的评估出现歧义。

Method: 利用 CLIP 模型开发了一个统一框架 DesignCLIP，用于设计专利应用，该框架结合了类感知分类和对比学习，利用生成的详细标题进行专利图像和多视图图像学习。

Result: DesignCLIP 在包括专利分类和专利检索在内的各种下游任务中都表现出有效性。此外，还探索了多模态专利检索，通过提供更多样化的灵感来源，从而具有增强设计创造力和创新的潜力。
...
### [72] [TPA: Temporal Prompt Alignment for Fetal Congenital Heart Defect Classification](https://arxiv.org/abs/2508.15298)
*Darya Taratynova,Alya Almsouti,Beknur Kalmakhanbet,Numan Saeed,Mohammad Yaqub*

Main category: cs.CV

TL;DR: TPA: a new method for fetal CHD classification in ultrasound videos using temporal modeling, prompt-aware contrastive learning, and uncertainty quantification, achieving state-of-the-art results.
...
Result: TPA achieves state-of-the-art macro F1 scores of 85.40% for CHD diagnosis, while also reducing expected calibration error by 5.38% and adaptive ECE by 6.8%. On EchoNet-Dynamic's three-class task, it boosts macro F1 by 4.73% (from 53.89% to 58.62%).
...
### [73] [BasketLiDAR: The First LiDAR-Camera Multimodal Dataset for Professional Basketball MOT](https://arxiv.org/abs/2508.15299)
...
TL;DR: Introduces BasketLiDAR, a multimodal dataset for basketball player tracking, and a novel MOT framework that improves tracking accuracy and reduces computational cost.


<details>
  <summary>Details</summary>
Motivation: Traditional multi-camera systems are constrained by the two-dimensional nature of video data and the need for complex 3D reconstruction, making real-time analysis challenging. Basketball, in particular, represents a difficult scenario due to rapid and complex player movements and frequent occlusions.

Method: A novel MOT algorithm that leverages LiDAR's high-precision 3D spatial information; a real-time tracking pipeline using LiDAR alone and a multimodal tracking pipeline that fuses LiDAR and camera data.

Result: The approach achieves real-time operation and superior tracking performance even under occlusion conditions. The BasketLiDAR dataset contains 4,445 frames and 3,105 player IDs.

Conclusion: The proposed approach achieves real-time operation and superior tracking performance, even under occlusion conditions.
...
### [74] [First RAG, Second SEG: A Training-Free Paradigm for Camouflaged Object Detection](https://arxiv.org/abs/2508.15313)
*Wutao Liu,YiDan Wang,Pan Gao*

Main category: cs.CV

TL;DR: RAG-SEG is a training-free method for camouflaged object detection that uses retrieval-augmented generation to create prompts for SAM, achieving state-of-the-art performance with high computational efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing COD approaches rely on heavy training and large computational resources. Foundation models like SAM struggle with COD tasks without fine-tuning and require high-quality prompts, which are costly to generate manually.

Method: A training-free paradigm called First RAG, Second SEG (RAG-SEG) that decouples COD into Retrieval-Augmented Generation (RAG) for generating coarse masks and SAM-based segmentation (SEG) for refinement. It constructs a compact retrieval database via unsupervised clustering for fast feature retrieval and uses retrieved features to produce pseudo-labels that guide precise mask generation using SAM.

Result: RAG-SEG performs on par with or surpasses state-of-the-art methods on benchmark COD datasets. Experiments were conducted on a personal laptop, highlighting computational efficiency.

Conclusion: RAG-SEG achieves competitive or superior performance compared to state-of-the-art methods on COD datasets, while being computationally efficient and practical, as demonstrated by experiments on a personal laptop.
...
### [75] [VideoEraser: Concept Erasure in Text-to-Video Diffusion Models](https://arxiv.org/abs/2508.15314)
*Naen Xu,Jinghuai Zhang,Changjiang Li,Zhi Chen,Chunyi Zhou,Qingming Li,Tianyu Du,Shouling Ji*

Main category: cs.CV

TL;DR: VideoEraser是一个无需训练的框架，可防止 T2V 扩散模型生成包含不良概念的视频，即使在明确提示这些概念时也是如此，它在四个任务上的平均抑制率提高了 46%。


<details>
  <summary>Details</summary>
Motivation: 文本到视频 (T2V) 扩散模型的快速增长引发了人们对隐私、版权和安全的担忧，因为它们可能被滥用于生成有害或误导性内容。这些模型通常在大量数据集上进行训练，包括未经授权的个人身份、艺术创作和有害材料，这可能导致此类内容的不受控制的生产和传播。

Method: VideoEraser，一个无需训练的框架，通过选择性提示嵌入调整 (SPEA) 和对抗鲁棒噪声引导 (ARNG) 的两阶段过程，与代表性的 T2V 扩散模型无缝集成。

Result: VideoEraser 在功效、完整性、保真度、鲁棒性和泛化性方面始终优于先前的方法。在抑制 T2V 生成过程中不期望内容方面实现了最先进的性能。

Conclusion: VideoEraser在抑制T2V生成过程中不期望内容方面表现出色，与基线相比，在四个任务上的平均抑制率提高了 46%。
...
### [76] [Predicting Road Crossing Behaviour using Pose Detection and Sequence Modelling](https://arxiv.org/abs/2508.15336)
*Subhasis Dasgupta,Preetam Saha,Agniva Roy,Jaydip Sen*

Main category: cs.CV

TL;DR: 本研究着重于预测行人过马路的意图，通过实验分析了不同的序列建模技术，发现1D CNN在速度方面表现最佳。


<details>
  <summary>Details</summary>
Motivation: 预测行人是否要过马路对于自动驾驶车辆至关重要。

Method: 使用深度学习模型来预测姿势和序列建模来进行时间预测。

Result: 分析了三种不同的序列建模以了解预测行为。
...
### [77] [RCDINO: Enhancing Radar-Camera 3D Object Detection with DINOv2 Semantic Features](https://arxiv.org/abs/2508.15353)
*Olga Matykina,Dmitry Yudin*

Main category: cs.CV

TL;DR: RCDINO, a multimodal transformer model, fuses camera and radar data for 3D object detection, achieving state-of-the-art results on the nuScenes dataset.
...
### [78] [An Empirical Study on How Video-LLMs Answer Video Questions](https://arxiv.org/abs/2508.15360)
*Chenhui Gou,Ziyu Ma,Zicheng Duan,Haoyu He,Feng Chen,Akide Liu,Bohan Zhuang,Jianfei Cai,Hamid Rezatofighi*

Main category: cs.CV

TL;DR: This paper interprets Video-LLMs using attention knockouts, revealing insights about layer functionality and attention mechanisms, which can improve efficiency.


<details>
  <summary>Details</summary>
Motivation: Limited attention to understanding the internal mechanisms of Video-LLMs despite their strong capabilities.

Method: Attention knockouts with three variants: Video Temporal Knockout, Video Spatial Knockout, and Language-to-Video Knockout.

Result: Key findings: (1) Video information extraction primarily occurs in early layers; (2) Certain intermediate layers exert an outsized impact; (3) Spatial-temporal modeling relies more on language-guided retrieval.
...
### [79] [Transfer learning optimization based on evolutionary selective fine tuning](https://arxiv.org/abs/2508.15367)
*Jacinto Colan,Ana Davila,Yasuhisa Hasegawa*

Main category: cs.CV

TL;DR: BioTune is an evolutionary adaptive fine-tuning technique that selectively fine-tunes layers to enhance transfer learning efficiency.


<details>
  <summary>Details</summary>
Motivation: Traditional fine-tuning often involves updating all model parameters, which can potentially lead to overfitting and higher computational costs.
...
### [80] [Image-Conditioned 3D Gaussian Splat Quantization](https://arxiv.org/abs/2508.15372)
*Xinshuang Liu,Runfa Blark Li,Keito Suzuki,Truong Nguyen*

Main category: cs.CV

TL;DR: 提出了一种新的3DGS压缩方法ICGS-Quantizer，它提高了压缩效率并提供了对归档后场景变化的适应性，将存储需求降低到千字节范围。


<details>
  <summary>Details</summary>
Motivation: 现有的3DGS压缩方法存在两个局限性：(1)它们仅将中等规模的场景压缩到兆字节范围，这对于大规模场景或广泛的场景集合仍然是不切实际的；(2)它们缺乏在长期归档后适应场景变化的机制。

Method: 提出了一种图像条件高斯溅射量化器(ICGS-Quantizer)，它通过联合利用高斯间和属性间的相关性，并使用跨所有训练场景的共享码本，从而提高量化效率。在解码时，ICGS-Quantizer以捕获的图像为条件进行场景解码。

Result: ICGS-Quantizer有效地将3DGS的存储需求降低到千字节范围，同时保持视觉保真度。

Conclusion: ICGS-Quantizer在压缩效率和适应场景变化方面优于现有方法。
...
### [81] [DriveSplat: Decoupled Driving Scene Reconstruction with Geometry-enhanced Partitioned Neural Gaussians](https://arxiv.org/abs/2508.15376)
*Cong Wang,Xianda Guo,Wenbo Xu,Wei Tian,Ruiqi Song,Chenming Zhang,Lingxi Li,Long Chen*

Main category: cs.CV

TL;DR: DriveSplat, a high-quality reconstruction method for driving scenarios based on neural Gaussian representations with dynamic-static decoupling is introduced to address the limitations of existing methods in 3D scene reconstruction for driving scenarios.
...
### [82] [DIO: Refining Mutual Information and Causal Chain to Enhance Machine Abstract Reasoning Ability](https://arxiv.org/abs/2508.15387)
*Ruizhuo Song,Beiming Yuan*

Main category: cs.CV

TL;DR: This paper analyzes the limitations of current deep learning models in abstract reasoning using RPM problems, identifies issues with the DIO model, and proposes three improvement methods.


<details>
  <summary>Details</summary>
Motivation: The paper aims to address the unresolved bottleneck in abstract reasoning of deep learning models, using Raven's Progressive Matrices (RPM) problems as a benchmark.

Method: The paper adopts a ``causal chain modeling'' perspective to analyze the RPM task and designs the network architecture of the baseline model DIO.

Result: Experiments reveal that the optimization objective formulated for DIO fails to enable the model to genuinely acquire the predefined human reasoning logic.

Conclusion: This paper identifies limitations in the DIO model's ability to acquire human reasoning logic due to the tightness of the lower bound in mutual information maximization and the failure of mutual information to capture causal relationships. It proposes three progressive improvement methods to address these limitations.
...
### [83] [Spiking Variational Graph Representation Inference for Video Summarization](https://arxiv.org/abs/2508.15389)
*Wenrui Li,Wei Han,Liang-Jian Deng,Ruiqin Xiong,Xiaopeng Fan*

Main category: cs.CV

TL;DR: This paper proposes a Spiking Variational Graph (SpiVG) Network for video summarization, which enhances information density and reduces computational complexity. It outperforms existing methods on several datasets.
...
### [84] [From Linearity to Non-Linearity: How Masked Autoencoders Capture Spatial Correlations](https://arxiv.org/abs/2508.15404)
*Anthony Bisulco,Rahul Ramesh,Randall Balestriero,Pratik Chaudhari*

Main category: cs.CV

TL;DR: This paper investigates how MAEs learn spatial correlations in the input image and how to select MAE hyper-parameters in practice.
...
Method: analytically derive the features learned by a linear MAE and extend this analysis to non-linear MAEs
...
Conclusion: MAE representations adapt to spatial correlations in the dataset, beyond second-order statistics, and provide insights on how to select MAE hyper-parameters in practice.
...
TL;DR: BIRD: A bidirectional temporal information propagation method for moving infrared small target detection, outperforming existing methods with faster speed.
...
### [86] [The Impact of Image Resolution on Face Detection: A Comparative Analysis of MTCNN, YOLOv XI and YOLOv XII models](https://arxiv.org/abs/2507.23341)
...
<details>
  <summary>Details</summary>
Motivation: 人脸检测是许多人工智能驱动应用（如监控、生物特征认证和人机交互）中的一个关键组成部分。然而，低分辨率图像等现实条件带来了降低检测性能的重大挑战。
...
Conclusion: YOLOv11在检测精度方面优于YOLOv12和MTCNN，尤其是在较高分辨率下，而YOLOv12的召回率略好。MTCNN在landmark localization方面具有竞争力，但在实时推理速度方面滞后。
...
### [87] [A Curated Dataset and Deep Learning Approach for Minor Dent Detection in Vehicles](https://arxiv.org/abs/2508.15431)
*Danish Zia Baig,Mohsin Kamal*

Main category: cs.CV

TL;DR: 本文提出了一种基于YOLOv8的深度学习解决方案，用于自动检测汽车外观上的微观表面缺陷，特别是微小的凹痕。


<details>
  <summary>Details</summary>
Motivation: 传统的汽车损伤检测技术是劳动密集型、手动操作，并且经常忽略微小的表面缺陷，例如微小的凹痕。机器学习为更快、更精确的检测方法的需求提供了创新的解决方案。

Method: 使用YOLOv8对象识别框架，并创建包含各种光照、角度和纹理下汽车表面注释图像的定制数据集，使用实时数据增强方法训练YOLOv8m模型及其定制变体YOLOv8m-t4和YOLOv8m-t42。

Result: 实验结果表明，该技术具有出色的检测精度和较低的推理延迟，使其适用于实时应用，例如自动保险评估和汽车检查。YOLOv8m-t42模型在识别微观表面缺陷方面的精度为 0.86，召回率为 0.84，F1 分数为 0.85，优于 YOLOv8m-t4 模型（精度：0.81，召回率：0.79，F1 分数：0.80）。

Conclusion: YOLOv8m-t42在识别微观表面缺陷方面优于YOLOv8m-t4模型，更适合实际应用，尽管收敛速度较慢。
...
### [88] [Aligning Moments in Time using Video Queries](https://arxiv.org/abs/2508.15439)
*Yogesh Kumar,Uday Agarwal,Manish Gupta,Anand Mishra*

Main category: cs.CV

TL;DR: MATR是一种用于视频到视频时刻检索的Transformer模型，通过双阶段序列对齐和自监督预训练，在两个数据集上都取得了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 视频到视频时刻检索(Vid2VidMR)任务需要语义帧级别对齐和建模查询和目标视频之间的复杂依赖关系。

Method: 提出了一种基于Transformer的模型MATR，使用双阶段序列对齐来编码相关性和依赖性，并使用自监督预训练技术。

Result: MATR在ActivityNet-VRL数据集上R@1提升13.1%，mIoU提升8.1%。在SportsMoments数据集上R@1提升14.7%，mIoU提升14.4%。

Conclusion: MATR在ActivityNet-VRL和SportsMoments数据集上显著优于现有方法。
...
TL;DR: 提出了一种在极稀疏视图下，无需SfM的3DGS方法，通过密集立体、视图插值和多尺度正则化来提升相机姿态估计和3D重建质量，实验结果表明该方法优于现有技术。


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting (3DGS)在 novel view synthesis 中表现出了卓越的实时性能，但其有效性严重依赖于具有精确已知相机姿态的密集多视图输入，这在现实场景中很少可用。当输入视图变得非常稀疏时，3DGS 所依赖的 Structure-from-Motion (SfM) 方法无法准确重建场景的 3D 几何结构，从而导致渲染质量下降。

Method: 提出了一种新的基于SfM-free的3DGS方法，该方法联合估计相机姿态并从极稀疏视图输入重建3D场景。具体来说，使用密集立体模块逐步估计相机姿态信息，并重建全局密集点云以进行初始化。提出了相干视图插值模块，该模块基于训练视图对插值相机姿态，并生成视点一致的内容作为额外的训练监督信号。引入了多尺度拉普拉斯一致性正则化和自适应空间感知多尺度几何正则化，以提高几何结构和渲染内容的质量。

Result: 该方法显著优于其他state-of-the-art的基于3DGS的方法，在极稀疏视图条件下（仅使用2个训练视图）实现了显著的2.75dB的PSNR改进。

Conclusion: 该方法在极稀疏视图条件下实现了显著的性能提升，PSNR提高了2.75dB，合成图像失真小，高频细节丰富，视觉质量优于现有技术。
...
TL;DR: LGMSNet是一种新的轻量级医学图像分割框架，它通过局部和全局双重多尺度方法，在保持高性能的同时，降低了计算开销，并在泛化能力方面表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有的轻量级模型通常以牺牲性能换取效率，很少采用计算成本高的注意力机制，限制了全局上下文感知能力。此外，目前的架构忽略了医学图像中相同卷积核下的通道冗余问题，阻碍了有效的特征提取。

Method: 提出了一种基于局部和全局双重多尺度的轻量级框架LGMSNet，采用异构层内内核提取局部高频信息，并整合稀疏transformer-卷积混合分支捕获低频全局信息。

Result: LGMSNet在六个公共数据集上表现优于现有方法，并在四个未见数据集的zero-shot泛化测试中保持了出色的性能。

Conclusion: LGMSNet在六个公共数据集上表现优于现有方法，并在zero-shot泛化测试中表现出色，适合资源受限的医疗场景。
...
### [91] [MExECON: Multi-view Extended Explicit Clothed humans Optimized via Normal integration](https://arxiv.org/abs/2508.15500)
*Fulden Ece Uğur,Rafael Redondo,Albert Barreiro,Stefan Hristov,Roger Marí*

Main category: cs.CV

TL;DR: MExECON：一种用于从稀疏的多视角 RGB 图像中进行服装人体化身 3D 重建的新流程，它优于单视角基线并实现了具有竞争力的性能。


<details>
  <summary>Details</summary>
Motivation: 本研究提出了一种新的流程 MExECON，用于从稀疏的多视角 RGB 图像中进行服装人体化身的 3D 重建。在单视角方法 ECON 的基础上，MExECON 扩展了其功能以利用多个视角，从而改进了几何和身体姿势估计。

Method: 联合多视角身体优化 (JMBO) 算法，该算法在所有输入视图中联合拟合单个 SMPL-X 身体模型，从而增强多视角一致性。优化的身体模型用作低频先验，指导后续的表面重建，其中通过法线贴图集成添加了几何细节。

Result: MExECON 集成了来自正面和背面视角的法线贴图，以准确捕获细粒度的表面细节，例如衣服褶皱和发型。所有多视角增益的实现都无需任何网络重新训练。

Conclusion: MExECON在保真度上始终优于单视角基线，并且与现代少镜头3D重建方法相比，实现了具有竞争力的性能。
...
### [92] [Task-Generalized Adaptive Cross-Domain Learning for Multimodal Image Fusion](https://arxiv.org/abs/2508.15505)
*Mengyu Wang,Zhenyu Liu,Kun Li,Yu Wang,Yuwei Wang,Yanyan Wei,Fei Wang*

Main category: cs.CV

TL;DR: AdaSFFuse是一种用于多模态图像融合的新框架，它使用自适应小波变换和Mamba块来提高融合性能，同时降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 当前的MMIF方法仍然面临着模态不对齐、高频细节破坏和任务特定的限制等挑战。

Method: 提出了一种名为AdaSFFuse的新框架，用于通过自适应跨域协同融合学习实现任务通用MMIF。该框架引入了自适应近似小波变换（AdaWAT）和空间频率Mamba块。

Result: 在红外-可见光图像融合（IVF）、多焦点图像融合（MFF）、多曝光图像融合（MEF）和医学图像融合（MIF）四个MMIF任务上的大量实验表明，AdaSFFuse具有卓越的融合性能。

Conclusion: AdaSFFuse通过结合自适应近似小波变换（AdaWAT）和空间频率Mamba块，在多种MMIF任务上表现出卓越的融合性能，同时保持低计算成本和紧凑的网络。
...
### [93] [ExtraGS: Geometric-Aware Trajectory Extrapolation with Uncertainty-Guided Generative Priors](https://arxiv.org/abs/2508.15529)
...
TL;DR: ExtraGS是一种轨迹外推的整体框架，它集成了几何和生成先验，以提高外推视图的真实感和几何一致性。


<details>
  <summary>Details</summary>
Motivation: 从记录的驾驶日志中合成外推视图对于模拟自动驾驶车辆的驾驶场景至关重要，但仍然是一项具有挑战性的任务。最近的方法利用生成先验作为伪地面真值，但通常导致较差的几何一致性和过度平滑的渲染。

Method: 一种基于混合高斯-有符号距离函数（SDF）设计的道路表面高斯（RSG）表示，以及使用可学习缩放因子以有效处理远处物体的远场高斯（FFG）。

Result: 在多个数据集、不同的多相机设置和各种生成先验上的大量实验表明，
...
### [94] [Multi-Object Sketch Animation with Grouping and Motion Trajectory Priors](https://arxiv.org/abs/2508.15535)
*Guotao Liang,Juncheng Hu,Ximing Xing,Jing Zhang,Qian Yu*

Main category: cs.CV

TL;DR: GroupSketch 是一种新的向量草图动画方法，可以有效地处理多对象交互和复杂运动。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理多对象交互和复杂运动方面存在困难，要么仅限于单对象情况，要么遭受时间不一致和泛化性差的问题。

Method: GroupSketch 采用两阶段流程，包括运动初始化和运动细化。第一阶段通过插值生成粗略动画；第二阶段提出了一种基于组的位移网络 (GDN)，通过预测特定于组的位移场来细化粗略动画，利用来自文本到视频模型的先验知识。GDN 进一步结合了专门的模块，例如上下文条件特征增强 (CCFE)，以提高时间一致性。

Result: GroupSketch 在为复杂的多对象草图生成高质量、时间上一致的动画方面，明显优于现有方法。

Conclusion: GroupSketch 显著优于现有方法，能够为复杂的多对象草图生成高质量、时间上一致的动画，从而扩展了草图动画的实际应用。
...
### [95] [A Fully Spectral Neuro-Symbolic Reasoning Architecture with Graph Signal Processing as the Computational Backbone](https://arxiv.org/abs/2508.14923)
*Andrew Kiruluta*

Main category: cs.AI

TL;DR: This paper introduces a graph signal processing based neuro-symbolic reasoning architecture.


<details>
  <summary>Details</summary>
Motivation: integrating symbolic logic and neural inference

Method: a fully spectral, neuro-symbolic reasoning architecture that leverages Graph Signal Processing (GSP) as the primary computational backbone for integrating symbolic logic and neural inference. The entire reasoning pipeline is formulated in the graph spectral domain. Logical entities and relationships are encoded as graph signals, processed via learnable spectral filters, and mapped into symbolic predicates for rule-based inference.
...
### [96] [Goals and the Structure of Experience](https://arxiv.org/abs/2508.15013)
*Nadav Amir,Stas Tiomkin,Angela Langdon*

Main category: cs.AI

TL;DR: 本文提出了一个目标导向的状态表征计算框架，其中世界模型的描述性和规范性方面共同产生。


<details>
  <summary>Details</summary>
Motivation: 有目的的行为是自然和人工智能的标志。它的获得通常被认为依赖于世界模型，世界模型包括描述性（是什么）和规范性（什么是理想的）方面，分别识别和评估世界中的事态。有目的行为的规范计算帐户，如强化学习，假设世界模型的不同组成部分包括状态表示（描述性方面）和奖励函数（规范性方面）。然而，另一种可能性是，这两个方面反而相互依赖地从agent的目标中共同出现。

Method: 基于佛教认识论，我们引入了目标导向或目的性状态的结构，定义为目标等效经验分布的类别。Telic状态根据行为策略和理想经验特征之间的统计差异，为目标导向学习提供了一种简约的解释。

Result: Telic状态根据行为策略和理想经验特征之间的统计差异，为目标导向学习提供了一种简约的解释。我们回顾了支持这一新视角的经验和理论文献，并讨论了其在提供跨不同基质的有目的行为的行为、现象学和神经维度统一解释方面的潜力。

Conclusion: 我们提出了认知代理中目标导向状态表征的计算框架，其中世界模型的描述性和规范性方面来自agent与环境的互动序列或经验。
...
### [97] [Collab-REC: An LLM-based Agentic Framework for Balancing Recommendations in Tourism](https://arxiv.org/abs/2508.15030)
*Ashmi Banerjee,Fitri Nur Aisyah,Adithi Satish,Wolfgang Wörndl,Yashar Deldjoo*

Main category: cs.AI

TL;DR: Collab-REC, a multi-agent framework, improves diversity and relevance in tourism recommendations by combining LLM-based agents and a moderator.
...
Method: a multi-agent framework with three LLM-based agents and a non-LLM moderator

Result: improves diversity and overall relevance compared to a single-agent baseline

Conclusion: Collab-REC improves diversity and overall relevance and addresses over-tourism.
...
TL;DR: This paper introduces a novel method using LLMs to control crowd agent movement through dialogue and language-driven navigation, resulting in more realistic crowd simulations with emergent group behaviors.
...
Result: Grouping and ungrouping of agents automatically occur. The method serves as an information-passing mechanism within the crowd.

Conclusion: The proposed method produces more realistic crowd simulations, with emergent group behaviours arising naturally from any environmental setting.
...
### [99] [Don't Think Twice! Over-Reasoning Impairs Confidence Calibration](https://arxiv.org/abs/2508.15050)
*Romain Lacombe,Kerrie Wu,Eddie Dilworth*

Main category: cs.AI

TL;DR: 大型语言模型在知识密集型任务中，信息获取比推理更重要，能够提高置信度校准。


<details>
  <summary>Details</summary>
Motivation: 避免大型语言模型作为问答工具部署时过度自信，需要稳健的校准。

Method: 使用 ClimateX 数据集，并将其扩展到人类和地球健康领域，系统地评估了推理能力和预算如何影响置信度评估的准确性。

Result: 增加推理预算会持续损害而非改善校准。扩展的推理会导致系统性的过度自信，随着思考预算的增加而恶化，产生递减和负回报。搜索增强生成显著优于纯推理，通过检索相关证据实现了 89.3% 的准确率。
...
### [100] [Demonstrating Onboard Inference for Earth Science Applications with Spectral Analysis Algorithms and Deep Learning](https://arxiv.org/abs/2508.15053)
*Itai Zilberstein,Alberto Candela,Steve Chien,David Rijlaarsdam,Tom Hendrix,Leonie Buckley,Aubrey Dunne*

Main category: cs.AI

TL;DR: demonstrates data analysis onboard CS-6 for Earth science using deep learning and spectral analysis


<details>
  <summary>Details</summary>
Motivation: Performing data analysis at the edge (e.g. onboard) can enable new Earth science measurements and responses.

Method: deep learning and spectral analysis algorithms

Result: demonstrating state-of-the-art data analysis onboard CogniSAT-6/HAMMER (CS-6)

Conclusion: We will demonstrate data analysis and inference onboard CS-6 for numerous applications using deep learning and spectral analysis algorithms.
...
### [101] [S3LoRA: Safe Spectral Sharpness-Guided Pruning in Adaptation of Agent Planner](https://arxiv.org/abs/2508.15068)
*Shuang Ao,Gopal Rumchurn*

Main category: cs.AI

TL;DR: S3LoRA is a lightweight framework that improves the safety of LoRA-adapted LLM agents by pruning potentially unsafe layers, without sacrificing performance or requiring additional data or model checkpoints.


<details>
  <summary>Details</summary>
Motivation: Adapting Large Language Models (LLMs) using parameter-efficient fine-tuning (PEFT) techniques can compromise safety alignment, leading to unsafe behaviors in agent planning tasks. Existing safety-aware adaptation methods are often impractical due to requiring access to unavailable model checkpoints.

Method: S3LoRA, a lightweight, data-free, and model-independent framework, mitigates safety risks in LoRA-adapted models by inspecting only the fine-tuned weight updates. It uses Magnitude-Aware Spherically Normalized SVD (MAS-SVD) to analyze LoRA updates and the Spectral Sharpness Index (SSI) to detect and prune potentially unsafe layers.
...
### [102] [Argumentation for Explainable Workforce Optimisation (with Appendix)](https://arxiv.org/abs/2508.15118)
*Jennifer Leigh,Dimitrios Letsios,Alessandro Mella,Lucio Machetti,Francesca Toni*

Main category: cs.AI

TL;DR: 劳动力管理可以被理解为抽象论证，从而适应变化并提供解释，实验表明该方法优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 劳动力管理是一个复杂的问题，需要优化操作员团队完成一组工作所需的完工时间和行驶距离，使用一组仪器。劳动力管理的一个关键挑战是在执行时适应变化，以便向所有相关人员提供解释。

Method: 将劳动力管理理解为抽象论证

Result: 我们的工具和解释比传统的手动解决方案更快、更准确地解决问题。

Conclusion: 通过将劳动力管理理解为工业应用中的抽象论证，我们可以适应变化并获得忠实的解释。用户研究表明，我们的工具和解释比传统的手动解决方案更快、更准确地解决问题。
...
### [103] [Open-Universe Assistance Games](https://arxiv.org/abs/2508.15119)
*Rachel Ma,Jingyi Qu,Andreea Bobu,Dylan Hadfield-Menell*

Main category: cs.AI

TL;DR: 论文介绍了一种名为 GOOD 的数据高效在线方法，用于提取自然语言形式的目标，并推断自然语言目标的分布，从而使具身 AI 智能体能够在开放世界辅助游戏中更好地与人类交互。


<details>
  <summary>Details</summary>
Motivation: 具身 AI 智能体必须以可解释的方式推断和执行各种未预定义的人类目标和偏好。为了形式化这种设置，我们引入了开放世界辅助游戏 (OU-AG)，这是一个框架，智能体必须在无限且不断演变的目标空间中进行推理。

Method: GOOD 是一种数据高效的在线方法，可以在与人类交互期间以自然语言的形式提取目标，并推断自然语言目标的分布。

Result: GOOD 提示 LLM 模拟具有不同复杂意图的用户，使用其响应对候选目标执行概率推理。这种方法无需大型离线数据集即可实现丰富的目标表示和不确定性估计。

Conclusion: 该方法在文本杂货店购物领域和文本操作的模拟家庭机器人环境 (AI2Thor) 中优于没有明确目标跟踪的基线，这已通过基于 LLM 和人工评估得到证实。
...
### [104] [aiXiv: A Next-Generation Open Access Ecosystem for Scientific Discovery Generated by AI Scientists](https://arxiv.org/abs/2508.15126)
...
TL;DR: 提出了aiXiv，一个面向人类和AI科学家的开放获取平台，以解决AI生成研究内容缺乏合适发表场所的问题。


<details>
  <summary>Details</summary>
Motivation: 大量高质量的人工智能生成研究缺乏适当的传播场所，阻碍了其推动科学进步的潜力。为了应对这些挑战，我们引入了aiXiv。

Method: 引入aiXiv，一个面向人类和人工智能科学家的下一代开放获取平台。其多智能体架构允许人类和人工智能科学家提交、审查和迭代改进研究提案和论文。它还提供API和MCP接口，可以无缝集成异构的人类和人工智能科学家，从而创建一个可扩展的自主科学发现生态系统。

Result: 通过广泛的实验，证明aiXiv是一个可靠且强大的平台，经过在aiXiv上的迭代修改和审查，可以显著提高人工智能生成的研究提案和论文的质量。

Conclusion: aiXiv是一个可靠且强大的平台，经过在aiXiv上的迭代修改和审查，可以显著提高人工智能生成的研究提案和论文的质量。该平台为下一代人工智能科学家开放获取生态系统奠定了基础，加速了高质量人工智能生成研究内容的出版和传播。
...
### [105] [Mobile-Agent-v3: Foundamental Agents for GUI Automation](https://arxiv.org/abs/2508.15144)
*Jiabo Ye,Xi Zhang,Haiyang Xu,Haowei Liu,Junyang Wang,Zhaoqing Zhu,Ziwei Zheng,Feiyu Gao,Junjie Cao,Zhengxi Lu,Jitong Liao,Qi Zheng,Fei Huang,Jingren Zhou,Ming Yan*

Main category: cs.AI

TL;DR: 该论文介绍了GUI-Owl和Mobile-Agent-v3，它们是用于GUI任务的先进开源代理，通过自进化轨迹生成和强化学习实现高性能。


<details>
  <summary>Details</summary>
Motivation: 构建能够理解和操作图形用户界面（GUI）的智能代理。

Method: 该论文介绍了GUI-Owl，它通过大规模环境基础设施、多样化的基础代理能力和可扩展的环境强化学习来实现。

Result: GUI-Owl-7B在AndroidWorld上达到66.4，在OSWorld上达到29.4。Mobile-Agent-v3在AndroidWorld上达到73.3，在OSWorld上达到37.7。

Conclusion: GUI-Owl和Mobile-Agent-v3是开源的，并在多个GUI基准测试中取得了最先进的性能。
...
### [106] [PuzzleClone: An SMT-Powered Framework for Synthesizing Verifiable Data](https://arxiv.org/abs/2508.15180)
...
### [107] [SurgWound-Bench: A Benchmark for Surgical Wound Diagnosis](https://arxiv.org/abs/2508.15189)
...
TL;DR: 我们创建了一个包含 697 张带注释的手术伤口图像的开源数据集 SurgWound，并提出了一个用于手术伤口诊断的三阶段学习框架 WoundQwen。


<details>
  <summary>Details</summary>
Motivation: 手术部位感染 (SSI) 是最常见和代价最高的医疗相关感染之一，手术伤口护理仍然是预防 SSI 和改善患者预后的一个重大临床挑战。目前，没有公开可用的数据集或基准包含各种类型的手术伤口，导致缺乏开源的手术伤口筛查工具。

Method: 我们提出了一个三阶段学习框架 WoundQwen，用于手术伤口诊断。

Result: 我们提出了 SurgWound，这是第一个具有各种手术伤口类型的开源数据集。它包含 697 张手术伤口图像，由 3 位专业外科医生注释了 8 个细粒度临床属性。我们引入了第一个手术伤口诊断基准，其中包括视觉问题解答 (VQA) 和报告生成任务，以全面评估模型性能。

Conclusion: 我们提出了一个三阶段学习框架，可以分析详细的手术伤口特征，并根据手术图像为患者提供后续指导，从而为个性化伤口护理、及时干预和改善患者预后铺平道路。
...
### [108] [LLM4Sweat: A Trustworthy Large Language Model for Hyperhidrosis Support](https://arxiv.org/abs/2508.15192)
*Wenjie Lin,Jin Wei-Kocsis*

Main category: cs.AI

TL;DR: LLM4Sweat是一个用于多汗症的开源LLM框架，它通过数据增强、微调和专家评估的三阶段流程，为罕见疾病提供诊断、个性化治疗建议和心理支持。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）在医疗保健领域显示出前景，但它们在罕见疾病中的应用仍然受到稀缺且不可靠的微调数据集的阻碍。多汗症是一种超出身体需要的过度出汗疾病，影响了2-3%的人口，并严重影响了身体舒适度和心理健康。目前还没有专门为LLM定制的，以推进多汗症的诊断或治疗。

Method: 该系统遵循一个三阶段流程：数据增强阶段、微调阶段以及推理和专家评估阶段。

Result: 实验表明，LLM4Sweat优于基线模型，并提供了首个用于多汗症的开源LLM框架。

Conclusion: LLM4Sweat优于基线模型，并为多汗症提供了首个开源LLM框架，为其他具有类似数据和可信度挑战的罕见疾病提供了一种通用方法。
...
### [109] [R-ConstraintBench: Evaluating LLMs on NP-Complete Scheduling](https://arxiv.org/abs/2508.15204)
*Raj Jain,Marc Wetter*

Main category: cs.AI

TL;DR: R-ConstraintBench 是一个评估 LLM 在资源约束项目调度问题上的可靠性的框架。结果表明，约束交互是主要瓶颈，并且在干净的合成斜坡上的性能不能保证转移到领域基础的场景。


<details>
  <summary>Details</summary>
Motivation: 在高约束情况下进行推理时，大型语言模型 (LLM) 的可靠性尚未得到充分表征。为了解决这一差距，

Method: 我们提出了 R-ConstraintBench，这是一个可扩展的框架，用于评估资源约束项目调度问题 (RCPSP) 上的模型，这是一个 NP-Complete 可行性类，而难度通过约束中的线性增长来增加。R-ConstraintBench 以有向无环图 (DAG) 递增地增加非冗余优先级约束，然后引入停机时间、时间窗口和分离约束。

Result: 通过可行性和误差分析，识别出与失败最相关的退化阈值和约束类型。

Conclusion: 强模型在仅具有优先级的 DAG 上接近上限，但当停机时间、时间窗口和分离约束相互作用时，可行性性能会崩溃，这意味着约束交互，而不是图深度，是主要瓶颈。在干净的合成斜坡上的性能也不能保证转移到领域基础的场景，突出了有限的泛化。
...
### [110] [See it. Say it. Sorted: Agentic System for Compositional Diagram Generation](https://arxiv.org/abs/2508.15222)
*Hantao Zhang,Jingyang Liu,Ed Li*

Main category: cs.AI

TL;DR: See it. Say it. Sorted. is a training-free agentic system using VLMs and LLMs to convert hand sketches into editable SVGs, outperforming other LLMs in layout and structure reconstruction.


<details>
  <summary>Details</summary>
Motivation: Diffusion models struggle with spatial precision, alignment, and symbolic structure required for flowcharts when converting rough hand sketches into precise diagrams.

Method: The system couples a Vision-Language Model (VLM) with Large Language Models (LLMs) in an iterative loop involving a Critic VLM, multiple candidate LLMs, and a Judge VLM.
...
Conclusion: The paper introduces See it. Say it. Sorted., a training-free agentic system that reconstructs layouts and structures from hand sketches into editable SVGs, outperforming frontier closed-source image generation LLMs.
...
### [111] [Computational Intelligence based Land-use Allocation Approaches for Mixed Use Areas](https://arxiv.org/abs/2508.15240)
*Sabab Aosaf,Muhammad Ali Nayeem,Afsana Haque,M Sohel Rahmana*

Main category: cs.AI

TL;DR: The paper introduces new computational intelligence approaches for optimizing land-use allocation, achieving improvements in land-use compatibility and price optimization through custom algorithms and constraint relaxation techniques.


<details>
  <summary>Details</summary>
Motivation: Urban land-use allocation is a complex multi-objective optimization problem critical for sustainable urban development policy, with inherent trade-offs between land-use compatibility and economic objectives.

Method: The study develops multiple optimization algorithms, including custom variants integrating differential evolution with multi-objective genetic algorithms, such as CR+DES and MSBX+MO.

Result: CR+DES achieves 3.16% improvement in land-use compatibility, while MSBX+MO excels in price optimization with 3.3% improvement. Statistical analysis confirms algorithms incorporating difference vectors significantly outperform traditional approaches. The constraint relaxation technique enables broader solution space exploration while maintaining practical constraints.

Conclusion: The study provides evidence-based computational tools for urban planners and policymakers to balance competing objectives in land-use allocation, supporting more effective urban development policies in rapidly urbanizing regions.
...
### [112] [Multiple Memory Systems for Enhancing the Long-term Memory of Agent](https://arxiv.org/abs/2508.15294)
*Gaoke Zhang,Bo Wang,Yunlong Ma,Dongming Zhao,Zifei Yu*

Main category: cs.AI

TL;DR: A multiple memory system (MMS) is designed to enhance knowledge and utilize historical data by processing short-term memory into retrieval and contextual memory units. Experiments on LoCoMo dataset proves it's effectiveness.


<details>
  <summary>Details</summary>
Motivation: Effectively handling vast amounts of historical data generated during interactions remains a challenge. Existing methods have poor quality of stored memory content, which affects recall performance and response quality.

Method: A multiple memory system (MMS) inspired by cognitive psychology theory is designed to process short-term memory into multiple long-term memory fragments, constructing retrieval and contextual memory units.

Result: Experiments on LoCoMo dataset compared our method with three others, proving its effectiveness. Ablation studies confirmed the rationality of our memory units. The robustness regarding the number of selected memory segments and the storage overhead was analyzed, demonstrating its practical value.

Conclusion: The MMS method's effectiveness, the rationality of the memory units, and its practical value were confirmed through experiments.
...
### [113] [Coarse-to-Fine Grounded Memory for LLM Agent Planning](https://arxiv.org/abs/2508.15305)
*Wei Yang,Jinwei Xiao,Hongming Zhang,Qingyang Zhang,Yanna Wang,Bo Xu*

Main category: cs.AI

TL;DR: Coarse-to-Fine Grounded Memory (Ours) grounds coarse-to-fine memories with LLM, thereby fully leverage them for flexible adaptation to diverse scenarios.
...
Result: Ours retrieves task-relevant experiences and tips to support planning
...
### [114] [Search-Based Credit Assignment for Offline Preference-Based Reinforcement Learning](https://arxiv.org/abs/2508.15327)
*Xiancheng Gao,Yufeng Shi,Wengang Zhou,Houqiang Li*

Main category: cs.AI

TL;DR: 提出了一种名为SPW的算法，它结合了专家演示和偏好两种反馈，以解决离线强化学习中奖励函数设计困难的问题。


<details>
  <summary>Details</summary>
Motivation: 离线强化学习通常依赖于定义明确的奖励函数，而奖励函数的设计既困难又昂贵。人类反馈是一种有吸引力的替代方案，但其两种常见形式，专家演示和偏好，具有互补的局限性。演示提供逐步监督，但收集成本高昂，并且通常反映有限的专家行为模式。相比之下，偏好更容易收集，但不清楚行为的哪些部分对轨迹段的贡献最大，从而导致信用分配未解决。

Method: 本文介绍了一种基于搜索的偏好加权（SPW）方案，以统一这两种反馈源。对于偏好标记轨迹中的每个transition，SPW从专家演示中搜索最相似的状态-动作对，并直接根据它们的相似性得分导出逐步的重要性权重。然后，这些权重用于指导标准偏好学习，从而实现传统方法难以实现的更准确的信用分配。
...
### [116] [DiagECG: An LLM-Driven Framework for Diagnostic Reasoning via Discretized ECG Tokenization](https://arxiv.org/abs/2508.15338)
*Jinning Yang,Wen Shi*

Main category: cs.AI

TL;DR: DiagECG integrates time-series and language modeling to process 12-lead ECG signals for clinical text generation tasks. It discretizes continuous ECG embeddings into symbolic tokens and uses these tokens to extend the vocabulary of LLM. The model is pretrained on an autoregressive ECG forecasting task and instruction tuned on ECG question answering and diagnostic report generation.


<details>
  <summary>Details</summary>
Motivation: Existing automated approaches often struggle to generalize across clinical tasks and offer limited support for open-ended reasoning in cardiovascular diagnostics.

Method: The approach discretizes continuous ECG embeddings into symbolic tokens using a lead-independent encoder and quantization module. These tokens are then used to extend the vocabulary of LLM, allowing the model to handle both ECG and natural language inputs in a unified manner. The model is pretrained on an autoregressive ECG forecasting task, enabling the LLM to model temporal dynamics using its native language modeling capabilities. Instruction tuning is performed on both ECG question answering and diagnostic report generation.

Result: DiagECG achieves strong performance across tasks while maintaining generalization to out-of-distribution settings. Experiments demonstrate the effectiveness of each component.

Conclusion: DiagECG achieves strong performance across tasks while maintaining generalization to out-of-distribution settings. Integrating symbolic ECG representations into LLMs has potential for medical reasoning.
...
### [117] [Planning with Minimal Disruption](https://arxiv.org/abs/2508.15358)
*Alberto Pozanco,Marianela Morales,Daniel Borrajo,Manuela Veloso*

Main category: cs.AI

TL;DR: introduce plan disruption and define various planning-based compilations that aim to jointly optimize both the sum of action costs and plan disruption
...
### [118] [GraSP: A Unified Graph-Based Framework for Scalable Generation, Quality Tagging, and Management of Synthetic Data for SFT and DPO](https://arxiv.org/abs/2508.15432)
*Bidyapati Pradhan,Surajit Dasgupta,Amit Kumar Saha,Omkar Anustoop,Sriram Puttagunta,Vipul Mittal,Gopal Sarda*

Main category: cs.AI

TL;DR: 论文提出了一个合成数据生成框架，用于大规模生成高质量对话数据，以减少LLM训练的数据准备工作。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）的发展关键依赖于高质量数据集的可用性，这些数据集用于监督微调（SFT）和直接偏好优化（DPO）等对齐任务。

Method: 该框架采用模块化和基于配置的流水线，能够以最小的人工干预来建模复杂的对话流程。它使用双阶段质量标记机制，结合启发式规则和基于LLM的评估，自动过滤和评分从OASST格式的对话中提取的数据。

Result: 生成的数据集在灵活的模式下构建，支持SFT和DPO用例，从而可以无缝集成到各种训练工作流程中。

Conclusion: 这篇论文提出了一个全面的合成数据生成框架，可以大规模、可配置地生成高质量的合成数据，用于监督微调（SFT）和直接偏好优化（DPO）等对齐任务，从而显著减少LLM训练流程中数据准备的开销。
...
### [119] [From Bits to Boardrooms: A Cutting-Edge Multi-Agent LLM Framework for Business Excellence](https://arxiv.org/abs/2508.15447)
*Zihao Wang,Junming Zhang*

Main category: cs.AI

TL;DR: BusiAgent, a new LLM-based multi-agent framework, improves enterprise decision-making by integrating granular insights with high-level strategy.


<details>
  <summary>Details</summary>
Motivation: Current LLM approaches struggle to reconcile intricate operational analyses with overarching strategic goals, leading to fragmented workflows and reduced collaboration.

Method: a novel multi-agent framework leveraging LLMs, integrating an extended Continuous Time Markov Decision Process (CTMDP), a generalized entropy measure, and a multi-level Stackelberg game.
...
### [120] [Think in Blocks: Adaptive Reasoning from Direct Response to Deep Reasoning](https://arxiv.org/abs/2508.15507)
*Yekun Zhu,Guang Chen,Chengjun Mao*

Main category: cs.AI

TL;DR: The paper proposes a Think in Blocks framework for LLMs, allowing them to adaptively adjust reasoning length based on task complexity, using a three-stage training pipeline and block-structured reasoning.


<details>
  <summary>Details</summary>
Motivation: Excessively long chains of thought in LLMs can lead to overthinking, causing computational waste and slower responses. The paper addresses the question of whether LLMs can dynamically adjust the length of their reasoning processes based on task complexity.

Method: The paper proposes a three-stage pipeline - Supervised Fine-Tuning, reward-guided Direct Preference Optimization, and Reinforcement Learning - to train an adaptive model.

Result: The paper establishes an explicit block-structured paradigm in which the model first predicts an integer reasoning budget and then partitions its reasoning accordingly. The explicit block count is exploited to dynamically control reasoning depth at inference time.

Conclusion: The paper introduces a Think in Blocks framework that enables LLMs to adaptively adjust the length of their reasoning processes based on task complexity.
...
### [121] [Super-additive Cooperation in Language Model Agents](https://arxiv.org/abs/2508.15510)
*Filippo Tonini,Lukas Galke*

Main category: cs.AI

TL;DR: Repeated interactions and inter-group rivalry boost cooperation in language model agents playing Prisoner's Dilemma, suggesting insights for AI system design.
...
Method: We devised a virtual tournament where language model agents, grouped into teams, face each other in a Prisoner's Dilemma game. By simulating both internal team dynamics and external competition, we discovered that this blend substantially boosts both overall and initial, one-shot cooperation levels.

Result: This blend substantially boosts both overall and initial, one-shot cooperation levels.

Conclusion: Intergroup competition can lead to more cooperative behavior, crucial for designing future multi-agent AI systems.
...
### [122] [DeepThink3D: Enhancing Large Language Models with Programmatic Reasoning in Complex 3D Situated Reasoning Tasks](https://arxiv.org/abs/2508.15548)
*Jiayi Song,Rui Wan,Lipeng Ma,Weidong Yang,Qingyuan Zhou,Yixuan Li,Ben Fei*

Main category: cs.AI

TL;DR: DeepThink3D旨在增强大型语言模型在复杂3D场景中执行复杂推理的能力。


<details>
  <summary>Details</summary>
Motivation: 为了解决数据集问题简单，生成的程序推理链相对较短这一主要挑战。

Method: 提出了一种在SQA3D基准上进行组合和迭代演化方法，以生成更复杂的问题。在此基础上，对大型语言模型进行微调，使其更精通3D工具的使用。

Result: 通过采用直接偏好优化（DPO），直接优化模型生成的工具链策略，从而提高它们在复杂任务中的准确性。

Conclusion: 通过直接偏好优化（DPO）直接优化模型生成的工具链策略，从而提高它们在复杂任务中的准确性。
...
### [123] [A Dynamical Systems Framework for Reinforcement Learning Safety and Robustness Verification](https://arxiv.org/abs/2508.15588)
*Ahmed Nasir,Abdelhafid Zenati*

Main category: cs.AI

TL;DR: This paper introduces a framework using dynamical systems theory to analyze RL policies, providing safety guarantees and robustness metrics, and identifying flaws in seemingly successful policies.
...
Method: analyzing the combination of an RL agent and its environment as a discrete-time autonomous dynamical system, leveraging tools from dynamical systems theory, specifically the Finite-Time Lyapunov Exponent (FTLE), we identify and visualize Lagrangian Coherent Structures (LCS) that act as the hidden "skeleton" governing the system's behavior. To move beyond qualitative visualization, we introduce a suite of quantitative metrics, Mean Boundary Repulsion (MBR), Aggregated Spurious Attractor Strength (ASAS), and Temporally-Aware Spurious Attractor Strength (TASAS), to formally measure a policy's safety margin and robustness. We further provide a method for deriving local stability guarantees and extend the analysis to handle model uncertainty.

Result: repelling LCS function as safety barriers around unsafe regions, while attracting LCS reveal the system's convergence properties and potential failure modes, such as unintended "trap" states. Through experiments in both discrete and continuous control environments, we show that this framework provides a comprehensive and interpretable assessment of policy behavior, successfully identifying critical flaws in policies that appear successful based on reward alone.
...
### [124] [Transduction is All You Need for Structured Data Workflows](https://arxiv.org/abs/2508.15610)
*Alfio Gliozzo,Naweed Khan,Christodoulos Constantinides,Nandana Mihindukulasooriya,Nahuel Defosse,Junkyu Lee*

Main category: cs.AI

TL;DR: Agentics is a modular framework for agent-based systems that focuses on modeling data rather than crafting prompts, achieving strong results in question answering, semantic parsing and prompt optimization.


<details>
  <summary>Details</summary>
Motivation: This paper introduces Agentics, a modular framework for building agent-based systems capable of structured reasoning and compositional generalization over complex data. Agentics encourages AI developers to focus on modeling data rather than crafting prompts.

Method: The framework uses agents abstracted from the logical flow and used internally to the data type to enable logical transduction among data. It enables a declarative language in which data types are provided by LLMs and composed through logical transduction, which is executed by LLMs when types are connected.

Result: The framework achieves state-of-the-art accuracy or improved scalability without sacrificing performance.

Conclusion: The Agentics framework achieves state-of-the-art accuracy or improved scalability without sacrificing performance across domain-specific multiple-choice question answering, semantic parsing for text-to-SQL, and automated prompt optimization tasks.
...
### [125] [Adapting A Vector-Symbolic Memory for Lisp ACT-R](https://arxiv.org/abs/2508.15630)
*Meera Ray,Christopher L. Dancy*

Main category: cs.AI

TL;DR: HDM是一种ACT-R的声明性记忆的向量符号替代方案，它具有可扩展性等优点。


<details>
  <summary>Details</summary>
Motivation: 全息声明性记忆（HDM）是ACT-R声明性记忆（DM）系统的向量符号替代方案，可以带来可扩展性以及DM块之间架构定义的相似性等优势。

Method: 将HDM适配为与ACT-R一起工作，开发了基于向量的ACT-R函数版本，建立了一个文本处理管道以将大型文档的内容添加到ACT-R记忆中，并创建了一种有用的新机制，可以仅使用令牌的向量表示来检索整个记忆块。

Result: 初步结果表明，HDM可以保持其向量符号优势（例如，无需存储实际块的块召回以及与缩放相关的其他优势）。

Conclusion: HDM可以保持其向量符号优势，同时扩展它，以便先前的ACT-R模型可以在对模型的实际程序和声明性记忆部分进行很少（或可能没有）修改的情况下与系统一起工作。
...
### [126] [Understanding Action Effects through Instrumental Empowerment in Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2508.15652)
*Ardian Selmonaj,Miroslav Strupl,Oleg Szehr,Alessandro Antonucci*

Main category: cs.AI

TL;DR: 该论文提出了一种新方法，通过分析策略分布来推断MARL系统中agent的贡献，无需价值反馈。


<details>
  <summary>Details</summary>
Motivation: 为了可靠地部署多智能体强化学习（MARL）系统，理解团队中各个智能体的行为至关重要。以往的工作通常基于显式奖励信号或学习的价值函数来评估整体团队表现，但目前尚不清楚在没有任何价值反馈的情况下如何推断智能体的贡献。

Method: 该方法基于信息论的Shapley值，用于量化每个agent对其合作者工具性赋权的影响，称为预期合作价值（ICV）。

Result: 通过分析合作和竞争性MARL环境，揭示了智能体采取相似或不同策略的程度。通过比较策略和价值函数之间的行动效果，该方法识别出哪些agent行为对团队成功有益。

Conclusion: 该方法通过促进确定性决策或为未来的行动选择保留灵活性，从而确定哪些agent行为有益于团队成功，为合作动态提供了新的见解，并增强了MARL系统的可解释性。
...
### [127] [Futurity as Infrastructure: A Techno-Philosophical Interpretation of the AI Lifecycle](https://arxiv.org/abs/2508.15680)
*Mark Cote,Susana Aires*

Main category: cs.AI

TL;DR: 本文分析了欧盟人工智能法案，揭示了人工智能系统中数据生命周期的递归价值链挑战了现有的负责任人工智能框架，并提出了相应的监管措施。


<details>
  <summary>Details</summary>
Motivation: 现有的负责任人工智能框架无法应对人工智能系统中数据生命周期的长期动态，尤其是在数据摄取到部署过程中产生的递归价值链。

Method: 跨学科方法，借鉴西蒙东的技术哲学，改造他的个体化概念来建模人工智能生命周期。

Result: 提出了一个概念工具来构建人工智能pipeline，强调了数据、训练机制、架构、特征存储和迁移学习的重要性。引入了“未来性”的概念，即人工智能的自我强化生命周期。

Conclusion: 有效监管必须解决这些基础设施和时间动态，并提出措施，包括生命周期审计、时间可追溯性、反馈问责制、递归透明度以及质疑递归重用的权利。
...
### [128] [GRAFT: GRaPH and Table Reasoning for Textual Alignment -- A Benchmark for Structured Instruction Following and Visual Reasoning](https://arxiv.org/abs/2508.15690)
*Abhigya Verma,Sriram Puttagunta,Seganrasan Subramanian,Sravan Ramachandran*

Main category: cs.AI

TL;DR: GRAFT是一个结构化的多模态基准，用于评估模型在指令跟随、视觉推理和视觉-文本对齐任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 评估模型在指令跟随、视觉推理和视觉-文本对齐任务上的表现。

Method: 程序化生成的图表和合成渲染的表格，使用Python可视化库创建，以确保对数据语义、结构和清晰度的控制。

Result: 引入了推理类型的分类，包括比较、趋势识别、排序、聚合、比例估计和异常检测，以实现全面的评估。参考答案遵循严格的事实和格式指南，以进行精确的、基于方面的评估。

Conclusion: GRAFT提供了一个统一、可扩展的框架，用于对多模态模型在视觉基础上的结构化推理任务进行细粒度基准测试，为该领域设定了新的评估标准。
...
### [129] [NiceWebRL: a Python library for human subject experiments with reinforcement learning environments](https://arxiv.org/abs/2508.15693)
...
<details>
  <summary>Details</summary>
Motivation: to enable researchers to use machine reinforcement learning (RL) environments for online human subject experiments

Method: a Python library that allows any Jax-based environment to be transformed into an online interface

Result: demonstrate its potential to help develop Human-like AI, Human-compatible AI, and Human-assistive AI
...
### [130] [Measuring the environmental impact of delivering AI at Google Scale](https://arxiv.org/abs/2508.15734)
...
TL;DR: 本文测量了Gemini AI助手服务的能源使用、碳排放和用水量，发现其环境影响相对较低，但仍需关注，并提出全面测量AI服务环境指标的重要性。
...
Result: Gemini Apps文本提示的能源消耗中位数为0.24 Wh，谷歌的软件效率提升和清洁能源采购使Gemini Apps文本提示的能源消耗降低了33倍，碳足迹降低了44倍。Gemini Apps文本提示的能源消耗低于观看9秒电视，用水量相当于5滴水。

Conclusion: 对AI服务的环境影响的全面测量对于准确比较模型以及适当激励整个AI服务堆栈中的效率提升至关重要。
...
### [131] [Response and Prompt Evaluation to Prevent Parasocial Relationships with Chatbots](https://arxiv.org/abs/2508.15748)
*Emma Rath,Stuart Armstrong,Rebecca Gorman*

Main category: cs.AI

TL;DR: A language model-based framework can evaluate conversations for parasocial cues in real time to prevent harmful parasocial relationships with AI agents.


<details>
  <summary>Details</summary>
Motivation: The development of parasocial relationships with AI agents has severe, and in some cases, tragic effects for human well-being. Preventing such dynamics is challenging because parasocial cues often emerge gradually and not all forms of emotional engagement are inherently harmful.

Method: Introducing a response evaluation framework, created by repurposing a state-of-the-art language model, that evaluates ongoing conversations for parasocial cues in real time. Testing with a small synthetic dataset of thirty dialogues.
...
### [132] [Language-Guided Tuning: Enhancing Numeric Optimization with Textual Feedback](https://arxiv.org/abs/2508.15757)
*Yuxing Lu,Yucheng Hu,Nan Sun,Xukai Zhao*

Main category: cs.AI

TL;DR: LGT 使用大型语言模型和自然语言推理来优化机器学习配置，优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 配置优化仍然是机器学习中的一个关键瓶颈，需要在模型架构、训练策略、特征工程和超参数之间进行协调调整。传统方法独立地处理这些维度，缺乏可解释性，而最近的自动化方法在动态适应性和关于优化决策的语义推理方面存在困难。

Method: LGT 采用多智能体大型语言模型，通过自然语言推理智能地优化配置。LGT 协调三个专门的代理：提出配置更改的 Advisor、评估进度的 Evaluator 和改进决策过程的 Optimizer，从而创建一个自我完善的反馈循环。

Result: LGT 优于传统的优化方法，实现了性能提升，同时保持了较高的可解释性。
...
### [133] [Random Sampling over Spatial Range Joins](https://arxiv.org/abs/2508.15070)
*Daichi Amagata*
...
<details>
  <summary>Details</summary>
Motivation: 空间范围连接在地理信息系统、基于位置的社交网络服务、神经科学和可视化等领域有许多应用。然而，连接不仅产生昂贵的计算成本，而且产生过大的结果集。一个实用且合理的方法是返回连接结果的随机样本。虽然这对于许多涉及空间范围连接的应用程序来说是有希望和充分的，但有效地计算随机样本并非易事。这是因为我们必须在不运行空间范围连接的情况下获得随机连接样本。

Method: 提出了一种新的数据结构，可以在$\\tilde{O}(n + m + t)$预期时间和$O(n+m)$空间内处理该问题。

Result: 使用四个真实的空间数据集进行了广泛的实验，结果表明，在大多数测试中，该算法明显快于基线算法。

Conclusion: 提出的算法在时间和空间效率上优于现有的随机抽样技术。
...
### [134] [Temporal $k$-Core Query, Revisited](https://arxiv.org/abs/2508.15238)
*Yinyu Liu,Kaiqiang Yu,Shengxin Liu,Cheng Long,Zhaoquan Gu*

Main category: cs.DB

TL;DR: 本文提出了一种新的CoreT算法，用于解决时间k核查询问题，该算法通过动态记录顶点或边进入k核的最早时间戳来实现高效的剪枝，实验结果表明CoreT比现有算法快四个数量级。


<details>
  <summary>Details</summary>
Motivation: 在时间图中查询有凝聚力的子图对于理解现实世界网络的动态结构至关重要。

Method: 提出了一种名为CoreT的新算法，该算法动态记录每个顶点或边进入k核的最早时间戳。

Result: CoreT只需要对查询间隔进行一次扫描，并且实现了改进的时间复杂度，该复杂度在查询间隔内的时间边数和间隔的持续时间内都是线性的。

Conclusion: CoreT算法在时间k核分析方面比现有最先进的OTCD算法快四个数量级，展示了其有效性和可扩展性。
...
### [135] [AmbiSQL: Interactive Ambiguity Detection and Resolution for Text-to-SQL](https://arxiv.org/abs/2508.15276)
*Zhongjun Ding,Yin Lin,Tianjing Zeng*

Main category: cs.DB

TL;DR: AmbiSQL, an interactive system, detects query ambiguities, clarifies user intent through multiple-choice questions, and improves SQL accuracy by 50%.
...
### [136] [Efficient Cloud-Edge-Device Query Execution Based on Collaborative Scan Operator](https://arxiv.org/abs/2508.15285)
...
TL;DR: 提出了一种新的云边端协同查询处理方法，通过协同扫描算子实现云边查询执行的无缝切换，从而提高查询性能。


<details>
  <summary>Details</summary>
Motivation: 在云边端(CED)协同查询(CQ)处理中，充分利用云和边缘资源的优势，但难以实现查询执行过程中云边灵活切换的协同算子。本文旨在提高边缘资源达到瓶颈时的查询性能。

Method: 提出了一种基于协同扫描算子的CED协同框架，实现了云边之间查询执行的无缝切换。

Result: 大量实验表明，在足够的网络下载带宽下，CED协同扫描算子可以有效缓解边缘端高I/O负载和CPU等待时间导致的扫描算子性能下降。它还在云和边缘之间实现了平衡的资源调度。

Conclusion: CED协同扫描算子可以有效缓解边缘端高I/O负载和CPU等待时间导致的扫描算子性能下降，并实现云边之间的平衡资源调度。
...
### [137] [Gorgeous: Revisiting the Data Layout for Disk-Resident High-Dimensional Vector Search](https://arxiv.org/abs/2508.15290)
*Peiqi Yin,Xiao Yan,Qihui Zhou,Hui Li,Xiaolu Li,Lin Zhang,Meiling Wang,Xin Yao,James Cheng*

Main category: cs.DB

TL;DR: Gorgeous prioritizes graph structure in disk-based vector search, improving performance by caching adjacency lists and enhancing data locality.


<details>
  <summary>Details</summary>
Motivation: Existing disk-based vector search systems have sub-optimal data layouts that don't effectively use memory and suffer from poor locality, hindering performance.

Method: The Gorgeous system prioritizes graph structure over vectors, featuring a memory cache for adjacency lists and a disk block format that stores neighbors' adjacency lists with a vector.

Result: Gorgeous boosts average query throughput by over 60% and reduces query latency by over 35% compared to state-of-the-art disk-based systems.

Conclusion: Gorgeous outperforms existing systems by prioritizing graph structure, boosting throughput by over 60% and reducing latency by over 35%.
...
### [138] [GoVector: An I/O-Efficient Caching Strategy for High-Dimensional Vector Nearest Neighbor Search](https://arxiv.org/abs/2508.15694)
*Yijie Zhou,Shengyuan Lin,Shufeng Gong,Song Yu,Shuhao Fan,Yanfeng Zhang,Ge Yu*

Main category: cs.DB

TL;DR: GoVector is an I/O-efficient caching strategy tailored for disk-based graph indices, which reduces I/O operations, increases query throughput and lowers query latency.
...
### [139] [Privacy Preserving Inference of Personalized Content for Out of Matrix Users](https://arxiv.org/abs/2508.14905)
*Michael Sun,Tai Vu,Andrew Wang*

Main category: cs.IR

TL;DR: DeepNaniNet is a deep neural recommendation framework that addresses the challenges of data sparsity, cold start, and privacy constraints in recommender systems. It achieves state-of-the-art results in cold start scenarios and outperforms existing methods in warm start scenarios.


<details>
  <summary>Details</summary>
Motivation: Recommender systems for niche and dynamic communities face persistent challenges from data sparsity, cold start users and items, and privacy constraints. Traditional collaborative filtering and content-based approaches underperform in these settings, either requiring invasive user data or failing when preference histories are absent.

Method: a deep neural recommendation framework that addresses these challenges through an inductive graph-based architecture combining user-item interactions, item-item relations, and rich textual review embeddings derived from BERT

Result: DeepNaniNet achieves state-of-the-art cold start results on the CiteULike benchmark, matches DropoutNet in user recall without performance degradation for out-of-matrix users, and outperforms Weighted Matrix Factorization (WMF) and DropoutNet on AnimeULike warm start by up to 7x and 1.5x in Recall@100, respectively.
...
### [140] [Collaborative Filtering using Variational Quantum Hopfield Associative Memory](https://arxiv.org/abs/2508.14906)
*Amir Kermanshahani,Ebrahim Ardeshir-Larijani,Rakesh Saini,Saif Al-Kuwari*

Main category: cs.IR

TL;DR: Proposes a hybrid quantum-classical recommendation system using Quantum Hopfield Associative Memory and deep neural networks, achieving promising results on the MovieLens 1M dataset even in noisy environments, and optimizing qubit overhead.
...
Result: achieving an ROC value of 0.9795, an accuracy of 0.8841, and an F-1 Score of 0.8786 in an ideal environment, and an ROC of 0.9177, an accuracy of 0.8013, and an F-1 Score equal to 0.7866 in a noisy environment
...
### [141] [Closing the Performance Gap in Generative Recommenders with Collaborative Tokenization and Efficient Modeling](https://arxiv.org/abs/2508.14910)
*Simon Lepage,Jeremie Mary,David Picard*

Main category: cs.IR

TL;DR: COSETTE and MARIUS improve generative recommendation, making them competitive with ID-based models.


<details>
  <summary>Details</summary>
Motivation: Generative recommender systems underperform compared to ID-based models due to lack of collaborative signal and inefficiencies in the encoder-decoder architecture.

Method: Introduces COSETTE, a contrastive tokenization method, and MARIUS, a lightweight generative model.

Result: The proposed approach narrows or eliminates the performance gap between generative and modern ID-based models.

Conclusion: This paper introduces COSETTE and MARIUS to improve generative recommender systems, narrowing the performance gap with ID-based models.
...
### [142] [Personalized Recommendations via Active Utility-based Pairwise Sampling](https://arxiv.org/abs/2508.14911)
*Bahar Boroomand,James R. Wright*

Main category: cs.IR

TL;DR: Propose a generalized utility-based framework that learns preferences from pairwise comparisons to provide a more accurate, data-efficient, and user-centric personalized ranking.
...
Method: a generalized utility-based framework that learns preferences from simple and intuitive pairwise comparisons. A novel utility-based active sampling strategy for preference elicitation. Ground preference model in the probabilistic Plackett-Luce framework for pairwise data. Implementation using matrix factorization and neural network.
...
### [143] [Multimodal Recommendation via Self-Corrective Preference Alignmen](https://arxiv.org/abs/2508.14912)
*Yalong Guan,Xiang Chen,Mingyang Wang,Xiangyu Wu,Lihao Liu,Chao Qi,Shuang Yang,Tingting Gao,Guorui Zhou,Changjian Chen*

Main category: cs.IR

TL;DR: 提出了一种新的个性化作者推荐框架MSPA，该框架使用MLLM生成结构化的偏好文本，并通过自校正偏好对齐推荐器与作者的多模态特征对齐，从而显著提高了推荐的准确率、召回率和可解释性。


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖于单模态特征或将多模态特征视为补充，难以将用户的动态偏好与作者的多模态属性对齐，从而限制了准确性和可解释性。

Method: MSPA（多模态自校正偏好对齐），一个个性化的作者推荐框架，包含两个组成部分：（1）一个多模态偏好生成器，它使用MLLM从用户的打赏历史中生成结构化的偏好文本和嵌入；（2）一个自校正偏好对齐推荐器，它将这些偏好与作者的多模态特征对齐，以提高准确性和可解释性。

Result: MSPA在准确率、召回率和文本质量方面均优于基线。
...
### [144] [M-$LLM^3$REC: A Motivation-Aware User-Item Interaction Framework for Enhancing Recommendation Accuracy with LLMs](https://arxiv.org/abs/2508.15262)
*Lining Chen,Qingwen Zeng,Huaming Chen*

Main category: cs.IR

TL;DR: This paper introduces M-$LLM^3$REC, a novel recommendation framework using large language models to extract motivational signals, enhancing performance in cold-start scenarios.


<details>
  <summary>Details</summary>
Motivation: Traditional methods have achieved impressive results in recommendation systems. However, the cold-start and sparse-data scenarios are still challenging to deal with. Existing solutions either generate pseudo-interaction sequence, which often introduces redundant or noisy signals, or rely heavily on semantic similarity, overlooking dynamic shifts in user motivation.
...
Result: M-$LLM^3$REC demonstrates robust, personalized, and generalizable recommendations, particularly boosting performance in cold-start situations in comparison with the state-of-the-art frameworks.
...
### [145] [Curriculum Approximate Unlearning for Session-based Recommendation](https://arxiv.org/abs/2508.15263)
*Liu Yang,Zhaochun Ren,Ziqi Zhao,Pengjie Ren,Zhumin Chen,Xinyi Li,Shuaiqiang Wang,Dawei Yin,Xin Xin*

Main category: cs.IR

TL;DR: CAU是一个用于会话推荐的课程近似解学习框架，它通过多目标优化和课程学习来提高解学习的有效性，同时保持推荐性能。


<details>
  <summary>Details</summary>
Motivation: 近似解学习的梯度上升方法存在双重挑战：直接应用GA可能导致推荐性能下降；现有研究未能考虑同时处理多个解学习请求时解学习样本的排序，导致次优的推荐性能和解学习效果。

Method: CAU，一个为基于会话的推荐量身定制的课程近似解学习框架。CAU使用GA项处理解学习任务。

Result: CAU框架在解学习效果和推荐性能之间取得了平衡，并通过课程学习策略优化了解学习过程。

Conclusion: CAU通过寻找帕累托最优解来解决多目标优化问题，从而实现有效的解学习，并在推荐性能上做出很小的牺牲。为了解决第二个挑战，CAU采用基于课程的序列来对批量的解学习样本进行解学习。
...
### [146] [MMQ: Multimodal Mixture-of-Quantization Tokenization for Semantic ID Generation and User Behavioral Adaptation](https://arxiv.org/abs/2508.15281)
*Yi Xu,Moyu Zhang,Chenxuan Li,Zhihao Liao,Haibo Xing,Hao Deng,Jinxin Hu,Yu Zhang,Xiaoyi Zeng,Jing Zhang*

Main category: cs.IR

TL;DR: 提出了MMQ，一个用于推荐系统的多模态框架，通过统一多模态协同、特异性和行为适应性，改进了生成式检索和判别式排序任务。


<details>
  <summary>Details</summary>
Motivation: 传统的推荐系统使用唯一的标识符（ItemIDs）来表示项目，但这种方法难以处理大型、动态的项目语料库和稀疏的长尾数据，限制了可扩展性和泛化性。语义ID通过将项目映射到共享的语义空间，提供了一个有希望的替代方案，从而实现知识转移并改进对新的或稀有项目的推荐。然而，现有的方法面临两个关键挑战：（1）平衡跨模态协同与模态特定唯一性，以及（2）弥合语义-行为差距，其中语义表示可能与实际用户偏好不一致。

Method: 提出了多模态混合量化（MMQ）框架，该框架训练了一个新颖的多模态分词器。首先，一个共享-特定分词器利用具有模态特定和模态共享专家的多专家架构，使用正交正则化来捕获全面的多模态信息。其次，行为感知微调动态地调整语义ID以适应下游推荐目标，同时通过多模态重建损失来保留模态信息。

Result: 大量的离线实验和在线A/B测试表明MMQ是有效的。
...
### [148] [MLLMRec: Exploring the Potential of Multimodal Large Language Models in Recommender Systems](https://arxiv.org/abs/2508.15304)
*Yuzhuo Dang,Xin Zhang,Zhiqiang Pan,Yuxiao Duan,Wanyu Chen,Fei Cai,Honghui Chen*

Main category: cs.IR

TL;DR: MLLMRec uses a multimodal large language model (MLLM) to improve user preference modeling and refine item-item graphs for better recommendation performance.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal recommendation methods suffer from behavior-unperceived or noise-contaminated user representations and noisy item-item graphs.

Method: MLLM-driven multimodal recommendation framework with two item-item graph refinement strategies: threshold-controlled denoising and topology-aware enhancement.

Result: MLLMRec improves performance by 38.53% over baselines on three datasets.
...
### [149] [On the Effectiveness of Graph Reordering for Accelerating Approximate Nearest Neighbor Search on GPU](https://arxiv.org/abs/2508.15436)
*Yutaro Oguri,Mai Nishimura,Yusuke Matsui*

Main category: cs.IR

TL;DR: systematic investigation of graph reordering effects for graph-based ANNS on a GPU
...
Result: introducing analysis metrics that quantify the relationship between structural properties and memory layout effectiveness, comprehensive analysis across diverse datasets and state-of-the-art graph indices
...
### [150] [REG4Rec: Reasoning-Enhanced Generative Model for Large-Scale Recommendation Systems](https://arxiv.org/abs/2508.15308)
*Haibo Xing,Hao Deng,Yucheng Mao,Jinxin Hu,Yi Xu,Hao Zhang,Jiahao Wang,Shizhun Wang,Yu Zhang,Xiaoyi Zeng,Jing Zhang*

Main category: cs.IR

TL;DR: REG4Rec是一种推理增强的生成模型，通过构建多个动态语义推理路径和自反思过程，解决了传统序列推荐方法中信息交互不足和推理过程可靠性不足的问题。


<details>
  <summary>Details</summary>
Motivation: 传统的序列推荐方法通常存在信息交互不足的问题，而最近的生成推荐模型通过直接生成项目预测在一定程度上解决了这个问题。为了更好地捕捉用户意图，最近的研究将推理过程引入生成推荐中，显著提高了推荐性能。然而，这些方法受到项目语义表示的单一性的限制，面临着推理路径多样性有限和推理过程可靠性不足等挑战。

Method: REG4Rec构建了多个动态语义推理路径，并采用自反思过程，确保高置信度的推荐。具体来说，REG4Rec利用基于MoE的并行量化码本（MPQ）为每个项目生成多个无序语义tokens，从而构建更大规模的多样化推理空间。此外，为了提高推理的可靠性，我们提出了一个训练推理增强阶段，包括用于推理的偏好对齐（PARS）和多步奖励增强（MSRA）策略。在推理过程中，提出了面向一致性的自反思修剪（CORP），以丢弃不一致的推理路径，防止错误推理的传播。

Result: REG4Rec在真实数据集和在线评估中表现出色。

Conclusion: REG4Rec在真实数据集和在线评估中表现出色，具有显著的实际价值。
...
### [151] [Modeling Long-term User Behaviors with Diffusion-driven Multi-interest Network for CTR Prediction](https://arxiv.org/abs/2508.15311)
*Weijiang Lai,Beihong Jin,Yapeng Zhang,Yiyuan Zheng,Rui Zhao,Jian Dong,Jun Lei,Xingxing Wang*

Main category: cs.IR

TL;DR: DiffuMIN是一种新的CTR预测模型，它使用扩散模型来更好地捕捉用户兴趣，并在实验中表现良好。


<details>
  <summary>Details</summary>
Motivation: 以往的CTR预测模型难以捕捉用户多样化的兴趣，且容易受到噪声干扰。

Method: 提出了一种名为DiffuMIN的扩散驱动多兴趣网络，通过目标导向的多兴趣提取方法和扩散模块来建模长期用户行为，并利用对比学习确保生成的兴趣与用户偏好一致。

Result: 在公共数据集和工业数据集上的离线实验以及在线A/B测试表明，DiffuMIN优于现有模型，CTR提高1.52%，CPM提高1.10%。

Conclusion: DiffuMIN在离线和在线实验中均表现出色，能有效提高CTR和CPM。
...
### [152] [Exploring Scaling Laws of CTR Model for Online Performance Improvement](https://arxiv.org/abs/2508.15326)
*Weijiang Lai,Beihong Jin,Jiongyan Zhang,Yiyuan Zheng,Jian Dong,Jia Cheng,Jun Lei,Xingxing Wang*

Main category: cs.IR

TL;DR: This paper proposes SUAN, a CTR model inspired by LLM scaling laws, and distills it into a lightweight LightSUAN for online deployment, achieving significant CTR and CPM improvements.
...
### [153] [TrackRec: Iterative Alternating Feedback with Chain-of-Thought via Preference Alignment for Recommendation](https://arxiv.org/abs/2508.15388)
*Yu Xia,Rui Zhong,Zeyu Song,Wei Yang,Junchen Wan,Qingpeng Cai,Chi Lu,Peng Jiang*

Main category: cs.IR

TL;DR: TrackRec is a framework designed to enhance reasoning capabilities of LLMs for RS by accurately inferring recommendation CoT (RecCoT) for user preference using the knowledge from LLMs. It consists of a RecCoT generator (G) and a RecCoT validator (V) with alternating feedback learning.
...
Result: TrackRec surpasses state-of-the-art methods. Moreover, TrackRec has been deployed on a lagre advertising platform with hundreds of millions of users, achieving substantial gains.
...
### [154] [Test-time Corpus Feedback: From Retrieval to RAG](https://arxiv.org/abs/2508.15437)
*Mandeep Rathee,Venktesh V,Sean MacAvaney,Avishek Anand*

Main category: cs.IR

TL;DR: This survey reviews advanced retrieval and ranking mechanisms with feedback for RAG, aiming to make retrieval a dynamic component.


<details>
  <summary>Details</summary>
Motivation: Most RAG pipelines treat retrieval and reasoning as isolated components, limiting performance on complex tasks. Recent work has begun to close this gap by introducing adaptive retrieval and ranking methods that incorporate feedback.

Method: The survey categorizes feedback signals based on their source and role in improving the query, retrieved context, or document pool.

Result: The survey presents a structured overview of advanced retrieval and ranking mechanisms that integrate feedback in Retrieval-Augmented Generation (RAG).

Conclusion: This survey aims to bridge the gap between IR and NLP perspectives by highlighting retrieval as a dynamic, learnable component of end-to-end RAG systems.
...
### [155] [On Evaluating the Adversarial Robustness of Foundation Models for Multimodal Entity Linking](https://arxiv.org/abs/2508.15481)
*Fang Wang,Yongjie Wang,Zonghao Yang,Minghao Hu,Xiaoying Bai*

Main category: cs.IR

TL;DR: 首次全面评估了主流MEL模型在不同对抗攻击场景下的鲁棒性，发现模型对抗扰动能力不足，提出了LLM-RetLink模型，显著提高了模型在对抗环境下的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的研究没有系统地研究视觉对抗攻击对MEL模型的影响。

Method: 提出了LLM和检索增强实体链接(LLM-RetLink)。

Result: 实验结果表明，当前的MEL模型普遍缺乏足够的鲁棒性来抵抗视觉扰动。

Conclusion: LLM-RetLink通过利用大型视觉模型提取实体描述，并通过网络检索动态生成候选描述语句，显著提高了模型在对抗环境下的抗干扰能力，MEL准确率提高了0.4%-35.7%。
...
### [156] [LongRetriever: Towards Ultra-Long Sequence based Candidate Retrieval for Recommendation](https://arxiv.org/abs/2508.15486)
*Ren Qin,Chai Zheng,Xiao Xijun,Zheng Yuchao,Wu Di*

Main category: cs.IR

TL;DR: 本文提出了 LongRetriever，一个将超长序列加入推荐器检索阶段的实用框架。


<details>
  <summary>Details</summary>
Motivation: 精确建模用户超长序列对于工业推荐系统至关重要。目前的方法主要集中在在排序阶段利用超长序列，而候选检索阶段的研究仍未得到充分探索。

Method: 提出上下文训练和多上下文检索方法，实现用户序列和候选项目之间的特定交互，并确保在搜索范式下的训练-服务一致性。

Result: 大规模电商平台上的大量在线 A/B 测试表明，LongRetriever 具有显著的改进，证实了该框架的有效性。

Conclusion: LongRetriever已经在大型电商平台全面部署，影响数十亿用户，效果显著。
...
### [157] [Reading Between the Lines: A Study of Thematic Bias in Book Recommender Systems](https://arxiv.org/abs/2508.15643)
*Nityaa Kalra,Savvina Daniil*

Main category: cs.IR

TL;DR: This paper studies thematic bias in book recommendations, finding that it originates from content imbalances and is amplified by user engagement, negatively impacting users with niche interests.


<details>
  <summary>Details</summary>
Motivation: Recommender systems help users discover new content, but can also reinforce existing biases, leading to unfair exposure and reduced diversity. This paper introduces and investigates thematic bias in book recommendations, defined as a disproportionate favouring or neglect of certain book themes.
...
TL;DR: 通过安全强化学习，自动驾驶汽车可以在现实世界的交通场景中做出更道德的决策，同时保持良好的驾驶性能。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶汽车在减少交通事故死亡人数和提高交通效率方面具有巨大的前景，但它们的广泛应用取决于将强大的伦理推理嵌入到日常和紧急操作中。

Method: 分层安全强化学习 (Safe RL) 框架，该框架将道德考量与标准驾驶目标明确结合。

Result: 该方法在降低伦理风险和保持驾驶性能方面优于基线方法。这是首次通过实际场景中的安全强化学习对自动驾驶汽车的伦理决策进行研究。

Conclusion: 结合形式控制理论和数据驱动学习，以推进复杂、人车混合的交通环境中具有道德责任的自主性。
...
TL;DR: 提出了一个性化的肺癌风险预测Agent，它通过结合队列特异性知识与现代检索和推理技术，动态地为每个患者选择最合适的模型。


<details>
  <summary>Details</summary>
Motivation: 由于患者群体和临床环境的巨大差异，准确的肺癌风险预测仍然具有挑战性——没有一个单一的模型对所有队列都表现最佳。

Method: 结合了基于FAISS的相似性搜索的队列检索和基于LLM的推理。

Result: 该Agent通过结合特定队列的知识与现代检索和推理技术，动态地为每个患者选择最合适的模型。

Conclusion: 该Agent支持跨不同临床人群的灵活的、由队列驱动的模型选择，为在真实世界肺癌筛查中实现个体化风险评估提供了一条可行的路径。
...
TL;DR: 提出了一种统一的预测框架，该框架集成了结构感知和时间建模，在预测帕金森病进展方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 本研究旨在解决帕金森病进展预测中症状演变复杂性和时间依赖性建模不足的挑战。

Method: 利用图神经网络对多模态临床症状之间的结构关系进行建模，并引入基于图的表示来捕获症状之间的语义依赖关系。此外，还结合了Transformer架构来模拟疾病进展过程中的动态时间特征。为了融合结构和时间信息，设计了一种结构感知门控机制，以动态调整结构编码和时间特征之间的融合权重，从而增强模型识别关键进展阶段的能力。

Result: 结果表明，该方法在AUC、RMSE和IPW-F1指标方面优于现有方法。它有效地区分了进展阶段，并提高了模型捕获个性化症状轨迹的能力。
...
TL;DR: This paper introduces HHNAS-AM, a hierarchical hybrid NAS model with adaptive mutation policies, to efficiently explore diverse architectural configurations for text classification, achieving significant accuracy improvements.


<details>
  <summary>Details</summary>
Motivation: Existing NAS models for text classification lack a hybrid hierarchical structure and have a large, redundant search space, making it difficult for RL models to navigate effectively.
...
TL;DR: LPO是一种新的偏好对齐框架，通过梯度解耦、稳定性和可控的拒绝抑制来解决DPO的过度拟合和崩溃问题，并在各种任务中表现出改进的性能。


<details>
  <summary>Details</summary>
Motivation: DPO（直接偏好优化）由于其简单性和训练稳定性而成为一种广泛使用的离线偏好优化算法。然而，DPO容易过度拟合和崩溃。为了应对这些挑战

Method: 提出线性偏好优化（LPO），这是一种新颖的对齐框架，具有三个关键创新。首先，我们通过用绝对差分损失代替log-sigmoid函数来引入梯度解耦，从而隔离优化动态。其次，我们通过偏移约束和正则化项来提高稳定性，以保持所选响应的质量。第三，我们使用梯度分离以及直接估计和可调系数来实现可控的拒绝抑制，该系数线性地调节拒绝概率的下降。

Result: LPO在各种任务中持续提高性能，包括通用文本任务、数学任务和文本到语音（TTS）任务。

Conclusion: LPO在各种任务中持续提高性能，包括通用文本任务、数学任务和文本到语音（TTS）任务。这些结果证明LPO是偏好对齐的稳健且可调的范例。
...
TL;DR: LFM4Ads：一个全表示多粒度迁移框架，用于广告推荐，在腾讯的广告平台成功部署，显著提升了GMV。


<details>
  <summary>Details</summary>
Motivation: 在线广告依赖于精确的推荐模型，最近的研究进展使用预训练的大规模基础模型（LFMs）来捕获用户在多个场景和任务中的一般兴趣。然而，现有的方法存在关键的局限性：它们只提取和转移用户表示（URs），忽略了有价值的项目表示（IRs）和用户-项目交叉表示（CRs）；它们只是简单地使用UR作为下游应用中的特征，这未能弥合上游-下游的差距，并忽略了更多的转移粒度。

Method: 提出了LFM4Ads，一个用于广告推荐的全表示多粒度迁移框架。它全面迁移用户表示、项目表示和用户-项目交叉表示。为了有效利用交叉表示，它识别最佳提取层并将它们聚合成可转移的粗粒度形式。此外，我们通过多粒度机制增强可迁移性：用于特征级迁移的非线性适配器，用于模块级迁移的同构交互模块，以及用于模型级迁移的独立检索。

Result: 自2024年第四季度生产部署以来，LFM4Ads已在各种广告场景中实现了超过10次的成功发布，包括微信朋友圈和频道等主要场景。这些发布为整个平台带来了2.45%的GMV提升，转化为估计每年数亿美元的收入增长。

Conclusion: LFM4Ads在腾讯的工业级广告平台成功部署，并在各种广告场景中实现了超过10次的成功发布，为整个平台带来了2.45%的GMV提升，预计每年收入增加数亿美元。
...
TL;DR: DiffQAS-QLSTM是一种端到端可微分框架，可优化VQC参数和架构选择，在各种测试环境中始终优于手工设计的基线。


<details>
  <summary>Details</summary>
Motivation: 量子计算和机器学习的最新进展催生了量子机器学习（QML），人们对从序列数据中学习越来越感兴趣。像QLSTM这样的量子循环模型在时间序列预测、自然语言处理和强化学习方面很有前景。然而，设计有效的变分量子电路（VQC）仍然具有挑战性，而且通常是特定于任务的。

Method: DiffQAS-QLSTM，一个端到端可微分的框架，可以在训练期间优化VQC参数和架构选择。

Result: DiffQAS-QLSTM在各种测试环境中始终优于手工设计的基线，实现了更低的损失。

Conclusion: DiffQAS-QLSTM在各种测试环境中始终优于手工设计的基线，实现了更低的损失。该方法为可扩展和自适应的量子序列学习打开了大门。
...
TL;DR: CuMoLoS-MAE 是一种用于恢复大气廓线并量化不确定性的新型深度学习模型。


<details>
  <summary>Details</summary>
Motivation: 来自多普勒激光雷达、雷达和辐射计等遥感仪器的准确大气廓线经常被低信噪比门、距离折叠和虚假不连续性破坏。传统的间隙填充会模糊精细结构，而深度模型缺乏置信度估计。

Method: 课程引导的蒙特卡罗随机集成掩码自动编码器 (CuMoLoS-MAE)

Result: CuMoLoS-MAE 能够 (i) 恢复精细尺度特征，如上升和下降气流核心、剪切线和小涡旋，(ii) 学习大气场的data-driven先验，以及 (iii) 量化像素级不确定性。

Conclusion: 该新型深度学习工作流程实现了高保真重建，增强了对流诊断，支持实时数据同化，并改进了长期气候再分析。
...
TL;DR: Aura-CAPTCHA is a multi-modal CAPTCHA system using GANs, RL, and LLMs that is more secure and accessible than traditional CAPTCHAs.


<details>
  <summary>Details</summary>
Motivation: To address vulnerabilities in traditional CAPTCHA methods that are increasingly bypassed by AI technologies.

Method: The design integrated Generative Adversarial Networks (GANs), Reinforcement Learning (RL), and Large Language Models (LLMs).

Result: Evaluations demonstrated a 92% human success rate and a 10% bot bypass rate.

Conclusion: Aura-CAPTCHA provides a robust and scalable approach for securing online applications while remaining accessible to users, outperforming existing CAPTCHA systems.
...
TL;DR: 本文针对一类特定的{NOs}，即生成{均衡算子}(GEOs)，缩小了理论和实践之间的差距。


<details>
  <summary>Details</summary>
Motivation: 理论和实践之间存在差距：通用逼近定理的最坏情况参数界限表明，NO可能需要数量过多的参数才能解决大多数算子学习问题，这与大量实验证据直接矛盾。

Method: 使用有限维深度均衡层

Result: 当输入损失位于合适的无限维紧集中时，我们证明了这一点。

Conclusion: GEO可以一致地逼近相应解，且rank, depth, 和width仅以逼近误差倒数的对数形式增长。
...
TL;DR: This survey reviews quantization techniques, software and hardware for deploying QNNs on microcontrollers, and discusses challenges and future directions.
...
Method: This survey systematically reviews quantization techniques, software frameworks, and hardware platforms for QNN execution.

Result: The survey evaluates trade-offs among model performance and hardware capabilities.

Conclusion: This survey analyzes challenges and future directions of QNN deployment on microcontrollers.
...
TL;DR: 提出了一种结合静态编译器分析和蒙特卡罗树搜索的系统，以解决大型机器学习模型在分布式加速器系统上的分区问题，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的自动分区器经常遇到内存不足错误或在探索可能的分区的大空间时速度过慢。为了缓解这个问题，他们人为地限制了搜索空间，但是这种方法经常产生违反设备内存约束或导致次优性能的不可行解决方案。

Method: 该系统结合了一种新颖的静态编译器分析与蒙特卡罗树搜索。
...
TL;DR: This paper introduces SEAL, an interpretable GNN that attributes model predictions to meaningful molecular subgraphs by reducing inter-fragment message passing. It outperforms existing methods and provides more trustworthy explanations.


<details>
  <summary>Details</summary>
Motivation: Graph neural networks have demonstrated remarkable success in predicting molecular properties. However, their black-box nature reduces interpretability, which limits trust in their predictions for important applications such as drug discovery and materials design. Furthermore, existing explanation techniques often fail to reliably quantify the contribution of individual atoms or substructures due to the entangled message-passing dynamics.

Method: introduce SEAL (Substructure Explanation via Attribution Learning), a new interpretable graph neural network that attributes model predictions to meaningful molecular subgraphs. SEAL decomposes input graphs into chemically relevant fragments and estimates their causal influence on the output. The strong alignment between fragment contributions and model predictions is achieved by explicitly reducing inter-fragment message passing in our proposed model architecture.

Result: SEAL outperforms other explainability methods in both quantitative attribution metrics and human-aligned interpretability.

Conclusion: SEAL provides more intuitive and trustworthy explanations to domain experts and offers a promising direction for more transparent and actionable molecular modeling.
...
TL;DR: Twin-Boot integrates uncertainty estimation into optimization by training two identical models on independent bootstrap samples, improving calibration and generalization.


<details>
  <summary>Details</summary>
Motivation: Standard gradient descent methods yield point estimates with no measure of confidence, which is a limitation in overparameterized and low-data regimes where models can easily overfit. Bootstrapping is a classical statistical framework for uncertainty estimation, but naively applying it to deep learning is impractical.
...
Result: 联邦非线性系统识别的有效性的理论保证，与集中式方法相比，证明了收敛速度随着客户端数量的增加而提高。

Conclusion: 联邦学习始终提高任何单个客户端的收敛性，因为参与客户端的数量增加。
...
TL;DR: This paper addresses the challenges of caching feature maps in layer-freezing, proposing solutions to improve training accuracy and mitigate storage overhead, resulting in reduced training costs.


<details>
  <summary>Details</summary>
Motivation: Traditional layer-freezing methods limit the reduction of computation costs because frozen layers are still required for forward propagation. Caching feature maps presents challenges such as applying augmentations and storage overhead.

Method: The authors propose similarity-aware channel augmentation and a progressive compression strategy.
...
Conclusion: This paper provides a systematic solution to the challenges of caching feature maps in layer-freezing techniques, achieving significant reductions in training cost while maintaining model accuracy.
...
TL;DR: This paper studies robust estimation under heterogeneous corruption rates, providing minimax rates for mean estimation and linear regression. Samples beyond a corruption threshold may be discarded by the optimal estimators.
...
Result: Tight minimax rates for all heterogeneous corruption patterns for mean estimation. Minimax rate for squared error up to a factor of $\sqrt{d}$ for multivariate gaussian mean estimation and linear regression.
...
### [175] [See Beyond a Single View: Multi-Attribution Learning Leads to Better Conversion Rate Prediction](https://arxiv.org/abs/2508.15217)
*Sishuo Chen,Zhangming Chan,Xiang-Rong Sheng,Lei Zhang,Sheng Chen,Chenghuan Hou,Han Zhu,Jian Xu,Bo Zheng*

Main category: cs.LG

TL;DR: 提出了一种新的多重归因学习（MAL）框架，用于CVR预测，该框架集成了来自多个归因视角的信号，以更好地捕获驱动用户转化的潜在模式。
...
Method: MAL是一个联合学习框架，由归因知识聚合器（AKA）和主要目标预测器（PTP）组成。此外，我们提出了一种新颖的训练策略CAT，它利用所有归因标签组合的笛卡尔积来生成丰富的监督信号。

Result: MAL优于单一归因学习基线。

Conclusion: MAL在离线指标上实现了+0.51%的GAUC改进，在线实验表明MAL实现了+2.6%的ROI增长。
...
TL;DR: The paper introduces NGN-M, a more stable momentum-based optimizer that is less sensitive to step-size tuning.


<details>
  <summary>Details</summary>
Motivation: Modern optimizers are sensitive to hyperparameter choices, making tuning difficult. Recent efforts enhance stability across hyperparameter choices.

Method: The paper proposes a novel adaptation of the NGN step-size method, creating a momentum-based version (NGN-M).

Result: NGN-M attains the standard convergence rate of $\mathcal{O}(1/\sqrt{K})$ under less restrictive assumptions and demonstrates enhanced robustness to the choice of the step-size hyperparameter.

Conclusion: The paper introduces NGN-M, a momentum-based optimizer, and demonstrates its robustness to step-size hyperparameter choice while achieving comparable or superior performance to state-of-the-art optimizers.
...
TL;DR: DNNs容易被fooling examples欺骗，过度参数化会导致模型退化，但wormhole solution可以解决这个问题。
...
Result: DNNs在过度参数化状态下输出特征空间会collapse，添加更多层会导致退化。wormhole solution可以绕过这种退化。

Conclusion: DNNs在过度参数化状态下会退化，但可以通过wormhole solution避免。
...
TL;DR: This paper evaluates sparse autoencoders (SAEs) against base models concerning monosemanticity, showing that SAEs reduce polysemanticity and enable more precise concept-level control. It introduces a new intervention method, APP, for targeted concept removal.
...
Method: The paper introduces a fine-grained concept separability score based on the Jensen-Shannon distance to evaluate monosemanticity. It also proposes Attenuation via Posterior Probabilities (APP), a new intervention method for targeted suppression.

Result: SAEs reduce polysemanticity and achieve higher concept separability compared to base models. However, greater sparsity does not always yield better separability and often impairs downstream performance.

Conclusion: SAEs enable more precise concept-level control compared to base models, especially with partial suppression. The paper introduces APP, a new intervention method that outperforms existing approaches in targeted concept removal.
...
TL;DR: Hydra 是一种混合长上下文语言模型架构，它结合了条件计算、长上下文记忆机制和稀疏混合专家。


<details>
  <summary>Details</summary>
Motivation: Hydra 是一种混合长上下文语言模型的架构提案，它在约 1.6B 参数设计范围内结合了条件计算、长上下文记忆机制和稀疏混合专家。

Method: Hydra 整合了 Mamba 风格的结构化状态空间模型 (SSM) 主干与间歇性稀疏全局注意力、块级 MoE 前馈路由和双重（工作区加事实 PKM）记忆。

Result: 该规范附有说明性的玩具规模原型测量（在合成数据上数千万个参数），其唯一目的是演示实现可行性和定性缩放行为（例如，长上下文吞吐量交叉和可控专家路由），而不是声称具有竞争力的完整规模性能。

Conclusion: Hydra 通过结合 SSM 效率、选择性稀疏注意力、MoE 容量和可学习记忆，勾勒出通往模块化、输入自适应长上下文语言模型的路径；在目标规模上验证最终任务收益仍是未来的工作。
...
Result: CETs can be easily circumvented and present several side effects of concept erasure. experiments reveal that CETs can be circumvented by using superclass-subclass hierarchy and semantically similar prompts, such as compositional variants of the target. CETs suffer from attribute leakage and counterintuitive phenomena of attention concentration or dispersal.
...
TL;DR: 本文提出了一种免源unlearning方法，该方法无需访问原始训练数据集即可从训练模型中删除特定数据。


<details>
  <summary>Details</summary>
Motivation: 随着机器学习变得越来越普遍，并且数据隐私法规不断发展，从训练模型中删除私人或受版权保护信息的能力正成为一项日益关键的要求。现有的 unlearning 方法通常依赖于在遗忘过程中可以访问整个训练数据集的假设。但是，在无法访问原始训练数据（即，无源设置）的实际情况下，此假设可能不成立。

Method: 提出了一种估计未知剩余训练数据 Hessian 的方法，这对于有效的免源学习来说至关重要。

Result: 大量数据集的广泛实验验证了我们方法的有效性。

Conclusion: 该方法能够在保持剩余数据性能的同时，实现高效的零样本学习，并为学习性能提供可靠的理论保证。
...
TL;DR: Introduces a categorial generalization of RL called universal reinforcement learning (URL) using advanced mathematical concepts.
...
Method: introduce a categorial generalization of RL, termed universal reinforcement learning (URL), building on powerful mathematical abstractions from the study of coinduction on non-well-founded sets and universal coalgebras, topos theory, and categorial models of asynchronous parallel distributed computation

Result: The space of algorithms for MDPs or PSRs can be modeled as a functor category, where the co-domain category forms a topos, which admits all (co)limits, possesses a subobject classifier, and has exponential objects.
...
TL;DR: An R+R experiment on 11 SoTA DPML techniques shows varying degrees of reproducibility. The paper highlights challenges and best practices for reliable DPML research.


<details>
  <summary>Details</summary>
Motivation: Recent DPML research lacks consensus on the effectiveness and validity of proposed techniques due to heterogeneity in codebases, datasets, methodologies, and model architectures, making direct comparisons difficult.

Method: Reproducibility and replicability (R+R) experiment on 11 different SoTA DPML techniques.

Result: Results vary: some methods are robust, others fail outside initial conditions. The paper identifies challenges in DPML reproducibility and proposes solutions.

Conclusion: The reproducibility and replicability (R+R) experiment on 11 different SoTA DPML techniques reveals that some methods are robust while others fail outside their initial experimental conditions. The paper discusses challenges unique to the reproducibility of DPML and suggests best practices for reliable results.
...
<details>
  <summary>Details</summary>
Motivation: Accurate extraction of clinical information from electronic medical records is particularly critical to clinical research but require much trained expertise and manual labor.

Method: a fine-tuned RoBERTa model

Result: This model significantly outperformed the baseline model and a Large Language Model, Mistral 7B, achieving F1_Bertscore 0.98 and overall exact match of 80.61%.
...
TL;DR: SafeLLM是一种基于unlearning的防御框架，可以从LLM中unlearn有害知识，同时保持语言流畅性和通用能力。


<details>
  <summary>Details</summary>
Motivation: Jailbreak攻击通过构造对抗性prompt绕过对齐机制，导致模型产生有害、受限或有偏见的内容，对大型语言模型(LLM)的安全构成严重威胁。

Method: SafeLLM采用三阶段流程：(1) 使用混合方法进行动态不安全输出检测；(2) 通过前馈网络(FFN)激活进行token级别有害内容追踪；(3) 约束优化以抑制不安全行为。

Result: 在多个jailbreak基准测试中，SafeLLM显著降低了攻击成功率，同时保持了高通用性能。与监督微调和直接偏好优化等标准防御方法相比，SafeLLM提供了更强的安全保证，对有害行为更精确的控制，以及对未见攻击的更强鲁棒性。

Conclusion: SafeLLM通过识别并消除负责有害生成途径的FFN子结构，实现了有针对性和不可逆的遗忘，同时保持了一般的性能。这表明unlearning是提高LLM安全性的一个有希望的方向。
...
TL;DR: FairPrep是一个用于评估公平性感知预处理技术的基准测试框架，它填补了公平性基准测试领域的关键空白。


<details>
  <summary>Details</summary>
Motivation: 随着机器学习系统日益融入高风险决策过程，确保算法结果的公平性已成为一个关键问题。减轻偏见的方法通常分为三类：预处理、处理中和后处理。虽然后两者受到了极大的关注，但预处理方法在数据层面进行操作，并具有模型不可知性和改进的隐私合规性等优势，但受到的关注相对较少，并且缺乏标准化评估工具。

Method: 我们介绍了FairPrep，这是一个可扩展的模块化基准测试框架，旨在评估表格数据集上具有公平意识的预处理技术。FairPrep构建于AIF360平台之上，允许无缝集成数据集、公平性干预和预测模型。它具有批量处理界面，可以实现高效的实验和公平性以及效用指标的自动报告。

Result: 通过提供标准化管道并支持可重复的评估，
...
TL;DR: 提出了一种频率自适应张量神经网络算法，以提高张量神经网络在高维多尺度问题中的性能。


<details>
  <summary>Details</summary>
Motivation: 张量神经网络 (TNN) 在解决高维问题方面表现出了优越性。然而，与传统的神经网络类似，TNN 也受到频率原则的影响，这限制了它们准确捕获解决方案的高频特征的能力。

Method: 通过傅里叶分析分析了张量神经网络的训练动态，并通过结合随机傅里叶特征来增强其对高维多尺度问题的表达能力。利用张量神经网络固有的张量结构，提出了一种新的方法，通过对一维分量函数执行离散傅里叶变换来提取高维函数的频率特征。

Result: 所提出的频率自适应张量神经网络算法显著提高了张量神经网络解决复杂多尺度问题的能力。

Conclusion: 提出了一种频率自适应张量神经网络算法，并通过大量数值实验验证了该算法的有效性和鲁棒性。
...
TL;DR: 提出了一种名为SleepDIFFormer的多变量差分Transformer模型，用于提高睡眠分期分类的准确性和泛化能力，并在多个数据集上取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 人工检查脑电图特征进行睡眠分期耗时且容易出错。现有的机器学习和深度学习方法在脑电图和眼动图信号的非平稳性和变异性方面面临挑战，导致在新数据集上的泛化能力较差。

Method: 开发了一种多变量差分Transformer（SleepDIFFormer），用于联合脑电图（EEG）和眼动图（EOG）表示学习。使用多变量差分Transformer架构（MDTA）处理脑电图和眼动图信号，并通过跨域对齐进行训练。通过特征分布对齐，减轻了空间和时间注意力噪声，同时学习了域不变的联合脑电图-眼动图表示。

Result: 在五个不同的睡眠分期数据集上评估了该方法，并与现有方法进行了比较，取得了最先进的性能。对SleepDIFFormer进行了彻底的消融分析，并解释了差分注意力权重。

Conclusion: 提出的SleepDIFFormer方法在五个睡眠分期数据集中取得了最先进的性能，并进行了消融分析和差分注意力权重的解释，对自动睡眠分期分类及其在睡眠质量评估中的应用具有重要意义。
...
TL;DR: 提出了一种新的框架，用于合成具有局部最优性保证的可解释机器学习模型，该框架在准确性和可解释性之间实现了更好的权衡。


<details>
  <summary>Details</summary>
Motivation: 在黑盒机器学习模型中，准确性和可解释性通常是相互冲突的目标。现有的多目标解释合成技术通常缺乏关于Pareto最优性的形式保证，而提供此类保证的方法在探索Pareto最优空间时面临严重的可扩展性限制。

Method: 该方法基于局部最优性保证，使用多目标学习或搜索技术生成Pareto最优候选集，并通过SAT求解器验证局部最优性。

Result: 该方法能够生成具有局部最优性保证的Pareto最优解释集，并且在可扩展性方面有所提升。

Conclusion: 该方法在基准测试中表现出良好的效果，能够生成与提供全局保证的方法所合成的解释非常接近的解释。
...
TL;DR: Proposes poly-window contrastive learning for ECG analysis, improving performance and efficiency compared to existing methods by leveraging temporal structure in ECG data.


<details>
  <summary>Details</summary>
Motivation: Deep learning models for ECG analysis are constrained by limited annotated data. Existing self-supervised contrastive learning methods fail to leverage the rich temporal structure of ECG recordings and generate only pairwise augmented views.

Method: poly-window contrastive learning framework that extracts multiple temporal windows from each ECG instance to construct positive pairs and maximize their agreement via statistics, encouraging the model to learn temporally invariant and physiologically meaningful features

Result: Consistently outperforms conventional two-view methods in multi-label superclass classification, achieving higher AUROC (0.891 vs. 0.888) and F1 scores (0.680 vs. 0.679) while requiring fewer pre-training epochs and less pre-training time.

Conclusion: Poly-window contrastive learning is a highly efficient and scalable paradigm for automated ECG analysis and a promising framework for self-supervised representation learning in biomedical time-series data.
...
TL;DR: DeepConf is a method that enhances reasoning efficiency and performance by filtering out low-quality reasoning traces using model-internal confidence signals.
...
Result: DeepConf enhances both reasoning efficiency and performance at test time. It requires no additional model training or hyperparameter tuning and can be seamlessly integrated into existing serving frameworks.
...
TL;DR: CSG在知识图谱链接预测中效果不好，本文提出了一些其他的复杂性指标。


<details>
  <summary>Details</summary>
Motivation: 理解数据集复杂性对于评估和比较知识图谱（KG）上的链接预测模型至关重要。

Method: 引入并评估了一组结构和语义KG复杂性指标。

Result: 关系熵、节点级最大关系多样性和关系类型基数与MRR和Hit@1呈强负相关；平均度、度熵、PageRank和特征向量中心性与Hit@10呈正相关。

Conclusion: CSG在知识图谱链接预测中表现不佳，需要更稳定、可解释和与任务对齐的复杂性度量。
...
Method: The paper introduces PL-Reg, a novel partial-logic regularization term that allows models to reserve space for undefined logic formulas.
...
TL;DR: ExBigBang, a hybrid text-tabular approach, addresses limitations in persona classification by modeling rich contextual features and improving explainability.


<details>
  <summary>Details</summary>
Motivation: Earlier studies have advanced persona classification by modelling user behaviour, capturing contextual information, especially by integrating textual and tabular data, remains a key challenge. These models also often lack explainability, leaving their predictions difficult to interpret or justify. To address these limitations, we present ExBigBang

Method: a hybrid text-tabular approach that uses transformer-based architectures to model rich contextual features for persona classification. ExBigBang incorporates metadata, domain knowledge, and user profiling to embed deeper context into predictions. Through a cyclical process of user profiling and classification, our approach dynamically updates to reflect evolving user behaviours.

Result: Experiments on a benchmark persona classification dataset demonstrate the robustness of our model. An ablation study confirms the benefits of combining text and tabular data, while Explainable AI techniques shed light on the rationale behind the model's predictions.

Conclusion: ExBigBang, a hybrid text-tabular approach, demonstrates robustness on a benchmark persona classification dataset. Ablation studies confirm the benefits of combining text and tabular data, and Explainable AI techniques clarify the model's predictions.
...
TL;DR: 提出了一种新的2D时间序列预测模型，该模型在小数据集上表现出色，并在金融和营销预测方面具有潜力。


<details>
  <summary>Details</summary>
Motivation: 解决小型数据环境中的挑战。

Method: 引入了一种新颖的二维 (2D) 时间序列预测模型，该模型整合了随时间变化的队列行为。
...
Result: findings show that a subgroup of the minority can substantially reduce unfairness with a small impact on the overall prediction error.
...
TL;DR: EvoFormer通过结构感知Transformer模块和进化敏感时间模块来解决动态图嵌入中的结构访问偏差和突变盲点问题，并在多个任务上取得了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有的动态图级嵌入方法面临两个关键但未被充分探索的问题：结构访问偏差，即随机游走抽样不成比例地强调高学位节点，导致冗余和噪声结构表示；以及突变盲点，即由于刚性或过于简单的时间建模策略，未能有效检测到突然的结构变化，导致不一致的时间嵌入。

Method: EvoFormer，一个为动态图级表示学习量身定制的进化感知Transformer框架。为了减轻结构访问偏差，EvoFormer引入了一个结构感知Transformer模块，该模块结合了基于节点结构角色的位置编码，允许模型全局区分和准确地表示节点结构。为了克服突变盲点，EvoFormer采用了一种进化敏感的时间模块，该模块通过一个连续的三步策略显式地对时间演化进行建模：（I）随机游走时间戳分类，生成初始的时间戳感知图级嵌入；（II）图级时间分割，将图流分割成反映结构连贯周期的片段；（III）片段感知时间自注意与边缘进化预测任务相结合，使模型能够精确地捕获片段边界并感知结构进化趋势，从而有效地适应快速的时间变化。

Result: EvoFormer在五个基准数据集上进行了广泛的评估，证实了其在图相似度排序、时间异常检测和时间分割任务中实现了最先进的性能，验证了其在校正结构和时间偏差方面的有效性。

Conclusion: EvoFormer在图相似度排序、时间异常检测和时间分割任务中实现了最先进的性能，验证了其在校正结构和时间偏差方面的有效性。
...
TL;DR: introduce CITE, the first and largest heterogeneous text-attributed citation graph benchmark for catalytic materials, to address the shortage of large-scale benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: the lack of large-scale benchmark datasets has become a critical bottleneck, hindering the development and fair comparison of representation learning methods on heterogeneous text-attributed graphs

Method: introducing CITE - Catalytic Information Textual Entities Graph, the first and largest heterogeneous text-attributed citation graph benchmark for catalytic materials.
...
Conclusion: We provide (i) an overview of the CITE dataset, (ii) standardized evaluation protocols, and (iii) baseline and ablation experiments across diverse modeling paradigms.
...
TL;DR: An evolving fuzzy system within federated learning outperforms established classification methods on UCI datasets, but is computationally intensive.


<details>
  <summary>Details</summary>
Motivation: Adapts dynamically with the addition of new clusters and therefore does not require the number of clusters to be selected apriori. Federated Learning allows models to be trained locally on clients' devices, sharing only the model parameters with a central server instead of the data.

Method: Evolving Fuzzy System within the context of Federated Learning

Result: Our approach outperforms established classification methods on several well-known UCI datasets.

Conclusion: The proposed method demonstrates significant advantages in decentralized data processing.
...
TL;DR: This paper proposes an efficient hybrid least squares/gradient descent method to accelerate DeepONet training.


<details>
  <summary>Details</summary>
Motivation: building the LS system for all possible combinations of branch and trunk inputs yields a prohibitively large linear problem that is infeasible to solve directly.

Method: an efficient hybrid least squares/gradient descent method to accelerate DeepONet training. This method decomposes the large LS system into two smaller, more manageable subproblems one for the branch network and one for the trunk network and solves them separately.
...
TL;DR: This paper presents a hybrid framework for personalized HAR on wearable devices using few-shot learning, achieving accuracy improvements with minimal overhead on an energy-efficient microcontroller.


<details>
  <summary>Details</summary>
Motivation: Generalization of HAR using wearable devices remains limited when models are deployed to new users due to user-induced concept drift (UICD), highlighting the importance of efficient personalization.

Method: A hybrid framework that generalizes across users and then rapidly adapts to individual users using few-shot learning directly on-device, updating only the classifier layer with user-specific data.

Result: Post-deployment adaptation yields consistent accuracy improvements of 3.73%, 17.38%, and 3.70% respectively across three diverse HAR scenarios: RecGym, QVAR-Gesture, and Ultrasound-Gesture.

Conclusion: This paper confirms the feasibility of fast, lightweight, and effective personalization on embedded platforms for scalable and user-aware HAR systems.
...
TL;DR: 提出了一种新的快速重叠检测方法，适用于在线学习中的高斯簇。


<details>
  <summary>Details</summary>
Motivation: 在线学习中，数据流的概念漂移可能导致大量重叠簇，需要合并。常用的分布相异度量方法无法胜任在线学习环境下的重叠簇检测任务，因为它们无法处理所有形状的簇，并且计算成本高昂。

Method: 提出了一种新的相异度量方法，专门用于检测重叠。

Result: 该方法比现有方法快数倍，并且能够检测重叠簇，同时避免合并正交簇。

Conclusion: 提出了一个新的重叠检测方法，该方法能够更快地检测重叠簇，同时避免合并正交簇。
